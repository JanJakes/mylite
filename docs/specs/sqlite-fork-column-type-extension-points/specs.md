...
- add `mylite_sqlite_fork_set_set_column_type()` for attaching copied `SET`
  value-list descriptors to a live SQLite schema
...
- support strict assignment coercion for signed integer ranges, supported
  ...
  `DATETIME(fsp)`, `TIME(fsp)`, `YEAR`, `ENUM(...)`, and `SET(...)`
...
- valid set labels, label lists, numeric masks, quoted integer masks,
  numeric-looking labels, empty-string assignment, empty-string members,
  selected labels, 64th-bit masks, and numeric-context masks pass through the
  native descriptor path
- invalid set labels, malformed lists, trailing numeric text, decimal numeric
  text, and masks with bits outside the descriptor fail through the native
  opcode
