...
static int test_show_errors_limits_errors_and_unsupported_forms(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open errors memory");
    failures += execute_error(
        database,
        "BAD SQL",
        (struct expected_sql_error){
...
static int test_independent_show_errors_handles(void) {
    ...
    failures += expect_int(mylite_open_memory(&first), MYLITE_OK, "open first errors handle");
    failures += expect_int(mylite_open_memory(&second), MYLITE_OK, "open second errors handle");
...
static int expect_show_count_errors(
    ...
    failures +=
        expect_size(mylite_result_column_count(result), show_count_errors_column_count, context);
    failures += expect_text_or_null(
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_show_errors_%s_%d.mylite",
        name,
        current_process_id()
    );
