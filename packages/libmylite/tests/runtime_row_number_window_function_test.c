...
static int test_no_source_and_table_row_number(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen app database");
    if (failures == 0) {
...
static int open_app_database(mylite_db **out_database, char *path, size_t path_size) {
    ...
    if (make_test_path(path, path_size, "row_number") != 0) {
        return 1;
    }
    remove_related_files(path);
    failures += expect_int(mylite_open(path, out_database), MYLITE_OK, "open app database");
    if (failures == 0) {
...
static int expect_row_number_metadata(mylite_db *database) {
    ...
    failures += expect_size(mylite_result_column_count(result), 2U, "row_number metadata columns");
    failures += expect_text(mylite_result_column_name(result, 1U), "rn", "row_number alias");
    failures += expect_int(
        mylite_result_column_type(result, 1U),
        MYLITE_RESULT_COLUMN_TYPE_LONGLONG,
        "row_number type"
    );
    failures += expect_int(mylite_result_column_nullable(result, 1U), 0, "row_number nullable");
    failures += expect_int(
    ...
    failures +=
        expect_int64(mylite_result_affected_rows(result), 0, "row_number metadata affected");
    failures +=
        expect_size(mylite_result_warning_count(result), 0U, "row_number metadata warnings");
    mylite_result_free(result);
    return failures;
}
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-row-number-%s-%d.mylite",
        name,
        current_process_id()
    );
