...
static int test_chained_transaction_completion(void) {
    ...
    failures +=
        expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open transaction database");
    if (failures != 0) {
        return failures;
    }
...
static int test_rename_tables_alias_runtime(void) {
    ...
    failures +=
        expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open rename database");
    if (failures != 0) {
        return failures;
    }
...
static int test_admin_placeholders_runtime(void) {
    ...
    failures += expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open admin database");
    if (failures != 0) {
        return failures;
    }
