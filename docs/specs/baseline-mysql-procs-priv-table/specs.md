# Baseline mysql.procs_priv Table

This slice adds metadata-compatible, read-only placeholder support for
`mysql.procs_priv`, the MySQL grant table that stores routine-level privilege
assignments.

MyLite does not implement persisted routine-level grants in this slice. The
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_mysql_procs_priv_table_expectations.sh`.

The MySQL manual identifies `mysql.procs_priv` as the grant table for routine
...
MyLite supports `mysql.procs_priv` as a read-only empty system-table
placeholder:
...
`mysql.procs_priv` has 8 columns in the target runtime. The first five columns
identify the grant scope, `Grantor` records the account that issued the grant,
`Proc_priv` is a `SET` column, and `Timestamp` is maintained by MySQL.
...
| `Routine_name` | `char(64)` | `NO` | `PRI` | `` | `utf8mb3_general_ci` |
| `Routine_type` | `enum('FUNCTION','PROCEDURE')` | `NO` | `PRI` | `NULL` | `utf8mb3_bin` |
| `Grantor` | `varchar(288)` | `NO` | `MUL` | `` | `utf8mb3_bin` |
| `Proc_priv` | `set('Execute','Alter Routine','Grant')` | `NO` | `` | `` | `utf8mb3_general_ci` |
...
placeholder because routine grant descriptors, `GRANT`, `REVOKE`, grant table
...
| `TABLE_COMMENT` / `Comment` | `Procedure privileges` |
...
cardinality `0`. MySQL also exposes that secondary index through
...
- Direct reads operate on an empty placeholder and do not expose MyLite's
  synthetic privileges as persisted routine-level grant rows.
- No routine grant catalog, role catalog, grant reload, privilege filtering,
  privilege enforcement, account-management DDL, `SHOW GRANTS` expansion, or
  `FLUSH PRIVILEGES` behavior is added.
- No physical `mysql.procs_priv` table, SQLite virtual table, or SQLite fork
  patch is introduced.
...
- Analyzer/runtime: add a static mysql system-table definition and empty-row
  read support for `mysql.procs_priv`.
...
- Run:
  - `sh -n packages/libmylite/tests/mysql_baseline_mysql_procs_priv_table_expectations.sh`
  - `packages/libmylite/tests/mysql_baseline_mysql_procs_priv_table_expectations.sh`
  - `cmake --build --preset dev --target mylite_runtime_mysql_procs_priv_table_test`
  - `ctest --preset dev -R '^libmylite\.runtime\.mysql_procs_priv_table$' --output-on-failure`
  - `git diff --check`
