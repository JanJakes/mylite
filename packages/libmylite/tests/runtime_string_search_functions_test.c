...
static int test_no_source_dual_and_do_string_searches(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "string search do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "string search do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "string search do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "string search do warnings"
        );
    }
...
static int test_no_source_dual_and_do_find_in_set(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            mylite_result_column_count(result),
            0U,
            "find_in_set do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "find_in_set do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "find_in_set do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "find_in_set do warnings"
        );
    }
    mylite_result_free(result);
    failures += expect_query(
...
static int test_table_backed_find_in_set_predicates_and_dml(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "find update columns");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "find update rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            2,
            "find update affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "find delete columns");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "find delete rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            2,
            "find delete affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "find delete warnings"
        );
    }
    mylite_result_free(result);
    failures += expect_query(
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen find_in_set file");
    failures += execute_ok(database, "USE app", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
...
static int test_no_source_dual_and_do_strcmp(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "strcmp do columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "strcmp do rows");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "strcmp do affected");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 0U, "strcmp do warnings");
    }
    mylite_result_free(result);
    failures += expect_query(
        database,
...
static int test_table_backed_strcmp_and_reopen(void) {
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen strcmp file");
    failures += execute_ok(database, "USE app", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
