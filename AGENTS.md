...
MyLite is an embedded MySQL compatibility layer built on a bundled SQLite
...
- Treat the `.mylite` single-file format, SQLite integration, and targeted
  SQLite fork patches carefully.
...
- **SQLite deliberately:** Prefer SQLite extension APIs, use targeted SQLite
  fork patches only for missing extension points or avoidable overhead, and
...
5. **Implement** the feature end-to-end. When syntax is involved, start with
   grammar support, then AST/analyzer/runtime behavior, metadata, docs, and
