...
static int test_index_visibility_rename_and_foreign_key_enforcement(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "rename_fk") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open rename/fk file");
    failures += expect_statement_result(
        database,
        "CREATE DATABASE app",
...
static int test_catalog_v22_index_visibility_migration(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open migration file");
    failures += seed_visibility_schema(database);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen migration file");
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT IS_VISIBLE FROM INFORMATION_SCHEMA.STATISTICS "
