...
| AUTO_INCREMENT columns | 🟡 | top | Allocation, persistence, lock modes, explicit values, and overflow behavior. | Inline `AUTO_INCREMENT` and table-level `AUTO_INCREMENT` execute into metadata for supported `CREATE TABLE` statements. Allocation, persistence across inserts, insert ids, SQL-mode behavior, type/key/default semantic diagnostics, and warning records remain deferred. See [primary keys and AUTO_INCREMENT spec](docs/specs/primary-keys-auto-increment/specs.md) and [CREATE TABLE base execution spec](docs/specs/create-table-base-execution/specs.md). |
| Primary keys | 🟡 | top | Definition forms, implicit NOT NULL behavior, metadata, and errors. | Inline `PRIMARY KEY`, inline `KEY`, and table-level `PRIMARY KEY` constraints now execute into column and statistics metadata, including duplicate-primary diagnostics and implicit metadata `NOT NULL`. Physical uniqueness enforcement, full type/prefix suitability checks, `SHOW CREATE TABLE`, and storage-engine warnings are deferred. See [primary keys and AUTO_INCREMENT spec](docs/specs/primary-keys-auto-increment/specs.md) and [CREATE TABLE base execution spec](docs/specs/create-table-base-execution/specs.md). |
