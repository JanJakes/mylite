...
key/constraint-detail with decoded identifier names, key-part/key-option
handles, and table-option descriptor views. It also builds an initial semantic
`CREATE TABLE` view that anchors the table target and exposes opaque
column/key/key-part/key-option/table-option descriptor handles for the next
typed AST layer. Column descriptors include CST node anchors for the next
