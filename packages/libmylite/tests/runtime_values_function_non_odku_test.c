...
static int test_values_outside_odku_metadata(void) {
    ...
    if (result != NULL) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 3U, "values metadata cols");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 1U, "values metadata rows");
