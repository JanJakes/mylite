...
static void append_information_schema_character_set_row(
    sqlite3_str *sql,
    bool *first,
    const struct mylite_charset *character_set
);

static void append_information_schema_collation_row(
    sqlite3_str *sql,
    bool *first,
...
static void append_information_schema_keyword_row(
    sqlite3_str *sql,
    bool *first,
    const char *word,
    unsigned int flags
...
int mylite_information_schema_collations_sql(mylite_db *database, char **out_sql) {
    ...
    *out_sql = NULL;
    ...
    sqlite3_str_appendall(
        sql,
        ") ORDER BY COLLATION_NAME COLLATE NOCASE, "
