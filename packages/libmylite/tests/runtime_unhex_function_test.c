...
static int test_no_source_dual_and_do_unhex(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "unhex do columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "unhex do rows");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "unhex do affected");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 0U, "unhex do warnings");
    }
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 1U, "invalid do warning");
    }
    mylite_result_free(result);
...
static int test_table_backed_unhex_and_reopen(void) {
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen unhex file");
