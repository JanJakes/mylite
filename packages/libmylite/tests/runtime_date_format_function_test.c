...
static int test_no_source_dual_and_do_date_format(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "date_format do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "date_format do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "date_format do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "date_format do warnings"
        );
    }
...
static int test_date_format_diagnostics(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            mylite_result_column_count(result),
            1U,
            "invalid date_format column count"
        );
        failures += mylite_test_expect_size(
            mylite_result_row_count(result),
            1U,
            "invalid date_format row count"
        );
        failures += expect_result_value(result, 0U, 0U, NULL, "invalid date_format value");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "invalid date_format warnings"
        );
    }
    mylite_result_free(result);
