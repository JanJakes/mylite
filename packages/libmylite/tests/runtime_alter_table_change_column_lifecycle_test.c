...
static int test_change_column_success_persistence_and_dml(void) {
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "update after change");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "delete after change");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
...
static int test_change_column_text_family_replacement(void) {
    ...
    failures += mylite_test_expect_int(
        mylite_open(path, &database),
        MYLITE_OK,
        "reopen text-family change file"
    );
    failures += execute_ok(database, "USE app", &result);
    mylite_result_free(result);
    result = NULL;
...
static int test_change_column_diagnostics_and_rollback(void) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "multi change row count");
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "multi change affected");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
...
static int expect_change_ok(mylite_db *database, const char *sql, int64_t affected_rows) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "change column count");
    failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "change row count");
    ...
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "change warning count");
    mylite_result_free(result);
    failures += expect_query_values(
        database,
        (struct expected_query){
