...
static int test_create_time_composite_string_primary_key(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "create_composite_string_pk") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open create file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen create file");
