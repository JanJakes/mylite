...
| `SHOW COLUMNS` / `SHOW FIELDS` | 🟡 | Limited descriptor-driven column listing for persistent base tables; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE 'pattern'` filters with `Field`, integer, `decimal(M,D)`, `decimal(M,D) unsigned`, `date`, `char(n)`, `varchar(n)`, and baseline `TEXT` family `Type`, `Null`, `Key` including `PRI` for the current single-column primary-key descriptor, SQL `NULL`, canonical integer, canonical decimal, or canonical date-string `Default`, and `Extra`, including `auto_increment` for the current auto-increment descriptor and `INVISIBLE` for descriptor-invisible columns; no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, secondary indexes, string defaults outside canonical `DATE`, expression defaults, generated hidden columns, or `INFORMATION_SCHEMA` |
