...
| Role and privilege state | ❌ | high | CURRENT_ROLE(), grants, roles, DEFINER/INVOKER, and privilege-check-visible metadata. | `CURRENT_USER()` and bare `CURRENT_USER` are implemented only as embedded session identity scalar functions; role state, grant-table authentication, definers, invokers, and privilege filtering remain deferred. See [session identity functions spec](docs/specs/session-identity-functions/specs.md). |
