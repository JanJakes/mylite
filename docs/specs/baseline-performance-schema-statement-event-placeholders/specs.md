# Baseline Performance Schema Statement Event Placeholders
...
returns zero rows from each table until it has live statement instrumentation.
The metadata surface follows the MySQL 8.4.9 target runtime for table names,
columns, key/index metadata, information-schema rows, `SHOW COLUMNS`,
`DESCRIBE`, `SHOW INDEX`, and `SHOW TABLE STATUS`.
...
```sh
packages/libmylite/tests/mysql_baseline_performance_schema_statement_event_placeholders_expectations.sh
```
...
Current/history statement event tables expose a two-column HASH primary key on
`THREAD_ID, EVENT_ID`; `events_statements_history_long` remains the existing
no-index history-long placeholder. Statement histogram and digest summary
tables expose the target-runtime HASH primary or unique indexes. The account,
host, user, thread, and global summary tables expose the same HASH unique or
primary constraints as the corresponding MySQL 8.4.9 Performance Schema tables.

## Intentional Gaps

This slice does not implement live statement instrumentation, digest
aggregation, histogram bucket rows, query sampling, timer values, memory
tracking, or Performance Schema truncation/reset semantics. MySQL has nonzero
rows for many of these tables in the pinned target runtime; MyLite intentionally
returns empty rows until a real instrumentation pipeline exists.
...
- Build and run
  `mylite_runtime_performance_schema_statement_event_placeholders_test`.
- Run the focused CTest filter
  `^libmylite\.runtime\.performance_schema_statement_event_placeholders$`.
