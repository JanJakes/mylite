...
static int test_time_success_metadata_dml_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open time success file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen time success file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_time_independent_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first time file");
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second time file"
    );
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "reopen first time file"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "reopen second time file"
    );
    failures += expect_statement_ok(first, "USE app");
    failures += expect_statement_ok(second, "USE app");
    failures += expect_query_values(
