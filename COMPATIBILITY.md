...
| `ANALYZE TABLE` | ❌ | high | Statistics refresh, histogram update/drop, validation, and result-set metadata. | Parser recognizes table lists and histogram update/drop clauses, including numeric histogram bucket counts and string-literal histogram data, using the shared identifier grammar for tables and columns. |
