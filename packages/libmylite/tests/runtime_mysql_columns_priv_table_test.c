...
static int test_mysql_columns_priv_table(void);
...
static const char *const mysql_columns_priv_columns[mysql_columns_priv_column_count] = {
    "Host",
    "Db",
    "User",
    "Table_name",
...
int main(void) {
    return test_mysql_columns_priv_table() == 0 ? 0 : 1;
}
...
static int test_mysql_columns_priv_table(void) {
    ...
    if (make_test_path(path, sizeof(path), "mysql-columns-priv-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM mysql.columns_priv",
            .column_names = mysql_columns_priv_columns,
            .column_count = mysql_columns_priv_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.columns_priv direct empty read",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.columns_priv",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.columns_priv count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM columns_priv",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "selected mysql.columns_priv count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESC mysql.columns_priv",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = mysql_columns_priv_column_count,
            .context = "mysql.columns_priv describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.columns_priv LIKE 'Host'",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_host_values,
            .row_count = 1U,
            .context = "mysql.columns_priv show columns host",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.columns_priv WHERE Field = 'Timestamp'",
            ...
            .row_count = 1U,
            .context = "mysql.columns_priv show full columns timestamp",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.columns_priv",
            ...
            .context = "mysql.columns_priv show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_seven,
            .row_count = 1U,
            .context = "mysql.columns_priv information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, COLUMN_KEY, EXTRA, "
                   ...
                   "WHERE TABLE_SCHEMA = 'mysql' AND TABLE_NAME = 'columns_priv' "
                   "AND COLUMN_NAME IN ('Host', 'Column_name', 'Timestamp', 'Column_priv') "
            ...
            .context = "mysql.columns_priv information_schema.columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.columns_priv information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.columns_priv information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.columns_priv information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_index_row_count,
            .context = "mysql.columns_priv information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.columns_priv information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'columns_priv'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.columns_priv show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_columns_priv_table_%d_%s.mylite",
        current_process_id(),
        name
    );
