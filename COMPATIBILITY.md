...
| `DO` | ❌ | medium | Expression execution with warning and error semantics. | Parser recognizes comma-separated expression lists and rejects dangling separators plus top-level query-clause tails such as `FROM`/`WHERE`/`ORDER BY`/`LIMIT`. |
