...
static int test_insert_select_duplicate_success_and_persistence(void) {
    ...
    static const char *const values_row_scalar_rows[] = {"1", "10", "21"};
    static const char *const values_direct_row_scalar_rows[] = {"1", "10", "21"};
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            .sql = "SELECT id, v, n FROM t",
            .values = values_row_scalar_rows,
            .column_count = 3U,
            .row_count = 1U,
            .context = "row-scalar VALUES insert-select duplicate rows",
        }
    );
    failures += expect_statement_ok(database, "DELETE FROM t");
    failures += expect_statement_ok(database, "DELETE FROM s");
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            .sql = "SELECT id, v, n FROM t",
            .values = values_direct_row_scalar_rows,
            .column_count = 3U,
            .row_count = 1U,
            .context = "direct row-scalar VALUES insert-select duplicate rows",
        }
    );
    failures += expect_statement_ok(database, "DELETE FROM t");
...
static int test_insert_select_duplicate_source_shapes(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "source_shapes") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open source shape file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int test_insert_select_duplicate_independent_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first independent"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second independent"
    );
