...
static int test_drop_constraint_persistence_and_independent_handles(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "persisted") != 0 ||
        mylite_test_make_path(other_path, sizeof(other_path), "independent") != 0) {
        return 1;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open persisted db");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen persisted db");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
    ...
    failures +=
        mylite_test_expect_int(mylite_open(other_path, &other), MYLITE_OK, "open independent db");
