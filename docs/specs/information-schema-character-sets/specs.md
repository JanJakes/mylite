...
return a clean unsupported diagnostic for projections, filters, ordering,
limits, aliases, joins, aggregates, and qualified wildcards. Runtime execution
must restrict support to unqualified wildcard selection from
`INFORMATION_SCHEMA.CHARACTER_SETS`, with case-insensitive schema and table name
matching after identifier unquoting.
...
- explicit projection, `WHERE`, `ORDER BY`, `LIMIT`, and `COUNT(*)` return
  `MYLITE_UNSUPPORTED`
