...
static int test_zero_temporal_dml_modes(void) {
    ...
    if (make_test_path(path, sizeof(path), "dml_modes") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open DML modes file");
...
static int test_zero_temporal_defaults_predicates_and_persistence(void) {
    ...
    if (make_test_path(path, sizeof(path), "defaults_predicates") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open defaults file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += execute_statement_ok(database, "SET sql_mode = ''");
    failures += expect_statement_ok(
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen defaults file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_zero_temporal_update_replace_copy(void) {
    ...
    if (make_test_path(path, sizeof(path), "update_replace_copy") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_zero_temporal_sql_modes_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
