...
static int test_primary_key_metadata_dml_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen metadata file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_primary_key_type_and_mutation_coverage(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open type coverage file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_primary_key_lifecycle_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
