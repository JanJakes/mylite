...
static int test_integer_signed_attribute_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "signed_attribute") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open signed file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen signed file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_integer_type_alias_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "integer_aliases") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alias file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_statement_ok(
        database,
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen alias file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_integer_display_width_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "display_width") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open display width file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int test_bool_boolean_alias_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "bool_boolean") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open bool alias file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures +=
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen bool alias file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_boolean_literal_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "boolean_literals") != 0 ||
        mylite_test_make_path(second_path, sizeof(second_path), "boolean_literals_second") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_explicit_default_null_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "explicit_default_null") != 0 ||
        mylite_test_make_path(second_path, sizeof(second_path), "explicit_default_null_second") !=
            0) {
        return 1;
    }
    remove_related_files(path);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open default null file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_statement_ok(
        database,
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen default null file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int expect_display_width_warnings(
    ...
    failures += mylite_test_expect_size(mylite_result_row_count(result), expected_count, context);
    failures += mylite_test_expect_size(mylite_result_warning_count(result), 0U, context);
    for (size_t row = 0U; row < expected_count; ++row) {
        failures +=
            mylite_test_expect_text(mylite_result_value_text(result, row, 0U), "Warning", context);
        failures +=
            mylite_test_expect_text(mylite_result_value_text(result, row, 1U), "1681", context);
...
static int expect_column_descriptor(
    ...
    failures += mylite_test_expect_text(column.logical_type, expected.logical_type, context);
    failures += mylite_test_expect_text(column.physical_type, "INTEGER", context);
    failures += mylite_test_expect_true(column.is_nullable == expected.is_nullable, context);
