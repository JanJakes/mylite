...
static int test_information_schema_processlist_reopen_and_independent_handles(void) {
    ...
    mylite_close(first);
    first = NULL;
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first handle");
...
static int expect_processlist_result(
    ...
    failures +=
        expect_size(mylite_result_column_count(result), processlist_column_count, expected.context);
    ...
    if (info == NULL) {
        ...
        failures += expect_size(strlen(info), expected.expected_info_length, expected.context);
    }
...
static int expect_error(mylite_db *database, struct expected_sql_error expected) {
    ...
    failures += expect_text_contains(mylite_errmsg(database), expected.message_part, expected.sql);
    mylite_result_free(result);
    return failures;
}
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_processlist_%s_%d.mylite",
        name,
        current_process_id()
    );

    if (written < 0 || (size_t)written >= path_size) {
        fprintf(stderr, "failed to build processlist test path\n");
        return 1;
    }
    return 0;
}
