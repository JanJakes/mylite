...
static void validate_parenthesized_query_body_from(MyliteParseContext *ctx,
                                                   MyliteToken start);
static void validate_parenthesized_statement_from(
...
static int create_table_partition_key_algorithm_value_token(int token_id,
                                                            MyliteToken token);
static int create_table_partition_option_engine_value_token(int token_id,
                                                            MyliteToken token);
static int create_table_partition_option_name_value_token(int token_id,
                                                          MyliteToken token);
static int create_table_partition_option_number_token(int token_id,
                                                      MyliteToken token);
...
void mylite_parser_validate_select_statement(MyliteParseContext *ctx) {
  MyliteToken start = {0};
  validate_select_statement_from(ctx, 0, start, 0);
  if (!ctx->failed) {
...
void mylite_parser_validate_select_statement_from(MyliteParseContext *ctx,
                                                  MyliteToken start) {
  validate_select_statement_from(ctx, 1, start, 0);
  if (!ctx->failed) {
...
void mylite_parser_validate_table_statement_from(MyliteParseContext *ctx,
                                                 MyliteToken start) {
  validate_table_statement_from(ctx, start, 0, 0);
}
...
void mylite_parser_validate_parenthesized_statement(MyliteParseContext *ctx,
                                                    MyliteToken start) {
  validate_parenthesized_statement_from(ctx, start, 0);
}
...
static void validate_query_set_operand_after_operator_from(
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    validate_query_body_from(ctx, token_id, token, 0);
    return;
  }
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (depth == 1) {
      if (token_id == ML_SELECT) {
        ...
        validate_table_statement_from(ctx, token, 1, 0);
        if (ctx->failed) {
          return;
        }
        query_validated = 1;
...
void mylite_parser_validate_dml_statement(MyliteParseContext *ctx,
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (kind == MYLITE_STATEMENT_REPLACE && token_id == ML_ON &&
        insert_duplicate_clause_follows(ctx, token)) {
      mylite_parser_reject(ctx, token, "malformed REPLACE query tail");
      return;
    }
    ...
    if ((kind == MYLITE_STATEMENT_INSERT ||
        ...
        (token_id == ML_SELECT || token_id == ML_TABLE ||
         token_id == ML_WITH)) {
      if (token_id == ML_SELECT) {
        validate_select_statement_from(ctx, 1, token,
                                       kind == MYLITE_STATEMENT_INSERT);
        if (!ctx->failed) {
          validate_select_list_tails_from(ctx, 1, token,
                                          kind == MYLITE_STATEMENT_INSERT);
        }
      } else if (token_id == ML_TABLE) {
        validate_table_statement_from(ctx, token, 0,
                                      kind == MYLITE_STATEMENT_INSERT);
      } else {
        validate_with_statement_from(ctx, token, "incomplete WITH clause",
                                     kind == MYLITE_STATEMENT_INSERT);
      }
      if (ctx->failed) {
        return;
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (token_id == ML_WITH) {
      validate_with_statement_from(ctx, token,
                                   "incomplete EXPLAIN WITH clause", 0);
      return;
    }
...
void mylite_parser_validate_with_statement_from(MyliteParseContext *ctx,
                                                MyliteToken start) {
  validate_with_statement_from(ctx, start, "incomplete WITH clause", 0);
}
...
static void validate_query_body_from(MyliteParseContext *ctx, int token_id,
                                     ...
                                     int stop_at_insert_duplicate) {
  if (token_id == ML_SELECT) {
    validate_select_statement_from(ctx, 1, token, stop_at_insert_duplicate);
    if (!ctx->failed) {
      validate_select_list_tails_from(ctx, 1, token,
                                      stop_at_insert_duplicate);
    }
  ...
  if (token_id == ML_TABLE) {
    validate_table_statement_from(ctx, token, 0, stop_at_insert_duplicate);
    return;
  }
  ...
  if (token_id == ML_LP) {
    validate_parenthesized_statement_from(ctx, token,
                                          stop_at_insert_duplicate);
    return;
  }
...
static void validate_alter_table_option_values(MyliteParseContext *ctx,
                                               MyliteToken start) {
  enum {
    ALTER_TABLE_OPTION_READY,
    ALTER_TABLE_OPTION_AFTER_DEFAULT,
    ALTER_TABLE_OPTION_AFTER_CHARACTER,
    ALTER_TABLE_OPTION_AFTER_DATA,
  ...
  int in_table_options = 0;
  int state = ALTER_TABLE_OPTION_READY;
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (depth > 0) {
      if (token_opens_nested_expression(token_id)) {
        ...
        depth--;
        if (depth == 0 && state == ALTER_TABLE_OPTION_UNION_BODY) {
    ...
    if (state == ALTER_TABLE_OPTION_EXPECT_VALUE) {
    ...
    if (state == ALTER_TABLE_OPTION_AFTER_DEFAULT) {
      if (token_id == ML_CHARACTER) {
        state = ALTER_TABLE_OPTION_AFTER_CHARACTER;
        pending_token = token;
        previous_comma = 0;
        continue;
      }
    ...
    if (state == ALTER_TABLE_OPTION_AFTER_CHARACTER) {
      if (token_id != ML_SET) {
        mylite_parser_reject(ctx, pending_token,
                             "invalid ALTER TABLE table option");
        return;
    ...
    if (state == ALTER_TABLE_OPTION_AFTER_DATA ||
        state == ALTER_TABLE_OPTION_AFTER_INDEX) {
    ...
    if (state == ALTER_TABLE_OPTION_AFTER_START) {
    ...
    if (state == ALTER_TABLE_OPTION_AFTER_UNION) {
      if (token_id == ML_EQUALS && !value_saw_equals) {
        value_saw_equals = 1;
        continue;
      }
      ...
      state = ALTER_TABLE_OPTION_UNION_BODY;
      depth = 1;
      value_saw_equals = 0;
      previous_comma = 0;
      continue;
    ...
    if (token_id == ML_DEFAULT) {
      state = ALTER_TABLE_OPTION_AFTER_DEFAULT;
      pending_token = token;
      continue;
    }
    ...
    if (token_id == ML_DATA) {
      state = ALTER_TABLE_OPTION_AFTER_DATA;
      pending_token = token;
      continue;
    }
    ...
    if (token_id == ML_START) {
      state = ALTER_TABLE_OPTION_AFTER_START;
      pending_token = token;
      continue;
    }

    if (token_id == ML_UNION) {
      state = ALTER_TABLE_OPTION_AFTER_UNION;
      value_saw_equals = 0;
      pending_token = token;
      continue;
    }
    ...
    if (token_id == ML_AUTOEXTEND_SIZE) {
      state = ALTER_TABLE_OPTION_EXPECT_VALUE;
      value_kind = CREATE_TABLE_OPTION_VALUE_SIZE_NUMBER;
      value_saw_equals = 0;
      pending_token = token;
      continue;
    }

    if (token_id == ML_PACK_KEYS || token_id == ML_STATS_AUTO_RECALC ||
    ...
    if (token_id == ML_COMMENT || token_id == ML_COMPRESSION ||
        ...
        token_id == ML_SECONDARY_ENGINE_ATTRIBUTE) {
      state = ALTER_TABLE_OPTION_EXPECT_VALUE;
      value_kind = token_id == ML_ENCRYPTION
                       ? CREATE_TABLE_OPTION_VALUE_ENCRYPTION
                       : CREATE_TABLE_OPTION_VALUE_STRING;
      value_saw_equals = 0;
      pending_token = token;
    ...
    if (token_id == ML_ENGINE || token_id == ML_SECONDARY_ENGINE ||
    ...
    if (in_table_options) {
      if (!after_comma && token_id == ML_PARTITION) {
        in_table_options = 0;
        option_allowed = 0;
        continue;
      }
      if (!after_comma || !alter_table_action_start_token(token_id)) {
        mylite_parser_reject(ctx, token, "invalid ALTER TABLE table option");
        return;
      }
      in_table_options = 0;
    }
  ...
  if (state != ALTER_TABLE_OPTION_READY) {
    mylite_parser_reject(ctx, pending_token,
                         "incomplete ALTER TABLE table option");
  }
}
...
static void validate_alter_table_expression_tails(MyliteParseContext *ctx,
  ...
  enum {
    ...
    ALTER_EXPR_IN_PARTITION_METHOD,
    ALTER_EXPR_AFTER_PARTITION_METHOD,
    ALTER_EXPR_EXPECT_PARTITION_COUNT,
    ALTER_EXPR_AFTER_SUBPARTITION,
    ALTER_EXPR_AFTER_SUBPARTITION_BY
  };
  enum {
    ALTER_EXPR_PARTITION_METHOD_NONE,
    ALTER_EXPR_PARTITION_METHOD_HASH,
    ALTER_EXPR_PARTITION_METHOD_KEY,
    ALTER_EXPR_PARTITION_METHOD_RANGE,
    ALTER_EXPR_PARTITION_METHOD_LIST
  };
  enum {
    ALTER_EXPR_PARTITION_SCOPE_NONE,
    ALTER_EXPR_PARTITION_SCOPE_OUTER,
    ALTER_EXPR_PARTITION_SCOPE_SUBPARTITION
  };
  enum {
    ALTER_EXPR_PARTITION_KEY_ALGORITHM_NONE,
    ALTER_EXPR_PARTITION_KEY_ALGORITHM_EXPECT_EQUALS,
    ALTER_EXPR_PARTITION_KEY_ALGORITHM_EXPECT_VALUE,
    ALTER_EXPR_PARTITION_KEY_ALGORITHM_DONE
  };
  MyliteLexer lexer;
  MyliteToken token;
  MyliteToken pending_token = start;
  ...
  int skip_depth = 0;
  int partition_method = ALTER_EXPR_PARTITION_METHOD_NONE;
  int partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
  int partition_last_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
  int partition_linear_seen = 0;
  int partition_columns_seen = 0;
  int partition_definitions_seen = 0;
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (token_is_statement_terminator(token_id, token)) {
      if (state == ALTER_EXPR_AFTER_PARTITION_BY ||
          state == ALTER_EXPR_IN_PARTITION_METHOD ||
          state == ALTER_EXPR_EXPECT_PARTITION_COUNT ||
          state == ALTER_EXPR_AFTER_SUBPARTITION ||
          state == ALTER_EXPR_AFTER_SUBPARTITION_BY) {
        mylite_parser_reject(ctx, pending_token,
    ...
    if (state == ALTER_EXPR_AFTER_PARTITION_BY) {
      ...
      if (token_ascii_equal(token, "hash")) {
        partition_method = ALTER_EXPR_PARTITION_METHOD_HASH;
      } else if (token_id == ML_KEY) {
        partition_method = ALTER_EXPR_PARTITION_METHOD_KEY;
      } else if (token_ascii_equal(token, "range")) {
        if (partition_linear_seen) {
          mylite_parser_reject(ctx, token,
                               "invalid ALTER TABLE partition option");
          return;
        }
        partition_method = ALTER_EXPR_PARTITION_METHOD_RANGE;
      } else if (token_ascii_equal(token, "list")) {
        if (partition_linear_seen) {
          mylite_parser_reject(ctx, token,
                               "invalid ALTER TABLE partition option");
          return;
        }
        partition_method = ALTER_EXPR_PARTITION_METHOD_LIST;
      } else {
        state = ALTER_EXPR_READY;
        partition_method = ALTER_EXPR_PARTITION_METHOD_NONE;
        partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
      ...
      state = ALTER_EXPR_IN_PARTITION_METHOD;
      partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_OUTER;
      partition_columns_seen = 0;
      partition_key_algorithm_state =
          ALTER_EXPR_PARTITION_KEY_ALGORITHM_NONE;
      pending_token = token;
    ...
    if (state == ALTER_EXPR_AFTER_SUBPARTITION) {
      if (token_id != ML_BY) {
        mylite_parser_reject(ctx, pending_token,
                             "incomplete ALTER TABLE partition expression");
        return;
      ...
      state = ALTER_EXPR_AFTER_SUBPARTITION_BY;
      partition_method = ALTER_EXPR_PARTITION_METHOD_NONE;
      partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_SUBPARTITION;
      partition_linear_seen = 0;
      partition_columns_seen = 0;
      partition_key_algorithm_state =
          ALTER_EXPR_PARTITION_KEY_ALGORITHM_NONE;
    ...
    if (state == ALTER_EXPR_AFTER_SUBPARTITION_BY) {
      if (token_ascii_equal(token, "linear")) {
        if (partition_linear_seen) {
          mylite_parser_reject(ctx, token,
                               "invalid ALTER TABLE partition option");
    ...
    if (state == ALTER_EXPR_IN_PARTITION_METHOD) {
      ...
      if ((partition_method == ALTER_EXPR_PARTITION_METHOD_RANGE ||
           partition_method == ALTER_EXPR_PARTITION_METHOD_LIST) &&
      ...
      if (token_id == ML_LP) {
        ...
        partition_last_scope = partition_method_scope;
        partition_method = ALTER_EXPR_PARTITION_METHOD_NONE;
        partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
        partition_linear_seen = 0;
        partition_columns_seen = 0;
    ...
    if (state == ALTER_EXPR_AFTER_PARTITION_METHOD) {
      if (token_id == ML_PARTITIONS ||
        ...
        if (partition_definitions_seen ||
            (token_id == ML_PARTITIONS &&
             partition_last_scope != ALTER_EXPR_PARTITION_SCOPE_OUTER) ||
            (token_ascii_equal(token, "subpartitions") &&
             partition_last_scope !=
                 ALTER_EXPR_PARTITION_SCOPE_SUBPARTITION)) {
          mylite_parser_reject(ctx, token,
                               "invalid ALTER TABLE partition option");
          return;
    ...
    if (token_id == ML_PARTITION) {
      ...
      partition_method = ALTER_EXPR_PARTITION_METHOD_NONE;
      partition_method_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
      partition_last_scope = ALTER_EXPR_PARTITION_SCOPE_NONE;
      partition_linear_seen = 0;
      partition_columns_seen = 0;
      partition_definitions_seen = 0;
      partition_key_algorithm_state =
          ALTER_EXPR_PARTITION_KEY_ALGORITHM_NONE;
      continue;
    }
...
static int partition_count_token(int token_id, MyliteToken token) {
  ...
  return token_is_plain_unsigned_integer(token, &value) ||
...
static void validate_create_table_partition_definitions_from(
  ...
  enum {
    CREATE_TABLE_PARTITION_OPTION_NONE,
    CREATE_TABLE_PARTITION_OPTION_AFTER_DATA,
    CREATE_TABLE_PARTITION_OPTION_AFTER_INDEX,
    CREATE_TABLE_PARTITION_OPTION_AFTER_STORAGE,
    CREATE_TABLE_PARTITION_OPTION_EXPECT_ENGINE,
    CREATE_TABLE_PARTITION_OPTION_EXPECT_NAME,
    CREATE_TABLE_PARTITION_OPTION_EXPECT_NUMBER,
    CREATE_TABLE_PARTITION_OPTION_EXPECT_STRING
  };
  MyliteLexer lexer;
  MyliteToken token;
  MyliteToken pending_token = start;
  ...
  int option_state = CREATE_TABLE_PARTITION_OPTION_NONE;
  int option_saw_equals = 0;
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (depth == 1 && token_id == ML_RP) {
      if (values_state != CREATE_TABLE_PARTITION_VALUES_NONE ||
          option_state != CREATE_TABLE_PARTITION_OPTION_NONE ||
    ...
    if (depth == 1 &&
        option_state != CREATE_TABLE_PARTITION_OPTION_NONE) {
      if (option_state == CREATE_TABLE_PARTITION_OPTION_AFTER_DATA ||
          option_state == CREATE_TABLE_PARTITION_OPTION_AFTER_INDEX) {
        if (token_id != ML_DIRECTORY) {
          mylite_parser_reject(ctx, pending_token,
                               "malformed CREATE TABLE partition definitions");
          return;
        }
        option_state = CREATE_TABLE_PARTITION_OPTION_EXPECT_STRING;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_PARTITION_OPTION_AFTER_STORAGE) {
        if (token_id != ML_ENGINE) {
          mylite_parser_reject(ctx, pending_token,
                               "malformed CREATE TABLE partition definitions");
          return;
        }
        option_state = CREATE_TABLE_PARTITION_OPTION_EXPECT_ENGINE;
        option_saw_equals = 0;
        continue;
      }
      if (token_id == ML_EQUALS && !option_saw_equals) {
        option_saw_equals = 1;
        continue;
      }
      if (option_state == CREATE_TABLE_PARTITION_OPTION_EXPECT_ENGINE &&
          create_table_partition_option_engine_value_token(token_id, token)) {
        option_state = CREATE_TABLE_PARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_PARTITION_OPTION_EXPECT_NAME &&
          create_table_partition_option_name_value_token(token_id, token)) {
        option_state = CREATE_TABLE_PARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_PARTITION_OPTION_EXPECT_NUMBER &&
          create_table_partition_option_number_token(token_id, token)) {
        option_state = CREATE_TABLE_PARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_PARTITION_OPTION_EXPECT_STRING &&
          create_table_tail_option_string_token(token_id, token)) {
        option_state = CREATE_TABLE_PARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
    ...
    if (depth == 1 &&
        values_state == CREATE_TABLE_PARTITION_VALUES_NONE) {
      if (token_id == ML_DATA) {
        option_state = CREATE_TABLE_PARTITION_OPTION_AFTER_DATA;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      ...
      if (token_id == ML_STORAGE) {
        option_state = CREATE_TABLE_PARTITION_OPTION_AFTER_STORAGE;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      ...
      if (token_id == ML_TABLESPACE) {
        option_state = CREATE_TABLE_PARTITION_OPTION_EXPECT_NAME;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      if (token_id == ML_MAX_ROWS || token_id == ML_MIN_ROWS ||
          token_id == ML_NODEGROUP) {
        option_state = CREATE_TABLE_PARTITION_OPTION_EXPECT_NUMBER;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
    ...
    if (depth == 1 && token_id == ML_LP) {
      validate_create_table_subpartition_definitions_from(ctx, token);
      if (ctx->failed) {
        return;
      }
    }
...
static void validate_create_table_subpartition_definitions_from(
    MyliteParseContext *ctx, MyliteToken start) {
  enum {
    CREATE_TABLE_SUBPARTITION_DEFS_NEED_SUBPARTITION_OR_END,
    CREATE_TABLE_SUBPARTITION_DEFS_NEED_SUBPARTITION,
    CREATE_TABLE_SUBPARTITION_DEFS_NEED_NAME,
    CREATE_TABLE_SUBPARTITION_DEFS_IN_SUBPARTITION
  };
  enum {
    CREATE_TABLE_SUBPARTITION_OPTION_NONE,
    CREATE_TABLE_SUBPARTITION_OPTION_AFTER_DATA,
    CREATE_TABLE_SUBPARTITION_OPTION_AFTER_INDEX,
    CREATE_TABLE_SUBPARTITION_OPTION_AFTER_STORAGE,
    CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_ENGINE,
    CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NAME,
    CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NUMBER,
    CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_STRING
  };
  MyliteLexer lexer;
  MyliteToken token;
  MyliteToken pending_token = start;
  ...
  int saw_list = 0;
  int depth = 0;
  int list_state = CREATE_TABLE_SUBPARTITION_DEFS_NEED_SUBPARTITION_OR_END;
  int option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
  int option_saw_equals = 0;
  ...
  while ((token_id = mylite_lexer_next(&lexer, &token)) > 0) {
    ...
    if (depth == 1 && token_id == ML_RP) {
      if (option_state != CREATE_TABLE_SUBPARTITION_OPTION_NONE ||
          list_state != CREATE_TABLE_SUBPARTITION_DEFS_IN_SUBPARTITION) {
        mylite_parser_reject(ctx, pending_token,
                             "malformed CREATE TABLE subpartition definitions");
      }
      return;
    }
    ...
    if (depth == 1 && token_id == ML_COMMA) {
      ...
      list_state = CREATE_TABLE_SUBPARTITION_DEFS_NEED_SUBPARTITION;
      option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
      option_saw_equals = 0;
      pending_token = token;
      continue;
    }
    ...
    if (depth == 1 &&
        option_state != CREATE_TABLE_SUBPARTITION_OPTION_NONE) {
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_AFTER_DATA ||
          option_state == CREATE_TABLE_SUBPARTITION_OPTION_AFTER_INDEX) {
        if (token_id != ML_DIRECTORY) {
          mylite_parser_reject(
              ctx, pending_token,
              "malformed CREATE TABLE subpartition definitions");
        ...
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_STRING;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_AFTER_STORAGE) {
        if (token_id != ML_ENGINE) {
          mylite_parser_reject(
              ctx, pending_token,
              "malformed CREATE TABLE subpartition definitions");
        ...
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_ENGINE;
        option_saw_equals = 0;
        continue;
      }
      if (token_id == ML_EQUALS && !option_saw_equals) {
      ...
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_ENGINE &&
          create_table_partition_option_engine_value_token(token_id, token)) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NAME &&
          create_table_partition_option_name_value_token(token_id, token)) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NUMBER &&
          create_table_partition_option_number_token(token_id, token)) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
      }
      if (option_state == CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_STRING &&
          create_table_tail_option_string_token(token_id, token)) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_NONE;
        option_saw_equals = 0;
        continue;
    ...
    if (depth == 1 &&
      ...
      list_state = CREATE_TABLE_SUBPARTITION_DEFS_NEED_NAME;
      pending_token = token;
      continue;
    }
    ...
    if (depth == 1 &&
        list_state == CREATE_TABLE_SUBPARTITION_DEFS_NEED_NAME) {
      if (!create_table_partition_key_name_token(token_id, token)) {
        mylite_parser_reject(ctx, pending_token,
                             "malformed CREATE TABLE subpartition definitions");
        return;
      }
    ...
    if (depth == 1 &&
      ...
      if (token_id == ML_DATA) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_AFTER_DATA;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      ...
      if (token_id == ML_STORAGE) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_AFTER_STORAGE;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      ...
      if (token_id == ML_TABLESPACE) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NAME;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
      if (token_id == ML_MAX_ROWS || token_id == ML_MIN_ROWS ||
          token_id == ML_NODEGROUP) {
        option_state = CREATE_TABLE_SUBPARTITION_OPTION_EXPECT_NUMBER;
        option_saw_equals = 0;
        pending_token = token;
        continue;
      }
...
static int create_table_partition_option_engine_value_token(
    int token_id, MyliteToken token) {
  return create_table_tail_option_value_token(CREATE_TABLE_OPTION_VALUE_NAME,
                                              token_id, token);
}

static int create_table_partition_option_name_value_token(int token_id,
                                                          MyliteToken token) {
  return create_table_partition_key_name_token(token_id, token) ||
         token_id == ML_DOUBLE_QUOTED_STRING;
}
...
static int alter_table_action_start_token(int token_id) {
  return token_id == ML_ADD || token_id == ML_ALGORITHM ||
         token_id == ML_ALTER || token_id == ML_ANALYZE ||
         token_id == ML_CHANGE || token_id == ML_CHECK ||
         token_id == ML_COALESCE || token_id == ML_CONVERT ||
         token_id == ML_DISABLE || token_id == ML_DISCARD ||
         token_id == ML_DROP || token_id == ML_ENABLE ||
         token_id == ML_EXCHANGE || token_id == ML_FORCE ||
         token_id == ML_IMPORT || token_id == ML_LOCK ||
         token_id == ML_MODIFY || token_id == ML_OPTIMIZE ||
         token_id == ML_ORDER || token_id == ML_PARTITION ||
         token_id == ML_REBUILD || token_id == ML_REMOVE ||
         token_id == ML_RENAME || token_id == ML_REORGANIZE ||
         token_id == ML_REPAIR || token_id == ML_SECONDARY_LOAD ||
         token_id == ML_SECONDARY_UNLOAD || token_id == ML_STORAGE ||
         token_id == ML_TABLESPACE || token_id == ML_TRUNCATE ||
         token_id == ML_UNION;
}
