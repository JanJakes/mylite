...
Performance Schema variable-info, persisted-variable, or thread-variable tables remain unsupported.
...
| `character_set_database` | 🟡 | Scalar and `SHOW VARIABLES` readback with no scope, `session`, `local`, or fixed `global`; session/local/unscoped `SET` accepts `DEFAULT`, MySQL charset names and aliases, and integer collation IDs, couples `@@collation_database` to the charset default, emits MySQL-style deprecation warnings, and is reset by `USE` to selected-schema defaults. Global assignment is limited to exact/default fixed no-op forms; no persisted state, conversion, comparison semantics, or protocol charset metadata |
| `character_set_filesystem` | 🟡 | Scalar `SELECT @@character_set_filesystem` and `SHOW VARIABLES` readback with no scope, `session`, `local`, or `global`; session/local/unscoped `SET` accepts `DEFAULT`, MySQL charset names and aliases, and integer collation IDs, then stores handle-local readback; global remains fixed `binary` with exact/default no-op assignment only; no mutable global/persisted state, file-name conversion, server-side file operations, or protocol charset metadata |
...
| `character_set_server` | 🟡 | Scalar and `SHOW VARIABLES` readback with no scope, `session`, `local`, or fixed `global`; session/local/unscoped `SET` accepts `DEFAULT`, MySQL charset names and aliases, and integer collation IDs, then couples `@@collation_server` to the charset default. Global assignment is limited to exact/default fixed no-op forms; no startup options, persisted state, mutable server-global state, conversion, or protocol charset metadata |
...
| `collation_database` | 🟡 | Scalar and `SHOW VARIABLES` readback with no scope, `session`, `local`, or fixed `global`; session/local/unscoped `SET` accepts `DEFAULT`, MySQL collation names and aliases, and integer collation IDs, couples `@@character_set_database` to the collation charset, emits MySQL-style deprecation warnings, and is reset by `USE` to selected-schema defaults. Global assignment is limited to exact/default fixed no-op forms; no persisted state, conversion, string comparison semantics, or protocol collation metadata |
| `collation_server` | 🟡 | Scalar and `SHOW VARIABLES` readback with no scope, `session`, `local`, or fixed `global`; session/local/unscoped `SET` accepts `DEFAULT`, MySQL collation names and aliases, and integer collation IDs, then couples `@@character_set_server` to the collation charset. Global assignment is limited to exact/default fixed no-op forms; no startup options, persisted state, mutable server-global state, string comparison semantics, or protocol collation metadata |
...
| `default_collation_for_utf8mb4` | ✅ | Fixed default/session/global scalar and `SHOW VARIABLES` value `utf8mb4_0900_ai_ci`; session placeholder `SET` readback and fixed global no-op assignments are supported. No mutable collation policy, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `default_password_lifetime` | ✅ | Fixed global readback/SHOW and exact global no-op SET |
| `default_storage_engine` | 🟡 | Fixed scalar/SHOW value `InnoDB` with no scope, `session`, `local`, or `global`, plus exact/default `InnoDB` no-op SET across supported scopes; alternate engine assignments are rejected so metadata cannot diverge from MyLite's embedded storage path. No mutable global/session engine state, alternate engines, startup/persisted values, plugins, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `default_table_encryption` | ✅ | Fixed global/session readback/SHOW and exact/default no-op SET; no table-encryption behavior |
| `default_tmp_storage_engine` | 🟡 | Fixed scalar/SHOW value `InnoDB` with no scope, `session`, `local`, or `global`, plus exact/default `InnoDB` no-op SET across supported scopes; implicit temporary table creation continues to render `ENGINE=InnoDB`, and alternate engine assignments are rejected. No mutable global/session engine state, MEMORY/MyISAM temporary engines, default-engine routing, startup/persisted values, plugins, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `end_markers_in_json` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No JSON output end-marker behavior, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `keep_files_on_create` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No temporary-file retention behavior, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `old_alter_table` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No alternate ALTER TABLE algorithm routing, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `original_commit_timestamp` | ✅ | Session scalar/SHOW default `36028797018963968`, session/local SET/readback, and MySQL-shaped global-scope diagnostics; no replication commit metadata |
| `original_server_version` | ✅ | Session scalar/SHOW default `999999`, session/local SET/readback, and MySQL-shaped global-scope diagnostics; no replication source metadata |
...
| `print_identified_with_as_hex` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No account-authentication formatting behavior, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `require_row_format` | ✅ | Fixed default/session/local scalar `0`, `SHOW VARIABLES` `OFF`, and MySQL-style global-scope diagnostics; session placeholder `SET` readback is supported. No row-format enforcement, global value, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `require_secure_transport` | ✅ | Fixed global scalar `0`, `SHOW VARIABLES` `OFF`, global-only diagnostics, and exact no-op global `SET`; no transport enforcement, TLS state, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `resultset_metadata` | ✅ | Fixed default/session/local scalar and `SHOW VARIABLES` value `FULL`, plus MySQL-style global-scope diagnostics; session placeholder `SET` readback is supported. No protocol metadata negotiation, global value, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `secure_file_priv` | ✅ | Fixed global read-only scalar and `SHOW VARIABLES` value `/var/lib/mysql-files/`; no file import/export enforcement, path probing, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `select_into_disk_sync` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No server-side file output fsync behavior, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `session_track_gtids` | ✅ | Fixed default/session/global scalar and `SHOW VARIABLES` value `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No GTID state tracking, protocol session-state packets, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `session_track_schema` | ✅ | Fixed default/session/global scalar `1` and `SHOW VARIABLES` `ON`; session placeholder `SET` readback and fixed global no-op assignments are supported. No protocol session-state packets, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `session_track_state_change` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No protocol session-state packets, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `session_track_transaction_info` | ✅ | Fixed default/session/global scalar and `SHOW VARIABLES` value `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No transaction session-state packets, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `show_create_table_skip_secondary_engine` | ✅ | Fixed default/session/local scalar `0`, `SHOW VARIABLES` `OFF`, and MySQL-style global-scope diagnostics; session placeholder `SET` readback is supported. No secondary-engine SHOW CREATE rewriting, global value, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
| `show_create_table_verbosity` | ✅ | Fixed default/session/global scalar `0` and `SHOW VARIABLES` `OFF`; session placeholder `SET` readback and fixed global no-op assignments are supported. No extended SHOW CREATE verbosity changes, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `time_zone` | 🟡 | Limited fixed global value `SYSTEM`, session-local scalar reads and `SHOW VARIABLES`, session/local `SET time_zone` forms for `DEFAULT`, `SYSTEM`, `UTC`, and signed UTC offsets, plus exact/default global `SYSTEM` no-op assignment. Current date/time/timestamp materialization uses the session offset; `mysql.time_zone*` tables are metadata-only empty placeholders with no mutable global value, loaded named-zone rows, named-zone conversion beyond `UTC`, daylight-saving behavior, leap-second handling, or TIMESTAMP row storage/retrieval conversion |
...
| `tmpdir` | ✅ | Fixed global read-only scalar and `SHOW VARIABLES` value `/tmp`; no temp-file routing, startup options, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
...
| `use_secondary_engine` | ✅ | Fixed default/session/local scalar and `SHOW VARIABLES` value `ON`, plus MySQL-style global-scope diagnostics; session placeholder `SET` readback is supported. No HeatWave or secondary-engine routing, global value, persisted state, or Performance Schema variable-info, persisted-variable, or thread-variable tables |
