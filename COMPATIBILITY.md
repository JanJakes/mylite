...
| `SHOW ENGINE` | ❌ | low | Generic named-engine diagnostics dispatch, privilege checks, and storage-engine-specific result rows. | Parser records named engines for MySQL 8.4 `STATUS` and `MUTEX` forms. |
| `SHOW ENGINE LOGS` | ❌ | low | Historical storage-engine log diagnostics where applicable, or MySQL 8.4-compatible unsupported-form diagnostics. | MySQL 8.4 documents `SHOW ENGINE engine_name {STATUS | MUTEX}`; parser leaves legacy `LOGS` forms objectless. |
