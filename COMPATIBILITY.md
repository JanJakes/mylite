...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `SELECT` | 🟡 | Descriptor-driven single persistent base-table `SELECT *`, column-list reads with optional source qualifiers, optional source table aliases on supported table-backed forms, explicit `SELECT ALL` on supported non-distinct forms, and limited one-column `SELECT DISTINCT` / `DISTINCTROW column` with optional limited `WHERE`, single-column `ORDER BY`, and `LIMIT`/`OFFSET`, plus limited one-item `COUNT(*)`, `COUNT(column)`, `COUNT(integer/NULL/boolean literal)`, `COUNT(DISTINCT column)`, `MIN(column)`, and `MAX(column)` aggregate forms; `SELECT *` expands visible descriptor columns only while explicit references may name invisible columns; no joins, grouping, select-item aliases, qualified wildcards, general expression projection, full distinct rows, or locking clauses. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| Projection list | 🟡 | Wildcard uses visible descriptor columns in catalog ordinal order; explicit projections and limited one-column `SELECT DISTINCT` / `DISTINCTROW` resolve descriptor column names including invisible columns and supported source-qualified references; limited one-item `COUNT(*)`, `COUNT(column)`, `COUNT(integer/NULL/boolean literal)`, `COUNT(DISTINCT column)`, `MIN(column)`, and `MAX(column)` aggregate forms are supported; duplicate projected columns are allowed for non-distinct selects, with no select-item aliases, qualified wildcards, or full multi-expression distinct rows. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `ORDER BY` | 🟡 | One descriptor column for supported base-table `SELECT`, `DELETE`, and `UPDATE`, with `SELECT` supporting source-qualified references while `DELETE` and `UPDATE` remain unqualified; optional `ASC`/`DESC`, MySQL-compatible `NULL` placement, limited `SELECT DISTINCT column` ordering by that same selected descriptor column only, and no tie-order guarantee without additional keys. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `DISTINCT` / `DISTINCTROW` | 🟡 | Limited one-column `SELECT DISTINCT column FROM table` and synonymous `SELECT DISTINCTROW column FROM table` over one descriptor-backed persistent base table with optional source table alias, source-qualified column references, baseline `WHERE`, optional `ORDER BY` resolving to the same selected column, and existing select `LIMIT`/`OFFSET`; no wildcard, multiple select items, expressions, literals, select-item aliases, joins, grouping, or full distinct-row semantics. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `COUNT()` | 🟡 | Limited `COUNT(*)` and `COUNT(integer/NULL/boolean literal)` in one-item `SELECT` with no source, `FROM DUAL`, or one descriptor-backed persistent base table, plus limited `COUNT(column)` and `COUNT(DISTINCT column)` over one descriptor-backed persistent base table; optional source table alias, source-qualified column arguments, and baseline `WHERE` are supported for table-backed forms; no general `COUNT(expr)`, grouping, select-item aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `COUNT(DISTINCT)` | 🟡 | Limited one-item `COUNT(DISTINCT column)` over one descriptor-backed persistent base table with optional source table alias, source-qualified column argument, and baseline `WHERE`; integer/`NULL` descriptor columns only, no multiple expressions, expression arguments, literals, grouping, select-item aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `GROUP_CONCAT()` | ❌ | Concatenated aggregate string. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `MAX()` | 🟡 | Limited one-item `SELECT MAX(column)` over one descriptor-backed persistent base table with optional source table alias, source-qualified column argument, and baseline `WHERE`; integer/`NULL` descriptor columns only, no expression arguments, `DISTINCT`, grouping, select-item aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| `MIN()` | 🟡 | Limited one-item `SELECT MIN(column)` over one descriptor-backed persistent base table with optional source table alias, source-qualified column argument, and baseline `WHERE`; integer/`NULL` descriptor columns only, no expression arguments, `DISTINCT`, grouping, select-item aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
