...
enum {
    ...
    sql_big_selects_value_column_count = 6,
    sql_big_selects_label_column_count = 5,
    sql_big_selects_diagnostics_column_count = 4,
    sql_big_selects_selected_column_count = 2,
    sql_big_selects_independent_column_count = 3,
...
static int test_sql_big_selects_values_and_persistence(void);
static int test_sql_big_selects_qualifiers_and_errors(void);
static int test_independent_sql_big_selects_handles(void);
...
int main(void) {
    ...
    failures += test_sql_big_selects_values_and_persistence();
    failures += test_sql_big_selects_qualifiers_and_errors();
    failures += test_independent_sql_big_selects_handles();
...
static int test_sql_big_selects_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql big selects file");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "sql big selects values",
        }
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@SQL_BIG_SELECTS, @@Global.Sql_Big_Selects, "
        "@@session.`sql_big_selects`, @@`sql_big_selects`, (@@sql_big_selects)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_big_selects_label_column_count,
            .context = "sql big selects labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_big_selects, @@foreign_key_checks, @@unique_checks, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql big selects scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_big_selects, @@warning_count, @@error_count, ROW_COUNT()",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .count = sql_big_selects_diagnostics_column_count,
            .context = "sql big selects warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql big selects clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_big_selects_diagnostics_column_count,
            .context = "sql big selects error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql big selects clears errors");
    failures += expect_show_count_warnings(database, "0", "sql big selects clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql big selects reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql big selects reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql big selects preamble"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_big_selects, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_big_selects_selected_column_count,
            .context = "sql big selects with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql big selects does not alter descriptor select",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "explicit descriptor select limit still applies with sql big selects",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql big selects does not reject descriptor count",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql big selects does not reject descriptor select",
        }
    );
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql big selects file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_big_selects, @@global.sql_big_selects",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql big selects values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql big selects table rows",
        }
    );
...
static int test_sql_big_selects_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql big selects memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_BIG_SELECTS, @@SESSION.SQL_BIG_SELECTS, "
        "@@Local.Sql_Big_Selects, @@global.`sql_big_selects`, "
        "(@@sql_big_selects)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_big_selects_label_column_count,
            .context = "sql big selects qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        database,
        "SELECT @@no_such_sql_big_selects_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_big_selects_variable'",
        }
    );
    failures += execute_error(
        database,
        "SELECT @@global.no_such_sql_big_selects_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_big_selects_variable'",
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_big_selects_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_big_selects_variable'",
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_big_selects + 1");
...
static int test_independent_sql_big_selects_handles(void) {
    ...
    failures +=
        execute_ok(first, "SELECT @@sql_big_selects, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_big_selects_independent_column_count,
            .context = "first handle sql big selects variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@sql_big_selects, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_big_selects_independent_column_count,
            .context = "second handle sql big selects variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_big_selects_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
