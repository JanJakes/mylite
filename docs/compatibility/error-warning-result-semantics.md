...
| SQL notes variable | 🟡 | Limited scalar `@@sql_notes` reads report fixed enabled value `1`; no mutable note state, note-level diagnostics, note suppression, `max_error_count`, diagnostics stacks, or changed `SHOW WARNINGS` / `@@warning_count` behavior |
