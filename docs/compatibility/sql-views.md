...
| `INFORMATION_SCHEMA.VIEWS` | 🟡 | Queryable synthetic view catalog with MySQL 8.4.9-shaped columns and rows for baseline view descriptors plus the built-in `sys.version`, `sys.schema_auto_increment_columns`, and `sys.schema_object_overview` view-definition rows; definitions, fixed definer/security/check metadata, charset/collation, and deliberately non-updatable status are stored or synthesized by MyLite, but there is no privilege filtering, check-option enforcement, or view execution |
