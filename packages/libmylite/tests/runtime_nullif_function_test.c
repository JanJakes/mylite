...
static int test_nullif_function_unsupported_forms(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id FROM t ORDER BY NULLIF(id,0)",
            .columns = (const char *const[]){"id"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "NULLIF order expression",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT NULLIF(1,2) WHERE TRUE",
            .columns = (const char *const[]){"NULLIF(1,2)"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "nullif with where true",
        }
    );
    failures += expect_query(
        database,
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT NULLIF(1,2) ORDER BY 1",
            .columns = (const char *const[]){"NULLIF(1,2)"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "nullif with order by ordinal",
        }
    );
