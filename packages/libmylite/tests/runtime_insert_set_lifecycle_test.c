...
static int test_insert_set_success_persistence_rename_and_drop(void);
static int test_insert_set_schema_resolution_and_diagnostics(void);
static int test_insert_set_independent_handles(void);
...
static int expect_insert_ok(mylite_db *database, const char *sql);
...
int main(void) {
    ...
    failures += test_insert_set_success_persistence_rename_and_drop();
    failures += test_insert_set_schema_resolution_and_diagnostics();
    failures += test_insert_set_independent_handles();
...
static int test_insert_set_success_persistence_rename_and_drop(void) {
    ...
    static const char *const renamed_rows[] = {"1", "9", "2", "10", "3", "-4", "4", "44"};
    ...
    uint64_t catalog_generation_before_insert = 0U;
    ...
    if (catalog != NULL) {
        catalog_generation_before_insert = catalog->generation;
    }
    if (session != NULL) {
        sqlite_generation_before_insert = session->sqlite_schema_generation;
    }

    failures += expect_insert_ok(
        database,
        "INSERT INTO numbers SET id = 1, i = -2147483648, ii = 2147483647, "
        "iu = 4294967295, integeru = 0, b = -9223372036854775808, "
        "bu = 9223372036854775807, n = NULL, nn = 9"
    );
    failures += expect_insert_ok(database, "INSERT numbers SET id = 2, nn = +10");
    failures += expect_insert_ok(database, "INSERT INTO app.numbers SET id = 3, i = -3, nn = -4");
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .column_count = 1U,
            .row_count = 1U,
            .context = "insert set row count function",
        }
    );
    ...
    if (catalog != NULL) {
        failures += expect_uint64(
            catalog->generation,
            catalog_generation_before_insert,
            "insert set leaves catalog generation"
        );
    }
    if (session != NULL) {
        failures += expect_uint64(
            ...
            "insert set leaves SQLite schema generation"
        );
    }
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .row_count = 3U,
            .context = "insert set values",
        }
    );

    failures += execute_ok(database, "UPDATE numbers SET i = 77 WHERE id = 2", &result);
    failures += expect_int64(mylite_result_affected_rows(result), 1, "update inserted row");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
        (struct expected_query){
            ...
            .column_count = 4U,
            .row_count = 1U,
            .context = "updated insert set row",
        }
    );
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .column_count = 3U,
            .row_count = 1U,
            .context = "persisted insert set row",
        }
    );
    ...
    mylite_result_free(result);
    result = NULL;
    failures += expect_insert_ok(database, "INSERT INTO renamed_numbers SET id = 4, nn = 44");
    failures += expect_query_values(
        database,
        (struct expected_query){
            ...
            .column_count = 2U,
            .row_count = 4U,
            .context = "insert set after rename",
        }
    );
    ...
    failures += execute_error(
        database,
        "INSERT INTO renamed_numbers SET id = 5, nn = 55",
        (struct expected_sql_error){
...
static int test_insert_set_schema_resolution_and_diagnostics(void) {
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, nn = 2",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO missing_schema.numbers SET id = 1, nn = 2",
        (struct expected_sql_error){
    ...
    mylite_result_free(result);
    result = NULL;
    failures += expect_insert_ok(database, "INSERT INTO app.qualified_numbers SET id = 1, nn = 2");
    failures += expect_query_values(
        database,
        (struct expected_query){
            ...
            .column_count = 2U,
            .row_count = 1U,
            .context = "schema-qualified insert set",
        }
    );
    ...
    failures += execute_error(
        database,
        "INSERT INTO missing_table SET id = 1, nn = 2",
        (struct expected_sql_error){
            .code = mysql_error_table_does_not_exist,
            .sqlstate = "42S02",
    ...
    failures += execute_error(
        database,
        "INSERT INTO _mylite_reserved SET id = 1, nn = 2",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET missing = 1, nn = 2",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, id = 2, nn = 3",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, i = 2147483648, nn = 1",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, i = 2147483648",
        (struct expected_sql_error){
            .code = mysql_error_data_out_of_range,
            .sqlstate = "22003",
            .message_part = "Out of range value for column 'i' at row 1",
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, iu = -1, nn = 1",
        (struct expected_sql_error){
            .code = mysql_error_data_out_of_range,
            .sqlstate = "22003",
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = 1, bu = 9223372036854775808, nn = 1",
        (struct expected_sql_error){
            .code = mysql_error_data_out_of_range,
            .sqlstate = "22003",
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET numbers.id = 1, nn = 1",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "INSERT INTO numbers SET id = '1', nn = 1",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
...
static int test_insert_set_independent_handles(void) {
    ...
    failures += expect_insert_ok(first, "INSERT INTO numbers SET id = 1, nn = 11");
    failures += expect_insert_ok(second, "INSERT INTO numbers SET id = 2, nn = 22");
    failures += expect_query_values(
        first,
        (struct expected_query){
            ...
            .column_count = 2U,
            .row_count = 1U,
            .context = "first independent insert set rows",
        }
    );
    failures += expect_query_values(
        second,
        (struct expected_query){
            ...
            .column_count = 2U,
            .row_count = 1U,
            .context = "second independent insert set rows",
        }
    );
...
static int expect_insert_ok(mylite_db *database, const char *sql) {
    ...
    failures += expect_size(mylite_result_column_count(result), 0U, "insert set column count");
    failures += expect_size(mylite_result_row_count(result), 0U, "insert set row count");
    failures += expect_int64(mylite_result_affected_rows(result), 1, "insert set affected rows");
    failures += expect_size(mylite_result_warning_count(result), 0U, "insert set warning count");
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_insert_set_lifecycle_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
