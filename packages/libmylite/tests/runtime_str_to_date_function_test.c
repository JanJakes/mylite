...
static int test_no_source_dual_and_do_str_to_date(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "str_to_date do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "str_to_date do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "str_to_date do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "str_to_date do warnings"
        );
    }
...
static int test_table_backed_str_to_date_and_reopen(void) {
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen str_to_date file");
