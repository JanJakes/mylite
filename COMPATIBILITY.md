...
| `HANDLER` | ❌ | low | HANDLER OPEN, READ, and CLOSE cursor-like table access. | Parser recognizes one- and two-part table names, aliases, key names, MySQL's table-scan and indexed-read direction sets, equality/range tuple reads, `WHERE`, and numeric or identifier `LIMIT` tails, and rejects malformed tuple expressions, malformed `WHERE` adjacent operands, dangling operators, and `LIMIT` suffixes after `READ`. |
...
| `TABLE` | ❌ | medium | Table-value statement syntax and ordering/limit behavior. | Parser recognizes table references, set operators, `ORDER BY`, numeric or identifier `LIMIT` forms, `INTO` variable lists, text-string `INTO OUTFILE`/`INTO DUMPFILE` targets with quoted hex/bit rejection, `OUTFILE` charset names with quoted hex/bit rejection, field and line option tails, and malformed set-operation `SELECT` operands. |
...
| `SHOW BINLOG EVENTS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional text-string log-file names with quoted hex/bit rejection, numeric `FROM` positions, and numeric or identifier `LIMIT` tails. |
...
| `SHOW ERRORS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes numeric or identifier `LIMIT` tails. |
...
| `SHOW PROFILE` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes profile type lists, numeric `FOR QUERY` ids, and numeric or identifier `LIMIT` tails. |
...
| `SHOW WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes numeric or identifier `LIMIT` tails. |
