...
| `SHOW CREATE TABLE on_update_fsp` for `TIMESTAMP(3)` / `DATETIME(6)` columns using `ON UPDATE CURRENT_TIMESTAMP(fsp)` | The `ON UPDATE CURRENT_TIMESTAMP` clause preserves the column fractional precision. |
