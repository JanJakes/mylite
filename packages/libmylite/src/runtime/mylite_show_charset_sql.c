...
static void append_show_character_set_row(
    sqlite3_str *sql,
    bool *first,
    const struct mylite_charset *character_set
);

static void append_show_collation_row(
    sqlite3_str *sql,
    bool *first,
...
int mylite_show_character_set_sql(
    ...
    *out_sql = NULL;
    ...
    if (query->like_pattern != NULL) {
        sqlite3_str_appendf(sql, " WHERE Charset LIKE %Q", query->like_pattern);
        ...
        *out_sql = NULL;
        if (status == MYLITE_UNSUPPORTED) {
            (void)mylite_diagnostics_set_error_message(
                database,
                "SHOW CHARACTER SET WHERE expression is not supported"
            );
        }
        return status;
    }
...
int mylite_show_collation_sql(
    ...
    *out_sql = NULL;
    ...
    if (query->like_pattern != NULL) {
        sqlite3_str_appendf(sql, " WHERE Collation LIKE %Q", query->like_pattern);
        if (query->like_escape_backslash) {
            sqlite3_str_appendall(sql, " ESCAPE '\\'");
        }
    }
    ...
    if (status != MYLITE_OK) {
        ...
        *out_sql = NULL;
        if (status == MYLITE_UNSUPPORTED) {
            (void)mylite_diagnostics_set_error_message(
                database,
                "SHOW COLLATION WHERE expression is not supported"
            );
        }
        return status;
    }
