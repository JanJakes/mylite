...
static int clone_update_plan_nodes(
    mylite_stmt *stmt,
...
static int clone_update_ast_node(
    mylite_stmt *stmt,
...
static int clone_delete_plan_nodes(
    mylite_stmt *stmt,
    const struct mylite_sql_ast_node *statement,
    const char *sql,
    size_t sql_length
...
static int clone_delete_ast_node(
    mylite_stmt *stmt,
    const struct mylite_sql_ast_node *node,
    const char *source_sql,
    size_t sql_length,
...
static int prepare_insert_like_statement(
    mylite_db *database,
...
static int copy_insert_like_statement(
    enum mylite_stmt_kind kind,
    const struct mylite_sql_ast_node *statement,
    mylite_stmt *stmt
);

static int clone_insert_plan_nodes(
    mylite_stmt *stmt,
    const struct mylite_sql_ast_node *statement,
    const char *sql,
    size_t sql_length
...
static int clone_insert_values_nodes(
    mylite_stmt *stmt,
    const struct mylite_sql_ast_node *rows_node,
    const char *sql,
    size_t sql_length
);

static int clone_insert_set_nodes(
    mylite_stmt *stmt,
...
static const struct mylite_sql_ast_node *insert_unwrap_parenthesized_expression(
    const struct mylite_sql_ast_node *node
);

static int clone_insert_ast_node(
    mylite_stmt *stmt,
    const struct mylite_sql_ast_node *node,
    const char *source_sql,
    size_t sql_length,
...
static int copy_insert_like_statement(
    ...
    case MYLITE_STMT_INSERT_VALUES:
        return mylite_dml_copy_insert_values_statement(
            statement,
            &stmt->insert_values,
            &stmt->insert_update
        );
    case MYLITE_STMT_INSERT_SET:
        return mylite_dml_copy_insert_set_statement(
            statement,
            &stmt->insert_values,
            &stmt->insert_set,
            &stmt->insert_update
...
static int clone_insert_values_nodes(
    ...
    for (const struct mylite_sql_ast_node *row_node = rows_node->first_child; row_node != NULL;
        ...
        for (const struct mylite_sql_ast_node *value_node = row_node->first_child;
             value_node != NULL;
             value_node = value_node->next_sibling, ++value_index) {
            int status = MYLITE_OK;
...
static int clone_insert_set_nodes(
    ...
    for (const struct mylite_sql_ast_node *assignment_node = assignments_node->first_child;
        ...
        status = clone_insert_value_node(
            stmt,
            mylite_ast_child_at(assignment_node, 1U),
            sql,
            sql_length,
...
static int clone_update_plan_nodes(
    ...
    return clone_update_ast_node(
        stmt,
        mylite_ast_find_child_kind(statement, MYLITE_SQL_AST_UPDATE_LIMIT_CLAUSE),
        sql,
        sql_length,
        &stmt->update.limit_clause
    );
}
...
static int clone_delete_plan_nodes(
    ...
    if (statement->delete_form != MYLITE_SQL_AST_DELETE_SINGLE_TABLE) {
        status = clone_delete_ast_node(
            stmt,
            mylite_ast_child_at(statement, 1U),
            sql,
            sql_length,
    ...
    status = clone_delete_ast_node(
        ...
        sql,
        sql_length,
        &stmt->delete_plan.order_by_clause
    );
    if (status != MYLITE_OK) {
        return status;
    }
    return clone_delete_ast_node(
        stmt,
        mylite_ast_find_child_kind(statement, MYLITE_SQL_AST_DELETE_LIMIT_CLAUSE),
        sql,
        sql_length,
        &stmt->delete_plan.limit_clause
    );
}
