...
static int test_no_source_dual_and_do_scalar_functions(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "scalar do columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "scalar do rows");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "scalar do affected");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 0U, "scalar do warnings");
    }
...
static int test_table_backed_scalar_functions_and_reopen(void) {
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen scalar table");
    if (failures == 0) {
