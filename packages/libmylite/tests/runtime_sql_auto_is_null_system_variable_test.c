...
enum {
    ...
    sql_auto_is_null_value_column_count = 6,
    sql_auto_is_null_label_column_count = 5,
    sql_auto_is_null_diagnostics_column_count = 4,
    sql_auto_is_null_selected_column_count = 2,
    sql_auto_is_null_independent_column_count = 3,
...
static int test_sql_auto_is_null_values_and_persistence(void);
static int test_sql_auto_is_null_qualifiers_and_errors(void);
static int test_independent_sql_auto_is_null_handles(void);
...
int main(void) {
    ...
    failures += test_sql_auto_is_null_values_and_persistence();
    failures += test_sql_auto_is_null_qualifiers_and_errors();
    failures += test_independent_sql_auto_is_null_handles();
...
static int test_sql_auto_is_null_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql auto is null file");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "sql auto is null values",
        }
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@SQL_AUTO_IS_NULL, @@Global.Sql_Auto_Is_Null, "
        "@@session.`sql_auto_is_null`, @@`sql_auto_is_null`, (@@sql_auto_is_null)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_auto_is_null_label_column_count,
            .context = "sql auto is null labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_auto_is_null, @@foreign_key_checks, @@unique_checks, "
        "@@updatable_views_with_limit, @@sql_buffer_result, @@sql_safe_updates, "
        "@@sql_select_limit, @@sql_notes, @@sql_warnings, @@sql_quote_show_create, "
        "@@autocommit, @@default_storage_engine, @@character_set_server, "
        "@@version_comment",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql auto is null scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_auto_is_null_diagnostics_column_count,
            .context = "sql auto is null warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql auto is null clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_auto_is_null_diagnostics_column_count,
            .context = "sql auto is null error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql auto is null clears errors");
    failures += expect_show_count_warnings(database, "0", "sql auto is null clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql auto is null reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql auto is null reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql auto is null preamble"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_auto_is_null, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_auto_is_null_selected_column_count,
            .context = "sql auto is null with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql auto is null does not alter descriptor select",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "explicit descriptor select limit still applies with sql auto is null",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql auto is null does not alter stored-value null count",
        }
    );
    failures += expect_query_result(
        database,
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql auto is null file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_auto_is_null, @@global.sql_auto_is_null",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql auto is null values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql auto is null table rows",
        }
    );
...
static int test_sql_auto_is_null_qualifiers_and_errors(void) {
    ...
    failures +=
        expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql auto is null memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_AUTO_IS_NULL, @@SESSION.SQL_AUTO_IS_NULL, "
        "@@Local.Sql_Auto_Is_Null, @@global.`sql_auto_is_null`, "
        "(@@sql_auto_is_null)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_auto_is_null_label_column_count,
            .context = "sql auto is null qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_auto_is_null_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_auto_is_null_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_auto_is_null_variable'",
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_auto_is_null + 1");
...
static int test_independent_sql_auto_is_null_handles(void) {
    ...
    failures +=
        execute_ok(first, "SELECT @@sql_auto_is_null, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_auto_is_null_independent_column_count,
            .context = "first handle sql auto is null variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@sql_auto_is_null, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_auto_is_null_independent_column_count,
            .context = "second handle sql auto is null variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_auto_is_null_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
