...
| `VALUES` | ❌ | high | Standalone values statement and row constructor behavior. | Parser recognizes non-empty comma-separated `VALUES ROW(...)` contents with adjacent-operand and dangling-operator row-expression checks, applies the same row-expression checks in CTAS, view, and DML set-operation `VALUES` bodies, preserves `UNION`/`INTERSECT`/`EXCEPT`, validates parenthesized `ORDER BY` expressions plus combined `ORDER BY`/`LIMIT` tails, and rejects malformed set-operation `SELECT` operands. |
