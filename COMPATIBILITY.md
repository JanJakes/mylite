...
| Storage engines | 🟡 | high | Decide how ENGINE clauses, SHOW ENGINES, and engine-specific metadata map to SQLite-backed storage. | First storage-engine registry slice maps MyLite's SQLite-backed table storage to an `InnoDB` compatibility facade, keeps unsupported engine names rejected by `CREATE TABLE`, and exposes common unsupported MySQL engines through `SHOW ENGINES` as `Support=NO`. Full engine-specific table behavior, build/runtime-disabled states, and `INFORMATION_SCHEMA.ENGINES` are deferred. See [SHOW ENGINES spec](docs/specs/show-engines/specs.md) and [CREATE TABLE base execution spec](docs/specs/create-table-base-execution/specs.md). |
| `InnoDB` engine surface | 🟡 | high | Decide how MySQL's default engine metadata, syntax, and diagnostics map onto SQLite-backed storage. | First executable slice exposes `InnoDB` as the default supported storage-engine facade in `SHOW ENGINES`, with transactions and savepoints supported and XA unsupported. It does not claim MySQL InnoDB internals, row locks, foreign-key enforcement, tablespaces, redo/undo logs, or XA. See [SHOW ENGINES spec](docs/specs/show-engines/specs.md). |
