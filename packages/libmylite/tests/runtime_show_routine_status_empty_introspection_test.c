...
enum {
    ...
    show_routine_status_column_count = 12,
    decimal_base = 10,
...
static const char *const show_routine_status_names[show_routine_status_column_count] = {
    "Db",
    "Name",
    "Type",
    "Language",
...
static int test_show_routine_status_result_shape_schema_persistence_and_drop(void);
static int test_show_routine_status_diagnostics_and_unsupported_forms(void);
static int test_independent_show_routine_status_handles(void);
static int create_show_routine_status_schemas(mylite_db *database);
static int expect_show_routine_status_empty_result(
    mylite_db *database,
    struct expected_show_routine_status_empty_result expectation
);
...
int main(void) {
    ...
    failures += test_show_routine_status_result_shape_schema_persistence_and_drop();
    failures += test_show_routine_status_diagnostics_and_unsupported_forms();
    failures += test_independent_show_routine_status_handles();
...
static int test_show_routine_status_result_shape_schema_persistence_and_drop(void) {
    static const char *const forms[] = {
        ...
        "SHOW FUNCTION STATUS LIKE 'routine\\_%'",
    };
    ...
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            ...
            .context = "bare show procedure status without default schema",
        }
    );
    failures += expect_show_routine_status_empty_result(
        database,
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW FUNCTION STATUS LIKE '%'",
            .context = "bare show function status like without default schema",
        }
    );
    failures += create_show_routine_status_schemas(database);
    failures += execute_statement_ok(database, "USE app");
    failures += execute_statement_ok(database, "CREATE TABLE status_rows (id INT NOT NULL)");
    ...
    failures += expect_int64(
        ...
        "catalog generation after show routine status"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation after show routine status"
    );
    failures += expect_row_count(database, -1, "row count after show routine status");
    failures += read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble));
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after show routine status"
    );
    ...
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW PROCEDURE STATUS",
            .context = "reopened procedure status without default schema",
        }
    );
    failures += execute_statement_ok(database, "USE app");
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            ...
            .context = "reopened function status selected schema",
        }
    );
    ...
    failures += expect_show_routine_status_empty_result(
        database,
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW PROCEDURE STATUS LIKE 'missing%'",
            .context = "dropped unrelated schema remains empty routine status",
        }
    );
...
static int test_show_routine_status_diagnostics_and_unsupported_forms(void) {
    char path[test_path_capacity];
    mylite_db *database = NULL;
    mylite_result *zero_initialized_result = NULL;
    int failures = 0;
    ...
    mylite_result_free(zero_initialized_result);
    ...
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW PROCEDURE STATUS",
            .context = "no selected schema procedure status",
        }
    );
    failures += expect_show_routine_status_empty_result(
        database,
    ...
    failures += execute_error(
        database,
        "SHOW FUNCTION STATUS LIKE CONCAT('routine', '%')",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW PROCEDURE STATUS LIKE 'routine\\0%'",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
...
static int test_independent_show_routine_status_handles(void) {
    ...
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW PROCEDURE STATUS",
            .context = "first no selected schema procedure status",
        }
    );
    failures += expect_show_routine_status_empty_result(
        second,
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW FUNCTION STATUS",
            .context = "second no selected schema function status",
        }
    );
    ...
    failures += expect_show_routine_status_empty_result(
        ...
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW FUNCTION STATUS LIKE '%'",
            .context = "first selected schema function status",
        }
    );
    failures += expect_show_routine_status_empty_result(
        second,
        (struct expected_show_routine_status_empty_result){
            .sql = "SHOW PROCEDURE STATUS LIKE '%'",
            .context = "second selected schema procedure status",
        }
    );
...
static int expect_show_routine_status_empty_result(
    ...
    failures += expect_size(
        ...
        show_routine_status_column_count,
        "show routine status column count"
    );
    for (size_t column_index = 0U; column_index < show_routine_status_column_count;
         ++column_index) {
        failures += expect_text_or_null(
            mylite_result_column_name(result, column_index),
            show_routine_status_names[column_index],
            expectation.context
        );
    }
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_show_routine_status_empty_%s_%d.mylite",
        name,
        current_process_id()
    );
