...
static int test_no_source_dual_and_do_interval(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "interval do columns");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "interval do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "interval do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "interval do warnings"
        );
    }
