...
| `INFORMATION_SCHEMA.CHECK_CONSTRAINTS` | 🟡 | Queryable synthetic check-constraint catalog with MySQL 8.4.9-shaped columns, matching system metadata, and descriptor rows for the current limited persistent-table `CHECK` subset; no complete expression coverage, temporary-table checks, `ALTER CHECK`, privileges, or complete MySQL system catalogs |
