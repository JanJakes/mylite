...
static int test_information_schema_view_routine_usage_queries(void);
static int test_information_schema_view_routine_usage_reopen_preamble_and_handles(void);
static int seed_database(mylite_db *database, const char *schema_name, bool include_view);
...
int main(void) {
    ...
    failures += test_information_schema_view_routine_usage_queries();
    failures += test_information_schema_view_routine_usage_reopen_preamble_and_handles();
...
static int test_information_schema_view_routine_usage_queries(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM INFORMATION_SCHEMA.VIEW_ROUTINE_USAGE "
                   "WHERE TABLE_SCHEMA = 'app'",
            .column_names = usage_columns,
            .column_count = sizeof(usage_columns) / sizeof(usage_columns[0]),
            .values = NULL,
            .row_count = 0U,
            .context = "empty view routine usage wildcard",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "empty view routine usage count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "case-insensitive view routine usage table",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT u.TABLE_NAME FROM INFORMATION_SCHEMA.VIEW_ROUTINE_USAGE AS u "
                   "WHERE u.TABLE_SCHEMA = 'app' ORDER BY u.TABLE_NAME LIMIT 1",
            .column_names = table_name_column,
            .column_count = 1U,
            .values = NULL,
            .row_count = 0U,
            .context = "empty view routine usage alias order limit",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "view routine usage system table row",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, "
                   ...
                   "WHERE TABLE_SCHEMA = 'information_schema' "
                   "AND TABLE_NAME = 'VIEW_ROUTINE_USAGE' ORDER BY ORDINAL_POSITION",
            ...
            .context = "view routine usage columns metadata",
        }
    );
    failures += expect_row_count_status(database, "view routine usage row count status");
...
static int test_information_schema_view_routine_usage_reopen_preamble_and_handles(void) {
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "first handle view routine usage count",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "second handle view routine usage count",
        }
    );
    ...
    failures += expect_bytes(
        ...
        "preamble after view routine usage metadata query"
    );
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "reopened view routine usage count",
        }
    );
...
static int seed_database(mylite_db *database, const char *schema_name, bool include_view) {
    ...
    char create_table_sql[seed_sql_capacity];
    char create_view_sql[seed_sql_capacity];
    int create_database_written = 0;
    int create_table_written = 0;
    int create_view_written = 0;
    int failures = 0;
    ...
    create_view_written = snprintf(
        create_view_sql,
        sizeof(create_view_sql),
        "CREATE VIEW %s.v AS SELECT id FROM %s.base",
        schema_name,
        schema_name
    );
    if (create_database_written < 0 ||
        ...
        create_view_written < 0 || (size_t)create_view_written >= sizeof(create_view_sql)) {
    ...
    if (include_view) {
        failures += expect_statement_ok(database, create_view_sql);
    }
