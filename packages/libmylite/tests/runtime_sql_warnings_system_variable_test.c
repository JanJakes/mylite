...
enum {
    ...
    sql_warnings_value_column_count = 6,
    sql_warnings_label_column_count = 5,
    sql_warnings_diagnostics_column_count = 4,
    sql_warnings_selected_column_count = 2,
    sql_warnings_independent_column_count = 3,
...
static int test_sql_warnings_values_and_persistence(void);
static int test_sql_warnings_qualifiers_and_errors(void);
static int test_independent_sql_warnings_handles(void);
...
int main(void) {
    ...
    failures += test_sql_warnings_values_and_persistence();
    failures += test_sql_warnings_qualifiers_and_errors();
    failures += test_independent_sql_warnings_handles();
...
static int test_sql_warnings_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql warnings file");
    ...
    failures += execute_ok(
        ...
        "@@session.sql_warnings, @@local.sql_warnings, @@warning_count, "
        "ROW_COUNT() FROM DUAL",
        &result
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@SQL_WARNINGS, @@Global.Sql_Warnings, "
        "@@session.`sql_warnings`, @@`sql_warnings`, (@@sql_warnings)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_warnings_label_column_count,
            .context = "sql warnings labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql warnings scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_warnings_diagnostics_column_count,
            .context = "sql warnings warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql warnings clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_warnings_diagnostics_column_count,
            .context = "sql warnings error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql warnings clears errors");
    failures += expect_show_count_warnings(database, "0", "sql warnings clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql warnings reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql warnings reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql warnings preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after sql warnings reads"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_warnings, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_warnings_selected_column_count,
            .context = "sql warnings with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql warnings table DDL independence",
        }
    );
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql warnings file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_warnings, @@global.sql_warnings",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql warnings values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql warnings table row",
        }
    );
...
static int test_sql_warnings_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql warnings memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_WARNINGS, @@SESSION.SQL_WARNINGS, "
        "@@Local.Sql_Warnings, @@global.`sql_warnings`, "
        "(@@sql_warnings)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_warnings_label_column_count,
            .context = "sql warnings qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_warnings_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_warnings_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_warnings_variable'",
...
static int test_independent_sql_warnings_handles(void) {
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_warnings_independent_column_count,
            .context = "first handle sql warnings variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@sql_warnings, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_warnings_independent_column_count,
            .context = "second handle sql warnings variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_warnings_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
