...
static int test_values_show_and_scope(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB IO/log db");
    for (size_t index = 0U;
...
static int test_set_diagnostics(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB IO/log SET db");
    for (size_t index = 0U;
         index < sizeof(innodb_io_log_variables) / sizeof(innodb_io_log_variables[0]);
         ++index) {
        const struct innodb_io_log_variable *variable = &innodb_io_log_variables[index];
...
static int test_session_mutation(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB lock timeout db");
...
static int test_user_variable_assignments(void) {
    ...
    failures +=
        expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB IO/log user-var db");
    for (size_t index = 0U;
         index < sizeof(innodb_io_log_variables) / sizeof(innodb_io_log_variables[0]);
         ++index) {
        const struct innodb_io_log_variable *variable = &innodb_io_log_variables[index];
