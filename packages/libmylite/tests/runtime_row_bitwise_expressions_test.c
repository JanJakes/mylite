...
static int test_row_bitwise_projection_and_file_safety(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "projection") != 0) {
        return 1;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open projection file");
...
static int test_row_bitwise_predicates_and_ordering(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "predicates_ordering") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open predicates file");
    failures += prepare_bits_table(database);
    failures += expect_query(
        database,
        (struct expected_query){
