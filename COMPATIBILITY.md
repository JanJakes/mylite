...
| `ORDER BY` | ❌ | top | Expression, alias, ordinal, collation, ASC/DESC, and filesort metadata behavior. | Design specified for the single-table `SELECT` subset, including alias-before-column resolution, positive ordinals, qualified columns, Task 16 hidden sort expressions, `ASC`/`DESC`, `NULL` ordering, collation risks, warning behavior, and metadata preservation. Not implemented. See [ORDER BY, LIMIT, and OFFSET spec](docs/specs/order-limit-offset/specs.md). |
| `LIMIT` / `OFFSET` | ❌ | top | Row limiting, prepared markers, integer conversion, and error cases. | Design specified for `LIMIT row_count`, `LIMIT offset,row_count`, and `LIMIT row_count OFFSET offset`, including unsigned literal validation, MySQL syntax errors for invalid literals, and future prepared-marker behavior. Not implemented. See [ORDER BY, LIMIT, and OFFSET spec](docs/specs/order-limit-offset/specs.md). |
...
| Result ordering guarantees | ❌ | top | Match MySQL ordering only where SQL semantics or MySQL behavior require it. | Task 18 design documents MySQL-visible `ORDER BY` guarantees, nondeterministic equal-key ordering, and `LIMIT` interaction risks for the single-table `SELECT` subset. Not implemented. See [ORDER BY, LIMIT, and OFFSET spec](docs/specs/order-limit-offset/specs.md). |
