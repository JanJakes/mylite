...
- typed `SET` descriptors with statement form, assignment kind, variable scope,
  ...
  extended value spans for `SET NAMES ... COLLATE ...`
...
value span for `SET NAMES ... COLLATE ...`. `USE` records the decoded default
database target. These views intentionally keep assignment values as CST anchors
until the shared expression AST exists.
...
```text
mode=syntax queries=69541 iterations=100 parsed=6954100 failed=0 elapsed=14.077476 qps=493988 mbps=37.57 avg_us=2.024
...
parser support object: 207K on disk, 123,475 bytes text/data/other
