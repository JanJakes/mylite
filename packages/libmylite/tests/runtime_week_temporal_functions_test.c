...
static int test_no_source_dual_and_do_week_temporal_functions(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "week do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "week do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "week do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "week do warnings");
    }
...
static int test_table_backed_week_temporal_functions_and_reopen(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen week database");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-week-temporal-functions-%s-%d.mylite",
        name,
        current_process_id()
    );
