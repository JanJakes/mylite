...
static int test_admin_noop_warning_surface(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open admin no-op memory");
...
static int test_admin_noop_preserves_user_transaction(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "admin_noop_transaction") != 0) {
        return 1;
    }
...
static int test_existing_limited_call_still_runs(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "limited_procedure") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open limited procedure");
    failures += execute_statement_ok(database, "CREATE DATABASE app");
    failures += execute_statement_ok(database, "USE app");
