...
| `WHERE` | 🟡 | Descriptor-backed boolean expressions over supported integer/`NULL` predicate atoms for filtered table `SELECT`, aggregate source filters, `DELETE`, and `UPDATE`, with `AND`/`&&`, `OR`/`\|\|`, and parentheses following MySQL precedence; `SELECT` supports source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands; no `WHERE TRUE`, `XOR`, `NOT`, literal-left comparisons, `IS TRUE`, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
