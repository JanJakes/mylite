# Baseline sys.memory_global_total Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.memory_global_total` and `sys.x$memory_global_total` views. MySQL uses
these sys views to summarize total current Performance Schema memory allocation
globally. MyLite does not collect Performance Schema memory-summary rows, so
both views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.memory_global_total` and
  `sys.x$memory_global_total`:
  <https://dev.mysql.com/doc/refman/8.4/en/sys-memory-global-total.html>
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_memory_global_total_views_expectations.sh`.
...
read status behavior. Direct MySQL values are environment-dependent because
they reflect live Performance Schema memory instrumentation.
...
SELECT * FROM `x$memory_global_total`;
```

Both views return zero rows. This differs from a live MySQL server, which
normally returns one aggregate row, but preserves the queryable sys view
surface without inventing memory allocation totals.

## Column Metadata

`sys.memory_global_total` has one column:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `total_allocated` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |

`sys.x$memory_global_total` has the same logical column except that the total
exposes raw numeric metadata:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `total_allocated` | `decimal(41,0)` | `YES` | `NULL` | SQL `NULL` |
...
uses `format_bytes(sum(...CURRENT_NUMBER_OF_BYTES_USED...))`; the raw `x$` view
exposes the same `sum(...)` expression as `decimal(41,0)`.
...
- live `performance_schema.memory_summary_global_by_event_name` rows;
- memory instrumentation, current allocation counters, or aggregate totals;
- formatted byte calculation through `format_bytes()`;
- execution of the aggregate expression through the built-in view definition;
