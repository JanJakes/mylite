...
| `INFORMATION_SCHEMA.VIEW_ROUTINE_USAGE` | 🟡 | Queryable synthetic view-to-stored-function dependency catalog with MySQL 8.4.9-shaped columns and built-in dependency rows for `sys.innodb_lock_waits`, `sys.x$innodb_lock_waits`, `sys.latest_file_io`, `sys.schema_table_lock_waits`, and `sys.x$schema_table_lock_waits`; no stored routine descriptors, broader view-to-routine dependency analysis, sys helper-function execution, or native/loadable function rows |
