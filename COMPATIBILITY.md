...
| `DROP INDEX` | 🟡 | top | Standalone index removal semantics. | Implemented for `DROP INDEX name ON table` and `ALTER TABLE ... DROP INDEX` / `DROP KEY` against supported base tables, including schema/table/index validation, metadata cleanup from the internal index catalog and `INFORMATION_SCHEMA.STATISTICS`, and removal from later unique-conflict checks. Primary-key dependency checks, foreign-key dependency checks, implicit commits, and physical-index cleanup remain deferred. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md) and [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
...
| `ADD PRIMARY KEY` | 🟡 | top | Primary key addition and data validation. | Implemented for supported MyLite base tables with existing-row duplicate/`NULL` validation, implicit `NOT NULL` column metadata rewrite, statistics metadata, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` primary-key rows, write-path duplicate enforcement, mixed column/key atomicity, and rollback. `KEY_COLUMN_USAGE`, generated-column dependencies, foreign-key dependencies, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md) and [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md). |
| `DROP PRIMARY KEY` | 🟡 | top | Primary key removal and auto-increment restrictions. | Implemented for metadata-backed primary-key removal while preserving `NOT NULL` column state, rejecting `AUTO_INCREMENT` dependency, and removing the `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` primary-key row. Future foreign-key dependency checks, `KEY_COLUMN_USAGE`, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md) and [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md). |
| `ADD UNIQUE` | 🟡 | top | Unique index addition and duplicate validation. | Implemented for supported MyLite base tables using standalone index semantics: duplicate validation, nullable unique-key behavior, generated/named index behavior, visibility, prefix parts, statistics metadata, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` unique rows, DML conflict integration, and mixed column/key default-backfill validation. Functional/multi-valued key parts, `KEY_COLUMN_USAGE`, optimizer use, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md) and [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md). |
...
| `DROP INDEX` / `DROP KEY` | 🟡 | top | Index removal and constraint dependencies. | Standalone `DROP INDEX name ON table` and `ALTER TABLE DROP INDEX` / `DROP KEY` are implemented for metadata-backed indexes created by supported table/index DDL, including conflict-surface removal and statistics cleanup. Future foreign-key dependency checks and implicit commits remain deferred. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md) and [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `RENAME INDEX` / `RENAME KEY` | 🟡 | high | Index rename semantics. | Implemented as metadata-only rename of non-primary indexes while preserving key parts, visibility, uniqueness, conflict order, statistics rows, and `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` names for unique indexes. Exact primary-index rename diagnostics, `KEY_COLUMN_USAGE`, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md) and [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md). |
