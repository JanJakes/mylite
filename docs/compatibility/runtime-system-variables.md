...
| `sql_notes` | 🟡 | Limited scalar `SELECT @@sql_notes` with no scope, `session`, `local`, or `global`; returns MyLite's fixed enabled value `1`; limited fixed no-op `SET` forms may preserve `1`; note-level diagnostics are currently produced by limited table/schema-existence DDL no-ops and `VARCHAR` trailing-space truncation in admitted DML; no mutable global/session state, note suppression, `max_error_count`, changed `SHOW WARNINGS` / `@@warning_count` behavior beyond the documented missing-schema drop exception, or Performance Schema variable tables |
