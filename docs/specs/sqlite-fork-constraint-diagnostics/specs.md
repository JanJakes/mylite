...
Implemented for the first native constraint diagnostics slices. The current
scope maps SQLite-native `NOT NULL`, `UNIQUE`, `PRIMARY KEY`, `CHECK`, and
immediate foreign-key constraint failures into MySQL condition metadata exposed
through the existing fork diagnostics bridge.
...
- child-side foreign-key insert/update violations report MySQL error `1452`,
  SQLSTATE `23000`;
- parent-side foreign-key delete/update violations report MySQL error `1451`,
  SQLSTATE `23000`;
...
- native immediate foreign-key counters publish the same condition metadata
  when the statement-level FK check fails;
...
- deferred foreign-key diagnostics and transaction-commit-time condition
  ordering;
...
The right fork point for simple constraints is the VDBE constraint halt path.
SQLite already tags those halt opcodes with the constraint family, so MyLite
can attach MySQL condition metadata there without changing storage, query
planning, or normal SQLite error text yet.

Foreign keys need an additional fork point. SQLite's public error result and
plain `P5_ConstraintFK` tag do not distinguish a missing parent row from a
referenced parent-row mutation, while MySQL exposes different errors for those
cases. MyLite tags native FK counter increments with child-side or parent-side
context and remembers the relevant tag on the VDBE until the immediate FK check
turns the counter into a statement error.
...
- child-side foreign-key insert/update violations fail with
  `ERROR 1452 (23000)`;
- parent-side foreign-key delete/update violations fail with
  `ERROR 1451 (23000)`;
...
- direct child-side FK halts map to MySQL's child-row condition;
- FK counter increments generated by child-row checks carry child-side context;
- FK counter increments generated by parent-row checks carry parent-side
  context;
- statement-level immediate FK failure uses the stored context to publish the
  MySQL condition before returning SQLite's native FK error.
...
- child-side native FK insert/update failures set error, MySQL errno `1452`,
  and SQLSTATE `23000`;
- parent-side native FK delete/update failures set error, MySQL errno `1451`,
  and SQLSTATE `23000`;
...
`UNIQUE`, `PRIMARY KEY`, `CHECK`, and immediate foreign-key constraint
failures. Exact MySQL message rendering, warning demotion, deferred
foreign-key, generated-column, and broader constraint/affected-row mapping
remain deferred.
