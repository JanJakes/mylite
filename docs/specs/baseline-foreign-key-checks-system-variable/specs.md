# Baseline Foreign Key Checks System Variable
...
`SELECT` execution, diagnostics lifecycle, and MyLite's current no-foreign-key
descriptor model. MySQL exposes `foreign_key_checks` as mutable global and
session state that controls foreign-key enforcement and selected DDL behavior.
MyLite does not implement foreign key definitions or mutable system-variable
assignment in the baseline yet, so this slice exposes only the default enabled
scalar value.

This is not foreign key support. It does not implement `SET
foreign_key_checks`, foreign key DDL, referential integrity checks, cascades,
metadata, dependency checks, or DDL behavior changes.
...
`packages/libmylite/tests/mysql_baseline_foreign_key_checks_system_variable_expectations.sh`
records the runtime probes for this feature. Observed behavior:

- `SELECT @@foreign_key_checks`, `@@global.foreign_key_checks`,
  `@@session.foreign_key_checks`, `@@local.foreign_key_checks`, and
  `@@FOREIGN_KEY_CHECKS` return `1` in the tested default runtime.
- The variable has global and session scope. After
  `SET SESSION foreign_key_checks=0`, unscoped, `session`, and `local` reads
  return `0`, while `global` still returns `1`; resetting the session value to
...
- Backtick-quoted scope names, such as ``@@`session`.foreign_key_checks``, are
  syntax errors.
...
- MySQL accepts wider expression forms such as
  `SELECT @@foreign_key_checks + 1`. Those forms remain outside this MyLite
  slice.

The official MySQL system-variable documentation classifies
`foreign_key_checks` as a dynamic boolean variable with global and session
scope and default value `ON`. The foreign-key documentation states that
foreign-key checking is enabled by default and controlled by this variable.

## Scope

The implementation must add:

- runtime recognition of `foreign_key_checks` inside the existing scalar
  `SELECT` subset;
...
SELECT @@session.foreign_key_checks, @@local.foreign_key_checks
SELECT @@global.foreign_key_checks
SELECT @@session.`foreign_key_checks`, @@`foreign_key_checks`
SELECT @@foreign_key_checks, @@warning_count, ROW_COUNT()
```
...
- `SET`, startup options, persisted variables, `SET_VAR` hints, or mutable
  global/session `foreign_key_checks` state;
- variables other than `foreign_key_checks`;
- foreign key syntax in `CREATE TABLE` or `ALTER TABLE`;
- constraint descriptors, enforcement, cascades, referential actions,
  dependency checks, index requirements, metadata, `SHOW CREATE` rendering, or
  `INFORMATION_SCHEMA` rows;
- `DROP TABLE` or `DROP DATABASE` behavior changes based on foreign key
  dependencies;
...
- The catalog remains authoritative for descriptors. This variable slice does
  not create foreign-key descriptors or affect table lifecycle behavior.
...
MyLite limitation: no foreign key descriptors or mutable system-variable state
exist yet.
...
accepted or rejected DDL because foreign-key syntax and descriptors remain out
of scope.
...
Supported reads of `@@foreign_key_checks` do not emit warnings. This slice
does not implement MySQL's mutable `SET SESSION foreign_key_checks=...`
surface, so assignment diagnostics and DDL side effects are out of scope.
...
- mixed scalar reads with existing diagnostics, charset, engine, autocommit,
  quote-control, and version variables;
...
Do not overclaim mutable system variables, `SET`, `SHOW VARIABLES`, foreign
key DDL, enforcement, metadata, cascades, dependency checks, or
`INFORMATION_SCHEMA`.
