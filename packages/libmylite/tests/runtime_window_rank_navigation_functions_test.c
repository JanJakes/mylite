...
static int test_window_function_metadata(void) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 4U, "metadata column count");
    ...
    failures +=
        mylite_test_expect_int(mylite_result_column_nullable(result, 0U), 0, "rank nullable");
    failures += mylite_test_expect_int(
    ...
    failures += mylite_test_expect_int(
        mylite_result_column_nullable(result, 1U),
        0,
        "percent_rank nullable"
    );
    failures += mylite_test_expect_int(
    ...
    failures +=
        mylite_test_expect_int(mylite_result_column_nullable(result, 3U), 1, "lag nullable");
    failures += mylite_test_expect_int(
...
static int open_app_database(mylite_db **out_database, char *path, size_t path_size) {
    ...
    if (mylite_test_make_path(path, path_size, "window-rank-navigation") != 0) {
        return 1;
    }
    remove_related_files(path);
