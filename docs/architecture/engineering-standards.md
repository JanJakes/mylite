...
- SQLite integration should prefer public SQLite extension APIs.
- SQLite fork patches are acceptable only when MySQL compatibility or
  performance needs an extension point SQLite does not expose cleanly.
- SQLite fork patches should expose narrow hooks for MyLite rather than moving
  substantial MyLite compatibility behavior into SQLite itself.
- SQLite local changes must stay small, documented, and easy to rebase onto a
