...
static bool
predicate_bind_callbacks_are_valid(const struct mylite_select_predicate_bind_callbacks *callbacks);
static int bind_predicate_expression_in_clause(
    ...
    const struct mylite_select_predicate_bind_callbacks *callbacks)
{
    ...
    for (size_t index = 0U; index < plan->join_predicate_count; ++index) {
        ...
        int status = bind_predicate_expression_in_clause(database, predicate->expression, plan,
                                                         "on clause", predicate->first_table,
                                                         predicate->table_count, callbacks);
    ...
    const struct mylite_select_predicate_bind_callbacks *callbacks)
{
    ...
    return bind_predicate_expression_in_clause(database, expression, plan, "where clause", 0U,
                                               mylite_select_plan_table_count(plan), callbacks);
}
    ...
    size_t table_count, const struct mylite_select_predicate_bind_callbacks *callbacks)
{
    ...
    case MYLITE_SQL_AST_CASE_WHEN:
        for (const struct mylite_sql_ast_node *child = expression->first_child; child != NULL;
            ...
            int status = bind_predicate_expression_in_clause(database, child, plan, clause_context,
    ...
    case MYLITE_SQL_AST_CAST_EXPRESSION: {
        ...
        return bind_predicate_expression_in_clause(database, mylite_ast_child_at(expression, 0U),
                                                   plan, clause_context, first_table, table_count,
                                                   callbacks);
    }
    ...
    size_t table_count, const struct mylite_select_predicate_bind_callbacks *callbacks)
{
    ...
    status = bind_predicate_expression_in_clause(database, left, plan, clause_context, first_table,
                                                 table_count, callbacks);
    if (status != MYLITE_OK) {
        return status;
    }
