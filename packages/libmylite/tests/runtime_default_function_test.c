...
static int test_default_function_select_dml_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen default file");
...
static int expect_statement_ok(
    ...
    if (failures == 0) {
        ...
        failures +=
            expect_int64(mylite_result_affected_rows(result), expected.affected_rows, context);
        failures +=
            expect_size(mylite_result_warning_count(result), expected.warning_count, context);
    }
    mylite_result_free(result);
    return failures;
}
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "runtime_default_function_%s_%d.mylite",
        name,
        current_process_id()
    );
