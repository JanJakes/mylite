...
| `SHOW PROCEDURE STATUS` | 🟡 | Limited empty routine introspection with MySQL 8.4.9 column labels and `LIKE 'pattern'` filters; global and default-schema independent; related empty `INFORMATION_SCHEMA.ROUTINES` metadata is queryable, but there are no NUL-producing pattern escapes, persistent routine descriptors, routine rows, session-local procedure bridge rows, `WHERE`, or privileges |
