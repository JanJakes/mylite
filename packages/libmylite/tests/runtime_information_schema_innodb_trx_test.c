...
static int test_information_schema_innodb_trx_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb trx db");
...
static int test_information_schema_innodb_trx_open_transaction(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb trx tx db");
