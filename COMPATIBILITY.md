...
| `REPLACE ... VALUES` | ❌ | top | Delete-then-insert semantics, cascades, triggers, affected rows, and auto-increment behavior. | Parser recognizes empty and comma-separated column lists before write payloads and validates explicit `VALUE(S)` row-list tails, empty rows, and `ROW(...)` constructors. |
