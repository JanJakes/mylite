...
static int test_alter_add_primary_key_integer_types_and_defaults(void) {
    ...
    if (make_test_path(path, sizeof(path), "types_defaults") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open type/default file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int test_alter_add_primary_key_diagnostics(void) {
    ...
    failures += execute_error(
        database,
        "ALTER TABLE qualified_pk ADD PRIMARY KEY (id), ADD KEY k_id (id)",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "ALTER TABLE existing_pk DROP PRIMARY KEY",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
...
static int expect_alter_primary_key_ok(mylite_db *database, const char *sql) {
    ...
    failures += expect_size(mylite_result_column_count(result), 0U, "ADD PRIMARY KEY column count");
    failures += expect_size(mylite_result_row_count(result), 0U, "ADD PRIMARY KEY row count");
    failures +=
        expect_int64(mylite_result_affected_rows(result), 0, "ADD PRIMARY KEY affected rows");
    failures +=
        expect_size(mylite_result_warning_count(result), 0U, "ADD PRIMARY KEY warning count");
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_alter_add_primary_key_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
