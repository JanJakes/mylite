...
| `SHOW COUNT(*) ERRORS` | 🟡 | Limited previous-statement error-condition count with MySQL 8.4.9 column label `@@session.error_count`; counts MyLite's previous error condition only; no `@@error_count`, notes, `max_error_count`, `GET DIAGNOSTICS`, privileges, or full diagnostics-area behavior |
| `SHOW COUNT(*) WARNINGS` | 🟡 | Limited previous-statement diagnostics count with MySQL 8.4.9 column label; counts MyLite's previous error condition plus warning records; no notes, `max_error_count`, `sql_notes`, `GET DIAGNOSTICS`, privileges, or full diagnostics-area behavior |
...
| `SHOW ERRORS` | 🟡 | Limited previous-statement error-condition rows with `Level`, `Code`, and `Message`, plus unsigned decimal `LIMIT` slicing; reports MyLite's previous error condition only; no warning/note rows, `WHERE`, `LIKE`, expression filters, `max_error_count`, `@@error_count`, `GET DIAGNOSTICS`, privileges, or full diagnostics-area behavior |
