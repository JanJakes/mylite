...
static int run_single_stdin(int quiet, int permissive);
static int run_nul_stream(int quiet, int permissive);
...
int main(int argc, char **argv) {
  ...
  if (nul_stream) {
    return run_nul_stream(quiet, permissive);
  }

  return run_single_stdin(quiet, permissive);
}
...
static int run_single_stdin(int quiet, int permissive) {
  ...
  status = parse_one(buffer.data, buffer.length, 1, quiet, permissive);
  buffer_free(&buffer);
  return status;
}
