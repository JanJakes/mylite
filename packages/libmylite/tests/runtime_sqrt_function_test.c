...
static int test_sqrt_values_and_file_safety(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open sqrt values file");
...
static int test_sqrt_warnings_do_and_independent_handles(void) {
    ...
    if (result != NULL) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "sqrt DO column count");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "sqrt DO row count");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "sqrt DO warning count"
        );
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "sqrt DO affected rows"
        );
    }
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first sqrt file handle"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second sqrt file handle"
    );
    failures += expect_query(
        first,
        (struct expected_query){
...
static int test_sqrt_errors_and_unsupported_forms(void) {
    ...
    failures += execute_ok(database, "SELECT SQRT(id) FROM t ORDER BY id", NULL);
    failures += execute_error(
        database,
        "SELECT 1+SQRT(4)",
        (struct expected_sql_error){
