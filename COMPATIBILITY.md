...
| `AND`, `&&` | 🟡 | Limited descriptor-backed `WHERE` boolean expressions over existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; `AND` binds tighter than `OR`, and `&&` is accepted with MySQL-compatible deprecation warning 1287. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `WHERE` | 🟡 | Descriptor-backed boolean expressions over supported integer/`NULL` predicate atoms for filtered table `SELECT`, aggregate source filters, `DELETE`, and `UPDATE`, with keyword `NOT`, `AND`/`&&`, `OR`/`\|\|`, and parentheses following MySQL precedence; `SELECT` supports source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands and `BETWEEN` bounds; nonempty decimal integer, `TRUE`/`FALSE`, or `NULL` `IN` lists; descriptor-column `IS [NOT] TRUE` / `FALSE` / `UNKNOWN`; no `WHERE TRUE`, symbolic `!`, `XOR`, literal-left comparisons, expression-left truth tests, subquery membership, row constructors, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
