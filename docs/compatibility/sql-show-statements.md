...
| `SHOW COLUMNS` / `SHOW FIELDS` | 🟡 | Limited descriptor-driven column listing for persistent base tables; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE 'pattern'` filters with `Field`, integer, `decimal(M,D)`, `decimal(M,D) unsigned`, `char(n)`, `varchar(n)`, and baseline `TEXT` family `Type`, `Null`, `Key` including `PRI` for the current single-column primary-key descriptor, SQL `NULL`, canonical integer, or canonical decimal `Default`, and `Extra`, including `auto_increment` for the current auto-increment descriptor and `INVISIBLE` for descriptor-invisible columns; no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, secondary indexes, string defaults, expression defaults, generated hidden columns, or `INFORMATION_SCHEMA` |
...
| `SHOW CREATE TABLE` | 🟡 | Limited descriptor-driven MySQL-style DDL for persistent base tables with current integer-family, exact `DECIMAL`, `CHAR(0..255)`, `VARCHAR(0..255)`, and baseline `TEXT` family descriptors, nullability, implicit nullable-column `DEFAULT NULL` rendering for non-`TEXT` descriptors, dropped-default columns with no default clause, canonical quoted integer and decimal defaults except hidden auto-increment defaults created by `ALTER ... SET DEFAULT`, descriptor-invisible column comments, current single-column primary-key and auto-increment rendering, fixed InnoDB/utf8mb4 suffix, and fixed enabled `sql_quote_show_create` quoting; no views, temporary tables, secondary indexes, string defaults, expression defaults, named constraints, generated columns, privileges, mutable quote-control state, or disabled rendering |
