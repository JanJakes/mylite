...
static int test_filtered_select_success_persistence_rename_and_drop(void) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "filtered star row count");
    failures +=
        mylite_test_expect_text(mylite_result_column_name(result, 0U), "i", "filtered star i name");
    failures += mylite_test_expect_text(
        mylite_result_column_name(result, 1U),
        "iu",
        "filtered star iu name"
    );
    failures +=
        mylite_test_expect_text(mylite_result_column_name(result, 2U), "b", "filtered star b name");
    failures += mylite_test_expect_text(
        mylite_result_column_name(result, 3U),
        "bu",
        "filtered star bu name"
    );
    failures +=
        mylite_test_expect_text(mylite_result_column_name(result, 4U), "n", "filtered star n name");
    failures += mylite_test_expect_text(
    ...
    failures += mylite_test_expect_int64(
        ...
        0,
        "filtered star affected rows"
    );
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "filtered star warnings");
    mylite_result_free(result);
    result = NULL;
    ...
    failures += mylite_test_expect_size(
        mylite_result_column_count(result),
        2U,
        "duplicate filtered columns"
    );
    failures += mylite_test_expect_text(
        mylite_result_column_name(result, 0U),
        "i",
        "duplicate filtered col 1"
    );
    failures += mylite_test_expect_text(
        mylite_result_column_name(result, 1U),
        "i",
        "duplicate filtered col 2"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "duplicate filtered rows");
...
static int test_filtered_select_diagnostics(void) {
    ...
    failures += expect_query_single_value(
        database,
        (struct expected_single_value_query){
            .sql = "SELECT 'visible' WHERE ((1 + 2) * 3) = 9",
            .expected = "visible",
            .context = "nested tableless row arithmetic comparison predicate",
        }
    );
    failures += expect_query_single_value(
        database,
