...
| `AND`, `&&` | 🟡 | Limited descriptor-backed `WHERE` conjunctions over existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; `&&` is accepted with MySQL-compatible deprecation warning 1287. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `WHERE` | 🟡 | Descriptor-backed `AND`/`&&` conjunctions of supported integer/`NULL` predicate atoms for filtered table `SELECT`, aggregate source filters, `DELETE`, and `UPDATE`, with `SELECT` supporting source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands; no `WHERE TRUE`, `OR`, `XOR`, `NOT`, literal-left comparisons, `IS TRUE`, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `UPDATE` (single-table) | 🟡 | Limited persistent base-table `UPDATE table SET column = integer_or_NULL_or_boolean` with optional baseline `WHERE`, one unqualified descriptor `ORDER BY` column, and signed-64-range `LIMIT row_count`; assignment, predicate, and ordering columns may explicitly name invisible descriptor columns; changed-row affected counts; no aliases, partitions, modifiers, joins, multiple assignments, expression assignments, full ordering, or offset forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
