...
| `CREATE FUNCTION` (stored) | ❌ | medium | Stored-function definition, determinism, SQL data access, security, and body semantics. | Parser recognizes empty and comma-separated input parameter signatures with nested type bodies, and validates direct/compound `RETURN`, `DO`, `SET`, and local-variable `DECLARE` expression bodies where those reusable validators apply. |
