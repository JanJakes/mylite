...
static int test_no_source_dual_and_do_to_days_to_seconds(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "to_days do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "to_days do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "to_days do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "to_days do warnings");
    }
...
static int test_table_backed_to_days_to_seconds_and_reopen(void) {
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen to_days");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-to-days-to-seconds-functions-%s-%d.mylite",
        name,
        current_process_id()
    );
