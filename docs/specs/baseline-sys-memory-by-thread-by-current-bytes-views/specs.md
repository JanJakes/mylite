# Baseline sys.memory_by_thread_by_current_bytes Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.memory_by_thread_by_current_bytes` and
`sys.x$memory_by_thread_by_current_bytes` views. MySQL uses these sys views to
summarize current and cumulative Performance Schema memory allocation counters
grouped by thread. MyLite does not collect Performance Schema memory-summary
rows, so both views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.memory_by_thread_by_current_bytes` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_memory_by_thread_by_current_bytes_views_expectations.sh`.
...
reflect live Performance Schema thread and memory instrumentation.
...
SELECT * FROM `x$memory_by_thread_by_current_bytes`;
```
...
surface without inventing thread identifiers, users, thread names, or memory
allocation counters.

## Column Metadata

`sys.memory_by_thread_by_current_bytes` has seven columns:
...
`sys.x$memory_by_thread_by_current_bytes` has the same logical columns except
...
`INFORMATION_SCHEMA.COLUMNS` expose this shape, including unsigned thread IDs,
the `utf8mb4` user/name expression, formatted `varchar(11)` byte strings,
decimal precision/scale, and signed `BIGINT` numeric precision for the raw
maximum allocation. The views have no index or constraint metadata, so
...
corresponding numeric aggregates. Both definitions join
`performance_schema.memory_summary_by_thread_by_event_name` to
`performance_schema.threads`, build the display `user` from the connection user
and host for ordinary client threads or from the thread name for other threads,
group by thread and display user, and order by descending current allocated
bytes.
...
- live `performance_schema.memory_summary_by_thread_by_event_name` or
  `performance_schema.threads` rows;
- memory instrumentation, thread inventory, current allocation counters,
  cumulative allocation counters, or per-thread aggregation;
...
Performance Schema thread and memory counters.
