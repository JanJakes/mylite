...
static int test_mysql_plugin_table(void);
...
static const char *const plugin_columns[plugin_column_count] = {
    "name",
    "dl",
};
...
int main(void) {
    return test_mysql_plugin_table() == 0 ? 0 : 1;
}
...
static int test_mysql_plugin_table(void) {
    ...
    static const char *const direct_values[] = {
        "CONNECTION_CONTROL",
    ...
    if (make_test_path(path, sizeof(path), "mysql-plugin-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT name, dl FROM mysql.plugin ORDER BY name",
            .column_names = plugin_columns,
            .column_count = plugin_column_count,
            .values = direct_values,
            .row_count = sizeof(direct_values) / sizeof(direct_values[0]) / plugin_column_count,
            .context = "mysql.plugin direct read",
        }
    );
    failures += expect_row_count_status(database, "row count after mysql.plugin read");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.plugin",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_two,
            .row_count = 1U,
            .context = "mysql.plugin count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT name, dl FROM plugin WHERE name LIKE 'CONNECTION%' ORDER BY name",
            .column_names = plugin_columns,
            .column_count = plugin_column_count,
            .values = direct_values,
            .row_count = sizeof(direct_values) / sizeof(direct_values[0]) / plugin_column_count,
            .context = "mysql.plugin unqualified filtered rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.plugin",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.plugin show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.plugin",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.plugin show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = show_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.plugin unqualified desc",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.plugin WHERE Key_name = 'PRIMARY'",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = show_index_values,
            .row_count = 1U,
            .context = "mysql.plugin show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW KEYS FROM plugin WHERE Key_name = 'PRIMARY'",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = show_index_values,
            .row_count = 1U,
            .context = "mysql.plugin unqualified show keys",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_columns_row_count,
            .context = "mysql.plugin information_schema.columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.plugin information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.plugin information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.plugin information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.plugin information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.plugin information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'plugin' "
                   "AND Update_time IS NOT NULL",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.plugin show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_plugin_table_%d_%s.mylite",
        current_process_id(),
        name
    );
