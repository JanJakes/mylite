# Baseline WHERE IN Predicates Tasks
...
- [x] Probe MySQL 8.4.9 runtime behavior for `IN`, `NOT IN`, precedence,
  parentheses, nullable tested values, `NULL` list values, DML affected rows,
  ordering, limiting, and warnings.
...
- [ ] Commit the start-feature artifacts.
- [ ] Add parser grammar, AST kind/operator support, parser helpers, and token
  mapping for `IN` membership predicates.
- [ ] Extend descriptor-driven predicate planning for membership leaves, cleanup,
