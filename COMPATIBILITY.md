...
| `DISTINCT` / `DISTINCTROW` | ❌ | top | Duplicate elimination semantics and metadata. | Started; specification and MySQL 8.4.9 runtime expectations are documented, but parser/analyzer/runtime support and comparison tests are not implemented. See [SELECT DISTINCT spec](docs/specs/select-distinct/specs.md). |
...
| SELECT modifiers | ❌ | top | ALL, HIGH_PRIORITY, SQL_SMALL_RESULT, SQL_BIG_RESULT, SQL_BUFFER_RESULT, SQL_CALC_FOUND_ROWS, and STRAIGHT_JOIN. | `ALL` is specified as part of Task 28 duplicate-mode parsing, but remains unimplemented with `DISTINCT`/`DISTINCTROW`; the remaining optimizer and execution-strategy modifiers are deferred. See [SELECT DISTINCT spec](docs/specs/select-distinct/specs.md). |
