...
| `ADD PRIMARY KEY` | ❌ | top | Primary key addition and data validation. | Specified for metadata-backed ALTER execution with existing-row duplicate/`NULL` validation, implicit `NOT NULL` column metadata rewrite, statistics/key-constraint metadata, and DML conflict-order integration; pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP PRIMARY KEY` | ❌ | top | Primary key removal and auto-increment restrictions. | Specified for primary-key metadata removal while preserving `NOT NULL` column state and rejecting `AUTO_INCREMENT` or future foreign-key dependencies; pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD UNIQUE` | ❌ | top | Unique index addition and duplicate validation. | Specified for metadata-backed ALTER execution using standalone index validation, nullable unique-key semantics, duplicate validation, generated/named index behavior, visibility, and DML conflict integration; pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD INDEX` / `ADD KEY` | ❌ | top | Secondary index addition and metadata. | Specified for metadata-backed nonunique index addition with generated names, key-part metadata, visibility, comments, duplicate-index warnings, and Task 35 multi-action atomicity; pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD FULLTEXT` | ❌ | high | Full-text index addition. | Specified as a parsed ALTER surface with deterministic unsupported diagnostics until full-text metadata, hidden-column behavior, parser plugins, and search runtime are implemented. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD SPATIAL` | ❌ | medium | Spatial index addition. | Specified as a parsed ALTER surface with deterministic unsupported diagnostics until spatial types, SRID checks, and spatial index runtime are implemented. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP INDEX` / `DROP KEY` | 🟡 | top | Index removal and constraint dependencies. | Standalone `DROP INDEX name ON table` is implemented for metadata-backed indexes created by supported table/index DDL. The `ALTER TABLE DROP INDEX` / `DROP KEY` action is specified but pending implementation, including primary/unique conflict-surface removal and future foreign-key dependency checks. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md) and [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `RENAME INDEX` / `RENAME KEY` | ❌ | high | Index rename semantics. | Specified for metadata-only rename of non-primary indexes while preserving key parts, visibility, uniqueness, and conflict order; pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ALTER INDEX VISIBLE` / `INVISIBLE` | ❌ | medium | Index visibility metadata and optimizer behavior. | Specified for metadata visibility toggles with uniqueness still enforced and explicit/implicit primary-key invisibility rejected; optimizer effects remain deferred. Pending implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD CONSTRAINT CHECK` | ❌ | high | Check constraint addition and validation. | Specified for CHECK parser/catalog/runtime behavior, including enforced/not-enforced validation and metadata exposure. First executable slice may return unsupported before mutation until expression-backed CHECK catalogs are implemented. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP CHECK` | ❌ | high | Check constraint removal. | Specified for CHECK metadata removal and missing-name diagnostics; pending CHECK catalog implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ALTER CHECK ENFORCED` / `NOT ENFORCED` | ❌ | high | Check enforcement toggling. | Specified for enforcement metadata toggles with existing-row validation before enabling; pending CHECK catalog and expression runtime implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD CONSTRAINT FOREIGN KEY` | ❌ | high | Foreign key addition, validation, indexes, and actions. | Specified for parser, naming, validation, supporting-index, `foreign_key_checks`, and metadata behavior. First executable slice should return unsupported before mutation until foreign-key catalogs, enforcement, cascades, and dependency checks exist. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP FOREIGN KEY` | ❌ | high | Foreign key removal and metadata cleanup. | Specified for foreign-key metadata removal while preserving supporting child indexes; pending foreign-key catalog implementation. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
