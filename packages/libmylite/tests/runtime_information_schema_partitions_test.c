...
static int test_information_schema_partitions_queries(void) {
    ...
    failures += expect_int(rc, MYLITE_OK, "open partitions db");
    if (rc != MYLITE_OK) {
        return failures + 1;
    }
...
static int test_information_schema_partitions_reopen_preamble_and_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first partitions db");
    failures +=
        expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second partitions db");
    ...
    failures +=
        expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first partitions db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT TABLE_SCHEMA, TABLE_NAME, TABLE_ROWS "
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_partitions_%s_%d.mylite",
        name,
        current_process_id()
    );
