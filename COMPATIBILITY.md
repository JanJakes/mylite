...
| `ANALYZE TABLE` | ❌ | high | Statistics refresh, histogram update/drop, validation, and result-set metadata. | Parser recognizes table lists and histogram update/drop clauses after table lists, including numeric histogram bucket counts, automatic/manual histogram update modes, and text-string histogram data with quoted hex/bit rejection, using the shared identifier grammar for tables and columns. |
