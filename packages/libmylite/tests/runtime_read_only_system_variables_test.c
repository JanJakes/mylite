...
static int test_read_only_variable_values_and_noop_sets(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open read-only variable file");
    ...
    failures +=
        expect_int(mylite_open(path, &database), MYLITE_OK, "reopen read-only variable file");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int expect_set_ok(mylite_db *database, const char *sql) {
    ...
    failures += expect_size(mylite_result_column_count(result), 0U, "read-only SET column count");
    failures += expect_size(mylite_result_row_count(result), 0U, "read-only SET row count");
    failures += expect_int64(mylite_result_affected_rows(result), 0, "read-only SET affected rows");
    failures += expect_size(mylite_result_warning_count(result), 0U, "read-only SET warning count");
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_read_only_system_variables_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
