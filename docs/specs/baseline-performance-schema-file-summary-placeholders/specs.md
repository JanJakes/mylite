# Baseline Performance Schema File Summary Placeholders
...
returns zero rows from each table until live Performance Schema file I/O
...
```sh
packages/libmylite/tests/mysql_baseline_performance_schema_file_summary_placeholders_expectations.sh
```
...
The `file_summary_by_event_name` table has a primary HASH key on `EVENT_NAME`.
The `file_summary_by_instance` table has a primary HASH key on
`OBJECT_INSTANCE_BEGIN` and nonunique HASH indexes on `FILE_NAME` and
`EVENT_NAME`.

## Intentional Gaps

This slice does not implement live Performance Schema file I/O aggregation. A
MySQL 8.4.9 runtime exposes nonzero rows when file instruments exist and file
activity has been observed. MyLite intentionally returns empty rows until it has
a real file instrumentation pipeline and an explicit summary lifecycle. It also
does not implement the MySQL `TRUNCATE TABLE` reset semantics for these summary
tables.
...
- Build and run `mylite_runtime_performance_schema_file_summary_placeholders_test`.
- Run the focused CTest filter
  `^libmylite\.runtime\.performance_schema_file_summary_placeholders$`.
