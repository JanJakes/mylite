...
enum {
    ...
    information_schema_statistics_row_count = 3,
    information_schema_tables_projection_count = 5,
    information_schema_tables_row_count = 2,
    show_columns_column_count = 6,
    show_event_columns_row_count = 3,
    show_index_column_count = 15,
    show_index_instance_row_count = 2,
    show_instance_columns_row_count = 3,
...
static int test_performance_schema_socket_summary_placeholders(void);
...
int main(void) {
    return test_performance_schema_socket_summary_placeholders() == 0 ? 0 : 1;
}
...
static int test_performance_schema_socket_summary_placeholders(void) {
    ...
    static const char *const show_event_columns_rows[] = {
        "EVENT_NAME",
        "varchar(128)",
        "NO",
        "PRI",
    ...
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.socket_summary_by_event_name",
        "socket_summary_by_event_name count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.socket_summary_by_instance",
        "socket_summary_by_instance count"
    );
    failures += execute_ok(database, "USE performance_schema");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM socket_summary_by_event_name",
            .column_names = one_count_column,
            .values = zero_count,
            .column_count = 1U,
            .row_count = 1U,
            .context = "selected schema socket summary read",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM performance_schema.socket_summary_by_event_name "
                   "WHERE Field IN ("
                   "'EVENT_NAME', 'SUM_NUMBER_OF_BYTES_READ', 'MAX_TIMER_MISC')",
            .column_names = show_columns_columns,
            .values = show_event_columns_rows,
            .column_count = show_columns_column_count,
            .row_count = show_event_columns_row_count,
            .context = "show socket summary by event columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM performance_schema.socket_summary_by_instance "
                   "WHERE Field IN ("
                   "'EVENT_NAME', 'OBJECT_INSTANCE_BEGIN', 'SUM_NUMBER_OF_BYTES_WRITE')",
            .column_names = show_columns_columns,
            .values = show_instance_columns_rows,
            .column_count = show_columns_column_count,
            .row_count = show_instance_columns_row_count,
            .context = "show socket summary by instance columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM performance_schema.socket_summary_by_instance",
            ...
            .context = "show socket summary by instance index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM performance_schema "
                   "WHERE Name = 'socket_summary_by_instance' "
            ...
            .row_count = 1U,
            .context = "show table status for socket summary",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_TYPE, "
                   ...
                   "AND ((TABLE_NAME = 'socket_summary_by_event_name' "
                   "AND COLUMN_NAME IN ("
                   "'EVENT_NAME', 'SUM_NUMBER_OF_BYTES_READ', 'MAX_TIMER_MISC')) "
            ...
            .context = "information schema socket summary columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema socket summary statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema socket summary constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema socket summary key usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema socket summary constraint extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema socket summary tables",
        }
    );
    failures += execute_error(
        database,
        "INSERT INTO performance_schema.socket_summary_by_event_name "
        "(EVENT_NAME) VALUES ('wait/io/socket/sql/server_tcpip_socket')",
        (struct expected_sql_error){
