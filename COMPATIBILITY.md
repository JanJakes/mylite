...
| --- | --- | --- | --- | --- |
| Lemon parser prototype | 🟡 | top | Lean C parser foundation with MySQL-aware lexing, statement-family recognition, permissive corpus-fragment mode, and corpus acceptance. | Current grammar recognizes statement starts plus selected closed keyword subgrammars, including view check-option tails for `TABLE`, `VALUES`, and parenthesized query-expression forms, and validates selected top-level `SELECT` clause ordering, top-level and set-operand `SELECT` list tails including MySQL's bare-`*` placement rule, context-sensitive `DEFAULT` expressions, row/default assignments, SET system-variable-only literals, `DO` wildcard select-item rules, parenthesized query-body `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails, CTE-body query-expression starts and `SELECT` list tails, top-level `WITH` `UPDATE`/`DELETE` versus invalid `INSERT`/`REPLACE` placement, `TABLE` statement targets/tails, top-level/CTE `VALUES ... INTO` targets, and parenthesized query `INTO` variable/file-output tails. `*`, operator-like tokens, numeric literals, single-quoted literals, and standalone reserved `CASCADE`/`RESTRICT` identifiers are handled by explicit wildcard/body productions or rejected in strict object-name and account-principal grammar rather than as generic identifier fallbacks, while quoted and dotted reserved-name references remain valid. The parser does not yet implement full clause-level MySQL syntax. Verified against the WordPress SQLite Database Integration MySQL query corpus, with a strict-mode fixture for known corpus fragments. See [docs/parser-lemon-prototype.md](docs/parser-lemon-prototype.md). |
...
| `ALTER LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names that reject quoted hex/bit literals, unsigned size `INITIAL_SIZE` including numeric/quoted hex and suffixes, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
| `ALTER PROCEDURE` | ❌ | medium | Stored-procedure metadata characteristics. | Parser recognizes routine characteristic clauses: string-literal comments with quoted hex/bit rejection, `LANGUAGE SQL`, SQL data access, and SQL security. |
| `ALTER SERVER` | ❌ | low | Foreign server metadata changes. | Parser recognizes the documented `OPTIONS` names and value requirements, including text-string validation for string-valued options. |
| `ALTER TABLE` | ❌ | top | Full table rebuild/in-place/instant surface; see section 3.2. | Parser recognizes selected closed actions including comma-separated `ADD`/`CHANGE`/`MODIFY` bodies, `FORCE`, `ENABLE/DISABLE KEYS`, `RENAME` forms, `DROP` forms including column-drop `RESTRICT`/`CASCADE` tails, `ALTER` subactions with validated `SET DEFAULT` expression tails, charset/order changes with validated `ORDER BY` expression-list tails, partition definition/maintenance/exchange/reorganize forms with binlog modifiers, validated partition-method expression/key lists, key algorithm values, and partition/subpartition counts, and validated `PARTITION BY ... (...)`, `ADD PARTITION (...)`, and `REORGANIZE PARTITION ... INTO (...)` partition-definition envelopes, CHECK/REPAIR options, unsigned integer/lowercase-hex coalesce/add-partition counts, tablespace/storage/union changes with closed storage values, secondary-engine load/unload actions with optional concrete partition lists, table option changes with charset/collation continuations, unsigned decimal, boolean, and default-boolean value domains, decimal-versus-size number domains including quoted-hex size values, bounded numeric and quoted-hex `STATS_SAMPLE_PAGES`, double-comma and invalid continuation rejection, closed `INSERT_METHOD` and `ROW_FORMAT` values, string-literal data/index directories, string-literal compression/password/connection/engine attributes, `AUTOEXTEND_SIZE`, `TABLE_CHECKSUM`, `START TRANSACTION`, identifier-valued `ALGORITHM`/`LOCK` options, tablespace discard/import forms, validated `ADD`/`CHANGE`/`MODIFY` column type starts plus common numeric and `ENUM`/`SET` type-parameter lists, MySQL-shaped numeric type parameter literals and `DOUBLE`/`REAL` precision lists, MySQL's `DOUBLE PRECISION` modifier placement, MySQL-scoped numeric versus character type modifiers, MySQL-ordered character-set modifier choices, repeated column `COLLATE` rejection, repeated column `COMMENT` attributes with all MySQL string-literal token classes, quoted hex/bit literal rejection for text option values, `CONSTRAINT` prefixes restricted to `CHECK` in column definitions, `DEFAULT` rejection for table-option charset values and column charset/collation values, required `ENUM`/`SET` member lists, required lengths for `VARCHAR`, `VARBINARY`, `NVARCHAR`, and `CHAR VARYING` forms, parameterless `LONG VARCHAR`/`LONG VARBINARY`/`LONG CHAR VARYING` aliases, national-character binary modifiers, column-definition attribute starts, and selected closed attribute values including `SERIAL DEFAULT VALUE` attribute sequences, column `ENGINE_ATTRIBUTE`/`SECONDARY_ENGINE_ATTRIBUTE` string attributes, integer `SRID` values, generated column heads (`AS (...)` or `GENERATED ALWAYS AS (...)`), parenthesized default/generated expression bodies with scalar comma-list rejection, temporal precision lists for `DEFAULT`/`ON UPDATE` functions, stray attribute-parenthesis rejection, and column-level references without explicit parent column lists, validated `ADD` standalone reserved index/constraint-name rejection, index key-part prefix lengths plus `ASC`/`DESC` tails, and index option values including unsigned decimal/lowercase-hex/quoted-hex `KEY_BLOCK_SIZE` and closed `USING`/`TYPE` values, validated `ADD FOREIGN KEY` child lists, referenced table names, optional parent column-list envelopes, and referential-action tails with duplicate `MATCH`/`ON UPDATE`/`ON DELETE` rejection and MySQL's `MATCH`-before-actions order, and validated `ADD CHECK` expression bodies with valid `ENFORCED`/`NOT ENFORCED` tails. `DROP`, `EXCHANGE`, `REORGANIZE PARTITION`, and secondary-engine partition lists require concrete partition names; `REORGANIZE PARTITION ... INTO (...)` also requires concrete partition names and a non-empty `INTO (...)` body. |
| `ALTER TABLESPACE` | ❌ | low | General tablespace alterations and diagnostics. | Parser recognizes rename, `ADD/DROP DATAFILE` with text-string literal validation, unsigned size values including suffixes and numeric/quoted hex, `WAIT`, string-literal encryption values, repeated simple `ENGINE`/`STORAGE ENGINE` option lists, and string-literal attribute clauses with quoted hex/bit rejection in text-string positions. |
...
| `ALTER VIEW` | ❌ | high | View replacement while preserving MySQL metadata and security semantics. | Parser recognizes column lists, view body starts, and `WITH [CASCADED|LOCAL] CHECK OPTION` tails on non-plain-`SELECT` bodies including parenthesized query expressions, with validation for direct body `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized query-body `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails, parenthesized query `ORDER BY`/`LIMIT` suffixes, and malformed set-operation `SELECT` operands. |
| `CREATE DATABASE` / `CREATE SCHEMA` | ❌ | high | Database creation syntax, defaults, warnings, and single-file mapping. | Parser recognizes charset, collation, and string-literal encryption option clauses. |
| `CREATE EVENT` | ❌ | medium | Scheduled event definition, body, definer, comments, and scheduler metadata. | Parser recognizes schedule, completion, enable/disable including `DISABLE ON REPLICA`/`SLAVE`, comment with text-string validation, and required `DO` clauses, validates `AT`/`EVERY` schedule tails including interval units and `STARTS`/`ENDS`, and routes recognized single-statement bodies including direct `SELECT`/`WITH`/`TABLE` query tails and flow-control conditions through existing statement validators. |
| `CREATE FUNCTION` (stored) | ❌ | medium | Stored-function definition, determinism, SQL data access, security, and body semantics. | Parser recognizes empty and comma-separated input parameter signatures with nested type bodies, routine comments with quoted hex/bit rejection, and validates direct/compound `RETURN`, `DO`, `SET`, local-variable `DECLARE`, and compound control-flow condition expressions where those reusable validators apply. |
| `CREATE FUNCTION` (loadable) | ❌ | low | Loadable-function registration syntax with embedded-compatible diagnostics. | Parser recognizes UDF return types and requires text-string `SONAME` values with quoted hex/bit rejection. |
| `CREATE INDEX` | ❌ | top | Standalone index creation over MySQL index types and attributes. | Parser recognizes non-empty functional/key-part lists, validates key-part prefix lengths and optional `ASC`/`DESC` tails, rejects standalone reserved `CASCADE`/`RESTRICT` index names, and recognizes index options for parser plugins, string-literal comments, visibility, string-literal engine attributes with quoted hex/bit literal rejection in text-string positions, unsigned decimal/lowercase-hex/quoted-hex `KEY_BLOCK_SIZE`, closed `USING`/`TYPE` values, and identifier-valued `ALGORITHM`/`LOCK` options with duplicate option rejection. |
| `CREATE LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names that reject quoted hex/bit literals, NDB unsigned size options including numeric/quoted hex and suffixes, unsigned integer/lowercase-hex nodegroups, comments, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
| `CREATE PROCEDURE` | ❌ | medium | Stored procedure parameters, body, characteristics, and diagnostics. | Parser recognizes empty and comma-separated `IN`/`OUT`/`INOUT` parameter signatures with nested type bodies, routine comments with quoted hex/bit rejection, and validates direct/compound `DO`, `SET`, `RETURN`, local-variable `DECLARE`, direct DML/`VALUES`/`WITH`/`TABLE` bodies, direct `SELECT` list tails, and compound control-flow condition expressions where those reusable validators apply. |
| `CREATE SERVER` | ❌ | low | Foreign server metadata syntax. | Parser recognizes `FOREIGN DATA WRAPPER` with documented `OPTIONS` names, text-string option values, and unsigned decimal plus lowercase-hex ports. |
| `CREATE SPATIAL REFERENCE SYSTEM` | ❌ | medium | Spatial reference system catalog DDL. | Parser recognizes unsigned integer/lowercase-hex SRS ids, bare, `IF NOT EXISTS`, `OR REPLACE`, and documented text-string SRS attribute syntax with quoted hex/bit rejection and unsigned integer/lowercase-hex organization authority codes. |
| `CREATE TABLE` | ❌ | top | Column definitions, constraints, indexes, table options, generated columns, and partitions; see section 3.1. | Parser requires non-empty comma-separated table-definition elements, validates column definitions start with a known MySQL type family plus common attribute starts and selected closed attribute values, validates common numeric and `ENUM`/`SET` type-parameter lists including hex literal enum/set members, requires `ENUM`/`SET` member lists, preserves MySQL's numeric type parameter token shapes, `YEAR(4)` width rule, and two-integer `DOUBLE`/`REAL` precision lists, preserves MySQL's `DOUBLE PRECISION` modifier placement without treating `PRECISION` as a generic type modifier, restricts `SIGNED`/`UNSIGNED`/`ZEROFILL` to MySQL numeric/year type families, restricts `ASCII`/`UNICODE`/`BYTE`/`BINARY` and charset introducers to character/text/`ENUM`/`SET` families using MySQL's ordered character-set modifier choices, rejects repeated column `COLLATE` clauses, accepts repeated column `COMMENT` attributes with all MySQL string-literal token classes, rejects quoted hex/bit literals in text option values, restricts `CONSTRAINT` prefixes to `CHECK` in column definitions, and rejects `DEFAULT` charset/collation values in column and table-option contexts, requires lengths for `VARCHAR`, `VARBINARY`, `NVARCHAR`, and `CHAR VARYING` forms while preserving parameterless `LONG VARCHAR`/`LONG VARBINARY`/`LONG CHAR VARYING`, validates national-character `BINARY` modifiers, preserves nested column/constraint definition bodies, validates generated column heads (`AS (...)` or `GENERATED ALWAYS AS (...)`), validates parenthesized default/generated column expression bodies for dangling operators and scalar comma lists, validates temporal precision lists for `DEFAULT`/`ON UPDATE` functions, rejects stray attribute parentheses, validates table-level index key-part prefix lengths plus `ASC`/`DESC` tails, closed `USING`/`TYPE` values, and standalone reserved index/constraint names, validates column/table foreign-key child lists, referenced table names, optional parent column-list envelopes, and referential-action tails with duplicate `MATCH`/`ON UPDATE`/`ON DELETE` rejection and MySQL's `MATCH`-before-actions order, accepts column-level references without explicit parent column lists, validates column/table `CHECK` expression bodies for dangling operators and scalar comma lists, validates `CHECK` `ENFORCED`/`NOT ENFORCED` tails, validates table-option starts and selected value domains including unsigned decimal, boolean, default-boolean, and decimal-versus-size numeric options with quoted-hex size values, bounded numeric and quoted-hex `STATS_SAMPLE_PAGES`, string-literal options, closed `INSERT_METHOD`, closed `ROW_FORMAT`, storage values without equality signs, validated `UNION` table-name lists, and `START TRANSACTION`, and recognizes no-definition and post-definition CTAS forms with table/partition options and `IGNORE`/`REPLACE` duplicate-handling modifiers while requiring no-definition option forms to include a query body and validating direct CTAS `SELECT` list tails, parenthesized CTAS query-body `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails, CTE-body `SELECT` list tails, and `VALUES ROW(...)` CTAS row-expression tails. |
| `CREATE TEMPORARY TABLE` | ❌ | high | Session-scoped table lifecycle and name shadowing. | Parser recognizes `CREATE TEMPORARY TABLE` definitions, `LIKE`, and CTAS forms through the shared `CREATE TABLE` grammar. |
| `CREATE TABLE ... LIKE` | ❌ | high | Exact metadata cloning rules. | Parser recognizes `CREATE [TEMPORARY] TABLE [IF NOT EXISTS] target LIKE source` with one- or two-part target/source names, and rejects table options or trailing tokens after the source table. |
| `CREATE TABLE ... SELECT` | ❌ | high | CTAS type inference, default handling, indexes, and atomicity. | Parser recognizes no-definition and post-definition CTAS forms, including parenthesized query bodies, and validates direct CTAS `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized query-body `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails, parenthesized query `ORDER BY`/`LIMIT` suffixes, malformed set-operation `SELECT` operands, and `VALUES ROW(...)` row-expression bodies. |
| `CREATE TABLESPACE` | ❌ | low | General and NDB tablespace syntax and diagnostics. | Parser recognizes string-literal data files with text-string validation, unsigned autoextend/file-block/extent/initial/max size values including suffixes and numeric/quoted hex, string-literal encryption values, NDB logfile/size/comment options, unsigned integer/lowercase-hex nodegroups, `ENGINE`/`STORAGE ENGINE`, and string-literal engine attributes with quoted hex/bit rejection in text-string positions. |
...
| `CREATE TRIGGER` | ❌ | high | Trigger timing, event, ordering, body, definer, and metadata. | Parser recognizes trigger timing, event, target table, optional ordering, definer forms, and stored-program statement starts for single-statement trigger bodies, routing recognized body starts including direct `SELECT` list tails, `RETURN` expression tails, and flow-control conditions through existing statement validators. |
| `CREATE VIEW` | ❌ | high | View column names, algorithms, security, check options, and metadata. | Parser recognizes algorithm, definer, SQL security, column lists, view body starts, and `WITH [CASCADED|LOCAL] CHECK OPTION` tails on non-plain-`SELECT` bodies including parenthesized query expressions and `VALUES` bodies, with validation for direct body `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized query-body `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails, parenthesized query `ORDER BY`/`LIMIT` suffixes, malformed set-operation `SELECT` operands, and `VALUES ROW(...)` row-expression tails. |
| `DROP DATABASE` / `DROP SCHEMA` | ❌ | high | Schema removal, metadata cleanup, warnings, and embedded single-file constraints. |  |
| `DROP EVENT` | ❌ | medium | Event metadata deletion. | Parser recognizes optional `IF EXISTS` and one- or two-part event names. |
...
| `DROP INDEX` | ❌ | top | Standalone index removal semantics. | Parser recognizes optional identifier-valued `ALGORITHM` and `LOCK` clauses with duplicate option rejection. |
...
| `DROP PROCEDURE` | ❌ | medium | Stored-procedure deletion and metadata cleanup. | Parser recognizes optional `IF EXISTS` and one- or two-part procedure names. |
| `DROP SERVER` | ❌ | low | Foreign server metadata deletion. | Parser recognizes optional `IF EXISTS` and server names. |
| `DROP SPATIAL REFERENCE SYSTEM` | ❌ | medium | Spatial reference system deletion and dependency checks. | Parser recognizes unsigned integer/lowercase-hex SRS ids and optional `IF EXISTS`. |
| `DROP TABLE` | ❌ | top | Multi-table drop, temporary tables, foreign-key checks, and warnings. | Parser recognizes table lists plus optional `RESTRICT`/`CASCADE` tails and rejects standalone reserved `CASCADE`/`RESTRICT` table names while preserving quoted and dotted references. |
...
| `DROP TRIGGER` | ❌ | high | Trigger deletion and metadata cleanup. | Parser recognizes optional `IF EXISTS` and one- or two-part trigger names. |
| `DROP VIEW` | ❌ | high | Multi-view drop and warnings. | Parser recognizes view lists plus optional `RESTRICT`/`CASCADE` tails and rejects standalone reserved `CASCADE`/`RESTRICT` view names while preserving quoted and dotted references. |
| `RENAME TABLE` | ❌ | top | Atomic multi-table rename semantics. | Parser recognizes `RENAME TABLE`/`RENAME TABLES` with comma-separated one- or two-part source/target pairs. |
| `TRUNCATE TABLE` | ❌ | top | DDL-like truncate, auto-increment reset, implicit commit, and foreign-key restrictions. | Parser recognizes optional `TABLE` and one- or two-part table references using the shared identifier grammar, rejecting standalone reserved `CASCADE`/`RESTRICT` names while preserving quoted and dotted references. |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| `CALL` | ❌ | medium | Procedure invocation, result sets, OUT/INOUT parameters, and diagnostics. | Parser recognizes one- and two-part routine names plus comma-separated argument lists with nested expression bodies, and rejects malformed argument adjacent operands, dangling operators, and invalid plain parenthesized groups, including `CALL` bodies embedded in stored programs, events, and triggers. |
| `DELETE` (single-table) | ❌ | top | Single-table delete with aliases, partitions, ORDER BY, LIMIT, LOW_PRIORITY, QUICK, and IGNORE. | Parser recognizes priority modifiers, table aliases before optional partition lists, `WHERE`/`ORDER BY`/`LIMIT` tails, and rejects incomplete, repeated, out-of-order, and non-integer numeric DML clause tails plus invalid top-level `ORDER BY` direction sequences and malformed `WHERE`/`ORDER BY` adjacent operands, dangling operators, and trailing separators including inside plain parenthesized expression groups. |
| `DELETE` (multi-table) | ❌ | high | Multi-table delete forms using FROM and USING, join semantics, and affected rows. | Parser recognizes `DELETE tbl_list FROM ...` and `DELETE FROM tbl_list USING ...` forms, including comma-separated delete targets, qualified `tbl.*` targets, joined source table references including join `USING (...)` conditions, and `WHERE` tails, and rejects MySQL-invalid `ORDER BY`/`LIMIT` tails plus mixed source-level `DELETE tbl_list FROM source USING source` forms. |
| `DO` | ❌ | medium | Expression execution with warning and error semantics. | Parser recognizes comma-separated expression lists including MySQL's first-item bare `*` and qualified wildcard select items, and rejects dangling separators, dangling operators, malformed nested expression groups, invalid `DEFAULT` values, adjacent top-level operands, misplaced later bare wildcards, wildcard aliases/arithmetic, and top-level query-clause tails such as `FROM`/`WHERE`/`ORDER BY`/`LIMIT`. |
| `HANDLER` | ❌ | low | HANDLER OPEN, READ, and CLOSE cursor-like table access. | Parser recognizes one- and two-part table names, aliases, key names, MySQL's table-scan and indexed-read direction sets, equality/range tuple reads, `WHERE`, and integer-or-identifier `LIMIT` tails with non-integer numeric rejection, and rejects malformed tuple expressions, malformed `WHERE` adjacent operands, dangling operators, and `LIMIT` suffixes after `READ`. |
| `IMPORT TABLE` | ❌ | low | Transportable tablespace import syntax and diagnostics. | Parser recognizes comma-separated text-string import file lists with quoted hex/bit rejection. |
| `INSERT ... VALUES` | ❌ | top | Multi-row values, defaults, generated columns, warnings, affected rows, and insert ids. | Parser recognizes empty and comma-separated column lists before write payloads and validates explicit `VALUE(S)` row-list tails, empty rows, row expression adjacent operands, dangling operators, and trailing separators, `VALUES ROW(...)` versus singular `VALUE (...)`, mixed `ROW(...)`/plain tuple rejection, optional row aliases only for plain tuples, and `ON DUPLICATE KEY UPDATE` assignment-list tails. |
| `INSERT ... SET` | ❌ | top | MySQL SET-form insert semantics. | Parser validates comma-separated assignment lists, dotted assignment targets, nested expression values, rejects top-level assignment value adjacent operands and dangling operators plus adjacent operands, dangling operators, and trailing separators inside plain parenthesized assignment value groups, repeated `SET` continuations and stray top-level `SELECT` continuations after assignment values, optional row aliases, and duplicate-key update tails. |
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser recognizes direct, parenthesized, and CTE-backed query payloads, validates direct payload `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized payload `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails plus outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, plus `ON DUPLICATE KEY UPDATE` continuations. |
| `INSERT ... ON DUPLICATE KEY UPDATE` | ❌ | top | Conflict target resolution, VALUES()/row aliases, affected rows, and warnings. | Parser validates the `ON DUPLICATE KEY UPDATE` keyword chain after value, set, direct query, parenthesized query, and CTE-backed `SELECT`/`TABLE`/`VALUES`/parenthesized-query payloads, rejects dangling select-list operands before the duplicate-key tail, non-empty comma-separated assignment lists, top-level assignment value adjacent operands and dangling operators, adjacent operands, dangling operators, and trailing separators inside plain parenthesized assignment value groups, and malformed post-value continuations while preserving `VALUES()` assignment expressions and nested subqueries. |
| `INSERT IGNORE` | ❌ | top | Duplicate, conversion, and constraint warning demotion rules. | Parser recognizes `IGNORE` in MySQL's post-priority modifier position and rejects reversed or repeated modifier order. |
| `INSERT DELAYED` | ❌ | low | Deprecated delayed insert syntax and MySQL-compatible diagnostics. | Parser recognizes the deprecated modifier in MySQL's priority slot and rejects combinations with other priority modifiers. |
| `INSERT LOW_PRIORITY` / `HIGH_PRIORITY` | ❌ | low | Priority modifiers and embedded-compatible treatment. | Parser recognizes either priority modifier before optional `IGNORE` and rejects repeated or mixed priority modifiers. |
| `LOAD DATA INFILE` | ❌ | high | Server-side text import syntax, field/line options, user variables, SET clause, warnings, and security restrictions. | Parser recognizes optional `FROM`, `INFILE`/`URL`/`S3` sources, text-string source names and compression values with quoted hex/bit rejection, priority/local modifiers, duplicate handling, source counts, primary-key-order hints, concrete partition names with `ALL` rejected, character sets, field/line options using MySQL's broader string-literal forms, unsigned-integer ignored-row counts, column/user-variable lists, validated comma-separated `SET` assignment tails, and unambiguous parallel/memory/bulk algorithm options. |
| `LOAD DATA LOCAL INFILE` | ❌ | high | Client-side LOCAL INFILE request flow, security controls, warnings, and protocol interaction. | Parser recognizes the `LOCAL` form and the same text-string data-file clauses as non-local `LOAD DATA`. |
| `LOAD XML INFILE` | ❌ | low | Server-side XML import syntax, row matching, namespaces, SET clause, warnings, and security restrictions. | Parser recognizes optional `FROM`, `INFILE`/`URL`/`S3` sources, text-string source names and compression values with quoted hex/bit rejection, priority/local modifiers, duplicate handling, source counts, primary-key-order hints, character sets, `ROWS IDENTIFIED BY`, field and line options in MySQL order, unsigned-integer ignored-row counts, field/user-variable lists, validated comma-separated `SET` assignment tails, and unambiguous parallel/memory/bulk algorithm options. |
| `LOAD XML LOCAL INFILE` | ❌ | low | Client-side XML import request behavior and embedded-compatible diagnostics. | Parser recognizes the `LOCAL` form and the same text-string XML-file clauses as non-local `LOAD XML`. |
| `REPLACE ... VALUES` | ❌ | top | Delete-then-insert semantics, cascades, triggers, affected rows, and auto-increment behavior. | Parser recognizes empty and comma-separated column lists before write payloads and validates explicit `VALUE(S)` row-list tails, empty rows, row expression adjacent operands, dangling operators, trailing separators, `ROW(...)` constructors, and mixed `ROW(...)`/plain tuple rejection. |
| `REPLACE ... SET` | ❌ | high | SET-form replace semantics. | Parser validates comma-separated assignment lists, dotted assignment targets, nested expression values, rejects top-level assignment value adjacent operands and dangling operators plus adjacent operands, dangling operators, and trailing separators inside plain parenthesized assignment value groups, repeated `SET` continuations and stray top-level `SELECT` continuations after assignment values, and rejects unsupported row-alias suffixes. |
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser recognizes direct, parenthesized, and CTE-backed query payloads, validates direct payload `SELECT` list tails, parenthesized payload `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails plus outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, and rejects unsupported `ON DUPLICATE KEY UPDATE` tails. |
| `REPLACE LOW_PRIORITY` / `DELAYED` | ❌ | low | Priority and deprecated delayed modifiers for REPLACE. | Parser recognizes either modifier in MySQL's single modifier slot and rejects repeated or mixed modifiers. |
| `SELECT` | ❌ | top | Full query expression surface; see section 2. | Parser rejects missing operands for major top-level clause starts such as `FROM`, `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY`, `LIMIT`, `QUALIFY`, and `INTO`, duplicate top-level `FROM`/`WHERE`/`GROUP BY`/`HAVING`/`ORDER BY`/`LIMIT`/`INTO` clauses within a query block while preserving `NTH_VALUE(... FROM FIRST\|LAST ... OVER ...)` modifiers and window-function `RESPECT NULLS`/`IGNORE NULLS` continuations, out-of-order top-level major clauses, removed `PROCEDURE ANALYSE` clause shapes, misplaced top-level `ON`/`USING` clauses while preserving nested outer-join condition chains, malformed top-level `WHERE`/`HAVING`/`QUALIFY` adjacent operands and dangling operators, malformed top-level `GROUP BY` dangling operators and repeated directions, malformed top-level `ORDER BY` adjacent operands and dangling operators, adjacent operands, dangling operators, and trailing separators inside plain parenthesized `WHERE`/`HAVING`/`QUALIFY`/`GROUP BY`/`ORDER BY` expression groups, malformed explicit and implicit SELECT-list aliases including pure numeric aliases, numeric-literal prefixes, quoted hex/bit literals, and charset-introduced string literals, empty `EXISTS()` predicates, `BETWEEN` without a lower bound before `AND`, malformed `LIMIT` comma/`OFFSET`, adjacent-operand, and non-integer numeric option tails, malformed named `WINDOW` clauses, trailing top-level comma separators, incomplete `UNION`/`INTERSECT`/`EXCEPT` set operations, repeated set-operation option tokens, malformed set-operation `SELECT` operands in parenthesized query contexts, malformed top-level parenthesized query suffixes including `ORDER BY` expression-list and `INTO` file-output tails, malformed `LOCK IN SHARE MODE` tails, malformed `FOR UPDATE`/`FOR SHARE` locking tails while preserving index-hint `FOR` forms, `INTO OUTFILE`/`INTO DUMPFILE` without file names, and stray file-output suffixes; full expression/query grammar remains incomplete. |
| `SELECT ... INTO var_list` | ❌ | high | User/local variable assignment semantics. | Parser validates comma-separated local/user-variable target lists and rejects missing user-variable names, dangling commas, and adjacent variable targets. |
| `SELECT ... INTO OUTFILE` | ❌ | medium | File export syntax and embedded-compatible diagnostics. | Parser requires a text-string file name with quoted hex/bit rejection and validates basic character-set names including quoted names with quoted hex/bit rejection, plus field and line option tails; export execution is not implemented. |
| `SELECT ... INTO DUMPFILE` | ❌ | medium | Binary file export syntax and embedded-compatible diagnostics. | Parser requires a text-string file name with quoted hex/bit rejection; export execution is not implemented. |
| `TABLE` | ❌ | medium | Table-value statement syntax and ordering/limit behavior. | Parser recognizes table references, set operators, `ORDER BY`, integer-or-identifier `LIMIT` forms with non-integer numeric rejection, `INTO` variable lists, text-string `INTO OUTFILE`/`INTO DUMPFILE` targets with quoted hex/bit rejection, `OUTFILE` charset names with quoted hex/bit rejection, field and line option tails, and malformed set-operation `SELECT` operands. |
| `UPDATE` (single-table) | ❌ | top | Assignment order, generated columns, ORDER BY, LIMIT, LOW_PRIORITY, and IGNORE. | Parser validates table aliases including required aliases after `AS`, comma-separated assignment lists, dotted assignment targets, nested expression values, rejects top-level assignment value adjacent operands and dangling operators, adjacent operands, dangling operators, and trailing separators inside plain parenthesized assignment/`WHERE`/`ORDER BY` expression groups, repeated `SET` continuations, stray top-level `FROM` continuations after assignment values, and incomplete, repeated, out-of-order, adjacent-operand, dangling-operator, or non-integer numeric `WHERE`/`ORDER BY`/`LIMIT` tails. |
| `UPDATE` (multi-table) | ❌ | high | Joined update semantics, assignment evaluation, and affected rows. | Parser recognizes joined and comma-separated table references before `SET`, validates table aliases including required aliases after `AS`, validates the same assignment-list and expression-tail checks as single-table `UPDATE`, and rejects single-table-only `ORDER BY`/`LIMIT` tails when they appear in malformed order. |
| `VALUES` | ❌ | high | Standalone values statement and row constructor behavior. | Parser recognizes non-empty comma-separated `VALUES ROW(...)` contents with adjacent-operand and dangling-operator row-expression checks, applies the same row-expression checks in CTAS, view, and DML set-operation `VALUES` bodies, preserves `UNION`/`INTERSECT`/`EXCEPT`, validates parenthesized `ORDER BY` expressions plus combined `ORDER BY`/`LIMIT` tails with non-integer numeric limit rejection, accepts top-level and CTE-body `INTO` variable lists plus text-string `OUTFILE`/`DUMPFILE` targets with quoted-hex/bit rejection in text and charset slots, and rejects malformed set-operation `SELECT` operands. |
...
| `XA RECOVER` | ❌ | low | XA recovery result-set metadata. | Parser recognizes bare `XA RECOVER` and `XA RECOVER CONVERT XID`. |
...
| `CHANGE REPLICATION SOURCE TO` | ❌ | low | Source connection/channel options and diagnostics. | Parser recognizes documented MySQL 8.4 source option names, typed numeric values including decimal-only heartbeat periods and decimal/lowercase-hex/quoted-hex retry, delay, port, compression-level, source SSL/public-key/auto-position, `REQUIRE_ROW_FORMAT`, and `IGNORE_SERVER_IDS` values, strict integer-or-hex `GTID_ONLY` and `SOURCE_CONNECTION_AUTO_FAILOVER` boolean values, text-string-or-`NULL` values with quoted hex/bit rejection, privilege-check users, fixed primary-key-check enums, GTID assignment values, text-string channel names, unsigned decimal source log positions, and MySQL's unsigned decimal plus lowercase-hex relay log-position forms. |
| `RESET REPLICA` | ❌ | low | Replica metadata reset syntax. | Parser recognizes `ALL`, text-string channel names with quoted hex/bit rejection, and comma-separated combinations with `RESET BINARY LOGS AND GTIDS` options. |
...
| `START REPLICA` | ❌ | low | Replica start syntax, channels, threads, and until conditions. | Parser recognizes `IO_THREAD`/`RELAY_THREAD` and `SQL_THREAD`, until, connection, and channel clauses with text-string log/GTID/user option values and channel names with quoted hex/bit rejection, source/relay log coordinate option lists in MySQL parser order, unsigned decimal source log positions, and MySQL's unsigned decimal plus lowercase-hex relay log-position forms. |
...
| `STOP REPLICA` | ❌ | low | Replica stop syntax and channel handling. | Parser recognizes `IO_THREAD`/`RELAY_THREAD` and `SQL_THREAD` plus text-string channel clauses with quoted hex/bit rejection. |
...
| `START GROUP_REPLICATION` | ❌ | low | Group Replication start syntax and user credentials. | Parser recognizes comma-separated text-string user, password, and default-auth options with quoted hex/bit rejection. |
| `STOP GROUP_REPLICATION` | ❌ | low | Group Replication stop syntax. | Parser recognizes the standalone statement and rejects trailing options. |
| `PREPARE` | ❌ | high | Prepare from literal or user variable, parameter marker rules, and errors. | Parser recognizes prepared-statement names, text-string sources with quoted hex/bit rejection, and user-variable sources. |
...
| `BEGIN ... END` | ❌ | medium | Compound statement block scope for stored programs and events. | Parser recognizes compound body starts and `END` tails in stored-program and event bodies, with nested statement tails preserved for later semantic analysis. |
| Statement labels | ❌ | medium | Label declaration, LEAVE/ITERATE binding, and duplicate-label diagnostics. | Parser recognizes unquoted `label:` and quoted identifier label declarations before `BEGIN`, `LOOP`, `REPEAT`, and `WHILE` forms, including trigger and handler bodies, plus quoted and unquoted `LEAVE`/`ITERATE` and end-label references; binding and duplicate-label semantics are not implemented. |
...
| `DECLARE ... CONDITION` | ❌ | medium | Named condition declarations. | Parser recognizes named conditions with unsigned decimal, lowercase-hex, and quoted-hex MySQL error codes or five-character `SQLSTATE [VALUE]` literals. |
...
| `DECLARE ... HANDLER` | ❌ | medium | CONTINUE/EXIT handler declarations for SQLSTATE, errors, warnings, and NOT FOUND. | Parser recognizes unsigned decimal, lowercase-hex, and quoted-hex MySQL error codes, named conditions, `SQLSTATE [VALUE]` literals, `SQLWARNING`, `NOT FOUND`, `SQLEXCEPTION`, and handler bodies starting with compound blocks, flow-control, cursor, DML, diagnostics, and return statements. |
...
| `LOOP` | ❌ | medium | Stored-program LOOP semantics. | Parser recognizes `LOOP` statement bodies, labels, and `END LOOP` tails. |
...
| `SIGNAL` | ❌ | medium | User-raised SQLSTATE and condition item semantics. | Parser recognizes five-character `SQLSTATE [VALUE]` literals and condition item assignments with MySQL-shaped simple `MYSQL_ERRNO` values, including unsigned numeric/string/hex/bit literals, local/user/system variables, and dotted identifiers while rejecting signed, `DEFAULT`, parenthesized, and operator expressions, with embedded stored-program bodies reusing `SET` assignment expression validation. |
| `RESIGNAL` | ❌ | medium | Handler rethrow and diagnostics mutation. | Parser recognizes five-character `SQLSTATE [VALUE]` literals and condition item assignments with MySQL-shaped simple `MYSQL_ERRNO` values, including unsigned numeric/string/hex/bit literals, local/user/system variables, and dotted identifiers while rejecting signed, `DEFAULT`, parenthesized, and operator expressions, with embedded stored-program bodies reusing `SET` assignment expression validation. |

### 1.4 Account, Resource, Plugin, Maintenance, SHOW, and Utility Statements

| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| `ALTER USER` | ❌ | medium | Authentication plugins, passwords, MFA, TLS, resources, lock/expire, comments, attributes, and default roles. | Parser recognizes account lists using shared unreserved identifier grammar, rejects standalone reserved `CASCADE`/`RESTRICT` principals while preserving quoted principals and hosts, authentication clauses with string-literal and numeric-start identifier plugin names, unsigned decimal MFA factor add/modify/drop numbers, WebAuthn registration clauses, repeatable TLS/resource/password/lock options with unsigned decimal/lowercase-hex resource limits and unsigned integer/lowercase-hex password policy counts, authentication/TLS/comment/attribute text-string values with quoted hex/bit rejection, default roles, `USER()`, and `DISCARD OLD PASSWORD`. |
| `CREATE USER` | ❌ | medium | User creation syntax, IF NOT EXISTS, auth factors, TLS, resources, password options, and comments. | Parser recognizes account lists using shared unreserved identifier grammar, rejects standalone reserved `CASCADE`/`RESTRICT` principals while preserving quoted principals and hosts, authentication clauses with string-literal and numeric-start identifier plugin names, multi-factor authentication chains, initial-authentication clauses, default roles, repeatable TLS/resource/password/lock options with unsigned decimal/lowercase-hex resource limits and unsigned integer/lowercase-hex password policy counts, and authentication/TLS/comment/attribute text-string values with quoted hex/bit rejection. |
| `CREATE ROLE` | ❌ | medium | Role creation syntax and metadata. | Parser recognizes role account names using shared unreserved identifier grammar and rejects standalone reserved `CASCADE`/`RESTRICT` principals. |
| `DROP USER` | ❌ | medium | User deletion syntax and privilege cleanup. | Parser recognizes `CURRENT_USER`/`CURRENT_USER()` and account references using shared unreserved identifier grammar, rejecting standalone reserved `CASCADE`/`RESTRICT` principals. |
| `DROP ROLE` | ❌ | medium | Role deletion syntax and grant cleanup. | Parser recognizes role account names using shared unreserved identifier grammar and rejects standalone reserved `CASCADE`/`RESTRICT` principals. |
| `GRANT` | ❌ | medium | Privilege and role grants, WITH GRANT OPTION, PROXY, dynamic privileges, and partial revoke semantics. | Parser recognizes recipient authentication/resource clauses with unsigned decimal/lowercase-hex resource limits, `WITH GRANT OPTION`, `WITH ADMIN OPTION`, and `AS user WITH ROLE ...` tails, with account principals using the shared reserved-name checks. |
| `RENAME USER` | ❌ | medium | User rename syntax and privilege metadata. | Parser recognizes account pairs using the shared account-reference grammar, including `CURRENT_USER` and `CURRENT_USER()`, and rejects standalone reserved `CASCADE`/`RESTRICT` principals. |
| `REVOKE` | ❌ | medium | Privilege and role revocation semantics. | Parser recognizes `IGNORE UNKNOWN USER`, proxy revocation shape, all/grant-option revocation tails, and shared account-principal reserved-name checks. |
...
| `SET PASSWORD` | ❌ | medium | Password assignment semantics. | Parser recognizes text-string and random password assignment, `CURRENT_USER` account references, text-string `REPLACE`, quoted hex/bit rejection, and `RETAIN CURRENT PASSWORD`. |
| `SET ROLE` | ❌ | medium | Active-role selection. | Parser recognizes `DEFAULT`, `NONE`, `ALL`, `ALL EXCEPT`, and explicit role lists. |
| `CREATE RESOURCE GROUP` | ❌ | low | Thread resource group creation syntax. | Parser recognizes `TYPE`, optional unsigned-integer VCPU ranges with MySQL's optional comma separators, optional-negative-integer thread priority, and enable/disable clauses. |
| `ALTER RESOURCE GROUP` | ❌ | low | Resource group modification syntax. | Parser recognizes unsigned-integer VCPU ranges with MySQL's optional comma separators, optional-negative-integer thread priority, enable, and `DISABLE [FORCE]` clauses. |
| `DROP RESOURCE GROUP` | ❌ | low | Resource group deletion syntax. | Parser recognizes the optional `FORCE` modifier. |
| `SET RESOURCE GROUP` | ❌ | low | Thread assignment to resource groups. | Parser recognizes optional unsigned integer/lowercase-hex `FOR` thread-id lists with MySQL's optional comma separators. |
| `ANALYZE TABLE` | ❌ | high | Statistics refresh, histogram update/drop, validation, and result-set metadata. | Parser recognizes table lists and histogram update/drop clauses after table lists, including unsigned-integer histogram bucket counts, automatic/manual histogram update modes, and text-string histogram data with quoted hex/bit rejection, using the shared identifier grammar for tables and columns. |
...
| `CHECKSUM TABLE` | ❌ | high | Table checksum syntax and result-set metadata. | Parser recognizes `TABLE`/`TABLES`, table lists, and optional `QUICK`/`EXTENDED` modifiers. |
...
| `INSTALL COMPONENT` | ❌ | low | Component installation syntax and diagnostics. | Parser recognizes text-string component file lists with quoted hex/bit rejection and optional `SET` assignments with global/persist scopes, and rejects malformed assignment values with adjacent operands, dangling operators, and invalid plain parenthesized groups. |
| `UNINSTALL COMPONENT` | ❌ | low | Component uninstallation syntax and diagnostics. | Parser recognizes comma-separated text-string component file lists with quoted hex/bit rejection. |
| `INSTALL PLUGIN` | ❌ | low | Plugin installation syntax and diagnostics. | Parser recognizes plugin names using the shared identifier grammar and required text-string `SONAME` file values with quoted hex/bit rejection. |
...
| `CLONE` | ❌ | low | Local and remote clone syntax and diagnostics. | Parser recognizes local clone and remote `CLONE INSTANCE` syntax, including unsigned decimal plus lowercase-hex remote ports, text-string passwords and data directories with quoted hex/bit rejection, and SSL clauses. |
| `SET` | ❌ | top | Variable assignment, user variables, system variables, persisted variables, names, charset, and transaction forms. | Parser recognizes comma-separated variable assignment lists with optional per-assignment scopes and nested value expressions, rejects malformed non-`@@` assignment values with adjacent operands, dangling operators, and invalid plain parenthesized groups, preserves `@@` system-variable values as token tails for MySQL server-variable mini-language cases, and rejects repeated top-level `SET` continuations after completed assignment values. |
...
| `CACHE INDEX` | ❌ | low | MyISAM key cache assignment syntax. | Parser recognizes table references, MySQL's single-table partition form including `ALL`, optional empty or named key/index lists, and key-cache names using the shared identifier grammar. |
| `FLUSH` | ❌ | medium | FLUSH variants for logs, tables, privileges, status, optimizer costs, and user resources, plus MySQL 8.4 diagnostics for removed forms. | Parser recognizes `LOCAL`/`NO_WRITE_TO_BINLOG` modifiers, table forms, simple option lists, binary/engine/error/general/relay/slow log forms, and channel-qualified relay logs. Removed `FLUSH HOSTS` syntax is permissive-corpus-only. |
| `KILL` | ❌ | medium | Connection/query kill syntax and diagnostics. | Parser recognizes `CONNECTION`/`QUERY` modes, expression-style targets including function-call tails, and exact user-variable targets, and rejects extra top-level tokens after the target. |
| `LOAD INDEX INTO CACHE` | ❌ | low | MyISAM index preload syntax. | Parser recognizes MySQL's single-table partition form including `ALL`, optional empty or named key/index lists, comma-separated non-partition table lists, and `IGNORE LEAVES`. |
...
| `RESET PERSIST` | ❌ | low | Persisted system variable reset syntax. | Parser recognizes no-target reset and one- and two-part persisted variable names using the shared identifier grammar, while rejecting wildcard targets that MySQL 8.4 does not parse. |
...
| `DESCRIBE` / `DESC` | ❌ | top | Table, column, and statement description semantics. | Parser recognizes table/column forms using the shared identifier grammar plus EXPLAIN-synonym variants, including unsigned integer-or-hex `FOR CONNECTION` ids with optional `FORMAT` clauses and validated explainable statement tails. |
| `EXPLAIN` | ❌ | high | Explain SELECT/TABLE/INSERT/UPDATE/DELETE, formats, ANALYZE, and FOR CONNECTION. | Parser recognizes table-description forms, `FORMAT=JSON INTO @var` with user-variable targets, `FOR SCHEMA`/`FOR DATABASE` schema specifiers, unsigned integer-or-hex `FOR CONNECTION` ids with optional `FORMAT` clauses, explicit `ANALYZE` statement starts, and validated explainable statement tails including `TABLE`. |
| `HELP` | ❌ | low | Server help lookup result-set semantics. | Parser recognizes required text-string, identifier, and unreserved keyword help topics while rejecting quoted hex/bit text values, bare help, and reserved-keyword topics. |
| `USE` | ❌ | top | Default schema selection in the embedded single-file model. | Parser recognizes one-part unquoted schema names using the shared identifier grammar and unreserved keyword names while rejecting string, numeric, variable, operator, and reserved-keyword targets including `CASCADE` and `RESTRICT`. |

### 1.5 SHOW Statements

Shared parser-level `SHOW ... LIKE` filters require text-string patterns with
quoted hex/bit rejection.
...
| `SHOW BINLOG EVENTS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional text-string log-file names with quoted hex/bit rejection, unsigned decimal/exponent `FROM` positions with malformed/sign/hex rejection, and integer-or-identifier `LIMIT` tails with non-integer numeric rejection. |
| `SHOW CHARACTER SET` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `SHOW CHARACTER SET` and `SHOW CHARSET` with `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW COLLATION` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW COLUMNS` / `SHOW FIELDS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `EXTENDED`, `FULL`, and `EXTENDED FULL`, `FROM`/`IN` table selectors, optional schema selectors, and `LIKE`/`WHERE` filters. |
| `SHOW COUNT(*) ERRORS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the exact `SHOW COUNT(*) ERRORS` shape and rejects other `COUNT` targets. |
| `SHOW COUNT(*) WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the exact `SHOW COUNT(*) WARNINGS` shape and rejects other `COUNT` targets. |
| `SHOW CREATE DATABASE` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `IF NOT EXISTS` for `DATABASE` and `SCHEMA`. |
| `SHOW CREATE EVENT` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part event names using the shared table-reference grammar. |
| `SHOW CREATE FUNCTION` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part function names using the shared table-reference grammar. |
| `SHOW CREATE PROCEDURE` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part procedure names using the shared table-reference grammar. |
| `SHOW CREATE TABLE` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part table names and rejects `IF NOT EXISTS` on table targets. |
| `SHOW CREATE TRIGGER` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part trigger names using the shared table-reference grammar. |
| `SHOW CREATE USER` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes account references using the shared account-reference grammar, including `CURRENT_USER` and `CURRENT_USER()`, and rejects standalone reserved `CASCADE`/`RESTRICT` principals. |
| `SHOW CREATE VIEW` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes one- and two-part view names using the shared table-reference grammar. |
| `SHOW DATABASES` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `SHOW DATABASES` and `SHOW SCHEMAS` with `LIKE` text-string filters and `WHERE` expression filters. |
...
| `SHOW ENGINES` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `SHOW ENGINES` and `SHOW STORAGE ENGINES` and rejects unsupported `LIKE`/`WHERE` filters. |
| `SHOW ERRORS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes integer-or-identifier `LIMIT` tails with non-integer numeric rejection. |
| `SHOW EVENTS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional schema selectors and `LIKE`/`WHERE` filters. |
...
| `SHOW FUNCTION STATUS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW GRANTS` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `FOR` account references, optional `USING` role lists, `CURRENT_USER` forms, and shared account-principal reserved-name checks. |
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | ❌ | top | Result-set shape, filtering, WHERE clauses, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `EXTENDED`, table/schema selectors, and `WHERE` filters while rejecting `LIKE` and `FULL` forms that MySQL does not parse. |
...
| `SHOW OPEN TABLES` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional schema selectors and `LIKE`/`WHERE` filters. |
| `SHOW PARSE_TREE` | ❌ | low | JSON parse-tree result for the MySQL 8.4.9 grammar when enabled; syntax error when built without `WITH_SHOW_PARSE_TREE`. | Accepted only in permissive corpus mode for now, matching the normal MySQL 8.4.9 runtime syntax error while keeping the conditional debug/development surface cataloged. |
| `SHOW PLUGINS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the standalone statement and rejects unsupported filters. |
| `SHOW PRIVILEGES` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the standalone statement and rejects unsupported filters. |
...
| `SHOW PROCEDURE STATUS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW PROCESSLIST` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes bare and `FULL` forms and rejects unsupported filters. |
| `SHOW PROFILE` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes profile type lists, unsigned-integer `FOR QUERY` ids, and integer-or-identifier `LIMIT` tails with non-integer numeric rejection. |
| `SHOW PROFILES` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the deprecated standalone statement and rejects unsupported `LIMIT`/filter tails. |
| `SHOW RELAYLOG EVENTS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional text-string log-file names with quoted hex/bit rejection, unsigned decimal/exponent `FROM` positions with malformed/sign/hex rejection, integer-or-identifier `LIMIT` tails with non-integer numeric rejection, and channel clauses. |
| `SHOW REPLICA STATUS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `FOR CHANNEL` text-string clauses with quoted hex/bit rejection. |
| `SHOW REPLICAS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes the standalone MySQL 8.4 statement and rejects unsupported filters. |
...
| `SHOW STATUS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `GLOBAL`/`LOCAL`/`SESSION` scopes plus `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW TABLE STATUS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional schema selectors and `LIKE`/`WHERE` filters. |
| `SHOW TABLES` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `EXTENDED`, `FULL`, and `EXTENDED FULL`, schema selectors, and `LIKE`/`WHERE` filters. |
...
| `SHOW VARIABLES` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional `GLOBAL`/`LOCAL`/`SESSION` scopes plus `LIKE` text-string filters and `WHERE` expression filters. |
| `SHOW WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes integer-or-identifier `LIMIT` tails with non-integer numeric rejection. |
...
| `WITH` common table expressions | ❌ | high | Non-recursive CTEs, column lists, name shadowing, and scope. | Parser recognizes CTE lists, requires each CTE body to begin with a query-expression form (`SELECT`, `TABLE`, `VALUES`, nested `WITH`, or parenthesized query), rejects DML bodies inside CTE definitions, routes main `SELECT`, `TABLE`, `VALUES`, parenthesized query, `UPDATE`, and `DELETE` bodies through their existing syntax validators, rejects invalid top-level `WITH ... INSERT`/`REPLACE` placement, and keeps `INSERT`/`REPLACE ... WITH ... SELECT` payloads valid, including EXPLAIN and cursor-declaration wrappers. |
| `WITH RECURSIVE` | ❌ | high | Recursive CTE execution, cycle behavior, limits, and type inference. | Parser recognizes `WITH RECURSIVE` CTE lists, column lists, recursive `UNION` query bodies, and nested parenthesized query forms using the same parser-level CTE body validation as non-recursive `WITH`; execution semantics are not implemented. |
...
| SELECT modifiers | ❌ | top | ALL, HIGH_PRIORITY, SQL_SMALL_RESULT, SQL_BIG_RESULT, SQL_BUFFER_RESULT, SQL_CALC_FOUND_ROWS, SQL_NO_CACHE, and STRAIGHT_JOIN. | Parser recognizes `SQL_NO_CACHE`, rejects removed `SQL_CACHE` modifier forms, and rejects incompatible `ALL`/`DISTINCT` combinations; modifier execution and deprecation warnings are not implemented. |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| Column definition grammar | ❌ | top | Type, nullability, defaults, visibility, comments, storage, format, references, and constraints. | `CREATE TABLE` and `ALTER TABLE ADD`/`CHANGE`/`MODIFY` validate type starts, common numeric and `ENUM`/`SET` type-parameter lists, MySQL's `DOUBLE PRECISION` modifier placement, required lengths for `VARCHAR`, `VARBINARY`, `NVARCHAR`, and `CHAR VARYING` forms, common attribute starts, closed `COLUMN_FORMAT`/`STORAGE` values, generated column heads (`AS (...)` or `GENERATED ALWAYS AS (...)`), reject repeated column `COLLATE` clauses, accept repeated column `COMMENT` attributes with all MySQL string-literal token classes, reject quoted hex/bit literals in text-string column attributes, restrict `CONSTRAINT` prefixes to `CHECK`, and reject `DEFAULT` as a column charset/collation value, require generated-column storage modifiers to follow generated expressions, reject repeated generated-column storage modifiers, validate parenthesized default/generated expression bodies for dangling operators and scalar comma lists, validate temporal precision lists for `DEFAULT`/`ON UPDATE` functions, reject stray attribute parentheses, and validate column-level `REFERENCES` envelopes including MySQL's optional parent column list. |
...
| Generated columns | ❌ | high | Virtual/stored generated columns, dependencies, indexes, and metadata. | Parser recognizes MySQL's generated-column heads (`AS (...)` and `GENERATED ALWAYS AS (...)`), rejects `GENERATED AS` and bare `ALWAYS AS`, and rejects bare, repeated, or conflicting `VIRTUAL`/`STORED` storage modifiers in `CREATE TABLE` and `ALTER TABLE ADD`/`CHANGE`/`MODIFY` column definitions. |
...
| Foreign keys | ❌ | high | Definition, matching, cascades, restrict/no action, set null/default, checks, and metadata. | Parser validates column/table foreign-key child/reference column-list envelopes, optional parent column lists, `MATCH` options, referential-action tails, MySQL's `MATCH`-before-actions order, and rejects duplicate `MATCH`, `ON UPDATE`, and `ON DELETE` clauses in `CREATE TABLE` and `ALTER TABLE ADD`/`MODIFY` definitions. |
| CHECK constraints | ❌ | high | Expression validation, enforcement, names, and metadata. | Parser validates column/table `CHECK` expression bodies for empty groups, dangling operators, and scalar comma lists, and recognizes `ENFORCED`/`NOT ENFORCED` tails. |
...
| `CHANGE COLUMN` | ❌ | top | Rename plus type/attribute change semantics. | Parser validates column-definition type starts for `ALTER TABLE ... CHANGE [COLUMN] old_name new_name ...`. |
| `MODIFY COLUMN` | ❌ | top | Type/attribute change without rename. | Parser validates column-definition type starts for `ALTER TABLE ... MODIFY [COLUMN] name ...`. |
...
| Partition maintenance | ❌ | low | ADD, DROP, DISCARD, IMPORT, TRUNCATE, COALESCE, REORGANIZE, EXCHANGE, ANALYZE, CHECK, OPTIMIZE, REBUILD, REPAIR, REMOVE PARTITIONING. | Parser recognizes MySQL-shaped ALTER partition maintenance verbs, binlog modifier placement, CHECK/REPAIR option tails, unsigned integer/lowercase-hex coalesce counts, concrete-name requirements for `DROP`, `EXCHANGE`, and `REORGANIZE`, `ALL` where MySQL accepts it for maintenance and tablespace transfer lists, and non-empty `ADD`/`REORGANIZE ... INTO (...)` partition-definition envelopes. |

### 3.3 Partitioning Surface

| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| `PARTITION BY RANGE` | ❌ | medium | Range partition syntax, VALUES LESS THAN, MAXVALUE, and pruning-visible metadata. | Parser recognizes range method expressions and partition definition lists, rejects invalid `LINEAR RANGE`, requires each explicit partition definition to include values, validates non-empty `VALUES LESS THAN (...)` expression lists, permits `VALUES LESS THAN MAXVALUE`, and rejects malformed/trailing partition-list envelopes. |
| `PARTITION BY RANGE COLUMNS` | ❌ | medium | Range columns syntax and comparison semantics. | Parser recognizes `RANGE COLUMNS (...)` and validates non-empty identifier-list and tuple value-list envelopes while rejecting expression, prefix-length, and direction forms. |
| `PARTITION BY LIST` | ❌ | medium | List partition syntax and value matching. | Parser recognizes list method expressions and partition definitions, rejects invalid `LINEAR LIST`, requires each explicit partition definition to include values, validates non-empty `VALUES IN (...)` expression lists, and rejects malformed/trailing partition-list envelopes. |
| `PARTITION BY LIST COLUMNS` | ❌ | medium | List columns syntax and tuple matching. | Parser recognizes `LIST COLUMNS (...)` and validates non-empty identifier-list and tuple value-list envelopes while rejecting expression, prefix-length, and direction forms. |
| `PARTITION BY HASH` | ❌ | medium | Hash partition syntax and function semantics. | Parser recognizes hash method expressions, validates non-empty parenthesized expressions and integer/lowercase-hex partition counts, and rejects `KEY`-only `ALGORITHM`/`COLUMNS` modifiers. |
...
| Subpartitioning | ❌ | low | HASH/KEY subpartition syntax and metadata. | Parser recognizes `SUBPARTITION BY HASH/KEY` method tails, rejects `RANGE`/`LIST` subpartition methods, validates integer/lowercase-hex `SUBPARTITIONS` counts, and validates explicit subpartition definition lists with non-empty comma-separated `SUBPARTITION name` entries plus partition-option value checks. |
| Partition options | ❌ | low | ENGINE, COMMENT, DATA/INDEX DIRECTORY, MAX_ROWS, MIN_ROWS, TABLESPACE, and nodegroup syntax. | Parser recognizes explicit partition options for `ENGINE`/`STORAGE ENGINE`, `COMMENT`, `DATA DIRECTORY`, `INDEX DIRECTORY`, `MAX_ROWS`, `MIN_ROWS`, `TABLESPACE`, and `NODEGROUP`, including optional equals where MySQL accepts it, string-literal text values with quoted hex/bit rejection, identifier/double-quoted tablespace values, and unsigned integer/lowercase-hex/quoted-hex row and nodegroup numbers. |
