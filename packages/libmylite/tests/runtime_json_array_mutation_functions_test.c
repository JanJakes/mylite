...
static int test_literal_json_array_mutation_values(void) {
    ...
    if (failures == 0) {
        failures +=
            expect_size(mylite_result_column_count(result), 1U, "json_array_append metadata");
        failures += expect_int(
        ...
        failures +=
            expect_int(mylite_result_column_nullable(result, 0U), 1, "json_array_append nullable");
        failures +=
...
static int test_dual_do_and_status(void) {
    ...
    if (failures == 0) {
        failures +=
            expect_size(mylite_result_column_count(result), 0U, "JSON array mutation do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "JSON array mutation do rows");
        failures +=
            expect_int64(mylite_result_affected_rows(result), 0, "JSON array mutation do affected");
        failures +=
            expect_size(mylite_result_warning_count(result), 0U, "JSON array mutation do warnings");
    }
    mylite_result_free(result);
    failures += expect_query(
        database,
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_json_array_mutation_%s_%d.mylite",
        name,
        current_process_id()
    );
