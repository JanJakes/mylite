...
| `SHOW VARIABLES` | 🟡 | top | Result-set shape, filtering, scope modifiers, system-variable values, and MySQL 8.4-compatible diagnostics. | First executable slice is implemented for `SHOW [GLOBAL\|SESSION\|LOCAL] VARIABLES [LIKE 'pattern' \| WHERE expr]`, including MySQL-compatible `Variable_name` / `Value` columns, `LOCAL` as a session synonym, deterministic variable-name ordering, case-insensitive `LIKE` filtering with backslash escapes, and a practical catalog covering connection charset/collation state, schema/server charset defaults, autocommit, transaction defaults, diagnostics counters/defaults, `sql_mode`, `sql_notes`, and version variables. `WHERE` is parsed but returns `SHOW VARIABLES WHERE is not supported` until shared SHOW filtering lands. Complete variable catalog, `SET` assignment, `SELECT @@...`, Performance Schema variable tables, persisted variables, and mutable global/session separation remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| Collation catalog entries | 🟡 | top | Expose every MySQL 8.4 collation through metadata, SHOW COLLATION, and charset/collation validation. | Internal registry validates and exposes `binary`, `latin1_swedish_ci`, `latin1_bin`, `utf8mb3_general_ci`, `utf8mb3_bin`, `utf8mb4_0900_ai_ci`, and `utf8mb4_bin` through `SHOW COLLATION` and the first wildcard-only `INFORMATION_SCHEMA.COLLATIONS` and `INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY` slices; all MySQL collations and full collation semantics are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SHOW COLLATION spec](docs/specs/show-collation/specs.md), [INFORMATION_SCHEMA.COLLATIONS spec](docs/specs/information-schema-collations/specs.md), and [INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY spec](docs/specs/information-schema-collation-character-set-applicability/specs.md). |
...
| `sql_mode` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes MySQL 8.4's default mode string for compatibility; mutable `SET sql_mode` and complete mode-sensitive semantics remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
