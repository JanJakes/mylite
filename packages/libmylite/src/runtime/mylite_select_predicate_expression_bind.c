...
static int bind_predicate_binary_expression(
    mylite_db *database,
...
static int bind_predicate_row_constructor(
    mylite_db *database,
...
static int bind_function_arguments(
    mylite_db *database,
    const struct mylite_sql_ast_node *expression,
    const struct mylite_select_plan *plan,
    const char *clause_context,
...
int mylite_select_bind_predicate_expression_in_clause(
    ...
    case MYLITE_SQL_AST_BINARY_EXPRESSION:
        return bind_predicate_binary_expression(
            database,
            expression,
            plan,
            clause_context,
    ...
    case MYLITE_SQL_AST_FUNCTION_CALL:
        return bind_function_arguments(
            database,
            expression,
            plan,
            clause_context,
    ...
    size_t table_count, const struct mylite_select_predicate_bind_callbacks *callbacks)
{
    if (mylite_select_subquery_binary_expression_is_row(expression)) {
        return bind_predicate_row_subquery_expression(
            database,
            expression,
            plan,
            clause_context,
    ...
    if (mylite_select_subquery_binary_expression_is_in(expression)) {
        return bind_predicate_in_subquery_expression(
            database,
            expression,
            plan,
            clause_context,
...
static int bind_predicate_in_subquery_expression(
    ...
    status = mylite_select_bind_predicate_expression_in_clause(
        database,
        left,
        plan,
        clause_context,
...
static int bind_predicate_row_subquery_expression(
    ...
    status = bind_predicate_row_constructor(
        database,
        left,
        plan,
        clause_context,
