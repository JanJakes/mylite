...
| `SELECT ... INTO var_list` | ❌ | high | User/local variable assignment semantics. | Parser validates a single top-level `INTO` clause, user/local variable-list separators, and records the first user/local variable target; assignment semantics, column-count checks, and multi-target metadata expansion are not implemented. |
| `SELECT ... INTO OUTFILE` | ❌ | medium | File export syntax and embedded-compatible diagnostics. | Parser validates the literal export-file target plus optional `CHARACTER SET`, `FIELDS` / `COLUMNS`, and `LINES` option shape, and records the literal export-file target; file I/O behavior is not implemented. |
| `SELECT ... INTO DUMPFILE` | ❌ | medium | Binary file export syntax and embedded-compatible diagnostics. | Parser validates the literal export-file target without OUTFILE formatting options and records the literal export-file target; binary file I/O behavior is not implemented. |
