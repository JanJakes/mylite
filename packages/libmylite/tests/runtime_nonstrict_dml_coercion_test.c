...
static int test_nonstrict_insert_replace_defaults_and_persistence(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "insert_replace") != 0) {
        return 1;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open insert replace file");
...
static int test_nonstrict_update_null_and_default(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "update") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_nonstrict_remaining_descriptor_families(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "family_defaults") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_nonstrict_insert_select_coercion(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "insert_select") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open insert select file");
    failures += seed_schema(database);
    failures += expect_statement_ok(
        database,
        "SET sql_mode=''",
...
static int test_nonstrict_string_truncation(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "string_truncation") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open string file");
    failures += seed_schema(database);
    failures += expect_statement_ok(
        database,
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen string file");
    failures += expect_statement_ok(
        database,
        "USE app",
        (struct expected_statement){0, 0U},
...
static int test_nonstrict_guardrails(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "guardrails") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open guardrails file");
    failures += seed_schema(database);
    failures += expect_statement_ok(
        database,
...
static int expect_single_bytes(
    ...
    failures += mylite_test_expect_size(actual_size, expected_size, context);
