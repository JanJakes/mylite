...
static int test_server_capability_values_and_show_rows(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open capability db");
...
static int test_server_capability_set_and_diagnostics(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open capability SET db");
