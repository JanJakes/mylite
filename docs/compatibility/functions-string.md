...
| `CONCAT()` | 🟡 | Limited scalar projection support with one or more arguments over string, integer, boolean, `NULL`, existing `DATABASE()` / `SCHEMA()` session values, and descriptor-backed integer, exact `DECIMAL`, string, baseline `TEXT` family, and temporal columns in no-source, `DUAL`, and single-table row-scalar `SELECT` forms; any `NULL` argument returns `NULL`. No `CONCAT_WS()`, binary-string result typing, non-ASCII collation semantics, approximate numeric formatting, table-backed use outside the limited row-scalar SELECT path, DML assignments, predicates, ordering expressions, subqueries, or expression metadata |
