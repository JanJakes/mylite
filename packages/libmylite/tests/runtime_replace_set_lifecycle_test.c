...
static int test_replace_set_success_persistence_rename_and_drop(void) {
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "update replaced row");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
    ...
    failures += mylite_test_expect_int(physical_rows, 4, "physical rows after replace set");
...
static int test_replace_set_schema_resolution_and_diagnostics(void) {
    ...
    failures += expect_replace_ok(database, "REPLACE INTO numbers SET id = 1 + 2, nn = 1");
    failures += execute_error(
        database,
        "REPLACE INTO numbers SET id = 'abc', nn = 1",
        (struct expected_sql_error){
    ...
    failures += expect_replace_ok(database, "REPLACE INTO numbers SET id = ABS(1), nn = 1");
    failures += execute_error(
        database,
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .column_count = 1U,
            .row_count = 3U,
            .context = "scalar expression replace set rows",
        }
    );
...
static int expect_replace_ok(mylite_db *database, const char *sql) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "replace set column count");
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "replace set row count");
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        1,
        "replace set affected rows"
    );
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "replace set warning count"
    );
    mylite_result_free(result);
