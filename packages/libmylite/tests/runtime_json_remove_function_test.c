...
static int test_literal_json_remove_values(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 1U, "json_remove metadata");
        failures += expect_int(
        ...
        failures +=
            expect_int(mylite_result_column_nullable(result, 0U), 1, "json_remove nullable");
...
static int test_dual_do_and_status(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "json_remove do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "json_remove do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "json_remove do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "json_remove do warnings");
    }
    mylite_result_free(result);
    failures += expect_query(
        database,
...
static int test_table_backed_json_remove_values(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen json_remove");
    failures += execute_ok(database, "USE app", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_json_remove_%s_%d.mylite",
        name,
        current_process_id()
    );
