...
static int test_insert_select_union_success_persistence_and_file_safety(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "insert_select_union") != 0) {
        return 1;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open union insert file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen union insert file");
    failures += expect_dml_ok(
        database,
...
static int test_insert_select_union_diagnostics_and_unsupported_forms(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "insert_select_union_diagnostics") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_insert_select_union_independent_handles(void) {
    ...
    if (mylite_test_make_path(first_path, sizeof(first_path), "insert_select_union_first") != 0 ||
        mylite_test_make_path(second_path, sizeof(second_path), "insert_select_union_second") !=
            0) {
        return 1;
    }
...
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected) {
    ...
    failures +=
        mylite_test_expect_int(mylite_errcode(database), expected.code, expected.message_part);
