...
static int test_information_schema_st_geometry_columns_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open st geometry db");
...
static int test_information_schema_st_geometry_columns_reopen_and_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first st geometry db"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second st geometry db"
    );
