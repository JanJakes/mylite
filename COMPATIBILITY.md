...
| `CREATE FUNCTION` (stored) | ❌ | medium | Stored-function definition, determinism, SQL data access, security, and body semantics. | Parser records the function target and validates optional definer and `IF NOT EXISTS`, parameter-list shape, required `RETURNS` type, documented routine characteristics, and nonempty body; stored-function metadata, execution, determinism checks, privileges, warnings, and diagnostics are not implemented. |
...
| `CREATE PROCEDURE` | ❌ | medium | Stored procedure parameters, body, characteristics, and diagnostics. | Parser records the procedure target and validates optional definer and `IF NOT EXISTS`, procedure parameter-list shape, documented routine characteristics, and nonempty body; stored-procedure metadata, execution, parameter semantics, privileges, warnings, and diagnostics are not implemented. |
...
| `TABLE` | ❌ | medium | Table-value statement syntax and ordering/limit behavior. | Parser records direct and parenthesized `TABLE` query-expression table targets, validates documented `ORDER BY`, `LIMIT`, `OFFSET`, final `INTO` variable-list and export-file clause shapes, and accepts direct set-operator tails; result rows, ordering, limit, field/line option behavior, export behavior, and assignment semantics are not implemented. |
