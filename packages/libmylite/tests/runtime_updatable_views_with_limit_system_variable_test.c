...
enum {
    ...
    updatable_views_with_limit_value_column_count = 6,
    updatable_views_with_limit_label_column_count = 5,
    updatable_views_with_limit_diagnostics_column_count = 4,
    updatable_views_with_limit_selected_column_count = 2,
    updatable_views_with_limit_independent_column_count = 3,
...
static int test_updatable_views_with_limit_values_and_persistence(void);
static int test_updatable_views_with_limit_qualifiers_and_errors(void);
static int test_independent_updatable_views_with_limit_handles(void);
...
int main(void) {
    ...
    failures += test_updatable_views_with_limit_values_and_persistence();
    failures += test_updatable_views_with_limit_qualifiers_and_errors();
    failures += test_independent_updatable_views_with_limit_handles();
...
static int test_updatable_views_with_limit_values_and_persistence(void) {
    ...
    static const char *const label_values[] = {"YES", "YES", "YES", "YES", "YES"};
    ...
    failures +=
        expect_int(mylite_open(path, &database), MYLITE_OK, "open updatable views with limit file");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "updatable views with limit values",
        }
    );
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = label_values,
            .count = updatable_views_with_limit_label_column_count,
            .context = "updatable views with limit labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@updatable_views_with_limit, @@foreign_key_checks, @@unique_checks, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed updatable views with limit scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = updatable_views_with_limit_diagnostics_column_count,
            .context = "updatable views with limit warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures +=
        expect_show_count_warnings(database, "0", "updatable views with limit clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = updatable_views_with_limit_diagnostics_column_count,
            .context = "updatable views with limit error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "updatable views with limit clears errors");
    failures += expect_show_count_warnings(
        database,
        "0",
        "updatable views with limit clears error warnings"
    );
    ...
    failures += expect_int64(
        ...
        "catalog generation unchanged by updatable views with limit reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by updatable views with limit reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read updatable views with limit preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after updatable views with limit reads"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@updatable_views_with_limit, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = updatable_views_with_limit_selected_column_count,
            .context = "updatable views with limit with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "updatable views with limit table DDL independence",
        }
    );
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "reopen updatable views with limit file"
    );
    failures += expect_query_result(
        database,
        "SELECT @@updatable_views_with_limit, @@global.updatable_views_with_limit",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened updatable views with limit values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened updatable views with limit table row",
        }
    );
...
static int test_updatable_views_with_limit_qualifiers_and_errors(void) {
    ...
    static const char *const scoped_values[] = {"YES", "YES", "YES", "YES", "YES"};
    mylite_db *database = NULL;
    mylite_result *result = NULL;
    int failures = 0;
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "open updatable views with limit memory"
    );
    failures += execute_ok(
        database,
        "SELECT @@UPDATABLE_VIEWS_WITH_LIMIT, @@SESSION.UPDATABLE_VIEWS_WITH_LIMIT, "
        "@@Local.Updatable_Views_With_Limit, @@global.`updatable_views_with_limit`, "
        "(@@updatable_views_with_limit)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = updatable_views_with_limit_label_column_count,
            .context = "updatable views with limit qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_updatable_views_with_limit_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_updatable_views_with_limit_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_updatable_views_with_limit_variable'",
...
static int test_independent_updatable_views_with_limit_handles(void) {
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "open first updatable views with limit handle"
    );
    failures += expect_int(
    ...
    failures += execute_ok(
        first,
        "SELECT @@updatable_views_with_limit, @@warning_count, @@error_count",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = updatable_views_with_limit_independent_column_count,
            .context = "first handle updatable views with limit variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = updatable_views_with_limit_independent_column_count,
            .context = "second handle updatable views with limit variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_updatable_views_with_limit_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
