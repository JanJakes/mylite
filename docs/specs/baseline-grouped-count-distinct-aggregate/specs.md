...
The planner resolves the aggregate argument against MyLite descriptors, verifies
that it is an integer descriptor column, and lowers the aggregate to SQLite as
`COUNT(DISTINCT "physical_column")`. SQLite owns source scanning, filtering,
grouping, distinct aggregation, `HAVING`, ordering, and limiting. MyLite owns
descriptor resolution, generated identifier quoting, parameter binding, result
metadata, diagnostics, and public result formatting.
