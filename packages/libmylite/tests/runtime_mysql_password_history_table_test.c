...
static int test_mysql_password_history_table(void);
...
static const char *const mysql_password_history_columns[mysql_password_history_column_count] = {
    "Host",
    "User",
    "Password_timestamp",
    "Password",
...
int main(void) {
    return test_mysql_password_history_table() == 0 ? 0 : 1;
}
...
static int test_mysql_password_history_table(void) {
    ...
    static const char *const show_full_columns_password_timestamp_values[] = {
        "Password_timestamp",
        "timestamp(6)",
        NULL,
        "NO",
    ...
    static const char *const key_column_usage_values[] = {
        "PRIMARY", "Host", "1",  NULL, NULL, NULL, NULL,      "PRIMARY",
        "User",    "2",    NULL, NULL, NULL, NULL, "PRIMARY", "Password_timestamp",
    ...
    static const char *const statistics_values[] = {
        "0",   "PRIMARY", "1",   "Host", "A",   "0",       NULL, NULL,
        "",    "BTREE",   "",    "",     "YES", NULL,      "0",  "PRIMARY",
        "2",   "User",    "A",   "0",    NULL,  NULL,      "",   "BTREE",
        "",    "",        "YES", NULL,   "0",   "PRIMARY", "3",  "Password_timestamp",
        "D",   "0",       NULL,  NULL,   "",    "BTREE",   "",   "",
        "YES", NULL,
    };
    ...
    if (make_test_path(path, sizeof(path), "mysql-password-history-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM mysql.password_history",
            .column_names = mysql_password_history_columns,
            .column_count = mysql_password_history_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.password_history direct empty read",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.password_history",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.password_history count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM password_history",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "selected mysql.password_history count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESC mysql.password_history",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = mysql_password_history_column_count,
            .context = "mysql.password_history describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.password_history LIKE 'Password_timestamp'",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_password_timestamp_values,
            .row_count = 1U,
            .context = "mysql.password_history show columns password timestamp",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.password_history WHERE Field = "
                   "'Password_timestamp'",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_password_timestamp_values,
            .row_count = 1U,
            .context = "mysql.password_history show full columns password timestamp",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.password_history",
            ...
            .context = "mysql.password_history show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_four,
            .row_count = 1U,
            .context = "mysql.password_history information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.password_history information_schema.columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.password_history information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.password_history information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.password_history information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_index_row_count,
            .context = "mysql.password_history information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.password_history information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'password_history'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.password_history show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_password_history_table_%d_%s.mylite",
        current_process_id(),
        name
    );
