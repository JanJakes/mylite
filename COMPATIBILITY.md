...
| `INFORMATION_SCHEMA.CHARACTER_SETS` | 🟡 | high | Available character sets | `SELECT * FROM INFORMATION_SCHEMA.CHARACTER_SETS` exposes the shared supported MyLite charset registry with uppercase MySQL columns and numeric `MAXLEN`, case-insensitive quoted table resolution, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. MyLite exposes only implemented charsets; projections, explicit duplicate modifiers, filters, ordering, limits, joins, aliases, aggregates, qualified wildcards, and full MySQL field metadata for information-schema SELECTs remain deferred. See [INFORMATION_SCHEMA.CHARACTER_SETS spec](docs/specs/information-schema-character-sets/specs.md). |
| `INFORMATION_SCHEMA.CHECK_CONSTRAINTS` | 🟡 | high | Table and column CHECK constraints | `SELECT * FROM INFORMATION_SCHEMA.CHECK_CONSTRAINTS` exposes the exact four-column MySQL table shape as a read-only system view, resolves qualified names case-insensitively including quoted forms, returns zero rows until MyLite has CHECK DDL/catalog/enforcement support, and appears in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. Actual CHECK rows, projections, explicit duplicate modifiers, filters, ordering, limits, joins, aliases, aggregates, qualified wildcards, privileges, and full MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.CHECK_CONSTRAINTS spec](docs/specs/information-schema-check-constraints/specs.md). |
| `INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY` | 🟡 | high | Character set applicable to each collation | `SELECT * FROM INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY` exposes the shared supported MyLite collation registry as `COLLATION_NAME` / `CHARACTER_SET_NAME`, with case-insensitive quoted table resolution and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. MyLite exposes only implemented collations; projections, explicit duplicate modifiers, filters, ordering, limits, joins, aliases, aggregates, qualified wildcards, and full MySQL field metadata for information-schema SELECTs remain deferred. See [INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY spec](docs/specs/information-schema-collation-character-set-applicability/specs.md). |
| `INFORMATION_SCHEMA.COLLATIONS` | 🟡 | high | Collations for each character set | `SELECT * FROM INFORMATION_SCHEMA.COLLATIONS` exposes the shared supported MyLite collation registry, including `utf8mb4_unicode_520_ci`, with uppercase MySQL columns, numeric `ID` and `SORTLEN`, case-insensitive quoted table resolution, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. MyLite exposes only implemented collations; projections, explicit duplicate modifiers, filters, ordering, limits, joins, aliases, aggregates, qualified wildcards, and full MySQL field metadata for information-schema SELECTs remain deferred. See [INFORMATION_SCHEMA.COLLATIONS spec](docs/specs/information-schema-collations/specs.md) and [utf8mb4 unicode 520 collation spec](docs/specs/utf8mb4-unicode-520-ci/specs.md). |
...
| `INFORMATION_SCHEMA.ENGINES` | 🟡 | high | Storage engine properties | `SELECT * FROM INFORMATION_SCHEMA.ENGINES` exposes the shared MyLite storage-engine registry with uppercase MySQL columns, `InnoDB` as the default supported SQLite-backed facade, unsupported common engines as `SUPPORT=NO`, nullable capability columns, and case-insensitive quoted table resolution. Projections, filters, ordering, limits, joins, aliases, aggregates, full MySQL field metadata for information-schema SELECTs, and the full build-dependent MySQL engine catalog remain deferred. See [INFORMATION_SCHEMA.ENGINES spec](docs/specs/information-schema-engines/specs.md). |
...
| `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` | 🟡 | high | Which tables have constraints | `SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS` exposes primary-key and unique constraints derived from `__mylite_index_catalog`, with exact seven-column shape, deterministic row ordering, case-insensitive quoted table resolution, DDL side effects for supported key metadata, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. CHECK and actual foreign-key constraint rows, projections, explicit duplicate modifiers, filters, ordering, limits, joins, aliases, aggregates, qualified wildcards, privileges, and full MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md) and [INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS spec](docs/specs/information-schema-referential-constraints/specs.md). |
...
| `INFORMATION_SCHEMA.TABLES` | 🟡 | top | Table information | `SELECT * FROM INFORMATION_SCHEMA.TABLES` exposes the MySQL 8.4.9 column shape, scoped information-schema system-view rows, and the internal table catalog with `ENGINE='InnoDB'` for supported user base tables and `ENGINE IS NULL` for information-schema views. A focused query-shape slice supports simple projections, optional aliases and qualified references, `TABLE_SCHEMA`/`TABLE_NAME`/`ENGINE` equality and null filters, `DATABASE()`, conjunctions, and single-column ordering for common engine-filtering probes. Broader projections, joins, aggregates, arbitrary predicates, privileges, and full table metadata are deferred. See [core metadata catalog spec](docs/specs/core-metadata-catalog/specs.md) and [INFORMATION_SCHEMA.TABLES engine filter spec](docs/specs/information-schema-tables-engine-filter/specs.md). |
