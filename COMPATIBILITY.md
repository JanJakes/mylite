...
| `INSERT IGNORE` | ❌ | top | Duplicate, conversion, and constraint warning demotion rules. | Started; scoped to the currently supported `INSERT ... VALUES` and `INSERT ... SET` surfaces, with design, MySQL 8.4.9 probes, warning/diagnostic handoff, affected-row, last-insert-id, auto-increment, and atomicity requirements captured in the [INSERT IGNORE spec](docs/specs/insert-ignore/specs.md). |
