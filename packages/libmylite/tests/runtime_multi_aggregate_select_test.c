...
static int test_multi_aggregate_values(void) {
    ...
    static const char *const count_values[] = {"4", "3", "3"};
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*), COUNT(n), COUNT(DISTINCT n) FROM t",
...
static int seed_database(mylite_db *database) {
    ...
    failures += execute_ok(
        database,
        "CREATE TABLE t(id INT NOT NULL, n INT NULL, m INT NOT NULL, label VARCHAR(20) NULL)",
        &result
    );
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(
        database,
        "INSERT INTO t VALUES (1, NULL, 5, 'a'), (2, 10, 7, 'b'), "
        "(3, 20, 9, 'c'), (4, 30, 11, NULL)",
        &result
    );
    mylite_result_free(result);
    return failures;
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_multi_aggregate_select_%d_%s.mylite",
        current_process_id(),
        name
    );
