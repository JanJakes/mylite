...
static int copy_insert_update_assignment(
    ...
    status = mylite_dml_copy_insert_column_reference(
        mylite_ast_child_at(assignment, 0U),
        &insert_assignment.target
    );
    if (status == MYLITE_OK) {
        status = mylite_dml_copy_insert_value(
