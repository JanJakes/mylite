...
| `>` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with decimal integer or `TRUE`/`FALSE` right operands only; specified for upcoming limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
...
| `<=` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with decimal integer or `TRUE`/`FALSE` right operands only; specified for upcoming limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
| `<=>` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with non-`NULL` decimal integer or `TRUE`/`FALSE` right operands only; specified for upcoming limited no-source/`DUAL` signed-64 scalar NULL-safe equality projection. | [operators](docs/compatibility/operators.md) |
...
| `=` (comparison) | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with non-`NULL` decimal integer or `TRUE`/`FALSE` right operands only; specified for upcoming limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
