# Baseline ISNULL Function Tasks

- [x] Read current compatibility, scalar select, literal projection, `IF()`,
  `IFNULL()`, `COALESCE()`, `NULLIF()`, parser, runtime, result, storage, and
  test context.
- [x] Research official MySQL 8.4 `ISNULL()` / comparison-function
  documentation.
- [x] Probe MySQL 8.4.9 runtime behavior for argument count, `NULL` and
  non-`NULL` operands, booleans, integer values, labels, nested calls, aliases,
  identifiers, `FROM DUAL`, warnings, row count, syntax errors, and broader
  deferred forms.
...
- [x] Add lexer/parser/AST support and parser tests for `ISNULL()`.
- [x] Extend the no-source/`FROM DUAL` scalar projection runtime to evaluate
  supported `ISNULL()` expressions without SQLite SQL.
