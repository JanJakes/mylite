...
static int test_information_schema_mysql_system_constraints(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "information-schema-mysql-system-constraints") !=
        0) {
        return 1;
    }
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE "
                   "TABLE_SCHEMA = 'mysql' AND TABLE_NAME = "
            ...
            .row_count = 1U,
            .context =
                "unsupported mysql replication_group_configuration_version constraints omitted",
        }
    );
