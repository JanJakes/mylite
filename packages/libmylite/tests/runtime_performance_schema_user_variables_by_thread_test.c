...
static int test_performance_schema_user_variables_by_thread(void);
...
int main(void) {
    return test_performance_schema_user_variables_by_thread() == 0 ? 0 : 1;
}

static int test_performance_schema_user_variables_by_thread(void) {
    static const char *const count_column[] = {"COUNT(*)"};
    static const char *const value_column[] = {"VARIABLE_VALUE"};
    static const char *const name_value_columns[] = {"VARIABLE_NAME", "VARIABLE_VALUE"};
    static const char *const user_variable_columns[] = {
        "THREAD_ID",
        "VARIABLE_NAME",
        "VARIABLE_VALUE",
    };
    ...
    static const char *const four_count[] = {"4"};
    static const char *const user_variables_rows[] = {
        NULL,
        "dec",
        "1.25",
        NULL,
    ...
    static const char *const preserved_display_name_value[] = {"MixedName", "B"};
    static const char *const selected_schema_value[] = {"42"};
    ...
    static const char *const information_schema_columns[] = {
        "THREAD_ID",      "1", "NO",  "bigint unsigned", "PRI", NULL, NULL,
        "VARIABLE_NAME",  "2", "NO",  "varchar(64)",     "PRI", NULL, "utf8mb4_0900_ai_ci",
        "VARIABLE_VALUE", "3", "YES", "longblob",        "",    NULL, NULL,
    };
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM performance_schema.user_variables_by_thread",
            .column_names = count_column,
            .values = zero_count,
            .column_count = 1U,
            .row_count = 1U,
            .context = "initial user_variables_by_thread row count",
        }
    );
    failures +=
        execute_ok(database, "SET @MixedName = 'A', @nullish = NULL, @num = 42, @dec = 1.25");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM performance_schema.user_variables_by_thread",
            ...
            .column_count = 1U,
            .row_count = 1U,
            .context = "assigned user_variables_by_thread row count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT THREAD_ID, VARIABLE_NAME, VARIABLE_VALUE "
                   "FROM performance_schema.user_variables_by_thread "
                   "ORDER BY VARIABLE_NAME",
            .column_names = user_variable_columns,
            .values = user_variables_rows,
            .column_count = user_variables_column_count,
            .row_count = 4U,
            .context = "user variable rows",
        }
    );
    failures += execute_ok(database, "SET @mixedname = 'B'");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT VARIABLE_NAME, VARIABLE_VALUE "
                   "FROM performance_schema.user_variables_by_thread "
                   "WHERE VARIABLE_NAME = 'MixedName'",
            .column_names = name_value_columns,
            .values = preserved_display_name_value,
            .column_count = 2U,
            .row_count = 1U,
            .context = "preserved user variable display name",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM performance_schema.user_variables_by_thread",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM performance_schema.user_variables_by_thread",
    ...
    failures += execute_error(
        database,
        "UPDATE performance_schema.user_variables_by_thread SET VARIABLE_VALUE = 'x'",
        access_denied
    );
        ...
        if (query.values != NULL) {
            for (size_t row = 0U; row < query.row_count; ++row) {
                for (size_t column = 0U; column < query.column_count; ++column) {
                    ...
                    if (query.values[value_index] != NULL) {
                        failures += mylite_test_expect_text(
                            mylite_result_value_text(result, row, column),
                            query.values[value_index],
                            query.context
                        );
                    } else if (column == 0U && query.column_count == user_variables_column_count) {
                        failures += mylite_test_expect_int(
                            mylite_result_value_text(result, row, column) != NULL,
                            1,
                            query.context
                        );
                    } else {
