...
| `sql_generate_invisible_primary_key` | 🟡 | Limited read-only scalar `SELECT @@sql_generate_invisible_primary_key` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; no `SET`, mutable global/session state, hidden `my_row_id` columns, generated primary keys, invisible-column metadata, auto-increment, changed `CREATE TABLE` behavior, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_log_bin` | 🟡 | Limited read-only scalar `SELECT @@sql_log_bin` with no scope, `session`, or `local`; returns MyLite's fixed enabled value `1`; `global` scope is rejected as session-only; no `SET`, mutable session state, binary log files, GTID behavior, replication side effects, privilege checks, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_log_off` | 🟡 | Limited read-only scalar `SELECT @@sql_log_off` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; no `SET`, mutable global/session state, general query log files, `mysql.general_log`, slow query logging, privilege checks, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_mode` | 🟡 | Limited read-only scalar `SELECT @@sql_mode` with no scope, `session`, `local`, or `global`; returns MySQL 8.4.9's default mode string `ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION`; no `SET`, mutable global/session state, mode-dependent parsing, mode-dependent coercion, changed statement behavior, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_notes` | 🟡 | Limited read-only scalar `SELECT @@sql_notes` with no scope, `session`, `local`, or `global`; returns MyLite's fixed enabled value `1`; note-level diagnostics are currently produced only by limited table/schema-existence DDL no-ops; no `SET`, mutable global/session state, note suppression, `max_error_count`, changed `SHOW WARNINGS` / `@@warning_count` behavior beyond the documented missing-schema drop exception, `SHOW VARIABLES`, or Performance Schema variable tables |
...
| `sql_require_primary_key` | 🟡 | Limited read-only scalar `SELECT @@sql_require_primary_key` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled primary-key-requirement value `0`; no `SET`, mutable global/session state, primary-key constraints, DDL enforcement, table import behavior, replication policy, privilege semantics, `SHOW VARIABLES`, or Performance Schema variable tables |
