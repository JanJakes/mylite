...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `CREATE TABLE` | 🟡 | Limited persistent base-table creation with optional `IF NOT EXISTS`, explicit integer-family columns (`TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`/`INTEGER`, `BIGINT`) and the `INT1`/`INT2`/`INT3`/`INT4`/`INT8` plus `BOOL`/`BOOLEAN` aliases, optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases, `NULL`/`NOT NULL`, optional `DEFAULT NULL` for nullable columns, optional explicit `ENGINE [=] InnoDB`, and optional fixed default `utf8mb4` / `utf8mb4_0900_ai_ci` table charset/collation options; display width emits warning 1681 and only signed `TINYINT(1)`/`INT1(1)` plus `BOOL`/`BOOLEAN` persist as `tinyint(1)` metadata; existing-table `IF NOT EXISTS` is a no-op with a MySQL-compatible note; no other options, keys, non-`NULL` defaults, expression defaults, or temporary tables. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| Column definition grammar | 🟡 | Limited integer column descriptors, `INT1`/`INT2`/`INT3`/`INT4`/`INT8` aliases, bare `BOOL`/`BOOLEAN` aliases, optional unsigned decimal display width before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases, nullability, and optional trailing `DEFAULT NULL` only. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
| Default expressions | 🟡 | Explicit `DEFAULT NULL` only in the supported nullable integer-family column-definition paths; no non-`NULL` literal defaults, expression defaults, default metadata, `ALTER COLUMN SET/DROP DEFAULT`, or DML `DEFAULT` keyword values. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| `ADD COLUMN` | 🟡 | Limited append-only persistent base-table `ALTER TABLE ... ADD [COLUMN]` for one integer-family column, `INT1`/`INT2`/`INT3`/`INT4`/`INT8` alias, or bare `BOOL`/`BOOLEAN` alias with optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases, optional nullability, and optional `DEFAULT NULL` for nullable columns; existing nullable rows backfill `NULL`, existing `NOT NULL` integer rows backfill `0`; no non-`NULL` defaults, positioning, multiple actions, non-integer types, keys, constraints, temporary tables, views, algorithms, locks, or privilege semantics. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| `CHANGE COLUMN` | 🟡 | Limited single-action persistent base-table `ALTER TABLE ... CHANGE [COLUMN] old_col new_col integer_type [NULL\|NOT NULL] [DEFAULT NULL]`; preserves column id and ordinal, supports integer-family and `INT1`/`INT2`/`INT3`/`INT4`/`INT8` plus bare `BOOL`/`BOOLEAN` alias name/type/nullability replacement with optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases and case-only spelling changes, treats omitted nullability as nullable, accepts `DEFAULT NULL` only for nullable definitions, validates existing integer/`NULL` rows, keeps display-width-only and bool-alias range-equivalent changes metadata-only except for physical column renames, rebuilds the physical table when needed, and reports MySQL-compatible affected-row counts for the tested subset; rebuilds currently require at least one unshadowed SQLite rowid alias; no non-`NULL` defaults, positioning, non-integer types, multiple actions, table-qualified column names, algorithms, locks, temporary tables, views, or privilege semantics. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
| `MODIFY COLUMN` | 🟡 | Limited single-action persistent base-table `ALTER TABLE ... MODIFY [COLUMN] column_name integer_type [NULL\|NOT NULL] [DEFAULT NULL]`; preserves column id and ordinal, supports integer-family and `INT1`/`INT2`/`INT3`/`INT4`/`INT8` plus bare `BOOL`/`BOOLEAN` alias type/nullability replacement with optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases and case-only spelling changes, accepts `DEFAULT NULL` only for nullable definitions, validates existing integer/`NULL` rows, keeps display-width-only and bool-alias range-equivalent changes metadata-only, rebuilds the physical table when needed, and reports MySQL-compatible affected-row counts for the tested subset; rebuilds currently require at least one unshadowed SQLite rowid alias; no non-`NULL` defaults, positioning, non-integer types, multiple actions, table-qualified column names, algorithms, locks, temporary tables, views, or privilege semantics. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| `BIGINT` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, deprecated display width `0..255`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion, and single-column sort support; display widths normalize away with warning 1681; `BIGINT UNSIGNED` is capped at the signed 64-bit SQLite integer range in this slice; no `ZEROFILL`, repeated/combined attributes, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| Integer type aliases | 🟡 | Limited `INT1`, `INT2`, `INT3`, `INT4`, and `INT8` aliases normalize to existing integer-family descriptors, with optional single `SIGNED` or `UNSIGNED`, deprecated display width `0..255`, descriptor-driven DDL/DML/conversion/introspection coverage, and normalized type rendering; only signed `INT1(1)` persists/renders as `tinyint(1)`, other alias widths normalize away with warning 1681; no `SERIAL`, `ZEROFILL`, combined/repeated attributes, casts, or protocol-grade metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
...
| `INSERT ... VALUES` | 🟡 | Limited single- and multi-row inserts into persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, strict range checks, affected rows, and statement atomicity; no defaults, keys, generated values, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `SHOW COLUMNS` | 🟡 | Limited descriptor-driven column listing for persistent base tables with integer-family descriptors; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE` filters, and reports SQL `NULL` default values for nullable/no-default columns, but no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, indexes, non-`NULL` defaults, expression defaults, or hidden columns. | [SQL SHOW statements](docs/compatibility/sql-show-statements.md) |
