...
static int test_global_resource_values_and_show_rows(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open resource db");
...
static int test_global_resource_set_and_diagnostics(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open resource SET db");
