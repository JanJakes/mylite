...
3. Parser and AST
   - [x] Extend the existing count-literal grammar for bare `TRUE` and `FALSE`,
   ...
   - [x] Add parser tests for supported count-boolean syntax and unsupported
     argument/spacing forms.

4. Analyzer/planner/runtime execution
   - [x] Extend the count aggregate select path for `COUNT(TRUE)` and
...
5. Physical SQLite aggregate
   ...
   - [x] Bind the count-boolean parameter and predicate values through
     prepared statements.
