...
| `CREATE INDEX` | ❌ | top | Standalone index creation over MySQL index types and attributes. | Parser recognizes non-empty functional/key-part lists, validates key-part prefix lengths and optional `ASC`/`DESC` tails, and recognizes index options for parser plugins, string-literal comments, visibility, string-literal engine attributes, numeric `KEY_BLOCK_SIZE`, closed `USING`/`TYPE` values, and identifier-valued `ALGORITHM`/`LOCK` options. |
