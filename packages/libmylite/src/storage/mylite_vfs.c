...
static int mylite_vfs_open(sqlite3_vfs *vfs, sqlite3_filename name, sqlite3_file *file, int flags,
                           int *out_flags);
...
// SQLite's VFS callback signature fixes the order and types of these parameters.
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
static int mylite_file_write(sqlite3_file *file, const void *buffer, int amount,
...
static int mylite_file_shm_map(sqlite3_file *file, int page, int page_size, int extend,
...
// SQLite's VFS callback signature fixes the order and types of these parameters.
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
static int mylite_file_fetch(sqlite3_file *file, sqlite3_int64 offset, int amount,
                             void **out_mapping);
...
static int translate_offset(struct mylite_vfs_file *file, sqlite3_int64 offset,
                            sqlite3_int64 *out_offset);
static int translate_file_size(struct mylite_vfs_file *file, sqlite3_int64 size,
...
static int mylite_vfs_full_pathname(sqlite3_vfs *vfs, const char *name, int output_size,
                                    char *output);
...
static int mylite_file_read(sqlite3_file *file, void *buffer, int amount, sqlite3_int64 offset)
{
    ...
    return mylite_file->real_methods->xRead(mylite_file->real_file, buffer, amount,
                                            physical_offset);
}
                             ...
                             sqlite3_int64 offset)
{
    ...
    return mylite_file->real_methods->xWrite(mylite_file->real_file, buffer, amount,
                                             physical_offset);
}
...
static int mylite_file_control(sqlite3_file *file, int opcode, void *arg)
{
    ...
    if (opcode == SQLITE_FCNTL_SIZE_HINT && mylite_file->payload_offset > 0) {
        ...
        return mylite_file->real_methods->xFileControl(mylite_file->real_file, opcode,
                                                       &physical_size);
    }
                               ...
                               void volatile **out_map)
{
    ...
    return mylite_file->real_methods->xShmMap(mylite_file->real_file, page, page_size, extend,
                                              out_map);
}
                             ...
                             void **out_mapping)
{
    ...
    return mylite_file->real_methods->xFetch(mylite_file->real_file, physical_offset, amount,
                                             out_mapping);
}
