...
| Unique checks system variable | 🟡 | Limited scalar reads for `@@unique_checks` expose MyLite's fixed enabled unique-checking baseline `1`; limited fixed no-op `SET` forms may preserve `1`; no mutable checking state, unique index DDL, duplicate-key enforcement, index metadata, optimizer effects, or import optimizations. | [runtime system variables](docs/compatibility/runtime-system-variables.md), [SQL indexes and constraints](docs/compatibility/sql-indexes-constraints.md) |
...
| SQL require primary key system variable | 🟡 | Limited scalar reads for `@@sql_require_primary_key` expose MyLite's fixed disabled primary-key-requirement baseline `0`; limited fixed no-op `SET` forms may preserve `0`; no mutable state, primary-key constraints, DDL enforcement, replication policy, or privilege semantics. | [runtime system variables](docs/compatibility/runtime-system-variables.md), [SQL table DDL](docs/compatibility/sql-table-ddl.md), [SQL replication](docs/compatibility/sql-replication.md) |
...
| Table charset and collation options | 🟡 | Optional fixed default `CHARSET` / `CHARACTER SET utf8mb4` and `COLLATE utf8mb4_0900_ai_ci` options for the limited persistent `CREATE TABLE` subset plus fixed metadata no-op `ALTER TABLE ... [DEFAULT] CHARSET` / `CHARACTER SET utf8mb4` and `ALTER TABLE ... [DEFAULT] COLLATE utf8mb4_0900_ai_ci` for persistent base tables, with matching static `SHOW CHARACTER SET` / `SHOW COLLATION` rows; no non-default charsets/collations, descriptor metadata, string semantics, conversion, or full charset/collation catalogs. | [SQL table DDL](docs/compatibility/sql-table-ddl.md), [collations](docs/compatibility/collations.md) |
| `ADD COLUMN` | 🟡 | Limited append-only persistent base-table `ALTER TABLE ... ADD [COLUMN]` for one integer-family column, `INT1`/`INT2`/`INT3`/`INT4`/`INT8` alias, or bare `BOOL`/`BOOLEAN` alias with optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED` for integer-family and `INT*` aliases, optional nullability, and optional `DEFAULT NULL` or in-range decimal integer/`TRUE`/`FALSE` default; existing rows backfill descriptor defaults, nullable no-default columns backfill `NULL`, and not-null no-default columns backfill `0`; no positioning, multiple actions, non-integer types, keys, constraints, temporary tables, views, algorithms, locks, or privilege semantics. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| `ALTER COLUMN SET DEFAULT` | 🟡 | Limited catalog-only persistent base-table `ALTER TABLE table_name ALTER [COLUMN] column_name SET DEFAULT value` for one unqualified integer-family descriptor column, where `value` is `NULL` for nullable columns or an in-range decimal integer/`TRUE`/`FALSE` literal; existing rows and physical SQLite schema are unchanged, future omitted-column inserts and introspection use the new descriptor default; no expression/string/decimal/float/hex/bit defaults, table-qualified columns, multiple actions, temporary tables, views, algorithms, locks, or privilege semantics. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| `utf8mb4` character set | 🟡 | Limited static `SHOW CHARACTER SET` row, fixed `CREATE TABLE` option acceptance, fixed metadata no-op `ALTER TABLE ... [DEFAULT] CHARSET` / `CHARACTER SET utf8mb4`, scalar connection charset variable reads, scalar `@@character_set_server`, scalar `@@character_set_database`, and fixed no-op `SET NAMES` / `SET CHARACTER SET` forms only; no conversions, string types, mutable connection/server/database/table charset state, or full catalog semantics. | [character sets](docs/compatibility/character-sets.md) |
...
| `>` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with decimal integer or `TRUE`/`FALSE` right operands only; limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
...
| `<=` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with decimal integer or `TRUE`/`FALSE` right operands only; limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
| `<=>` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with non-`NULL` decimal integer or `TRUE`/`FALSE` right operands only; limited no-source/`DUAL` signed-64 scalar NULL-safe equality projection. | [operators](docs/compatibility/operators.md) |
...
| `=` (assignment) | 🟡 | One unqualified descriptor-column single-table `UPDATE` assignment to a supported decimal integer literal, `TRUE`, `FALSE`, `NULL`, or descriptor-resolved `DEFAULT`; limited fixed no-op system-variable `SET` assignments; expression assignments remain unsupported. | [operators](docs/compatibility/operators.md), [SQL SET statements](docs/compatibility/sql-set-statements.md), [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `=` (comparison) | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates with non-`NULL` decimal integer or `TRUE`/`FALSE` right operands only; limited no-source/`DUAL` signed-64 scalar comparison projection; no table-backed expression support. | [operators](docs/compatibility/operators.md) |
...
| `IS NOT NULL` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates; limited no-source/`DUAL` scalar projection over admitted signed-64 scalar operands. | [operators](docs/compatibility/operators.md) |
| `IS NULL` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates; limited no-source/`DUAL` scalar projection over admitted signed-64 scalar operands. | [operators](docs/compatibility/operators.md) |
...
| `INSERT ... VALUES` | 🟡 | Limited single- and multi-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [IGNORE] [INTO] table ... VALUES ...` into persistent base tables with integer/`NULL`/`TRUE`/`FALSE`/`DEFAULT` values, descriptor column resolution, no-column-list mapping to visible descriptor columns only, explicit column-list access to invisible columns, omitted-column and explicit-`DEFAULT` descriptor default filling, strict range/nullability/no-default checks, limited `IGNORE` warning demotion for omitted no-default, explicit `DEFAULT` for no-default, `NULL` into `NOT NULL`, and integer range clipping, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no keys, generated values, `DEFAULT(col_name)`, duplicate-key discard, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SET` | 🟡 | Limited one-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [IGNORE] [INTO] table SET column = integer_or_NULL_or_boolean_or_DEFAULT[, ...]` into persistent base tables with descriptor assignment resolution including explicitly named invisible columns, omitted-column and explicit-`DEFAULT` descriptor default filling, strict required-column/range/nullability diagnostics, limited `IGNORE` warning demotion for omitted no-default, explicit `DEFAULT` for no-default, `NULL` into `NOT NULL`, and integer range clipping, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no qualified assignment targets, expressions, `DEFAULT(col_name)`, generated values, duplicate-key discard, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SELECT` | 🟡 | Limited descriptor-backed `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [INTO] target [(columns)] SELECT ... FROM source` over persistent base tables, reusing the existing source `SELECT` column/alias/WHERE/ORDER/LIMIT subset plus simple no-op source locking clauses, and descriptor-driven target defaults/nullability/range checks, with no-op priority modifiers and delayed deprecation warning; no `IGNORE`, partitions, joins, expressions, literal projection sources, `TABLE`, row constructors, `ON DUPLICATE KEY UPDATE`, keys, auto-increment, triggers, locking options, or warning demotion. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... ON DUPLICATE KEY UPDATE` | ❌ | Duplicate-key update semantics. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT IGNORE` | 🟡 | Limited `INSERT IGNORE ... VALUES` and `INSERT IGNORE ... SET` warning demotion for descriptor integer/`NULL`/`DEFAULT` inputs: omitted no-explicit-default columns and explicit `DEFAULT` for no-default columns warn and store `0` for numeric `NOT NULL` or `NULL` for nullable dropped-default columns, explicit `NULL` into numeric `NOT NULL` stores `0` with warnings, out-of-range integer values clip to the supported descriptor range with warnings, and non-ignorable syntax/name/shape errors remain errors; no `INSERT IGNORE ... SELECT`, duplicate-key discard, partitions, generated values, `DEFAULT(col_name)`, general expression conversion, or full non-strict mode behavior. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `REPLACE ... VALUES` | 🟡 | Limited no-key-descriptor `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] table [(columns)] VALUES (...)` insert-equivalent path for persistent base tables with integer/`NULL`/`TRUE`/`FALSE`/`DEFAULT` values, descriptor column resolution, omitted-column and explicit-`DEFAULT` defaults, strict diagnostics, affected rows, statement atomicity, and delayed deprecation warning; duplicate-key delete-insert semantics await primary/unique key descriptors, and no `VALUE`, row constructors, partitions, `DEFAULT(col_name)`, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `REPLACE ... SET` | 🟡 | Limited no-key-descriptor one-row `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] table SET column = integer_or_NULL_or_boolean_or_DEFAULT[, ...]` insert-equivalent path for persistent base tables with descriptor assignment resolution, omitted-column and explicit-`DEFAULT` defaults, strict diagnostics, one affected row, statement atomicity, and delayed deprecation warning; duplicate-key delete-insert semantics await primary/unique key descriptors, and no partitions, qualified assignment targets, expressions, `DEFAULT(col_name)`, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `REPLACE ... SELECT` | 🟡 | Limited no-key-descriptor `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] target [(columns)] SELECT ... FROM source` insert-equivalent path for persistent base tables, reusing the existing source `SELECT` subset plus simple no-op source locking clauses, and descriptor-driven target defaults/nullability/range checks; duplicate-key delete-insert semantics await primary/unique key descriptors, and no partitions, `TABLE`, joins, literal/expression projection sources, generated values, locking options, warning demotion, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `UPDATE` (single-table) | 🟡 | Limited persistent base-table `UPDATE table SET column = integer_or_NULL_or_boolean_or_DEFAULT` with optional baseline `WHERE` predicate boolean expression, one unqualified descriptor `ORDER BY` column, and signed-64-range `LIMIT row_count`; assignment, predicate, and ordering columns may explicitly name invisible descriptor columns; changed-row affected counts; no aliases, partitions, modifiers, joins, multiple assignments, expression assignments, `DEFAULT(col_name)`, full ordering, or offset forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `LAST_INSERT_ID()` | 🟡 | Limited zero-argument one-row scalar select returns the current no-generated-id baseline value `0`; no `AUTO_INCREMENT`, `LAST_INSERT_ID(expr)`, protocol insert-id metadata, C API `mysql_insert_id()`, or generated id behavior. | [system functions](docs/compatibility/functions-system.md) |
...
| `SHOW COLUMNS` | 🟡 | Limited descriptor-driven column listing for persistent base tables with integer-family descriptors; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE` filters, and reports SQL `NULL` or canonical integer default values plus `INVISIBLE` in `Extra` for descriptor-invisible columns, but no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, indexes, expression defaults, generated hidden columns, or `INFORMATION_SCHEMA`. | [SQL SHOW statements](docs/compatibility/sql-show-statements.md) |
...
| `SHOW CREATE TABLE` | 🟡 | Limited descriptor-driven MySQL-style DDL for persistent base tables with current integer-family descriptors, nullability, implicit nullable-column `DEFAULT NULL` rendering, dropped-default columns with no default clause, canonical quoted integer defaults, descriptor-invisible column comments, fixed InnoDB/utf8mb4 suffix, and fixed enabled `sql_quote_show_create` quoting; no views, temporary tables, indexes, expression defaults, constraints, generated columns, auto-increment, privileges, mutable quote-control state, or disabled rendering. | [SQL SHOW statements](docs/compatibility/sql-show-statements.md) |
