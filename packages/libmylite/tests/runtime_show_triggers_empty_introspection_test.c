...
struct expected_show_triggers_empty_result {
    const char *sql;
    const char *context;
};

static const char *const show_triggers_names[show_triggers_column_count] = {
    "Trigger",
    "Event",
    "Table",
    "Statement",
...
static int test_show_triggers_result_shape_schema_like_persistence_and_drop(void);
static int test_show_triggers_diagnostics_and_unsupported_forms(void);
static int test_independent_show_triggers_handles(void);
static int create_show_triggers_schemas(mylite_db *database);
static int expect_show_triggers_empty_result(
    mylite_db *database,
    struct expected_show_triggers_empty_result expectation
);
...
int main(void) {
    ...
    failures += test_show_triggers_result_shape_schema_like_persistence_and_drop();
    failures += test_show_triggers_diagnostics_and_unsupported_forms();
    failures += test_independent_show_triggers_handles();
...
static int test_show_triggers_result_shape_schema_like_persistence_and_drop(void) {
    static const char *const forms[] = {
        ...
        "SHOW FULL TRIGGERS",
        "SHOW TRIGGERS LIKE '%'",
        "SHOW FULL TRIGGERS LIKE 'account\\_%'",
        "SHOW TRIGGERS FROM app",
        "SHOW TRIGGERS IN app",
        "SHOW FULL TRIGGERS FROM app",
        "SHOW FULL TRIGGERS IN app LIKE 'account'",
        "SHOW TRIGGERS FROM other LIKE 'missing%'",
    };
    ...
    failures += create_show_triggers_schemas(database);
    failures += expect_show_triggers_empty_result(
        database,
        (struct expected_show_triggers_empty_result){
            .sql = "SHOW TRIGGERS FROM app",
            .context = "qualified show triggers without default schema",
        }
    );
    ...
    failures += expect_int64(
        ...
        "catalog generation after show triggers"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation after show triggers"
    );
    failures += expect_row_count(database, -1, "row count after show triggers");
    failures += read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble));
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after show triggers"
    );
    ...
    failures += expect_show_triggers_empty_result(
        database,
        (struct expected_show_triggers_empty_result){
            .sql = "SHOW FULL TRIGGERS",
            .context = "reopened show full triggers",
        }
    );
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM other",
        (struct expected_sql_error){
            .code = mysql_error_unknown_database,
            .sqlstate = "42000",
            .message_part = "Unknown database 'other'",
        }
    );
...
static int test_show_triggers_diagnostics_and_unsupported_forms(void) {
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM missing_schema",
        (struct expected_sql_error){
            .code = mysql_error_unknown_database,
            .sqlstate = "42000",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS IN missing_schema",
        (struct expected_sql_error){
            .code = mysql_error_unknown_database,
            .sqlstate = "42000",
            .message_part = "Unknown database 'missing_schema'",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM _mylite_catalog",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app ORDER BY `Trigger`",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIMIT 1",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIKE NULL",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIKE N'account'",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIKE _utf8mb4'account'",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIKE CONCAT('account', '%')",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW TRIGGERS FROM app LIKE 'account\\0%'",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
...
static int expect_show_triggers_empty_result(
    ...
    failures += expect_size(
        ...
        show_triggers_column_count,
        "show triggers column count"
    );
    for (size_t column_index = 0U; column_index < show_triggers_column_count; ++column_index) {
        failures += expect_text_or_null(
            mylite_result_column_name(result, column_index),
            show_triggers_names[column_index],
            expectation.context
        );
    }
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_show_triggers_empty_%s_%d.mylite",
        name,
        current_process_id()
    );
