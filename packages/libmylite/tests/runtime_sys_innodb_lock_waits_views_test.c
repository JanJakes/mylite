...
enum {
    ...
    innodb_lock_waits_column_count = 30,
    ...
    information_schema_columns_sample_row_count = 6,
...
static int test_sys_innodb_lock_waits_views(void);
...
static const char *const innodb_lock_waits_columns[innodb_lock_waits_column_count] = {
    "wait_started",
    "wait_age",
    "wait_age_secs",
    "locked_table",
...
int main(void) {
    return test_sys_innodb_lock_waits_views() == 0 ? 0 : 1;
}

static int test_sys_innodb_lock_waits_views(void) {
    static const char *const count_column[] = {"COUNT(*)"};
    static const char *const count_zero[] = {"0"};
    static const char *const count_sixty[] = {"60"};
    static const char *const row_count_column[] = {"ROW_COUNT()"};
    static const char *const row_count_minus_one[] = {"-1"};
    static const char *const show_columns_formatted_sample_values[] = {
        "waiting_query",
        "longtext",
        "YES",
        "",
    ...
    static const char *const show_columns_raw_sample_values[] = {
        "waiting_query",
        "varchar(1024)",
        "YES",
        "",
    ...
    static const char *const show_full_columns_formatted_sample_values[] = {
        "waiting_query",
        "longtext",
        "utf8mb4_0900_ai_ci",
        "YES",
    ...
    static const char *const show_full_columns_raw_sample_values[] = {
        "waiting_query",
        "varchar(1024)",
        "utf8mb3_general_ci",
        "YES",
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .column_names = innodb_lock_waits_columns,
            .column_count = innodb_lock_waits_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.innodb_lock_waits direct rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = NULL,
            .row_count = 0U,
            .context = "sys.x innodb_lock_waits direct rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys innodb_lock_waits row_count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.innodb_lock_waits",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys.innodb_lock_waits count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM `x$innodb_lock_waits`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys.x innodb_lock_waits selected schema count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM innodb_lock_waits",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = innodb_lock_waits_column_count,
            .context = "sys.innodb_lock_waits show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM `x$innodb_lock_waits`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = innodb_lock_waits_column_count,
            .context = "sys.x innodb_lock_waits show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE innodb_lock_waits",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = innodb_lock_waits_column_count,
            .context = "sys.innodb_lock_waits describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESC `x$innodb_lock_waits`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = innodb_lock_waits_column_count,
            .context = "sys.x innodb_lock_waits desc",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM innodb_lock_waits WHERE Field IN "
                   "('waiting_query', 'blocking_query', 'sql_kill_blocking_query')",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_formatted_sample_values,
            .row_count = 3U,
            .context = "sys.innodb_lock_waits show columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM `x$innodb_lock_waits` WHERE Field IN "
                   "('waiting_query', 'blocking_query', 'sql_kill_blocking_query')",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_raw_sample_values,
            .row_count = 3U,
            .context = "sys.x innodb_lock_waits show columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM innodb_lock_waits WHERE Field IN "
                   "('waiting_query', 'blocking_query', 'sql_kill_blocking_query')",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_formatted_sample_values,
            .row_count = 3U,
            .context = "sys.innodb_lock_waits show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM `x$innodb_lock_waits` WHERE Field IN "
                   "('waiting_query', 'blocking_query', 'sql_kill_blocking_query')",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_raw_sample_values,
            .row_count = 3U,
            .context = "sys.x innodb_lock_waits show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM innodb_lock_waits",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.innodb_lock_waits show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM `x$innodb_lock_waits`",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.x innodb_lock_waits show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "sys innodb lock waits information_schema.columns sample rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_sixty,
            .row_count = 1U,
            .context = "sys innodb lock waits information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys innodb lock waits information_schema.tables rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys innodb lock waits show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys innodb lock waits information_schema.views rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "sys innodb lock waits view_table_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 3U,
            .context = "sys innodb lock waits view_routine_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys innodb lock waits empty statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys innodb lock waits empty table constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys innodb lock waits empty key column usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys innodb lock waits empty constraint extensions",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW innodb_lock_waits",
            ...
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `innodb_lock_waits`",
            .context = "sys.innodb_lock_waits show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            ...
            .row_index = 0U,
            .column_index = 1U,
            .needle = "`sys`.`format_statement`(`r`.`trx_query`) AS `waiting_query`",
            .context = "sys.innodb_lock_waits show create formatted query",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE `x$innodb_lock_waits`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `x$innodb_lock_waits`",
            .context = "sys.x innodb_lock_waits show create table",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW `x$innodb_lock_waits`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "`r`.`trx_query` AS `waiting_query`",
            .context = "sys.x innodb_lock_waits show create raw query",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_sys_innodb_lock_waits_views_%d_%s.mylite",
        current_process_id(),
        name
    );
