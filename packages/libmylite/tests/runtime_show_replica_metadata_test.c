...
static int test_show_replica_metadata_file_reopen_and_preamble(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open replica file");
    if (database == NULL) {
        remove_related_files(path);
        return failures;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen replica file");
    if (database == NULL) {
        remove_related_files(path);
        return failures;
    }
...
static int test_independent_show_replica_metadata_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first replica handle");
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second replica handle"
    );
...
static int expect_show_replica_metadata(mylite_db *database, struct expected_show_result expected) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "replica metadata row count");
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "replica affected rows");
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "replica warning count");
...
static int expect_row_count_status(mylite_db *database, const char *context) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "replica status row count");
    failures += mylite_test_expect_text_or_null(
        mylite_result_value_text(result, 0U, 0U),
        "0",
        "replica warning count"
    );
    failures +=
