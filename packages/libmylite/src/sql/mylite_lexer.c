...
static const struct mylite_keyword_entry mylite_keyword_catalog[] = {
...
bool mylite_sql_keyword_catalog_at(size_t index, const char **out_word, unsigned int *out_flags) {
    ...
    if (index >= mylite_sql_keyword_catalog_count() || out_word == NULL || out_flags == NULL) {
        return false;
    }

    *out_word = mylite_keyword_catalog[index].word;
    *out_flags = mylite_keyword_catalog[index].flags;
    return true;
}
...
bool mylite_sql_keyword_lookup(const char *text, size_t length, unsigned int *out_flags) {
    ...
    high = sizeof(mylite_keyword_catalog) / sizeof(mylite_keyword_catalog[0]);
    while (low < high) {
        size_t middle = low + ((high - low) / 2U);
        int compare = strcmp(folded, mylite_keyword_catalog[middle].word);
        if (compare == 0) {
            if (out_flags != NULL) {
                *out_flags = mylite_keyword_catalog[middle].flags;
            }
            return true;
        }
        if (compare < 0) {
