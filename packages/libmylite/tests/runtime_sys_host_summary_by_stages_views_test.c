...
enum {
    ...
    host_summary_by_stages_view_table_usage_row_count = 2,
...
static int test_sys_host_summary_by_stages_views(void);
...
static const char
    *const sys_host_summary_by_stages_columns[sys_host_summary_by_stages_column_count] = {
        "host",
        "event_name",
        "total",
        "total_latency",
...
int main(void) {
    return test_sys_host_summary_by_stages_views() == 0 ? 0 : 1;
}
...
static int test_sys_host_summary_by_stages_views(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.host_summary_by_stages",
            .column_names = sys_host_summary_by_stages_columns,
            .column_count = sys_host_summary_by_stages_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary by stages rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.`x$host_summary_by_stages`",
            .column_names = sys_host_summary_by_stages_columns,
            .column_count = sys_host_summary_by_stages_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x host summary by stages rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys host summary by stages row_count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.host_summary_by_stages",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.`x$host_summary_by_stages`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys x host summary by stages count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM host_summary_by_stages",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM `x$host_summary_by_stages`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys x host summary by stages selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM host_summary_by_stages",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = formatted_show_columns_values,
            .row_count = sys_host_summary_by_stages_column_count,
            .context = "sys host summary by stages show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM `x$host_summary_by_stages`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = raw_show_columns_values,
            .row_count = sys_host_summary_by_stages_column_count,
            .context = "sys x host summary by stages show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE host_summary_by_stages",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = formatted_show_columns_values,
            .row_count = sys_host_summary_by_stages_column_count,
            .context = "sys host summary by stages describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM host_summary_by_stages",
            ...
            .row_count = sys_host_summary_by_stages_column_count,
            .context = "sys host summary by stages show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM `x$host_summary_by_stages`",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = raw_show_full_columns_values,
            .row_count = sys_host_summary_by_stages_column_count,
            .context = "sys x host summary by stages show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM host_summary_by_stages",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary by stages show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM `x$host_summary_by_stages`",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x host summary by stages show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, IS_NULLABLE, "
                   ...
                   "AND ((TABLE_NAME = 'host_summary_by_stages') OR "
                   "(TABLE_NAME = 'x$host_summary_by_stages'))",
            ...
            .context = "sys host summary by stages information_schema.columns rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_five,
            .row_count = 1U,
            .context = "sys host summary by stages information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_five,
            .row_count = 1U,
            .context = "sys x host summary by stages information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS "
                   ...
                   "'host_summary_by_stages') OR (TABLE_NAME = "
                   "'x$host_summary_by_stages'))",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_ten,
            .row_count = 1U,
            .context = "sys host summary by stages information_schema.columns combined count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_SCHEMA, TABLE_NAME, TABLE_TYPE, ENGINE, TABLE_ROWS, "
                   ...
                   "WHERE TABLE_SCHEMA = 'sys' AND ((TABLE_NAME = 'host_summary_by_stages') "
                   "OR (TABLE_NAME = 'x$host_summary_by_stages')) ORDER BY TABLE_NAME",
            ...
            .row_count = 2U,
            .context = "sys host summary by stages information_schema.tables rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'host_summary_by_stages'",
            ...
            .row_count = 1U,
            .context = "sys host summary by stages show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'x$host_summary_by_stages'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = raw_show_table_status_values,
            .row_count = 1U,
            .context = "sys x host summary by stages show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_SCHEMA, TABLE_NAME, CHECK_OPTION, IS_UPDATABLE, DEFINER, "
                   ...
                   "AND ((TABLE_NAME = 'host_summary_by_stages') OR "
                   "(TABLE_NAME = 'x$host_summary_by_stages')) ORDER BY TABLE_NAME",
            ...
            .row_count = 2U,
            .context = "sys host summary by stages information_schema.views rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT VIEW_SCHEMA, VIEW_NAME, TABLE_SCHEMA, TABLE_NAME "
                   ...
                   "AND ((VIEW_NAME = 'host_summary_by_stages') OR "
                   "(VIEW_NAME = 'x$host_summary_by_stages'))",
            ...
            .row_count = host_summary_by_stages_view_table_usage_row_count,
            .context = "sys host summary by stages view_table_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary by stages empty view_routine_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS "
                   "WHERE TABLE_SCHEMA = 'sys' AND ((TABLE_NAME = 'host_summary_by_stages') "
                   "OR (TABLE_NAME = 'x$host_summary_by_stages'))",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages empty statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages empty constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages empty key column usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary by stages empty constraint extensions",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW host_summary_by_stages",
            ...
            .needle = "CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `host_summary_by_stages`",
            .context = "sys host summary by stages show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW `x$host_summary_by_stages`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `x$host_summary_by_stages`",
            .context = "sys x host summary by stages show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE sys.host_summary_by_stages",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `sys`.`host_summary_by_stages`",
            .context = "sys host summary by stages qualified show create table",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE `x$host_summary_by_stages`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "from `performance_schema`.`events_stages_summary_by_host_by_event_name` "
                      "where",
            .context = "sys x host summary by stages show create source",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_sys_host_summary_by_stages_views_%d_%s.mylite",
        current_process_id(),
        name
    );
