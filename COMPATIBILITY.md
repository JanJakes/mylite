...
| `-` (unary) | 🟡 | Specified for upcoming limited no-source/`DUAL` signed-64 scalar unary arithmetic projection with unary `+` and unary `-`; not implemented yet, and no table-backed expression support. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
