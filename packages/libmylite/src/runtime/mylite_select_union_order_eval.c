...
static int evaluate_union_order_key(
    mylite_stmt *stmt,
    const struct mylite_table_select_row *row,
...
static int resolve_union_expression_identifier(
    void *user_data,
    const struct mylite_sql_ast_node *identifier,
    struct mylite_expression_value *out_value
);

static int evaluate_union_session_function(
    void *user_data,
...
int mylite_select_union_evaluate_order_values(
    ...
    for (size_t index = 0U; index < stmt->select_plan.order_key_count; ++index) {
        int status = evaluate_union_order_key(
            stmt,
            row,
            &stmt->select_plan.order_keys[index],
...
static int resolve_union_expression_identifier(
    ...
    if (part_count == 1U) {
        ...
        if (output_matches == 1U && output_index < context->row->output_value_count) {
            status = mylite_expression_value_copy(
                         &context->row->output_values[output_index],
                         out_value
                     ) == 0
                         ? 0
                         : MYLITE_NOMEM;
