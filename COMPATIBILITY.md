...
| `RESET BINARY LOGS AND GTIDS` | ❌ | low | Binary log and GTID reset syntax. | Parser records the binary-log collection, including optional `TO` file-index forms and legacy `RESET MASTER` spellings for explicit unsupported-form routing; binary-log deletion and GTID reset behavior are not implemented. |
