...
static int expect_semantic_ast_materialization(void) {
  ...
  const MyliteSemanticAstNode *row =
      first_semantic_child_with_kind(source, MYLITE_SEMANTIC_NODE_ROW);
  const MyliteSemanticAstNode *value_descriptor =
      first_semantic_child_with_descriptor_kind(
          row, MYLITE_SEMANTIC_DESCRIPTOR_VALUE);
  const MyliteSemanticAstNode *direct_value_descriptor =
      first_semantic_child_with_descriptor_kind(
  ...
  if (root == NULL ||
    ...
    fprintf(stderr,
            ...
            "targets=%zu sources=%zu rows=%zu descriptors=%zu clauses=%zu "
            ...
            counts.sources, counts.rows, counts.descriptors, counts.clauses,
            counts.expressions, counts.operators, counts.leaf_values);
    failed = 1;
  }
  mylite_semantic_ast_free(semantic_ast);

  const char *insert_set_sql = "INSERT INTO t SET a = 1, b = a + 2";
  semantic_ast = NULL;
  status = mylite_parse_sql_semantic_ast(insert_set_sql, &semantic_ast,
                                         &result);
  if (status != MYLITE_PARSE_OK) {
    fprintf(stderr,
            "semantic AST INSERT SET parse failed: status=%s offset=%zu "
            "token=%d message=%s\n",
            mylite_parse_status_name(status), result.offset, result.token,
            result.message);
    return failed + 1;
  ...
  const MyliteSemanticAstNode *direct_assignment =
      first_semantic_child_with_descriptor_kind(
          statement, MYLITE_SEMANTIC_DESCRIPTOR_ASSIGNMENT);
  SemanticCounts insert_set_counts = {0};
  count_semantic_nodes(root, &insert_set_counts);
  if (statement == NULL ||
      mylite_semantic_ast_node_statement_kind(statement) !=
          MYLITE_STATEMENT_INSERT ||
      source == NULL ||
      ...
      insert_set_counts.leaf_values < 3) {
    fprintf(stderr,
            "semantic AST INSERT SET shape mismatch: nodes=%zu sources=%zu "
            "descriptors=%zu expressions=%zu operators=%zu leaf_values=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast),
            insert_set_counts.sources, insert_set_counts.descriptors,
            insert_set_counts.expressions, insert_set_counts.operators,
            insert_set_counts.leaf_values);
    failed = 1;
  }
  mylite_semantic_ast_free(semantic_ast);

  const char *replace_values_sql =
      "REPLACE INTO t (a, b) VALUES (1, DEFAULT), (2, a + 3)";
  semantic_ast = NULL;
  status = mylite_parse_sql_semantic_ast(replace_values_sql, &semantic_ast,
                                         &result);
  if (status != MYLITE_PARSE_OK) {
    fprintf(stderr,
            "semantic AST REPLACE VALUES parse failed: status=%s offset=%zu "
            "token=%d message=%s\n",
            mylite_parse_status_name(status), result.offset, result.token,
            result.message);
    return failed + 1;
  ...
  direct_value_descriptor = first_semantic_child_with_descriptor_kind(
      statement, MYLITE_SEMANTIC_DESCRIPTOR_VALUE);
  SemanticCounts replace_values_counts = {0};
  count_semantic_nodes(root, &replace_values_counts);
  if (statement == NULL ||
      mylite_semantic_ast_node_statement_kind(statement) !=
          MYLITE_STATEMENT_REPLACE ||
      source == NULL ||
      ...
      replace_values_counts.leaf_values < 4) {
    fprintf(stderr,
            "semantic AST REPLACE VALUES shape mismatch: nodes=%zu "
            "sources=%zu rows=%zu descriptors=%zu expressions=%zu operators=%zu "
            "leaf_values=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast),
            replace_values_counts.sources, replace_values_counts.rows,
            replace_values_counts.descriptors, replace_values_counts.expressions,
            replace_values_counts.operators,
            replace_values_counts.leaf_values);
    failed = 1;
  }
  mylite_semantic_ast_free(semantic_ast);
  ...
  const char *values_query_sql =
      "VALUES ROW(1, DEFAULT), ROW(2, a + 3) ORDER BY 1 LIMIT 1";
  semantic_ast = NULL;
  status = mylite_parse_sql_semantic_ast(values_query_sql, &semantic_ast,
                                         &result);
  if (status != MYLITE_PARSE_OK) {
    fprintf(stderr,
            "semantic AST VALUES query parse failed: status=%s offset=%zu "
            "token=%d message=%s\n",
            mylite_parse_status_name(status), result.offset, result.token,
            result.message);
    return failed + 1;
  ...
  statement = mylite_semantic_ast_node_child_at(root, 0);
  query = first_semantic_child_with_kind(statement, MYLITE_SEMANTIC_NODE_QUERY);
  row = first_semantic_child_with_kind(query, MYLITE_SEMANTIC_NODE_ROW);
  const MyliteSemanticAstNode *query_value =
      first_semantic_child_with_descriptor_kind(
          row, MYLITE_SEMANTIC_DESCRIPTOR_VALUE);
  const MyliteSemanticAstNode *direct_value =
      first_semantic_child_with_descriptor_kind(
          statement, MYLITE_SEMANTIC_DESCRIPTOR_VALUE);
  direct_clause =
      first_semantic_child_with_kind(statement, MYLITE_SEMANTIC_NODE_CLAUSE);
  ...
  SemanticCounts values_query_counts = {0};
  count_semantic_nodes(root, &values_query_counts);
  if (statement == NULL ||
      mylite_semantic_ast_node_statement_kind(statement) !=
          MYLITE_STATEMENT_SELECT ||
      query == NULL ||
      mylite_semantic_ast_node_query_block_count(query) != 0 ||
      row == NULL ||
      mylite_semantic_ast_node_row_index(row) != 0 ||
      mylite_semantic_ast_node_child_count(row) != 2 ||
      query_value == NULL ||
      mylite_semantic_ast_node_child_count(query_value) != 1 ||
      direct_value != NULL || direct_clause != NULL ||
      order_by_clause == NULL || limit_clause == NULL ||
      values_query_counts.queries != 1 ||
      values_query_counts.rows != 2 ||
      values_query_counts.descriptors != 4 ||
      values_query_counts.clauses != 2 ||
      values_query_counts.expressions < 6 ||
      values_query_counts.operators < 1 ||
      values_query_counts.leaf_values < 4) {
    fprintf(stderr,
            "semantic AST VALUES query shape mismatch: nodes=%zu "
            "queries=%zu rows=%zu descriptors=%zu clauses=%zu expressions=%zu "
            "operators=%zu leaf_values=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast),
            values_query_counts.queries, values_query_counts.rows,
            values_query_counts.descriptors, values_query_counts.clauses,
            values_query_counts.expressions, values_query_counts.operators,
            values_query_counts.leaf_values);
    failed = 1;
  }
  mylite_semantic_ast_free(semantic_ast);
  ...
  const MyliteSemanticAstNode *table =
      first_semantic_child_with_kind(statement, MYLITE_SEMANTIC_NODE_TABLE);
  column_descriptor = first_semantic_child_with_descriptor_kind(
      table, MYLITE_SEMANTIC_DESCRIPTOR_COLUMN);
  const MyliteSemanticAstNode *direct_column_descriptor =
      first_semantic_child_with_descriptor_kind(
          statement, MYLITE_SEMANTIC_DESCRIPTOR_COLUMN);
  ...
  if (statement == NULL ||
      ...
      table == NULL ||
      mylite_semantic_ast_node_target_kind(table) !=
          MYLITE_STATEMENT_TARGET_TABLE ||
      ...
      ddl_counts.data_type_numeric_parameters != 0 ||
      ddl_counts.data_type_elements != 0 ||
    ...
    fprintf(stderr,
            ...
            "tables=%zu descriptors=%zu clauses=%zu data_types=%zu "
            "data_type_numeric_parameters=%zu data_type_elements=%zu "
            "expressions=%zu operators=%zu leaf_values=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast), ddl_counts.targets,
            ddl_counts.tables, ddl_counts.descriptors, ddl_counts.clauses,
            ddl_counts.data_types, ddl_counts.data_type_numeric_parameters,
            ddl_counts.data_type_elements, ddl_counts.expressions,
            ddl_counts.operators,
  ...
  if (status != MYLITE_PARSE_OK) {
    fprintf(stderr,
            "semantic AST type parse failed: status=%s offset=%zu token=%d "
            "message=%s\n",
            mylite_parse_status_name(status), result.offset, result.token,
            result.message);
    return failed + 1;
  ...
  root = mylite_semantic_ast_root(semantic_ast);
  statement = mylite_semantic_ast_node_child_at(root, 0);
  table = first_semantic_child_with_kind(statement, MYLITE_SEMANTIC_NODE_TABLE);
  column_descriptor = first_semantic_child_with_descriptor_kind(
      table, MYLITE_SEMANTIC_DESCRIPTOR_COLUMN);
  direct_column_descriptor = first_semantic_child_with_descriptor_kind(
      statement, MYLITE_SEMANTIC_DESCRIPTOR_COLUMN);
  data_type = first_semantic_child_with_kind(column_descriptor,
                                            MYLITE_SEMANTIC_NODE_DATA_TYPE);
  SemanticCounts type_counts = {0};
  count_semantic_nodes(root, &type_counts);
  if (statement == NULL ||
      mylite_semantic_ast_node_statement_kind(statement) !=
          MYLITE_STATEMENT_CREATE ||
      table == NULL ||
      !value_matches_when_expected(mylite_semantic_ast_node_value(table),
                                   mylite_semantic_ast_node_value_length(table),
                                   "type_t") ||
      direct_column_descriptor != NULL ||
      data_type == NULL ||
      mylite_semantic_ast_node_data_type_kind(data_type) !=
          MYLITE_CREATE_TABLE_COLUMN_TYPE_KIND_DECIMAL ||
      mylite_semantic_ast_node_data_type_numeric_parameter_count(data_type) !=
          2 ||
      mylite_semantic_ast_node_data_type_numeric_parameter_at(data_type, 0) !=
          10 ||
      mylite_semantic_ast_node_data_type_numeric_parameter_at(data_type, 1) !=
          2 ||
      !value_matches_when_expected(mylite_semantic_ast_node_value(data_type),
                                   mylite_semantic_ast_node_value_length(
                                       data_type),
      ...
      type_counts.tables != 1 ||
      type_counts.data_type_numeric_parameters != 4 ||
      type_counts.data_type_elements != 2 ||
      type_counts.data_type_attributes != 5) {
    fprintf(stderr,
            "semantic AST type shape mismatch: nodes=%zu tables=%zu "
            "data_types=%zu numeric_parameters=%zu elements=%zu "
            "attributes=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast),
            type_counts.tables, type_counts.data_types,
  ...
  if (statement == NULL ||
      ...
      !value_matches_when_expected(
          ...
          "SOURCE_HOST") ||
      admin_counts.descriptors != 2 || admin_counts.clauses != 0) {
    fprintf(stderr,
            "semantic AST admin shape mismatch: nodes=%zu descriptors=%zu "
            "clauses=%zu\n",
            mylite_semantic_ast_node_count(semantic_ast),
            admin_counts.descriptors, admin_counts.clauses);
    failed = 1;
  }
  mylite_semantic_ast_free(semantic_ast);
  ...
  const ExpectedSemanticClause values_structural_clauses[] = {
      {MYLITE_SEMANTIC_CLAUSE_ORDER_BY, 0},
      {MYLITE_SEMANTIC_CLAUSE_LIMIT, 0},
  };
  failed += expect_semantic_clauses(
      "VALUES structural clauses", "VALUES ROW(1) ORDER BY 1 LIMIT 1",
      values_structural_clauses,
      sizeof(values_structural_clauses) /
  ...
  failed += expect_semantic_clauses(
      ...
      sizeof(kill_clauses) / sizeof(kill_clauses[0]));
  return failed;
}
...
static int expect_semantic_clauses(const char *label, const char *sql,
  ...
  MyliteSemanticAst *semantic_ast = NULL;
  MyliteParseStatus status =
      mylite_parse_sql_semantic_ast(sql, &semantic_ast, &result);
  if (status != MYLITE_PARSE_OK) {
    fprintf(stderr,
            "%s parse failed: status=%s offset=%zu token=%d message=%s\n",
            label, mylite_parse_status_name(status), result.offset,
            result.token, result.message);
    return 1;
  }

  const MyliteSemanticAstNode *root = mylite_semantic_ast_root(semantic_ast);
  const MyliteSemanticAstNode *statement =
      mylite_semantic_ast_node_child_at(root, 0);
  const MyliteSemanticAstNode *direct_expression =
      first_semantic_child_with_kind(statement, MYLITE_SEMANTIC_NODE_EXPRESSION);
...
static void count_semantic_nodes(const MyliteSemanticAstNode *node,
  ...
  if (mylite_semantic_ast_node_kind(node) == MYLITE_SEMANTIC_NODE_TABLE) {
    counts->tables++;
  }
  ...
  if (mylite_semantic_ast_node_kind(node) == MYLITE_SEMANTIC_NODE_SOURCE) {
    counts->sources++;
  }
  if (mylite_semantic_ast_node_kind(node) == MYLITE_SEMANTIC_NODE_ROW) {
    counts->rows++;
  }
  ...
  if (mylite_semantic_ast_node_kind(node) == MYLITE_SEMANTIC_NODE_DATA_TYPE) {
    counts->data_types++;
    counts->data_type_numeric_parameters +=
