# Baseline sys.host_summary_by_file_io Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.host_summary_by_file_io` and `sys.x$host_summary_by_file_io` views.
MySQL uses these sys views to summarize file I/O waits grouped by Performance
Schema host. MyLite does not collect Performance Schema wait summaries, so both
views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.host_summary_by_file_io` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_host_summary_by_file_io_views_expectations.sh`.
...
surface without inventing host names or I/O latency values.

## Column Metadata

`sys.host_summary_by_file_io` has three columns:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `host` | `varchar(255)` | `YES` | `NULL` | `ascii_general_ci` |
| `ios` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `io_latency` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |

`sys.x$host_summary_by_file_io` has the same column names with raw numeric
latency metadata:
...
| `host` | `varchar(255)` | `YES` | `NULL` | `ascii_general_ci` |
| `ios` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `io_latency` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
...
uses `format_pico_time()` around the summed timer wait counter; the raw `x$`
view exposes the raw summed timer wait value.
...
- live host rows, file I/O counts, or latency totals;
