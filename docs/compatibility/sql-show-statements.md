...
| `SHOW COLUMNS` / `SHOW FIELDS` | 🟡 | Limited descriptor-driven column listing for persistent base tables; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE 'pattern'` filters with `Field`, integer, `decimal(M,D)`, `decimal(M,D) unsigned`, `date`, `datetime`, `timestamp`, `char(n)`, `varchar(n)`, and baseline `TEXT` family `Type`, `Null`, `Key` including `PRI` for the current single-column primary-key descriptor or first `NOT NULL` unique index when no primary key exists, `UNI` for supported unique-index columns, and `MUL` for supported nonunique secondary-index columns, SQL `NULL`, canonical integer, canonical decimal, canonical date-string, canonical datetime-string, or canonical timestamp-string `Default`, and `Extra`, including `auto_increment` for the current auto-increment descriptor and `INVISIBLE` for descriptor-invisible columns; no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, unsupported index metadata, string defaults outside admitted canonical temporal defaults, expression defaults, generated hidden columns, or full `INFORMATION_SCHEMA` parity |
...
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | 🟡 | Limited descriptor-resolved persistent base-table introspection with MySQL 8.4.9 column labels, zero rows for no-index tables, one `PRIMARY` BTREE row for the current single-column primary-key descriptor, and one BTREE row per supported unique or nonunique secondary index; no fulltext/spatial/composite/prefix/functional/descending index metadata, `EXTENDED`, `WHERE`, temporary tables, views, privileges, or real storage statistics |
