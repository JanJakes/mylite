...
review has independent surfaces worth splitting, such as MySQL source behavior,
runtime verification, docs, and tests.

## Review checklist

1. Read `README.md`, `AGENTS.md`, `COMPATIBILITY.md`, the design document, and
   the implementation diff.
2. Verify the feature against MySQL 8.4.9 behavior: results, errors, warnings,
   metadata, side effects, SQL modes, and edge cases.
3. Check architecture boundaries: parser/AST, analysis, metadata, translation,
   runtime hooks, SQLite usage, storage format, and integration packages.
4. Check tests for depth and relevance. Add tests for missing MySQL cases,
   regressions, or high-risk paths when the fix is clear; otherwise report the
   gap as blocking.
5. Check documentation: compatibility matrix, guide/design docs, comments, and
   intentionally unsupported behavior.
6. Run relevant tests and checks. Fix failures when the fix is clear; otherwise
   report the blocker with the failing command and evidence.
7. Review final diff quality: focused scope, no unrelated refactors, performance
