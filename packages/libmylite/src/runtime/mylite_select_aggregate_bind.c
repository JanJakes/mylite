...
static bool
aggregate_bind_callbacks_are_valid(const struct mylite_select_aggregate_bind_callbacks *callbacks);
static int
...
int mylite_select_bind_aggregate_aware_expression(
    ...
    const struct mylite_select_aggregate_bind_callbacks *callbacks)
{
    if (!aggregate_bind_callbacks_are_valid(callbacks)) {
    ...
    const struct mylite_select_aggregate_bind_callbacks *callbacks)
{
    ...
    case MYLITE_SQL_AST_AGGREGATE_CALL:
        return bind_aggregate_call(database, expression, plan, callbacks);
