...
static int test_mysql_default_roles_table(void);
...
static const char *const mysql_default_roles_columns[mysql_default_roles_column_count] = {
    "HOST",
    "USER",
    "DEFAULT_ROLE_HOST",
    "DEFAULT_ROLE_USER",
...
int main(void) {
    return test_mysql_default_roles_table() == 0 ? 0 : 1;
}
...
static int test_mysql_default_roles_table(void) {
    ...
    static const char *const show_full_columns_default_role_host_values[] = {
        "DEFAULT_ROLE_HOST",
        "char(255)",
        "ascii_general_ci",
        "NO",
    ...
    if (make_test_path(path, sizeof(path), "mysql-default-roles-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM mysql.default_roles",
            .column_names = mysql_default_roles_columns,
            .column_count = mysql_default_roles_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.default_roles direct empty read",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.default_roles",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.default_roles count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM default_roles",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "selected mysql.default_roles count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESC mysql.default_roles",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = mysql_default_roles_column_count,
            .context = "mysql.default_roles describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.default_roles LIKE 'HOST'",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_host_values,
            .row_count = 1U,
            .context = "mysql.default_roles show columns host",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.default_roles WHERE Field = 'DEFAULT_ROLE_HOST'",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_default_role_host_values,
            .row_count = 1U,
            .context = "mysql.default_roles show full columns default role host",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.default_roles",
            ...
            .context = "mysql.default_roles show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_four,
            .row_count = 1U,
            .context = "mysql.default_roles information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.default_roles information_schema.columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.default_roles information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.default_roles information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.default_roles information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_index_row_count,
            .context = "mysql.default_roles information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.default_roles information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'default_roles'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.default_roles show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_default_roles_table_%d_%s.mylite",
        current_process_id(),
        name
    );
