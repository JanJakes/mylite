...
static int test_named_unique_constraint_metadata_dml_drop_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open named unique file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen named unique file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_named_unique_constraint_variants_prefixes_and_diagnostics(void) {
    ...
    if (make_test_path(path, sizeof(path), "variants") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open variant file db");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures +=
...
static int test_named_unique_constraint_independent_handles(void) {
    ...
    failures +=
        expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first named unique db");
    failures +=
        expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second named unique db");
...
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected) {
    ...
    failures +=
        expect_size(mylite_result_column_count(result), 0U, "failed statement column count");
    failures += expect_size(mylite_result_row_count(result), 0U, "failed statement row count");
    mylite_result_free(result);
    return failures;
}
...
static int expect_physical_index_count(
    ...
    if (sqlite3_step(statement) == SQLITE_ROW) {
        failures += expect_int(sqlite3_column_int(statement, 0), expected_count, context);
    } else {
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_named_unique_constraint_%d_%s.mylite",
        current_process_id(),
        name
    );
