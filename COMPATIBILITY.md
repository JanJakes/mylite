...
| `CREATE TABLE` | ❌ | top | Column definitions, constraints, indexes, table options, generated columns, and partitions; see section 3.1. | Parse-only coverage exists for column definitions, inline primary/unique attributes, `AUTO_INCREMENT`, table-level primary keys, and table-level unique/secondary indexes; executable DDL is deferred. |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| Column definition grammar | ⚪ | top | Type, nullability, defaults, visibility, comments, storage, format, references, and constraints. | Parse-only `CREATE TABLE` column definitions are accepted for integer, boolean, string, binary, exact numeric, approximate numeric, and temporal column types plus common column attributes, inline `AUTO_INCREMENT`, inline primary-key and unique-key attributes, table-level primary-key constraints, and table-level unique/secondary indexes, and return `MYLITE_UNSUPPORTED` until table DDL execution lands; generated columns, references, checks, most table constraints, table options, executable metadata, and storage behavior remain unsupported. See [integer and boolean column types spec](docs/specs/integer-boolean-column-types/specs.md), [string and binary column types spec](docs/specs/string-binary-column-types/specs.md), [numeric column types spec](docs/specs/numeric-column-types/specs.md), [temporal column types spec](docs/specs/temporal-column-types/specs.md), [column attributes spec](docs/specs/column-attributes/specs.md), [primary keys and AUTO_INCREMENT spec](docs/specs/primary-keys-auto-increment/specs.md), and [CREATE TABLE unique and secondary indexes spec](docs/specs/create-table-indexes/specs.md). |
| Silent column specification changes | ❌ | high | MySQL automatic rewrites of column specifications and SHOW CREATE output. |  |
| Default expressions | ⚪ | top | Literal and expression defaults, CURRENT_TIMESTAMP variants, and error cases. | Parser accepts literal defaults, parenthesized defaults over MyLite's current expression subset, and targeted `CURRENT_TIMESTAMP[(fsp)]` defaults/update clauses for parse-only `CREATE TABLE`; executable default evaluation, generated-default metadata, type-specific default diagnostics, general function-call expressions, subqueries, variables, and warning records are deferred. See [column attributes spec](docs/specs/column-attributes/specs.md). |
| Generated columns | ❌ | high | Virtual/stored generated columns, dependencies, indexes, and metadata. |  |
| Invisible columns | ⚪ | medium | Implicit column lists, SELECT * behavior, and metadata flags. | Parser accepts `VISIBLE` and `INVISIBLE` column attributes for parse-only `CREATE TABLE`; all-invisible table validation, implicit column-list behavior, `SELECT *`, `INFORMATION_SCHEMA.COLUMNS.EXTRA`, and `SHOW CREATE TABLE` formatting are deferred. See [column attributes spec](docs/specs/column-attributes/specs.md). |
| Column comments | ⚪ | high | Column `COMMENT` attributes, metadata, length checks, and SHOW output. | Parser accepts `COMMENT 'string'` in parse-only column definitions; catalog storage, length validation, escaping normalization, `SHOW FULL COLUMNS`, and information-schema output are deferred. See [column attributes spec](docs/specs/column-attributes/specs.md). |
...
| AUTO_INCREMENT columns | ⚪ | top | Allocation, persistence, lock modes, explicit values, and overflow behavior. | Parser accepts inline `AUTO_INCREMENT` in parse-only `CREATE TABLE`, including MySQL syntax shapes whose type, key-membership, duplicate-auto-column, or default-conflict diagnostics are deferred until executable DDL; allocation, persistence, insert ids, SQL-mode behavior, metadata, and warnings remain unsupported. See [primary keys and AUTO_INCREMENT spec](docs/specs/primary-keys-auto-increment/specs.md). |
| Primary keys | ⚪ | top | Definition forms, implicit NOT NULL behavior, metadata, and errors. | Parser accepts inline `PRIMARY KEY`, inline `KEY`, and table-level `PRIMARY KEY` constraints with optional constraint/index names, identifier key parts, prefix lengths, ASC/DESC, and common primary-key index options for parse-only `CREATE TABLE`; uniqueness, implicit NOT NULL, duplicate diagnostics, catalog/index metadata, `SHOW CREATE TABLE`, and storage enforcement are deferred. See [primary keys and AUTO_INCREMENT spec](docs/specs/primary-keys-auto-increment/specs.md). |
| Unique indexes | ⚪ | top | NULL handling, prefix lengths, functional key parts, and conflict semantics. | Parser accepts inline `UNIQUE`/`UNIQUE KEY` and table-level `UNIQUE`, `UNIQUE KEY`, `UNIQUE INDEX`, and `CONSTRAINT ... UNIQUE` forms for parse-only `CREATE TABLE`, including identifier key parts, prefix lengths, `ASC`/`DESC`, `USING BTREE/HASH`, and common index options; uniqueness enforcement, duplicate diagnostics, metadata, `SHOW CREATE TABLE`, and information-schema population are deferred. See [CREATE TABLE unique and secondary indexes spec](docs/specs/create-table-indexes/specs.md). |
| Nonunique indexes | ⚪ | top | BTREE/HASH clauses, visibility, comments, and parser options. | Parser accepts table-level `KEY` and `INDEX` forms in parse-only `CREATE TABLE`, including optional names, pre-list and post-list `USING BTREE/HASH`, identifier key parts, prefix lengths, `ASC`/`DESC`, and common index options; executable index creation, metadata, normalization, warnings, and storage-engine diagnostics are deferred. See [CREATE TABLE unique and secondary indexes spec](docs/specs/create-table-indexes/specs.md). |
| Descending indexes | ⚪ | high | DESC key-part syntax and ordering semantics. | Parser records `ASC` and `DESC` key-part markers for parse-only primary, unique, and secondary indexes; physical order, optimizer behavior, metadata collation flags, and engine-specific validation are deferred. See [CREATE TABLE unique and secondary indexes spec](docs/specs/create-table-indexes/specs.md). |
| Prefix indexes | ⚪ | top | Prefix length parsing, byte/character semantics, and limits. | Parser records integer prefix lengths for identifier key parts in parse-only primary, unique, and secondary indexes; type suitability, byte/character semantics, storage limits, and warnings are deferred. See [CREATE TABLE unique and secondary indexes spec](docs/specs/create-table-indexes/specs.md). |
