...
enum {
    ...
    sql_safe_updates_value_column_count = 6,
    sql_safe_updates_label_column_count = 5,
    sql_safe_updates_diagnostics_column_count = 4,
    sql_safe_updates_selected_column_count = 2,
...
static int test_sql_safe_updates_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql safe updates file");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "sql safe updates values",
        }
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@SQL_SAFE_UPDATES, @@Global.Sql_Safe_Updates, "
        "@@session.`sql_safe_updates`, @@`sql_safe_updates`, (@@sql_safe_updates)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_safe_updates_label_column_count,
            .context = "sql safe updates labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_safe_updates, @@foreign_key_checks, @@unique_checks, "
        "@@updatable_views_with_limit, @@sql_quote_show_create, @@autocommit, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql safe updates scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_safe_updates_diagnostics_column_count,
            .context = "sql safe updates warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql safe updates clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_safe_updates_diagnostics_column_count,
            .context = "sql safe updates error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql safe updates clears errors");
    failures += expect_show_count_warnings(database, "0", "sql safe updates clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql safe updates reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql safe updates reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql safe updates preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after sql safe updates reads"
    );
    ...
    failures += execute_statement_ok(database, "UPDATE child SET id = 2");
    failures += execute_statement_ok(database, "DELETE FROM child");
    failures += execute_statement_ok(database, "INSERT INTO child (id) VALUES (1)");
    failures += expect_query_result(
        database,
        "SELECT @@sql_safe_updates, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_safe_updates_selected_column_count,
            .context = "sql safe updates with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql safe updates table DDL independence",
        }
    );
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql safe updates file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_safe_updates, @@global.sql_safe_updates",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql safe updates values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql safe updates table row",
        }
    );
...
static int test_sql_safe_updates_qualifiers_and_errors(void) {
    ...
    failures +=
        expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql safe updates memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_SAFE_UPDATES, @@SESSION.SQL_SAFE_UPDATES, "
        "@@Local.Sql_Safe_Updates, @@global.`sql_safe_updates`, "
        "(@@sql_safe_updates)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_safe_updates_label_column_count,
            .context = "sql safe updates qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_safe_updates_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_safe_updates_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_safe_updates_variable'",
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_safe_updates + 1");
...
static int test_independent_sql_safe_updates_handles(void) {
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_safe_updates_independent_column_count,
            .context = "first handle sql safe updates variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@sql_safe_updates, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_safe_updates_independent_column_count,
            .context = "second handle sql safe updates variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_safe_updates_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
