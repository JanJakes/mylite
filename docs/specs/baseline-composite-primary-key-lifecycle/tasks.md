...
- [x] Replace runtime single-column loaded-index assumptions with ordered
  descriptor key-part arrays while keeping catalog schema unchanged.
- [x] Admit table-level `PRIMARY KEY (a,b,...)` for two or more unqualified
  integer-family planned columns in `CREATE TABLE`.
- [x] Reject duplicate key parts, unknown key parts, qualified parts,
...
- [x] Mark every admitted key part `NOT NULL`, preserve non-`NULL` defaults,
  and store ordered index-column descriptors.
- [x] Generate quoted physical SQLite composite unique-index SQL from
  descriptors and stable physical names.
- [x] Update duplicate-key handling for `INSERT`, `INSERT IGNORE`, and
  single-assignment `UPDATE` so composite tuples produce MySQL-shaped
  diagnostics and warning demotion.
- [x] Preserve `CREATE TABLE ... LIKE` composite key cloning and
  `CREATE TABLE ... SELECT` key omission.
- [x] Render descriptor-backed `SHOW COLUMNS`, `SHOW CREATE TABLE`,
