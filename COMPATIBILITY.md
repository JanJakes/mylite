...
| `CREATE INDEX` | ❌ | top | Standalone index creation over MySQL index types and attributes. | Parser recognizes non-empty functional/key-part lists plus index options for parser plugins, string-literal comments, visibility, engine attributes, numeric `KEY_BLOCK_SIZE`, `USING`, `ALGORITHM`, and `LOCK`. |
