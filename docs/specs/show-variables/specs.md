...
| `SHOW VARIABLES LIKE 'rand\_seed%'` | Returns session-only `rand_seed1` and `rand_seed2` rows with value `0`. `SHOW GLOBAL VARIABLES LIKE 'rand\_seed%'` returns no rows. |
...
| --- | --- | --- | --- | --- | --- | ---: | --- |
| session/local/default | `Variable_name` | `performance_schema` | `session_variables` | `VAR_STRING` | `latin1_swedish_ci` | 64 | `NOT_NULL NO_DEFAULT_VALUE` |
| session/local/default | `Value` | `performance_schema` | `session_variables` | `VAR_STRING` | `latin1_swedish_ci` | 1024 | none |
| global | `Variable_name` | `performance_schema` | `global_variables` | `VAR_STRING` | `latin1_swedish_ci` | 64 | `NOT_NULL NO_DEFAULT_VALUE` |
| global | `Value` | `performance_schema` | `global_variables` | `VAR_STRING` | `latin1_swedish_ci` | 1024 | none |
...
| `rand_seed1` | `0` | omitted | Session-only compatibility variable. Direct session assignment seeds the next unseeded `RAND()` sequence but expression reads and `SHOW VARIABLES` continue to expose `0`, matching MySQL 8.4.9. |
| `rand_seed2` | `0` | omitted | Session-only compatibility variable. Direct session assignment seeds the next unseeded `RAND()` sequence but expression reads and `SHOW VARIABLES` continue to expose `0`, matching MySQL 8.4.9. |
