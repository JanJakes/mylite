...
static int test_alter_add_unique_success_metadata_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open add unique file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen add unique file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_alter_add_unique_validation_dml_and_drop(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "validation") != 0) {
        return 1;
    }
    remove_related_files(path);
