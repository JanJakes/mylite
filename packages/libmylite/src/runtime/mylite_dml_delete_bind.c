...
static int bind_delete_predicate_expression(mylite_db *database,
                                            const struct mylite_select_table *table,
                                            const struct mylite_sql_ast_node *expression,
                                            const char *clause_context);
static int bind_delete_function_call(mylite_db *database, const struct mylite_select_table *table,
                                     const struct mylite_sql_ast_node *expression,
                                     const char *clause_context);
                                    ...
                                    const struct mylite_select_table *table)
{
    ...
    return bind_delete_predicate_expression(database, table, predicate, "where clause");
}
                                            ...
                                            const char *clause_context)
{
    ...
    case MYLITE_SQL_AST_QUALIFIED_IDENTIFIER: {
        ...
        if (column_index == table->column_count) {
            ...
            status =
                mylite_dml_set_delete_unknown_column_error(database, reference, clause_context);
            free(reference);
            return status;
        }
        return MYLITE_OK;
    ...
    case MYLITE_SQL_AST_CASE_WHEN:
        for (const struct mylite_sql_ast_node *child = expression->first_child; child != NULL;
            ...
            int status = bind_delete_predicate_expression(database, table, child, clause_context);
    ...
    case MYLITE_SQL_AST_CAST_EXPRESSION: {
        ...
        return bind_delete_predicate_expression(
            database, table, mylite_ast_child_at(expression, 0U), clause_context);
    }
    case MYLITE_SQL_AST_FUNCTION_CALL:
        return bind_delete_function_call(database, table, expression, clause_context);
                                     ...
                                     const char *clause_context)
{
    ...
    for (const struct mylite_sql_ast_node *child = arguments == NULL ? NULL
        ...
        status = bind_delete_predicate_expression(database, table, child, clause_context);
...
static int bind_delete_order_expression(mylite_db *database,
                                        ...
                                        const struct mylite_sql_ast_node *expression)
{
    return bind_delete_predicate_expression(database, table, expression, "order clause");
}
