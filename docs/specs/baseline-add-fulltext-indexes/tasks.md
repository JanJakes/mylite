...
- [ ] Extend parser/AST support for `ALTER TABLE ... ADD FULLTEXT` and
  `CREATE FULLTEXT INDEX`.
- [ ] Plan and validate added fulltext descriptors from MyLite catalog column
  descriptors.
- [ ] Store fulltext descriptors and index-column rows without creating
  physical SQLite indexes or bumping SQLite schema generation.
- [ ] Append the MySQL-compatible first-fulltext-add warning only after
  successful descriptor mutation.
- [ ] Add parser and runtime C tests for success metadata, warnings,
...
- [ ] Run focused MySQL expectation, parser/runtime/index tests, and full
