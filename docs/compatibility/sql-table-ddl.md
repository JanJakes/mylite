...
| --- | --- | --- |
| `ALTER TABLE` | 🟡 | Limited single-action persistent base-table `ALTER TABLE ... RENAME [TO\|AS]` and append-only `ALTER TABLE ... ADD [COLUMN]` integer-column forms only; see [ALTER TABLE actions](#alter-table-actions) |
...
| Atomic DDL | 🟡 | Catalog descriptor rows and generated physical SQLite table changes commit or roll back atomically for the limited create/drop/rename/truncate and single-action alter rename/add-column subsets only |
