# Baseline mysql.gtid_executed Table Tasks
...
- [x] Read the MyLite repository guidance, engineering standards,
      ...
      specs/tests.
- [x] Verify MySQL 8.4.9 behavior for direct reads, column metadata, composite
      primary-key metadata, table-status metadata, and row-count behavior.
...
- [x] Add the MySQL-runtime expectation script for the supported
      `gtid_executed` table surface.
- [x] Implement MyLite-owned `mysql.gtid_executed` definition, empty reads,
      column comments, composite primary-key ordering, and table-status
      metadata.
