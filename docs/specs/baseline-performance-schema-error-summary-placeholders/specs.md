# Baseline Performance Schema Error Summary Placeholders
...
returns zero rows from each table until live Performance Schema error
...
```sh
packages/libmylite/tests/mysql_baseline_performance_schema_error_summary_placeholders_expectations.sh
```
...
The account, host, thread, user, and global tables expose unique HASH
constraints named `ACCOUNT`, `HOST`, `THREAD_ID`, `USER`, and `ERROR_NUMBER`
respectively. The grouping key always includes `ERROR_NUMBER`, plus the
connection grouping column or columns for the account, host, thread, and user
tables.

## Intentional Gaps

This slice does not implement live Performance Schema error instrumentation. A
MySQL 8.4.9 runtime exposes nonzero rows because it tracks the server error
catalog and current instrumentation aggregates. MyLite intentionally returns
empty rows until it has a real error instrumentation pipeline and an explicit
error summary lifecycle. It also does not implement Performance Schema
truncation/reset semantics for these summaries.
...
- Build and run
  `mylite_runtime_performance_schema_error_summary_placeholders_test`.
- Run the focused CTest filter
  `^libmylite\.runtime\.performance_schema_error_summary_placeholders$`.
