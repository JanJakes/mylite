...
- [x] Add parser support for comparison binary expressions in scalar expression
  grammar without changing descriptor-backed predicate planning.
- [x] Admit no-source and `FROM DUAL` scalar comparison projection without
  widening table-backed expression projection.
- [x] Add MyLite-owned signed-64 comparison evaluation with ordinary `NULL`
  propagation and `<=>` NULL-safe equality semantics.
- [x] Preserve child arithmetic warnings, in-statement diagnostics-count, and
