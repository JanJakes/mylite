...
- MySQL accepts broader shapes such as null-safe equality, other formats,
  ...
  and expression assignment. This phase defers those shapes.
...
WHERE date_format_numeric_comparison
...
- `<=>`, `IS`, `BETWEEN`, `IN`, `LIKE`, `REGEXP`, truth-only, boolean,
...
- unsupported nonliteral or nonnumeric comparison operand in this `WHERE`
  grammar:
