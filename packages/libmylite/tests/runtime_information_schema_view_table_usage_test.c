...
static int test_information_schema_view_table_usage_queries(void);
static int test_information_schema_view_table_usage_reopen_preamble_and_handles(void);
...
int main(void) {
    ...
    failures += test_information_schema_view_table_usage_queries();
    failures += test_information_schema_view_table_usage_reopen_preamble_and_handles();
...
static int test_information_schema_view_table_usage_queries(void) {
    ...
    static const char *const view_name_column[] = {"VIEW_NAME"};
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open view usage query db");
    failures += seed_database(database, "app");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM INFORMATION_SCHEMA.VIEW_TABLE_USAGE WHERE VIEW_SCHEMA = 'app'",
            .column_names = usage_columns,
            .column_count = sizeof(usage_columns) / sizeof(usage_columns[0]),
            .values = NULL,
            .row_count = 0U,
            .context = "empty view table usage wildcard",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEW_TABLE_USAGE WHERE VIEW_SCHEMA = "
                   "'app'",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "empty view table usage count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "case-insensitive view table usage table",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT u.VIEW_NAME FROM INFORMATION_SCHEMA.VIEW_TABLE_USAGE AS u "
                   "WHERE u.VIEW_SCHEMA = 'app' ORDER BY u.VIEW_NAME LIMIT 1",
            .column_names = view_name_column,
            .column_count = 1U,
            .values = NULL,
            .row_count = 0U,
            .context = "empty view table usage alias order limit",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "view table usage system table row",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, "
                   ...
                   "WHERE TABLE_SCHEMA = 'information_schema' "
                   "AND TABLE_NAME = 'VIEW_TABLE_USAGE' ORDER BY ORDINAL_POSITION",
            ...
            .context = "view table usage columns metadata",
        }
    );
    failures += expect_row_count_status(database, "view table usage row count status");
...
static int test_information_schema_view_table_usage_reopen_preamble_and_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first view usage db"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second view usage db"
    );
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "first handle view table usage count",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEW_TABLE_USAGE "
                   "WHERE VIEW_SCHEMA = 'other'",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "second handle view table usage count",
        }
    );
    ...
    failures += expect_bytes(
        ...
        "preamble after view table usage metadata query"
    );
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "reopened view table usage count",
        }
    );
