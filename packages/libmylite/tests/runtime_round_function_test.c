...
static int test_round_warnings_and_do(void) {
    ...
    if (result != NULL) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "do places columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "do places rows");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "do places warning count"
        );
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "do places affected");
    }
    mylite_result_free(result);
    failures += expect_query(
        database,
...
static int test_round_errors_and_unsupported_forms(void) {
    ...
    failures += execute_ok(database, "DO ROUND(@@warning_count)", NULL);
