...
| `REPLACE ... VALUES` | 🟡 | Limited no-key-descriptor `REPLACE [INTO] table [(columns)] VALUES (...)` insert-equivalent path for persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, omitted-column defaults, strict diagnostics, affected rows, and statement atomicity; duplicate-key delete-insert semantics await primary/unique key descriptors, and no `VALUE`, row constructors, modifiers, partitions, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `REPLACE ... SET` | 🟡 | Limited no-key-descriptor one-row `REPLACE [INTO] table SET column = integer_or_NULL_or_boolean[, ...]` insert-equivalent path for persistent base tables with descriptor assignment resolution, omitted-column defaults, strict diagnostics, one affected row, and statement atomicity; duplicate-key delete-insert semantics await primary/unique key descriptors, and no modifiers, partitions, qualified assignment targets, expressions, DML `DEFAULT`, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `REPLACE ... SELECT` | 🟡 | Limited no-key-descriptor `REPLACE [INTO] target [(columns)] SELECT ... FROM source` insert-equivalent path for persistent base tables, reusing the existing source `SELECT` subset and descriptor-driven target defaults/nullability/range checks; duplicate-key delete-insert semantics await primary/unique key descriptors, and no modifiers, partitions, `TABLE`, joins, literal/expression projection sources, generated values, warnings, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
