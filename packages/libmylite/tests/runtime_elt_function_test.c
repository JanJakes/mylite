...
static int test_no_source_dual_and_do_elt(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "elt do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "elt do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "elt do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "elt do warnings");
    }
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-elt-function-%s-%d.mylite",
        name,
        current_process_id()
    );
