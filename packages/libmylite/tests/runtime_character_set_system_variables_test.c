...
static int test_independent_character_set_system_variable_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first charset handle");
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second charset handle"
    );
...
static int expect_select_numbers_row(mylite_db *database) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 2U, "numbers column count");
    failures += mylite_test_expect_size(mylite_result_row_count(result), 1U, "numbers row count");
    failures += mylite_test_expect_text_or_null(
