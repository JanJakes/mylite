...
ABSENT_SYSTEM_VARIABLE_NAMES=$(cat <<'NAMES'
...
fail() {
    printf '%s\n' "mysql_baseline_show_variables_optional_absence_expectations: $1" >&2
    exit 1
}
...
system_variable_names_sql() {
    printf '%s\n' "$ABSENT_SYSTEM_VARIABLE_NAMES" \
...
expect_unknown_variable() {
    name=$1
    sql=$2

    if output=$(run_mysql "$sql" 2>&1); then
        fail "$sql: expected unknown system variable error, got success [$output]"
    fi
    case "$output" in
        *"ERROR 1193 (HY000)"*"Unknown system variable '$name'"*) ;;
        *) fail "$sql: unexpected error [$output]" ;;
    esac
}
...
names_sql=$(system_variable_names_sql)

expect_absent_variables \
    "variables_info optional system variables absent" \
    "SELECT VARIABLE_NAME FROM performance_schema.variables_info
      WHERE LOWER(VARIABLE_NAME) IN ($names_sql)
      ORDER BY VARIABLE_NAME;"
expect_absent_variables \
    "default optional system variables absent" \
    "SHOW VARIABLES WHERE Variable_name IN ($names_sql);"
expect_absent_variables \
    "session optional system variables absent" \
    "SHOW SESSION VARIABLES WHERE Variable_name IN ($names_sql);"
expect_absent_variables \
    "local optional system variables absent" \
    "SHOW LOCAL VARIABLES WHERE Variable_name IN ($names_sql);"
expect_absent_variables \
    "global optional system variables absent" \
    "SHOW GLOBAL VARIABLES WHERE Variable_name IN ($names_sql);"

expect_unknown_variable "audit_log_buffer_size" \
    "SELECT @@audit_log_buffer_size;"
expect_unknown_variable "component_masking.masking_database" \
    "SELECT @@component_masking.masking_database;"
expect_unknown_variable "Ndb_system_name" \
    "SELECT @@Ndb_system_name;"
expect_unknown_variable "validate_password.length" \
    "SELECT @@validate_password.length;"

printf '%s\n' "mysql_baseline_show_variables_optional_absence_expectations: ok"
