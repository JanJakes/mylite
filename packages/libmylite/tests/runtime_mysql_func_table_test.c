...
static int test_mysql_func_table(void);
...
static const char *const func_columns[func_column_count] = {
    "name",
    "ret",
    "dl",
    "type",
...
int main(void) {
    return test_mysql_func_table() == 0 ? 0 : 1;
}
...
static int test_mysql_func_table(void) {
    ...
    static const char *const show_type_values[] = {
        "type",
        "enum('function','aggregate')",
        "utf8mb3_general_ci",
        "NO",
    ...
    if (make_test_path(path, sizeof(path), "mysql-func-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT name, ret, dl, type FROM mysql.func ORDER BY name",
            .column_names = func_columns,
            .column_count = func_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.func direct empty read",
        }
    );
    failures += expect_row_count_status(database, "row count after mysql.func read");
    failures += expect_query(
        database,
        (struct expected_query){
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.func unqualified count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.func",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.func show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.func",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.func show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.func WHERE Field = 'type'",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_type_values,
            .row_count = 1U,
            .context = "mysql.func show full columns where",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.func WHERE Key_name = 'PRIMARY'",
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .row_count = show_columns_row_count,
            .context = "mysql.func information_schema.columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.func information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.func information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.func information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.func information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.func information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'func' "
            ...
            .row_count = 1U,
            .context = "mysql.func show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_func_table_%d_%s.mylite",
        current_process_id(),
        name
    );
