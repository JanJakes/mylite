...
| `REPLACE ... VALUES` | ❌ | top | Delete-then-insert semantics, cascades, triggers, affected rows, and auto-increment behavior. | Started/speced only; no parser or runtime support yet. The spec covers `VALUES`, `VALUE`, and `VALUES ROW(...)`, delete-all-conflicting-rows behavior, source-order multi-row semantics, affected rows, duplicate counts, warnings/errors, auto-increment and last-insert-id side effects, rollback, and deferred trigger/foreign-key/generated-column interactions. See [REPLACE spec](docs/specs/replace/specs.md). |
| `REPLACE ... SET` | ❌ | high | SET-form replace semantics. | Started/speced only; no parser or runtime support yet. The spec covers candidate-row assignment evaluation, qualified assignment targets, defaults, conflict handling, affected rows, and auto-increment side effects. See [REPLACE spec](docs/specs/replace/specs.md). |
...
| `REPLACE LOW_PRIORITY` / `DELAYED` | ❌ | low | Priority and deprecated delayed modifiers for REPLACE. | Started/speced only; no parser or runtime support yet. `LOW_PRIORITY` is documented as an embedded no-op and `DELAYED` as warning 3005 plus normal replacement; `HIGH_PRIORITY` and `IGNORE` remain syntax errors. See [REPLACE spec](docs/specs/replace/specs.md). |
