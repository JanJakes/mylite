...
enum catalog_column_insert_bind_index {
    ...
    catalog_column_insert_generation_bind = 13,
};
...
enum catalog_column_replace_bind_index {
    ...
    catalog_column_replace_generation_bind = 11,
    catalog_column_replace_table_id_bind = 12,
    catalog_column_replace_column_id_bind = 13,
};
...
enum catalog_column_select_column_index {
    ...
    catalog_column_select_descriptor_version_column = 13,
    catalog_column_select_created_generation_column = 14,
    catalog_column_select_updated_generation_column = 15,
};
...
int mylite_catalog_replace_column_in_mutation(
    ...
    rc = prepare_statement(
        ...
        "default_text = ?9, on_update_current_timestamp = ?10, "
        "descriptor_version = descriptor_version + 1, updated_catalog_generation = ?11 "
        "WHERE table_id = ?12 AND column_id = ?13",
        &statement
    );
    if (rc == MYLITE_OK) {
...
int mylite_catalog_for_each_column_in_table(
    ...
    rc = prepare_statement(
        ...
        "default_text, on_update_current_timestamp, "
        "descriptor_version, created_catalog_generation, updated_catalog_generation "
...
static int migrate_catalog_schema(
    ...
    if (rc == MYLITE_OK && schema_version == catalog_schema_version_v5) {
        ...
        schema_version = catalog_schema_version_v6;
    }
    if (rc == MYLITE_OK && schema_version == catalog_schema_version_v6) {
        rc = migrate_catalog_schema_v6_to_v7(database->sqlite);
        schema_version = catalog_schema_version_v7;
    }
    if (rc == MYLITE_OK && schema_version == catalog_schema_version_v7) {
        rc = migrate_catalog_schema_v7_to_v8(database->sqlite);
        schema_version = catalog_schema_version_v8;
    }
    ...
    if (rc == MYLITE_OK && schema_version == catalog_schema_version_v10) {
        ...
        schema_version = catalog_schema_version_v11;
    }
    if (rc == MYLITE_OK && schema_version == catalog_schema_version_v11) {
        rc = migrate_catalog_schema_v11_to_v12(database->sqlite);
...
static int bind_catalog_column_insert_values(
    ...
    if (rc == MYLITE_OK) {
        rc = bind_i64(
            statement,
            catalog_column_insert_default_kind_bind,
            (int64_t)values->default_kind
        );
    }
    if (rc == MYLITE_OK) {
        rc = bind_nullable_i64(
...
static int bind_catalog_column_replace_values(
    ...
    if (rc == MYLITE_OK) {
        rc = bind_i64(
            statement,
            catalog_column_replace_default_kind_bind,
            (int64_t)values->default_kind
        );
    }
    if (rc == MYLITE_OK) {
