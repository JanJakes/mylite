...
| `SHOW ENGINE` | ❌ | low | Generic named-engine diagnostics dispatch, privilege checks, and storage-engine-specific result rows. | Parser records named engines for `STATUS`, `MUTEX`, and runtime-accepted `LOGS` forms. |
...
| `SHOW GRANTS` | ❌ | high | Current-user and named-account grant rows, role `USING` behavior, privilege checks, and MySQL 8.4 global/dynamic privilege formatting. | Parser records the user object kind for current-user grants, explicit `FOR` account names, and documented `CURRENT_USER()` targets. |
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | ❌ | top | Index metadata rows, table targets, optional schema qualification, `EXTENDED` hidden index behavior, WHERE filtering, generated invisible key behavior, and privileges. | Parser records the inspected table target across `FROM`/`IN`, schema-qualified, `EXTENDED`, and synonym forms. |
...
| `SHOW PARSE_TREE` | ❌ | low | JSON parse-tree result for the MySQL 8.4.9 grammar when enabled; syntax error when built without `WITH_SHOW_PARSE_TREE`. | Parser records documented `SELECT` payloads as query targets; runtime JSON output and build-flag diagnostics are not implemented. |
...
| `SHOW PROFILE` | ❌ | low | Deprecated profiling rows for the most recent statement or explicit `FOR QUERY` id, session profiling controls, and MySQL-compatible diagnostics. | Parser records the query object kind, including numeric `FOR QUERY` ids when present. |
...
| `SHOW RELAYLOG EVENTS` | ❌ | low | Relay-log event rows, default first-relay-log behavior, channel selection, limits, privileges, and replication diagnostics. | Parser records explicit relay-log names, bare relay-log targets, or channel targets when only `FOR CHANNEL` is present. |
| `SHOW REPLICA STATUS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records explicit `FOR CHANNEL` names or the replication-channel object kind for the channel collection, including deprecated `SHOW SLAVE STATUS` aliases. |
...
| `SHOW VARIABLES` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the system-variable object kind, including the `LIKE` pattern when present. |
| `SHOW WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the diagnostics-area object kind; result rows and `LIMIT` semantics are not implemented. |
