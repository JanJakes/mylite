...
| Feature | Status | Notes |
| --- | --- | --- |
| `CHAR` | 🟡 | Limited bare `CHAR` and `CHAR(0..255)` descriptors with UTF-8 non-`NUL` `TEXT` storage/readback in MySQL's default trimmed `CHAR` shape, silent excess-trailing-space truncation, strict nonspace overlength errors, literal descriptor defaults, `NULL` and empty-string behavior including `CHAR(0)`, admitted `INSERT`/`REPLACE`/`UPDATE` string assignments, `IS NULL` / `IS NOT NULL` predicates, descriptor cloning/copying, supported secondary-index declaration/metadata, and `SHOW` / `INFORMATION_SCHEMA.COLUMNS` rendering; no `CHARACTER` aliases, warning-producing overlength default truncation, `PAD_CHAR_TO_FULL_LENGTH`, charset conversion, collation comparison, ordering/grouping/distinct, primary or unique keys, collation-aware index semantics, or protocol-grade metadata |
| `VARCHAR` | 🟡 | Limited `VARCHAR(0..255)` descriptors with UTF-8 non-`NUL` `TEXT` storage/readback, `NULL` and empty-string behavior, strict overlength errors, literal descriptor defaults, admitted `INSERT`/`REPLACE`/`UPDATE` string assignments, `IS NULL` / `IS NOT NULL` predicates, descriptor cloning/copying, supported secondary-index declaration/metadata, and `SHOW` / `INFORMATION_SCHEMA.COLUMNS` rendering; no aliases, warning-producing overlength default truncation, lengths above 255, charset conversion, collation comparison, ordering/grouping/distinct, string aggregates, primary or unique keys, collation-aware index semantics, or protocol-grade metadata |
