...
static int test_information_schema_keywords(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "keywords") != 0) {
        return 1;
    }
    ...
    failures += mylite_test_expect_int(rc, MYLITE_OK, "open file database");
    if (rc != MYLITE_OK) {
        return failures + 1;
    }
    ...
    rc = mylite_open(path, &database);
    failures += mylite_test_expect_int(rc, MYLITE_OK, "reopen file database");
    if (rc == MYLITE_OK) {
        failures += expect_query(
            database,
            (struct expected_query){
...
static int expect_query(mylite_db *database, struct expected_query expected) {
    ...
    failures += mylite_test_expect_int64(mylite_result_affected_rows(result), 0, expected.sql);
    failures += mylite_test_expect_size(mylite_result_warning_count(result), 0U, expected.sql);
