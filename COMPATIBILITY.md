...
| `ALTER LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names that reject quoted hex/bit literals, numeric `INITIAL_SIZE` including quoted hex, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
...
| `CREATE LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names that reject quoted hex/bit literals, NDB numeric size/nodegroup options including quoted hex, comments, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
...
| `CREATE TABLESPACE` | ❌ | low | General and NDB tablespace syntax and diagnostics. | Parser recognizes string-literal data files with text-string validation, numeric autoextend/file-block sizes including quoted hex, string-literal encryption values, NDB logfile/size/nodegroup/comment options, `ENGINE`/`STORAGE ENGINE`, and string-literal engine attributes with quoted hex/bit rejection in text-string positions. |
