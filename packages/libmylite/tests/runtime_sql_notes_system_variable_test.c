...
static int test_sql_notes_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql notes file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql notes file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_notes, @@global.sql_notes",
        (struct expected_result){
...
static int test_sql_notes_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql notes memory");
    failures += execute_ok(
        database,
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_notes_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
