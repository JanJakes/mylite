...
| --- | --- | --- |
| `ALTER LOGFILE GROUP` | ⚪ | Accepted as an embedded no-op with warning `1105`; no NDB undo-file creation, logfile metadata, data-node state, or persistence |
...
| `CREATE LOGFILE GROUP` | ⚪ | Accepted as an embedded no-op with warning `1105`; no NDB logfile group, undo file, data-node state, or information-schema mutation |
...
| `DROP LOGFILE GROUP` | ⚪ | Accepted as an embedded no-op with warning `1105`; no NDB logfile group lookup, storage cleanup, or metadata mutation |
