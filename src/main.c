...
static int parse_single_sql(const char *sql, size_t length, int quiet, int show_tokens);
...
static int run_cli(int argc, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++) {
		if (strcmp(argv[argi], "--quiet") == 0) {
			...
			nul_batch = 1;
		} else if (strcmp(argv[argi], "--tokens") == 0) {
			show_tokens = 1;
	...
	if (nul_batch) {
		...
		rc = parse_single_sql(buffer.data, buffer.length, quiet, show_tokens);
	}
	free_buffer(&buffer);
	return rc;
}
...
static void print_tokens(const mylite_parse_result *result, const char *sql)
{
	...
	for (i = 0; i < result->token_count; i++) {
		const mylite_token *token = &result->tokens[i];
		printf("token %zu %s [%u:%u-%u:%u] %.*s\n",
		       i + 1,
		       mylite_token_kind_name(token->kind),
		       token->start_line,
		       token->start_column,
		       token->end_line,
		       token->end_column,
...
static void print_usage(const char *program)
{
	fprintf(stderr, "usage: %s [--quiet] [--nul] [--tokens] [sql...]\n", program);
}
