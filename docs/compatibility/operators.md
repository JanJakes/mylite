...
| `IS NOT NULL` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates; limited no-source/`DUAL` scalar projection and limited `DO` expression execution over admitted signed-64 scalar operands |
| `IS NULL` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates; limited no-source/`DUAL` scalar projection and limited `DO` expression execution over admitted signed-64 scalar operands |
