...
| Default collation selection | 🟡 | Limited `CREATE TABLE` acceptance of fixed default `utf8mb4` / `utf8mb4_0900_ai_ci` table options plus static `SHOW CHARACTER SET` / `SHOW COLLATION` rows only; no `SET NAMES`, `SET CHARACTER SET`, database defaults, `ALTER` changes, string semantics, or full charset/collation catalogs |
...
| `utf8mb4_0900_ai_ci` | 🟡 | Limited static `SHOW COLLATION` row and fixed `CREATE TABLE` option acceptance; utf8mb4; id 255; default; sortlen 0; NO PAD; no comparison semantics |
