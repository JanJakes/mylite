...
| --- | --- | --- |
| `SET` | 🟡 | Limited fixed no-op session/local system-variable assignments preserving MyLite's current baseline values, including selected boolean variables and `sql_mode = DEFAULT`; no mutable session state, global/persist assignment, user variables, assignment lists, `:=`, arbitrary expressions, parameters, subqueries, or SQLite pass-through |
