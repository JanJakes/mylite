...
| SQL quote SHOW CREATE system variable | 🟡 | Limited scalar reads for `@@sql_quote_show_create` expose MyLite's fixed enabled quoted-SHOW-CREATE baseline `1`; limited fixed no-op `SET` forms may preserve `1`; no mutable quote-control state, disabled rendering, or `SHOW VARIABLES`. | [runtime system variables](docs/compatibility/runtime-system-variables.md), [SQL SHOW statements](docs/compatibility/sql-show-statements.md) |
