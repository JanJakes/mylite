...
static int test_table_statement_success_and_session_state(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open table file");
    ...
    failures += expect_int64(mylite_result_affected_rows(result), 0, "table affected rows");
    failures += expect_size(mylite_result_warning_count(result), 0U, "table warning count");
...
static int test_table_statement_persistence_shadowing_rename_and_drop(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen table file");
...
static int test_independent_table_statement_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first table file");
    failures += expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second table file");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_table_statement_%d_%s.mylite",
    ...
    if (written < 0 || (size_t)written >= path_size) {
        (void)fprintf(stderr, "test path too long\n");
        return 1;
    }
    return 0;
}
