...
| `ALTER TABLESPACE` | ❌ | low | General tablespace alterations and diagnostics. | Parser recognizes rename, `ADD/DROP DATAFILE`, numeric size, `WAIT`, `'Y'`/`'N'` encryption, `ENGINE`/`STORAGE ENGINE`, and string-literal attribute clauses. |
...
| `CREATE TABLESPACE` | ❌ | low | General and NDB tablespace syntax and diagnostics. | Parser recognizes string-literal data files, numeric autoextend/file-block sizes, `'Y'`/`'N'` encryption, NDB logfile/size/nodegroup/comment options, `ENGINE`/`STORAGE ENGINE`, and string-literal engine attributes. |
