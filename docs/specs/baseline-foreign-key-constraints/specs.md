...
- `INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS` becomes queryable for supported
  FK descriptors;
- `CREATE TABLE ... LIKE` clones supported FK descriptors only when the
  referenced parent table is also cloned in a later multi-table copy slice;
  until then, `LIKE` over FK-bearing tables is rejected deterministically;
- `CREATE TABLE ... SELECT` does not copy foreign keys.

Supported enforcement:

- child-side `INSERT ... VALUES`, `INSERT ... SET`, and single-table
  `UPDATE child SET child_column = value` check non-`NULL` child values against
  the parent descriptor table before completing;
...
- `DROP PRIMARY KEY`, `DROP INDEX`, and column-rebuild `ALTER TABLE` paths
  ...
  child key column;
- enforcement is implemented with set-based SQLite probes generated by MyLite,
  not row-by-row materialization in MyLite.
...
- `REPLACE`, `INSERT ... SELECT`, `INSERT ... ON DUPLICATE KEY UPDATE`, and
  multi-table DML on FK-related tables until their FK semantics are specified;
...
Successful FK DDL and DML produces `warning_count == 0`.
