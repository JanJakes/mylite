...
| `SET TRANSACTION` | ❌ | high | Isolation level and access mode at global/local/session/next-transaction scope. | Parser records `SET [GLOBAL | LOCAL | SESSION] TRANSACTION` as transaction metadata; isolation and access-mode semantics are not implemented. |
