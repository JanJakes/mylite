...
static int test_no_source_dual_do_and_warnings(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open scalar db");
    ...
    if (result != NULL) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "digest do columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "digest do rows");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 1U, "digest do warnings");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "digest do affected");
    }
    ...
    if (result != NULL) {
        failures += mylite_test_expect_size(
            ...
            3U,
            "digest metadata columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 1U, "digest metadata rows");
...
static int test_dml_constant_digest_values(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open dml db");
    failures += execute_ok(database, "CREATE DATABASE app", NULL);
    failures += execute_ok(database, "USE app", NULL);
    failures +=
...
static int test_table_backed_digest_projection(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open table db");
    failures += execute_ok(database, "CREATE DATABASE app", NULL);
    failures += execute_ok(database, "USE app", NULL);
    failures += execute_ok(
        database,
...
static int test_digest_update_contexts(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open update db");
    failures += execute_ok(database, "CREATE DATABASE app", NULL);
    failures += execute_ok(database, "USE app", NULL);
    failures += execute_ok(
        database,
