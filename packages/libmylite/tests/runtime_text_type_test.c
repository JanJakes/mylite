...
static int test_text_success_persistence_and_introspection(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open text success file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen text success file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_text_length_arguments(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "text-length") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open text length file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_statement_ok(
        database,
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen text length file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_text_parenthesized_defaults(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "text-defaults") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open text defaults file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_statement_ok(
        database,
...
static int test_text_diagnostics(void) {
    ...
    if (result != NULL) {
        ...
        failures += mylite_test_expect_true(actual != NULL, "TEXT NUL byte result value");
        if (actual != NULL) {
            failures +=
    ...
    if (result != NULL) {
        ...
        failures +=
            mylite_test_expect_true(actual != NULL, "TEXT INSERT SELECT NUL byte result value");
        if (actual != NULL) {
            failures += expect_bytes(
                actual,
                nul_text_value,
    ...
    failures += mylite_test_expect_size(
        ...
        0U,
        "empty text default columns"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "empty text default rows");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        2U,
        "empty text default warnings"
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
...
static int test_text_literal_dml_truncation(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "dml_truncation") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_text_insert_select_dml_truncation(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "dml_insert_select_truncation") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_text_independent_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first text file");
    failures += mylite_test_expect_int(
