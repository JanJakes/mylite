...
| IGNORE warning demotion | 🟡 | Limited `INSERT IGNORE ... VALUES` / `SET` demotion for descriptor integer, `NULL`, and `DEFAULT` row inputs: explicit `NULL` into numeric `NOT NULL`, omitted no-explicit-default columns, explicit `DEFAULT` for no-default columns, and out-of-range descriptor integer inputs become warnings with adjusted values; omitted or explicit-`DEFAULT` numeric `NOT NULL` no-default columns store `0`, while omitted or explicit-`DEFAULT` nullable dropped-default columns store `NULL`; no duplicate-key discard, broader type conversion, `INSERT IGNORE ... SELECT`, `UPDATE IGNORE`, `DELETE IGNORE`, or `LOAD DATA IGNORE` demotion |
