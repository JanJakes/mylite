...
static int test_no_source_dual_and_do_unix_timestamp(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "unix_timestamp do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "unix_timestamp do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "unix_timestamp do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "unix_timestamp do warnings"
        );
    }
...
static int test_table_backed_unix_timestamp_and_reopen(void) {
    ...
    database = NULL;
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen unix_timestamp");
...
static int test_unix_timestamp_warnings_and_diagnostics(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 3U, "row warning columns");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 2U, "row warning rows");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 4U, "row warnings");
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 3U, "invalid columns");
