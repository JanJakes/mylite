...
static int test_show_full_columns_temporary_shadow_and_collation(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "temporary") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open temporary file");
    failures += create_full_columns_schema(database);
    failures += execute_statement_ok(database, "USE app");
    failures +=
...
static int test_show_full_columns_diagnostics_and_independent_handles(void) {
    ...
    if (mylite_test_make_path(first_path, sizeof(first_path), "diagnostics-a") != 0 ||
        mylite_test_make_path(second_path, sizeof(second_path), "diagnostics-b") != 0) {
        return 1;
    }
