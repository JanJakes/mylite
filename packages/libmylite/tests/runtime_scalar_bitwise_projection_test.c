...
static int test_scalar_bitwise_warnings_and_do(void) {
    ...
    failures += expect_size(mylite_result_column_count(result), 0U, "DO bitwise column count");
    failures += expect_size(mylite_result_row_count(result), 0U, "DO bitwise row count");
    failures += expect_size(mylite_result_warning_count(result), 0U, "DO bitwise warning count");
    failures += expect_int64(mylite_result_affected_rows(result), 0, "DO bitwise affected rows");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_scalar_bitwise_projection_%d_%s.mylite",
        tmpdir,
        current_process_id(),
        name
    );
