# Baseline Performance Schema Metadata Locks

## Scope

This slice adds a MySQL 8.4.9-shaped metadata placeholder for
`performance_schema.metadata_locks`.

The table is exposed as a read-only Performance Schema base table. MyLite
returns zero rows until live metadata-lock instrumentation exists. The metadata
surface follows the MySQL 8.4.9 target runtime for table name, columns,
key/index metadata, information-schema rows, `SHOW COLUMNS`, `DESCRIBE`,
`SHOW INDEX`, and `SHOW TABLE STATUS`.

## Compatibility Sources

Official MySQL 8.4 reference pages used for the feature surface:

- <https://dev.mysql.com/doc/refman/8.4/en/performance-schema-metadata-locks-table.html>
- <https://dev.mysql.com/doc/refman/8.4/en/performance-schema-table-characteristics.html>
- <https://dev.mysql.com/doc/refman/8.4/en/performance-schema-table-descriptions.html>

Expected metadata was verified against a local MySQL 8.4.9 runtime container:

```sh
docker exec -i mylite-mysql-849 mysql --protocol=TCP -h127.0.0.1 -uroot \
  --batch --raw --skip-column-names
```

The verification script for this slice is:

```sh
packages/libmylite/tests/mysql_baseline_performance_schema_metadata_locks_expectations.sh
```

## Semantics

MyLite implements `metadata_locks` as a metadata placeholder:

- `SELECT` from the table succeeds and returns an empty result set with the
  MySQL-shaped column list.
- `USE performance_schema` followed by unqualified reads resolves the table.
- `COUNT(*)` returns `0`.
- `SHOW COLUMNS`, `SHOW FULL COLUMNS`, `DESCRIBE`, and `SHOW INDEX` expose the
  MySQL 8.4.9 metadata observed from the target runtime.
- `INFORMATION_SCHEMA.COLUMNS`, `STATISTICS`, `TABLE_CONSTRAINTS`,
  `KEY_COLUMN_USAGE`, `TABLE_CONSTRAINTS_EXTENSIONS`, and `TABLES` include the
  observed metadata rows.
- `SHOW TABLE STATUS` exposes the same engine, row format, row estimate,
  auto-increment, and collation metadata as MySQL 8.4.9 for the covered fields.
- Schema, table, index, rename, truncate, and single-table DML writes targeting
  `performance_schema.metadata_locks` return MyLite's existing Performance
  Schema access-denied diagnostic.

The table exposes a HASH primary key on `OBJECT_INSTANCE_BEGIN`, a nonunique
HASH index named `OBJECT_TYPE` on
`OBJECT_TYPE, OBJECT_SCHEMA, OBJECT_NAME, COLUMN_NAME`, and a nonunique HASH
index named `OWNER_THREAD_ID` on `OWNER_THREAD_ID, OWNER_EVENT_ID`.

## Intentional Gaps

This slice does not implement live metadata-lock rows, blocking session
discovery, lock wait graph construction, owner event mapping, `KILL` behavior,
lock-state transitions, or instrumentation setup integration. A MySQL 8.4.9
runtime may expose live rows for the current server/session. MyLite
intentionally returns empty rows until it has a real metadata-lock
instrumentation pipeline and lifecycle.

The placeholder approach uses MyLite-owned catalog descriptors and runtime
dispatch. It does not require a SQLite fork hook because the feature is metadata
surface plus empty fixed rows, not SQLite planner, storage, or type behavior.

## Test Plan

- Verify the expectation script syntax with `sh -n`.
- Run the expectation script against MySQL 8.4.9 to lock observed metadata.
- Build and run `mylite_runtime_performance_schema_metadata_locks_test`.
- Run the focused CTest filter
  `^libmylite\.runtime\.performance_schema_metadata_locks$`.
- Run the broader Performance Schema runtime CTest filter.
- Run `git diff --check`, `git diff --cached --check`, formatting checks, and
  the full `cmake --workflow --preset check` before committing.
