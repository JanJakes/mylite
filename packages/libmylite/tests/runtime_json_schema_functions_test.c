...
static int test_json_schema_valid_values(void) {
    ...
    int failures =
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open values database");
...
static int test_json_schema_validation_reports(void) {
    ...
    mylite_db *database = NULL;
    int failures =
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open report database");
...
static int test_json_schema_row_backed_values(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open rows database");
...
static int test_json_schema_nulls_user_variables_and_do(void) {
    ...
    mylite_db *database = NULL;
    int failures =
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open variables database");
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "json schema do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "json schema do rows");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "json schema do rows");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "json schema do warnings"
        );
    }
