...
static int test_temporary_index_shadowing_cleanup_and_handles(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "shadow") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &first), MYLITE_OK, "open first shadow handle");
    failures += seed_schema(first);
    failures +=
        mylite_test_expect_int(mylite_open(path, &second), MYLITE_OK, "open second shadow handle");
    ...
    failures += mylite_test_expect_int(mylite_open(path, &first), MYLITE_OK, "reopen shadow file");
...
static int seed_schema(mylite_db *database) {
    ...
    failures += mylite_test_expect_int(rc, MYLITE_OK, "CREATE DATABASE IF NOT EXISTS app");
    if (rc != MYLITE_OK) {
