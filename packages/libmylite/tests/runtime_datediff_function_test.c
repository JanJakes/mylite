...
static int test_no_source_dual_and_do_datediff(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "datediff do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "datediff do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "datediff do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "datediff do warnings");
    }
...
static int test_table_backed_datediff_and_reopen(void) {
    ...
    if (failures == 0) {
        ...
        failures += expect_size(mylite_result_warning_count(result), 1U, "table warnings");
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 1U, "text warning columns");
        failures += expect_size(mylite_result_row_count(result), 1U, "text warning rows");
        failures += expect_size(mylite_result_warning_count(result), 1U, "text warning count");
        failures += expect_text(
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen datediff database");
...
static int test_datediff_warnings_and_diagnostics(void) {
    ...
    if (failures == 0) {
        ...
        failures += expect_size(mylite_result_warning_count(result), 4U, "zero warnings");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-datediff-function-%s-%d.mylite",
        name,
        current_process_id()
    );
