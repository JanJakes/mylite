...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `CREATE TABLE` | 🟡 | Limited persistent base-table creation with optional `IF NOT EXISTS`, explicit integer-family columns (`TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`/`INTEGER`, `BIGINT`) and the `INT1`/`INT2`/`INT3`/`INT4`/`INT8` aliases, optional single `SIGNED` or `UNSIGNED`, `NULL`/`NOT NULL`, optional explicit `ENGINE [=] InnoDB`, and optional fixed default `utf8mb4` / `utf8mb4_0900_ai_ci` table charset/collation options; existing-table `IF NOT EXISTS` is a no-op with a MySQL-compatible note; no other options, keys, defaults, or temporary tables. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| Column definition grammar | 🟡 | Limited integer column descriptors, `INT1`/`INT2`/`INT3`/`INT4`/`INT8` aliases, optional single `SIGNED` or `UNSIGNED`, and nullability only. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `TINYINT` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion with MyLite's current descriptor-range literal check, and single-column sort support; no display width, `ZEROFILL`, repeated/combined attributes, aliases, general expression semantics, compact physical storage, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| `SMALLINT` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion with MyLite's current descriptor-range literal check, and single-column sort support; no display width, `ZEROFILL`, repeated/combined attributes, aliases, general expression semantics, compact physical storage, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| `MEDIUMINT` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion with MyLite's current descriptor-range literal check, and single-column sort support; no display width, `ZEROFILL`, repeated/combined attributes, aliases, general expression semantics, compact physical storage, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| `INT` / `INTEGER` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion, and single-column sort support; no general expression semantics, display width, repeated/combined attributes, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| `BIGINT` | 🟡 | Limited DDL descriptors, optional single explicit `SIGNED` or `UNSIGNED`, `ALTER TABLE ... MODIFY [COLUMN]` and `CHANGE [COLUMN]` descriptor replacement, integer/`NULL` `INSERT` and `UPDATE` assignment, text readback, descriptor-driven filtered `SELECT`/`DELETE`/`UPDATE` predicate conversion, and single-column sort support; `BIGINT UNSIGNED` is capped at the signed 64-bit SQLite integer range in this slice; no display width, `ZEROFILL`, repeated/combined attributes, or protocol-grade result metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| Integer type aliases | 🟡 | Limited `INT1`, `INT2`, `INT3`, `INT4`, and `INT8` aliases normalize to existing integer-family descriptors, with optional single `SIGNED` or `UNSIGNED`, descriptor-driven DDL/DML/conversion/introspection coverage, and normalized type rendering; no `BOOL`, `BOOLEAN`, `SERIAL`, display width, `ZEROFILL`, combined/repeated attributes, casts, or protocol-grade metadata. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
