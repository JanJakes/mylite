...
static int test_information_schema_innodb_metrics_queries(void) {
    ...
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open metrics db");
...
static int expect_columns_metadata(mylite_db *database) {
    ...
    failures += mylite_test_expect_int(rc, MYLITE_OK, "metrics columns metadata");
    if (rc != MYLITE_OK) {
