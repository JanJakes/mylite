...
| Last insert id | 🟡 | Limited connection-local unsigned 64-bit state for the current persistent and session-temporary auto-increment insert subset plus literal `LAST_INSERT_ID(expr)` scalar/`DO` calls; manual values are not persisted and are overwritten by later successful generated inserts; no table-backed expression evaluation, protocol insert-id packets, C API state, mixed-mode allocation parity, or stored-program behavior |
