...
static int test_mysql_servers_table(void);
...
static const char *const servers_columns[servers_column_count] = {
    "Server_name",
    "Host",
    "Db",
    "Username",
...
int main(void) {
    return test_mysql_servers_table() == 0 ? 0 : 1;
}

static int test_mysql_servers_table(void) {
    enum {
        show_columns_row_count = 9,
    ...
    static const char *const show_columns_values[] = {
        "Server_name", "char(64)", "NO", "PRI", "", "", "Host",     "char(255)", "NO", "", "",  "",
        "Db",          "char(64)", "NO", "",    "", "", "Username", "char(64)",  "NO", "", "",  "",
        "Password",    "char(64)", "NO", "",    "", "", "Port",     "int",       "NO", "", "0", "",
        "Socket",      "char(64)", "NO", "",    "", "", "Wrapper",  "char(64)",  "NO", "", "",  "",
        "Owner",       "char(64)", "NO", "",    "", "",
    };
    ...
    static const char *const show_host_values[] = {
        "Host",
        "char(255)",
        "ascii_general_ci",
        "NO",
    ...
    if (make_test_path(path, sizeof(path), "mysql-servers-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT Server_name, Host, Db, Username, Password, Port, Socket, Wrapper, Owner "
                   "FROM mysql.servers ORDER BY Server_name",
            .column_names = servers_columns,
            .column_count = servers_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.servers direct empty read",
        }
    );
    failures += expect_row_count_status(database, "row count after mysql.servers read");
    failures += expect_query(
        database,
        (struct expected_query){
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.servers unqualified count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.servers",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.servers show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.servers",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_values,
            .row_count = show_columns_row_count,
            .context = "mysql.servers show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.servers WHERE Field = 'Host'",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_host_values,
            .row_count = 1U,
            .context = "mysql.servers show full columns where",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.servers WHERE Key_name = 'PRIMARY'",
            ...
            .context = "mysql.servers show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_columns_row_count,
            .context = "mysql.servers information_schema.columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.servers information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.servers information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.servers information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.servers information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.servers information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'servers' "
            ...
            .row_count = 1U,
            .context = "mysql.servers show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_servers_table_%d_%s.mylite",
        current_process_id(),
        name
    );
