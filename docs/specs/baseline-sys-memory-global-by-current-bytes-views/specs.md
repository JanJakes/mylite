# Baseline sys.memory_global_by_current_bytes Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.memory_global_by_current_bytes` and
`sys.x$memory_global_by_current_bytes` views. MySQL uses these sys views to
show global Performance Schema memory counters by event name. MyLite does not
collect Performance Schema memory-summary rows, so both views are
metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.memory_global_by_current_bytes` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_memory_global_by_current_bytes_views_expectations.sh`.
...
SELECT * FROM `x$memory_global_by_current_bytes`;
```
...
surface without inventing memory event names or allocation counters.

## Column Metadata

`sys.memory_global_by_current_bytes` has seven columns:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `event_name` | `varchar(128)` | `NO` | `NULL` | `utf8mb4_0900_ai_ci` |
| `current_count` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `current_alloc` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `current_avg_alloc` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `high_count` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `high_alloc` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `high_avg_alloc` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |

`sys.x$memory_global_by_current_bytes` has the same logical columns except that
byte totals expose raw numeric metadata:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `event_name` | `varchar(128)` | `NO` | `NULL` | `utf8mb4_0900_ai_ci` |
| `current_count` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `current_alloc` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `current_avg_alloc` | `decimal(23,4)` | `NO` | `0.0000` | SQL `NULL` |
| `high_count` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `high_alloc` | `bigint` | `NO` | `NULL` | SQL `NULL` |
| `high_avg_alloc` | `decimal(23,4)` | `NO` | `0.0000` | SQL `NULL` |

`SHOW COLUMNS`, `SHOW FULL COLUMNS`, `DESCRIBE`, and
`INFORMATION_SCHEMA.COLUMNS` expose this shape, including formatted
`varchar(11)` byte strings, signed `BIGINT` numeric precision, and raw
`decimal(23,4)` averages. The views have no index or constraint metadata, so
...
`ALGORITHM=MERGE` and the `mysql.sys@localhost` definer. The formatted view uses
`format_bytes()` for current and high memory byte counters; the raw `x$` view
exposes the corresponding numeric counters. Both definitions filter to
`CURRENT_NUMBER_OF_BYTES_USED > 0` and order by descending current bytes.
...
- memory instrumentation, current allocation counters, high-water allocation
  counters, or event-name aggregation;
- formatted byte calculation through `format_bytes()`;
- execution of the expressions through the built-in view definition;
