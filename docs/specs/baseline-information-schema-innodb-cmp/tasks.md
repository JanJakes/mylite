# Baseline INFORMATION_SCHEMA InnoDB CMP Tables Tasks

- [x] Verify MySQL 8.4.9 `INNODB_CMP` and `INNODB_CMP_RESET` table shape,
  metadata, baseline page-size rows, warning count, and `ROW_COUNT()`
  behavior.
- [x] Record official MySQL 8.4 documentation sources and runtime evidence in
  `specs.md`.
- [x] Add MySQL expectation coverage for both InnoDB compression
