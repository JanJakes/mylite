...
static int test_temporary_create_table_select_source_shadowing_and_lifetime(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "lifetime") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &first), MYLITE_OK, "open first lifetime handle");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &first), MYLITE_OK, "reopen lifetime file");
    failures += expect_statement(first, "USE app", (struct expected_statement){0, 0U});
    failures += expect_error(
        first,
        "SELECT id FROM shadow_copy",
...
static int expect_statement(
    ...
    if (rc == MYLITE_OK) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "statement columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "statement rows");
...
static int expect_query_contains(
    ...
    if (rc == MYLITE_OK) {
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result) > row ? 1U : 0U, 1U, context);
        failures += mylite_test_expect_size(
            mylite_result_column_count(result) > column ? 1U : 0U,
            1U,
            context
        );
