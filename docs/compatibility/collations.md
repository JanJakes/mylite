...
| `latin1_swedish_ci` | 🟡 | Metadata catalog row plus limited descriptor/session readback, scalar `CONVERT(value USING latin1)` default metadata, and scalar postfix `COLLATE latin1_swedish_ci` validation for ASCII scalar values; latin1; id 8; default; sortlen 1; PAD SPACE; no non-ASCII transcoding, storage, or comparison semantics |
