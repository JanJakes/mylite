...
static int test_no_source_dual_and_do_char(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "char do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "char do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "char do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "char do warnings");
    }
...
static int test_table_backed_char_and_reopen(void) {
    ...
    mylite_close(database);
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen char file");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written =
        snprintf(path, path_size, "runtime_char_function_%s_%d.mylite", name, current_process_id());
