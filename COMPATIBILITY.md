...
| `WHERE` | 🟡 | One descriptor column predicate on supported integer/`NULL` columns for filtered table `SELECT`, `DELETE`, and `UPDATE`, with `SELECT` supporting source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands; no `WHERE TRUE`, boolean composition, literal-left comparisons, `IS TRUE`, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
