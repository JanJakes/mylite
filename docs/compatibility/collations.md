...
| Binary collations | 🟡 | Limited static metadata row for `binary`, explicit column-level `COLLATE binary` normalization to binary string/BLOB descriptors, and metadata admission plus session/table/column default preservation for `utf8mb4_bin` and `utf8mb4_0900_bin`; no binary comparison, table-default binary inheritance, `NO PAD` trailing-space comparison, or ordering semantics |
...
| `binary` | 🟡 | Limited static catalog row and explicit column-level normalization for admitted `CHAR`, `VARCHAR`, and bare `TEXT` family definitions; binary; id 63; default; sortlen 1; NO PAD; no table-default inheritance or binary comparison semantics |
