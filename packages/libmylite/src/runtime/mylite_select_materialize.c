                                        ...
                                        const struct mylite_select_eval_callbacks *callbacks)
{
    ...
    while ((rc = sqlite3_step(stmt->sqlite_stmt)) == SQLITE_ROW) {
        ...
        status = evaluate_table_select_row_matches(stmt, &row, &matches, callbacks);
        if (status != MYLITE_OK) {
                                         ...
                                         const struct mylite_select_eval_callbacks *callbacks)
{
    ...
    *out_matches = false;
    ...
    status = copy_select_base_table_row_values(stmt->database, &candidate, right_table,
                                               ...
                                               rows->right_index);
    if (status == MYLITE_OK) {
        status = evaluate_table_select_join_step_conditions(stmt, state, &candidate, step,
    ...
    const struct mylite_select_eval_callbacks *callbacks)
{
    ...
    status = evaluate_table_select_join_stage_conditions(stmt, state, scan, &matches, callbacks);
    if (status != MYLITE_OK) {
        return status;
    }
    if (!matches) {
