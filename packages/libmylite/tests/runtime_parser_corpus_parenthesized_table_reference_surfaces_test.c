...
static int test_parenthesized_table_reference_surfaces(void);
...
int main(void) {
    return test_parenthesized_table_reference_surfaces() == 0 ? 0 : 1;
}

static int test_parenthesized_table_reference_surfaces(void) {
    static const char *const flat_join_rows[] = {"1", "10", "100"};
    ...
    failures += execute_ok(database, "CREATE TABLE t1 (id INT PRIMARY KEY, v INT)");
    failures += execute_ok(database, "CREATE TABLE t2 (id INT PRIMARY KEY, v INT)");
    failures += execute_ok(database, "CREATE TABLE t3 (id INT PRIMARY KEY, v INT)");
    failures += execute_ok(database, "INSERT INTO t1 VALUES (1, 10), (2, 20)");
    failures += execute_ok(database, "INSERT INTO t2 VALUES (1, 100), (3, 300)");
    failures += execute_ok(database, "INSERT INTO t3 VALUES (1, 1000), (3, 3000)");

    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT t1.id, t1.v, t2.v FROM t1 JOIN t2 ON t1.id = t2.id",
            .values = flat_join_rows,
            .column_count = 3U,
            .row_count = 1U,
            .context = "existing flat join support",
        }
    );
    ...
    failures += execute_error(
        database,
        "SELECT * FROM t1 LEFT JOIN (t2 LEFT JOIN t3 ON t2.id = t3.id) "
        "ON t1.id = t2.id",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
    ...
    failures += execute_error(
        database,
        "SELECT * FROM t1 LEFT JOIN (t2, t3) ON t1.id = t2.id",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "utility statement is not supported",
    ...
    failures += execute_error(
        database,
        "SELECT * FROM t1, t2 LEFT JOIN t3 ON t2.id = t3.id",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "utility statement is not supported",
    ...
    failures += execute_error(
        database,
        "SELECT * FROM t2 LEFT JOIN t3 ON t2.id = t3.id, t1",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "utility statement is not supported",
