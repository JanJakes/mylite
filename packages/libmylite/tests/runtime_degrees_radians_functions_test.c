...
static int test_angle_values_and_file_safety(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open angle values file");
...
static int test_angle_warnings_do_and_independent_handles(void) {
    ...
    if (result != NULL) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "angle DO column count"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "angle DO row count");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            2U,
            "angle DO warning count"
        );
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "angle DO affected rows"
        );
    }
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first angle file handle"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second angle file handle"
    );
    failures += expect_query(
        first,
        (struct expected_query){
