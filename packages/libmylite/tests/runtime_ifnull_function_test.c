...
enum {
    ...
    nonnull_column_count = 6,
    fallback_column_count = 4,
    label_column_count = 6,
...
static int test_ifnull_function_values_and_file_safety(void);
static int test_ifnull_function_unsupported_forms(void);
static int test_ifnull_function_independent_handles(void);
...
int main(void) {
    ...
    failures += test_ifnull_function_values_and_file_safety();
    failures += test_ifnull_function_unsupported_forms();
    failures += test_ifnull_function_independent_handles();
...
static int test_ifnull_function_values_and_file_safety(void) {
    ...
    static const char *const nonnull_values[] = {"1", "0", "0", "1", "0", "-1"};
    ...
    static const char *const fallback_values[] = {"10", NULL, "1", "0"};
    static const char *const label_columns[] = {
        "chosen",
        "fallback",
        "IFNULL (NULL,10)",
        "(IFNULL(NULL,10))",
        "IFNULL(IFNULL(NULL,1),2)",
        "IFNULL(IF(0,NULL,4),5)",
    };
    static const char *const label_values[] = {"1", "3", "10", "10", "1", "4"};
    static const char *const cross_columns[] = {"IF(IFNULL(NULL,0),1,2)"};
    static const char *const cross_values[] = {"2"};
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT IFNULL(1,0), IFNULL(0,2), IFNULL(FALSE,2), IFNULL(TRUE,2), "
                   "IFNULL(+0,2), IFNULL(-1,2)",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT ALL IFNULL(NULL,10), IFNULL(NULL,NULL), IFNULL(NULL,TRUE), "
                   "IFNULL(NULL,FALSE) FROM DUAL",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT IFNULL(1,2) AS chosen, IFNULL(NULL,3) fallback, "
                   "IFNULL (NULL,10), (IFNULL(NULL,10)), IFNULL(IFNULL(NULL,1),2), "
                   "IFNULL(IF(0,NULL,4),5)",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT IF(IFNULL(NULL,0),1,2)",
            .columns = cross_columns,
            .column_count = 1U,
            .values = cross_values,
            .row_count = 1U,
            .context = "nested IFNULL in IF",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT IFNULL(0001,0002) normalized, IFNULL(NULL,-0002) "
                   "fallback_negative, IFNULL(-0001,0002) negative, "
                   "IFNULL(9223372036854775807,0) max_value",
    ...
    failures += execute_ok(database, "SELECT IFNULL(NULL,10)", &result);
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "row count after IFNULL select",
        }
    );
    ...
    failures += expect_int64(
        ...
        "IFNULL select leaves catalog generation unchanged"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "IFNULL select leaves sqlite schema generation unchanged"
    );
    ...
    failures += expect_int(
        ...
        0,
        "read IFNULL preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "IFNULL select leaves preamble unchanged"
    );
...
static int test_ifnull_function_unsupported_forms(void) {
    ...
    failures += execute_ok(database, "CREATE TABLE ifnull(ifnull INT)", NULL);
    failures += execute_ok(database, "INSERT INTO ifnull VALUES (7)", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT ifnull FROM ifnull",
            .columns = identifier_columns,
            .column_count = 1U,
            .values = identifier_values,
            .row_count = 1U,
            .context = "IFNULL keyword identifiers",
        }
    );
    ...
    failures += execute_error(
        database,
        "SELECT IFNULL(9223372036854775808,0)",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SELECT IFNULL() supports only signed 64-bit integer",
    ...
    failures += execute_error(
        database,
        "SELECT IFNULL(1,9223372036854775808)",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SELECT IFNULL() supports only signed 64-bit integer",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id FROM t WHERE id = IFNULL(1,0)",
            .columns = (const char *const[]){"id"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "IFNULL column predicate RHS",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id FROM t ORDER BY IFNULL(id,0)",
            .columns = (const char *const[]){"id"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "IFNULL order expression",
        }
    );
    failures += execute_error(
        database,
        "SELECT id, COUNT(*) FROM t GROUP BY IFNULL(id,0)",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
    ...
    failures += execute_error(
        database,
        "SELECT id, COUNT(*) FROM t GROUP BY id HAVING IFNULL(1,0)",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "utility statement is not supported",
...
static int test_ifnull_function_independent_handles(void) {
    ...
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT IFNULL(NULL,10) AS first_result",
            .columns = first_columns,
            .column_count = 1U,
            .values = first_values,
            .row_count = 1U,
            .context = "first handle IFNULL",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            .sql = "SELECT IFNULL(20,0) AS second_result",
            .columns = second_columns,
            .column_count = 1U,
            .values = second_values,
            .row_count = 1U,
            .context = "second handle IFNULL",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-ifnull-function-%s-%d.mylite",
        name,
        current_process_id()
    );
