...
static int test_bit_count_values_and_file_safety(void);
static int test_bit_count_warnings_and_do(void);
static int test_bit_count_errors_and_unsupported_forms(void);
static int test_bit_count_independent_handles(void);
...
int main(void) {
    ...
    failures += test_bit_count_values_and_file_safety();
    failures += test_bit_count_warnings_and_do();
    failures += test_bit_count_errors_and_unsupported_forms();
    failures += test_bit_count_independent_handles();
...
static int test_bit_count_values_and_file_safety(void) {
    ...
    static const char *const dual_values[] = {"1", "64", "1", "0", "1", "3"};
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT BIT_COUNT(NULL),BIT_COUNT(0),BIT_COUNT(1),BIT_COUNT(64),"
                   "BIT_COUNT(127),BIT_COUNT(TRUE),BIT_COUNT(FALSE),BIT_COUNT(+1),"
                   "BIT_COUNT(-0),BIT_COUNT(-1),BIT_COUNT(-2),"
                   "BIT_COUNT(-9223372036854775808),"
                   "BIT_COUNT(9223372036854775807),"
                   "BIT_COUNT(9223372036854775808),"
                   "BIT_COUNT(18446744073709551615)",
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "core bit_count values",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT BIT_COUNT(5&3) AS a,BIT_COUNT(~0) b,BIT_COUNT(1<<63) c,"
                   "BIT_COUNT(1<<64) d,BIT_COUNT(5 DIV 2) e,"
                   "BIT_COUNT(IFNULL(NULL,7)) f FROM DUAL",
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "dual bit_count operands",
        }
    );
    ...
    failures += expect_int64(
        ...
        "bit_count catalog generation unchanged"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "bit_count sqlite schema generation unchanged"
    );
...
static int test_bit_count_warnings_and_do(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT BIT_COUNT(5 DIV 0),@@warning_count,ROW_COUNT()",
            ...
            .warning_count = 1U,
            .affected_rows = 0,
            .context = "bit_count select warning",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DO BIT_COUNT(64),BIT_COUNT(NULL),BIT_COUNT(-1)",
            .columns = NULL,
            .column_count = 0U,
            .values = NULL,
            .row_count = 0U,
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "bit_count do no warning",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "bit_count do count snapshot",
        }
    );

    failures += execute_ok(database, "DO BIT_COUNT(5 DIV 0)", &result);
    if (result != NULL) {
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "bit_count do warning snapshot",
        }
    );
...
static int test_bit_count_errors_and_unsupported_forms(void) {
    ...
    failures += execute_error(
        database,
        "SELECT CASE WHEN TRUE THEN BIT_COUNT(7) END",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
    ...
    failures += execute_ok(database, "DO BIT_COUNT(@@warning_count)", NULL);
...
static int test_bit_count_independent_handles(void) {
    static const char *const first_columns[] = {"BIT_COUNT(~0)"};
    static const char *const first_values[] = {"64"};
    static const char *const second_columns[] = {"BIT_COUNT(1<<63)"};
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "first handle bit_count",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            .sql = "SELECT BIT_COUNT(1<<63)",
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "second handle bit_count",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_bit_count_function_%d_%s.mylite",
        tmpdir,
        current_process_id(),
        name
    );
