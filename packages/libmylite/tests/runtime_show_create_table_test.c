...
static int test_show_create_descriptor_failure_paths(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "descriptor-failures") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open descriptor database");
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .message_part =
                ...
                "approximate numeric, YEAR, DATE, TIME, DATETIME, and TIMESTAMP column "
                "descriptors",
        }
    );
    failures += execute_error(
