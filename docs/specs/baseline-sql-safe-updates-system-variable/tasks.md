...
safe-updates baseline: `@@sql_safe_updates`.
...
2. Runtime resolver
   - Add `sql_safe_updates` to the existing system-variable resolver.
...
5. Compatibility docs
   ...
   - Do not claim mutable safe-updates state, `SET`, `sql_select_limit`,
     `max_join_size`, key-aware DML checks, changed `UPDATE`/`DELETE`
     behavior, or `SHOW VARIABLES`.
...
- Do not implement `SET`, startup options, persisted variables, mysql client
  `--safe-updates` initialization, safe-updates mode, key descriptors,
  key-aware DML checks, `sql_select_limit`, `max_join_size`, `SHOW VARIABLES`,
