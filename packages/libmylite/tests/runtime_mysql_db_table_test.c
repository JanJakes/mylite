...
static int test_mysql_db_table(void);
...
static const char *const mysql_db_columns[mysql_db_column_count] = {
    "Host",
    "Db",
    "User",
    "Select_priv",
...
int main(void) {
    return test_mysql_db_table() == 0 ? 0 : 1;
}

static int test_mysql_db_table(void) {
    enum {
        show_index_row_count = 4,
        information_schema_columns_sample_column_count = 5,
    ...
    static const char *const show_full_columns_select_values[] = {
        "Select_priv",
        "enum('N','Y')",
        "utf8mb3_general_ci",
        "NO",
    ...
    static const char *const show_index_values[] = {
        ...
        "db", "0", "PRIMARY", "2", "User", "A", "2", NULL, NULL, "", "BTREE", "", "", "YES", NULL,
        "db", "0", "PRIMARY", "3", "Db",   "A", "2", NULL, NULL, "", "BTREE", "", "", "YES", NULL,
        "db", "1", "User",    "1", "User", "A", "2", NULL, NULL, "", "BTREE", "", "", "YES", NULL,
    };
    ...
    static const char *const information_schema_columns_sample_values[] = {
        "Host",         "1",  "",  "PRI", "ascii_general_ci",
        "Db",           "2",  "",  "PRI", "utf8mb3_bin",
        "User",         "3",  "",  "PRI", "utf8mb3_bin",
        "Trigger_priv", "22", "N", "",    "utf8mb3_general_ci",
    };
    ...
    static const char *const key_column_usage_values[] = {
        "PRIMARY", "Host", "1",  NULL,      NULL, NULL, NULL, "PRIMARY", "User", "2",  NULL,
        NULL,      NULL,   NULL, "PRIMARY", "Db", "3",  NULL, NULL,      NULL,   NULL,
    };
    ...
    static const char *const statistics_values[] = {
        ...
        "1", "User",    "1", "User", "A", "2", NULL, NULL, "", "BTREE", "", "", "YES", NULL,
    };
    ...
    if (make_test_path(path, sizeof(path), "mysql-db-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .column_names = mysql_db_columns,
            .column_count = mysql_db_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.db direct empty read",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.db",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.db count",
        }
    );
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "selected mysql.db count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = NULL,
            .row_count = mysql_db_column_count,
            .context = "mysql.db describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.db LIKE 'Host'",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_host_values,
            .row_count = 1U,
            .context = "mysql.db show columns host",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.db WHERE Field = 'Select_priv'",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_select_values,
            .row_count = 1U,
            .context = "mysql.db show full columns privilege",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.db",
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.db information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, COLUMN_KEY, "
                   "COLLATION_NAME FROM INFORMATION_SCHEMA.COLUMNS "
                   "WHERE TABLE_SCHEMA = 'mysql' AND TABLE_NAME = 'db' "
                   "AND COLUMN_NAME IN ('Host', 'Db', 'User', 'Trigger_priv') "
            ...
            .row_count = 4U,
            .context = "mysql.db information_schema.columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.db information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 3U,
            .context = "mysql.db information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT CONSTRAINT_NAME, TABLE_NAME, ENGINE_ATTRIBUTE, "
                   ...
                   "WHERE CONSTRAINT_SCHEMA = 'mysql' AND TABLE_NAME = 'db' "
            ...
            .row_count = 2U,
            .context = "mysql.db information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_index_row_count,
            .context = "mysql.db information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.db information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'db'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.db show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_db_table_%d_%s.mylite",
        current_process_id(),
        name
    );
