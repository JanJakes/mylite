...
| `CREATE TRIGGER` | ❌ | high | Trigger timing, event, ordering, body, definer, and metadata. | Parser recognizes trigger timing, event, target table, optional ordering, definer forms, and stored-program statement starts for single-statement trigger bodies, routing recognized body starts including flow-control conditions through existing statement validators. |
...
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser recognizes parenthesized query payloads and validates their outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, plus `ON DUPLICATE KEY UPDATE` continuations. |
...
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser recognizes parenthesized query payloads and validates their outer `ORDER BY`/`LIMIT` tails plus malformed set-operation `SELECT` operands. |
...
| `GET DIAGNOSTICS` | ❌ | medium | Current and stacked diagnostics retrieval. | Parser recognizes optional `CURRENT`/`STACKED` diagnostics-area selectors, diagnostics item lists, local/user-variable assignment targets, and literal/simple, user-variable, system-variable, or dotted identifier condition numbers. |
