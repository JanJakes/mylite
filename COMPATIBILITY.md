...
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser recognizes direct and parenthesized query payloads, validates direct payload `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized payload `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails plus outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, plus `ON DUPLICATE KEY UPDATE` continuations. |
| `INSERT ... ON DUPLICATE KEY UPDATE` | ❌ | top | Conflict target resolution, VALUES()/row aliases, affected rows, and warnings. | Parser validates the `ON DUPLICATE KEY UPDATE` keyword chain, non-empty comma-separated assignment lists, top-level assignment value adjacent operands and dangling operators, adjacent operands, dangling operators, and trailing separators inside plain parenthesized assignment value groups, and malformed post-value continuations while preserving `VALUES()` assignment expressions and nested subqueries. |
...
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser recognizes direct and parenthesized query payloads and validates direct payload `SELECT` list tails, parenthesized payload `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails plus outer `ORDER BY`/`LIMIT` tails, and malformed set-operation `SELECT` operands. |
