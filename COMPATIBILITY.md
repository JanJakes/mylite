...
| `ALTER TABLE` | 🟡 | top | Full table rebuild/in-place/instant surface; see section 3.2. | Column operations are implemented for existing MyLite base tables: `ADD`, `DROP`, `RENAME COLUMN`, `CHANGE`, and `MODIFY`, including multi-action atomic shadow rewrites, `FIRST`/`AFTER`, default backfill, column/index catalog rewrites, duplicate-index warnings, and non-default `ALGORITHM`/`LOCK` rejection before mutation. Key operations are implemented for metadata-backed `ADD PRIMARY KEY`, `ADD UNIQUE`, `ADD INDEX` / `ADD KEY`, `DROP PRIMARY KEY`, `DROP INDEX` / `DROP KEY`, `RENAME INDEX` / `RENAME KEY`, and `ALTER INDEX` visibility, including existing-row primary/unique validation, generated index names, warnings, duplicate-key write-path effects, and mixed column/key atomic rewrites. Table rename is implemented for supported base tables through `ALTER TABLE ... RENAME`, `RENAME TO`, and `RENAME AS`, including selected-schema/schema-qualified resolution, cross-schema moves, physical table renames, table/column/index catalog rewrites, and deterministic unsupported diagnostics for mixed rename actions. `FULLTEXT`, `SPATIAL`, CHECK, and foreign-key alter actions parse but return deterministic unsupported diagnostics before mutation. Generated-column dependencies, foreign-key dependencies, temporary-table rename, physical optimizer indexes, implicit commits, and remaining ALTER surfaces are deferred. See [ALTER TABLE column operations spec](docs/specs/alter-table-column-operations/specs.md), [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md), and [RENAME TABLE spec](docs/specs/rename-table/specs.md). |
...
| `CREATE INDEX` | 🟡 | top | Standalone index creation over MySQL index types and attributes. | Implemented for metadata-backed ordinary and unique standalone indexes over supported base tables, including schema resolution, duplicate-name and missing-column diagnostics, existing-row unique validation, prefix/order metadata, `INFORMATION_SCHEMA.STATISTICS` rows, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` rows for unique indexes, `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` rows for unique index parts, write-path conflict effects for `INSERT`, ODKU, `REPLACE`, and `UPDATE`, warning 3502 for `USING HASH`, and warning 1831 for redundant indexes. Full-text, spatial, functional, multi-valued, optimizer, physical-index, implicit-commit, and full storage-engine validation surfaces remain deferred. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md), [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md), and [INFORMATION_SCHEMA.KEY_COLUMN_USAGE spec](docs/specs/information-schema-key-column-usage/specs.md). |
...
| `DROP INDEX` | 🟡 | top | Standalone index removal semantics. | Implemented for `DROP INDEX name ON table` and `ALTER TABLE ... DROP INDEX` / `DROP KEY` against supported base tables, including schema/table/index validation, metadata cleanup from the internal index catalog, `INFORMATION_SCHEMA.STATISTICS`, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS`, and `INFORMATION_SCHEMA.KEY_COLUMN_USAGE`, and removal from later unique-conflict checks. Primary-key dependency checks, foreign-key dependency checks, implicit commits, and physical-index cleanup remain deferred. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md), [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md), [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md), and [INFORMATION_SCHEMA.KEY_COLUMN_USAGE spec](docs/specs/information-schema-key-column-usage/specs.md). |
...
| `RENAME TABLE` | 🟡 | top | Atomic multi-table rename semantics. | First executable slice is implemented for supported MyLite base tables, including multi-pair parser/AST coverage, selected-schema and schema-qualified name resolution, cross-schema moves, left-to-right validation, existing-target/missing-source/missing-schema diagnostics, physical SQLite table renames, table/column/index catalog rewrites, `INFORMATION_SCHEMA.TABLES`, `COLUMNS`, and `STATISTICS` updates, and all-or-nothing statement atomicity. Temporary tables, views, triggers, foreign keys, privilege checks, metadata locks, and implicit commits remain deferred. See [RENAME TABLE spec](docs/specs/rename-table/specs.md). |
| `TRUNCATE TABLE` | 🟡 | top | DDL-like truncate, auto-increment reset, implicit commit, and foreign-key restrictions. | First executable slice is implemented for supported MyLite base tables, including parser/AST coverage, selected-schema and schema-qualified resolution, physical row removal, metadata/index preservation, `AUTO_INCREMENT` reset, affected rows `0`, deterministic diagnostics, and statement atomicity. Temporary tables, foreign-key restrictions, triggers, locks, partitions, privileges, Performance Schema summary-table behavior, and implicit commits remain deferred. See [TRUNCATE TABLE spec](docs/specs/truncate-table/specs.md). |
...
| `DELETE` (single-table) | 🟡 | top | Single-table delete with aliases, partitions, ORDER BY, LIMIT, LOW_PRIORITY, QUICK, and IGNORE. | Executable for one user base table created by the supported `CREATE TABLE` subset, including selected-schema and schema-qualified target resolution, aliases, `WHERE`, `ORDER BY`, `LIMIT row_count`, Task 25 CASE expression predicates/order keys, physical deletion, affected rows, deterministic diagnostics, rollback, default strict-mode expression warning promotion, and `AUTO_INCREMENT` sequence preservation. `LOW_PRIORITY`, `QUICK`, `IGNORE`, partitions, CTEs, subqueries, broad expressions beyond the supported scalar subset, trigger/foreign-key/generated-column effects, and full SQL-mode warning-demotion behavior remain deferred. See [single-table DELETE spec](docs/specs/delete-single-table/specs.md) and [CASE expression spec](docs/specs/case-expression/specs.md). |
| `DELETE` (multi-table) | 🟡 | high | Multi-table delete forms using FROM and USING, join semantics, and affected rows. | Implemented for supported base-table join sources using `DELETE target_list FROM ...` and `DELETE FROM target_list USING ...`, including target alias resolution, joined-row matching, delete-each-target-row-once semantics, multi-target affected rows, statement rollback, and MySQL-compatible rejection of `ORDER BY`/`LIMIT` in multi-table form. Partitions, CTEs, triggers, foreign keys, temporary tables, and full multi-table expression coverage remain deferred. See [multi-table DELETE spec](docs/specs/delete-multi-table/specs.md). |
...
| `REPLACE ... VALUES` | 🟡 | top | Delete-then-insert semantics, cascades, triggers, affected rows, and auto-increment behavior. | Implemented for supported MyLite base tables with optional `INTO`, optional/empty column lists, `VALUE`, `VALUES ROW(...)`, all-default rows, schema-qualified targets, supported scalar source expressions and functions, explicit primary/unique conflict scanning, delete-all-conflicting-rows behavior, source-order multi-row replacement, nullable unique parts not conflicting, affected rows as inserted plus deleted rows, statement rollback on fatal errors, and current `INSERT` auto-increment/last-insert-id semantics. Query-source replace, partitions, triggers, foreign keys, generated columns, broad conversion fidelity, and protocol OK-info formatting remain deferred. See [REPLACE spec](docs/specs/replace/specs.md) and [INSERT/REPLACE scalar expression values spec](docs/specs/insert-scalar-expression-values/specs.md). |
| `REPLACE ... SET` | 🟡 | high | SET-form replace semantics. | Implemented for supported MyLite base tables with qualified and unqualified assignment targets, candidate-row defaults/order semantics reused from `INSERT ... SET`, supported scalar source expressions and assignment-order references, required-column validation before conflict deletes, explicit delete-then-insert conflict handling, affected rows, rollback, and auto-increment/last-insert-id behavior. Query-source replace and advanced table features remain deferred. See [REPLACE spec](docs/specs/replace/specs.md) and [INSERT/REPLACE scalar expression values spec](docs/specs/insert-scalar-expression-values/specs.md). |
...
| `SET` | 🟡 | top | Variable assignment, user variables, system variables, persisted variables, names, charset, and transaction forms. | `SET NAMES`, `SET CHARACTER SET`, session/local `SET sql_mode`, and session/local `SET group_concat_max_len` are implemented for the currently supported connection-state surfaces, including `DEFAULT`, low-value clamping/warnings for `group_concat_max_len`, and the focused `REPLACE(@@SESSION.sql_mode, 'ONLY_FULL_GROUP_BY', '')` removal idiom. General variable assignment, user variables, persisted variables, mutable global variables, and transaction forms are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md), and [`group_concat_max_len` system variable spec](docs/specs/group-concat-max-len-variable/specs.md). |
| `SET CHARACTER SET` | 🟡 | top | Connection character-set shorthand semantics. | Implemented for `utf8mb4`, `utf8mb3`, `latin1`, `binary`, and `DEFAULT`, with handle-owned session state, selected-schema/default-schema collation behavior, and first-slice `SHOW VARIABLES` exposure. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `SHOW CHARACTER SET` / `SHOW CHARSET` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First executable slice exposes the supported MyLite charset registry (`binary`, `latin1`, `utf8mb3`, `utf8mb4`) through `SHOW {CHARACTER SET\|CHARSET}` plus the MySQL-runtime-accepted `SHOW CHAR SET` spelling, with exact `Charset` / `Description` / `Default collation` / numeric `Maxlen` columns, case-insensitive `LIKE` filtering with backslash escapes, and nondiagnostic clearing. `WHERE` is parsed but returns `SHOW CHARACTER SET WHERE is not supported` until shared SHOW filtering lands. `INFORMATION_SCHEMA.CHARACTER_SETS` shares the same registry through its wildcard-selection slice. Full MySQL charset catalog, broader information-schema query forms, and privilege behavior remain deferred. See [SHOW CHARACTER SET spec](docs/specs/show-character-set/specs.md) and [INFORMATION_SCHEMA.CHARACTER_SETS spec](docs/specs/information-schema-character-sets/specs.md). |
| `SHOW COLLATION` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First executable slice exposes the supported MyLite collation registry (`binary`, `latin1_bin`, `latin1_swedish_ci`, `utf8mb3_bin`, `utf8mb3_general_ci`, `utf8mb4_0900_ai_ci`, `utf8mb4_bin`, `utf8mb4_unicode_520_ci`) with exact `Collation` / `Charset` / numeric `Id` / `Default` / `Compiled` / numeric `Sortlen` / `Pad_attribute` columns, MySQL-verified default flags, sort lengths, pad attributes, case-insensitive `LIKE` filtering with backslash escapes, and nondiagnostic clearing. `WHERE` is parsed but returns `SHOW COLLATION WHERE is not supported` until shared SHOW filtering lands. `INFORMATION_SCHEMA.COLLATIONS` shares the same registry through its wildcard-selection slice. Full MySQL collation catalog and privilege behavior remain deferred. See [SHOW COLLATION spec](docs/specs/show-collation/specs.md) and [INFORMATION_SCHEMA.COLLATIONS spec](docs/specs/information-schema-collations/specs.md). |
| `SHOW COLUMNS` / `SHOW FIELDS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First executable slice targets supported persistent MyLite base tables, including parser/AST coverage for `SHOW [EXTENDED] [FULL] {COLUMNS\|FIELDS} {FROM\|IN} tbl_name [{FROM\|IN} db_name] [LIKE pattern \| WHERE expr]`, `FIELDS` synonym support, selected-schema, explicit-schema, and `db.table` target resolution, MySQL-compatible non-`FULL` and `FULL` result column names, catalog-backed type/null/key/default/extra/collation/privilege/comment values, `LIKE` filtering with backslash escapes, `EXTENDED` no-op behavior for the current catalog, and deterministic no-selected-schema, missing-schema, and missing-table diagnostics. `WHERE` filtering and `information_schema` system-view column descriptions are parsed but return clear unsupported diagnostics until shared SHOW filtering and system-view metadata land. Hidden storage-engine columns, user views, temporary tables, generated invisible primary key settings, privilege filtering, locks, counters, and broader metadata behavior remain deferred. See [SHOW COLUMNS spec](docs/specs/show-columns/specs.md). |
...
| `SHOW CREATE TABLE` | 🟡 | top | Result-set shape and metadata formatting for base tables. | First executable slice is implemented for supported persistent MyLite base tables, including selected-schema and schema-qualified targets, two-column `Table` / `Create Table` result shape, catalog-backed deterministic CREATE text for the supported column, primary-key, unique-key, nonunique-key, visibility, comments, charset/collation, auto-increment, and table-option subset, backtick identifier quoting, and selected-schema, missing-schema, missing-table, known-system-table, and unknown-system-table diagnostics. Views, temporary tables, foreign keys, checks, generated columns, partitions, functional indexes, storage-engine-specific options, privilege filtering, `SHOW CREATE VIEW`, and `sql_quote_show_create = 0` remain deferred. See [SHOW CREATE TABLE spec](docs/specs/show-create-table/specs.md). |
...
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First executable slice targets persistent MyLite base-table indexes represented in `__mylite_index_catalog`, including parser/AST coverage for `SHOW [EXTENDED] {INDEX\|INDEXES\|KEYS} {FROM\|IN} tbl_name [{FROM\|IN} db_name] [WHERE expr]`, `INDEXES` and `KEYS` synonym support, selected-schema, explicit-schema, `db.table`, and explicit-schema override resolution, MySQL-compatible result column names/order, `WHERE` filtering over the supported displayed-column expression subset, catalog-backed primary, unique, nonunique, multi-part, prefix, ordering, comment, visibility, nullable, and index-type metadata, known `information_schema` tables returning empty index results, and deterministic no-selected-schema, missing-schema, missing-table, and unknown-system-table diagnostics. `EXTENDED` currently returns visible catalog rows because MyLite has no hidden storage-engine key parts. Functional key parts, generated invisible primary key rows, temporary tables, user views, privilege filtering, locks, counters, broader WHERE expressions, and statistics maintenance remain deferred. See [SHOW INDEX spec](docs/specs/show-index/specs.md). |
...
| `SHOW TABLE STATUS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First executable slice targets supported persistent MyLite base tables and existing `information_schema` metadata views, including parser/AST coverage for `SHOW TABLE STATUS [FROM\|IN db] [LIKE pattern \| WHERE expr]`, selected-schema and explicit-schema resolution, MySQL-compatible status column names/order, catalog-backed table metadata, deterministic `0` placeholders for user base-table storage counters until physical statistics are maintained, case-sensitive `LIKE` filtering with backslash escapes, information-schema pattern normalization, empty results, and no-selected/missing-schema diagnostics. `WHERE` filtering is parsed and returns a clear unsupported diagnostic until shared SHOW filtering lands. Temporary tables, user views, privilege filtering, lower-case table-name modes, accurate storage statistics, locks, counters, and broader system-schema contents remain deferred. See [SHOW TABLE STATUS spec](docs/specs/show-table-status/specs.md). |
...
| `SHOW VARIABLES` | 🟡 | top | Result-set shape, filtering, scope modifiers, system-variable values, and MySQL 8.4-compatible diagnostics. | First executable slice is implemented for `SHOW [GLOBAL\|SESSION\|LOCAL] VARIABLES [LIKE 'pattern' \| WHERE expr]`, including MySQL-compatible `Variable_name` / `Value` columns, `LOCAL` as a session synonym, deterministic variable-name ordering, case-insensitive `LIKE` filtering with backslash escapes, and a practical catalog covering connection charset/collation state, schema/server charset defaults, autocommit, transaction defaults, diagnostics counters/defaults, mutable session `sql_mode`, mutable session `group_concat_max_len`, `sql_notes`, and version variables. The same catalog is readable through `@@`, `@@SESSION`, `@@LOCAL`, and `@@GLOBAL` expression forms with MySQL-compatible scope diagnostics and expression metadata. `WHERE` is parsed but returns `SHOW VARIABLES WHERE is not supported` until shared SHOW filtering lands. Complete variable catalog, Performance Schema variable tables, persisted variables, and mutable global/session separation remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md), [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md), and [`group_concat_max_len` system variable spec](docs/specs/group-concat-max-len-variable/specs.md). |
| `SHOW WARNINGS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First diagnostic slice is implemented for current handle-owned MyLite warning, error, and implemented note conditions, including `SHOW WARNINGS`, `SHOW WARNINGS LIMIT row_count`, `LIMIT offset, row_count`, `LIMIT row_count OFFSET offset`, and `SHOW COUNT(*) WARNINGS`, with MySQL-compatible result column names and diagnostic-statement preservation. `LIKE`/`WHERE` remain syntax errors because MySQL does not accept them for this statement. `@@warning_count` expression reads are implemented for the current nondiagnostic-cleared system-variable slice. Broader note generation, mutable `sql_notes`, separate generated-condition counters under `max_error_count`, full generated-condition counter semantics, and `GET DIAGNOSTICS` remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md) and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| `DISTINCT` / `DISTINCTROW` | 🟡 | top | Duplicate elimination semantics and metadata. | Implemented for no-table scalar SELECT and current table-backed SELECT surfaces, including `ALL`, repeated same-mode modifiers, mixed-mode error 1221, projected-row duplicate comparison, `NULL` duplicate collapsing, supported collation-sensitive text comparison, metadata preservation, joins, aggregates, `ORDER BY`/`LIMIT`, and DISTINCT/order error 3065 for hidden non-selected columns when `ONLY_FULL_GROUP_BY` is active. Clearing `ONLY_FULL_GROUP_BY` permits ordering by non-selected columns for currently executable query shapes. Top-level `UNION` supports default and explicit `DISTINCT` duplicate modes over the current union slice. Aggregate-local `DISTINCT`, non-UNION set-operation duplicate modes, derived tables, subqueries, optimizer pushdown, and deferred row sources remain deferred. See [SELECT DISTINCT spec](docs/specs/select-distinct/specs.md), [UNION query expressions spec](docs/specs/union-query-expressions/specs.md), and [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md). |
...
| `DROP COLUMN` | 🟡 | top | Dependency checks, generated columns, indexes, constraints, and errors. | Implemented for stored values and catalog metadata, including index key-part cleanup, empty-index removal, duplicate-index warnings after rewrites, auto-increment cleanup, dropping-every-column rejection, and atomic rollback. Generated-column, trigger, check, and foreign-key dependency catalogs remain deferred. |
| `RENAME COLUMN` | 🟡 | top | Metadata rewrite and dependency updates. | Implemented for column metadata, stored-value preservation, and index key-part metadata updates. Generated-column and foreign-key dependency rewrites remain deferred. |
| `CHANGE COLUMN` | 🟡 | top | Rename plus type/attribute change semantics. | Implemented for complete replacement definitions, optional rename/repositioning, metadata/storage shadow rewrite, index metadata updates, not-null validation, and affected-row reporting for copied rows. Full MySQL value conversion and later dependency surfaces remain deferred. |
| `MODIFY COLUMN` | 🟡 | top | Type/attribute change without rename. | Implemented for complete replacement definitions, optional repositioning, visibility changes, metadata/storage shadow rewrite, missing-column diagnostics, not-null validation, and all-invisible rejection. Full MySQL value conversion and later dependency surfaces remain deferred. |
...
| `DROP INDEX` / `DROP KEY` | 🟡 | top | Index removal and constraint dependencies. | Standalone `DROP INDEX name ON table` and `ALTER TABLE DROP INDEX` / `DROP KEY` are implemented for metadata-backed indexes created by supported table/index DDL, including conflict-surface removal and `INFORMATION_SCHEMA.STATISTICS`, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS`, and `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` metadata cleanup. Future foreign-key dependency checks and implicit commits remain deferred. See [standalone CREATE INDEX and DROP INDEX spec](docs/specs/create-drop-index/specs.md), [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md), [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md), and [INFORMATION_SCHEMA.KEY_COLUMN_USAGE spec](docs/specs/information-schema-key-column-usage/specs.md). |
| `RENAME INDEX` / `RENAME KEY` | 🟡 | high | Index rename semantics. | Implemented as metadata-only rename of non-primary indexes while preserving key parts, visibility, uniqueness, conflict order, statistics rows, `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` names, and `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` names for unique indexes. Exact primary-index rename diagnostics and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md), [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md), and [INFORMATION_SCHEMA.KEY_COLUMN_USAGE spec](docs/specs/information-schema-key-column-usage/specs.md). |
...
| `ADD CONSTRAINT CHECK` | ⚪ | high | Check constraint addition and validation. | Parsed as an ALTER surface with deterministic unsupported diagnostics before mutation until expression-backed CHECK catalogs, validation, and metadata exposure are implemented. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP CHECK` | ⚪ | high | Check constraint removal. | Parsed as an ALTER surface with deterministic unsupported diagnostics before mutation until CHECK catalog implementation lands. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ALTER CHECK ENFORCED` / `NOT ENFORCED` | ⚪ | high | Check enforcement toggling. | Parsed as an ALTER surface with deterministic unsupported diagnostics before mutation until CHECK catalog and expression runtime support lands. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD CONSTRAINT FOREIGN KEY` | ⚪ | high | Foreign key addition, validation, indexes, and actions. | Parsed as an ALTER surface with deterministic unsupported diagnostics before mutation until foreign-key catalogs, enforcement, cascades, supporting-index dependency checks, and actual foreign-key metadata rows exist. `INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS` currently exposes the MySQL-compatible empty system-view shape only. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md) and [INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS spec](docs/specs/information-schema-referential-constraints/specs.md). |
| `DROP FOREIGN KEY` | ⚪ | high | Foreign key removal and metadata cleanup. | Parsed as an ALTER surface with deterministic unsupported diagnostics before mutation until foreign-key catalog implementation lands. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DISABLE KEYS` / `ENABLE KEYS` | ❌ | low | MyISAM-style key maintenance syntax and diagnostics. |  |
| `RENAME TO` | 🟡 | top | Table rename via ALTER TABLE. | Implemented for supported MyLite base tables through `ALTER TABLE t RENAME u`, `ALTER TABLE t RENAME TO u`, and `ALTER TABLE t RENAME AS u`, sharing the `RENAME TABLE` runtime path for schema resolution, physical renames, metadata rewrites, and statement atomicity. Temporary-table rename, mixed rename plus rebuild actions, dependencies, and implicit commits remain deferred. See [RENAME TABLE spec](docs/specs/rename-table/specs.md). |
...
| `utf8mb4` | 🟡 | top | Recognize, store, expose, and apply MySQL-compatible character set metadata and conversions. | Recognized in the internal registry with default collation `utf8mb4_0900_ai_ci` and compatibility validation for `utf8mb4_bin` and `utf8mb4_unicode_520_ci` in schema defaults and connection charset state; conversion/comparison semantics and full metadata listing are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [utf8mb4 unicode 520 collation spec](docs/specs/utf8mb4-unicode-520-ci/specs.md). |
...
| Unicode Collation Algorithm families | 🟡 | top | Match utf8mb4_0900, unicode, language-specific, accent-sensitive, case-sensitive, and kana-sensitive behavior where MySQL exposes it. | `utf8mb4_0900_ai_ci` and `utf8mb4_unicode_520_ci` are recognized and exposed with MySQL-compatible catalog metadata for supported DDL/session validation. Full Unicode weight tables, contractions, expansions, accent/case/kana sensitivity, and optimizer/index interactions remain deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [utf8mb4 unicode 520 collation spec](docs/specs/utf8mb4-unicode-520-ci/specs.md). |
...
| `->` | 🟡 | high | Return value from JSON column after evaluating path; equivalent to JSON_EXTRACT(). | Implemented for qualified-identifier left operands and string-literal paths in supported `SELECT` projection, `WHERE`, and `ORDER BY` paths, including JSON result metadata, MySQL-style JSON `null` versus SQL `NULL`, multi-match autowrapping, and path diagnostics. Native JSON-column storage and broader left/right operand forms remain deferred. See [JSON path extraction spec](docs/specs/json-path-extraction/specs.md). |
| `->>` | 🟡 | high | Return value from JSON column after evaluating path and unquoting the result; equivalent to JSON_UNQUOTE(JSON_EXTRACT()). | Implemented for qualified-identifier left operands and string-literal paths in supported `SELECT` projection, `WHERE`, and `ORDER BY` paths, including unquoted string results, JSON result serialization for non-string values, SQL `NULL` for missing paths, and long-blob-compatible result metadata. Native JSON-column storage and broader left/right operand forms remain deferred. See [JSON path extraction spec](docs/specs/json-path-extraction/specs.md). |
...
| `CHARSET()` | 🟡 | high | Return the character set of the argument | Implemented for supported scalar expression call sites over MyLite's current charset/collation metadata surface, including literals under `SET NAMES`, `NULL`, numeric expressions, supported string functions, `CHAR(... USING ...)`, `HEX()`/`UNHEX()`, `CAST(... AS CHAR/BINARY)`, table character/binary columns, table `WHERE`/`ORDER BY`, `UPDATE`, and `DELETE`. Full collation aggregation, introducers, expression `COLLATE`, charset conversion, and exact native wrong-arity diagnostics remain deferred. See [charset and collation introspection functions spec](docs/specs/charset-collation-functions/specs.md). |
...
| `COERCIBILITY()` | 🟡 | high | Return the collation coercibility value of the string argument | Implemented for supported scalar expression call sites over MyLite's current descriptor/AST-visible subset, including MySQL-compatible ranks for columns, literals, `NULL`, numeric expressions, supported string functions, `CHAR(... USING ...)`, `HEX()`/`UNHEX()`, `CAST(... AS CHAR/BINARY)`, table `WHERE`/`ORDER BY`, `UPDATE`, and `DELETE`. Full collation coercion conflict resolution, introducers, expression `COLLATE`, and exact native wrong-arity diagnostics remain deferred. See [charset and collation introspection functions spec](docs/specs/charset-collation-functions/specs.md). |
...
| `CONVERT()` | 🟡 | high | Cast a value as a certain type or convert a string expression to a target character set. | Implemented for the CAST-equivalent ODBC form over the current CAST target subset and for `CONVERT(expr USING charset_name)` over MyLite's initial charset registry (`binary`, `latin1`, `utf8mb3`, `utf8mb4`), including parser/AST support, runtime results, warnings, charset/collation/coercibility introspection, invalid-charset diagnostics, metadata tests for the CAST-aligned subset, and DML strictness tests. Full transcoding, exact non-binary explicit-charset column metadata, temporal/JSON/spatial casts, fixed-length binary padding, `AT TIME ZONE`, and exhaustive overflow/SQL-mode behavior remain deferred. See [CONVERT cast expressions spec](docs/specs/convert-function/specs.md) and [CAST expression spec](docs/specs/cast-expression/specs.md). |
...
| `DATE_FORMAT()` | 🟡 | top | Format date as specified | Implemented for supported scalar expression call sites, including common MySQL date/time format tokens, `NULL` propagation, invalid-date `NULL` results with warning 1292, unsupported-token fallback, table projection/`WHERE`/`ORDER BY`, and `UPDATE`. Full locale catalogs, zero/incomplete-date formatting, time-zone-sensitive `TIMESTAMP` behavior, exact native arity diagnostics, and broader temporal conversion surfaces remain deferred. See [DATE_FORMAT function spec](docs/specs/date-format-function/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `ELT()` | 🟡 | high | Return string at index number | Implemented in the string list/index function slice for supported scalar expression call sites, including integer index conversion, out-of-range and `NULL` index results, selected-element `NULL`, selected-element short-circuiting, UTF-8 string pass-through, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and verified text/numeric-list metadata. Binary-string behavior, all-`NULL` list metadata, exact decimal/real string-conversion formatting for selected numeric elements, `max_allowed_packet`, full collation/coercibility, and exact native arity diagnostics remain deferred. See [string list and index functions spec](docs/specs/string-list-index-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `EXTRACT()` | 🟡 | high | Extract part of a date | Implemented for `EXTRACT(unit FROM expr)` with `YEAR`, `MONTH`, `DAY`, `HOUR`, `MINUTE`, and `SECOND` over supported scalar expression call sites, including AST unit metadata separate from evaluatable child expressions, `NULL` propagation, incomplete-date zero part extraction, invalid temporal warning 1292, DML warning promotion, and nullable numeric metadata. `WEEK`, combined units, `MICROSECOND`, standalone time-only parsing, exact native warning-message variants, and exact syntax diagnostics remain deferred. See [temporal part scalar functions spec](docs/specs/temporal-part-functions/specs.md). |
...
| `FROM_BASE64()` | 🟡 | medium | Decode base64 encoded string and return result | Implemented for supported scalar expression call sites, including `NULL` propagation, empty input, ASCII whitespace ignored during decoding, strict length/padding validation, invalid-input `NULL` results without warnings, binary result metadata under `utf8mb4` and `latin1`, nested byte round-trips through `HEX(FROM_BASE64(...))`, embedded-NUL byte lengths through `LENGTH(FROM_BASE64(...))`, table projection/`WHERE`/`ORDER BY`, `UPDATE`, and `DELETE`. Exact native arity diagnostics, full binary-string behavior across all expression functions, hex/bit literal evaluation, `max_allowed_packet`, and exact metadata in deferred expression contexts remain deferred. See [Base64 string functions spec](docs/specs/base64-string-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `HEX()` | 🟡 | high | Hexadecimal representation of decimal or string value | Implemented for supported scalar expression call sites, including byte encoding for text input, numeric longlong bit-pattern output, `NULL` propagation, UTF-8 bytes, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and connection-charset result metadata under `utf8mb4` and `latin1`. Exact native arity diagnostics, generated-column charset exceptions, hex/bit literal evaluation, `max_allowed_packet`, and exact-versus-approximate numeric preservation for every expression path remain deferred. See [string `HEX()` and `UNHEX()` functions spec](docs/specs/string-hex-unhex-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `JSON_EXTRACT()` | 🟡 | high | Return data from JSON document | Implemented over text JSON documents for one or more paths, including member/index/range/wildcard/recursive path evaluation, multi-path and multi-match autowrapping, JSON `null` serialization, missing-path SQL `NULL`, invalid JSON/path diagnostics, and JSON result metadata. Native JSON-column storage and exhaustive charset/path edge cases remain deferred. See [JSON path extraction spec](docs/specs/json-path-extraction/specs.md). |
| `JSON_INSERT()` | ❌ | high | Insert data into JSON document |  |
| `JSON_KEYS()` | 🟡 | high | Array of keys from JSON document | Implemented over text JSON documents with optional path, sorted key array serialization, SQL `NULL` for missing or non-object targets, rejection of wildcard/range paths, invalid JSON/path diagnostics, and JSON result metadata. Native JSON-column storage and exhaustive charset/path edge cases remain deferred. See [JSON path extraction spec](docs/specs/json-path-extraction/specs.md). |
...
| `JSON_QUOTE()` | 🟡 | high | Quote JSON document | Implemented for supported scalar expression call sites, including SQL NULL propagation, string escaping for JSON string literals, non-string argument diagnostics, connection-character-set text metadata, table predicates/order/update/delete expression paths, and runtime tests. Full character-set conversion nuances and JSON-column interactions remain deferred. See [JSON scalar foundation spec](docs/specs/json-scalar-foundation/specs.md). |
...
| `LPAD()` | 🟡 | medium | Return the string argument, left-padded with the specified string | Implemented in the string padding/repeat function slice for supported scalar expression call sites, including `NULL` propagation, zero and negative target lengths, empty pad strings, leftmost truncation, UTF-8 character lengths, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and constant metadata. Binary-string behavior, `max_allowed_packet`, full collation/coercibility, and exact variable-length metadata remain deferred. See [string padding, repeat, space, and reverse functions spec](docs/specs/string-padding-repeat-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `NOT REGEXP` | 🟡 | high | Negation of REGEXP | Implemented for supported scalar expression call sites with `NULL` propagation, result metadata, predicate execution, and MySQL-style diagnostics over MyLite's byte/ASCII regular-expression matcher. Full MySQL ICU Unicode and collation-aware regular expression behavior remains deferred. See [REGEXP predicates spec](docs/specs/regexp-predicates/specs.md). |
...
| `QUOTE()` | 🟡 | high | Escape the argument for use in an SQL statement | Implemented for supported scalar expression call sites, including ordinary strings, empty strings, numeric inputs, UTF-8 pass-through, single-quote/backslash/Control+Z escaping, newline/tab/carriage-return pass-through, literal text `NULL` for SQL `NULL`, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and constant metadata. Embedded `NUL` values, binary-string behavior, `max_allowed_packet`, full collation/coercibility, and exact native arity error-code exposure remain deferred. See [string `QUOTE()` function spec](docs/specs/string-quote-function/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `REPEAT()` | 🟡 | high | Repeat a string the specified number of times | Implemented in the string padding/repeat function slice for supported scalar expression call sites, including `NULL` propagation, zero and negative counts, empty strings, numeric count rounding, parser-level syntax rejection for invalid arity, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and constant metadata. Binary-string behavior, `max_allowed_packet`, and full collation/coercibility remain deferred. See [string padding, repeat, space, and reverse functions spec](docs/specs/string-padding-repeat-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `RLIKE` | 🟡 | high | Whether string matches regular expression | Implemented as a `REGEXP` synonym for supported scalar expression call sites, including `NOT RLIKE`; full MySQL ICU Unicode and collation-aware regular expression behavior remains deferred. See [REGEXP predicates spec](docs/specs/regexp-predicates/specs.md). |
...
| `RPAD()` | 🟡 | medium | Append string the specified number of times | Implemented in the string padding/repeat function slice for supported scalar expression call sites, including `NULL` propagation, zero and negative target lengths, empty pad strings, leftmost truncation, UTF-8 character lengths, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and constant metadata. Binary-string behavior, `max_allowed_packet`, full collation/coercibility, and exact variable-length metadata remain deferred. See [string padding, repeat, space, and reverse functions spec](docs/specs/string-padding-repeat-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `UNHEX()` | 🟡 | high | Return a binary string decoded from hexadecimal text | Implemented for supported scalar expression call sites, including upper/lower hex digits, odd-length input, empty string, `NULL`, invalid-input warning 1411 with `NULL` result, integer and approximate numeric argument stringification, nested byte round-trips through `HEX(UNHEX(...))`, embedded-NUL byte lengths through `LENGTH(UNHEX(...))`, UTF-8 byte results where representable, table projection/`WHERE`/`ORDER BY`, `UPDATE`, `DELETE`, and binary result metadata under `utf8mb4` and `latin1`. Exact native arity diagnostics, full binary-string behavior across all expression functions, hex/bit literal evaluation, `max_allowed_packet`, exact-versus-approximate numeric preservation for every expression path, and exact warning message text for every source-expression shape remain deferred. See [string `HEX()` and `UNHEX()` functions spec](docs/specs/string-hex-unhex-functions/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
| `UNIX_TIMESTAMP()` | 🟡 | top | Return a Unix timestamp | Implemented for supported scalar expression call sites, including statement-stable no-argument timestamps, date/datetime/text/numeric temporal conversion, fractional-second preservation, native zero results for valid out-of-range dates, `NULL` propagation, invalid temporal `0.000000` results with warning 1292, table projection/`WHERE`/`ORDER BY`, `UPDATE`, and `DELETE`. Session time zone variables, exact native arity diagnostics, and broader temporal conversion surfaces remain deferred. MyLite currently treats the effective session time zone as UTC. See [UNIX_TIMESTAMP function spec](docs/specs/unix-timestamp-function/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
...
| `VERSION()` | 🟡 | top | Return a string that indicates the MySQL server version | Implemented as `mylite_version()` with MySQL-compatible scalar result metadata for the covered expression paths. Replication warnings and exact arity error code reporting remain deferred. See [session information functions spec](docs/specs/session-information-functions/specs.md). |
...
| `INFORMATION_SCHEMA.CHECK_CONSTRAINTS` | 🟡 | high | Table and column CHECK constraints | `SELECT * FROM INFORMATION_SCHEMA.CHECK_CONSTRAINTS` exposes the exact four-column MySQL table shape as a read-only system view, resolves qualified names case-insensitively including quoted forms, returns zero rows until MyLite has CHECK DDL/catalog/enforcement support, and appears in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. Actual CHECK rows, privilege filtering, and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.CHECK_CONSTRAINTS spec](docs/specs/information-schema-check-constraints/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
| `INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY` | 🟡 | high | Character set applicable to each collation | `SELECT * FROM INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY` exposes the shared supported MyLite collation registry as `COLLATION_NAME` / `CHARACTER_SET_NAME`, with case-insensitive quoted table resolution and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. MyLite exposes only implemented collations; privilege filtering and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.COLLATION_CHARACTER_SET_APPLICABILITY spec](docs/specs/information-schema-collation-character-set-applicability/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
| `INFORMATION_SCHEMA.COLLATIONS` | 🟡 | high | Collations for each character set | `SELECT * FROM INFORMATION_SCHEMA.COLLATIONS` exposes the shared supported MyLite collation registry, including `utf8mb4_unicode_520_ci`, with uppercase MySQL columns, numeric `ID` and `SORTLEN`, case-insensitive quoted table resolution, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. MyLite exposes only implemented collations; privilege filtering and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.COLLATIONS spec](docs/specs/information-schema-collations/specs.md), [utf8mb4 unicode 520 collation spec](docs/specs/utf8mb4-unicode-520-ci/specs.md), and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
...
| `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` | 🟡 | top | Which key columns have constraints | `SELECT * FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE` exposes primary-key and unique key-part rows derived from `__mylite_index_catalog`, with exact twelve-column shape, deterministic row ordering, composite-key ordinals, case-insensitive quoted table resolution, DDL side effects for supported key metadata, nonunique and expression-only key-part exclusion, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. Foreign-key rows, privilege filtering, and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.KEY_COLUMN_USAGE spec](docs/specs/information-schema-key-column-usage/specs.md), [INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS spec](docs/specs/information-schema-referential-constraints/specs.md), and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
| `INFORMATION_SCHEMA.KEYWORDS` | 🟡 | medium | MySQL keywords | `SELECT * FROM INFORMATION_SCHEMA.KEYWORDS` exposes MyLite's shared lexer-supported keyword catalog with uppercase `WORD` and numeric `RESERVED` columns, case-insensitive quoted table resolution, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. MyLite does not yet claim exact MySQL 8.4.9 keyword catalog completeness; privilege filtering and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.KEYWORDS spec](docs/specs/information-schema-keywords/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
...
| `INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS` | 🟡 | high | Foreign key information | `SELECT * FROM INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS` exposes the exact eleven-column MySQL table shape as a read-only system view, resolves qualified names case-insensitively including quoted forms, returns zero rows until MyLite has foreign-key DDL/catalog/enforcement support, and appears in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. Actual foreign-key rows, privilege filtering, and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS spec](docs/specs/information-schema-referential-constraints/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
...
| `INFORMATION_SCHEMA.SCHEMATA` | 🟡 | top | Schema information | `SELECT * FROM INFORMATION_SCHEMA.SCHEMATA` exposes MyLite schema catalog rows, seeded system schemas, schema defaults, and create/drop changes. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. Privileges and full result metadata remain deferred. See [core metadata catalog spec](docs/specs/core-metadata-catalog/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
...
| `INFORMATION_SCHEMA.STATISTICS` | 🟡 | top | Table index statistics | `SELECT * FROM INFORMATION_SCHEMA.STATISTICS` exposes the MySQL 8.4.9 column shape over the internal index catalog and returns an empty result before index metadata exists. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. Privileges and full index metadata remain deferred. See [core metadata catalog spec](docs/specs/core-metadata-catalog/specs.md) and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
| `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` | 🟡 | high | Which tables have constraints | `SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS` exposes primary-key and unique constraints derived from `__mylite_index_catalog`, with exact seven-column shape, deterministic row ordering, case-insensitive quoted table resolution, DDL side effects for supported key metadata, and a `SYSTEM VIEW` row in `INFORMATION_SCHEMA.TABLES` / `SHOW TABLES FROM information_schema`. The table is also selectable through the shared information-schema system-view path for projections, filters, aliases, ordering, limits, duplicate modifiers, and aggregates where those SELECT features are implemented. CHECK rows, actual foreign-key constraint rows, privilege filtering, and exact MySQL field metadata remain deferred. See [INFORMATION_SCHEMA.TABLE_CONSTRAINTS spec](docs/specs/information-schema-table-constraints/specs.md), [INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS spec](docs/specs/information-schema-referential-constraints/specs.md), and [composable information-schema SELECT spec](docs/specs/information-schema-composable-select/specs.md). |
...
| Connection character set state | 🟡 | top | character_set_client, character_set_connection, character_set_results, collation_connection, and SET NAMES behavior. | Handle-owned state is implemented for `SET NAMES` and `SET CHARACTER SET` over the initial registry, with first-slice `SHOW VARIABLES` visibility and `@@` expression reads for session and default/global values; protocol metadata and direct system-variable assignment are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
| Time zone state | ❌ | top | global/session time_zone, system_time_zone, temporal functions, TIMESTAMP conversion, and named time zones. | Deferred; the current temporal scalar function slice uses UTC as MyLite's fixed effective session time zone until mutable time-zone variables and named-zone tables are implemented. See [current temporal functions spec](docs/specs/current-temporal-functions/specs.md). |
...
| Last insert id | 🟡 | top | LAST_INSERT_ID(), OK packet insert id, explicit LAST_INSERT_ID(expr), and multi-row behavior. | `INSERT ... VALUES` and `INSERT ... SET` track generated `AUTO_INCREMENT` values and expose the session value through `mylite_last_insert_id()`. SQL `LAST_INSERT_ID()` and `LAST_INSERT_ID(expr)` are implemented for supported scalar expression paths, including `NULL` reset and unsigned wrap for negative values. Protocol OK packet insert id and remaining insert-expression interactions remain deferred. See [INSERT ... VALUES spec](docs/specs/insert-values/specs.md), [INSERT ... SET spec](docs/specs/insert-set/specs.md), and [session information functions spec](docs/specs/session-information-functions/specs.md). |
...
| `character_set_client` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice and `@@character_set_client` expression reads; direct assignment is deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| `character_set_results` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice and `@@character_set_results` expression reads; direct assignment is deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| `collation_connection` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice and `@@collation_connection` expression reads; direct assignment is deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md), [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| `group_concat_max_len` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state with MySQL default `1024`, exposed through `SHOW VARIABLES` and `@@group_concat_max_len` / `@@GLOBAL.group_concat_max_len` expression reads, mutable through session/local `SET` and `DEFAULT`, clamped to minimum `4` with warning 1292, and applied to `GROUP_CONCAT()` truncation and metadata. Global mutation, persisted values, arbitrary assignment expressions, and exact binary-argument metadata remain deferred. See [`group_concat_max_len` system variable spec](docs/specs/group-concat-max-len-variable/specs.md), [GROUP_CONCAT function spec](docs/specs/group-concat-function/specs.md), [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| `sql_mode` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | MyLite stores handle-owned session `sql_mode`, initializes it to the MySQL 8.4 default, supports session/local assignment plus `DEFAULT` and the focused `REPLACE(@@SESSION.sql_mode, 'ONLY_FULL_GROUP_BY', '')` removal idiom, canonicalizes recognized mode lists, reports error 1231 for unknown modes, exposes session/global values through `SHOW VARIABLES` and `@@sql_mode` / `@@GLOBAL.sql_mode` expression reads, and applies `ONLY_FULL_GROUP_BY` to current grouping and DISTINCT-order validators. Persisted/global mutation and complete mode-sensitive behavior outside this slice remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md), and [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md). |
...
| `version` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` and global-only `@@version` expression reads expose `mylite_version()`; broader protocol and `VERSION()` surfaces are tracked separately. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md) and [system variable expression reads spec](docs/specs/system-variable-expression-reads/specs.md). |
...
| Result ordering guarantees | 🟡 | top | Match MySQL ordering only where SQL semantics or MySQL behavior require it. | Implemented for Task 18's `SELECT ... ORDER BY ... LIMIT/OFFSET` subset, supported joined row sources, and the Task 25 aggregate grouping slice with deterministic tests only when explicit sort keys fully define row order. Equal-key ordering remains intentionally unspecified, and set operations beyond the current `UNION` slice, optimizer-sensitive order preservation, and full collation edge cases remain deferred. See [ORDER BY, LIMIT, and OFFSET spec](docs/specs/order-limit-offset/specs.md), [inner joins spec](docs/specs/inner-joins/specs.md), [outer joins spec](docs/specs/outer-joins/specs.md), and [aggregate functions and grouping spec](docs/specs/aggregate-grouping/specs.md). |
...
| Storage engines | 🟡 | high | Decide how ENGINE clauses, SHOW ENGINES, and engine-specific metadata map to SQLite-backed storage. | First storage-engine registry slice maps MyLite's SQLite-backed table storage to an `InnoDB` compatibility facade, keeps unsupported engine names rejected by `CREATE TABLE`, and exposes common unsupported MySQL engines through `SHOW ENGINES` and selectable `INFORMATION_SCHEMA.ENGINES` rows. Full engine-specific table behavior, build/runtime-disabled states, and full MySQL field metadata for information-schema SELECTs are deferred. See [SHOW ENGINES spec](docs/specs/show-engines/specs.md), [INFORMATION_SCHEMA.ENGINES spec](docs/specs/information-schema-engines/specs.md), and [CREATE TABLE base execution spec](docs/specs/create-table-base-execution/specs.md). |
| `InnoDB` engine surface | 🟡 | high | Decide how MySQL's default engine metadata, syntax, and diagnostics map onto SQLite-backed storage. | First executable slice exposes `InnoDB` as the default supported storage-engine facade in `SHOW ENGINES` and `INFORMATION_SCHEMA.ENGINES`, with transactions and savepoints supported and XA unsupported. It does not claim MySQL InnoDB internals, row locks, foreign-key enforcement, tablespaces, redo/undo logs, or XA. See [SHOW ENGINES spec](docs/specs/show-engines/specs.md) and [INFORMATION_SCHEMA.ENGINES spec](docs/specs/information-schema-engines/specs.md). |
