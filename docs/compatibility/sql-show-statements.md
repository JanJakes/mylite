...
| `SHOW STATUS` | 🟡 | Limited runtime-owned status registry with MySQL 8.4.9 column labels, optional `GLOBAL`/`SESSION`/`LOCAL` scope, `LIKE 'pattern'` filters, limited trailing `WHERE` predicates over `Variable_name` and `Value`, the target MySQL 8.4.9 `Bytes_%`, `Connections`, session/local `Compression`, `Prepared_stmt_count`, `Queries`, `Questions`, `Threads_%`, `Uptime%`, `Binlog_%`, `Com_%`, `Created_%`, `Handler_%`, `Open%`, `Select_%`, `Slow_%`, `Sort_%`, `Ssl_%`, `Table_locks_%`, `Table_open_cache_%`, and `Tc_log_%` counter/resource name surfaces plus `Connection_%` diagnostic placeholders; values are embedded placeholders and do not implement live counters, TLS state, arbitrary `WHERE` expressions, Performance Schema status tables, privileges, or full status-variable coverage |
