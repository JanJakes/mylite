...
static int test_diagnostics_count_variable_lifecycle_and_preamble(void) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "preserved warning count row");
...
static int test_independent_diagnostics_count_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first count handle");
    failures +=
        mylite_test_expect_int(mylite_open_memory(&second), MYLITE_OK, "open second count handle");
