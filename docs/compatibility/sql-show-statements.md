...
| `SHOW CHARACTER SET` / `SHOW CHARSET` | 🟡 | Metadata-only MySQL 8.4.9 catalog rows for all 41 character sets with MySQL column labels plus `LIKE 'pattern'` and documented `WHERE` filters; catalog visibility does not imply DDL admission, conversion, comparison semantics, or privileges |
| `SHOW COLLATION` | 🟡 | Metadata-only MySQL 8.4.9 catalog rows for all 286 collations with MySQL column labels plus `LIKE 'pattern'` and documented `WHERE` filters; catalog visibility does not imply DDL admission, conversion, comparison semantics, or privileges |
