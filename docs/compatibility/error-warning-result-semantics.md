...
| IGNORE warning demotion | 🟡 | Limited `INSERT IGNORE ... VALUES` / `SET` demotion for descriptor integer, decimal, approximate, string, binary string, `BIT`, `YEAR`, temporal, `NULL`, and `DEFAULT` row inputs: explicit `NULL` into supported `NOT NULL` descriptors, omitted no-explicit-default columns, explicit `DEFAULT` for no-default columns, out-of-range descriptor integer/decimal/time/year inputs, invalid year or temporal inputs, overlength binary strings, over-width `BIT` inputs, and duplicate-key rows become warnings with adjusted or skipped values; omitted or explicit-`DEFAULT` numeric `NOT NULL` no-default columns store `0`, `YEAR NOT NULL` stores `0000`, `DATE NOT NULL` stores `0000-00-00`, `TIME NOT NULL` stores `00:00:00`, `DATETIME NOT NULL` or `TIMESTAMP NOT NULL` stores `0000-00-00 00:00:00`, string `NOT NULL` stores empty string, binary string and `BIT NOT NULL` store their current implicit values, and nullable dropped-default columns store `NULL`; out-of-range `TIME` values clip to `-838:59:59` or `838:59:59`; no broader type conversion, `INSERT IGNORE ... SELECT`, `UPDATE IGNORE`, `DELETE IGNORE`, or `LOAD DATA IGNORE` demotion |
