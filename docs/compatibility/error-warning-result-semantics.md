...
| Diagnostics area | 🟡 | Limited previous-statement diagnostics snapshot for `SHOW WARNINGS`, `SHOW ERRORS`, `SHOW COUNT(*) WARNINGS`, `SHOW COUNT(*) ERRORS`, and scalar `@@warning_count` / `@@error_count`; supports warning records and note records currently produced by limited table-existence DDL no-ops and existing-schema `CREATE DATABASE IF NOT EXISTS`; missing-schema `DROP DATABASE IF EXISTS` exposes a statement warning count but intentionally stores no diagnostics row; no diagnostics stacks, `GET DIAGNOSTICS`, `max_error_count`, or broader counted-but-not-stored conditions |
