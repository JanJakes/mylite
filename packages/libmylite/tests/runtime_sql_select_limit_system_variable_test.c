...
static int test_sql_select_limit_qualifiers_and_errors(void) {
    ...
    failures += mylite_test_expect_size(
        ...
        0U,
        "negative sql select limit"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "negative sql select limit");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        1U,
        "negative sql select limit warning"
    );
    mylite_result_free(result);
    result = NULL;
    failures +=
