...
static int test_literal_projection_values_and_file_safety(void) {
    ...
    if (snprintf(select_digits81, sizeof(select_digits81), "SELECT %s", digits81) < 0) {
        failures += mylite_test_expect_int(1, 0, "format 81 digit query");
    } else {
        failures += expect_query(
            database,
            (struct expected_query){
...
static int test_literal_projection_diagnostics_and_table_selects(void) {
    ...
    if (snprintf(select_digits82, sizeof(select_digits82), "SELECT %s", digits82) < 0) {
        failures += mylite_test_expect_int(1, 0, "format 82 digit query");
    } else {
        failures += execute_error(
            database,
            select_digits82,
...
static int test_independent_literal_projection_handles(void) {
    ...
    failures += mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first memory");
    failures +=
        mylite_test_expect_int(mylite_open_memory(&second), MYLITE_OK, "open second memory");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT 1 AS one",
