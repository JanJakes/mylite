...
| `DISTINCT` / `DISTINCTROW` | 🟡 | Limited one-column `SELECT DISTINCT column FROM table` and synonymous `SELECT DISTINCTROW column FROM table` over one descriptor-backed persistent base table with optional baseline `WHERE`, optional `ORDER BY` on the same selected column, and existing select `LIMIT`/`OFFSET`; no wildcard, multiple select items, expressions, literals, aliases, table-qualified references, joins, grouping, or full distinct-row semantics. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
