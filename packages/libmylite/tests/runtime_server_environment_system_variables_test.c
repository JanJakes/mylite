...
static int test_server_environment_values_and_show_rows(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open environment memory");
...
static int test_server_environment_file_safety_and_independent_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open environment file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen environment file");
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT @@basedir, @@datadir, @@hostname, @@port, @@socket",
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first environment handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second environment handle"
    );
