# Baseline sys.innodb_buffer_stats_by_table Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.innodb_buffer_stats_by_table` and
`sys.x$innodb_buffer_stats_by_table` views. MySQL uses these sys views to
summarize `INFORMATION_SCHEMA.INNODB_BUFFER_PAGE` rows grouped by object
schema and table name. MyLite does not expose a live InnoDB buffer-pool page
inventory, so both views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.innodb_buffer_stats_by_table` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_innodb_buffer_stats_by_table_views_expectations.sh`.
...
SELECT * FROM `x$innodb_buffer_stats_by_table`;
```
...
inventing schema names, table names, page counts, byte totals, or cached-row
estimates.

## Column Metadata

`sys.innodb_buffer_stats_by_table` has eight columns:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `object_schema` | `text` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `object_name` | `text` | `YES` | `NULL` | `utf8mb3_general_ci` |
...
`sys.x$innodb_buffer_stats_by_table` has the same columns except that byte
...
`NULL` table name, group by `object_schema` and `object_name`, and order by
descending allocated bytes.
...
- InnoDB buffer-pool page accounting by table;
- formatted byte calculation or cached-row aggregation;
- execution of `format_bytes()`, `locate()`, `substring_index()`, `replace()`,
  or aggregate expressions through the built-in view definition;
