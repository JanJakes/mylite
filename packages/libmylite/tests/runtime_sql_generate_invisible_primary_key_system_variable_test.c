...
enum {
    ...
    sql_generate_invisible_primary_key_value_column_count = 6,
    sql_generate_invisible_primary_key_label_column_count = 5,
    sql_generate_invisible_primary_key_diagnostics_column_count = 4,
    sql_generate_invisible_primary_key_selected_column_count = 2,
    sql_generate_invisible_primary_key_independent_column_count = 3,
...
static int test_sql_generate_invisible_primary_key_values_and_persistence(void);
static int test_sql_generate_invisible_primary_key_qualifiers_and_errors(void);
static int test_independent_sql_generate_invisible_primary_key_handles(void);
...
int main(void) {
    ...
    failures += test_sql_generate_invisible_primary_key_values_and_persistence();
    failures += test_sql_generate_invisible_primary_key_qualifiers_and_errors();
    failures += test_independent_sql_generate_invisible_primary_key_handles();
...
static int test_sql_generate_invisible_primary_key_values_and_persistence(void) {
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "open sql generate invisible primary key file"
    );
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = value_values,
            .count = sql_generate_invisible_primary_key_value_column_count,
            .context = "sql generate invisible primary key values",
        }
    );
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_generate_invisible_primary_key_label_column_count,
            .context = "sql generate invisible primary key labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_generate_invisible_primary_key, @@foreign_key_checks, @@unique_checks, "
        "@@updatable_views_with_limit, @@sql_auto_is_null, @@sql_big_selects, "
        "@@sql_buffer_result, @@sql_safe_updates, @@sql_select_limit, @@sql_notes, "
        "@@sql_warnings, @@sql_quote_show_create, @@autocommit, @@default_storage_engine, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql generate invisible primary key scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_generate_invisible_primary_key_diagnostics_column_count,
            .context = "sql generate invisible primary key warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(
        database,
        "0",
        "sql generate invisible primary key clears warnings"
    );
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_generate_invisible_primary_key_diagnostics_column_count,
            .context = "sql generate invisible primary key error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures +=
        expect_show_count_errors(database, "0", "sql generate invisible primary key clears errors");
    failures += expect_show_count_warnings(
        database,
        "0",
        "sql generate invisible primary key clears error warnings"
    );
    ...
    failures += expect_int64(
        ...
        "catalog generation unchanged by sql generate invisible primary key reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql generate invisible primary key reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql generate invisible primary key preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after sql generate invisible primary key reads"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_generate_invisible_primary_key, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_generate_invisible_primary_key_selected_column_count,
            .context = "sql generate invisible primary key with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql generate invisible primary key does not alter descriptor select",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "explicit descriptor select limit still applies with sql generate invisible "
                       "primary key",
        }
    );
    failures += expect_query_result(
        ...
        (struct expected_result){
            ...
            .count = sizeof(show_create_columns) / sizeof(show_create_columns[0]),
            .context = "sql generate invisible primary key does not alter show create table",
        }
    );
    ...
    failures += expect_int(
        mylite_open(path, &database),
        MYLITE_OK,
        "reopen sql generate invisible primary key file"
    );
    failures += expect_query_result(
        database,
        "SELECT @@sql_generate_invisible_primary_key, @@global.sql_generate_invisible_primary_key",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql generate invisible primary key values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql generate invisible primary key table rows",
        }
    );
...
static int test_sql_generate_invisible_primary_key_qualifiers_and_errors(void) {
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "open sql generate invisible primary key memory"
    );
    failures += execute_ok(
        database,
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_generate_invisible_primary_key_label_column_count,
            .context = "sql generate invisible primary key qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_generate_invisible_primary_key_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part =
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_generate_invisible_primary_key + 1");
...
static int test_independent_sql_generate_invisible_primary_key_handles(void) {
    ...
    failures += expect_int(
        ...
        MYLITE_OK,
        "open first sql generate invisible primary key handle"
    );
    failures += expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second sql generate invisible primary key handle"
    );
    failures += execute_statement_ok(first, "SHOW PROCESSLIST");

    failures += execute_ok(
        first,
        "SELECT @@sql_generate_invisible_primary_key, @@warning_count, @@error_count",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_generate_invisible_primary_key_independent_column_count,
            .context = "first handle sql generate invisible primary key variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_generate_invisible_primary_key_independent_column_count,
            .context = "second handle sql generate invisible primary key variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_generate_invisible_primary_key_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
