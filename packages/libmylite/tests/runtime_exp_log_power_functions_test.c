...
static int test_exp_log_power_values_and_file_safety(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open exp log power file");
...
static int test_exp_log_power_warnings_do_and_independent_handles(void) {
    ...
    if (result != NULL) {
        ...
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "DO row count");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 2U, "DO warning count");
        failures +=
