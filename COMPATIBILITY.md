...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `SELECT` | 🟡 | Descriptor-driven single persistent base-table `SELECT *`, column-list reads with optional source qualifiers, optional source table aliases on supported table-backed forms, limited select-item aliases on supported non-wildcard items, explicit `SELECT ALL` on supported non-distinct forms, limited no-source/`DUAL` decimal integer/`NULL`/boolean literal projection, limited no-source/`DUAL` scalar `IF()`/`IFNULL()`/`COALESCE()`/`NULLIF()` projection over signed-64 integer/boolean/`NULL` operands and supported nested control-flow calls, limited one-column `SELECT DISTINCT` / `DISTINCTROW column` with optional limited `WHERE`, single-column `ORDER BY`, and `LIMIT`/`OFFSET`, limited one-item aggregate forms, and limited `SELECT group_column, aggregate FROM table ... GROUP BY group_column [HAVING ...]` for one descriptor group column and one supported aggregate result; `SELECT *` expands visible descriptor columns only while explicit references may name invisible columns; no joins, full grouping, qualified wildcards, table-backed literal projection, general expression projection, mixed literal/control-flow scalar projection, full distinct rows, or locking clauses. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| Projection list | 🟡 | Wildcard uses visible descriptor columns in catalog ordinal order; explicit table projections and limited one-column `SELECT DISTINCT` / `DISTINCTROW` resolve descriptor column names including invisible columns and supported source-qualified references; no-source and `FROM DUAL` projection may return unparenthesized decimal integer, `NULL`, `TRUE`, and `FALSE` literals, or homogeneous scalar `IF()`/`IFNULL()`/`COALESCE()`/`NULLIF()` item lists over signed-64 integer/boolean/`NULL` operands and supported nested control-flow calls; supported non-wildcard select items may use identifier, quoted-identifier, or string-literal aliases as result labels; limited one-item aggregate forms and limited two-item grouped aggregate projection are supported; duplicate projected columns and duplicate aliases are allowed for non-distinct selects, with no mixed literal/control-flow scalar projection, qualified wildcards, or full multi-expression distinct rows. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `COALESCE()` | 🟡 | Limited no-source and `FROM DUAL` scalar `SELECT COALESCE(value[, ...])` over signed-64 decimal integer, `TRUE`/`FALSE`, `NULL`, nested `COALESCE()` operands, supported nested `IFNULL()` operands, supported nested `IF()` operands, and supported nested `NULLIF()` operands; no table-backed evaluation, predicates, DML assignments, string/decimal/float/hex/bit operands, arithmetic arguments, subqueries, side effects, or expression metadata. | [control-flow functions](docs/compatibility/functions-control-flow.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `IF()` | 🟡 | Limited no-source and `FROM DUAL` scalar `SELECT IF(condition, true_value, false_value)` over signed-64 decimal integer, `TRUE`/`FALSE`, `NULL`, nested `IF()` operands, supported nested `IFNULL()` operands, supported nested `COALESCE()` operands, and supported nested `NULLIF()` operands; no table-backed evaluation, predicates, DML assignments, string/decimal/float/hex/bit operands, arithmetic arguments, subqueries, side effects, or expression metadata. | [control-flow functions](docs/compatibility/functions-control-flow.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| `IFNULL()` | 🟡 | Limited no-source and `FROM DUAL` scalar `SELECT IFNULL(value, fallback)` over signed-64 decimal integer, `TRUE`/`FALSE`, `NULL`, nested `IFNULL()` operands, supported nested `IF()` operands, supported nested `COALESCE()` operands, and supported nested `NULLIF()` operands; no table-backed evaluation, predicates, DML assignments, string/decimal/float/hex/bit operands, arithmetic arguments, subqueries, side effects, or expression metadata. | [control-flow functions](docs/compatibility/functions-control-flow.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| `INTERVAL()` | ❌ | Argument interval index. | [comparison functions](docs/compatibility/functions-comparison.md) |
| `ISNULL()` | ❌ | NULL test function; a limited scalar baseline is specified in `docs/specs/baseline-isnull-function/` but not implemented yet. | [comparison functions](docs/compatibility/functions-comparison.md) |
| `LEAST()` | ❌ | Smallest argument. | [comparison functions](docs/compatibility/functions-comparison.md) |
| `NULLIF()` | 🟡 | Limited no-source and `FROM DUAL` scalar `SELECT NULLIF(left_value, right_value)` over signed-64 decimal integer, `TRUE`/`FALSE`, `NULL`, nested `NULLIF()` operands, supported nested `IF()` operands, supported nested `IFNULL()` operands, and supported nested `COALESCE()` operands; no table-backed evaluation, predicates, DML assignments, string/decimal/float/hex/bit operands, arithmetic arguments, subqueries, side effects, first-argument double-evaluation semantics, or expression metadata. | [control-flow functions](docs/compatibility/functions-control-flow.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
