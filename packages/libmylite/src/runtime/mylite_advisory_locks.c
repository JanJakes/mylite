...
static int copy_lock_name_text(const struct mylite_expression_value *value, char **out_text,
                               size_t *out_length);
static int validate_lock_name(mylite_db *database, const char *name, size_t length,
                              struct mylite_expression_warnings *warnings);
static int set_lock_name_error(mylite_db *database, struct mylite_expression_warnings *warnings,
...
static void remove_lock_entry(struct mylite_advisory_lock *previous,
                              ...
                              struct mylite_expression_warnings *warnings)
{
    if (name == NULL) {
        ...
        return set_lock_name_error(database, warnings, MYLITE_MYSQL_ER_USER_LOCK_WRONG_NAME,
                                   null_name_message);
    }
    ...
    if (length > MYLITE_ADVISORY_LOCK_NAME_MAX) {
        ...
        status = set_lock_name_error(database, warnings, MYLITE_MYSQL_ER_USER_LOCK_NAME_TOO_LONG,
                                     message);
        sqlite3_free(message);
        return status;
    }
                               ...
                               unsigned int code, const char *message)
{
    ...
    if (mylite_expression_warnings_append_condition(warnings, MYLITE_EXPRESSION_WARNING_LEVEL_ERROR,
                                                    code, message) != 0) {
        (void)mylite_diagnostics_set_error_message(database, "out of memory");
        return MYLITE_NOMEM;
    }
