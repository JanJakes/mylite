...
| `SHOW GRANTS` | ❌ | high | Current-user and named-account grant rows, role `USING` behavior, privilege checks, and MySQL 8.4 global/dynamic privilege formatting. | Parser records the user object kind for current-user grants and preserves explicit `FOR` account names. |
