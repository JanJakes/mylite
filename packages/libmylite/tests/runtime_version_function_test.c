...
static int test_version_function_unsupported_forms(void) {
    ...
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "SELECT VERSION() FROM t", &result);
    failures += expect_scalar_result(
        result,
        (struct expected_scalar_result){
            .columns = (const char *const[]){"VERSION()"},
            .values = version_limit_values,
            .count = 1U,
            .context = "source-backed version value",
        }
    );
    mylite_result_free(result);
    result = NULL;
...
static int test_version_system_variable_independent_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first version handle");
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second version handle"
    );
