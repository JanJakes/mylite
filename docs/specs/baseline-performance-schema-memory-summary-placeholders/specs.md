# Baseline Performance Schema Memory Summary Placeholders
...
returns zero rows from each table until live Performance Schema memory
...
```sh
packages/libmylite/tests/mysql_baseline_performance_schema_memory_summary_placeholders_expectations.sh
```
...
The account, host, and user summary tables expose unique HASH indexes named
`ACCOUNT`, `HOST`, and `USER`. The thread summary table exposes a composite
primary HASH key on `THREAD_ID, EVENT_NAME`. The global summary table exposes a
primary HASH key on `EVENT_NAME`.

## Intentional Gaps

This slice does not implement live Performance Schema memory instrumentation,
memory event registration, per-thread/account/user/host aggregation, high/low
watermark tracking, or reset behavior. A MySQL 8.4.9 runtime exposes nonzero
rows for memory instruments; MyLite intentionally returns empty rows until it
has a real instrumentation pipeline and an explicit summary lifecycle.
...
- Build and run `mylite_runtime_performance_schema_memory_summary_placeholders_test`.
- Run the focused CTest filter
  `^libmylite\.runtime\.performance_schema_memory_summary_placeholders$`.
