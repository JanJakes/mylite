...
static int expect_log_bin_trust_deprecation_warnings(
    ...
    for (size_t row = 0U; row < row_count; ++row) {
        ...
        failures +=
            mylite_test_expect_text(mylite_result_value_text(result, row, 1U), "1287", context);
...
static int expect_set_ok(mylite_db *database, const char *sql) {
    ...
    failures += mylite_test_expect_size(
        mylite_result_column_count(result),
        0U,
        "server identity SET columns"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "server identity SET rows");
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        0,
        "server identity SET affected"
    );
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "server identity SET warnings"
    );
    mylite_result_free(result);
