...
static int test_no_source_dual_and_do_from_unixtime(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "from_unixtime do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "from_unixtime do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "from_unixtime do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "from_unixtime do warnings"
        );
    }
...
static int test_table_backed_from_unixtime_and_reopen(void) {
    ...
    database = NULL;
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen from_unixtime");
