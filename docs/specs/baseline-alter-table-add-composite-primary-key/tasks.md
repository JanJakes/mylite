...
- [x] Extend the current ALTER ADD PRIMARY KEY planner from one key column to
  an ordered key-part list while preserving the one-column path.
- [x] Resolve every key part from descriptors, rejecting qualified parts,
