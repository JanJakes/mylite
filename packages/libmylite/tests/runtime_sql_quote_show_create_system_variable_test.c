...
static int test_sql_quote_show_create_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql quote file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql quote file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_quote_show_create, @@global.sql_quote_show_create",
        (struct expected_result){
...
static int test_sql_quote_show_create_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql quote memory");
    failures += execute_ok(
        database,
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_quote_show_create_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
