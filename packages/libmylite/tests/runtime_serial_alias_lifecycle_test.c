...
static int test_serial_alias_metadata_dml_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open serial metadata file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen serial metadata file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_serial_alias_forms_and_diagnostics(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open serial forms file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int test_serial_alias_independent_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first serial file");
    ...
    failures += expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second serial file");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_serial_alias_lifecycle_%ld_%s.mylite",
