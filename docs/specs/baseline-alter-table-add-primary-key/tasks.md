...
- [x] Add SQLite-side existing-row validation for `NULL` and duplicate target
  values without materializing the full table in C memory.
- [x] Add catalog mutation support to make the descriptor column `NOT NULL`,
...
- [x] Create the generated SQLite physical unique index from stable descriptor
...
- [x] Ensure future DML, `TRUNCATE`, `CREATE TABLE ... LIKE`,
  ...
  paths.
- [x] Reject unsupported target types, composite keys, named constraints,
