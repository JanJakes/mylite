...
| `sql_replica_skip_counter` | ✅ | Fixed scalar/`SHOW VARIABLES` value `0`, global-only diagnostics, and exact/default global no-op `SET` forms; no mutable global state, replica SQL thread state, relay log/event skipping, channels, GTID restrictions, or Performance Schema variable tables |
