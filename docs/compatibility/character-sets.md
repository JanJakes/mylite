...
| `utf8mb4` | 🟡 | Limited static `SHOW CHARACTER SET` row, one-row `INFORMATION_SCHEMA.CHARACTER_SETS` metadata, fixed `CREATE TABLE` option acceptance, fixed metadata no-op `ALTER TABLE ... [DEFAULT] CHARSET` / `CHARACTER SET utf8mb4`, scalar `@@character_set_client` / `@@character_set_connection` / `@@character_set_results` reads, scalar `@@character_set_server`, scalar `@@character_set_database`, fixed no-op `SET NAMES` / `SET CHARACTER SET` forms that preserve `utf8mb4`, and limited UTF-8 validation for `CHAR`, `VARCHAR(0..16383)`, and baseline `TEXT` family storage; no conversions, alternate column charsets, mutable connection/server/database/table state, or full catalog semantics |
