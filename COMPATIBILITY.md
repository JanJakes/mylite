...
| `CREATE TABLE` | ❌ | top | Column definitions, constraints, indexes, table options, generated columns, and partitions; see section 3.1. | Parser requires non-empty comma-separated table-definition elements, validates column definitions start with a known MySQL type family, preserves nested column/constraint definition bodies, validates table-level index key-part prefix lengths plus `ASC`/`DESC` tails, validates table-level foreign-key child/reference column-list envelopes plus referential-action tails, requires non-empty column/table `CHECK` expression bodies and validates table-level `CHECK` `ENFORCED`/`NOT ENFORCED` tails, validates table-option starts including `AUTOEXTEND_SIZE` and `START TRANSACTION`, and recognizes no-definition and post-definition CTAS forms including `IGNORE`/`REPLACE` duplicate-handling modifiers. |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| Column definition grammar | ❌ | top | Type, nullability, defaults, visibility, comments, storage, format, references, and constraints. | `ALTER TABLE ADD`/`CHANGE`/`MODIFY` validates type starts, common attribute starts, and closed `COLUMN_FORMAT`/`STORAGE` values while keeping expression bodies permissive. |
