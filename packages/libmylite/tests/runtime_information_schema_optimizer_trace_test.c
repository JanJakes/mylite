...
static int test_information_schema_optimizer_trace_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open optimizer trace db");
...
static int test_information_schema_optimizer_trace_reopen_preamble_and_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first trace db");
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second trace db"
    );
    failures += expect_query(
        first,
        (struct expected_query){
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first trace db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.OPTIMIZER_TRACE",
