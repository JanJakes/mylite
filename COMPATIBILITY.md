...
| `SHOW COUNT(*) ERRORS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Implemented as part of the first diagnostic `SHOW ERRORS` slice, returning a one-row `@@session.error_count` result for stored error conditions while preserving the diagnostics area. `LIKE`/`WHERE` and `LIMIT` are syntax errors for this count form, matching MySQL. Complete MySQL error-code/SQLSTATE propagation, `max_error_count`, SQL `@@error_count`, and `GET DIAGNOSTICS` remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md). |
| `SHOW COUNT(*) WARNINGS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Implemented as part of the first diagnostic `SHOW WARNINGS` slice, returning a one-row `@@session.warning_count` result for stored warning, error, and note conditions while preserving the diagnostics area. `LIKE`/`WHERE` and `LIMIT` are syntax errors for this count form, matching MySQL. Broader note generation, `sql_notes`, `max_error_count`, SQL `@@warning_count`, and `GET DIAGNOSTICS` remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md). |
...
| `SHOW ERRORS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First diagnostic slice is implemented for current handle-owned MyLite error conditions, including `SHOW ERRORS`, `SHOW ERRORS LIMIT row_count`, `LIMIT offset, row_count`, `LIMIT row_count OFFSET offset`, and `SHOW COUNT(*) ERRORS`, with MySQL-compatible result column names and diagnostic-statement preservation. `LIKE`/`WHERE` remain syntax errors because MySQL does not accept them for this statement. Complete MySQL error-code/SQLSTATE propagation, broader note handling, `max_error_count`, `@@error_count`, and `GET DIAGNOSTICS` remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md). |
...
| `SHOW VARIABLES` | 🟡 | top | Result-set shape, filtering, scope modifiers, system-variable values, and MySQL 8.4-compatible diagnostics. | First executable slice is implemented for `SHOW [GLOBAL\|SESSION\|LOCAL] VARIABLES [LIKE 'pattern' \| WHERE expr]`, including MySQL-compatible `Variable_name` / `Value` columns, `LOCAL` as a session synonym, deterministic variable-name ordering, case-insensitive `LIKE` filtering with backslash escapes, and a practical catalog covering connection charset/collation state, schema/server charset defaults, autocommit, transaction defaults, diagnostics counters/defaults, mutable session `sql_mode`, mutable session `group_concat_max_len`, `sql_notes`, and version variables. `WHERE` is parsed but returns `SHOW VARIABLES WHERE is not supported` until shared SHOW filtering lands. Complete variable catalog, `SELECT @@...`, Performance Schema variable tables, persisted variables, and mutable global/session separation remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md), [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md), and [`group_concat_max_len` system variable spec](docs/specs/group-concat-max-len-variable/specs.md). |
| `SHOW WARNINGS` | 🟡 | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | First diagnostic slice is implemented for current handle-owned MyLite warning, error, and implemented note conditions, including `SHOW WARNINGS`, `SHOW WARNINGS LIMIT row_count`, `LIMIT offset, row_count`, `LIMIT row_count OFFSET offset`, and `SHOW COUNT(*) WARNINGS`, with MySQL-compatible result column names and diagnostic-statement preservation. `LIKE`/`WHERE` remain syntax errors because MySQL does not accept them for this statement. Broader note generation, `sql_notes`, separate generated-condition counters under `max_error_count`, `@@warning_count`, and `GET DIAGNOSTICS` remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md). |
...
| `autocommit` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes fixed `ON` for the current default autocommit behavior documented by Task 21; direct `SET autocommit` remains unimplemented. See [transaction statements spec](docs/specs/transaction-statements/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `character_set_client` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_connection` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_database` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Reflected through schema catalog defaults for selected-schema behavior in `SET CHARACTER SET` and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_filesystem` | 🟡 | high | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes fixed `binary` for this embedded runtime; direct assignment and broader filesystem path semantics are deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_results` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_server` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Fixed internally as `utf8mb4` for default connection and schema behavior and visible through the first `SHOW VARIABLES` slice; direct assignment is deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_set_system` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes fixed `utf8mb3`, matching MySQL 8.4's system charset convention for this slice. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `character_sets_dir` | 🟡 | high | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes an empty embedded-runtime value because MyLite uses a built-in charset registry instead of a filesystem charset directory. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `collation_connection` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state, updated by `SET NAMES`/`SET CHARACTER SET`, and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `collation_database` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Reflected through schema catalog defaults for selected-schema behavior in `SET CHARACTER SET` and visible through the first `SHOW VARIABLES` slice; direct assignment and `SELECT @@...` are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `collation_server` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Fixed internally as `utf8mb4_0900_ai_ci` for default connection and schema behavior and visible through the first `SHOW VARIABLES` slice; direct assignment is deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `error_count` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes session `error_count=0` for the statement itself, matching MySQL nondiagnostic clearing behavior; SQL `@@error_count` and full generated-condition counter semantics are deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `group_concat_max_len` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Stored as handle-owned session state with MySQL default `1024`, exposed through `SHOW VARIABLES`, mutable through session/local `SET` and `DEFAULT`, clamped to minimum `4` with warning 1292, and applied to `GROUP_CONCAT()` truncation and metadata. Global mutation, persisted values, `SELECT @@...`, arbitrary assignment expressions, and exact binary-argument metadata remain deferred. See [`group_concat_max_len` system variable spec](docs/specs/group-concat-max-len-variable/specs.md), [GROUP_CONCAT function spec](docs/specs/group-concat-function/specs.md), and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `max_error_count` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes the MySQL default `1024`; mutable storage caps and assignment behavior are deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `sql_mode` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | MyLite stores handle-owned session `sql_mode`, initializes it to the MySQL 8.4 default, supports session/local assignment plus `DEFAULT` and the focused `REPLACE(@@SESSION.sql_mode, 'ONLY_FULL_GROUP_BY', '')` removal idiom, canonicalizes recognized mode lists, reports error 1231 for unknown modes, exposes session/global values through `SHOW VARIABLES`, and applies `ONLY_FULL_GROUP_BY` to current grouping and DISTINCT-order validators. `SELECT @@...`, persisted/global mutation, and complete mode-sensitive behavior outside this slice remain deferred. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md) and [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md). |
| `sql_notes` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes `ON`; mutable assignment and complete note-generation gating remain deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `transaction_isolation` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes fixed `REPEATABLE-READ`; isolation-level assignment and enforcement remain deferred. See [transaction statements spec](docs/specs/transaction-statements/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `transaction_prealloc_size` | ❌ | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. |  |
| `transaction_read_only` | 🟡 | high | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes fixed `OFF` for the default transaction mode, matching MySQL's behavior that `START TRANSACTION READ ONLY` does not change this variable; mutable session defaults remain deferred. See [transaction statements spec](docs/specs/transaction-statements/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `version` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes `mylite_version()`; broader protocol and `VERSION()` surfaces are tracked separately. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `version_comment` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes an embedded-runtime comment rather than pretending to be a GPL MySQL Community Server build. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `version_compile_machine` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes an empty embedded-runtime value until MyLite defines stable compile-machine reporting. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `version_compile_os` | 🟡 | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes an empty embedded-runtime value until MyLite defines stable compile-OS reporting. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
| `version_compile_zlib` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes an empty embedded-runtime value until MyLite defines stable zlib-version reporting. See [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
...
| `warning_count` | 🟡 | medium | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | `SHOW VARIABLES` exposes session `warning_count=0` for the statement itself, matching MySQL nondiagnostic clearing behavior; SQL `@@warning_count` and full generated-condition counter semantics are deferred. See [SHOW diagnostics spec](docs/specs/show-diagnostics/specs.md) and [SHOW VARIABLES spec](docs/specs/show-variables/specs.md). |
