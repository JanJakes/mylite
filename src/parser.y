...
%token SET_T SHOW_T USE_T DESCRIBE_T DESC_T EXPLAIN_T HELP_T START_T STOP_T BEGIN_T
...
%token UNINSTALL_T CACHE_T CLONE_T CHANGE_T BINLOG_T PURGE_T SIGNAL_T RESIGNAL_T GET_T
%token CLOSE_T DECLARE_T FETCH_T ITERATE_T LEAVE_T OPEN_T RETURN_T
...
%token CHAIN_T COLLATE_T COLUMNS_T CONSTRAINT_T CROSS_T CURSOR_T DATA_T DATABASE_T
%token DEFAULT_T DELAYED_T DISTINCT_T ENGINE_T EVENT_T EXISTS_T FALSE_T FIELDS_T
%token FORMAT_T FROM_T FULL_T FUNCTION_T GROUP_T HAVING_T HIGH_PRIORITY_T
%token IGNORE_T IN_T INDEX_T INFILE_T INNER_T INTO_T IS_T JOIN_T JSON_T KEY_T
%token LEFT_T LIKE_T LIMIT_T LOCAL_T LOW_PRIORITY_T NATURAL_T NO_T NOT_T NULL_T
%token OFFSET_T ON_T OR_T ORDER_T OUTER_T PRIMARY_T PROCEDURE_T QUICK_T READ_T
%token REFERENCES_T REGEXP_T RIGHT_T RLIKE_T ROLE_T SCHEMA_T SPATIAL_T
%token TEMPORARY_T TO_T TRANSACTION_T TRIGGER_T TRUE_T UNION_T UNIQUE_T USER_T
%token USING_T VALUE_T VIEW_T WHEN_T WHERE_T WRITE_T XOR_T
...
statement_head:
	...
	| STOP_T        { BEGIN_STATEMENT(MYLITE_STATEMENT_STOP, 1); }
	...
	| CLONE_T       { BEGIN_STATEMENT(MYLITE_STATEMENT_CLONE, 1); }
	...
	| RESIGNAL_T    { BEGIN_STATEMENT(MYLITE_STATEMENT_RESIGNAL, 0); }
	| GET_T         { BEGIN_STATEMENT(MYLITE_STATEMENT_GET, 1); }
	| DECLARE_T     { BEGIN_STATEMENT(MYLITE_STATEMENT_DECLARE, 1); }
	| OPEN_T        { BEGIN_STATEMENT(MYLITE_STATEMENT_OPEN, 1); }
	| FETCH_T       { BEGIN_STATEMENT(MYLITE_STATEMENT_FETCH, 1); }
	| CLOSE_T       { BEGIN_STATEMENT(MYLITE_STATEMENT_CLOSE, 1); }
	| IF_T          { BEGIN_STATEMENT(MYLITE_STATEMENT_IF, 1); }
	| CASE_T        { BEGIN_STATEMENT(MYLITE_STATEMENT_CASE, 1); }
	| LOOP_T        { BEGIN_STATEMENT(MYLITE_STATEMENT_LOOP, 1); }
	| REPEAT_T      { BEGIN_STATEMENT(MYLITE_STATEMENT_REPEAT, 1); }
	| WHILE_T       { BEGIN_STATEMENT(MYLITE_STATEMENT_WHILE, 1); }
	| LEAVE_T       { BEGIN_STATEMENT(MYLITE_STATEMENT_LEAVE, 1); }
	| ITERATE_T     { BEGIN_STATEMENT(MYLITE_STATEMENT_ITERATE, 1); }
	| RETURN_T      { BEGIN_STATEMENT(MYLITE_STATEMENT_RETURN, 1); }
