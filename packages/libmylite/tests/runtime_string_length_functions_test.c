...
static int test_no_source_dual_and_do_lengths(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "string length do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "string length do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "string length do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "string length do warnings"
        );
    }
