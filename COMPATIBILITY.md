...
| `INSERT ... VALUES` | 🟡 | Limited single- and multi-row inserts into persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, no-column-list mapping to visible descriptor columns only, explicit column-list access to invisible columns, omitted-column descriptor default filling, strict range/nullability/no-default checks, affected rows, and statement atomicity; no DML `DEFAULT` keyword values, keys, generated values, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SET` | 🟡 | Limited one-row `INSERT [INTO] table SET column = integer_or_NULL_or_boolean[, ...]` into persistent base tables with descriptor assignment resolution including explicitly named invisible columns, omitted-column descriptor default filling, strict required-column/range/nullability diagnostics, affected rows, and statement atomicity; no modifiers, qualified assignment targets, expressions, DML `DEFAULT` keyword values, generated values, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SELECT` | 🟡 | Limited descriptor-backed `INSERT [INTO] target [(columns)] SELECT ... FROM source` over persistent base tables, reusing the existing source `SELECT` column/alias/WHERE/ORDER/LIMIT subset and descriptor-driven target defaults/nullability/range checks; no modifiers, `IGNORE`, partitions, joins, expressions, literal projection sources, `TABLE`, row constructors, `ON DUPLICATE KEY UPDATE`, keys, auto-increment, triggers, or warning demotion. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
