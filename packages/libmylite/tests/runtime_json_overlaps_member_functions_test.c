...
static int test_no_source_dual_and_do_json_overlaps_member(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "json overlaps member do columns"
        );
        failures += mylite_test_expect_size(
            mylite_result_row_count(result),
            0U,
            "json overlaps member do rows"
        );
...
static int open_app_database(
    ...
    if (failures == 0) {
        remove_related_files(path);
        failures +=
            mylite_test_expect_int(mylite_open(path, out_database), MYLITE_OK, "open database");
    }
    if (failures == 0) {
