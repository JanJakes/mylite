...
static int test_no_source_dual_and_do_time_second_conversion(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "time second do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "time second do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "time second do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "time second do warnings");
    }
...
static int test_table_backed_time_second_conversion_and_reopen(void) {
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen time second");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-time-second-conversion-functions-%s-%d.mylite",
        name,
        current_process_id()
    );
