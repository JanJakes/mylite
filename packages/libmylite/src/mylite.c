...
enum mylite_stmt_kind {
    ...
    MYLITE_STMT_SAVEPOINT = 19,
    MYLITE_STMT_ROLLBACK_TO_SAVEPOINT = 20,
    MYLITE_STMT_RELEASE_SAVEPOINT = 21,
};
...
static void field_descriptor_set_nullable(struct mylite_field_descriptor *descriptor,
                                          bool nullable);
static void field_descriptor_set_not_null(struct mylite_field_descriptor *descriptor,
                                          bool not_null);
...
static int load_select_column_from_catalog_row(mylite_db *database,
                                               struct mylite_select_table *table,
                                               sqlite3_stmt *select);
static int resolve_select_using_requests(mylite_db *database, struct mylite_select_plan *plan);
...
static int add_select_order_key(struct mylite_select_plan *plan,
                                const struct mylite_select_order_key *order_key);
static int add_select_group_key(struct mylite_select_plan *plan,
                                const struct mylite_select_group_key *group_key);
...
static void mark_select_output_order_reference(struct mylite_select_plan *plan,
                                               size_t output_index);
static size_t select_plan_table_count(const struct mylite_select_plan *plan);
static struct mylite_select_table *select_plan_table(struct mylite_select_plan *plan,
                                                     size_t table_index);
static const struct mylite_select_table *
select_plan_table_const(const struct mylite_select_plan *plan, size_t table_index);
static size_t select_plan_column_count(const struct mylite_select_plan *plan);
...
static int evaluate_table_select_join_stage_conditions_uncached(
    ...
    bool *out_matches);
static bool select_join_stage_cache_range(mylite_db *database,
                                          const struct mylite_select_plan *plan,
...
static bool select_join_stage_cache_range_merge(struct mylite_select_table_range candidate,
                                                struct mylite_select_table_range *out_range,
                                                bool *out_seen);
static int lookup_table_select_join_condition_cache(
...
static int append_table_select_result_row_copy(mylite_stmt *stmt,
                                               const struct mylite_table_select_row *row);
static int table_select_row_copy(const struct mylite_table_select_row *row,
                                 struct mylite_table_select_row *out_row);
...
static int materialize_table_select_output_values(mylite_stmt *stmt,
                                                  struct mylite_table_select_row *row);
static bool table_select_distinct_row_exists(const mylite_stmt *stmt,
                                             const struct mylite_table_select_row *row);
static bool table_select_output_values_equal(const mylite_stmt *stmt,
                                             const struct mylite_table_select_row *left,
                                             const struct mylite_table_select_row *right);
static int compare_table_select_distinct_values(const struct mylite_expression_value *left,
                                                const struct mylite_expression_value *right,
...
static int rollback_to_user_savepoint(mylite_db *database, size_t index);
static int release_user_savepoint(mylite_db *database, size_t index);
static size_t find_user_savepoint(const mylite_db *database, const char *normalized_name);
...
static void savepoint_state_deinit(struct mylite_savepoint_state *state);
...
void mylite_close(mylite_db *database)
{
    ...
    savepoint_state_deinit(&database->savepoints);
                                          ...
                                          mylite_stmt **out_stmt)
{
    ...
    if (status == MYLITE_OK && select_plan_table_count(&plan) > 1U &&
                                              ...
                                              size_t output_index)
{
    ...
    const struct mylite_select_column *column =
        select_plan_column_const(plan, output->column_index, &table);
...
static int infer_identifier_descriptor(mylite_db *database, const struct mylite_select_plan *plan,
                                       ...
                                       struct mylite_field_descriptor *out_descriptor)
{
    size_t column_index = plan == NULL ? 0U : select_plan_column_count(plan);
                                    ...
                                    struct mylite_field_descriptor *out_descriptor)
{
    ...
    case MYLITE_SQL_AST_LITERAL_NATIONAL_STRING:
        ...
        field_descriptor_set_not_null(&descriptor, true);
        break;
                                             ...
                                             struct mylite_field_descriptor *out_descriptor)
{
    ...
    case MYLITE_SQL_AST_OPERATOR_NEGATIVE:
        ...
        field_descriptor_set_nullable(&operand, nullable);
                         ...
                         const struct mylite_case_descriptor_aggregate *aggregate)
{
    ...
    field_descriptor_set_nullable(&descriptor, aggregate->nullable);
    return descriptor;
}
                                        ...
                                        struct mylite_field_descriptor *out_descriptor)
{
    ...
    field_descriptor_set_nullable(&descriptor, source.nullable);
...
static void field_descriptor_set_nullable(struct mylite_field_descriptor *descriptor, bool nullable)
{
    if (nullable) {
        field_descriptor_set_not_null(descriptor, false);
    } else {
        field_descriptor_set_not_null(descriptor, true);
    }
}

static void field_descriptor_set_not_null(struct mylite_field_descriptor *descriptor, bool not_null)
{
    if (descriptor == NULL) {
        return;
    }
...
static uint64_t catalog_integer_display_width(const char *column_type)
{
    ...
    for (const char *cursor = open + 1; cursor < close; ++cursor) {
        ...
        value = (value * mylite_decimal_radix) + (uint64_t)(*cursor - '0');
    }
    return value;
}
                                                 ...
                                                 struct mylite_select_table_range *out_range)
{
    ...
    *out_range = (struct mylite_select_table_range){
        .first_table = select_plan_table_count(plan),
    };
                                            ...
                                            struct mylite_select_plan *plan)
{
    ...
    return select_plan_table_count(plan) == 0U ? MYLITE_UNSUPPORTED : MYLITE_OK;
}
...
static int validate_select_table_aliases(mylite_db *database, const struct mylite_select_plan *plan)
{
    ...
    for (size_t left = 0U; left < table_count; ++left) {
        const struct mylite_select_table *left_table = select_plan_table_const(plan, left);
        const char *left_name =
            left_table->alias == NULL ? left_table->table_name : left_table->alias;

        for (size_t right = left + 1U; right < table_count; ++right) {
            const struct mylite_select_table *right_table = select_plan_table_const(plan, right);
...
static int load_select_plan_columns(mylite_db *database, struct mylite_select_plan *plan)
{
    ...
    for (size_t index = 0U; index < table_count; ++index) {
        ...
        status = load_select_columns(database, table);
        if (status != MYLITE_OK) {
            return status;
        }
                                               ...
                                               size_t *out_column)
{
    ...
    for (size_t index = 0U; index < range.table_count; ++index) {
        const struct mylite_select_table *table =
            select_plan_table_const(plan, range.first_table + index);
                                            ...
                                            const struct mylite_select_plan *plan)
{
    return bind_select_predicate_expression_in_clause(database, expression, plan, "where clause",
                                                      0U, select_plan_table_count(plan));
}
                                                      ...
                                                      size_t first_table, size_t table_count)
{
    ...
    case MYLITE_SQL_AST_QUALIFIED_IDENTIFIER: {
        ...
        return status == MYLITE_OK && column_index >= select_plan_column_count(plan)
                   ? MYLITE_UNSUPPORTED
                   : status;
    }
    case MYLITE_SQL_AST_UNARY_EXPRESSION:
    ...
    *out_resolved = false;
    ...
    if (part_count == 1U) {
        size_t column_matches = count_select_plan_column_parts_matches(
            plan, parts, part_count, 0U, select_plan_table_count(plan), &column_index);
                                          ...
                                          struct mylite_select_plan *plan)
{
    ...
    size_t table_index = select_plan_table_count(plan);
    int status = resolve_select_plan_wildcard(database, plan, wildcard, &table_index, &all_tables);

    if (status != MYLITE_OK) {
        return status;
    }
    if (!all_tables && table_index == select_plan_table_count(plan)) {
...
static int append_select_all_wildcard_outputs(mylite_db *database, struct mylite_select_plan *plan)
{
    if (plan->from_range_count == 0U) {
        return append_select_range_wildcard_outputs(
            ...
            (struct mylite_select_table_range){
                .first_table = 0U,
                .table_count = select_plan_table_count(plan),
            });
    }
                                       ...
                                       struct mylite_select_plan *plan)
{
    ...
    if (column_index == select_plan_column_count(plan)) {
        return MYLITE_UNSUPPORTED;
    }
                                        ...
                                        size_t *out_table_index, bool *out_all)
{
    ...
    *out_all = false;
    ...
    for (size_t index = 0U; index < table_count; ++index) {
        const struct mylite_select_table *table = select_plan_table_const(plan, index);
...
static int resolve_select_plan_column_reference(mylite_db *database,
                                                ...
                                                const char *clause_context, size_t *out_index)
{
    return resolve_select_plan_column_reference_in_scope(
        database, plan, expression, clause_context, 0U, select_plan_table_count(plan), out_index);
}
...
static int resolve_select_plan_column_parts(mylite_db *database,
                                            ...
                                            size_t *out_index)
{
    return resolve_select_plan_column_parts_in_scope(database, plan, parts, part_count,
                                                     clause_context, 0U,
                                                     select_plan_table_count(plan), out_index);
}
...
static int resolve_select_plan_column_parts_in_scope(mylite_db *database,
                                                     ...
                                                     size_t table_count, size_t *out_index)
{
    size_t plan_table_count = select_plan_table_count(plan);
    size_t match_count = 0U;
    size_t match_index = select_plan_column_count(plan);
                                                     ...
                                                     size_t *match_index)
{
    ...
    *match_index = select_plan_column_count(plan);
    ...
    struct mylite_select_distinct_order_column_error_context context)
{
    const struct mylite_select_table *table = NULL;
    const struct mylite_select_column *column =
        select_plan_column_const(plan, context.column_index, &table);
...
static char *build_select_physical_sql(mylite_db *database, const struct mylite_select_plan *plan)
{
    ...
    for (size_t index = 0U; index < plan->output_count; ++index) {
        const struct mylite_select_output_column *output = &plan->outputs[index];
        const struct mylite_select_column *column =
            select_plan_column_const(plan, output->column_index, NULL);
    ...
    sqlite3_str_appendf(sql, " FROM \"%w\"", select_plan_table_const(plan, 0U)->physical_name);
...
static char *build_select_scan_sql(mylite_db *database, const struct mylite_select_plan *plan)
{
    ...
    sqlite3_str_append(sql, "SELECT ", (int)strlen("SELECT "));
    for (size_t table_index = 0U; table_index < select_plan_table_count(plan); ++table_index) {
        const struct mylite_select_table *table = select_plan_table_const(plan, table_index);
...
static int collect_table_select_aggregate_bindings(mylite_stmt *stmt)
{
    ...
    clear_select_aggregate_bindings(&stmt->select_plan);
                                    ...
                                    mylite_stmt **out_stmt)
{
    ...
    *stmt = (mylite_stmt){
    ...
    case MYLITE_STMT_RELEASE_SAVEPOINT:
        status = copy_savepoint_statement(statement, stmt);
        break;
...
static int execute_custom_statement(mylite_stmt *stmt)
{
    ...
    case MYLITE_STMT_ROLLBACK:
        ...
        break;
    case MYLITE_STMT_SAVEPOINT:
        status = execute_savepoint_statement(stmt);
        break;
    case MYLITE_STMT_ROLLBACK_TO_SAVEPOINT:
        status = execute_rollback_to_savepoint_statement(stmt);
        break;
    case MYLITE_STMT_RELEASE_SAVEPOINT:
        status = execute_release_savepoint_statement(stmt);
        break;
    case MYLITE_STMT_SCALAR_SELECT:
        return execute_scalar_select_statement(stmt);
    case MYLITE_STMT_TABLE_SELECT:
...
static int execute_savepoint_statement(mylite_stmt *stmt)
{
    ...
    status = create_user_savepoint(stmt->database, stmt->savepoint.name,
...
static int execute_rollback_to_savepoint_statement(mylite_stmt *stmt)
{
    size_t index = find_user_savepoint(stmt->database, stmt->savepoint.normalized_name);
    int status = MYLITE_OK;
    ...
    status = rollback_to_user_savepoint(stmt->database, index);
    stmt->affected_rows = status == MYLITE_OK ? 0 : -1;
    return status;
}
...
static int execute_release_savepoint_statement(mylite_stmt *stmt)
{
    ...
    status = release_user_savepoint(stmt->database, index);
    stmt->affected_rows = status == MYLITE_OK ? 0 : -1;
    return status;
}
...
static int execute_table_select_statement(mylite_stmt *stmt)
{
    if (stmt->sqlite_stmt == NULL && select_plan_table_count(&stmt->select_plan) <= 1U) {
        return MYLITE_MISUSE;
    }
    stmt->executed = true;
    stmt->affected_rows = -1;
...
static int materialize_table_select_result(mylite_stmt *stmt)
{
    ...
    if (select_plan_table_count(&stmt->select_plan) > 1U) {
...
{
    const struct mylite_select_table *table =
        select_plan_table_const(&stmt->select_plan, table_index);
    sqlite3_stmt *scan = NULL;
    char *scan_sql = NULL;
    int rc = SQLITE_OK;
...
{
    const struct mylite_select_table *table =
        select_plan_table_const(&stmt->select_plan, scan->table_index);
                                              ...
                                              bool *out_finished)
{
    ...
    --scan->table_index;
    table = select_plan_table_const(&stmt->select_plan, scan->table_index);
    if (table == NULL) {
        return MYLITE_UNSUPPORTED;
    }
                                      ...
                                      const struct mylite_table_select_join_scan_state *scan)
{
    for (size_t index = 0U; index < scan->table_count; ++index) {
        const struct mylite_select_table *table =
            select_plan_table_const(&stmt->select_plan, index);
    ...
    const struct mylite_table_select_join_scan_state *scan, bool *out_matches)
{
    ...
    if (!select_join_stage_cache_range(stmt->database, &stmt->select_plan, scan->table_index + 1U,
                                       &cache_range)) {
        return evaluate_table_select_join_stage_conditions_uncached(
            stmt, scan->row, scan->table_index + 1U, out_matches);
    }

    status = lookup_table_select_join_condition_cache(&state->condition_cache, scan, cache_range,
                                                      &lookup);
    if (status != MYLITE_OK) {
        return status;
    }
    ...
    status = evaluate_table_select_join_stage_conditions_uncached(
        stmt, scan->row, scan->table_index + 1U, out_matches);
    if (status == MYLITE_OK) {
        status = store_table_select_join_condition_cache(stmt, &state->condition_cache, scan,
                                                         cache_range, *out_matches);
    }
    return status;
}
    ...
    *out_matches = true;
    for (size_t index = 0U; index < stmt->select_plan.using_column_count; ++index) {
        ...
        if (left->kind == MYLITE_EXPRESSION_VALUE_NULL ||
            ...
            compare_table_select_values(left, right) != 0) {
            *out_matches = false;
            return MYLITE_OK;
        }
    }
                                                 ...
                                                 bool *out_duplicate)
{
    ...
    *out_duplicate = table_select_distinct_row_exists(stmt, row);
    return MYLITE_OK;
}
...
static int copy_table_select_sqlite_row(mylite_stmt *stmt, struct mylite_table_select_row *out_row)
{
    size_t column_count = select_plan_column_count(&stmt->select_plan);
    ...
    *out_group = NULL;
    ...
    for (size_t group_index = 0U; group_index < group_count; ++group_index) {
        ...
        for (size_t value_index = 0U; matches && value_index < value_count; ++value_index) {
            if (compare_table_select_values(&groups[group_index].group_values[value_index],
...
{
    size_t column_count =
        row == NULL ? select_plan_column_count(&stmt->select_plan) : row->value_count;
                                     ...
                                     const struct mylite_table_select_row *row)
{
    for (size_t index = 0U; index < stmt->select_plan.aggregate_binding_count; ++index) {
        int status =
            update_table_select_aggregate_state(stmt, &group->aggregate_states[index],
                                                &stmt->select_plan.aggregate_bindings[index], row);
                                       ...
                                       struct mylite_table_select_row *out_row)
{
    ...
    for (size_t index = 0U; index < group->aggregate_state_count; ++index) {
        int status = finalize_table_select_aggregate_state(
                                    ...
                                    const struct mylite_table_select_row *row)
{
    ...
    case MYLITE_SQL_AST_AGGREGATE_AVG: {
        ...
        status = aggregate_value_to_double(&stmt->database->warnings, &value, &numeric);
        if (status == MYLITE_OK) {
    ...
    case MYLITE_SQL_AST_AGGREGATE_MAX:
        if (!state->has_value ||
            (binding->kind == MYLITE_SQL_AST_AGGREGATE_MIN &&
             compare_table_select_values(&value, &state->value) < 0) ||
                                      ...
                                      struct mylite_expression_value *out_value)
{
    ...
    case MYLITE_SQL_AST_AGGREGATE_SUM:
        ...
        return aggregate_format_double(state->sum, out_value);
    case MYLITE_SQL_AST_AGGREGATE_AVG:
        if (state->non_null_count == 0U) {
            *out_value = (struct mylite_expression_value){.kind = MYLITE_EXPRESSION_VALUE_NULL};
            return MYLITE_OK;
        ...
        return aggregate_format_double(state->sum / (double)state->non_null_count, out_value);
    case MYLITE_SQL_AST_AGGREGATE_MIN:
    case MYLITE_SQL_AST_AGGREGATE_MAX:
        if (!state->has_value) {
            *out_value = (struct mylite_expression_value){.kind = MYLITE_EXPRESSION_VALUE_NULL};
                                             ...
                                             const struct mylite_table_select_row *row)
{
    for (size_t index = 0U; index < stmt->select_result.row_count; ++index) {
        if (table_select_output_values_equal(stmt, &stmt->select_result.rows[index], row)) {
            return true;
        }
    }
    return false;
...
static bool table_select_output_values_equal(const mylite_stmt *stmt,
                                             ...
                                             const struct mylite_table_select_row *right)
{
    if (left->output_value_count != stmt->select_plan.output_count ||
        right->output_value_count != stmt->select_plan.output_count) {
        return false;
    }

    for (size_t index = 0U; index < stmt->select_plan.output_count; ++index) {
        const struct mylite_field_descriptor *descriptor =
            index < stmt->result_metadata.column_count
                ? &stmt->result_metadata.columns[index].descriptor
                : NULL;

        if (compare_table_select_distinct_values(&left->output_values[index],
                                                      ...
                                                      struct mylite_expression_value *out_value)
{
    ...
    if (status != MYLITE_OK ||
        column_index == select_plan_column_count(&context->stmt->select_plan)) {
        return -1;
    }
    if (context->row != NULL) {
                                        ...
                                        mylite_stmt *stmt)
{
    ...
    for (const struct mylite_sql_ast_node *item = select_list->first_child; item != NULL;
        ...
        status = infer_scalar_expression_descriptor(
            stmt->database, expression, &stmt->scalar_result.values[index],
            &stmt->result_metadata.columns[index].descriptor);
        if (status != MYLITE_OK) {
            return status;
        }
...
{
    struct mylite_aggregate_numeric_value numeric = {0};
    int status = aggregate_value_to_double(&stmt->scalar_result.warnings, argument, &numeric);
...
static int create_user_savepoint(mylite_db *database, const char *name, const char *normalized_name)
{
    ...
    if (status == MYLITE_OK) {
        size_t existing = find_user_savepoint(database, normalized_name);
