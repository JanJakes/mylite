...
description: Continuously advance MyLite compatibility by selecting and implementing batches of unimplemented features from COMPATIBILITY.md, optionally using subagents only when explicit delegation is allowed. Use only when the user explicitly asks for ongoing/batch compatibility work such as "work hard", "next batch", "keep advancing compatibility", or continuing beyond one completed feature. Do not use for simple git commands or one-off edits.
---
...
explicitly allow delegation. When allowed, cap live subagents at two: one
implementer for a disjoint feature or file ownership area and one
reviewer/verifier. Do not run many concurrent reviewers. Keep local work on the
...
3. For each feature, run the full path:
   ...
   - use `mylite-implement-feature` to implement grammar support first when
     syntax is involved, then runtime behavior and verification;
   - use `mylite-review-feature` to close documentation, tests, and review gaps.
4. Use subagents for feature work only when explicitly allowed. Give each
   subagent disjoint ownership, cap live agents as described above, and tell
   each one the repo has other active work and it must not revert others'
   changes.
