...
variable_assignment_output=$("$parser" "SELECT a INTO @x FROM t; SELECT a INTO local_var FROM t; SELECT a FROM t INTO @x; SELECT a INTO OUTFILE '/tmp/x' FROM t; SELECT a INTO DUMPFILE '/tmp/y' FROM t; SET @x = 1; SET @@session.sql_mode = 'ANSI'; SET SESSION sql_mode = 'ANSI'; SET sql_log_bin = 0; SET x = 1; GET DIAGNOSTICS @n = NUMBER; GET CURRENT DIAGNOSTICS CONDITION 1 @state = RETURNED_SQLSTATE")
...
case "$variable_assignment_output" in
	*"select"*/user_variable:@x*"select"*/local_variable:local_var*"select"*/user_variable:@x*"select"*/outfile:"'/tmp/x'"*"select"*/dumpfile:"'/tmp/y'"*"set"*/user_variable:@x*"set"*/system_variable:@@session.sql_mode*"set"*/system_variable:sql_mode*"set"*/system_variable:sql_log_bin*"get"*/user_variable:@n*"get"*/diagnostics_condition:1*) ;;
	*)
...
reset_output=$("$parser" 'RESET PERSIST max_connections; RESET PERSIST IF EXISTS autocommit; RESET PERSIST; RESET BINARY LOGS AND GTIDS; RESET BINARY LOGS AND GTIDS TO 100')
case "$reset_output" in
	*"reset"*/system_variable:max_connections*"reset"*/system_variable:autocommit*"reset[11:12"*"reset"*/binary_log*"reset"*/binary_log*) ;;
	*)
