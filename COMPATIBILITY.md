...
| --- | --- | --- | --- | --- |
| Bison parser foundation | 🟡 | top | Accept MySQL statement text through a Bison-owned parser boundary and produce a token stream plus AST shell for later analysis and translation. | Prototype in [docs/design/mysql-bison-parser.md](docs/design/mysql-bison-parser.md). Validates balanced groups, `BEGIN ... END`, `CASE ... END`, stored-program control token matching, semicolon-delimited compound and flow-control statement spans, statement sequencing, MySQL lexical forms including prefixed string/bit/hex literals, unary signs adjacent to assignment/comparison operators, and digit-leading identifiers, executable version-comment syntax gated for MySQL 8.4.9, expanded common keyword tokenization including `CLONE` and `STOP`, statement token spans, source byte spans, token stream spans, structural token matching, `WITH` outer DML classification, parenthesized query-expression classification including `TABLE` target names, SELECT/WITH query targets, SELECT projection and major-clause shell validation, standalone `VALUES` and `DO` query targets, DDL/admin object-kind classification including instance, server, logfile-group, tablespace, undo-tablespace, and resource-group targets, DML first-target table classification for `INSERT`, `REPLACE`, `UPDATE`, and `DELETE` including derived-table-leading `UPDATE` references, `INSERT` / `REPLACE` target and source skeleton validation, `UPDATE` target, assignment-list, and tail validation, `DELETE` single-table and multi-table structural validation, direct utility target classification for unambiguous table/schema statements including USE and TRUNCATE target validation, table-maintenance targets, and corpus-observed plural `TABLES` spellings, IMPORT SDI files plus basic file-list validation, CALL procedure targets plus basic call-shape validation, SIGNAL/RESIGNAL SQLSTATE and condition targets plus signal-information item-list validation, cache-index, load-index, KILL query and connection processlist-id expressions plus basic syntax validation, DESCRIBE/EXPLAIN table and query shell validation plus EXPLAIN user-variable and connection targets, SHOW PROFILE query targets and ids, SHOW character-set, collation, database, diagnostics-area, engine, plugin, privilege, processlist, routine-status, schema-scoped table/event/trigger collection and pattern, system-variable, status-variable, binary-log, relay-log, replica-status, and grant targets plus conservative SHOW statement-shape validation, BINLOG event payload validation, binary log files, binary-log collection metadata, SELECT and TABLE OUTFILE/DUMPFILE export files, relay log targets, FLUSH table names and global option targets, RESET PERSIST variables, explicit and unadorned assignment variable targets including qualified, structured, and keyword-shaped system variables for SELECT INTO, TABLE INTO, SET, and GET DIAGNOSTICS forms, GET DIAGNOSTICS optional-area, condition-number, target, and statement/condition item-list validation, SET character-set targets, generic SET assignment-list validation, SET transaction characteristic validation, transaction-control boundaries, replication channels including SHOW REPLICAS, SHOW REPLICA STATUS, and FLUSH RELAY LOGS, XA transaction XIDs and recovery collection targets, HELP quoted and unquoted topics, server lifecycle instance targets, unknown statement rejection with a narrow charset-fixture escape, and conservative `DESCRIBE`, `EXPLAIN`, `SHOW`, `CREATE`, `ALTER`, `DROP`, and `RENAME` forms including explicit schema-scoped, routine-code, and engine diagnostic SHOW targets, prepared-statement handle classification, component/plugin target classification, account/principal target classification including account-management `SET` and account-introspection `SHOW` forms, savepoint target classification, stored-program declaration validation, statement-head, local-variable, cursor-handle, condition-declaration, handler-condition, diagnostics-condition, label-target, and leading-label classification, and first target-name spans, including qualified names with reserved words after qualifier dots. Corpus gate parses 69,577 WordPress SQLite Database Integration MySQL queries, but detailed statement productions and semantic validation are still incomplete. |
...
| `ALTER EVENT` | ❌ | medium | Event scheduler metadata and body changes. | Parser records the event target and validates optional definer, schedule, completion, rename, status, comment, and body clause shapes; scheduler metadata changes, event execution, privilege checks, warnings, and diagnostics are not implemented. |
| `ALTER FUNCTION` | ❌ | medium | Stored-function metadata characteristics. | Parser records the function target and validates documented routine characteristics (`COMMENT`, `LANGUAGE SQL`, SQL data-access clauses, and `SQL SECURITY`); routine metadata changes, privilege checks, stored `sql_mode`, warnings, and diagnostics are not implemented. |
| `ALTER INSTANCE` | ❌ | low | Instance reload, TLS, keyring, and master-key operations with embedded-compatible behavior. | Parser records the instance target and validates documented redo-log enable/disable, InnoDB/binlog master-key rotation, TLS reload channel/no-rollback, and keyring reload forms; embedded-compatible operation handling, privilege checks, warnings, and diagnostics are not implemented. |
...
| `ALTER PROCEDURE` | ❌ | medium | Stored-procedure metadata characteristics. | Parser records the procedure target and validates documented routine characteristics (`COMMENT`, `LANGUAGE SQL`, SQL data-access clauses, and `SQL SECURITY`); routine metadata changes, privilege checks, stored `sql_mode`, warnings, and diagnostics are not implemented. |
...
| `ALTER TABLE` | ❌ | top | Full table rebuild/in-place/instant surface; see section 3.2. | Parser records the table target and validates top-level action-list structure for common `ADD`, `DROP`, `CHANGE`, `MODIFY`, `ALTER`, `RENAME`, `ORDER BY`, algorithm, lock, validation, partition, and table-option heads; detailed column, index, constraint, partition, rebuild, online-DDL, metadata, warning, and diagnostic semantics are not implemented. |
...
| `ALTER VIEW` | ❌ | high | View replacement while preserving MySQL metadata and security semantics. | Parser records the view target and validates algorithm, definer, SQL security, optional column-list, required `AS` query body, and check-option clause shapes; view replacement, privilege checks, metadata updates, dependency validation, and diagnostics are not implemented. |
...
| `CREATE EVENT` | ❌ | medium | Scheduled event definition, body, definer, comments, and scheduler metadata. | Parser records the event target and validates optional definer and `IF NOT EXISTS`, required `ON SCHEDULE` and `DO` clauses, completion behavior, status, and comments; scheduler metadata, event execution, privilege checks, warnings, and diagnostics are not implemented. |
| `CREATE FUNCTION` (stored) | ❌ | medium | Stored-function definition, determinism, SQL data access, security, and body semantics. | Parser records the function target and validates optional definer and `IF NOT EXISTS`, parameter-list shape, required `RETURNS` type, documented routine characteristics, and nonempty body, including single compound-statement body spans; stored-function metadata, execution, determinism checks, privileges, warnings, and diagnostics are not implemented. |
| `CREATE FUNCTION` (loadable) | ❌ | low | Loadable-function registration syntax with embedded-compatible diagnostics. | Parser records the function target and validates unambiguous loadable-function forms (`CREATE [AGGREGATE] FUNCTION [IF NOT EXISTS] ... RETURNS ... SONAME ...`); loadable-function registry updates, shared-library loading, namespace checks, privilege checks, and diagnostics are not implemented. |
| `CREATE INDEX` | ❌ | top | Standalone index creation over MySQL index types and attributes. | Parser records the index target and validates standalone index modifiers, documented and corpus-observed index type placement, table targets, nonempty key-part lists, parser/comment/visibility/attribute options, and algorithm/lock clauses; index metadata creation, generated/expression key semantics, option range checks, online DDL behavior, and diagnostics are not implemented. |
...
| `CREATE PROCEDURE` | ❌ | medium | Stored procedure parameters, body, characteristics, and diagnostics. | Parser records the procedure target and validates optional definer and `IF NOT EXISTS`, procedure parameter-list shape, documented routine characteristics, and nonempty body, including single compound-statement body spans; stored-procedure metadata, execution, parameter semantics, privileges, warnings, and diagnostics are not implemented. |
...
| `CREATE TABLE` | ❌ | top | Column definitions, constraints, indexes, table options, generated columns, and partitions; see section 3.1. | Parser records the table target and validates the create-table head plus nonempty top-level definition-list structure, including quoted table identifiers; detailed column, index, constraint, table-option, partition-option, generated-column, metadata, execution, warning, and diagnostic semantics are not implemented. |
| `CREATE TEMPORARY TABLE` | ❌ | high | Session-scoped table lifecycle and name shadowing. | Parser validates temporary-table `LIKE` and query-backed `CREATE TABLE` forms; temporary table metadata, session scope, name shadowing, and execution are not implemented. |
| `CREATE TABLE ... LIKE` | ❌ | high | Exact metadata cloning rules. | Parser records the destination table and validates direct and parenthesized `LIKE` source-table forms; metadata cloning, generated/default/check preservation, privilege checks, warnings, and diagnostics are not implemented. |
| `CREATE TABLE ... SELECT` | ❌ | high | CTAS type inference, default handling, indexes, and atomicity. | Parser records the destination table and validates query-backed CTAS shapes including optional definition groups, table-option prefixes, `IGNORE` / `REPLACE`, optional `AS`, and nested parenthesized query expressions; result-column inference, row insertion, indexes, atomicity, warnings, and diagnostics are not implemented. |
...
| `CREATE TRIGGER` | ❌ | high | Trigger timing, event, ordering, body, definer, and metadata. | Parser records the trigger target and validates optional definer and `IF NOT EXISTS`, trigger timing/event, table target, `FOR EACH ROW`, optional ordering, and nonempty body shapes; trigger metadata, execution, privilege checks, warnings, and diagnostics are not implemented. |
| `CREATE VIEW` | ❌ | high | View column names, algorithms, security, check options, and metadata. | Parser records the view target and validates `OR REPLACE`, algorithm, definer, SQL security, optional column-list, required `AS` query body, and check-option clause shapes; view metadata, query expansion, privilege checks, dependency validation, and diagnostics are not implemented. |
...
| `DROP LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser records and validates the logfile group target with the required `ENGINE [=] name` clause; storage-engine behavior and diagnostics are not implemented. |
...
| `DROP SERVER` | ❌ | low | Foreign server metadata deletion. | Parser records and validates the server target with optional `IF EXISTS`; foreign-server metadata cleanup and diagnostics are not implemented. |
| `DROP SPATIAL REFERENCE SYSTEM` | ❌ | medium | Spatial reference system deletion and dependency checks. | Parser records and validates optional `IF EXISTS` with a numeric SRID target; data-dictionary dependency checks and diagnostics are not implemented. |
...
| `DROP TABLESPACE` | ❌ | low | Tablespace deletion syntax and diagnostics. | Parser records and validates the tablespace target, including corpus-observed optional `ENGINE [=] name` tails; storage-engine behavior and diagnostics are not implemented. |
| `DROP UNDO TABLESPACE` | ❌ | low | Undo tablespace deletion syntax present in the MySQL 8.4 parser source. | Parser records and validates the undo tablespace target, including corpus-observed optional `ENGINE [=] name` tails; storage-engine behavior is not implemented. |
...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| `CALL` | ❌ | medium | Procedure invocation, result sets, OUT/INOUT parameters, and diagnostics. | Parser records and validates the procedure target plus optional argument-list shape; procedure lookup, parameter binding, OUT/INOUT semantics, and result sets are not implemented. |
| `DELETE` (single-table) | ❌ | top | Single-table delete with aliases, partitions, ORDER BY, LIMIT, LOW_PRIORITY, QUICK, and IGNORE. | Parser records the target table and validates documented modifiers, required `FROM` target, optional aliases and partition lists, optional `WHERE`, `ORDER BY`, and `LIMIT`; row deletion, ordering semantics, partition pruning, warnings, and affected rows are not implemented. |
| `DELETE` (multi-table) | ❌ | high | Multi-table delete forms using FROM and USING, join semantics, and affected rows. | Parser records the first delete target and validates both `DELETE targets FROM table_references` and `DELETE FROM targets USING table_references`, target lists including `.*`, nonempty table-reference spans, and optional `WHERE`; join resolution, writable-table checks, foreign-key behavior, and affected rows are not implemented. |
...
| `INSERT ... VALUES` | ❌ | top | Multi-row values, defaults, generated columns, warnings, affected rows, and insert ids. | Parser records the target table and validates optional `INTO`, partition lists, optional column lists including empty `()`, `VALUES` / `VALUE` row groups, `VALUES ROW(...)` row constructors including corpus-observed `ROW()`, row aliases, and duplicate-key tails; row insertion and diagnostics are not implemented. |
| `INSERT ... SET` | ❌ | top | MySQL SET-form insert semantics. | Parser records the target table and validates nonempty top-level assignment lists plus optional row aliases and duplicate-key tails; assignment evaluation and metadata effects are not implemented. |
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser records the target table and validates query-backed sources beginning with `SELECT`, `WITH`, `TABLE`, or parenthesized query expressions, including corpus-observed parenthesized `SELECT ... UNION` forms; source query analysis and insert execution are not implemented. |
| `INSERT ... ON DUPLICATE KEY UPDATE` | ❌ | top | Conflict target resolution, VALUES()/row aliases, affected rows, and warnings. | Parser validates the `ON DUPLICATE KEY UPDATE` marker sequence and nonempty assignment-list shape after `VALUES` and `SET` forms; conflict detection, alias resolution, affected rows, warnings, and diagnostics are not implemented. |
| `INSERT IGNORE` | ❌ | top | Duplicate, conversion, and constraint warning demotion rules. | Parser validates documented `IGNORE` placement after optional priority modifiers; warning-demotion behavior is not implemented. |
| `INSERT DELAYED` | ❌ | low | Deprecated delayed insert syntax and MySQL-compatible diagnostics. | Parser accepts the documented modifier in the insert head; deprecated-syntax warnings and delayed behavior are not implemented. |
| `INSERT LOW_PRIORITY` / `HIGH_PRIORITY` | ❌ | low | Priority modifiers and embedded-compatible treatment. | Parser validates a single documented priority modifier before optional `IGNORE`; priority scheduling behavior is not implemented. |
...
| `REPLACE ... VALUES` | ❌ | top | Delete-then-insert semantics, cascades, triggers, affected rows, and auto-increment behavior. | Parser records the target table and validates optional `INTO`, partition lists, optional column lists, `VALUES` row groups, and `VALUES ROW(...)` row constructors including corpus-observed `ROW()`; delete-then-insert execution is not implemented. |
| `REPLACE ... SET` | ❌ | high | SET-form replace semantics. | Parser records the target table and validates nonempty assignment-list shape; assignment evaluation and replacement semantics are not implemented. |
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser records the target table and validates `SELECT`, `TABLE`, and parenthesized query sources; source query analysis and replacement execution are not implemented. |
| `REPLACE LOW_PRIORITY` / `DELAYED` | ❌ | low | Priority and deprecated delayed modifiers for REPLACE. | Parser validates a single documented `LOW_PRIORITY` or `DELAYED` modifier; priority handling and deprecated-syntax warnings are not implemented. |
...
| `SELECT` | ❌ | top | Full query expression surface; see section 2. | Parser records ordinary `SELECT` and `WITH` query statements as query targets and validates nonempty projection lists, set-operator RHS query-expression heads, and major clause ordering and shells, including `LIMIT` comma and `OFFSET` forms, for direct, parenthesized, nested, and set-operation RHS `SELECT` terms; query expression AST, table-reference detail, expression semantics, result metadata, and execution semantics are not implemented. |
| `SELECT ... INTO var_list` | ❌ | high | User/local variable assignment semantics. | Parser validates a single top-level `INTO` clause, user/local variable-list separators, parenthesized query-expression boundaries, and records the first user/local variable target; assignment semantics, column-count checks, and multi-target metadata expansion are not implemented. |
| `SELECT ... INTO OUTFILE` | ❌ | medium | File export syntax and embedded-compatible diagnostics. | Parser validates the literal export-file target plus optional `CHARACTER SET`, `FIELDS` / `COLUMNS`, and `LINES` option shape across direct and parenthesized query-expression forms, and records the literal export-file target; file I/O behavior is not implemented. |
| `SELECT ... INTO DUMPFILE` | ❌ | medium | Binary file export syntax and embedded-compatible diagnostics. | Parser validates the literal export-file target without OUTFILE formatting options across direct and parenthesized query-expression forms and records the literal export-file target; binary file I/O behavior is not implemented. |
| `TABLE` | ❌ | medium | Table-value statement syntax and ordering/limit behavior. | Parser records direct and parenthesized `TABLE` query-expression table targets, validates documented `ORDER BY`, `LIMIT`, `OFFSET`, final `INTO` variable-list and export-file clause shapes, accepts direct set-operator tails with query-expression RHS validation, and validates outer tails after parenthesized forms; result rows, ordering, limit, field/line option behavior, export behavior, and assignment semantics are not implemented. |
| `UPDATE` (single-table) | ❌ | top | Assignment order, generated columns, ORDER BY, LIMIT, LOW_PRIORITY, and IGNORE. | Parser records the first target table and validates optional `LOW_PRIORITY` / `IGNORE`, a nonempty table-reference span, required `SET` assignment lists, and optional `WHERE`, `ORDER BY`, and `LIMIT` tail shape; assignment evaluation, ordering semantics, row limits, warnings, and affected rows are not implemented. |
| `UPDATE` (multi-table) | ❌ | high | Joined update semantics, assignment evaluation, and affected rows. | Parser accepts comma and join table-reference spans before `SET` and validates the shared assignment-list and optional `WHERE` tail structure; join resolution, writable-table checks, assignment order, and affected-row behavior are not implemented. |
| `VALUES` | ❌ | high | Standalone values statement and row constructor behavior. | Parser records standalone `VALUES` statements as query targets and validates nonempty `ROW(...)` constructor lists, consistent row arity, unsupported standalone `DEFAULT` values, optional `ORDER BY`, `LIMIT` including `OFFSET` and parameter markers, parenthesized outer tails, and set-operation tails with query-expression RHS validation; result metadata and execution semantics are not implemented. |
...
| `BEGIN` / `BEGIN WORK` | ❌ | top | Transaction begin statement distinct from compound BEGIN ... END. | Parser validates bare `BEGIN` and `BEGIN WORK` as transaction forms, rejects extra transaction tails, preserves matched compound `BEGIN ... END` blocks with optional end labels, and rejects unsupported `BEGIN [NOT] ATOMIC`; transaction state and block scope are not implemented. |
...
| `SAVEPOINT` | ❌ | top | Nested savepoint creation and replacement semantics. | Parser records and validates single identifier-like savepoint names; nested savepoint state and replacement semantics are not implemented. |
| `ROLLBACK TO SAVEPOINT` | ❌ | top | Partial rollback semantics and errors. | Parser records and validates single identifier-like savepoint names across `ROLLBACK [WORK] TO [SAVEPOINT]` forms; partial rollback behavior is not implemented. |
| `RELEASE SAVEPOINT` | ❌ | top | Savepoint release semantics and errors. | Parser records and validates single identifier-like savepoint names; release behavior and missing-savepoint errors are not implemented. |
| `SET TRANSACTION` | ❌ | high | Isolation level and access mode at global/local/session/next-transaction scope. | Parser records `SET [GLOBAL | LOCAL | SESSION] TRANSACTION` and `SET CHARACTERISTICS AS TRANSACTION` as transaction metadata and validates comma-separated isolation-level and access-mode characteristic shape; isolation and access-mode semantics are not implemented. |
...
| `SET sql_log_bin` | ❌ | low | Session binary logging toggle and privilege semantics. | Parser records unqualified `SET sql_log_bin = ...` targets through direct unadorned system-variable assignment metadata plus scoped/system-variable spellings; binary logging behavior and privilege checks are not implemented. |
| `CHANGE REPLICATION FILTER` | ❌ | low | Replication filter syntax and diagnostics. | Parser records explicit `FOR CHANNEL` names and default-channel operations, validates documented filter names with nonempty parenthesized value lists, and requires any channel clause to be final; filter semantics are not implemented. |
| `CHANGE REPLICATION SOURCE TO` | ❌ | low | Source connection/channel options and diagnostics. | Parser records explicit `FOR CHANNEL` names and default-channel operations, validates nonempty option assignment lists plus final channel clauses, and includes legacy `CHANGE MASTER TO` routing for compatibility; source option semantics are not implemented. |
| `RESET REPLICA` | ❌ | low | Replica metadata reset syntax. | Parser records and validates explicit string `FOR CHANNEL` names, optional `ALL`, and default-channel operations, including legacy `SLAVE` spelling; replica metadata reset behavior is not implemented. |
| `START REPLICA` | ❌ | low | Replica start syntax, channels, threads, and until conditions. | Parser records explicit `FOR CHANNEL` names and default-channel operations and validates optional thread-type lists, `UNTIL` shapes, connection options, and final channel clauses, including legacy `SLAVE` spelling; replica thread state is not implemented. |
| `STOP REPLICA` | ❌ | low | Replica stop syntax and channel handling. | Parser records explicit `FOR CHANNEL` names and default-channel operations and validates optional thread-type lists plus channel clause shape; replica thread state is not implemented. |
| `START GROUP_REPLICATION` | ❌ | low | Group Replication start syntax and user credentials. | Parser records the group-replication subsystem target and validates optional `USER`, `PASSWORD`, and `DEFAULT_AUTH` credential option shape; group membership, credential storage, recovery, and privilege behavior are not implemented. |
| `STOP GROUP_REPLICATION` | ❌ | low | Group Replication stop syntax. | Parser records and validates the bare group-replication subsystem target; group leave, channel shutdown, timeout, and privilege behavior are not implemented. |
| `PREPARE` | ❌ | high | Prepare from literal or user variable, parameter marker rules, and errors. | Parser records and validates the prepared-statement handle plus literal/user-variable source; SQL text parsing, parameter metadata, and registry behavior are not implemented. |
| `EXECUTE` | ❌ | high | Prepared-statement execution with USING variables and result metadata. | Parser records the prepared-statement handle and validates optional `USING` user-variable bindings; execution and result metadata are not implemented. |
| `DEALLOCATE PREPARE` / `DROP PREPARE` | ❌ | high | Prepared statement cleanup. | Parser records and validates the prepared-statement handle; per-session statement registry cleanup is not implemented. |
| `BEGIN ... END` | ❌ | medium | Compound statement block scope for stored programs and events. | Parser matches compound blocks, keeps semicolon-delimited bodies in one statement span, and preserves optional end labels; block scope and declaration-order semantics are not implemented. |
| Statement labels | ❌ | medium | Label declaration, LEAVE/ITERATE binding, and duplicate-label diagnostics. | Parser records leading labels on compound blocks and loop statements, records LEAVE/ITERATE targets, accepts unrestricted nonreserved label keywords, and avoids treating MySQL's restricted label keywords as declarations or targets; duplicate-label checks, end-label validation, binding, and the 16-character label limit are not implemented. |
| `DECLARE` local variables | ❌ | medium | Stored-program local variable declarations, defaults, and scope. | Parser records the first declared variable name and validates comma-separated variable names, a nonempty type span, and optional nonempty `DEFAULT` expression; type semantics, declaration ordering, and block scope are not implemented. |
| `DECLARE ... CONDITION` | ❌ | medium | Named condition declarations. | Parser records the declared condition-name target and validates `CONDITION FOR` condition values including SQLSTATE strings, numeric MySQL error codes, and accepted condition-class shorthands; stored-program declaration ordering and runtime condition binding are not implemented. |
| `DECLARE ... CURSOR` | ❌ | medium | Cursor declaration over SELECT statements. | Parser records the cursor handle and validates `CURSOR FOR` with a nonempty `SELECT` / `WITH` query body; cursor scope, declaration ordering, SELECT result shape, `INTO` restrictions, and lifecycle semantics are not implemented. |
| `DECLARE ... HANDLER` | ❌ | medium | CONTINUE/EXIT handler declarations for SQLSTATE, errors, warnings, and NOT FOUND. | Parser records the first handled condition value and validates handler actions, required `FOR`, comma-separated condition lists, and a nonempty handler body; declaration ordering, handler binding, and runtime control flow are not implemented. |
| `CASE` statement | ❌ | medium | Stored-program CASE statement semantics. | Parser matches `CASE ... END CASE`, validates top-level `WHEN ... THEN` clauses, optional `ELSE`, nonempty conditions/branch bodies, and keeps semicolon-delimited bodies in one statement span; branch semantics are not implemented. |
| `IF` statement | ❌ | medium | Stored-program IF/ELSEIF/ELSE semantics. | Parser matches `IF ... END IF`, validates top-level `THEN`, `ELSEIF`, and `ELSE` clause ordering plus nonempty conditions/branch bodies, and keeps semicolon-delimited bodies in one statement span; branch semantics are not implemented. |
| `LOOP` | ❌ | medium | Stored-program LOOP semantics. | Parser matches `LOOP ... END LOOP`, validates a nonempty loop body, preserves optional end labels in the span, and keeps semicolon-delimited bodies in one statement span; loop execution is not implemented. |
| `REPEAT` | ❌ | medium | Stored-program REPEAT UNTIL semantics. | Parser matches `REPEAT ... END REPEAT`, validates a nonempty body and top-level nonempty `UNTIL` condition, preserves optional end labels in the span, and keeps semicolon-delimited bodies in one statement span; loop execution and condition evaluation are not implemented. |
| `WHILE` | ❌ | medium | Stored-program WHILE semantics. | Parser matches `WHILE ... END WHILE`, validates a nonempty condition before `DO` and a nonempty loop body, preserves optional end labels in the span, and keeps semicolon-delimited bodies in one statement span; loop execution and condition evaluation are not implemented. |
...
| `SIGNAL` | ❌ | medium | User-raised SQLSTATE and condition item semantics. | Parser records explicit SQLSTATE or condition-name targets and validates required condition values plus optional documented signal-information item assignment lists; runtime diagnostics behavior is not implemented. |
| `RESIGNAL` | ❌ | medium | Handler rethrow and diagnostics mutation. | Parser accepts and validates bare, SET-only, SQLSTATE, and condition-name forms, records explicit condition values, and validates documented signal-information item assignment lists; handler-context and diagnostics semantics are not implemented. |

### 1.4 Account, Resource, Plugin, Maintenance, SHOW, and Utility Statements

| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| `ALTER USER` | ❌ | medium | Authentication plugins, passwords, MFA, TLS, resources, lock/expire, comments, attributes, and default roles. | Parser records the first account target and validates optional `IF EXISTS`, account lists with authentication changes, `USER()` password changes, secondary-password clauses, factor operations, WebAuthn registration clauses, default roles, TLS requirements, resource limits, password-management options, account locks, comments, and attributes; account metadata, privilege checks, password verification, registration state, and diagnostics are not implemented. |
...
| `GRANT` | ❌ | medium | Privilege and role grants, WITH GRANT OPTION, PROXY, dynamic privileges, and partial revoke semantics. | Parser records the first granted-to user or role target and validates required `TO` target lists plus documented `WITH GRANT OPTION`, `WITH ADMIN OPTION`, and `AS ... WITH ROLE` tail shapes; privilege graph updates and diagnostics are not implemented. |
...
| `REVOKE` | ❌ | medium | Privilege and role revocation semantics. | Parser records the first revoked-from user or role target and validates optional `IF EXISTS`, required `FROM` target lists, `CURRENT_USER()` principals, and `IGNORE UNKNOWN USER` tails; privilege graph updates and diagnostics are not implemented. |
...
| `SET PASSWORD` | ❌ | medium | Password assignment semantics. | Parser records explicit `FOR` account targets and bare current-user password assignments as user targets, and validates literal password assignment, `TO RANDOM`, `REPLACE`, and `RETAIN CURRENT PASSWORD` tails; password validation, storage, and random generation are not implemented. |
...
| `CREATE RESOURCE GROUP` | ❌ | low | Thread resource group creation syntax. | Parser records the resource group target and validates required `TYPE`, optional `VCPU`, `THREAD_PRIORITY`, and `ENABLE`/`DISABLE` clauses; resource group metadata, CPU/priority range checks, and diagnostics are not implemented. |
| `ALTER RESOURCE GROUP` | ❌ | low | Resource group modification syntax. | Parser records the resource group target and validates `VCPU`, `THREAD_PRIORITY`, `ENABLE`, and `DISABLE [FORCE]` clauses; resource group metadata, CPU/priority range checks, thread reassignment, and diagnostics are not implemented. |
...
| `SET RESOURCE GROUP` | ❌ | low | Thread assignment to resource groups. | Parser records the resource group target and validates optional `FOR` thread-id lists; thread assignment, privilege checks, warnings, and diagnostics are not implemented. |
...
| `UNINSTALL PLUGIN` | ❌ | low | Plugin uninstallation syntax and diagnostics. | Parser records and validates the plugin name target; plugin unloading and registry behavior are not implemented. |
| `CLONE` | ❌ | low | Local and remote clone syntax and diagnostics. | Parser records local clone directory targets and remote donor server endpoints and validates local directory, remote endpoint, password, optional data-directory, and optional SSL-requirement clause shape; clone plugin behavior, privilege checks, copying, and restart semantics are not implemented. |
| `SET` | ❌ | top | Variable assignment, user variables, system variables, persisted variables, names, charset, and transaction forms. | Parser records explicit user-variable, scoped system-variable, structured system-variable, keyword-shaped system-variable, and direct unadorned assignment targets plus account-management, character-set, and transaction targets, and validates comma-separated assignment shape; multi-assignment expansion and local-variable disambiguation remain semantic-analysis work. |
| `SET CHARACTER SET` | ❌ | top | Connection character-set shorthand semantics. | Parser records the requested character-set target and validates the shorthand statement shape, including following comma-separated SET assignments; charset validation and session variable updates are not implemented. |
| `SET NAMES` | ❌ | top | Connection character set and collation semantics. | Parser records the requested character-set target and validates optional `COLLATE` clause shape plus following comma-separated SET assignments; charset/collation catalog validation and session variable updates are not implemented. |
| `CACHE INDEX` | ❌ | low | MyISAM key cache assignment syntax. | Parser records the first table/index target and validates table/index lists, optional partition lists, and `IN` key-cache targets; key-cache assignment behavior is not implemented. |
| `FLUSH` | ❌ | medium | FLUSH variants for logs, tables, privileges, status, hosts, optimizer costs, and user resources. | Parser records and validates table targets or unnamed table collections, relay-log channels, documented log/cache/privilege/status/user-resource option targets, legacy `FLUSH HOSTS` routing, option lists, and optional `LOCAL` / `NO_WRITE_TO_BINLOG` modifiers; runtime flush behavior is not implemented. |
| `KILL` | ❌ | medium | Connection/query kill syntax and diagnostics. | Parser records single processlist-id expression targets, distinguishes `KILL QUERY` from connection termination, and rejects missing or comma-separated targets; termination behavior and privilege diagnostics are not implemented. |
| `LOAD INDEX INTO CACHE` | ❌ | low | MyISAM index preload syntax. | Parser records the first table target and validates table/index lists, optional partition lists, and `IGNORE LEAVES`; index preload behavior is not implemented. |
| `RESET` | ❌ | medium | RESET variants for source/replica/persist-style operations exposed by MySQL 8.4. | Parser records and validates comma-separated reset targets for binary logs, legacy binary-log reset spellings, replica channels, and persisted variables; runtime reset behavior is not implemented. |
| `RESET PERSIST` | ❌ | low | Persisted system variable reset syntax. | Parser records and validates explicit persisted-variable names and bare full-reset system-variable collection targets; persisted variable storage is not implemented. |
| `RESTART` | ❌ | low | Server restart syntax and embedded-compatible diagnostics. | Parser records the instance target and rejects body tokens; embedded-compatible restart behavior and diagnostics are not implemented. |
...
| `DESCRIBE` / `DESC` | ❌ | top | Table, column, and statement description semantics. | Parser records and validates direct table/column targets and explainable query targets; result-set shape and query-plan semantics are not implemented. |
| `EXPLAIN` | ❌ | high | Explain SELECT/TABLE/INSERT/UPDATE/DELETE, formats, ANALYZE, INTO, and FOR CONNECTION. | Parser records direct table targets, named connection targets, `EXPLAIN ... INTO` user-variable targets, and explainable query targets, and validates documented format/schema/connection/analyze shells plus existing `VALUES` and parenthesized query-expression forms; optimizer plans and result rows are not implemented. |
...
| `SHOW CHARACTER SET` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the character-set object kind, including the `LIKE` pattern when present, and validates optional `LIKE` / `WHERE` tails. |
| `SHOW COLLATION` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the collation object kind, including the `LIKE` pattern when present, and validates optional `LIKE` / `WHERE` tails. |
| `SHOW COLUMNS` / `SHOW FIELDS` | ❌ | top | Column metadata rows, table/view targets, optional schema qualification, `EXTENDED`/`FULL` modifiers, LIKE/WHERE filtering, generated/invisible primary key behavior, and privileges. | Parser records the inspected table target across `FROM`/`IN`, schema-qualified, `EXTENDED`, `FULL`, and synonym forms, and rejects missing targets or dangling filters. |
| `SHOW COUNT(*) ERRORS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the diagnostics-area object kind and validates the `COUNT(*)` shape; row-count semantics are not implemented. |
| `SHOW COUNT(*) WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the diagnostics-area object kind and validates the `COUNT(*)` shape; row-count semantics are not implemented. |
...
| `SHOW CREATE USER` | ❌ | medium | Recreate SQL text, account authentication/resource/password options, privileges, and result metadata. | Parser records explicit account targets and preserves documented current-user function-call spans. |
...
| `SHOW ENGINE` | ❌ | low | Generic named-engine diagnostics dispatch, privilege checks, and storage-engine-specific result rows. | Parser records named engines for `STATUS`, `MUTEX`, and runtime-accepted `LOGS` forms and rejects missing diagnostic-type tails. |
| `SHOW ENGINE LOGS` | ❌ | low | Historical storage-engine log diagnostics where applicable, or MySQL 8.4-compatible unsupported-form diagnostics. | Parser records the named engine target; result rows and unsupported-engine diagnostics are not implemented. |
| `SHOW ENGINE MUTEX` | ❌ | low | InnoDB mutex/rw-lock statistics rows, Performance Schema alternatives, privilege checks, and runtime collection controls. | Parser records the named engine target. |
| `SHOW ENGINE STATUS` | ❌ | high | Named storage-engine operational status rows, especially InnoDB monitor output, privilege checks, and engine-specific diagnostics. | Parser records the named engine target. |
...
| `SHOW ERRORS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the diagnostics-area object kind and validates optional `LIMIT`; result rows and error-count semantics are not implemented. |
| `SHOW EVENTS` | ❌ | medium | Event metadata rows, current-schema and explicit-schema behavior, LIKE/WHERE filtering, privileges, and event scheduler details. | Parser records explicit schema names, event collection targets, or `LIKE` event-name patterns. |
...
| `SHOW FUNCTION STATUS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the function object kind, including the `LIKE` pattern when present. |
| `SHOW GRANTS` | ❌ | high | Current-user and named-account grant rows, role `USING` behavior, privilege checks, and MySQL 8.4 global/dynamic privilege formatting. | Parser records the user object kind for current-user grants, explicit `FOR` account names, documented `CURRENT_USER()` targets, `USING` role tails, and corpus-observed trailing-`@` account syntax. |
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | ❌ | top | Index metadata rows, table targets, optional schema qualification, `EXTENDED` hidden index behavior, WHERE filtering, generated invisible key behavior, and privileges. | Parser records the inspected table target across `FROM`/`IN`, schema-qualified, `EXTENDED`, and synonym forms, and rejects missing targets or dangling `WHERE` filters. |
| `SHOW MASTER STATUS` | ❌ | low | No longer supported in MySQL 8.4; match parser/diagnostic behavior rather than returning binary-log status rows. | Parser records the binary-log object kind so later runtime diagnostics can route this legacy form. Replacement statement is `SHOW BINARY LOG STATUS`. |
| `SHOW OPEN TABLES` | ❌ | high | Open table-cache rows, current-schema and explicit-schema behavior, LIKE/WHERE filtering, table privileges, and lock counters. | Parser records explicit schema names, table collection targets, or `LIKE` table-name patterns. |
| `SHOW PARSE_TREE` | ❌ | low | JSON parse-tree result for the MySQL 8.4.9 grammar when enabled; syntax error when built without `WITH_SHOW_PARSE_TREE`. | Parser records documented `SELECT` payloads as query targets and rejects non-`SELECT` payloads; runtime JSON output and build-flag diagnostics are not implemented. |
| `SHOW PLUGINS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the plugin object kind; result rows are not implemented. |
| `SHOW PRIVILEGES` | ❌ | high | Result-set shape, supported static and dynamic privilege rows, and privileges. | Parser records the privilege object kind; result rows are not implemented. |
...
| `SHOW PROCEDURE STATUS` | ❌ | medium | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the procedure object kind, including the `LIKE` pattern when present. |
| `SHOW PROCESSLIST` | ❌ | high | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the connection object kind; processlist rows and privilege visibility are not implemented. |
| `SHOW PROFILE` | ❌ | low | Deprecated profiling rows for the most recent statement or explicit `FOR QUERY` id, session profiling controls, and MySQL-compatible diagnostics. | Parser records the query object kind, including numeric `FOR QUERY` ids when present, validates nonempty profile tails, and accepts `LIMIT` / `OFFSET` order variants observed in the MySQL corpus. |
| `SHOW PROFILES` | ❌ | low | Deprecated profiling history result set and session profiling controls. | Parser records the query object kind for the profile history collection. |
| `SHOW RELAYLOG EVENTS` | ❌ | low | Relay-log event rows, default first-relay-log behavior, channel selection, limits, privileges, and replication diagnostics. | Parser records explicit relay-log names, bare relay-log targets, or channel targets when only `FOR CHANNEL` is present, and validates `IN`, `FROM`, `LIMIT`, and final channel-clause shape. |
| `SHOW REPLICA STATUS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records explicit `FOR CHANNEL` names or the replication-channel object kind for the channel collection, including deprecated `SHOW SLAVE STATUS` aliases, and rejects incomplete channel clauses. |
| `SHOW REPLICAS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the replication-channel object kind, including the deprecated `SHOW SLAVE HOSTS` alias; result rows are not implemented. |
| `SHOW STATUS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the status-variable object kind, including the `LIKE` pattern when present. |
| `SHOW TABLE STATUS` | ❌ | top | Table and view status rows, current-schema and explicit-schema behavior, LIKE/WHERE filtering, storage-engine statistics, and privileges. | Parser records explicit schema names, table collection targets, or `LIKE` table-name patterns. |
| `SHOW TABLES` | ❌ | top | Table and view listing rows, current-schema and explicit-schema behavior, `FULL`/`EXTENDED` modifiers, LIKE/WHERE filtering, and privileges. | Parser records explicit schema names, table collection targets, or `LIKE` table-name patterns. |
| `SHOW TRIGGERS` | ❌ | high | Trigger metadata rows, current-schema and explicit-schema behavior, LIKE/WHERE filtering by table name, trigger privileges, and definer metadata. | Parser records explicit schema names, trigger collection targets, or `LIKE` table-name patterns. |
| `SHOW VARIABLES` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the system-variable object kind, including the `LIKE` pattern when present, and validates optional scope plus `LIKE` / `WHERE` tails. |
| `SHOW WARNINGS` | ❌ | top | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser records the diagnostics-area object kind and validates optional `LIMIT`; result rows and warning-count semantics are not implemented. |
...
| `autocommit` | ❌ | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Parser records direct `SET autocommit = ...` assignments as system-variable targets; runtime variable metadata and behavior are not implemented. |
...
| `sql_mode` | ❌ | top | Expose MySQL-compatible value, scope, mutability metadata, SET behavior, and diagnostics. | Parser records direct, scoped, and `@@` `SET sql_mode = ...` assignments as system-variable targets; runtime variable metadata and behavior are not implemented. |
