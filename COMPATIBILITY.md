...
| `IN` subqueries | ❌ | top | NULL-aware membership semantics and type conversion. | Started; broad spec: [subqueries](docs/specs/subqueries/specs.md). Focused scalar `IN` / `NOT IN` first-slice spec: [subquery `IN` predicates](docs/specs/subquery-in-predicates/specs.md). |
