...
| `autocommit` | 🟡 | Limited scalar `SELECT @@autocommit` with no scope, `session`, `local`, or `global`; returns MyLite's fixed enabled autocommit baseline `1`; limited fixed no-op `SET` forms may preserve `1`; no mutable global/session state, explicit transaction boundaries, session-state tracking, protocol status flags, or `SHOW VARIABLES` |
...
| `character_set_client` | 🟡 | Limited scalar `SELECT @@character_set_client` with no scope, `session`, `local`, or `global`; returns MyLite's fixed `utf8mb4` connection baseline; fixed no-op `SET NAMES` / `SET CHARACTER SET` forms may preserve the value; no mutable conversion, client negotiation, `SHOW VARIABLES`, or protocol charset metadata |
| `character_set_connection` | 🟡 | Limited scalar `SELECT @@character_set_connection` with no scope, `session`, `local`, or `global`; returns MyLite's fixed `utf8mb4` connection baseline; fixed no-op `SET NAMES` / `SET CHARACTER SET` forms may preserve the value; no mutable conversion, string literal semantics, `SHOW VARIABLES`, or protocol charset metadata |
...
| `character_set_results` | 🟡 | Limited scalar `SELECT @@character_set_results` with no scope, `session`, `local`, or `global`; returns MyLite's fixed `utf8mb4` connection baseline; fixed no-op `SET NAMES` / `SET CHARACTER SET` forms may preserve the value; no mutable result conversion, `NULL` results mode, `SHOW VARIABLES`, or protocol charset metadata |
...
| `collation_connection` | 🟡 | Limited scalar `SELECT @@collation_connection` with no scope, `session`, `local`, or `global`; returns MyLite's fixed `utf8mb4_0900_ai_ci` connection baseline; fixed no-op `SET NAMES ... COLLATE utf8mb4_0900_ai_ci` and `SET CHARACTER SET` forms may preserve the value; no mutable collation coercibility, string comparison semantics, `SHOW VARIABLES`, or protocol collation metadata |
...
| `sql_generate_invisible_primary_key` | 🟡 | Limited scalar `SELECT @@sql_generate_invisible_primary_key` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; limited fixed no-op `SET` forms may preserve `0`; no mutable global/session state, hidden `my_row_id` columns, generated primary keys, invisible-column metadata, auto-increment, changed `CREATE TABLE` behavior, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_log_bin` | 🟡 | Limited scalar `SELECT @@sql_log_bin` with no scope, `session`, or `local`; returns MyLite's fixed enabled value `1`; limited fixed no-op session/local `SET` forms may preserve `1`; `global` scope is rejected as session-only; no mutable session state, binary log files, GTID behavior, replication side effects, privilege checks, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_log_off` | 🟡 | Limited scalar `SELECT @@sql_log_off` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; limited fixed no-op `SET` forms may preserve `0`; no mutable global/session state, general query log files, `mysql.general_log`, slow query logging, privilege checks, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_mode` | 🟡 | Limited scalar `SELECT @@sql_mode` with no scope, `session`, `local`, or `global`; returns MySQL 8.4.9's default mode string `ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION`; limited fixed no-op `SET sql_mode = DEFAULT` and exact-default string forms preserve that value; no mutable global/session state, mode-dependent parsing, mode-dependent coercion, changed statement behavior, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_notes` | 🟡 | Limited scalar `SELECT @@sql_notes` with no scope, `session`, `local`, or `global`; returns MyLite's fixed enabled value `1`; limited fixed no-op `SET` forms may preserve `1`; note-level diagnostics are currently produced only by limited table/schema-existence DDL no-ops; no mutable global/session state, note suppression, `max_error_count`, changed `SHOW WARNINGS` / `@@warning_count` behavior beyond the documented missing-schema drop exception, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_quote_show_create` | 🟡 | Limited scalar `SELECT @@sql_quote_show_create` with no scope, `session`, `local`, or `global`; returns MyLite's fixed enabled SHOW CREATE quote-control value `1`; limited fixed no-op `SET` forms may preserve `1`; no mutable global/session state, disabled SHOW CREATE rendering, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_replica_skip_counter` | 🟡 | Limited read-only scalar `SELECT @@sql_replica_skip_counter` with no scope or `global`; returns MyLite's fixed replica-event-skip counter value `0`; `session` and `local` scopes are rejected as global-only; no `SET`, mutable global state, replica SQL thread state, relay log/event skipping, channels, GTID restrictions, `SHOW VARIABLES`, or Performance Schema variable tables; deprecated `sql_slave_skip_counter` alias support is tracked separately |
| `sql_require_primary_key` | 🟡 | Limited scalar `SELECT @@sql_require_primary_key` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled primary-key-requirement value `0`; limited fixed no-op `SET` forms may preserve `0`; no mutable global/session state, primary-key constraints, DDL enforcement, table import behavior, replication policy, privilege semantics, `SHOW VARIABLES`, or Performance Schema variable tables |
...
| `sql_slave_skip_counter` | 🟡 | Limited read-only scalar `SELECT @@sql_slave_skip_counter` with no scope or `global`; deprecated alias for MyLite's fixed `sql_replica_skip_counter` value `0`; emits MySQL-compatible deprecation warning `1287` once per successful alias reference; `session` and `local` scopes are rejected as global-only; no `SET`, mutable global state, replica SQL thread state, relay log/event skipping, channels, GTID restrictions, `SHOW VARIABLES`, or Performance Schema variable tables |
| `sql_warnings` | 🟡 | Limited scalar `SELECT @@sql_warnings` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; limited fixed no-op `SET` forms may preserve `0`; limited `INSERT IGNORE ... VALUES` / `SET` adjustment warnings are still recorded in diagnostics like MySQL; no mutable global/session state, broader warning-producing DML conversions, protocol information strings, changed `INSERT` behavior beyond the limited `IGNORE` subset, `SHOW VARIABLES`, or Performance Schema variable tables |
...
| `warning_count` | 🟡 | Limited read-only scalar `SELECT @@warning_count`, `@@session.warning_count`, and `@@local.warning_count` over the previous diagnostics snapshot; counts warning and note records currently stored by MyLite, while missing-schema `DROP DATABASE IF EXISTS` intentionally exposes only a statement result warning count; no global scope, `SET`, `SHOW VARIABLES`, `max_error_count`, mutable `sql_notes`, or broader counted-but-not-stored conditions |
