...
- [x] Extend duplicate-update planning to allow exactly one enforced composite
      primary or unique key descriptor.
- [x] Generate duplicate-row `UPDATE` predicates over every key part, reusing
      descriptor-built key expressions and bound tuple parameters.
- [x] Preserve existing duplicate assignment conversion, affected-row,
      warning, auto-increment, and transaction behavior.
- [x] Add runtime C coverage for composite primary keys, composite unique keys,
