...
static int test_no_source_dual_and_do_insert_function(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "INSERT() DO columns");
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "INSERT() DO rows");
...
static int test_table_backed_insert_function_and_reopen(void) {
    ...
    mylite_close(database);
    database = NULL;
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen INSERT()");
