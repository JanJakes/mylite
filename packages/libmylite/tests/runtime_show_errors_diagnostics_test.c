...
enum {
    ...
    show_count_errors_column_count = 1,
...
struct expected_show_diagnostics {
    const char *sql;
    size_t row_count;
    const char *level;
    const char *code;
...
static const char *const diagnostics_column_names[diagnostics_column_count] = {
    "Level",
    "Code",
    "Message",
};
...
static int test_show_errors_snapshot_lifecycle_and_preamble(void);
static int test_show_errors_limits_errors_and_unsupported_forms(void);
static int test_independent_show_errors_handles(void);
static int expect_show_errors_result(
    mylite_db *database,
    struct expected_show_diagnostics expectation
);
static int expect_show_warnings_result(
    mylite_db *database,
    struct expected_show_diagnostics expectation
);
static int expect_diagnostics_result(
    mylite_db *database,
    struct expected_show_diagnostics expectation
);
...
int main(void) {
    ...
    failures += test_show_errors_snapshot_lifecycle_and_preamble();
    failures += test_show_errors_limits_errors_and_unsupported_forms();
    failures += test_independent_show_errors_handles();
...
static int test_show_errors_snapshot_lifecycle_and_preamble(void) {
    ...
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS",
            .row_count = 0U,
            .context = "empty errors",
        }
    );
    failures += expect_show_count_errors(database, "0", "empty error count");
    ...
    failures += expect_show_errors_result(
        ...
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS",
            .row_count = 0U,
            .context = "warning-only errors",
        }
    );
    failures += expect_public_ok_diagnostics(database, "show errors public diagnostics");
    failures += expect_show_count_errors(database, "0", "warning-only error count");
    failures += expect_show_count_warnings(database, "1", "warning-only warning count");
    failures += expect_show_warnings_result(
        ...
        (struct expected_show_diagnostics){
            ...
            .context = "show errors preserves warning row",
        }
    );
    ...
    failures += expect_show_errors_result(
        ...
        (struct expected_show_diagnostics){
            ...
            .code = "1064",
            .message_part = "SQL syntax",
            .context = "parse error row",
        }
    );
    failures += expect_show_count_errors(database, "1", "parse error count");
    failures += expect_show_warnings_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW WARNINGS",
            ...
            .code = "1064",
            .message_part = "SQL syntax",
            .context = "show errors preserves error diagnostics",
        }
    );
    failures += expect_public_ok_diagnostics(database, "parse error show errors clears live error");

    failures += execute_statement_ok(database, "SELECT ROW_COUNT()");
    failures += expect_show_count_errors(database, "0", "ordinary statement clears errors");
...
static int test_show_errors_limits_errors_and_unsupported_forms(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open errors memory");
    failures += execute_error(
        database,
        "BAD SQL",
        (struct expected_sql_error){
    ...
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 0",
            .row_count = 0U,
            .context = "limit zero",
        }
    );
    failures += expect_show_count_errors(database, "1", "limit zero preserves count");
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
    ...
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 0, 1",
            .row_count = 1U,
            .level = "Error",
            .code = "1064",
            .message_part = "SQL syntax",
            .context = "comma offset includes error",
        }
    );
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 1, 1",
            .row_count = 0U,
            .context = "comma offset skips error",
        }
    );
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 1 OFFSET 0",
            .row_count = 1U,
            .level = "Error",
            .code = "1064",
            .message_part = "SQL syntax",
            .context = "offset includes error",
        }
    );
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 1 OFFSET 1",
            .row_count = 0U,
            .context = "offset skips error",
        }
    );
    failures += expect_show_errors_result(
        database,
        (struct expected_show_diagnostics){
            .sql = "SHOW ERRORS LIMIT 18446744073709551615",
            .row_count = 1U,
            .level = "Error",
            .code = "1064",
            .message_part = "SQL syntax",
    ...
    failures += execute_error(
        database,
        "SHOW ERRORS LIMIT 18446744073709551616",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
    ...
    failures += expect_show_errors_result(
        ...
        (struct expected_show_diagnostics){
            ...
            .code = "1064",
            .message_part = "SQL syntax",
            .context = "unsupported syntax diagnostic row",
        }
    );
...
static int test_independent_show_errors_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&first), MYLITE_OK, "open first errors handle");
    failures +=
        mylite_test_expect_int(mylite_open_memory(&second), MYLITE_OK, "open second errors handle");
    ...
    failures += expect_show_count_errors(first, "1", "first handle error count");
    failures += expect_show_count_errors(second, "0", "second handle error count");

    failures += execute_statement_ok(first, "SELECT ROW_COUNT()");
    failures += expect_show_count_errors(first, "0", "first handle cleared error count");
    failures += expect_show_count_errors(second, "0", "second handle still empty");
...
static int expect_show_errors_result(
    ...
    return expect_diagnostics_result(database, expectation);
}
...
static int expect_diagnostics_result(
    ...
    for (size_t column_index = 0U; column_index < diagnostics_column_count; ++column_index) {
        failures += mylite_test_expect_text_or_null(
            mylite_result_column_name(result, column_index),
            diagnostics_column_names[column_index],
            expectation.context
        );
    }
    failures += mylite_test_expect_size(
...
static int expect_show_count_errors(
    ...
    failures += mylite_test_expect_size(
        mylite_result_column_count(result),
        show_count_errors_column_count,
        context
    );
    failures += mylite_test_expect_text_or_null(
...
static int expect_public_ok_diagnostics(mylite_db *database, const char *context) {
    ...
    failures += mylite_test_expect_int(mylite_errcode(database), MYLITE_OK, context);
    failures += mylite_test_expect_text_or_null(mylite_sqlstate(database), "00000", context);
    failures += mylite_test_expect_text_or_null(mylite_errmsg(database), "not an error", context);
