...
enum {
    ...
    autocommit_value_column_count = 6,
    autocommit_label_column_count = 5,
...
static int test_autocommit_system_variable_values_and_persistence(void) {
    ...
    static const char *const mixed_values[] = {"1", "InnoDB", "utf8mb4", "MyLite"};
    ...
    static const char *const selected_columns[] = {"@@autocommit", "DATABASE()"};
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open autocommit file");
    ...
    failures += execute_ok(
        ...
        "@@local.autocommit, @@warning_count, ROW_COUNT() FROM DUAL",
        &result
    );
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .values = value_values,
            .count = autocommit_value_column_count,
            .context = "autocommit values",
        }
    );
    ...
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .values = label_values,
            .count = autocommit_label_column_count,
            .context = "autocommit labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@autocommit, @@default_storage_engine, @@character_set_server, "
        "@@version_comment",
        &result
    );
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .context = "mixed autocommit scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .count = autocommit_diagnostics_column_count,
            .context = "autocommit warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "autocommit clears warnings");
    ...
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .values = error_values,
            .count = autocommit_diagnostics_column_count,
            .context = "autocommit error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "autocommit clears errors");
    failures += expect_show_count_warnings(database, "0", "autocommit clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by autocommit reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by autocommit reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read autocommit preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after autocommit reads"
    );

    failures += execute_statement_ok(database, "CREATE DATABASE app");
    failures += execute_statement_ok(database, "USE app");
    failures += execute_ok(database, "SELECT @@autocommit, DATABASE()", &result);
    failures += expect_scalar_result(
        result,
        (struct expected_scalar_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = autocommit_selected_column_count,
            .context = "autocommit with selected database",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen autocommit file");
    ...
    failures += expect_scalar_result(
        ...
        (struct expected_scalar_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened autocommit values",
        }
    );
    mylite_result_free(result);
...
static int test_autocommit_system_variable_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open autocommit memory");
    failures += execute_ok(
        database,
        "SELECT @@AUTOCOMMIT, @@SESSION.AUTOCOMMIT, @@Local.AutoCommit, "
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_autocommit_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_autocommit_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_autocommit_variable'",
...
static int test_independent_autocommit_system_variable_handles(void) {
    ...
    failures += expect_int(mylite_open_memory(&first), MYLITE_OK, "open first autocommit handle");
    failures += expect_int(mylite_open_memory(&second), MYLITE_OK, "open second autocommit handle");
    failures += execute_statement_ok(first, "SHOW PROCESSLIST");

    failures += execute_ok(first, "SELECT @@autocommit, @@warning_count, @@error_count", &result);
    failures += expect_scalar_result(
        result,
        (struct expected_scalar_result){
            .columns = columns,
            .values = first_values,
            .count = autocommit_independent_column_count,
            .context = "first handle autocommit variables",
        }
    );
    mylite_result_free(result);
    result = NULL;

    failures += execute_ok(second, "SELECT @@autocommit, @@warning_count, @@error_count", &result);
    failures += expect_scalar_result(
        result,
        (struct expected_scalar_result){
            .columns = columns,
            ...
            .count = autocommit_independent_column_count,
            .context = "second handle autocommit variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_autocommit_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
