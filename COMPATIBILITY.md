...
| `AND`, `&&` | 🟡 | Limited descriptor-backed `WHERE` boolean expressions over existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; `AND` binds tighter than `XOR` and `OR`, and `&&` is accepted with MySQL-compatible deprecation warning 1287. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `NOT`, `!` | 🟡 | Limited keyword `NOT` over descriptor-backed `WHERE` boolean expressions for existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; symbolic `!` remains unsupported until MyLite owns its higher-precedence expression semantics and deprecation warning behavior. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `XOR` | 🟡 | Limited descriptor-backed `WHERE` boolean expressions over existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; `XOR` binds looser than `AND` and tighter than `OR`, with MySQL three-valued `NULL` propagation for the admitted predicate subset; no scalar expression-level XOR. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
