...
static void dump_semantic_node(const MyliteSemanticAstNode *node,
  ...
  if (kind == MYLITE_SEMANTIC_NODE_STATEMENT) {
  ...
  } else if (kind == MYLITE_SEMANTIC_NODE_QUERY) {
    printf(" query_blocks=%zu has_with=%d has_set_operation=%d",
    ...
    printf(" source_kind=%s",
           mylite_semantic_source_kind_name(
               mylite_semantic_ast_node_source_kind(node)));
  } else if (kind == MYLITE_SEMANTIC_NODE_ROW) {
    printf(" row_index=%zu", mylite_semantic_ast_node_row_index(node));
  ...
  } else if (kind == MYLITE_SEMANTIC_NODE_CLAUSE) {
    ...
    printf(" data_type_family=%s data_type_kind=%s storage_class=%s "
           "flags=0x%x numeric_params=%zu:%llu,%llu",
           mylite_create_table_column_type_family_name(
