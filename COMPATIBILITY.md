...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `CREATE TABLE` | 🟡 | Limited persistent base-table creation with optional `IF NOT EXISTS`, explicit integer-family columns (`TINYINT`, `SMALLINT`, `MEDIUMINT`, `INT`/`INTEGER`, `BIGINT`) and the `INT1`/`INT2`/`INT3`/`INT4`/`INT8` aliases, optional deprecated display width `0..255` before optional single `SIGNED` or `UNSIGNED`, `NULL`/`NOT NULL`, optional explicit `ENGINE [=] InnoDB`, and optional fixed default `utf8mb4` / `utf8mb4_0900_ai_ci` table charset/collation options; display width emits warning 1681 and only signed `TINYINT(1)`/`INT1(1)` persists as width-bearing metadata; existing-table `IF NOT EXISTS` is a no-op with a MySQL-compatible note; no other options, keys, defaults, or temporary tables. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
...
| Column definition grammar | 🟡 | Limited integer column descriptors, `INT1`/`INT2`/`INT3`/`INT4`/`INT8` aliases, optional unsigned decimal display width before optional single `SIGNED` or `UNSIGNED`, and nullability only. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
