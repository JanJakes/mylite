...
| `TABLE` | ❌ | medium | Table-value statement syntax and ordering/limit behavior. | Parser recognizes table references, set operators, `ORDER BY`, numeric or identifier `LIMIT` forms, `INTO` variable lists, `INTO OUTFILE`/`INTO DUMPFILE` targets, and malformed set-operation `SELECT` operands. |
...
| `CHANGE REPLICATION SOURCE TO` | ❌ | low | Source connection/channel options and diagnostics. | Parser recognizes documented MySQL 8.4 source option names, numeric values, boolean values, string-or-`NULL` values, `IGNORE_SERVER_IDS` lists, privilege-check users, fixed primary-key-check enums, GTID assignment values, and string-literal channel names. |
| `RESET REPLICA` | ❌ | low | Replica metadata reset syntax. | Parser recognizes `ALL`, string-literal channel names, and comma-separated combinations with `RESET BINARY LOGS AND GTIDS` options. |
...
| `START REPLICA` | ❌ | low | Replica start syntax, channels, threads, and until conditions. | Parser recognizes `IO_THREAD`/`RELAY_THREAD` and `SQL_THREAD`, until, connection, and channel clauses with string-literal log/GTID/user option values, source/relay log coordinate option lists in MySQL parser order, numeric log positions, and string-literal channel names. |
...
| `STOP REPLICA` | ❌ | low | Replica stop syntax and channel handling. | Parser recognizes `IO_THREAD`/`RELAY_THREAD` and `SQL_THREAD` plus string-literal channel clauses. |
...
| `CLONE` | ❌ | low | Local and remote clone syntax and diagnostics. | Parser recognizes local clone and remote `CLONE INSTANCE` syntax, including numeric remote ports, string-literal passwords, optional string-literal data directories, and SSL clauses. |
...
| `HELP` | ❌ | low | Server help lookup result-set semantics. | Parser recognizes required string, identifier, and unreserved keyword help topics while rejecting bare help and reserved-keyword topics. |
...
| `SHOW BINLOG EVENTS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional string-literal log-file names, numeric `FROM` positions, and numeric or identifier `LIMIT` tails. |
...
| `SHOW RELAYLOG EVENTS` | ❌ | low | Result-set shape, filtering, LIKE/WHERE clauses where supported, privileges, and MySQL 8.4 deprecation/removal behavior. | Parser recognizes optional string-literal log-file names, numeric `FROM` positions, numeric or identifier `LIMIT` tails, and channel clauses. |
