...
static int test_information_schema_innodb_ft_default_stopword_queries(void) {
    ...
    failures +=
        expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb ft default stopword db");
...
static int test_information_schema_innodb_ft_default_stopword_reopen_preamble_and_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first stopword db");
    failures += expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second stopword db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.INNODB_FT_DEFAULT_STOPWORD",
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first stopword db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.INNODB_FT_DEFAULT_STOPWORD",
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_innodb_ft_default_stopword_%s_%d.mylite",
        name,
        current_process_id()
    );
