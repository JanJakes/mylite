...
static int test_strict_quoted_numeric_dml_and_persistence(void) {
    ...
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open strict file");
    ...
    database = NULL;
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen strict file");
...
static int test_integer_string_prefix_scanning_and_adjustment(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open prefix db");
    failures += seed_schema(database);
    failures += create_numeric_table(database);
...
static int expect_statement_result(
    ...
    failures += mylite_test_expect_size(warning_count, expected.warning_count, "DML warnings");
...
static int expect_dml_result(
    ...
    failures += mylite_test_expect_int64(affected_rows, expected.affected_rows, "DML affected");
    failures += mylite_test_expect_size(warning_count, expected.warning_count, "DML warnings");
