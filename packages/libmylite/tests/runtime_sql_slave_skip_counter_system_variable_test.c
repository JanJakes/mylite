...
static int test_sql_slave_skip_counter_values_and_persistence(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open sql slave skip counter file"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT id, score FROM renamed_child WHERE id = 2 ORDER BY id",
        (struct expected_result){
            .columns = table_columns,
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
    ...
    failures += mylite_test_expect_int(
        mylite_open(path, &database),
        MYLITE_OK,
        "reopen sql slave skip counter file"
    );
    failures += expect_query_result(
        database,
        "SELECT @@sql_slave_skip_counter, @@global.sql_slave_skip_counter",
    ...
    failures += expect_query_result(
        database,
        "SELECT id, score FROM app.renamed_child WHERE id = 2 ORDER BY id",
        (struct expected_result){
            .columns = table_columns,
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
...
static int test_sql_slave_skip_counter_qualifiers_and_errors(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open sql slave skip counter memory"
    );
    failures += execute_ok(
        database,
        "SELECT @@SQL_SLAVE_SKIP_COUNTER, @@GLOBAL.SQL_SLAVE_SKIP_COUNTER, "
...
static int expect_show_single_deprecation_warning(mylite_db *database, const char *context) {
    ...
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 0U, 1U), "1287", context);
...
static int expect_show_deprecation_warning_and_scope_error(
    ...
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 1U, 0U), "Error", context);
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 1U, 1U), "1238", context);
    failures += mylite_test_expect_contains(
...
static int expect_show_deprecation_warning_and_set_global_error(
    ...
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 1U, 0U), "Error", context);
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 1U, 1U), "1229", context);
    failures += mylite_test_expect_contains(
        mylite_result_value_text(result, 1U, 2U),
...
static int expect_show_scope_error(mylite_db *database, const char *context) {
    ...
    failures += mylite_test_expect_size(mylite_result_row_count(result), 1U, context);
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 0U, 0U), "Error", context);
    failures +=
        mylite_test_expect_text_or_null(mylite_result_value_text(result, 0U, 1U), "1238", context);
    failures += mylite_test_expect_contains(
        mylite_result_value_text(result, 0U, 2U),
        "Variable 'sql_slave_skip_counter' is a GLOBAL variable",
        context
