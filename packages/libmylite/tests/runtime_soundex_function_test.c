...
static int test_no_source_dual_and_do_soundex(void) {
    ...
    if (failures == 0) {
        ...
        failures += expect_int64(mylite_result_affected_rows(result), 0, "soundex do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "soundex do warnings");
    }
...
static int test_table_backed_soundex_and_reopen(void) {
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen soundex");
...
static int test_soundex_diagnostics(void) {
    ...
    failures += execute_error(
        database,
        "SELECT SOUNDEX((SELECT 'abc')) FROM t",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "SOUNDEX() supports only string, integer, boolean, NULL",
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written =
        snprintf(path, path_size, "/tmp/mylite-soundex-%s-%d.mylite", name, current_process_id());
