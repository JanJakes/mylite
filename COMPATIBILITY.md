...
| Numeric literals | 🟡 | Decimal integer literals with optional unary sign only as supported no-source/`DUAL` literal projection values, column default values, `INSERT ... VALUES`, `INSERT ... SET`, `REPLACE ... VALUES`, `REPLACE ... SET`, and single-table `UPDATE` assignment inputs plus supported filtered `SELECT`/`DELETE`/`UPDATE` predicate right operands; unsigned decimal integer literals for supported `SELECT` `LIMIT`/`OFFSET` and `DELETE`/`UPDATE LIMIT`; no table-backed expression-level numeric semantics. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
| Boolean literals | 🟡 | `TRUE` and `FALSE` are accepted as `1` and `0` only in supported no-source/`DUAL` literal projection values, column default values, integer row-value, `INSERT ... SET`, `REPLACE ... VALUES`, `REPLACE ... SET`, single-table `UPDATE` assignment, and filtered `SELECT`/`DELETE`/`UPDATE` predicate right-operand positions; no `WHERE TRUE`, `IS TRUE`, bare boolean-literal operands in boolean expressions, unary boolean expressions, scalar truth metadata, or `LIMIT TRUE` / `LIMIT FALSE`. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md), [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| Type conversion | 🟡 | Limited strict conversion for supported integer/`NULL`/`TRUE`/`FALSE` defaults plus inserted, replaced, and updated values, descriptor-driven integer and boolean-literal predicate conversion for `SELECT`/`DELETE`/`UPDATE`, and unsigned signed-64 range conversion for supported `SELECT` `LIMIT`/`OFFSET` plus `DELETE`/`UPDATE LIMIT` literals only. | [type system, literals, and conversion](docs/compatibility/type-system-literals-conversion.md) |
...
| `WHERE` | 🟡 | Descriptor-backed boolean expressions over supported integer/`NULL` predicate atoms for filtered table `SELECT`, aggregate source filters, `DELETE`, and `UPDATE`, with keyword `NOT`, `AND`/`&&`, `OR`/`\|\|`, and parentheses following MySQL precedence; `SELECT` supports source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands; no `WHERE TRUE`, symbolic `!`, `XOR`, literal-left comparisons, `IS TRUE`, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
