...
`DESCRIBE`, `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS`, and
`INFORMATION_SCHEMA.STATISTICS` primary-key shape metadata for those two
tables. MyLite rejects schema, table, index, rename,
