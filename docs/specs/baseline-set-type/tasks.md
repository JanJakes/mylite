...
- [x] Add parser/AST support for `SET('member'[, ...])` column types while
  keeping `SET` usable as an identifier outside type contexts.
- [x] Add descriptor serialization/parsing helpers for set member lists,
...
- [x] Add strict set conversion for `INSERT`, `REPLACE`, and one-assignment
  `UPDATE`, including omitted/default no-explicit-default behavior.
- [x] Add descriptor-backed set predicate conversion for equality,
  null-safe equality, inequality, `IS NULL`, and `IS NOT NULL`.
- [x] Add set introspection and result metadata for `SHOW COLUMNS`,
...
- [x] Review the final diff for MySQL behavior, descriptor authority, member
