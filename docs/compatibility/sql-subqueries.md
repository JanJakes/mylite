...
| `EXISTS` subqueries | 🟡 | Limited descriptor-driven `SELECT ... WHERE [NOT] EXISTS (...)` predicates for one outer base table and tableless/`DUAL` or one-table inner subqueries with optional limited predicate, simple correlated integer equality/null-safe equality, and `LIMIT row_count`; no scalar projection, DML predicates, nested subqueries, derived tables, joins, arbitrary expressions, or optimizer parity |
