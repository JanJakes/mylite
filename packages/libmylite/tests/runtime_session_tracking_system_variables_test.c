...
static int test_session_tracking_variable_values_and_show_rows(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open session tracking db");
...
static int test_session_tracking_variable_set_and_diagnostics(void) {
    ...
    failures +=
        expect_int(mylite_open_memory(&database), MYLITE_OK, "open session tracking SET db");
