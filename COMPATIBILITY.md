...
| `INSERT ... VALUES` | 🟡 | Limited single- and multi-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [INTO] table ... VALUES ...` into persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, no-column-list mapping to visible descriptor columns only, explicit column-list access to invisible columns, omitted-column descriptor default filling, strict range/nullability/no-default checks, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no DML `DEFAULT` keyword values, keys, generated values, `IGNORE`, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SET` | 🟡 | Limited one-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [INTO] table SET column = integer_or_NULL_or_boolean[, ...]` into persistent base tables with descriptor assignment resolution including explicitly named invisible columns, omitted-column descriptor default filling, strict required-column/range/nullability diagnostics, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no `IGNORE`, qualified assignment targets, expressions, DML `DEFAULT` keyword values, generated values, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `INSERT LOW_PRIORITY` / `HIGH_PRIORITY` / `DELAYED` | 🟡 | Limited modifier support on currently supported `INSERT` forms: `LOW_PRIORITY` and `HIGH_PRIORITY` are accepted as no-ops and `DELAYED` executes as ordinary `INSERT` with warning `3005`; no multiple modifiers, `IGNORE`, storage scheduling changes, delayed queues, or warning demotion. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
