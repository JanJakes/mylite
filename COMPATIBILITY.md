...
| Feature | Status | Priority | Target behavior | Implementation notes |
| --- | --- | --- | --- | --- |
| Query expression grammar | ❌ | top | Parenthesized query expressions, query terms, and query primary rules. | First `UNION`-only query-expression slice is specified for top-level `SELECT` operands, parenthesized SELECT operands, and global `ORDER BY`/`LIMIT`; parser/runtime implementation is not complete. See [UNION query expressions spec](docs/specs/union-query-expressions/specs.md). |
...
| `UNION` | ❌ | high | ALL/DISTINCT semantics, column names/types, ordering, limits, and parentheses. | First implementable slice is specified for top-level `SELECT ... UNION [ALL\|DISTINCT] SELECT ...` over already-supported SELECT surfaces, including duplicate handling, column-count validation, first-operand labels, empty union origin metadata, global `ORDER BY`/`LIMIT`/`OFFSET`, and parenthesized operands for local `ORDER BY`/`LIMIT`. Implementation and MySQL-runtime comparison tests remain pending. See [UNION query expressions spec](docs/specs/union-query-expressions/specs.md). |
