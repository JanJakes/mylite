...
| Last insert id | 🟡 | Limited connection-local first-generated-id state for the current persistent and session-temporary auto-increment insert subset, exposed through zero-argument `LAST_INSERT_ID()`; no `LAST_INSERT_ID(expr)`, protocol insert-id packets, C API state, mixed-mode allocation parity, or stored-program behavior |
