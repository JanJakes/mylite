# Baseline sys.io_global_by_wait_by_bytes Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.io_global_by_wait_by_bytes` and
`sys.x$io_global_by_wait_by_bytes` views. MySQL uses these sys views to
summarize Performance Schema file I/O byte counters grouped by event. MyLite
does not collect Performance Schema file-summary counters, so both views are
metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.io_global_by_wait_by_bytes` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_io_global_by_wait_by_bytes_views_expectations.sh`.
...
SELECT * FROM `x$io_global_by_wait_by_bytes`;
```
...
`sys.io_global_by_wait_by_bytes` has thirteen columns:
...
| `total_requested` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |

`sys.x$io_global_by_wait_by_bytes` has the same column names with raw numeric
...
| `min_latency` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
...
| `total_requested` | `bigint` | `NO` | `0` | SQL `NULL` |
...
- live file-event byte, latency, average, or total-requested rows;
