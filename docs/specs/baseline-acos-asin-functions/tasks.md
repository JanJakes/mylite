...
- [ ] Add `ACOS` and `ASIN` lexer and parser keyword handling without making
...
- [ ] Preserve `NULL` propagation and signed value conversion for both
      functions.
- [ ] Return `NULL` without warnings for admitted integer values outside
      `-1..1`.
...
- [ ] Add `runtime_acos_asin_functions` C tests under
