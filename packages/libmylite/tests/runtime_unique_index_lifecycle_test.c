...
static int test_unique_index_metadata_dml_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open unique index file");
    ...
    failures += expect_statement_ok(
        database,
        "CREATE TABLE full_prefix (name VARCHAR(20) NOT NULL DEFAULT '')"
    );
    failures +=
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen unique index file");
