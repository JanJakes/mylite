...
static int test_show_variables_values_scopes_and_filters(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open variables memory");
...
static int test_show_variables_state_and_file_safety(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open variables file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen variables file");
    failures += expect_single_row(
        database,
        "SHOW VARIABLES LIKE 'autocommit'",
        (struct expected_variable_row){
...
static int test_show_variables_independent_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first variables handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second variables handle"
    );
...
static int expect_query_rows(
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), variable_column_count, context);
    ...
    for (size_t row = 0U; row < expected_row_count; ++row) {
        for (size_t column = 0U; column < variable_column_count; ++column) {
            ...
            failures += mylite_test_expect_text_or_null(actual, expected, context);
        }
    }
