...
| `INFORMATION_SCHEMA.COLUMNS` | 🟡 | Limited queryable synthetic rows for supported `information_schema` system views and MyLite descriptor columns, including current integer/exact `DECIMAL`/`CHAR`/`VARCHAR`/baseline `TEXT` family metadata, nullability, integer and decimal defaults, visibility, primary-key markers, auto-increment `EXTRA`, and decimal `NUMERIC_PRECISION` / `NUMERIC_SCALE`; no privileges, generated columns, comments, string defaults, secondary index metadata, full charset/collation behavior, or complete MySQL system catalogs |
...
| `INFORMATION_SCHEMA.KEY_COLUMN_USAGE` | ❌ | Constraint key columns |
