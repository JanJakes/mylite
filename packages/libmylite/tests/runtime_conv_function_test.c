...
static int test_conv_warnings_and_do(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open warning db");
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "CONV DO column count");
    failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "CONV DO row count");
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 2U, "CONV DO warning count");
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "CONV DO affected rows");
    ...
    failures += mylite_test_expect_size(
        ...
        0U,
        "CONV successful DO columns"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "CONV successful DO rows");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "CONV successful DO warnings"
    );
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "CONV successful DO rows");
    mylite_result_free(result);
...
static int test_conv_errors_and_unsupported_forms(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_test_open_temporary(&database), MYLITE_OK, "open error db");
