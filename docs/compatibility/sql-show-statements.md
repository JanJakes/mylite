...
| `SHOW STATUS` | 🟡 | Limited runtime-owned status registry with MySQL 8.4.9 column labels, optional `GLOBAL`/`SESSION`/`LOCAL` scope, `LIKE 'pattern'` filters, limited trailing `WHERE` predicates over `Variable_name` and `Value`, the target MySQL 8.4.9 `Bytes_%`, `Connections`, session/local `Compression`, `Prepared_stmt_count`, `Queries`, `Questions`, `Threads_%`, `Uptime%`, `Binlog_%`, `Com_%`, `Created_%`, `Delayed_%`, `Handler_%`, `Key_%`, `Last_query_%`, `Max_%`, `Open%`, `Select_%`, `Slow_%`, `Sort_%`, `Ssl_%`, `Table_locks_%`, `Table_open_cache_%`, `Tc_log_%`, `Telemetry_%`, and related server/legacy scalar surfaces plus `Connection_%` diagnostic placeholders; values are embedded placeholders and do not implement live counters, TLS state, arbitrary `WHERE` expressions, Performance Schema status tables, privileges, or full status-variable coverage |
