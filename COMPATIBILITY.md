...
| `ALTER LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names, numeric `INITIAL_SIZE`, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
...
| `CREATE LOGFILE GROUP` | ❌ | low | NDB logfile group syntax and diagnostics. | Parser recognizes `ADD UNDOFILE` with string-literal file names, NDB numeric size/nodegroup options, comments, `WAIT`, and optional `ENGINE`/`STORAGE ENGINE` options. |
...
| `CREATE TABLESPACE` | ❌ | low | General and NDB tablespace syntax and diagnostics. | Parser recognizes string-literal data files, numeric autoextend/file-block sizes, string-literal encryption values, NDB logfile/size/nodegroup/comment options, `ENGINE`/`STORAGE ENGINE`, and string-literal engine attributes. |
