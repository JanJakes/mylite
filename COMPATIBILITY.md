...
| Feature | Status | Notes | Full table |
| --- | :-: | --- | --- |
| `SELECT` | 🟡 | Descriptor-driven single persistent base-table `SELECT *`, unqualified column-list reads, and limited one-column `SELECT DISTINCT column` with optional limited `WHERE`, single-column `ORDER BY`, and `LIMIT`/`OFFSET`, plus limited one-item `COUNT(*)`, `COUNT(column)`, `COUNT(integer/NULL/boolean literal)`, `COUNT(DISTINCT column)`, `MIN(column)`, and `MAX(column)` aggregate forms; `SELECT *` expands visible descriptor columns only while explicit references may name invisible columns; no joins, grouping, aliases, general expression projection, full distinct rows, or locking clauses. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| Projection list | 🟡 | Wildcard uses visible descriptor columns in catalog ordinal order; explicit projections and limited one-column `SELECT DISTINCT` resolve unqualified descriptor column names including invisible columns; limited one-item `COUNT(*)`, `COUNT(column)`, `COUNT(integer/NULL/boolean literal)`, `COUNT(DISTINCT column)`, `MIN(column)`, and `MAX(column)` aggregate forms are supported; duplicate projected columns are allowed for non-distinct selects, with no aliases, table-qualified references, or full multi-expression distinct rows. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `DISTINCT` / `DISTINCTROW` | 🟡 | Limited one-column `SELECT DISTINCT column FROM table` over one descriptor-backed persistent base table with optional baseline `WHERE`, optional `ORDER BY` on the same selected column, and existing select `LIMIT`/`OFFSET`; no `DISTINCTROW`, explicit `ALL`, wildcard, multiple select items, expressions, literals, aliases, table-qualified references, joins, grouping, or full distinct-row semantics. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `COUNT()` | 🟡 | Limited `COUNT(*)` and `COUNT(integer/NULL/boolean literal)` in one-item `SELECT` with no source, `FROM DUAL`, or one descriptor-backed persistent base table, plus limited `COUNT(column)` and `COUNT(DISTINCT column)` over one descriptor-backed persistent base table; optional baseline `WHERE` is supported for table-backed forms; no general `COUNT(expr)`, grouping, aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `COUNT(DISTINCT)` | 🟡 | Limited one-item `COUNT(DISTINCT column)` over one descriptor-backed persistent base table with optional baseline `WHERE`; integer/`NULL` descriptor columns only, no multiple expressions, expression arguments, literals, table-qualified arguments, grouping, aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `GROUP_CONCAT()` | ❌ | Concatenated aggregate string. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
| `MAX()` | 🟡 | Limited one-item `SELECT MAX(column)` over one descriptor-backed persistent base table with optional baseline `WHERE`; integer/`NULL` descriptor columns only, no expression arguments, `DISTINCT`, grouping, aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
