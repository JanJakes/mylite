...
static const char *const explain_column_names[explain_column_count] = {
    "Field",
    "Type",
    "Null",
    "Key",
...
static const char *const numbers_rows[][explain_column_count] = {
    {"id", "int", "NO", "", NULL, ""},
    {"i", "int", "YES", "", NULL, ""},
...
static const char *const single_column_rows[][explain_column_count] = {
    {"id", "int", "NO", "", NULL, ""},
};

static const char *const id_i_iu_rows[][explain_column_count] = {
    {"id", "int", "NO", "", NULL, ""},
    {"i", "int", "YES", "", NULL, ""},
    {"iu", "int unsigned", "YES", "", NULL, ""},
};

static const char *const pair_column_rows[][explain_column_count] = {
    {"id", "int", "NO", "", NULL, ""},
    {"value", "bigint", "YES", "", NULL, ""},
};

static int test_explain_values_persistence_rename_and_drop(void);
static int test_explain_diagnostics_and_unsupported_forms(void);
static int test_independent_explain_handles(void);
static int create_numbers_schema(mylite_db *database);
static int expect_explain_result(
    mylite_db *database,
    const char *sql,
    const char *const expected_rows[][explain_column_count],
...
int main(void) {
    ...
    failures += test_explain_values_persistence_rename_and_drop();
    failures += test_explain_diagnostics_and_unsupported_forms();
    failures += test_independent_explain_handles();
...
static int test_explain_values_persistence_rename_and_drop(void) {
    ...
    failures += expect_explain_result(
        database,
        "EXPLAIN app.numbers",
        numbers_rows,
        sizeof(numbers_rows) / sizeof(numbers_rows[0]),
        "qualified explain without default schema"
    );
    ...
    failures += expect_row_count(database, -1, "row count after explain");
    failures += read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble));
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after explain"
    );
...
static int test_explain_diagnostics_and_unsupported_forms(void) {
    ...
    failures += execute_error(
        database,
        "EXPLAIN _mylite_catalog.numbers",
        (struct expected_sql_error){
    ...
    failures += expect_explain_result(
        ...
        "explain string filter"
    );
    failures += execute_error(
        database,
        "EXPLAIN numbers FROM app",
        (struct expected_sql_error){
...
static int expect_explain_result(
    ...
    failures += expect_size(mylite_result_column_count(result), explain_column_count, context);
    ...
    for (size_t column_index = 0U; column_index < explain_column_count; ++column_index) {
        failures += expect_text_or_null(
            mylite_result_column_name(result, column_index),
            explain_column_names[column_index],
            context
        );
    }
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_explain_table_%s_%d.mylite",
        name,
        current_process_id()
    );
