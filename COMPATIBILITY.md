...
| `ALTER VIEW` | ❌ | high | View replacement while preserving MySQL metadata and security semantics. | Parser recognizes column lists, view body starts, and `WITH [CASCADED|LOCAL] CHECK OPTION` tails on non-plain-`SELECT` bodies including parenthesized query expressions, with validation for direct body `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized query `ORDER BY`/`LIMIT` suffixes, and malformed set-operation `SELECT` operands. |
...
| `CREATE TABLE ... SELECT` | ❌ | high | CTAS type inference, default handling, indexes, and atomicity. | Parser recognizes no-definition and post-definition CTAS forms, including parenthesized query bodies, and validates direct CTAS `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized query `ORDER BY`/`LIMIT` suffixes, malformed set-operation `SELECT` operands, and `VALUES ROW(...)` row-expression bodies. |
...
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser recognizes direct and parenthesized query payloads, validates direct payload `SELECT` list tails, CTE-body `SELECT` list tails, parenthesized payload outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, plus `ON DUPLICATE KEY UPDATE` continuations. |
...
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser recognizes direct and parenthesized query payloads and validates direct payload `SELECT` list tails, parenthesized payload outer `ORDER BY`/`LIMIT` tails, and malformed set-operation `SELECT` operands. |
