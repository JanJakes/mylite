...
| `DELETE` (multi-table) | ❌ | high | Multi-table delete forms using FROM and USING, join semantics, and affected rows. | Parser recognizes `DELETE tbl_list FROM ...` and `DELETE FROM tbl_list USING ...` forms, including comma-separated delete targets, qualified `tbl.*` targets, joined source table references, and `WHERE` tails. |
