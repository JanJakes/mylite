...
static int expect_show_create_database_result(
    ...
    failures +=
        expect_size(mylite_result_column_count(result), show_create_database_column_count, context);
    ...
    failures +=
        expect_text_or_null(mylite_result_value_text(result, 0U, 1U), expected.create_sql, context);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_show_create_database_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
