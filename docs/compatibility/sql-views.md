...
| `INFORMATION_SCHEMA.VIEWS` | 🟡 | Queryable synthetic view catalog with MySQL 8.4.9-shaped columns and rows for baseline view descriptors plus the built-in `sys.version` view-definition row; definitions, fixed definer/security/check metadata, charset/collation, and deliberately non-updatable status are stored or synthesized by MyLite, but there is no privilege filtering, check-option enforcement, or view execution |
