# Baseline mysql.columns_priv table tasks
...
- [x] Specify the MyLite scope as an empty read-only metadata placeholder
  without column-level grant storage or enforcement.
...
- [x] Implement the `mysql.columns_priv` table definition and index metadata.
