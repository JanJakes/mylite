...
%type opt_show_schemas_filter { struct mylite_sql_ast_node * }
...
%fallback IDENTIFIER ADDDATE AFTER AGGREGATE AT AUTO_INCREMENT BEGIN BIT BOOL BOOLEAN BTREE CATALOG_NAME
    ...
    INVISIBLE INVOKER JSON KEY_BLOCK_SIZE LANGUAGE MEMORY MESSAGE_TEXT MINUTE MODIFY MONTH MYSQL_ERRNO
    LINESTRING MICROSECOND MULTILINESTRING MULTIPOINT MULTIPOLYGON NCHAR NO NULLS NVARCHAR OFFSET ONLY
...
insert_select_source_statement(A) ::= SELECT(T) select_modifiers(M) select_item_list(B) FROM(F)
        DUAL(D) opt_where_clause(E) opt_window_clause(W) opt_order_by_clause(I)
        opt_limit_clause(J). {
    A = mylite_sql_parser_make_select_statement(
...
show_schemas_statement(A) ::= SHOW(T) DATABASES(D) opt_show_schemas_filter(F). {
    A = mylite_sql_parser_make_show_schemas_statement(state, T, D, F);
}
show_schemas_statement(A) ::= SHOW(T) SCHEMAS(D) opt_show_schemas_filter(F). {
    A = mylite_sql_parser_make_show_schemas_statement(state, T, D, F);
}

opt_show_schemas_filter(A) ::= . {
    A = NULL;
}
opt_show_schemas_filter(A) ::= LIKE STRING(T). {
    A = mylite_sql_parser_make_literal(state, T, MYLITE_SQL_AST_LITERAL_STRING);
}
opt_show_schemas_filter(A) ::= where_clause(B). {
    A = B;
}
...
parser_placeholder_keyword(A) ::= JSON(T). {
    A = T;
}
...
create_table_statement(A) ::= CREATE(T) opt_temporary(U) TABLE opt_if_not_exists(B) table_name(C) table_option_list(E) opt_as select_statement(S). {
    A = mylite_sql_parser_make_create_table_select_statement(state, T, U, B, C, NULL, E, S);
}
create_table_statement(A) ::= CREATE(T) opt_temporary(U) TABLE opt_if_not_exists(B) table_name(C) LPAREN table_element_list(D) RPAREN table_option_list(E) opt_as select_statement(S). {
    A = mylite_sql_parser_make_create_table_select_statement(state, T, U, B, C, D, E, S);
}
...
select_union_operand(A) ::= SELECT(T) select_modifiers(M) select_item_list(B)
        FROM(F) DUAL(D) opt_where_clause(E) opt_window_clause(W). {
    A = mylite_sql_parser_make_select_statement(
        state, T, M, B, mylite_sql_parser_make_from_dual(state, F, D), E, NULL, NULL, W, NULL,
        NULL);
}
select_union_operand(A) ::= SELECT(T) select_modifiers(M) select_item_list(B)
...
select_union_operand(A) ::= SELECT(T) select_modifiers(M) STAR(S) FROM(F) DUAL(D)
        opt_where_clause(E) opt_window_clause(W). {
    A = mylite_sql_parser_make_select_statement(
        state, T, M, mylite_sql_parser_make_wildcard_select_list(state, S),
        mylite_sql_parser_make_from_dual(state, F, D), E, NULL, NULL, W, NULL, NULL);
}
...
cast_target_type(A) ::= JSON(T). {
    A = mylite_sql_parser_make_column_type(state, T, MYLITE_SQL_AST_COLUMN_TYPE_JSON);
}
...
interval_unit(A) ::= QUARTER. {
    A = MYLITE_SQL_AST_INTERVAL_UNIT_QUARTER;
}
interval_unit(A) ::= MICROSECOND. {
    A = MYLITE_SQL_AST_INTERVAL_UNIT_MICROSECOND;
}
