...
static int test_no_source_dual_metadata_and_do(void) {
    ...
    if (failures == 0) {
        ...
        failures += expect_size(mylite_result_row_count(result), 1U, "core calendar name rows");
        failures +=
            expect_int64(mylite_result_affected_rows(result), 0, "core calendar name affected");
        failures +=
            expect_size(mylite_result_warning_count(result), 0U, "core calendar name warnings");
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "calendar name do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "calendar name do rows");
        failures +=
            expect_int64(mylite_result_affected_rows(result), 0, "calendar name do affected");
        failures +=
            expect_size(mylite_result_warning_count(result), 0U, "calendar name do warnings");
    }
    mylite_result_free(result);
    failures += expect_query(
        database,
...
static int test_table_backed_day_month_names_and_reopen(void) {
    ...
    database = NULL;
    failures +=
        expect_int(mylite_open(path, &database), MYLITE_OK, "reopen calendar name database");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-day-month-name-functions-%s-%d.mylite",
        name,
        current_process_id()
    );
