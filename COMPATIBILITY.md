...
| Outer joins | ❌ | top | LEFT/RIGHT OUTER JOIN null-extension and predicate placement. | Task 27 is specified for the base-table SELECT slice, including required `ON`/`USING`, null extension, `ON` versus `WHERE` placement, wildcard/coalesced metadata, alias/name-resolution diagnostics, and warning behavior. Implementation remains pending. See [outer joins spec](docs/specs/outer-joins/specs.md). |
