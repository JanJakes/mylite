...
| `UTC_DATE()` | ❌ | high | Return the current UTC date | Deferred; this current-temporal slice intentionally implements only local/session current temporal functions. See [current temporal functions spec](docs/specs/current-temporal-functions/specs.md). |
| `UTC_TIME()` | ❌ | high | Return the current UTC time | Deferred; this current-temporal slice intentionally implements only local/session current temporal functions. See [current temporal functions spec](docs/specs/current-temporal-functions/specs.md). |
| `UTC_TIMESTAMP()` | ❌ | high | Return the current UTC date and time | Deferred; this current-temporal slice intentionally implements only local/session current temporal functions. See [current temporal functions spec](docs/specs/current-temporal-functions/specs.md). |
