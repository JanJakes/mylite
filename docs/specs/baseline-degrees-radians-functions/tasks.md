...
- [x] Add `DEGREES` and `RADIANS` lexer and parser keyword handling without
...
- [x] Preserve existing child arithmetic overflow and division-by-zero warning
      behavior.
- [x] Format non-`NULL` results as shortest round-tripping double text matching
...
- [x] Add `runtime_degrees_radians_functions` C tests under
