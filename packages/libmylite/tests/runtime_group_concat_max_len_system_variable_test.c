...
static int test_group_concat_max_len_values_and_assignment(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open variable memory");
...
static int test_group_concat_max_len_truncation_warnings_and_file_safety(void) {
    ...
    if (make_test_path(path, sizeof(path), "truncation") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open truncation file");
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen truncation file");
    failures += execute_discard(database, "USE app");
    failures += expect_variable_value(
        database,
        (struct expected_variable_value){
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_group_concat_max_len_%s_%d.mylite",
        name,
        current_process_id()
    );
