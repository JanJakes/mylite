...
The `baseline-sys-memory-by-thread-by-current-bytes-views` slice extends
`INFORMATION_SCHEMA.COLUMNS`, `TABLES`, `VIEWS`, and `VIEW_TABLE_USAGE` with
MySQL-shaped metadata for the supported empty synthetic
`sys.memory_by_thread_by_current_bytes` and
`sys.x$memory_by_thread_by_current_bytes` views, while preserving
MySQL-observed empty `VIEW_ROUTINE_USAGE` results for those views. It does not
add Performance Schema memory-summary collection, live thread rows, sys
helper-function execution, privilege filtering, or broader sys view execution.

The `baseline-sys-memory-by-user-by-current-bytes-views` slice extends
`INFORMATION_SCHEMA.COLUMNS`, `TABLES`, `VIEWS`, and `VIEW_TABLE_USAGE` with
MySQL-shaped metadata for the supported empty synthetic
`sys.memory_by_user_by_current_bytes` and
`sys.x$memory_by_user_by_current_bytes` views, while preserving MySQL-observed
empty `VIEW_ROUTINE_USAGE` results for those views. It does not add
Performance Schema memory-summary collection, live user rows, sys
helper-function execution, privilege filtering, or broader sys view execution.
...
| `INFORMATION_SCHEMA.FILES` | 🟡 | Queryable empty tablespace-file catalog with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally reports no rows because it has no physical InnoDB `.ibd` / `ibdata` / undo / temporary files or NDB Disk Data files, with no privileges or mutable storage-engine accounting |
...
| `INFORMATION_SCHEMA.INNODB_DATAFILES` | 🟡 | Queryable empty InnoDB datafile catalog with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally reports no default or user rows because it has no physical InnoDB files, mutable tablespaces, privileges, or InnoDB data dictionary |
...
| `INFORMATION_SCHEMA.INNODB_SESSION_TEMP_TABLESPACES` | 🟡 | Queryable empty session temporary-tablespace view with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally reports no file paths, sizes, state, purpose, or connection ownership because it has no InnoDB session temporary tablespaces, replica tablespaces, privilege filtering, or InnoDB temporary-tablespace state |
...
| `INFORMATION_SCHEMA.INNODB_TABLESPACES` | 🟡 | Queryable empty InnoDB tablespace catalog with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally reports no built-in or user rows because it has no physical `.ibd` storage, mutable tablespaces, privileges, or InnoDB data dictionary |
| `INFORMATION_SCHEMA.INNODB_TABLESPACES_BRIEF` | 🟡 | Queryable empty InnoDB tablespace summary catalog with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally reports no built-in or user rows because it has no physical InnoDB tablespaces, privileges, or InnoDB data dictionary |
...
| `INFORMATION_SCHEMA.SCHEMATA_EXTENSIONS` | 🟡 | Limited queryable synthetic schema-options rows for built-in and MyLite catalog schemas, with MySQL 8.4.9-shaped columns, matching `INFORMATION_SCHEMA.TABLES` / `INFORMATION_SCHEMA.COLUMNS` metadata, and empty `OPTIONS` values; `ALTER SCHEMA ... READ ONLY` parses to unsupported utility diagnostics without mutating this metadata; no mutable read-only state, privileges, or physical data dictionary tables |
...
| `INFORMATION_SCHEMA.TABLESPACES_EXTENSIONS` | 🟡 | Queryable empty tablespace extension-attribute view with MySQL 8.4.9-shaped columns and matching system metadata; MyLite intentionally maps neither baseline InnoDB names nor embedded persistent tables to tablespace rows because it has no physical tablespaces, storage-engine attributes, privileges, or tablespace DDL |
