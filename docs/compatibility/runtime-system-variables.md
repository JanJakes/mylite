...
| `warning_count` | 🟡 | Limited read-only scalar `SELECT @@warning_count`, `@@session.warning_count`, and `@@local.warning_count` over the previous diagnostics snapshot; counts warning and note records currently stored by MyLite; no global scope, `SET`, `SHOW VARIABLES`, `max_error_count`, mutable `sql_notes`, or counted-but-not-stored conditions |
