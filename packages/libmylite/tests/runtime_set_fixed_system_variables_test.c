...
static int test_set_fixed_system_variables_success_and_file_safety(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open fixed SET file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen fixed SET file");
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT @@autocommit, @@foreign_key_checks, @@unique_checks, "
...
static int expect_set_ok(mylite_db *database, const char *sql) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "fixed SET column count");
    failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "fixed SET row count");
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "fixed SET affected rows");
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "fixed SET warning count");
    mylite_result_free(result);
