...
TABLE_PRIV_TYPE="set('Select','Insert','Update','Delete','Create','Drop','Grant','References','Index','Alter','Create View','Show view','Trigger')"
COLUMN_PRIV_TYPE="set('Select','Insert','Update','References')"
PRIVILEGES="select,insert,update,references"

fail() {
    printf '%s\n' "mysql_baseline_mysql_tables_priv_table_expectations: $1" >&2
    exit 1
}
...
expect_show_table_status_row() {
    output=$(run_mysql "SHOW TABLE STATUS FROM mysql LIKE 'tables_priv';")
    ...
    if [ "$field_count" != "18" ]; then
        fail "SHOW TABLE STATUS tables_priv: expected 18 fields, got [$field_count]"
    fi
    expected_prefix=$(
        printf '%b' 'tables_priv\tInnoDB\t10\tDynamic\t2\t8192\t16384\t0\t16384\t4194304\tNULL'
    )
    if [ "$prefix" != "$expected_prefix" ]; then
        fail "SHOW TABLE STATUS tables_priv: expected prefix [$expected_prefix], got [$prefix]"
    fi
    case "$create_time" in
        ????-??-??\ ??:??:??) ;;
        *) fail "SHOW TABLE STATUS tables_priv: expected Create_time datetime, got [$create_time]" ;;
    esac
    expected_tail=$(
        printf '%b' \
            'NULL\tNULL\tutf8mb3_bin\tNULL\trow_format=DYNAMIC stats_persistent=0\tTable privileges'
    )
    if [ "$stable_tail" != "$expected_tail" ]; then
        fail "SHOW TABLE STATUS tables_priv: expected tail [$expected_tail], got [$stable_tail]"
    fi
}
...
expect_output \
    "mysql.tables_priv target row count" \
    "2" \
...
columns_expected=$(
    {
        ...
        printf '%b\n' "Table_priv\t$TABLE_PRIV_TYPE\tNO\t\t\t"
        printf '%b\n' "Column_priv\t$COLUMN_PRIV_TYPE\tNO\t\t\t"
    }
)
expect_output \
    "mysql.tables_priv SHOW COLUMNS rows" \
    "$columns_expected" \
...
full_columns_expected=$(
    {
        ...
        printf '%b\n' \
            "Table_priv\t$TABLE_PRIV_TYPE\tutf8mb3_general_ci\tNO\t\t\t\t$PRIVILEGES\t"
        printf '%b\n' \
            "Column_priv\t$COLUMN_PRIV_TYPE\tutf8mb3_general_ci\tNO\t\t\t\t$PRIVILEGES\t"
    }
)
expect_output \
    "mysql.tables_priv SHOW FULL COLUMNS rows" \
    "$full_columns_expected" \
    "SHOW FULL COLUMNS FROM mysql.tables_priv;"

show_index_expected=$(
    printf '%b' \
        'tables_priv\t0\tPRIMARY\t1\tHost\tA\t1\tNULL\tNULL\t\tBTREE\t\t\tYES\tNULL\n' \
        'tables_priv\t0\tPRIMARY\t2\tUser\tA\t2\tNULL\tNULL\t\tBTREE\t\t\tYES\tNULL\n' \
        'tables_priv\t0\tPRIMARY\t3\tDb\tA\t2\tNULL\tNULL\t\tBTREE\t\t\tYES\tNULL\n' \
        'tables_priv\t0\tPRIMARY\t4\tTable_name\tA\t2\tNULL\tNULL\t\tBTREE\t\t\tYES\tNULL\n' \
        'tables_priv\t1\tGrantor\t1\tGrantor\tA\t2\tNULL\tNULL\t\tBTREE\t\t\tYES\tNULL'
)
expect_output \
    "mysql.tables_priv SHOW INDEX rows" \
    "$show_index_expected" \
    "SHOW INDEX FROM mysql.tables_priv;"
...
information_schema_columns_expected=$(
    {
        ...
        printf '%b\n' "Grantor\t5\t\tNO\tvarchar\t288\t864\tNULL\tNULL\tNULL\tutf8mb3\tutf8mb3_bin\tvarchar(288)\tMUL\t\t$PRIVILEGES\t\t"
        ...
        printf '%b\n' "Table_priv\t7\t\tNO\tset\t98\t294\tNULL\tNULL\tNULL\tutf8mb3\tutf8mb3_general_ci\t$TABLE_PRIV_TYPE\t\t\t$PRIVILEGES\t\t"
        printf '%b\n' "Column_priv\t8\t\tNO\tset\t31\t93\tNULL\tNULL\tNULL\tutf8mb3\tutf8mb3_general_ci\t$COLUMN_PRIV_TYPE\t\t\t$PRIVILEGES\t\t"
    }
)
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.COLUMNS rows" \
    "$information_schema_columns_expected" \
...
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.STATISTICS rows" \
...
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.TABLE_CONSTRAINTS row" \
...
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.KEY_COLUMN_USAGE rows" \
...
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.TABLE_CONSTRAINTS_EXTENSIONS rows" \
    "$(printf '%b' 'PRIMARY\ttables_priv\tNULL\tNULL\nGrantor\ttables_priv\tNULL\tNULL')" \
...
expect_output \
    "mysql.tables_priv INFORMATION_SCHEMA.TABLES row" \
    "$(printf '%b' 'tables_priv\tBASE TABLE\tInnoDB\t10\tDynamic\t2\t8192\t16384\t0\t16384\t4194304\tNULL\t1\t1\t1\tutf8mb3_bin\t1\trow_format=DYNAMIC stats_persistent=0\tTable privileges')" \
...
printf '%s\n' "mysql_baseline_mysql_tables_priv_table_expectations: ok"
