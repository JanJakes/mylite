# Baseline sys.io_global_by_file_by_latency Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.io_global_by_file_by_latency` and
`sys.x$io_global_by_file_by_latency` views. MySQL uses these sys views to
summarize Performance Schema file-summary latency counters by file. MyLite does
not collect Performance Schema file I/O summaries, so both views are
metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.io_global_by_file_by_latency` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_io_global_by_file_by_latency_views_expectations.sh`.
...
SELECT * FROM `x$io_global_by_file_by_latency`;
```
...
surface without inventing file paths or latency values.

## Column Metadata

`sys.io_global_by_file_by_latency` has nine columns:
...
| `count_misc` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `misc_latency` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |

`sys.x$io_global_by_file_by_latency` has the same column names with raw numeric
latency metadata:
...
- live file path, event count, or latency rows;
- execution of `sys.format_path()` or `format_pico_time()`;
