...
- [ ] Add `ATAN` and `ATAN2` lexer and parser keyword handling without making
      bare names stop behaving as identifier lookups.
- [ ] Add function and argument-count-error AST nodes for both functions.
- [ ] Add Lemon grammar for one-argument calls, two-argument calls, and
...
- [ ] Preserve `NULL` propagation, signed value conversion, and two-argument
      first-argument `NULL` short-circuit behavior.
- [ ] Preserve existing child arithmetic overflow and division-by-zero warning
      behavior.
- [ ] Format non-`NULL` results through the existing scalar double text path
...
- [ ] Add `runtime_atan_functions` C tests under `packages/libmylite/tests/`.
- [ ] Register the test binary and dotted CTest entry in
      `packages/libmylite/CMakeLists.txt`.
- [ ] Cover one-argument and two-argument result values, labels, aliases,
