...
static int test_drop_column_success_descriptor_persistence_and_dml(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open drop-column file");
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "insert after drop rows");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "insert after drop warnings"
    );
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "UPDATE numbers SET b = -5 WHERE id = 3", &result);
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "update after drop rows");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "update after drop warnings"
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "delete after drop rows");
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "delete after drop warnings"
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen drop-column file");
    failures += execute_ok(database, "USE app", &result);
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
...
static int test_drop_column_key_dependency_updates(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "key_dependencies") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open key dependency file");
    failures += execute_ok(database, "CREATE DATABASE app", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "USE app", &result);
