...
static int test_scalar_intersect_except_values_and_metadata(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open scalar set operation memory"
    );
...
static int test_table_intersect_except_persistence_and_file_safety(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "table_set_operations") != 0) {
        return 1;
    }
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open set operation file");
...
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected) {
    ...
    failures += mylite_test_expect_int(result == NULL, 1, "error result is null");
    mylite_result_free(result);
    return failures;
}
