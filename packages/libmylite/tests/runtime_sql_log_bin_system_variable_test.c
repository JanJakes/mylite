...
static int test_sql_log_bin_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql log bin file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql log bin file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_log_bin, @@session.sql_log_bin",
        (struct expected_result){
...
static int test_sql_log_bin_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql log bin memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_LOG_BIN, @@SESSION.SQL_LOG_BIN, "
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_log_bin_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
