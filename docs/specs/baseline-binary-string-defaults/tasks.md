...
- [x] Add a catalog default kind for descriptor-owned binary default bytes.
- [x] Convert admitted `BINARY` and `VARBINARY` literal defaults through the
      existing MyLite binary literal conversion path.
- [x] Store binary defaults as byte-safe internal hex text in catalog
      descriptors.
- [x] Materialize omitted-column and explicit DML `DEFAULT` values as BLOB
      planned values.
- [x] Render binary defaults through `SHOW COLUMNS`, `DESCRIBE`,
