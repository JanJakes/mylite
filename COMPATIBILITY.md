...
| `CREATE TABLE ... SELECT` | ❌ | high | CTAS type inference, default handling, indexes, and atomicity. | Parser recognizes no-definition and post-definition CTAS forms, including parenthesized query bodies, and validates parenthesized query `ORDER BY`/`LIMIT` suffixes and malformed set-operation `SELECT` operands. |
...
| `VALUES` | ❌ | high | Standalone values statement and row constructor behavior. | Parser recognizes non-empty comma-separated `VALUES ROW(...)` contents with adjacent-operand and dangling-operator row-expression checks, `UNION`/`INTERSECT`/`EXCEPT`, combined `ORDER BY`/`LIMIT` tails, and malformed set-operation `SELECT` operands. |
