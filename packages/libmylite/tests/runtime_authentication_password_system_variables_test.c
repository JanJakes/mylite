...
static int test_authentication_password_values_and_show_rows(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open auth password db");
...
static int test_authentication_password_set_and_diagnostics(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open auth password SET db");
