...
static int evaluate_table_select_default_function(
    void *user_data,
    const struct mylite_sql_ast_node *function_call,
    struct mylite_expression_value *out_value
);

static int evaluate_table_select_aggregate_call(
    void *user_data,
    const struct mylite_sql_ast_node *aggregate,
    struct mylite_expression_value *out_value
);

static int evaluate_table_select_subquery_expression(
    void *user_data,
...
static int copy_table_select_column_value(
    mylite_stmt *stmt,
    size_t column_index,
...
void mylite_select_eval_expression_context_init(
    ...
    *expression_context = (struct mylite_expression_eval_context){
        ...
        .collation_connection =
            user_context != NULL && user_context->stmt != NULL
                ? mylite_connection_collation_connection(user_context->stmt->database)
                : NULL,
...
static int evaluate_table_select_cached_constant_expression(
    ...
    if (entry == NULL) {
        struct mylite_cached_expression_value *values = realloc(
            stmt->select_constant_values,
            (stmt->select_constant_value_count + 1U) * sizeof(*stmt->select_constant_values)
        );
...
static int evaluate_table_select_default_function(
    ...
    if (status == MYLITE_OK) {
        status = mylite_dml_resolve_default_function_value(
            context->stmt->database,
            &write_table.columns[table_column_index],
            out_value
        );
    }
