...
| `DATABASE()` scalar readback | ✅ | MySQL-runtime-verified one-row scalar `SELECT DATABASE() [AS alias]` with optional `FROM DUAL` returns the selected MyLite catalog or built-in schema, or `NULL` when no schema is selected |
| `DATABASE()` broader expression coverage | 🟡 | Additional limited `DO` expression execution is supported, but table-backed evaluation, clauses, and general expression support remain incomplete |
...
| `SCHEMA()` scalar readback | ✅ | MySQL-runtime-verified synonym for `DATABASE()` in the same one-row scalar-select slice |
| `SCHEMA()` broader expression coverage | 🟡 | Additional limited `DO` expression execution is supported as a `DATABASE()` synonym, but table-backed evaluation, clauses, and general expression support remain incomplete |
