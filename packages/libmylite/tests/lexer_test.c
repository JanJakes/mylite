...
static struct expected_token make_expected_token(
    enum mylite_sql_token_kind kind,
    const char *text,
    enum mylite_sql_operator_kind operator_kind,
    enum mylite_sql_lexer_error error,
...
static int expect_single_token_values(
    const char *sql,
    unsigned int modes,
    enum mylite_sql_token_kind kind,
    enum mylite_sql_lexer_error error
...
static int test_keywords(void) {
    ...
    failures += expect_keyword("exp", 0U);
    failures += expect_keyword("ln", 0U);
    failures += expect_keyword("log", 0U);
    failures += expect_keyword("log10", 0U);
    failures += expect_keyword("log2", 0U);
    failures += expect_keyword("pow", 0U);
    failures += expect_keyword("power", 0U);
    failures += expect_keyword("greatest", 0U);
    failures += expect_keyword("least", 0U);
    failures += expect_keyword("export_set", 0U);
    failures += expect_keyword("lcase", 0U);
    failures += expect_keyword("left", MYLITE_SQL_KEYWORD_RESERVED);
    failures += expect_keyword("lpad", 0U);
    ...
    failures += expect_keyword("make_set", 0U);
...
static int expect_single_token_values(
    ...
    return expect_single_token((struct single_token_expectation){
        .sql = sql,
        .modes = modes,
        .kind = kind,
        .error = error,
