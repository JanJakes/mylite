...
static int test_information_schema_innodb_virtual_rows(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb virtual db");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen innodb virtual db");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT v.TABLE_ID, v.POS, v.BASE_POS "
