# Baseline Foreign Key Checks System Variable Tasks

Add the narrow scalar system-variable slice for MyLite's fixed enabled
foreign-key-checking baseline: `@@foreign_key_checks`.
...
2. Runtime resolver
   - Add `foreign_key_checks` to the existing system-variable resolver.
...
5. Compatibility docs
   ...
   - Do not claim mutable foreign-key-checking state, `SET`, foreign key DDL,
     enforcement, metadata, cascades, dependency checks, or `SHOW VARIABLES`.
...
- Do not implement `SET`, startup options, persisted variables, foreign key
  syntax, constraint descriptors, enforcement, cascades, dependency checks,
  metadata, `SHOW VARIABLES`, Performance Schema variable tables, table-backed
