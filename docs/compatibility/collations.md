...
| `utf8mb4_0900_ai_ci` | 🟡 | Limited static catalog row, schema/table/column default metadata preservation, `SET NAMES` session readback, fixed scalar `@@collation_server`, descriptor-backed scalar `@@collation_database`, server/database collation variable assignment readback, MyLite-owned ASCII equality subset for admitted `CHAR` / `VARCHAR` primary and unique key enforcement, and limited explicit postfix `COLLATE` comparison/`LIKE`/`ORDER BY` behavior for documented ASCII descriptor/row-scalar operands; utf8mb4; id 255; default; sortlen 0; NO PAD; no full Unicode weight or general comparison semantics |
...
| `utf8mb4_0900_as_cs` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE utf8mb4_0900_as_cs` validation and limited explicit case-sensitive ASCII comparison/`LIKE`/`ORDER BY` behavior for documented descriptor/row-scalar operands through MyLite's ASCII case-sensitive collation; utf8mb4; id 278; sortlen 0; NO PAD; no DDL/default admission, full Unicode weight, accent-sensitive ordering parity, storage conversion, or general comparison semantics |
...
| `utf8mb4_0900_as_ci` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE utf8mb4_0900_as_ci` validation and limited explicit ASCII case-insensitive comparison/`LIKE`/`ORDER BY` behavior for documented descriptor/row-scalar operands using MyLite's current string-key collation; utf8mb4; id 305; sortlen 0; NO PAD; no DDL/default admission, full Unicode weight, accent-sensitive ordering parity, storage conversion, or general comparison semantics |
