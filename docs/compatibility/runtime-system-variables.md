...
| `sql_warnings` | 🟡 | Limited scalar `SELECT @@sql_warnings` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; limited fixed no-op `SET` forms may preserve `0`; limited `INSERT IGNORE ... VALUES` / `SET` adjustment, duplicate, and supported foreign-key warnings are still recorded in diagnostics like MySQL; no mutable global/session state, broader warning-producing DML conversions, protocol information strings, changed `INSERT` behavior beyond the limited `IGNORE` subset, or Performance Schema variable tables |
