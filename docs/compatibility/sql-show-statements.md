...
| `SHOW COUNT(*) ERRORS` | 🟡 | Limited previous-statement error-condition count with MySQL 8.4.9 column label `@@session.error_count`; counts current MyLite error conditions, including retained error warning rows and the current `max_error_count=0` parse-error snapshot suppression behavior; related scalar `@@error_count` is limited separately; no `GET DIAGNOSTICS`, privileges, diagnostics stacks, or full diagnostics-area behavior |
| `SHOW COUNT(*) WARNINGS` | 🟡 | Limited previous-statement diagnostics count with MySQL 8.4.9 column label; counts MyLite previous error conditions and total current warning/note/error records even when `@@max_error_count` caps retained rows, and honors `sql_notes=0` for current note producers. Missing-schema `DROP DATABASE IF EXISTS` intentionally leaves no stored count after reporting a statement warning count; no `GET DIAGNOSTICS`, privileges, diagnostics stacks, or full diagnostics-area behavior |
...
| `SHOW ERRORS` | 🟡 | Limited previous-statement error-condition rows with `Level`, `Code`, and `Message`, plus unsigned decimal `LIMIT` slicing; reports current retained MyLite error rows and obeys `@@max_error_count` retained-row capping; related scalar `@@error_count` is limited separately; no `WHERE`, `LIKE`, expression filters, `GET DIAGNOSTICS`, privileges, diagnostics stacks, or full diagnostics-area behavior |
...
| `SHOW WARNINGS` | 🟡 | Limited previous-statement diagnostics rows with `Level`, `Code`, and `Message`, plus unsigned decimal `LIMIT` slicing; reports current retained MyLite error, warning, and note records, obeys `@@max_error_count` retained-row capping, and honors `sql_notes=0` for current note producers. Missing-schema `DROP DATABASE IF EXISTS` intentionally stores no warning row after reporting a statement warning count; no `WHERE`, `LIKE`, expression filters, `GET DIAGNOSTICS`, privileges, diagnostics stacks, or full diagnostics-area behavior |
