...
| `INFORMATION_SCHEMA.COLUMNS` | 🟡 | Limited queryable synthetic rows for supported `information_schema` system views and MyLite descriptor columns, including current integer/exact `DECIMAL`/canonical `DATE`/canonical `DATETIME`/canonical `TIMESTAMP`/`CHAR`/`VARCHAR`/baseline `TEXT` family metadata, nullability, integer, decimal, canonical date-string, canonical datetime-string, and canonical timestamp-string defaults, visibility, primary-key plus supported unique/nonunique secondary-index `COLUMN_KEY` markers, auto-increment `EXTRA`, decimal `NUMERIC_PRECISION` / `NUMERIC_SCALE`, `NULL` temporal precision for `DATE`, and `DATETIME_PRECISION = 0` for `DATETIME` and `TIMESTAMP`; no privileges, generated columns, comments, string defaults outside admitted canonical temporal defaults, full charset/collation behavior, or complete MySQL system catalogs |
