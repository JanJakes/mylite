...
| `REPLACE ... SELECT` | ❌ | high | Replace from query expression semantics. | Parser recognizes direct and parenthesized query payloads, validates direct payload `SELECT` list tails, parenthesized payload `SELECT`/`WITH`/`TABLE`/`VALUES` expression tails plus outer `ORDER BY`/`LIMIT` tails, malformed set-operation `SELECT` operands, and rejects unsupported `ON DUPLICATE KEY UPDATE` tails. |
