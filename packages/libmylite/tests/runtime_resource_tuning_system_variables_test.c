...
static int test_resource_tuning_defaults_and_show_rows(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open defaults db");
...
static int test_resource_tuning_set_and_warnings(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open SET db");
...
static int test_resource_tuning_diagnostics_and_isolation(void) {
    ...
    failures += expect_int(mylite_open_memory(&first), MYLITE_OK, "open first diagnostics db");
    failures += expect_int(mylite_open_memory(&second), MYLITE_OK, "open second diagnostics db");
