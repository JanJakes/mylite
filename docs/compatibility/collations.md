...
| `utf8mb3_general_ci` | 🟡 | Metadata catalog row plus limited metadata collation for national `CHAR` / `VARCHAR` aliases in `SHOW CREATE TABLE`, `SHOW FULL COLUMNS`, `INFORMATION_SCHEMA.COLUMNS`, and result-column metadata, scalar `CONVERT(value USING utf8\|utf8mb3)` default metadata, `utf8_general_ci` alias canonicalization, and limited WordPress descriptor/session readback; no storage conversion or general comparison/order/group/distinct semantics |
