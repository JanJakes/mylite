...
static int test_rename_index_success_metadata_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open rename index file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen rename index file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_rename_index_diagnostics(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open rename diagnostics db");
    failures += execute_error(
        database,
...
static int test_rename_index_foreign_key_rename_and_drop_interaction(void) {
    ...
    if (make_test_path(path, sizeof(path), "foreign") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open rename fk db");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
...
static int test_rename_index_independent_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first rename handle");
    failures +=
        expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second rename handle");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_rename_index_%d_%s.mylite",
        current_process_id(),
        name
    );
