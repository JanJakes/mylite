# Baseline INFORMATION_SCHEMA ROUTINES Tasks

## Goal

Add a queryable empty `INFORMATION_SCHEMA.ROUTINES` system view with MySQL
8.4.9 column metadata, without implementing stored routine DDL, descriptors,
parameters, bodies, privileges, or execution.

## Tasks

1. Design and documentation
   - Create `docs/specs/baseline-information-schema-routines/specs.md`.
...
2. MySQL expectations
   ...
   - Verify `ROUTINES` column labels, empty user-schema results, system
     `TABLES` metadata, system `COLUMNS` metadata, warning count, row-count
     behavior, and unknown predicate-column diagnostics.
   - Treat missing MySQL 8.4.9 runtime as a blocker.

3. Runtime metadata registry
   - Add `INFORMATION_SCHEMA.ROUTINES` to the synthetic table registry.
   - Add the 31 observed column definitions.
   - Ensure `INFORMATION_SCHEMA.TABLES` and `INFORMATION_SCHEMA.COLUMNS` emit
     system-view metadata for `ROUTINES`.
   - Emit no routine rows until routine descriptors are designed.
...
4. Tests
   ...
   - Cover wildcard and explicit projection, `COUNT(*)`, aliases, predicates,
     ...
     `COLUMNS` metadata, unknown predicate columns, independent handles, reopen
     safety, warning count, affected rows, and `ROW_COUNT()`.
...
storage, parameters, return metadata, persistence, privileges, definers, real
`INFORMATION_SCHEMA.ROUTINES` rows, `INFORMATION_SCHEMA.PARAMETERS` rows, and
SQLite function reflection.
