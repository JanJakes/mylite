...
| `CREATE INDEX` | ❌ | top | Standalone index creation over MySQL index types and attributes. | Parser recognizes non-empty functional/key-part lists, validates key-part prefix lengths and optional `ASC`/`DESC` tails, and recognizes index options for parser plugins, string-literal comments, visibility, string-literal engine attributes with quoted hex/bit literal rejection in text-string positions, numeric and quoted-hex `KEY_BLOCK_SIZE`, closed `USING`/`TYPE` values, and identifier-valued `ALGORITHM`/`LOCK` options. |
...
| `USE` | ❌ | top | Default schema selection in the embedded single-file model. | Parser recognizes one-part unquoted schema names using the shared identifier grammar and unreserved keyword names while rejecting string, numeric, variable, operator, and reserved-keyword targets. |
