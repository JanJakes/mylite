...
static int test_abs_values_and_file_safety(void);
static int test_abs_warnings_and_do(void);
static int test_abs_errors_and_unsupported_forms(void);
static int test_abs_independent_handles(void);
...
int main(void) {
    ...
    failures += test_abs_values_and_file_safety();
    failures += test_abs_warnings_and_do();
    failures += test_abs_errors_and_unsupported_forms();
    failures += test_abs_independent_handles();
...
static int test_abs_values_and_file_safety(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT ABS(NULL),ABS(0),ABS(1),ABS(-1),ABS(TRUE),ABS(FALSE),"
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "core abs values",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT ABS(5&3) AS a,ABS(~0) b,ABS(1<<63) c,"
                   "ABS(1<<64) d,ABS(5 DIV 2) e,ABS(IFNULL(NULL,-7)) f FROM DUAL",
    ...
    failures += mylite_test_expect_int64(
        ...
        "abs catalog generation unchanged"
    );
    failures += mylite_test_expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "abs sqlite schema generation unchanged"
    );
    failures += read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble));
    failures +=
        expect_bytes(actual_preamble, expected_preamble, sizeof(actual_preamble), "abs preamble");
...
static int test_abs_warnings_and_do(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open warning handle");
    failures += execute_ok(database, "DO 0", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT ABS(5 DIV 0),@@warning_count,ROW_COUNT()",
            ...
            .warning_count = 1U,
            .affected_rows = 0,
            .context = "abs select warning",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DO ABS(NULL),ABS(-1),ABS(18446744073709551615)",
            .columns = NULL,
            .column_count = 0U,
            .values = NULL,
            .row_count = 0U,
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "abs do count snapshot",
        }
    );

    failures += execute_ok(database, "DO ABS(5 DIV 0)", &result);
    if (result != NULL) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 0U, "do warning columns");
        failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "do warning rows");
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 1U, "do warning count");
        failures +=
            mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "do warning affected");
    }
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "abs do warning snapshot",
        }
    );
...
static int test_abs_errors_and_unsupported_forms(void) {
    ...
    failures += execute_error(
        database,
        "SELECT ABS(18446744073709551616)",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "SELECT CASE WHEN TRUE THEN ABS(7) END",
        (struct expected_sql_error){
            .code = mysql_error_parse,
            .sqlstate = "42000",
    ...
    failures += execute_ok(database, "DO ABS(@@warning_count)", NULL);
...
static int test_abs_independent_handles(void) {
    static const char *const first_columns[] = {"ABS(~0)"};
    static const char *const first_values[] = {"18446744073709551615"};
    static const char *const second_columns[] = {"ABS(-9223372036854775809)"};
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .warning_count = 0U,
            .affected_rows = 0,
            .context = "first handle abs",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            .sql = "SELECT ABS(-9223372036854775809)",
