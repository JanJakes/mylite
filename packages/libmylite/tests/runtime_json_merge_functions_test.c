...
static int test_literal_json_merge_values(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_column_count(result), 1U, "json_merge metadata");
...
static int test_warning_and_statement_status(void) {
    ...
    if (failures == 0) {
        failures +=
            mylite_test_expect_size(mylite_result_warning_count(result), 1U, "json_merge warning");
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "JSON_MERGE do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "JSON_MERGE do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "JSON_MERGE do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "JSON_MERGE do warnings"
        );
    }
    mylite_result_free(result);
    failures += expect_query(
...
static int test_table_backed_json_merge_values(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            1,
            "JSON_MERGE update affected rows"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "JSON_MERGE update warning count"
        );
    }
    mylite_result_free(result);
    result = NULL;
