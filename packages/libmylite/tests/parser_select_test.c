...
static int test_select_where_predicates(void) {
    ...
    failures += parser_test_parse_sql(
        "SELECT id FROM simple_lifecycle WHERE VERSION() = VERSION();",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    failures += parser_test_expect_node(
    ...
    failures += parser_test_expect_node(
        ...
        MYLITE_SQL_AST_VERSION_FUNCTION,
        "version predicate right value"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parser_test_parse_sql(
        "SELECT id FROM simple_lifecycle WHERE CONNECTION_ID() = CONNECTION_ID();",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    failures += parser_test_expect_node(
        ...
        "connection id predicate left value"
    );
    failures += parser_test_expect_node(
        parser_test_child_at(
            parser_test_child_at(
        ...
        MYLITE_SQL_AST_CONNECTION_ID_FUNCTION,
        "connection id predicate right value"
    );
    mylite_sql_parse_result_deinit(&result);
