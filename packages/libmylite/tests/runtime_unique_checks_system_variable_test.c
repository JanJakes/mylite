...
enum {
    ...
    unique_value_column_count = 6,
    unique_label_column_count = 5,
    unique_diagnostics_column_count = 4,
    unique_selected_column_count = 2,
...
static int test_unique_checks_values_and_persistence(void);
static int test_unique_checks_qualifiers_and_errors(void);
static int test_independent_unique_checks_handles(void);
...
int main(void) {
    ...
    failures += test_unique_checks_values_and_persistence();
    failures += test_unique_checks_qualifiers_and_errors();
    failures += test_independent_unique_checks_handles();
...
static int test_unique_checks_values_and_persistence(void) {
    ...
    static const char *const mixed_values[] = {"1", "1", "1", "1", "InnoDB", "utf8mb4", "MyLite"};
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open unique checks file");
    ...
    failures += execute_ok(
        ...
        "@@session.unique_checks, @@local.unique_checks, @@warning_count, "
        "ROW_COUNT() FROM DUAL",
        &result
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@UNIQUE_CHECKS, @@Global.Unique_Checks, "
        "@@session.`unique_checks`, @@`unique_checks`, (@@unique_checks)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = unique_label_column_count,
            .context = "unique checks labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@unique_checks, @@foreign_key_checks, @@sql_quote_show_create, @@autocommit, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed unique checks scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = unique_diagnostics_column_count,
            .context = "unique checks warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "unique checks clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = unique_diagnostics_column_count,
            .context = "unique checks error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "unique checks clears errors");
    failures += expect_show_count_warnings(database, "0", "unique checks clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by unique checks reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by unique checks reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read unique checks preamble"
    );
    failures += expect_bytes(
        actual_preamble,
        expected_preamble,
        sizeof(expected_preamble),
        "preamble after unique checks reads"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@unique_checks, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = unique_selected_column_count,
            .context = "unique checks with selected database",
        }
    );
    failures += expect_query_result(
        database,
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen unique checks file");
    failures += expect_query_result(
        database,
        "SELECT @@unique_checks, @@global.unique_checks",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened unique checks values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened unique checks table row",
        }
    );
...
static int test_unique_checks_qualifiers_and_errors(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open unique checks memory");
    failures += execute_ok(
        database,
        "SELECT @@UNIQUE_CHECKS, @@SESSION.UNIQUE_CHECKS, "
        "@@Local.Unique_Checks, @@global.`unique_checks`, "
        "(@@unique_checks)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = unique_label_column_count,
            .context = "unique checks qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_unique_checks_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_unique_checks_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_unique_checks_variable'",
...
static int test_independent_unique_checks_handles(void) {
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = unique_independent_column_count,
            .context = "first handle unique checks variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@unique_checks, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = unique_independent_column_count,
            .context = "second handle unique checks variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_unique_checks_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
