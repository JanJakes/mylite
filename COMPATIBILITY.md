...
| `WHERE` | 🟡 | Descriptor-backed boolean expressions over supported integer/`NULL` predicate atoms for filtered table `SELECT`, aggregate source filters, `DELETE`, and `UPDATE`, with keyword `NOT`, `AND`/`&&`, `OR`/`\|\|`, and parentheses following MySQL precedence; `SELECT` supports source-qualified references while `DELETE` and `UPDATE` remain unqualified; decimal integer or `TRUE`/`FALSE` comparison right operands and `BETWEEN` bounds; no `WHERE TRUE`, symbolic `!`, `XOR`, literal-left comparisons, `IS TRUE`, or general expression predicates. | [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
