...
| `%, MOD` | 🟡 | Specified for upcoming limited no-source/`DUAL` signed-64 scalar modulo projection with `%`, infix `MOD`, and `MOD(left, right)`; not implemented yet, and no table-backed expression support. | [operators](docs/compatibility/operators.md), [numeric and math functions](docs/compatibility/functions-numeric-math.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
