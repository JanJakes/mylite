...
| `latin1` | 🟡 | Limited scalar `CONVERT(value USING latin1)` for ASCII scalar values in no-source, `DUAL`, and `DO` contexts, with `latin1_swedish_ci` result metadata and scalar postfix `COLLATE latin1_swedish_ci` / `latin1_bin` metadata; no table/database/column charset admission, static catalog rows, `SET NAMES`, non-ASCII transcoding, storage conversion, or comparison semantics |
