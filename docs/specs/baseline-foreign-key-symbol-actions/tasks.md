...
- [x] Extend parser/AST support for optional FK index symbols and action
  clauses.
- [x] Store planned FK index symbols and `update_rule` / `delete_rule` values
  in create-time and alter-time FK plans.
- [x] Preserve existing descriptor authority and generated child-index naming
  rules while adding optional FK index-symbol naming.
- [x] Implement descriptor-driven `SHOW CREATE TABLE` rendering for `CASCADE`
  and `RESTRICT` actions.
- [x] Implement direct set-based `ON DELETE CASCADE` for supported parent
  `DELETE` plans without changing public affected-row counts.
- [x] Implement direct set-based `ON UPDATE CASCADE` for supported parent key
  update plans without changing public affected-row counts.
- [x] Keep existing `RESTRICT` / `NO ACTION` rejection behavior for parent
  writes with matching child rows.
- [x] Reject unsupported action values, duplicate action timings, recursive or
  self-referential cascade shapes, and unsafe FK-related DML deterministically.
- [x] Add runtime/parser tests for DDL, metadata, cascades, diagnostics,
