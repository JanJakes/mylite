...
static int test_numeric_dual_do_and_status(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open dual/do handle");
    failures += expect_query(
        database,
        (struct expected_query){
...
static int test_numeric_errors_and_unsupported_forms(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open error handle");
