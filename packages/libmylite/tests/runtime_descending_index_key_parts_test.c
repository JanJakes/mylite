...
static int test_descending_key_part_metadata_dml_and_persistence(void) {
    ...
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open metadata db");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen metadata db");
...
static int test_alter_table_add_descending_primary_key(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "alter_primary") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alter primary db");
...
static int test_descending_key_part_independent_handles(void) {
    ...
    failures += mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first db");
    failures +=
        mylite_test_expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second db");
