...
| `UUID()` | 🟡 | high | Return a Universal Unique Identifier (UUID) | Implemented for supported scalar expression call sites, including lowercase canonical version-1 UUID text, per-call dynamic evaluation, handle-local generated node and clock-sequence state, MySQL-compatible field metadata under connection charsets, `utf8mb3` charset/collation introspection, table projection/`WHERE`/`ORDER BY`, `UPDATE`, and `DELETE`. `UUID_SHORT()`, binary-log/replication safety warnings, exact native arity diagnostics, and insert expression paths remain deferred. See [UUID function spec](docs/specs/uuid-function/specs.md) and [scalar built-in functions spec](docs/specs/scalar-built-in-functions/specs.md). |
