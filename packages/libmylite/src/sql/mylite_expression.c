...
enum {
    ...
    MYLITE_TEMPORAL_COMPACT_TIME_DIGITS = 6,
    MYLITE_TEMPORAL_COMPACT_TIME_BUFFER_SIZE = MYLITE_TEMPORAL_COMPACT_TIME_DIGITS + 1,
    MYLITE_TEMPORAL_COMPACT_TIME_HOUR_OFFSET = 0,
    MYLITE_TEMPORAL_COMPACT_TIME_MINUTE_OFFSET = 2,
    MYLITE_TEMPORAL_COMPACT_TIME_SECOND_OFFSET = 4,
    MYLITE_TEMPORAL_COMPACT_TIME_COMPONENT_DIGITS = 2,
    ...
    MYLITE_TEMPORAL_MAX_TIME_HOUR = 838,
...
enum temporal_part_kind {
    ...
    TEMPORAL_PART_MONTH,
    TEMPORAL_PART_DAY,
    TEMPORAL_PART_DAYOFWEEK,
    TEMPORAL_PART_DAYOFYEAR,
    TEMPORAL_PART_QUARTER,
    TEMPORAL_PART_HOUR,
...
enum mylite_scalar_function_id {
    ...
    MYLITE_SCALAR_FUNCTION_UNIX_TIMESTAMP = 117,
    MYLITE_SCALAR_FUNCTION_DATE_FORMAT = 118,
    ...
    MYLITE_SCALAR_FUNCTION_DAYOFWEEK = 124,
    MYLITE_SCALAR_FUNCTION_DAYOFYEAR = 125,
    ...
    MYLITE_SCALAR_FUNCTION_GET_LOCK = 143,
    MYLITE_SCALAR_FUNCTION_RELEASE_LOCK = 144,
    ...
    MYLITE_SCALAR_FUNCTION_JSON_VALID = 151,
    MYLITE_SCALAR_FUNCTION_JSON_TYPE = 152,
    MYLITE_SCALAR_FUNCTION_JSON_QUOTE = 153,
...
enum json_expression_constant {
    json_literal_null_length = 4U,
    json_literal_true_length = 4U,
    json_literal_false_length = 5U,
};

enum regexp_expression_constant {
    regexp_like_min_arity = 2U,
    regexp_like_max_arity = 3U,
    regexp_instr_min_arity = 2U,
    regexp_instr_max_arity = 6U,
    regexp_substr_min_arity = 2U,
    regexp_substr_max_arity = 5U,
    regexp_replace_min_arity = 3U,
    regexp_replace_max_arity = 6U,
};
...
static int transcode_known_charset_text(
    enum char_function_charset source_charset,
    enum char_function_charset target_charset,
    const char *text,
    size_t text_length,
    char **out_text,
    size_t *out_length
...
static int transcode_latin1_to_utf8(
    const char *text,
    size_t text_length,
    char **out_text,
    size_t *out_length
...
static enum char_function_charset char_cast_target_charset(
    const struct mylite_sql_ast_node *target,
    const struct mylite_expression_eval_context *context
);

static enum char_function_charset expression_text_charset(
    enum mylite_expression_text_charset charset
);

static enum mylite_expression_text_charset expression_text_charset_from_char_function(
...
bool mylite_expression_is_supported_function_call(const struct mylite_sql_ast_node *expression) {
    ...
    case MYLITE_SCALAR_FUNCTION_REGEXP_LIKE:
        return arity == regexp_like_min_arity || arity == regexp_like_max_arity;
    case MYLITE_SCALAR_FUNCTION_REGEXP_INSTR:
        return arity >= regexp_instr_min_arity && arity <= regexp_instr_max_arity;
    case MYLITE_SCALAR_FUNCTION_REGEXP_SUBSTR:
        return arity >= regexp_substr_min_arity && arity <= regexp_substr_max_arity;
    case MYLITE_SCALAR_FUNCTION_REGEXP_REPLACE:
        return arity >= regexp_replace_min_arity && arity <= regexp_replace_max_arity;
    ...
    case MYLITE_SCALAR_FUNCTION_RELEASE_LOCK:
    case MYLITE_SCALAR_FUNCTION_IS_FREE_LOCK:
    case MYLITE_SCALAR_FUNCTION_IS_USED_LOCK:
    case MYLITE_SCALAR_FUNCTION_INET_ATON:
    case MYLITE_SCALAR_FUNCTION_INET_NTOA:
    case MYLITE_SCALAR_FUNCTION_IS_UUID:
    case MYLITE_SCALAR_FUNCTION_JSON_VALID:
    case MYLITE_SCALAR_FUNCTION_JSON_TYPE:
    case MYLITE_SCALAR_FUNCTION_JSON_QUOTE:
    ...
    case MYLITE_SCALAR_FUNCTION_MICROSECOND:
    ...
    case MYLITE_SCALAR_FUNCTION_GET_LOCK:
        return arity == 2U;
    ...
    case MYLITE_SCALAR_FUNCTION_CURRENT_ROLE:
...
static int eval_char_cast(
    ...
    status = transcode_char_cast_text(target, value, context, &text, &text_length);
    if (status != 0) {
        free(text);
        return status;
    }
...
static int transcode_char_cast_text(
    ...
    enum char_function_charset source_charset = CHAR_FUNCTION_CHARSET_UNKNOWN;
    enum char_function_charset target_charset = CHAR_FUNCTION_CHARSET_UNKNOWN;
    char *converted = NULL;
    size_t converted_length = 0U;
    int status = 0;

    if (value == NULL || text == NULL || text_length == NULL) {
        return -1;
    }
    source_charset = expression_text_charset(value->text_charset);
    target_charset = char_cast_target_charset(target, context);
    status = transcode_known_charset_text(
        source_charset,
        target_charset,
        *text,
        *text_length,
...
static int transcode_known_charset_text(
    ...
    *out_text = NULL;
    *out_length = 0U;
    if (source_charset == CHAR_FUNCTION_CHARSET_LATIN1 &&
        (target_charset == CHAR_FUNCTION_CHARSET_UTF8MB4 ||
         target_charset == CHAR_FUNCTION_CHARSET_UTF8MB3)) {
        return transcode_latin1_to_utf8(text, text_length, out_text, out_length);
    }
    if ((source_charset == CHAR_FUNCTION_CHARSET_UTF8MB4 ||
         source_charset == CHAR_FUNCTION_CHARSET_UTF8MB3) &&
...
static int transcode_latin1_to_utf8(
    ...
    if (text_length > SIZE_MAX / 2U - 1U) {
        return -1;
    }
    result = malloc((text_length * 2U) + 1U);
    if (result == NULL) {
        return -1;
    }
    for (size_t index = 0U; index < text_length; ++index) {
        unsigned char character = source[index];

        if (character <= MYLITE_ASCII_MAX) {
            result[result_length++] = (char)character;
        } else {
            result[result_length++] = (char)(0xC0U | (character >> 6U));
            result[result_length++] = (char)(MYLITE_UTF8_CONTINUATION_MARKER | (character & 0x3FU));
        }
    }
...
static bool utf8_codepoint_at(
    ...
    const unsigned char *source = (const unsigned char *)(text == NULL ? "" : text);
    size_t index = offset == NULL ? 0U : *offset;
    unsigned char first = index < text_length ? source[index] : 0U;
    struct utf8_sequence sequence = {0};
    uint32_t codepoint = 0U;

    if (offset == NULL || out_codepoint == NULL || index >= text_length) {
        return false;
    }
    ...
    if (!utf8_sequence_from_first(first, true, &sequence) ||
    ...
    case MYLITE_UTF8_TWO_BYTE_LENGTH:
        if (!utf8_continuation_byte(source[index + 1U])) {
            return false;
        }
        codepoint = ((uint32_t)(first & 0x1FU) << 6U) | (uint32_t)(source[index + 1U] & 0x3FU);
        break;
    case MYLITE_UTF8_THREE_BYTE_LENGTH:
        if (!utf8_continuation_byte(source[index + 1U]) ||
            !utf8_continuation_byte(source[index + 2U])) {
            return false;
        }
        codepoint = ((uint32_t)(first & 0x0FU) << 12U) |
                    ((uint32_t)(source[index + 1U] & 0x3FU) << 6U) |
                    (uint32_t)(source[index + 2U] & 0x3FU);
        break;
    case MYLITE_UTF8_FOUR_BYTE_LENGTH:
        if (!utf8_continuation_byte(source[index + 1U]) ||
            !utf8_continuation_byte(source[index + 2U]) ||
            !utf8_continuation_byte(source[index + 3U])) {
            return false;
        }
        codepoint =
            ((uint32_t)(first & 0x07U) << 18U) | ((uint32_t)(source[index + 1U] & 0x3FU) << 12U) |
            ((uint32_t)(source[index + 2U] & 0x3FU) << 6U) | (uint32_t)(source[index + 3U] & 0x3FU);
        break;
    default:
        return false;
    }
...
static enum char_function_charset expression_text_charset(
    ...
    switch (charset) {
    case MYLITE_EXPRESSION_TEXT_CHARSET_BINARY:
        return CHAR_FUNCTION_CHARSET_BINARY;
    case MYLITE_EXPRESSION_TEXT_CHARSET_LATIN1:
        return CHAR_FUNCTION_CHARSET_LATIN1;
    case MYLITE_EXPRESSION_TEXT_CHARSET_UTF8MB4:
        return CHAR_FUNCTION_CHARSET_UTF8MB4;
    case MYLITE_EXPRESSION_TEXT_CHARSET_UTF8MB3:
        return CHAR_FUNCTION_CHARSET_UTF8MB3;
    case MYLITE_EXPRESSION_TEXT_CHARSET_ASCII:
        return CHAR_FUNCTION_CHARSET_ASCII;
    case MYLITE_EXPRESSION_TEXT_CHARSET_UNKNOWN:
        break;
    }
    return CHAR_FUNCTION_CHARSET_UNKNOWN;
}
...
static enum mylite_expression_text_charset expression_text_charset_from_char_function(
    ...
    case CHAR_FUNCTION_CHARSET_ASCII:
        return MYLITE_EXPRESSION_TEXT_CHARSET_ASCII;
...
static int eval_function_call(
    ...
    case MYLITE_SCALAR_FUNCTION_LAST_DAY:
        return eval_last_day_function(arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_TO_DAYS:
    ...
    case MYLITE_SCALAR_FUNCTION_ADDTIME:
    case MYLITE_SCALAR_FUNCTION_SUBTIME:
        return eval_addsubtime_function(function_id, arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_TIMESTAMP:
    ...
    case MYLITE_SCALAR_FUNCTION_JSON_VALID:
        return eval_json_valid_function(arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_JSON_TYPE:
        return eval_json_type_function(arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_JSON_QUOTE:
        return eval_json_quote_function(arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_JSON_UNQUOTE:
        return eval_json_unquote_function(arguments, context, warnings, out_value);
    case MYLITE_SCALAR_FUNCTION_JSON_ARRAY:
...
static int eval_json_valid_function(
    ...
    *out_value = (struct mylite_expression_value){
        .kind = MYLITE_EXPRESSION_VALUE_INT64,
        .int64_value =
            argument.kind == MYLITE_EXPRESSION_VALUE_TEXT &&
                    mylite_json_validate(argument.text_value, argument.text_length, NULL, NULL)
                ? 1
                : 0,
    };
...
static int eval_json_type_function(
    ...
    if (argument.kind != MYLITE_EXPRESSION_VALUE_TEXT) {
        status = append_json_invalid_data_error(warnings, "json_type");
        status = status == 0 ? MYLITE_EXEC_ERROR : status;
        goto cleanup;
    }
    if (!mylite_json_validate(argument.text_value, argument.text_length, &type, &error)) {
        status = append_json_invalid_text_error(warnings, "json_type", &error);
        status = status == 0 ? MYLITE_EXEC_ERROR : status;
        goto cleanup;
    }
    type_name = mylite_json_type_name(type);
    status = type_name == NULL ? -1 : set_text_value(type_name, strlen(type_name), out_value);
...
static int eval_json_quote_function(
    ...
    if (argument.kind != MYLITE_EXPRESSION_VALUE_TEXT) {
        status = append_json_incorrect_type_error(warnings, "json_quote");
        status = status == 0 ? MYLITE_EXEC_ERROR : status;
        goto cleanup;
    }
...
static int eval_json_unquote_function(
    ...
    if (argument.kind != MYLITE_EXPRESSION_VALUE_TEXT) {
        status = append_json_incorrect_type_error(warnings, "json_unquote");
        status = status == 0 ? MYLITE_EXEC_ERROR : status;
        goto cleanup;
    }
...
static int eval_json_array_function(
    ...
    bool first = true;
    int status = append_text(&json, &json_length, "[", 1U);
    ...
    for (const struct mylite_sql_ast_node *argument = arguments == NULL ? NULL
        ...
        if (!first) {
            status = append_text(&json, &json_length, ", ", 2U);
        }
        if (status == 0) {
            status = json_append_argument(&json, &json_length, argument, &value);
        }
        first = false;
        mylite_expression_value_deinit(&value);
        if (status != 0) {
    ...
    if (status == 0) {
        status = append_text(&json, &json_length, "]", 1U);
    }
    if (status == 0) {
...
static int eval_json_object_function(
    ...
    char *json = NULL;
    size_t json_length = 0U;
    int status = eval_json_object_members(arguments, context, warnings, &members, &member_count);

    if (status == 0) {
        status = json_object_to_text(members, member_count, &json, &json_length);
    }
    if (status == 0) {
        status = set_text_value(json, json_length, out_value);
    }
...
static int eval_json_object_members(
    ...
    for (const struct mylite_sql_ast_node *key_node = arguments == NULL ? NULL
                                                                        : arguments->first_child;
         key_node != NULL;) {
        ...
        key_node = value_node == NULL ? NULL : value_node->next_sibling;
    }
    return 0;
}
...
static int eval_json_object_member(
    ...
    size_t value_json_length = 0U;
    int status = eval_node(key_node, context, warnings, &key);

    if (status != 0) {
        goto cleanup;
    }
    if (is_null(&key)) {
        status = append_json_null_key_error(warnings);
        status = status == 0 ? MYLITE_EXEC_ERROR : status;
        goto cleanup;
    }
    status = json_argument_to_text(&key, &key_text, &key_length);
    if (status != 0) {
        goto cleanup;
    }
    status = eval_node(value_node, context, warnings, &value);
    if (status == 0) {
        status = json_append_argument(&value_json, &value_json_length, value_node, &value);
    }
    if (status != 0) {
        goto cleanup;
    }
...
cleanup:
    mylite_expression_value_deinit(&key);
    mylite_expression_value_deinit(&value);
    free(key_text);
    free(value_json);
    return status;
...
static int json_object_to_text(
    ...
    qsort(members, member_count, sizeof(*members), json_object_member_compare);
    status = append_text(&json, &json_length, "{", 1U);
    for (size_t index = 0U; status == 0 && index < member_count; ++index) {
        if (index != 0U) {
            status = append_text(&json, &json_length, ", ", 2U);
        }
        if (status == 0) {
            status = append_json_object_member(&json, &json_length, &members[index]);
        }
    }
    if (status == 0) {
        status = append_text(&json, &json_length, "}", 1U);
...
static int append_json_object_member(
    ...
    int status =
        mylite_json_quote_string(member->key, member->key_length, &quoted_key, &quoted_key_length);

    if (status == 0) {
        status = append_text(json, json_length, quoted_key, quoted_key_length);
    }
    if (status == 0) {
        status = append_text(json, json_length, ": ", 2U);
    }
    if (status == 0) {
        status = append_text(json, json_length, member->value, member->value_length);
    }
    free(quoted_key);
    return status;
}
...
static int json_append_argument(
    ...
    if (is_null(value)) {
        return append_text(json, json_length, "null", json_literal_null_length);
    }
    if (json_argument_is_true_literal(argument)) {
        return append_text(json, json_length, "true", json_literal_true_length);
    }
    if (json_argument_is_false_literal(argument)) {
        return append_text(json, json_length, "false", json_literal_false_length);
    }
    status = json_argument_to_text(value, &text, &text_length);
    if (status != 0) {
        return status;
    }
    if (value->kind == MYLITE_EXPRESSION_VALUE_TEXT && !json_argument_is_json_creation(argument)) {
        char *quoted = NULL;
        size_t quoted_length = 0U;

        status = mylite_json_quote_string(text, text_length, &quoted, &quoted_length);
        if (status == 0) {
            status = append_text(json, json_length, quoted, quoted_length);
        }
        free(quoted);
    } else {
        status = append_text(json, json_length, text, text_length);
    }
    free(text);
    return status;
}
...
static int json_argument_to_text(
    ...
    if (value->kind == MYLITE_EXPRESSION_VALUE_TEXT) {
        text = copy_span_text(value->text_value, value->text_length);
        if (text == NULL) {
            return -1;
        }
        *out_text = text;
    ...
    if (value->kind == MYLITE_EXPRESSION_VALUE_REAL && value->text_value != NULL) {
        text = copy_span_text(value->text_value, value->text_length);
        if (text == NULL) {
            return -1;
        }
    ...
    if (value->kind == MYLITE_EXPRESSION_VALUE_REAL) {
        ...
        text = copy_span_text(buffer, (size_t)length);
        if (text == NULL) {
            return -1;
        }
        *out_text = text;
...
static bool json_argument_is_json_creation(const struct mylite_sql_ast_node *argument) {
    ...
    argument = unwrap_parenthesized_node(argument);
    if (argument == NULL || argument->kind != MYLITE_SQL_AST_FUNCTION_CALL) {
        return false;
    }
    function_id = scalar_function_id(argument);
    return function_id == MYLITE_SCALAR_FUNCTION_JSON_ARRAY ||
...
static int json_object_append_member(
    ...
    for (size_t index = 0U; index < *count; ++index) {
        if ((*members)[index].key_length == key_length &&
            ...
            (*members)[index].value_length = value_length;
            return 0;
        }
    }

    updated = realloc(*members, (*count + 1U) * sizeof(*updated));
    if (updated == NULL) {
        return -1;
    }
    *members = updated;
    (*members)[*count] = (struct json_object_member){
        .key = key,
...
static int eval_unix_timestamp_function(
    ...
    status = temporal_date_from_value(&argument, true, warnings, &date, &valid);
    if (status != 0) {
        goto cleanup;
    }
    if (!valid) {
        status = set_text_value("0.000000", strlen("0.000000"), out_value);
        goto cleanup;
    }
    status = set_unix_timestamp_value(&date, out_value);
...
static int set_unix_timestamp_value(
    ...
    static const struct temporal_date_value unix_timestamp_max = {
        .year = 3001,
        .month = 1,
        .day = 18,
        .hour = 23,
    ...
    char buffer[MYLITE_EXPRESSION_TEXT_BUFFER_SIZE];
    int64_t seconds = ((temporal_day_number(date) - temporal_day_number(&unix_epoch)) *
                       MYLITE_TEMPORAL_SECONDS_PER_DAY) +
                      ((int64_t)date->hour * MYLITE_TEMPORAL_SECONDS_PER_HOUR) +
                      ((int64_t)date->minute * MYLITE_TEMPORAL_SECONDS_PER_MINUTE) +
                      (int64_t)date->second;
    int64_t max_seconds =
        ((temporal_day_number(&unix_timestamp_max) - temporal_day_number(&unix_epoch)) *
         MYLITE_TEMPORAL_SECONDS_PER_DAY) +
        ((int64_t)unix_timestamp_max.hour * MYLITE_TEMPORAL_SECONDS_PER_HOUR) +
        ((int64_t)unix_timestamp_max.minute * MYLITE_TEMPORAL_SECONDS_PER_MINUTE) +
        (int64_t)unix_timestamp_max.second;
    int length = 0;
    ...
    length = snprintf(buffer, sizeof(buffer), "%lld.%06d", (long long)seconds, date->microsecond);
    if (length <= 0 || (size_t)length >= sizeof(buffer)) {
        return -1;
    }
    return set_text_value(buffer, (size_t)length, out_value);
...
static int eval_date_format_function(
    ...
    bool valid = false;
    int status = eval_node(child_at(arguments, 0U), context, warnings, &date_value);
    ...
    if (is_null(&date_value) || is_null(&format_value)) {
        *out_value = (struct mylite_expression_value){.kind = MYLITE_EXPRESSION_VALUE_NULL};
        goto cleanup;
    }
    status = temporal_date_from_value(&date_value, true, warnings, &date, &valid);
    if (status != 0) {
        goto cleanup;
    }
    if (!valid) {
...
static int date_format_text_value(
    ...
    for (size_t offset = 0U; offset < format_length; ++offset) {
        ...
        if (character != '%') {
            ...
            status = append_date_format_token(&result, &result_length, date, format[++offset]);
        }
        if (status != 0) {
            free(result);
            return status;
...
static int append_date_format_token(
    ...
    case 'D':
        return append_date_format_day_ordinal(result, length, date->day);
    case 'd':
        return append_formatted_date_part(result, length, "%02d", date->day);
    case 'e':
        return append_formatted_date_part(result, length, "%d", date->day);
    case 'f':
        return append_formatted_date_part(result, length, "%06d", date->microsecond);
    case 'H':
        return append_formatted_date_part(result, length, "%02d", date->hour);
    case 'h':
    case 'I':
        return append_formatted_date_part(result, length, "%02d", date_format_hour_12(date->hour));
    case 'i':
        return append_formatted_date_part(result, length, "%02d", date->minute);
    case 'j':
        return append_formatted_date_part(result, length, "%03d", date_format_day_of_year(date));
    case 'k':
        return append_formatted_date_part(result, length, "%d", date->hour);
    case 'l':
    ...
    case 'p':
        return append_text(result, length, date->hour < 12 ? "AM" : "PM", 2U);
    case 'r':
        return append_date_format_time_12(result, length, date);
    case 'S':
    case 's':
        return append_formatted_date_part(result, length, "%02d", date->second);
    case 'T':
    ...
    case 'v':
        return append_date_format_week_year(result, length, date, true, false);
    case 'W':
        return append_text(result, length, weekday_names[weekday], strlen(weekday_names[weekday]));
    case 'w':
        return append_formatted_date_part(result, length, "%d", weekday);
    case 'X':
        return append_date_format_week_year(result, length, date, false, true);
    case 'x':
        return append_date_format_week_year(result, length, date, true, true);
    case 'Y':
        return append_formatted_date_part(result, length, "%04d", date->year);
    case 'y':
        return append_formatted_date_part(result, length, "%02d", date->year % 100);
    case '%':
...
static int eval_last_day_function(
    ...
    valid = parse_temporal_date_source(&source, true, true, &date);
    if (!valid && date.warning_kind == TEMPORAL_DATE_WARNING_NONE) {
    ...
    if (!valid || date.month == 0) {
        if (valid) {
            status = append_last_day_zero_month_warning(warnings, &source, &date);
            if (status != 0) {
                goto cleanup;
            }
        }
    ...
    date.day = temporal_month_day_limit(date.year, date.month);
    status = set_temporal_date_text_value(&date, out_value);
...
static int eval_addsubtime_function(
    ...
    status = addsubtime_interval_from_expression(&right_value, warnings, &interval, &right_valid);
    if (status != 0) {
        goto cleanup;
    }
    if (!right_valid) {
    ...
    interval = addsubtime_effective_interval(function_id, &interval);
    status = left.kind == TIMEDIFF_OPERAND_DATETIME
                 ? addsubtime_datetime_result(left.date, &interval, warnings, out_value)
                 : addsubtime_time_result(&left.time, &interval, warnings, out_value);
...
static int addsubtime_interval_from_expression(
    ...
    *out_valid = false;
    ...
    if (value->temporal_type == MYLITE_EXPRESSION_TEMPORAL_NONE &&
        addsubtime_untyped_datetime_interval(value)) {
        return 0;
    }
...
static bool addsubtime_untyped_datetime_interval(const struct mylite_expression_value *value) {
    ...
    return temporal_date_source_from_value(value, &source) == 0 &&
...
static int addsubtime_datetime_result(
    ...
    if (!timestamp_add_time(&date, interval, &upper_overflow)) {
        if (upper_overflow) {
...
static struct temporal_time_value addsubtime_effective_interval(
    ...
    if (function_id == MYLITE_SCALAR_FUNCTION_SUBTIME) {
        effective.negative = !effective.negative;
    }
    return effective;
}
...
static int time_value_from_expression(
    ...
    *out_valid = false;
    ...
    if (value->temporal_type == MYLITE_EXPRESSION_TEMPORAL_TIME ||
        value->temporal_type == MYLITE_EXPRESSION_TEMPORAL_DATETIME ||
        value->temporal_type == MYLITE_EXPRESSION_TEMPORAL_TIMESTAMP) {
        return time_value_from_temporal_text(value, warnings, out_time, out_valid);
    }
...
static int time_value_from_temporal_text(
    ...
    if (value->temporal_type == MYLITE_EXPRESSION_TEMPORAL_TIME) {
        ...
        size_t length = value->text_value == NULL ? 0U : value->text_length;

        parsed = parse_temporal_time_text(text, length, false);
        if (parsed.warn) {
            int status = append_temporal_time_warning(warnings, text, length);

            if (status != 0) {
                return status;
            }
        }
        *out_time = parsed.time;
        *out_valid = parsed.valid;
        return 0;
    }

    int status = temporal_date_from_value(value, true, warnings, &date, &date_valid);
    ...
    *out_time = (struct temporal_time_value){
        ...
        .fraction_digits = date.fraction_digits,
    };
    *out_valid = true;
    return 0;
}
...
static int time_value_from_untyped_value(
    ...
    if (value->kind == MYLITE_EXPRESSION_VALUE_REAL) {
        if (real_text_is_approximate(value)) {
...
static int time_value_from_approximate_real(
    ...
    if ((size_t)parse_length >= sizeof(parse_buffer)) {
        *out_time = (struct temporal_time_value){0};
        *out_valid = false;
...
static bool real_text_is_approximate(const struct mylite_expression_value *value) {
    ...
    for (size_t index = 0U; index < value->text_length; ++index) {
        if (value->text_value[index] == 'e' || value->text_value[index] == 'E') {
            return true;
        }
    }
    return false;
...
static int time_value_from_untyped_text_with_warning(
    ...
    if (out_time == NULL || out_valid == NULL) {
        return -1;
    }
    if (parse_temporal_date_source(&source, true, false, &date) && date.has_time) {
        if (date.warning_kind != TEMPORAL_DATE_WARNING_NONE) {
            int status = append_temporal_time_warning(warnings, warning_text, warning_length);
    ...
    parsed = parse_temporal_time_text(text, length, numeric);
    if (parsed.warn) {
        int status = append_temporal_time_warning(warnings, warning_text, warning_length);
...
static bool temporal_part_from_function(
    ...
    case MYLITE_SCALAR_FUNCTION_DAY:
        ...
        break;
    case MYLITE_SCALAR_FUNCTION_DAYOFWEEK:
        part = TEMPORAL_PART_DAYOFWEEK;
        break;
    case MYLITE_SCALAR_FUNCTION_DAYOFYEAR:
        part = TEMPORAL_PART_DAYOFYEAR;
        break;
    case MYLITE_SCALAR_FUNCTION_QUARTER:
...
static int temporal_part_value(
    ...
    case TEMPORAL_PART_DAYOFWEEK:
        return date == NULL ? 0 : date_format_weekday_sunday(date) + 1;
    case TEMPORAL_PART_DAYOFYEAR:
        return date == NULL ? 0 : date_format_day_of_year(date);
    case TEMPORAL_PART_QUARTER:
        return date == NULL || date->month == 0 ? 0 : ((date->month + 2) / 3);
    ...
    case TEMPORAL_PART_MICROSECOND:
        return date == NULL ? 0 : date->microsecond;
...
static int set_temporal_time_text_value(
    ...
    if (fraction_digits != 0U) {
        char fraction[temporal_time_fraction_text_length + 1U];
        int fraction_length =
            snprintf(fraction, sizeof(fraction), "%06d", time == NULL ? 0 : time->microsecond);
    ...
    if (offset < length && text[offset] == '.') {
        ...
        if (microsecond >= MYLITE_TEMPORAL_MICROSECOND_LIMIT) {
            ...
            if (!add_temporal_seconds(date, 1)) {
                date->warning_kind = TEMPORAL_DATE_WARNING_DATETIME_OVERFLOW;
                return false;
            }
        }
...
static struct temporal_time_parse_result parse_temporal_time_text(
    ...
    for (size_t index = offset; index < length; ++index) {
        if (text[index] == ':') {
            if (!parse_temporal_colon_time(text, length, offset, negative, &result)) {
                result.warn = true;
            }
            normalize_temporal_time_range(&result);
            return result;
        }
    }
    if (!parse_temporal_digit_time(text, length, offset, negative, numeric, &result)) {
        result.warn = true;
    }
    normalize_temporal_time_range(&result);
    return result;
...
static bool parse_temporal_colon_time(
    ...
    if (offset < length && text[offset] == '.') {
        if (!parse_temporal_time_fraction_part(text, length, &offset, &time)) {
            warned = true;
        }
    }
    if (offset < length) {
    ...
    if (time.minute > MYLITE_TEMPORAL_MAX_MINUTE_SECOND ||
        time.second > MYLITE_TEMPORAL_MAX_MINUTE_SECOND) {
        return false;
    }
    result->time = time;
    result->valid = true;
...
static bool parse_temporal_digit_time(
    ...
    digit_length = offset - digit_start;
    if (digit_length == 0U) {
        return false;
    }
    if (parse_temporal_compact_datetime_time_value(text + digit_start, digit_length, &time)) {
        time.negative = negative;
        return parse_temporal_digit_time_tail(text, length, &offset, &time, result);
    }
    if (digit_length > MYLITE_TEMPORAL_COMPACT_TIME_DIGITS &&
        (numeric || digit_length != MYLITE_TEMPORAL_LONG_DATE_DIGITS)) {
        return false;
    }
    if (digit_length <= MYLITE_TEMPORAL_COMPACT_TIME_DIGITS) {
        if (!parse_temporal_compact_digit_time(text, digit_start, digit_length, &time)) {
            return false;
        }
    } else {
        if (!parse_temporal_long_digit_time(text, digit_start, digit_length, &time)) {
            return false;
        }
    }
    return parse_temporal_digit_time_tail(text, length, &offset, &time, result);
...
static bool parse_temporal_compact_datetime_time_value(
    ...
    if (!parse_temporal_compact_date(text, length, false, false, &date) || !date.has_time) {
        return false;
    }
    *out_time = (struct temporal_time_value){
        .hour = date.hour,
...
static bool parse_temporal_time_fraction_part(
    ...
    if (time->microsecond >= MYLITE_TEMPORAL_MICROSECOND_LIMIT) {
        time->microsecond = 0;
        (void)temporal_time_add_second(time);
    }
    return true;
}
...
static void normalize_temporal_time_range(struct temporal_time_parse_result *result) {
    ...
    if (temporal_time_is_out_of_range(&result->time)) {
        clip_temporal_time(&result->time);
        result->warn = true;
    }
}
...
static void clip_temporal_time(struct temporal_time_value *time) {
    ...
    time->hour = MYLITE_TEMPORAL_MAX_TIME_HOUR;
    time->minute = MYLITE_TEMPORAL_MAX_MINUTE_SECOND;
    time->second = MYLITE_TEMPORAL_MAX_MINUTE_SECOND;
    time->microsecond = 0;
}
...
static int set_temporal_date_text_value(
    ...
    int status = set_text_value(buffer, temporal_date_text_length, out_value);
...
static int set_char_result(
    ...
    int status = set_text_value(text == NULL ? "" : text, text_length, out_value);

    if (status == 0) {
        out_value->text_charset = expression_text_charset_from_char_function(charset);
    }
    return status;
}
...
static char *copy_unary_real_text(
    ...
    if (operator_kind == MYLITE_SQL_AST_OPERATOR_NEGATIVE) {
        return copy_negated_real_text(text, text_length, out_length);
    }
...
static char *copy_negated_real_text(const char *text, size_t text_length, size_t *out_length) {
    ...
    if (text_length != 0U && text[0] == '-') {
        *out_length = text_length - 1U;
        return copy_span_text(text + 1U, *out_length);
    }
    if (text_length != 0U && text[0] == '+') {
        magnitude_offset = 1U;
    }
...
static char *copy_prefixed_text(
    ...
    *out_length = text_length + 1U;
    copy = malloc(*out_length + 1U);
    if (copy == NULL) {
        return NULL;
    }
    copy[0] = prefix;
    memcpy(copy + 1U, text, text_length);
    copy[*out_length] = '\0';
    return copy;
}
...
static enum mylite_scalar_function_id scalar_function_id_from_span(
    ...
    static const struct {
        ...
        {"JSON_VALID", MYLITE_SCALAR_FUNCTION_JSON_VALID},
        {"JSON_TYPE", MYLITE_SCALAR_FUNCTION_JSON_TYPE},
        {"JSON_QUOTE", MYLITE_SCALAR_FUNCTION_JSON_QUOTE},
        ...
        {"CURRENT_ROLE", MYLITE_SCALAR_FUNCTION_CURRENT_ROLE},
        ...
        {"LAST_DAY", MYLITE_SCALAR_FUNCTION_LAST_DAY},
        ...
        {"ADDTIME", MYLITE_SCALAR_FUNCTION_ADDTIME},
        {"SUBTIME", MYLITE_SCALAR_FUNCTION_SUBTIME},
        ...
        {"MICROSECOND", MYLITE_SCALAR_FUNCTION_MICROSECOND},
        ...
        {"GET_LOCK", MYLITE_SCALAR_FUNCTION_GET_LOCK},
        {"RELEASE_LOCK", MYLITE_SCALAR_FUNCTION_RELEASE_LOCK},
        {"IS_FREE_LOCK", MYLITE_SCALAR_FUNCTION_IS_FREE_LOCK},
        {"IS_USED_LOCK", MYLITE_SCALAR_FUNCTION_IS_USED_LOCK},
        {"RELEASE_ALL_LOCKS", MYLITE_SCALAR_FUNCTION_RELEASE_ALL_LOCKS},
