...
admits recognized well-formed operator expressions in otherwise failed `SET`
assignments as explicit unsupported placeholders. This covers MySQL-valid
arithmetic, bitwise, logical, and unary operator syntax after normal parsing
fails, while preserving existing executable `SET` assignment paths and syntax
errors for incomplete operator tails. See [parser corpus SET and DML expression
