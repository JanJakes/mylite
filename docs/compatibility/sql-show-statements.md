...
| `SHOW GRANTS` | 🟡 | Limited current-user forms `SHOW GRANTS`, `SHOW GRANTS FOR CURRENT_USER`, `SHOW GRANTS FOR CURRENT_USER()`, and simple named `SHOW GRANTS FOR 'root'@'%'` return two synthetic MySQL 8.4.9-shaped global grant rows for MyLite's embedded `root@%` identity; other simple named accounts return `1141 / 42000`; no roles, `USING`, account storage, grant descriptors, privilege enforcement, filters, ordering, or limits |
