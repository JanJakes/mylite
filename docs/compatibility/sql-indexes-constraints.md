...
| `ADD PRIMARY KEY` | 🟡 | Limited `ALTER TABLE table_name ADD PRIMARY KEY (column_name)` for persistent base tables and one existing unqualified integer-family descriptor column; validates existing rows for `NULL` with `1138 / 22004`, validates duplicate values with `1062 / 23000`, reports zero affected rows and warnings, and updates descriptor/physical index metadata atomically. No named constraints, composite or string keys, key options, multi-action `ALTER`, generated invisible keys, or auto-increment conversion |
