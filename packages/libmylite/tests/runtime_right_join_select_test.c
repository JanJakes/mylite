...
static int test_right_join_success_persistence_and_table_lifecycle(void) {
    ...
    database = NULL;
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen right join file");
...
static int test_independent_file_backed_right_join_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first right join file"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second right join file"
    );
