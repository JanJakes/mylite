...
| `COUNT()` | 🟡 | Limited `COUNT(*)` and `COUNT(integer/NULL/boolean literal)` in one-item `SELECT` with no source, `FROM DUAL`, or one descriptor-backed persistent base table, plus limited `COUNT(column)` over one descriptor-backed persistent base table; optional baseline `WHERE` is supported for table-backed forms; no general `COUNT(expr)`, `DISTINCT`, grouping, aliases, ordering, limiting, or window forms. | [aggregate functions](docs/compatibility/functions-aggregate.md) |
