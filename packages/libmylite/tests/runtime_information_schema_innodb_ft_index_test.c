...
static int test_information_schema_innodb_ft_index_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb ft index db");
...
static int test_information_schema_innodb_ft_index_reopen_preamble_and_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first ft index db"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second ft index db"
    );
    failures += expect_query(
        first,
        (struct expected_query){
