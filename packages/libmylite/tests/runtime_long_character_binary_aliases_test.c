...
static int test_alias_success_persistence_and_introspection(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alias success file");
...
static int test_alias_independent_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first alias file");
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second alias file"
    );
...
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected) {
    ...
    failures += mylite_test_expect_true(result == NULL, "failed execution result");
    mylite_result_free(result);
    return failures;
}
...
static int expect_alias_result_metadata(mylite_db *database) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "alias metadata row count");
...
static int expect_column_metadata(
    ...
    failures +=
        mylite_test_expect_uint16(mylite_result_column_decimals(result, column_index), 0U, context);
    failures += mylite_test_expect_int(
