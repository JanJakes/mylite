...
| `CURDATE()` | 🟡 | Limited zero-fractional statement date in no-source, `FROM DUAL`, `DO`, row-scalar projection, and compatible `DATE` DML assignment; respects the limited session `timestamp` override, uses fixed UTC formatting, and has no mutable time zones, expression defaults, or general temporal expression evaluation |
| `CURRENT_DATE(), CURRENT_DATE` | 🟡 | Limited zero-fractional statement date synonyms for `CURDATE()` in the same supported positions |
| `CURRENT_TIME(), CURRENT_TIME` | 🟡 | Limited zero-fractional statement time synonyms for `CURTIME()` in no-source, `FROM DUAL`, `DO`, row-scalar projection, and compatible `TIME` DML assignment; no fractional precision, mutable time zones, expression defaults, or general temporal expression evaluation |
...
| `CURTIME()` | 🟡 | Limited zero-fractional statement time in the same supported positions as `CURRENT_TIME`; respects the limited session `timestamp` override and uses fixed UTC formatting |
