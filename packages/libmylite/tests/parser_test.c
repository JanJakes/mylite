...
static int expect_placeholder_statement(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_placeholder_statement_kind expected_kind,
    const char *context
);
...
static int expect_function_call(
    const struct mylite_sql_ast_node *node,
...
static int expect_interval_function_call(
    const struct mylite_sql_ast_node *node,
    const char *expected_name,
...
static int expect_aggregate_call(
    const struct mylite_sql_ast_node *node,
...
static int expect_join_type(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_join_type expected,
    const char *context
);

static int expect_join_condition_type(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_join_condition_type expected,
    const char *context
);

static int expect_select_duplicate_mode(
    const struct mylite_sql_ast_node *node,
...
static int expect_union_duplicate_mode(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_set_duplicate_mode expected,
    const char *context
);

static int expect_set_operation(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_set_operation expected,
    const char *context
);

static int expect_subquery_quantifier(
    const struct mylite_sql_ast_node *node,
...
static int expect_schema_option(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_schema_option expected,
    const char *context
);

static int expect_column_type(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_column_type expected,
    const char *context
);

static int expect_column_attribute(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_column_attribute expected,
    const char *context
);

static int expect_column_format(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_column_format expected,
    const char *context
);

static int expect_column_storage(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_column_storage expected,
    const char *context
);

static int expect_key_part_order(
    const struct mylite_sql_ast_node *node,
...
static int expect_group_item_direction(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_key_part_order expected,
    const char *context
);

static int expect_limit_bound(
    const struct mylite_sql_ast_node *node,
    uint64_t expected,
    const char *context
);

static int expect_index_algorithm(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_index_algorithm expected,
    const char *context
);

static int expect_index_option(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_index_option expected,
    const char *context
);

static int expect_index_class(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_index_class expected,
    const char *context
);

static int expect_ddl_table_option(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_ddl_table_option expected,
    const char *context
);

static int expect_alter_table_action(
    const struct mylite_sql_ast_node *node,
...
static int expect_alter_table_column_position(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_alter_table_column_position expected,
    const char *context
);

static int expect_table_option(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_table_option expected,
    const char *context
);

static int expect_transaction_access_mode(
    const struct mylite_sql_ast_node *node,
    enum mylite_sql_ast_transaction_access_mode expected,
    const char *context
);

static int expect_transaction_completion(
    const struct mylite_sql_ast_node *node,
...
static int expect_current_timestamp(
    const struct mylite_sql_ast_node *node,
    bool has_precision,
    uint64_t precision,
    const char *context
...
static int test_schema_lifecycle_statements(void) {
    ...
    failures += parse_sql(
        "CREATE DATABASE IF NOT EXISTS mylite_app DEFAULT CHARACTER SET utf8mb4 "
    ...
    failures += expect_schema_option(
        child_at(options, 0U),
        MYLITE_SQL_AST_SCHEMA_OPTION_CHARACTER_SET,
        "create charset option"
    );
    failures += expect_schema_option(
        child_at(options, 1U),
        MYLITE_SQL_AST_SCHEMA_OPTION_COLLATE,
        "create collate option"
    );
    failures += expect_schema_option(
        child_at(options, 2U),
        MYLITE_SQL_AST_SCHEMA_OPTION_ENCRYPTION,
    ...
    failures += expect_literal(
        child_at(child_at(options, 0U), 0U),
        MYLITE_SQL_AST_LITERAL_STRING,
        "quoted charset value"
    );
    failures += expect_literal(
        child_at(child_at(options, 1U), 0U),
        MYLITE_SQL_AST_LITERAL_STRING,
    ...
    failures += parse_sql(
        "ALTER SCHEMA DEFAULT COLLATE utf8mb4_0900_ai_ci READ ONLY = 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += expect_schema_option(
        child_at(options, 1U),
        MYLITE_SQL_AST_SCHEMA_OPTION_READ_ONLY,
...
static int test_connection_charset_statements(void) {
    ...
    failures += expect_node(
        child_at(child_at(result.root, 1U), 0U),
        MYLITE_SQL_AST_DEFAULT,
        "set names default"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SET SESSION sql_mode = 'ANSI_QUOTES'; SET LOCAL @@session.sql_mode = "
    ...
    failures += expect_child_count(
        result.root,
        wp_priority_variable_statement_count,
...
static int test_create_table_integer_boolean_columns(void) {
    ...
    failures += expect_column_type(
        child_at(child_at(columns, integer_alias_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_INT,
        "integer alias column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, bigint_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BIGINT,
        "bigint column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, bool_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BOOL,
        "bool column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, boolean_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BOOLEAN,
        "boolean column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, int1_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_TINYINT,
        "int1 alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, int2_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_SMALLINT,
        "int2 alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, int3_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_MEDIUMINT,
        "int3 alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, middleint_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_MEDIUMINT,
        "middleint alias"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE signedness (a INT SIGNED UNSIGNED, "
    ...
    failures += expect_column_type(
        child_at(child_at(columns, int4_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_INT,
        "int4 alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, int8_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BIGINT,
        "int8 alias"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE `schema`.`widths` (`select` INT(0), `from` INT(255));",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_bool_width (a BOOL(1));",
    ...
    failures += parse_sql(
        "CREATE TABLE bad_bool_signed (a BOOL SIGNED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_bool_unsigned (a BOOL UNSIGNED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_boolean_width (a BOOLEAN(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_boolean_signed (a BOOLEAN SIGNED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_bool_unsigned (a BOOLEAN UNSIGNED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_int_width (a INT(256));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_int_negative (a INT(-1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_create_table_string_binary_columns(void) {
    ...
    failures += expect_column_type(
        child_at(child_at(columns, binary_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BINARY,
        "binary column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, varbinary_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_VARBINARY,
        "varbinary column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, tinytext_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_TINYTEXT,
        "tinytext column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, text_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_TEXT,
        "text column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, longtext_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_LONGTEXT,
        "longtext column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, blob_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_BLOB,
        "blob column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, longblob_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_LONGBLOB,
        "longblob column"
    );
    ...
    failures += expect_column_type(
        column_type,
        MYLITE_SQL_AST_COLUMN_TYPE_VARCHAR,
    ...
    failures += expect_column_type(
        column_type,
        MYLITE_SQL_AST_COLUMN_TYPE_CHAR,
    ...
    failures += parse_sql(
        "CREATE TABLE unicode_ci_string ("
    ...
    failures += parse_sql(
        "CREATE TABLE bad_char_length (a CHAR(256));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_binary_length (a BINARY(256));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varchar_missing (a VARCHAR);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varchar_length (a VARCHAR(16384));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varbinary_missing (a VARBINARY);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varbinary_length (a VARBINARY(65536));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_tinytext_length (a TINYTEXT(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_longtext_length (a LONGTEXT(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_blob_charset (a BLOB CHARACTER SET utf8mb4);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_mediumblob_length (a MEDIUMBLOB(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_long_varchar_length (a LONG VARCHAR(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_long_varbinary_length (a LONG VARBINARY(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varchar_byte_missing (a VARCHAR BYTE);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_text_length (a TEXT(4294967296));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_blob_length (a BLOB(4294967296));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_varchar_length_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_text_length_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_blob_length_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_collation (a CHAR CHARACTER SET utf8mb4 "
...
static int test_create_table_bit_columns(void) {
    enum {
        expected_column_count = 3,
        bit_default_column = 0,
        bit_width_column = 1,
        bit_name_column = 2,
        explicit_bit_width = 8,
    };
    struct mylite_sql_parse_result result;
    const struct mylite_sql_ast_node *columns = NULL;
    const struct mylite_sql_ast_node *column_type = NULL;
    ...
    failures += parse_sql(
        "CREATE TABLE app.bit_types (a BIT, b BIT(8), bit INT);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    columns = child_at(child_at(result.root, 0U), 1U);
    failures += expect_node(columns, MYLITE_SQL_AST_COLUMN_DEFINITION_LIST, "bit columns");
    failures += expect_child_count(columns, expected_column_count, "bit column count");

    column_type = child_at(child_at(columns, bit_default_column), 1U);
    failures += expect_column_type(column_type, MYLITE_SQL_AST_COLUMN_TYPE_BIT, "bit column");
    failures += expect_span_text(column_type, "BIT", "bit span");
    if (column_type != NULL && column_type->has_column_length) {
        fprintf(stderr, "bare bit unexpectedly recorded a length\n");
        failures = 1;
    }

    column_type = child_at(child_at(columns, bit_width_column), 1U);
    failures += expect_column_type(column_type, MYLITE_SQL_AST_COLUMN_TYPE_BIT, "bit width column");
    failures += expect_span_text(column_type, "BIT(8)", "bit width span");
    if (column_type != NULL && (!column_type->has_column_length ||
                                column_type->column_length != (uint64_t)explicit_bit_width)) {
        fprintf(stderr, "bit width was not recorded as 8\n");
        failures = 1;
    }

    failures +=
        expect_span_text(child_at(child_at(columns, bit_name_column), 0U), "bit", "bit identifier");
    failures += expect_column_type(
        child_at(child_at(columns, bit_name_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_INT,
        "bit identifier type"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_bit_sixty_five (a BIT(65));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_create_table_numeric_columns(void) {
    ...
    failures += expect_column_type(
        column_type,
        MYLITE_SQL_AST_COLUMN_TYPE_DECIMAL,
    ...
    failures += expect_column_type(
        child_at(child_at(columns, numeric_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DECIMAL,
        "numeric alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, fixed_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DECIMAL,
        "fixed alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, float_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_FLOAT,
        "float column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, float_selector_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_FLOAT,
        "float selector column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, float_scaled_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_FLOAT,
        "float scaled column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, double_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DOUBLE,
        "double column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, double_precision_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DOUBLE,
        "double precision column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, real_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DOUBLE,
        "real column"
    );
    failures += expect_column_type(
        child_at(child_at(columns, float4_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_FLOAT,
        "float4 alias"
    );
    failures += expect_column_type(
        child_at(child_at(columns, float8_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_DOUBLE,
        "float8 alias"
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_precision (a DECIMAL(66));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_scale (a DECIMAL(65,31));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_scale_gt_precision (a DECIMAL(10,11));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_negative (a DECIMAL(-1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_missing_scale (a DECIMAL(10,));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_missing_precision (a DECIMAL(,2));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_precision_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_decimal_zero_scale (a DECIMAL(0,1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_float_precision (a FLOAT(54));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_float_zero_display (a FLOAT(0,0));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_float_zero_scale (a FLOAT(0,1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_float_display (a FLOAT(256,30));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_float_scale (a FLOAT(10,31));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_double_precision_only (a DOUBLE(10));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_double_zero_display (a DOUBLE(0,0));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_double_zero_scale (a DOUBLE(0,1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_double_precision_only_zerofill "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_double_overflow "
...
static int test_create_table_temporal_columns(void) {
    ...
    failures += expect_column_type(
        ...
        MYLITE_SQL_AST_COLUMN_TYPE_TIME,
        "time column"
    );
    ...
    failures += expect_column_type(
        ...
        MYLITE_SQL_AST_COLUMN_TYPE_DATETIME,
        "datetime column"
    );
    ...
    failures += expect_column_type(
        child_at(child_at(columns, timestamp_column), 1U),
        MYLITE_SQL_AST_COLUMN_TYPE_TIMESTAMP,
        "timestamp column"
    );
    ...
    failures += expect_column_type(
        child_at(child_at(columns, year_column), 1U),
    ...
    failures += parse_sql(
        "CREATE TABLE temporal_keyword_names ("
    ...
    failures += parse_sql(
        "CREATE TABLE bad_date_fsp_one (a DATE(1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_datetime_fsp (a DATETIME(7));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_timestamp_fsp (a TIMESTAMP(7));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_negative (a TIME(-1));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_empty_fsp (a TIME());",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_string_fsp (a TIME('1'));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_scale (a TIME(1,2));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_datetime_scale (a DATETIME(1,2));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_overflow_scale "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_datetime_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_timestamp_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_year_zero (a YEAR(0));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_year_three (a YEAR(3));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_year_five (a YEAR(5));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_year_overflow "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_time_unsigned (a TIME UNSIGNED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_create_table_column_attributes(void) {
    ...
    failures += expect_column_attribute(
        ...
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_NULL,
        "null attribute"
    );
    ...
    failures += expect_column_attribute(
        attribute,
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_DEFAULT,
    ...
    failures += expect_literal(
        child_at(child_at(attributes, 0U), 0U),
    ...
    failures += expect_current_timestamp(
        child_at(child_at(attributes, 0U), 0U),
        false,
        0U,
    ...
    failures += expect_current_timestamp(
        ...
        true,
        timestamp_precision,
        "fsp current timestamp default"
    );
    failures += expect_column_attribute(
        child_at(attributes, 1U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_ON_UPDATE,
        "on update attribute"
    );
    failures += expect_current_timestamp(
        child_at(child_at(attributes, 1U), 0U),
        true,
        timestamp_precision,
        "fsp current timestamp update"
    );
    ...
    failures += expect_column_attribute(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_COMMENT,
        "comment attribute"
    );
    failures += expect_literal(
        child_at(child_at(attributes, 0U), 0U),
    ...
    failures += expect_column_attribute(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_VISIBLE,
        "visible attribute"
    );
    ...
    failures += expect_column_format(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_FORMAT_DEFAULT,
    ...
    failures += expect_column_format(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_FORMAT_FIXED,
        "column format fixed"
    );
    ...
    failures += expect_column_format(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_FORMAT_DYNAMIC,
        "column format dynamic"
    );

    attributes = child_at(child_at(columns, storage_default_column), 2U);
    failures += expect_column_storage(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_STORAGE_DEFAULT,
        "storage default"
    );
    ...
    failures += expect_column_storage(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_STORAGE_DISK,
        "storage disk"
    );
    ...
    failures += expect_column_storage(
        child_at(attributes, 0U),
        MYLITE_SQL_AST_COLUMN_STORAGE_MEMORY,
        "storage memory"
    );
    ...
    failures += expect_column_attribute(
        child_at(attributes, 2U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_DEFAULT,
        "repeated first default"
    );
    failures += expect_column_attribute(
        child_at(attributes, 3U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_DEFAULT,
        "repeated second default"
    );
    failures += expect_column_attribute(
        child_at(attributes, 4U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_VISIBLE,
        "repeated visible attribute"
    );
    failures += expect_column_attribute(
        child_at(attributes, repeated_invisible_attribute),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_INVISIBLE,
    ...
    failures += parse_sql(
        "CREATE TABLE attr_keyword_names ("
    ...
    failures += parse_sql(
        "CREATE TABLE timestamp_zero_fsp "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_identifier "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_comment_literal (a INT COMMENT 123);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_comment_equal (a INT COMMENT = 'x');",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_column_format (a INT COLUMN_FORMAT COMPRESSED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_column_format_equal (a INT COLUMN_FORMAT = FIXED);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_storage (a INT STORAGE FLASH);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_storage_equal (a INT STORAGE = DISK);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_equal (a INT DEFAULT = 1);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_on_update_missing_value (a TIMESTAMP ON UPDATE);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_on_missing_update (a TIMESTAMP ON CURRENT_TIMESTAMP);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_on_update_literal (a TIMESTAMP ON UPDATE 1);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_default_fsp "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_update_fsp "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_negative "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_string "
        "(a TIMESTAMP DEFAULT CURRENT_TIMESTAMP('1'));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_scale "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_current_timestamp_overflow "
        "(a TIMESTAMP DEFAULT "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_unparenthesized_expression "
        "(a INT DEFAULT 1 + 2);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_signed_string "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_signed_hex "
        "(a INT DEFAULT +0x10);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_signed_null "
        "(a INT DEFAULT -NULL);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_signed_boolean "
        "(a INT DEFAULT +TRUE);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_default_function (a VARCHAR(20) DEFAULT UPPER('x'));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE supported_auto_increment (a INT AUTO_INCREMENT);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE supported_inline_primary_key (a INT PRIMARY KEY);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_create_table_primary_keys_auto_increment(void) {
    ...
    failures += parse_sql(
        "CREATE TABLE app.primary_valid ("
    ...
    failures += expect_index_option(
        child_at(options, 0U),
        MYLITE_SQL_AST_INDEX_OPTION_KEY_BLOCK_SIZE,
        "key block option"
    );
    ...
    failures += expect_index_option(
        child_at(options, 1U),
        MYLITE_SQL_AST_INDEX_OPTION_COMMENT,
        "primary comment option"
    );
    failures += expect_index_option(
        child_at(options, 2U),
        MYLITE_SQL_AST_INDEX_OPTION_VISIBLE,
    ...
    failures += expect_index_option(
        child_at(options, 0U),
        MYLITE_SQL_AST_INDEX_OPTION_USING,
        "post-list using option"
    );
    failures += expect_index_algorithm(
        child_at(child_at(options, 0U), 0U),
        MYLITE_SQL_AST_INDEX_ALGORITHM_HASH,
        "post-list hash index type"
    );
    failures += expect_index_option(
        child_at(options, 1U),
        MYLITE_SQL_AST_INDEX_OPTION_INVISIBLE,
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_identifier (primary INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_key_identifier (key INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_constraint_identifier (constraint INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_asc_identifier (asc INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_desc_identifier (desc INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_index_identifier (index INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_empty (a INT, PRIMARY KEY ());",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_trailing (a INT, PRIMARY KEY (a,));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_overflow_prefix "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_negative_prefix "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_string_prefix "
        "(a VARCHAR(10), PRIMARY KEY (a('1')));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_key_block_string "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_key_block_negative "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_key_block_overflow "
        "(a INT, PRIMARY KEY (a) "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_prelist_comment "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_comment_equal "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_comment_number "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_engine_attribute_number "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_primary_secondary_engine_attribute_number "
...
static int test_create_table_unique_secondary_indexes(void) {
    ...
    failures += parse_sql(
        "CREATE TABLE app.indexes_valid ("
    ...
    failures += parse_sql(
        "CREATE TABLE index_keyword_names (unique INT);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_inline_unique_index (a INT UNIQUE INDEX);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_empty (a INT, KEY ());",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_trailing (a INT, KEY (a,));",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_missing_parts (a INT, KEY idx);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_unique_missing_parts (a INT, UNIQUE KEY idx);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_fulltext "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_spatial "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_comment_equal "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_secondary_key_block_string "
    ...
    failures += parse_sql(
        "CREATE TABLE bad_unique_overflow_prefix "
...
static int test_create_table_base_execution_syntax(void) {
    ...
    failures += parse_sql(
        "CREATE TABLE IF NOT EXISTS app.base_options "
    ...
    failures += expect_table_option(
        child_at(options, 0U),
        MYLITE_SQL_AST_TABLE_OPTION_ENGINE,
        "engine option"
    );
    failures += expect_span_text(child_at(child_at(options, 0U), 0U), "InnoDB", "engine value");
    failures += expect_table_option(
        child_at(options, 1U),
        MYLITE_SQL_AST_TABLE_OPTION_CHARACTER_SET,
        "charset option"
    );
    failures += expect_span_text(child_at(child_at(options, 1U), 0U), "latin1", "charset value");
    failures += expect_table_option(
        child_at(options, 2U),
        MYLITE_SQL_AST_TABLE_OPTION_COLLATE,
        "collate option"
    );
    failures += expect_span_text(
        child_at(child_at(options, 2U), 0U),
        "latin1_swedish_ci",
        "collation value"
    );
    failures += expect_table_option(
        child_at(options, 3U),
        MYLITE_SQL_AST_TABLE_OPTION_COMMENT,
        "comment option"
    );
    failures += expect_literal(
        child_at(child_at(options, 3U), 0U),
        MYLITE_SQL_AST_LITERAL_STRING,
        "comment value"
    );
    failures += expect_table_option(
    ...
    failures += parse_sql(
        "CREATE TABLE charset_short (a INT) DEFAULT CHARSET utf8mb4;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE option_defaults (a INT) DEFAULT CHARSET DEFAULT "
        "COLLATE DEFAULT;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE TABLE bad_engine_string (a INT) ENGINE='InnoDB';",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_comment_number (a INT) COMMENT = 1;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE TABLE bad_auto_increment_string (a INT) AUTO_INCREMENT = '1';",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_create_drop_index_syntax(void) {
    ...
    failures += parse_sql(
        "CREATE INDEX idx_a USING BTREE ON app.t "
    ...
    failures += expect_index_class(
        statement,
        MYLITE_SQL_AST_INDEX_CLASS_ORDINARY,
    ...
    failures += expect_key_part_order(
        child_at(key_parts, 1U),
        MYLITE_SQL_AST_KEY_PART_ORDER_ASC,
        "create index asc"
    );
    ...
    failures += parse_sql(
        "CREATE UNIQUE INDEX uq_a ON t (a) USING HASH TYPE BTREE VISIBLE;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    failures += expect_index_class(
        statement,
        MYLITE_SQL_AST_INDEX_CLASS_UNIQUE,
    ...
    failures += parse_sql(
        "CREATE FULLTEXT INDEX ft_body ON docs (body) WITH PARSER ngram;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    failures += expect_index_class(
        child_at(result.root, 0U),
        MYLITE_SQL_AST_INDEX_CLASS_FULLTEXT,
        "fulltext create index class"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "CREATE FULLTEXT INDEX ft_body_comment ON docs (body) WITH PARSER ngram "
        "COMMENT 'ft';",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "CREATE INDEX idx_parser ON t (a) WITH PARSER ngram;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE UNIQUE INDEX uq_parser ON t (a) WITH PARSER ngram;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += expect_index_class(
        child_at(result.root, 0U),
        MYLITE_SQL_AST_INDEX_CLASS_SPATIAL,
    ...
    failures += expect_ddl_table_option(
        child_at(ddl_options, 1U),
        MYLITE_SQL_AST_DDL_TABLE_OPTION_LOCK,
        "drop index lock option"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "CREATE INDEX idx_lock ON t (a) LOCK=SHARED ALGORITHM DEFAULT;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE INDEX idx ON t (a) COMMENT = 'bad';",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "CREATE INDEX idx ON t (a) WITH PARSER ngram;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_alter_table_column_operations_syntax(void) {
    ...
    failures += parse_sql(
        "ALTER TABLE app.t ADD c INT AFTER id, DROP COLUMN old_col, "
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ADD_COLUMN,
        false,
        "alter add action"
    );
    ...
    failures += expect_alter_table_column_position(
        position,
        MYLITE_SQL_AST_ALTER_TABLE_COLUMN_POSITION_AFTER,
        "alter add after"
    );
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_DROP_COLUMN,
        true,
        "alter drop action"
    );
    ...
    failures += expect_child_count(
        items,
        (size_t)alter_rename_change_modify_item_count,
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_RENAME_COLUMN,
        false,
        "alter rename action"
    );
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_CHANGE_COLUMN,
        false,
        "alter change action"
    );
    ...
    failures += expect_alter_table_column_position(
        position,
        MYLITE_SQL_AST_ALTER_TABLE_COLUMN_POSITION_FIRST,
        "alter change first"
    );
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_MODIFY_COLUMN,
        true,
        "alter modify action"
    );
    column = child_at(action, 0U);
    ...
    failures += expect_column_attribute(
        child_at(attributes, 1U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_DEFAULT,
        "alter complex default"
    );
    failures += expect_column_attribute(
        child_at(attributes, 2U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_COMMENT,
        "alter complex comment"
    );
    failures += expect_column_attribute(
        child_at(attributes, 3U),
        MYLITE_SQL_AST_COLUMN_ATTRIBUTE_INVISIBLE,
    ...
    failures += parse_sql(
        "ALTER TABLE t DROP old_col, CHANGE COLUMN old_col new_col INT, "
        "MODIFY new_col INT;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "ALTER TABLE t RENAME COLUMN old_name new_name;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "ALTER TABLE t ADD c INT FIRST AFTER id;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_alter_table_key_constraint_operations_syntax(void) {
    ...
    failures += parse_sql(
        "ALTER TABLE t ADD PRIMARY KEY USING BTREE (id, v(8) DESC) "
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ADD_PRIMARY_KEY,
        false,
        "alter add primary key action"
    );
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ADD_UNIQUE_INDEX,
        false,
    ...
    failures += expect_alter_table_action(
        ...
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ADD_SECONDARY_INDEX,
        false,
        "alter add key action"
    );
    index = child_at(action, 0U);
    failures += expect_node(
        child_at(index, 0U),
        MYLITE_SQL_AST_KEY_PART_LIST,
        "alter add unnamed key parts"
    );
    failures += expect_key_part_order(
        child_at(child_at(index, 0U), 0U),
        MYLITE_SQL_AST_KEY_PART_ORDER_DESC,
    ...
    failures += expect_index_option(
        child_at(options, 0U),
        MYLITE_SQL_AST_INDEX_OPTION_WITH_PARSER,
    ...
    failures += parse_sql(
        "ALTER TABLE t DROP PRIMARY KEY, DROP INDEX idx, DROP KEY old_k, "
    ...
    failures += expect_alter_table_action(
        child_at(items, 0U),
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_DROP_PRIMARY_KEY,
        false,
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_DROP_INDEX,
        false,
        "alter drop index action"
    );
    if (action != NULL &&
    ...
    failures += expect_alter_table_action(
        child_at(items, 3U),
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_RENAME_INDEX,
        false,
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ALTER_INDEX_VISIBILITY,
        false,
        "alter index invisible action"
    );
    ...
    failures += parse_sql(
        "ALTER TABLE child ADD CHECK (a > 0) NOT ENFORCED, "
    ...
    failures += expect_alter_table_action(
        child_at(items, 2U),
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_DROP_CHECK_OR_CONSTRAINT,
        false,
    ...
    failures += expect_alter_table_action(
        child_at(items, 4U),
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_ALTER_CHECK_OR_CONSTRAINT,
        false,
    ...
    failures += expect_alter_table_action(
        child_at(items, 7U),
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_DROP_FOREIGN_KEY,
        false,
    ...
    failures += parse_sql(
        "ALTER TABLE t RENAME INDEX old_idx new_idx;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "ALTER TABLE t ADD FOREIGN KEY fk (pid) REFERENCES parent ();",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_rename_table_syntax(void) {
    ...
    failures += parse_sql(
        "RENAME TABLE app.old_name TO app.new_name, "
        "`from` TO `to`, a TO b;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += expect_alter_table_action(
        action,
        MYLITE_SQL_AST_ALTER_TABLE_ACTION_RENAME_TABLE,
        false,
        "alter table rename bare action"
    );
...
static int test_truncate_table_syntax(void) {
    ...
    failures += parse_sql(
        "TRUNCATE TABLE app.old_name; TRUNCATE old_name; "
    ...
    failures += parse_sql(
        "CREATE TABLE truncate (id INT); TRUNCATE TABLE truncate; "
    ...
    failures += expect_function_call(
        ...
        "TRUNCATE",
        2U,
        "truncate scalar function keyword call"
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_table_maintenance_syntax(void) {
    ...
    failures += expect_placeholder_statement(
        statement,
        MYLITE_SQL_AST_PLACEHOLDER_CHECK_TABLE,
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, optimize_local_index),
        MYLITE_SQL_AST_PLACEHOLDER_OPTIMIZE_TABLE,
        "optimize local placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, optimize_no_write_index),
        MYLITE_SQL_AST_PLACEHOLDER_OPTIMIZE_TABLE,
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, repair_local_index),
        MYLITE_SQL_AST_PLACEHOLDER_REPAIR_TABLE,
...
static int test_table_lock_placeholder_syntax(void) {
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, lock_singular_index),
        MYLITE_SQL_AST_PLACEHOLDER_LOCK_TABLES,
        "lock singular placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, unlock_plural_index),
        MYLITE_SQL_AST_PLACEHOLDER_UNLOCK_TABLES,
        "unlock plural placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, unlock_singular_index),
        MYLITE_SQL_AST_PLACEHOLDER_UNLOCK_TABLES,
...
static int test_show_variables_syntax(void) {
    ...
    failures += expect_span_text(
        child_at(child_at(result.root, 0U), 0U),
        "variables",
...
static int test_show_status_syntax(void) {
    ...
    failures += parse_sql(
        "CREATE TABLE status (global INT, session INT, local INT, status INT);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
...
static int test_show_engines_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW STORAGE ENGINES WHERE Engine = 'InnoDB';",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_show_character_set_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW CHARACTER SET LIKE 'a%' WHERE TRUE;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_show_collation_syntax(void) {
    ...
    failures += parse_sql(
        "CREATE TABLE show_collation_identifier (collation INT);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_show_tables_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW TABLES; SHOW FULL TABLES; SHOW TABLES FROM app; "
    ...
    failures += parse_sql(
        "CREATE TABLE tables (id INT); CREATE TABLE full (id INT); "
    ...
    failures += expect_span_text(
        child_at(child_at(result.root, 2U), 0U),
        "extended",
...
static int test_show_table_status_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW TABLE STATUS; SHOW TABLE STATUS FROM app; "
    ...
    failures += parse_sql(
        "SHOW TABLE STATUS LIKE 'a%' WHERE Name = 'a';",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_show_columns_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW COLUMNS FROM t; SHOW FIELDS IN t; SHOW FULL COLUMNS FROM t; "
    ...
    failures += parse_sql(
        "CREATE TABLE columns (fields INT, columns INT, extended INT, "
        "full INT);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += expect_span_text(
        child_at(child_at(table_name, 0U), 0U),
        "fields",
        "fields keyword as column name"
    );
    failures += expect_span_text(
        child_at(child_at(table_name, 1U), 0U),
        "columns",
        "columns keyword as column name"
    );
    failures += expect_span_text(
        child_at(child_at(table_name, 2U), 0U),
        "extended",
        "extended keyword as column name"
    );
    failures += expect_span_text(
        child_at(child_at(table_name, 3U), 0U),
        "full",
    ...
    failures += parse_sql(
        "SHOW COLUMNS FROM t LIKE 'a%' WHERE TRUE;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SHOW COLUMNS FROM t FROM app FROM other;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_show_index_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW INDEX FROM t; SHOW INDEXES IN t; SHOW KEYS FROM t; "
    ...
    failures += parse_sql(
        "CREATE TABLE indexes (indexes INT, KEY indexes (indexes)); "
...
static int test_show_create_database_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW CREATE DATABASE app; SHOW CREATE SCHEMA app; "
...
static int test_show_create_table_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW CREATE TABLE t; SHOW CREATE TABLE app.t; "
...
static int test_show_diagnostics_syntax(void) {
    ...
    failures += parse_sql(
        "SHOW WARNINGS; SHOW ERRORS; SHOW COUNT(*) WARNINGS; "
    ...
    failures += expect_node(
        statement,
        MYLITE_SQL_AST_SHOW_DIAGNOSTICS_COUNT_STATEMENT,
        "show count warnings"
    );
    ...
    failures += expect_span_text(
        child_at(child_at(child_at(child_at(result.root, 0U), 1U), 0U), 0U),
        "errors",
        "errors keyword as column name"
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_describe_table_syntax(void) {
    ...
    failures += parse_sql(
        "DESCRIBE t; DESC t; EXPLAIN t; DESCRIBE app.t; "
...
static int test_explain_statement_placeholder_syntax(void) {
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, explain_select_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "EXPLAIN SELECT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, describe_select_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "DESCRIBE SELECT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, desc_select_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "DESC SELECT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, explain_format_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "EXPLAIN FORMAT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, explain_into_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "EXPLAIN INTO placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, explain_analyze_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "EXPLAIN ANALYZE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, explain_schema_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, explain_update_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
        "EXPLAIN UPDATE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, explain_table_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_EXPLAIN,
...
static int test_drop_table_syntax(void) {
    ...
    failures += parse_sql(
        "DROP TEMPORARY TABLE IF EXISTS `restrict` CASCADE;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
...
static int test_insert_values_syntax(void) {
    ...
    failures += parse_sql(
        "INSERT INTO t (a, `B`) VALUE (DEFAULT, CURRENT_TIMESTAMP);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
...
static int test_insert_set_syntax(void) {
    ...
    failures += parse_sql(
        "INSERT app.t SET a = 1, t.b = DEFAULT, app.t.c = a + 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += expect_node(
        assignments,
        MYLITE_SQL_AST_INSERT_SET_ASSIGNMENT_LIST,
    ...
    failures += expect_node(
        assignment,
        MYLITE_SQL_AST_INSERT_SET_ASSIGNMENT,
    ...
    failures += parse_sql(
        "INSERT INTO t SET `CamelCase` = NULL, b = CURRENT_TIMESTAMP, c = - 4;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += expect_literal(
        child_at(child_at(assignments, 0U), 1U),
        MYLITE_SQL_AST_LITERAL_NULL,
        "insert set NULL"
    );
    failures += expect_current_timestamp(
        child_at(child_at(assignments, 1U), 1U),
        false,
        0U,
        "insert set CURRENT_TIMESTAMP"
    );
    failures += expect_operator(
        child_at(child_at(assignments, 2U), 1U),
        MYLITE_SQL_AST_OPERATOR_NEGATIVE,
        "insert set unary value"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "INSERT INTO t SET a = 1 ON DUPLICATE KEY UPDATE a = 2",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_replace_syntax(void) {
    ...
    failures += parse_sql(
        "REPLACE LOW_PRIORITY INTO t (a, `B`) VALUE (DEFAULT, CURRENT_TIMESTAMP);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "REPLACE DELAYED INTO t VALUES ROW(1, 2), ROW(3, 4);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "REPLACE app.t SET a = 1, t.b = DEFAULT, app.t.c = a + 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "REPLACE DELAYED LOW_PRIORITY INTO t VALUES (1);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_insert_on_duplicate_key_update_syntax(void) {
    ...
    failures += parse_sql(
        "INSERT IGNORE INTO t (a, b) VALUES (1, 2) AS new_row(x, y) "
    ...
    failures += expect_node(
        child_at(child_at(assignments, 1U), 1U),
        MYLITE_SQL_AST_QUALIFIED_IDENTIFIER,
        "ODKU row alias qualified value"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "INSERT INTO t SET a = 1, b = 2 AS n(c1, c2) "
    ...
    failures += expect_span_text(
        child_at(child_at(assignments, 1U), 0U),
        "a",
    ...
    failures += parse_sql(
        "INSERT INTO t VALUES (1) ON DUPLICATE UPDATE a = 1",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "INSERT INTO t VALUES (1) ON DUPLICATE KEY a = 1",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "INSERT INTO t VALUES (1) AS n() ON DUPLICATE KEY UPDATE a = 1",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "INSERT INTO t VALUES (1) ON DUPLICATE KEY UPDATE a",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_update_single_table_syntax(void) {
    ...
    failures += parse_sql(
        "UPDATE app.t AS tt SET tt.a = tt.a + 1, app.t.b = DEFAULT "
    ...
    failures += expect_order_item_direction(
        child_at(order_items, 0U),
        MYLITE_SQL_AST_KEY_PART_ORDER_DESC,
        "update desc order"
    );
    failures += expect_order_item_direction(
        child_at(order_items, 1U),
        MYLITE_SQL_AST_KEY_PART_ORDER_ASC,
        "update asc order"
    );
    ...
    failures += parse_sql(
        "UPDATE app.t tt SET tt.a = 100, tt.a = tt.a + 1 WHERE tt.id IN (1,2)",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "UPDATE app.t AS tt USE INDEX (PRIMARY) "
    ...
    failures += parse_sql(
        "UPDATE t SET a = 1 LIMIT 18446744073709551616",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "UPDATE t JOIN u ON t.id = u.id SET t.a = u.a WHERE u.keep = 1",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "UPDATE t SET a = 1 WHERE id = 1 LIMIT 1 ORDER BY id",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_delete_single_table_syntax(void) {
    ...
    failures += parse_sql(
        "DELETE FROM t AS tt FORCE INDEX (PRIMARY) WHERE tt.id = 1",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "DELETE FROM t LIMIT 18446744073709551616",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "DELETE t FROM t JOIN u ON t.id = u.id WHERE u.keep = 0",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "DELETE a, b.* FROM a INNER JOIN b ON a.id = b.a_id",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += parse_sql(
        "DELETE FROM app.a.*, b USING app.a JOIN b ON a.id = b.a_id",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    statement = child_at(result.root, 0U);
    ...
    failures += expect_node(
        child_at(child_at(target_list, 0U), 0U),
        MYLITE_SQL_AST_QUALIFIED_IDENTIFIER,
        "multi delete qualified target"
    );
    failures += expect_node(
        from_clause,
        MYLITE_SQL_AST_FROM_TABLE_REFERENCES,
    ...
    failures += parse_sql(
        "DELETE a FROM a JOIN b ON a.id = b.a_id LIMIT 1",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_transaction_statement_syntax(void) {
    ...
    failures += expect_transaction_access_mode(
    ...
    failures += parse_sql(
        "CREATE TABLE transaction_keyword_names ("
    ...
    failures += expect_node(
        child_at(result.root, 3U),
        MYLITE_SQL_AST_COMMIT_STATEMENT,
        "commit work statement"
    );
    failures += expect_node(
        child_at(result.root, 4U),
        MYLITE_SQL_AST_ROLLBACK_STATEMENT,
        "rollback statement"
    );
    failures += expect_node(
        child_at(result.root, rollback_work_statement_index),
        MYLITE_SQL_AST_ROLLBACK_STATEMENT,
    ...
    failures += expect_transaction_completion(
        completion,
        MYLITE_SQL_AST_TRANSACTION_CHAIN_YES,
    ...
    failures += expect_transaction_completion(
        completion,
        MYLITE_SQL_AST_TRANSACTION_CHAIN_NO,
    ...
    failures += expect_transaction_completion(
        completion,
        MYLITE_SQL_AST_TRANSACTION_CHAIN_DEFAULT,
        MYLITE_SQL_AST_TRANSACTION_RELEASE_YES,
    ...
    failures += parse_sql(
        "COMMIT AND NO CHAIN; COMMIT AND NO CHAIN NO RELEASE; "
...
static int test_savepoint_statement_syntax(void) {
    ...
    failures += parse_sql(
        "SAVEPOINT s; SAVEPOINT `select`; SAVEPOINT `db.sp`; "
...
static int test_user_variable_and_prepared_statement_syntax(void) {
    ...
    failures += expect_node(
        assignment_list,
        MYLITE_SQL_AST_USER_VARIABLE_ASSIGNMENT_LIST,
    ...
    failures += expect_node(
        assignment,
        MYLITE_SQL_AST_USER_VARIABLE_ASSIGNMENT,
...
static int test_stored_program_placeholder_syntax(void) {
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, call_bare_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CALL,
        "CALL placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, call_args_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CALL,
        "CALL args placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_procedure_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_PROCEDURE,
        "CREATE PROCEDURE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_function_statement_index),
    ...
    failures += expect_placeholder_statement(
        ...
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_FUNCTION,
        "CREATE AGGREGATE FUNCTION placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_trigger_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_TRIGGER,
        "CREATE TRIGGER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_event_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_EVENT,
        "CREATE EVENT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_procedure_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_PROCEDURE,
        "DROP PROCEDURE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_function_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_FUNCTION,
        "DROP FUNCTION placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_trigger_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_TRIGGER,
        "DROP TRIGGER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_event_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_EVENT,
        "DROP EVENT placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, signal_statement_index),
    ...
    mylite_sql_parse_result_deinit(&result);
    failures +=
        parse_sql("CREATE PROCEDURE p(IN id) SELECT 1", MYLITE_SQL_PARSE_SYNTAX_ERROR, &result);
    mylite_sql_parse_result_deinit(&result);
    failures +=
        parse_sql("CREATE FUNCTION f() RETURNS RETURN 1", MYLITE_SQL_PARSE_SYNTAX_ERROR, &result);
    mylite_sql_parse_result_deinit(&result);
    failures += parse_sql(
...
static int test_account_privilege_placeholder_syntax(void) {
    ...
    failures += parse_sql(
        "ALTER USER IF EXISTS 'u'@'localhost' IDENTIFIED BY 'x' PASSWORD EXPIRE; "
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, alter_user_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_ALTER_USER,
        "ALTER USER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_user_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_USER,
        "CREATE USER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_role_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_CREATE_ROLE,
        "CREATE ROLE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_user_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_USER,
        "DROP USER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, drop_role_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_DROP_ROLE,
        "DROP ROLE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, grant_privilege_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_GRANT,
        "GRANT privilege placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, grant_role_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_GRANT,
        "GRANT role placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, rename_user_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_RENAME_USER,
        "RENAME USER placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, revoke_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_REVOKE,
        "REVOKE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, set_default_role_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_SET_DEFAULT_ROLE,
        "SET DEFAULT ROLE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, set_password_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_SET_PASSWORD,
        "SET PASSWORD placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, set_role_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_SET_ROLE,
        "SET ROLE placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, show_grants_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_SHOW_GRANTS,
        "SHOW GRANTS placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, show_privileges_statement_index),
...
static int test_partitioning_placeholder_syntax(void) {
    ...
    failures += expect_placeholder_statement(
        child_at(result.root, create_hash_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE hash partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_key_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE key partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_range_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE range partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_range_columns_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE range columns partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_list_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
    ...
    failures += expect_placeholder_statement(
        ...
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE list columns partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, create_subpartition_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "CREATE subpartition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_partition_by_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER partition by placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_add_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER add partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_drop_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER drop partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_discard_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER discard partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_import_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER import partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_truncate_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER truncate partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_coalesce_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
    ...
    failures += expect_placeholder_statement(
        ...
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER reorganize partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_exchange_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER exchange partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_analyze_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER analyze partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_check_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER check partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_optimize_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER optimize partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_rebuild_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER rebuild partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_repair_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
        "ALTER repair partition placeholder"
    );
    failures += expect_placeholder_statement(
        child_at(result.root, alter_remove_statement_index),
        MYLITE_SQL_AST_PLACEHOLDER_TABLE_PARTITIONING,
...
static int test_select_expression_list(void) {
    ...
    failures += parse_sql(
        "SELECT @@sql_mode, @@SESSION.sql_mode AS sm, "
...
static int test_expression_operator_foundation_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT 1 = 1, 1 <=> NULL, 1 <> 2, 1 != 2, 1 < 2, 1 <= 2, "
        "2 > 1, 2 >= 1",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += expect_operator(
        ...
        "angle not equal operator"
    );
    failures += expect_operator(
        child_at(child_at(select_list, 3U), 0U),
        MYLITE_SQL_AST_OPERATOR_NOT_EQUAL,
        "bang not equal operator"
    );
    failures += expect_operator(
        child_at(child_at(select_list, 4U), 0U),
    ...
    failures += expect_operator(
        child_at(child_at(select_list, 6U), 0U),
    ...
    failures += parse_sql(
        "SELECT !0, NOT 0, 1 AND 1, 1 && 1, 1 XOR 0, 1 OR 0, 1 || 0, "
    ...
    failures += expect_operator(
        child_at(child_at(select_list, 11U), 0U),
        MYLITE_SQL_AST_OPERATOR_SHIFT_LEFT,
        "shift left"
    );
    failures += expect_operator(
        child_at(child_at(select_list, 12U), 0U),
        MYLITE_SQL_AST_OPERATOR_SHIFT_RIGHT,
        "shift right"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT 1 IS NULL, 1 IS NOT TRUE, 2 BETWEEN 1 AND 3, "
    ...
    failures += expect_operator(
        child_at(child_at(select_list, 7U), 0U),
        MYLITE_SQL_AST_OPERATOR_NOT_REGEXP,
        "not regexp"
    );
    failures += expect_operator(
        child_at(child_at(select_list, 8U), 0U),
    ...
    failures += expect_operator(
        child_at(child_at(select_list, 9U), 0U),
        MYLITE_SQL_AST_OPERATOR_NOT_IN,
        "not in"
    );
    failures += expect_node(
        child_at(child_at(child_at(select_list, 9U), 0U), 1U),
        MYLITE_SQL_AST_EXPRESSION_LIST,
        "in expression list"
    );
    failures += expect_operator(
        child_at(child_at(select_list, 10U), 0U),
    ...
    failures += expect_operator(
        ...
        MYLITE_SQL_AST_OPERATOR_BINARY_CAST,
        "binary prefix"
    );
...
static int test_scalar_function_call_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT CONCAT('a','b') AS joined, PI() pi_value, "
    ...
    failures += expect_function_call(
        child_at(arguments, coalesce_nested_arg_index),
        "CONCAT",
        2U,
        "nested CONCAT call"
    );
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 18U), 0U),
        "REPLACE",
        3U,
        "REPLACE call"
    );
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 20U), 0U),
        "Exp",
        1U,
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 27U), 0U),
        "Power",
        2U,
        "POWER case-insensitive call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 28U), 0U),
        "Sqrt",
        1U,
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 32U), 0U),
        "AtAn",
        1U,
        "ATAN one-argument call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 33U), 0U),
        "ATAN",
        2U,
        "ATAN two-argument call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 34U), 0U),
        "ATAN2",
        1U,
        "ATAN2 one-argument call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 35U), 0U),
        "AtAn2",
        2U,
        "ATAN2 two-argument call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 36U), 0U),
        "Greatest",
        2U,
        "GREATEST call"
    );
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 40U), 0U),
        "DEFAULT",
        1U,
        "DEFAULT call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 41U), 0U),
        "CURRENT_ROLE",
        0U,
        "CURRENT_ROLE call"
    );
    ...
    if (!make_deeply_nested_coalesce_query(
            nested_query,
            sizeof(nested_query),
    ...
    mylite_sql_parse_result_deinit(&result);
    failures += parse_sql("SELECT DEFAULT() FROM t", MYLITE_SQL_PARSE_SYNTAX_ERROR, &result);
    mylite_sql_parse_result_deinit(&result);
    failures += parse_sql("SELECT DEFAULT(a, b) FROM t", MYLITE_SQL_PARSE_SYNTAX_ERROR, &result);
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 15U), 0U),
        "SUBSTRING_INDEX",
        3U,
        "SUBSTRING_INDEX call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 16U), 0U),
        "substring_index",
        3U,
        "lowercase SUBSTRING_INDEX call"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT ASCII('A'), ORD('\xE6\xB5\xB7'), LOCATE('a','alpha'), "
    ...
    failures += parse_sql(
        "SELECT REPEAT('ab', 3), SPACE(3), REVERSE('abc'), "
        "LPAD('hi', 5, '.'), RPAD('hi', 5, '.'), "
    ...
    failures += parse_sql(
        ...
        "MD5('abc'), SHA('abc'), Sha1('abc'), SHA2('abc', 256), "
        "INET_ATON('127.0.0.1'), "
    ...
    failures +=
        expect_function_call(child_at(child_at(select_list, 14U), 0U), "MD5", 1U, "MD5 call");
    failures +=
        expect_function_call(child_at(child_at(select_list, 15U), 0U), "SHA", 1U, "SHA call");
    failures += expect_function_call(
        child_at(child_at(select_list, 16U), 0U),
        "Sha1",
        1U,
        "SHA1 case-insensitive call"
    );
    failures +=
        expect_function_call(child_at(child_at(select_list, 17U), 0U), "SHA2", 2U, "SHA2 call");
    failures += expect_function_call(
        child_at(child_at(select_list, 18U), 0U),
        "INET_ATON",
        1U,
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 19U), 0U),
        "inet_ntoa",
        1U,
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 21U), 0U),
        "Uuid_To_Bin",
        2U,
        "UUID_TO_BIN case-insensitive call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 22U), 0U),
        "bin_to_uuid",
        1U,
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 24U), 0U),
        "UUID_SHORT",
        0U,
        "UUID_SHORT call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 25U), 0U),
        "GET_LOCK",
        2U,
        "GET_LOCK call"
    );
    failures += expect_function_call(
        child_at(child_at(select_list, 26U), 0U),
        "RELEASE_LOCK",
        1U,
        "RELEASE_LOCK call"
    );
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 29U), 0U),
        "RELEASE_ALL_LOCKS",
        0U,
        "RELEASE_ALL_LOCKS call"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT CHAR(65), CHAR(65,66 USING utf8mb4), "
    ...
    failures += parse_sql(
        "SELECT CONCAT ('a','b') AS spaced, LEFT('abc', 1) 'left alias';",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    select_list = child_at(child_at(result.root, 0U), 0U);
    ...
    failures += parse_sql(
        "SELECT CURRENT_USER(), CURRENT_ROLE(), CURRENT_DATE(), CURRENT_TIME(), "
    ...
    failures += parse_sql(
        "SELECT DATE('2024-02-29 12:34:56'), "
    ...
    failures += expect_function_call(
        child_at(child_at(select_list, 17U), 0U),
        "TIMEDIFF",
        2U,
    ...
    mylite_sql_parse_result_deinit(&result);
    failures +=
        parse_sql("SELECT last_day FROM temporal_part_names;", MYLITE_SQL_PARSE_OK, &result);
    mylite_sql_parse_result_deinit(&result);
    failures +=
    ...
    mylite_sql_parse_result_deinit(&result);
    failures +=
        parse_sql("SELECT time_format FROM temporal_part_names;", MYLITE_SQL_PARSE_OK, &result);
    mylite_sql_parse_result_deinit(&result);
    failures +=
    ...
    if (parse_sql(
            "SELECT YEAR('2024-02-29'), MONTH('2024-02-29'), "
        ...
        failures += expect_function_call(
            ...
            "MICROSECOND",
            1U,
            "MICROSECOND keyword function call"
        );
        failures += expect_function_call(
            child_at(child_at(select_list, 11U), 0U),
            "EXTRACT",
        ...
        failures += expect_bool(
            child_at(child_at(select_list, 11U), 0U)->interval_spec,
            true,
    ...
    failures += parse_sql(
        "SELECT year, month, day, dayofweek, dayofyear, quarter, "
    ...
    if (parse_sql(
            "SELECT WEEK('2008-02-20'), WEEK('2000-01-01', 2);",
            MYLITE_SQL_PARSE_OK,
            &result
        ) == 0) {
        select_list = child_at(child_at(result.root, 0U), 0U);
        failures += expect_function_call(
            ...
            "WEEK",
            1U,
            "WEEK one-argument function call"
        );
        failures += expect_function_call(
            child_at(child_at(select_list, 1U), 0U),
            "WEEK",
            2U,
            "WEEK two-argument function call"
        );
    } else {
        ++failures;
    }
    ...
    failures += parse_sql("SELECT week FROM temporal_part_names;", MYLITE_SQL_PARSE_OK, &result);
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT TIMESTAMPDIFF(DAY,'2024-02-28','2024-03-01'), "
    ...
    failures += parse_sql(
        "SELECT TIMESTAMPADD(DAY,2,'2024-02-28'), "
    ...
    failures += parse_sql(
        "SELECT DATE_ADD('2024-01-01', INTERVAL 1 BOGUS)",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    mylite_sql_parse_result_deinit(&result);
    failures += parse_sql(
        "SELECT TIMESTAMPADD(DAY,1,'2024-01-01','x')",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    mylite_sql_parse_result_deinit(&result);
    failures += parse_sql(
        "SELECT TIMESTAMPDIFF(DAY,'2024-01-01','2024-01-02','x')",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT NOW(), NOW(0), NOW(6), CURDATE(), CURTIME(), CURTIME(6), "
...
static int test_case_expression_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT CASE WHEN 0 THEN 1 WHEN 1 THEN CASE WHEN 1 THEN 2 ELSE 3 END "
        "ELSE 4 END;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
...
static int test_cast_expression_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT CAST('12.34' AS DECIMAL(6,2)), CAST('12' AS DEC(5));",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    select_list = child_at(child_at(result.root, 0U), 0U);
    ...
    when_list = child_at(case_expression, 0U);
    failures += expect_node(
        child_at(child_at(when_list, 0U), 1U),
        MYLITE_SQL_AST_CAST_EXPRESSION,
        "CASE CAST result"
    );
    failures += expect_node(
        child_at(case_expression, 1U),
        MYLITE_SQL_AST_CAST_EXPRESSION,
        "CASE CAST else"
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_convert_expression_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT CASE WHEN 1 THEN CONVERT(1, UNSIGNED) "
...
static int test_information_schema_select(void) {
    ...
    failures += parse_sql(
        "SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE TRUE;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    failures += expect_node(
        child_at(child_at(result.root, 0U), 2U),
        MYLITE_SQL_AST_WHERE_CLAUSE,
...
static int test_select_table_core_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT a AS x, b y, c AS `quoted alias`, d AS 'single alias', "
    ...
    failures += expect_literal(
        child_at(child_at(select_list, 4U), 1U),
        MYLITE_SQL_AST_LITERAL_STRING,
        "bare string alias"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += expect_span_text(
        child_at(child_at(select, 1U), 1U),
        "`quoted alias`",
        "quoted table alias"
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_select_inner_join_syntax(void) {
    ...
    failures += expect_join_condition_type(
        condition,
        MYLITE_SQL_AST_JOIN_CONDITION_USING,
    ...
    failures += expect_span_text(
        child_at(child_at(using_columns, 1U), 0U),
        "shared",
        "second USING column"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT * FROM app.l AS lefty JOIN r righty "
    ...
    failures += parse_sql(
        "SELECT * FROM l USE INDEX FOR JOIN (PRIMARY) "
...
static int test_select_outer_join_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT * FROM l LEFT OUTER JOIN r USING (id, shared);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    join = child_at(child_at(child_at(child_at(result.root, 0U), 1U), 0U), 0U);
...
static int test_select_where_clause_syntax(void) {
    ...
    failures += expect_span_text(
        child_at(child_at(child_at(result.root, 0U), 1U), 1U),
        "tt",
        "AS where alias"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT id FROM t WHERE s LIKE 'a%' ESCAPE '!' OR 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
...
static int test_select_order_limit_offset_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT a FROM t WHERE n = 1 ORDER BY n DESC, a ASC LIMIT 2 OFFSET 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    select = child_at(result.root, 0U);
    ...
    failures += expect_operator(
        child_at(child_at(order_items, 0U), 0U),
        MYLITE_SQL_AST_OPERATOR_NEGATIVE,
        "negative order constant"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT a FROM t ORDER BY a WHERE n = 1;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT a FROM t LIMIT 18446744073709551616;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_select_distinct_syntax(void) {
    ...
    failures += expect_select_duplicate_mode(
        select,
        MYLITE_SQL_AST_SELECT_DUPLICATES_ALL,
        true,
        false,
        1U,
        "all duplicate mode"
    ...
    failures += expect_select_duplicate_mode(
        ...
        false,
        2U,
        "repeated distinct duplicate mode"
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_select_window_syntax(void) {
    ...
    failures += expect_int(
        (int)window_call->window_function_kind,
    ...
    failures += parse_sql(
        "SELECT SUM(n) OVER w AS total FROM t "
    ...
    failures += expect_int(
        (int)window_call->window_function_kind,
        (int)MYLITE_SQL_AST_WINDOW_FUNCTION_AGGREGATE,
    ...
    failures += expect_node(
        child_at(window_spec, 0U),
        MYLITE_SQL_AST_WINDOW_PARTITION_CLAUSE,
        "window partition clause"
    );
    failures += expect_node(
        child_at(window_spec, 1U),
        MYLITE_SQL_AST_ORDER_BY_CLAUSE,
        "window order clause"
    );
    failures += expect_node(frame, MYLITE_SQL_AST_WINDOW_FRAME_CLAUSE, "window frame clause");
    failures += expect_int(
        (int)frame->window_frame_unit,
        (int)MYLITE_SQL_AST_WINDOW_FRAME_UNIT_ROWS,
        "window frame rows unit"
    );
    failures += expect_int(
        (int)child_at(frame, 0U)->window_frame_bound_kind,
        (int)MYLITE_SQL_AST_WINDOW_FRAME_BOUND_UNBOUNDED_PRECEDING,
        "window frame start"
    );
    failures += expect_int(
        (int)child_at(frame, 1U)->window_frame_bound_kind,
        (int)MYLITE_SQL_AST_WINDOW_FRAME_BOUND_CURRENT_ROW,
        "window frame end"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT LAG(n, 1, 0) RESPECT NULLS OVER "
    ...
    failures += expect_int(
        (int)window_call->window_function_kind,
        (int)MYLITE_SQL_AST_WINDOW_FUNCTION_LAG,
        "lag window function kind"
    );
    failures += expect_int(
        (int)child_at(window_call, 1U)->window_null_treatment,
        (int)MYLITE_SQL_AST_WINDOW_NULL_TREATMENT_RESPECT,
        "lag null treatment"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT NTH_VALUE(n, 2) IGNORE NULLS OVER "
    ...
    failures += expect_int(
        (int)window_call->window_function_kind,
        (int)MYLITE_SQL_AST_WINDOW_FUNCTION_NTH_VALUE,
        "nth_value window function kind"
    );
    failures += expect_int(
        (int)child_at(window_call, 1U)->window_null_treatment,
        (int)MYLITE_SQL_AST_WINDOW_NULL_TREATMENT_IGNORE,
        "nth_value null treatment"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT current, following, nulls, preceding, respect, unbounded FROM t;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_union_query_expression_syntax(void) {
    ...
    const struct mylite_sql_ast_node *local_query = NULL;
    ...
    failures += expect_union_duplicate_mode(
        top_union,
        MYLITE_SQL_AST_SET_DUPLICATES_DISTINCT,
    ...
    failures += expect_union_duplicate_mode(
        left_union,
        MYLITE_SQL_AST_SET_DUPLICATES_ALL,
        "left union all mode"
    );
    failures +=
    ...
    failures += parse_sql(
        "(SELECT n FROM t ORDER BY n DESC LIMIT 1) UNION ALL SELECT n FROM t;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    query = child_at(result.root, 0U);
    ...
    local_query = child_at(left_primary, 0U);
    local_select = child_at(local_query, 0U);
    failures += expect_union_duplicate_mode(
        top_union,
        MYLITE_SQL_AST_SET_DUPLICATES_ALL,
    ...
    query = child_at(result.root, 0U);
    top_union = child_at(query, 0U);
    local_query = child_at(child_at(top_union, 0U), 0U);
    left_primary = child_at(local_query, 0U);
    local_query = child_at(left_primary, 0U);
    local_select = child_at(local_query, 0U);
    failures += expect_node(
        child_at(top_union, 0U),
        MYLITE_SQL_AST_QUERY_PRIMARY,
    ...
    failures += expect_node(
        local_query,
        MYLITE_SQL_AST_QUERY_EXPRESSION,
        "nested parenthesized inner query"
    );
    failures +=
    ...
    failures += parse_sql("TABLE t ORDER BY n LIMIT 2;", MYLITE_SQL_PARSE_OK, &result);
    local_select = child_at(result.root, 0U);
    failures += expect_node(local_select, MYLITE_SQL_AST_SELECT_STATEMENT, "table query select");
    failures += expect_node(
        child_at(local_select, 0U),
        MYLITE_SQL_AST_SELECT_LIST,
        "table query select list"
    );
    failures += expect_node(
        child_at(local_select, 1U),
        MYLITE_SQL_AST_FROM_TABLE,
        "table query from table"
    );
    failures += expect_node(
        child_at(local_select, 2U),
        MYLITE_SQL_AST_ORDER_BY_CLAUSE,
        "table query order"
    );
    failures +=
    ...
    query = child_at(result.root, 0U);
    top_union = child_at(query, 0U);
    failures += expect_set_operation(
        top_union,
        MYLITE_SQL_AST_SET_OPERATION_UNION,
        "values union table operation"
    );
    failures += expect_node(
        child_at(top_union, 0U),
        MYLITE_SQL_AST_VALUES_STATEMENT,
        "values union left operand"
    );
    failures += expect_node(
        child_at(top_union, 1U),
        MYLITE_SQL_AST_SELECT_STATEMENT,
    ...
    failures += expect_set_operation(
        child_at(top_union, 1U),
        MYLITE_SQL_AST_SET_OPERATION_INTERSECT,
    ...
    failures += expect_set_operation(
        top_union,
        MYLITE_SQL_AST_SET_OPERATION_EXCEPT,
        "except query operation"
    );
    failures += expect_union_duplicate_mode(
        top_union,
        MYLITE_SQL_AST_SET_DUPLICATES_DISTINCT,
    ...
    failures += expect_set_operation(
        top_union,
        MYLITE_SQL_AST_SET_OPERATION_EXCEPT,
        "parenthesized set expression operation"
    );
    failures += expect_node(
        child_at(top_union, 0U),
        MYLITE_SQL_AST_QUERY_PRIMARY,
        "parenthesized set expression left operand"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql("(SELECT 1) ORDER BY 1 LIMIT 1;", MYLITE_SQL_PARSE_OK, &result);
    query = child_at(result.root, 0U);
    failures += expect_node(
        query,
        MYLITE_SQL_AST_QUERY_EXPRESSION,
        "standalone parenthesized query expression"
    );
    failures += expect_node(
        child_at(query, 0U),
        MYLITE_SQL_AST_QUERY_PRIMARY,
        "standalone parenthesized primary"
    );
    failures += expect_node(
        child_at(query, 1U),
        MYLITE_SQL_AST_ORDER_BY_CLAUSE,
        "standalone parenthesized global order"
    );
    failures += expect_node(
        child_at(query, 2U),
        MYLITE_SQL_AST_LIMIT_CLAUSE,
        "standalone parenthesized global limit"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT n FROM t ORDER BY n LIMIT 1 UNION ALL SELECT n FROM t;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT 1 UNION SELECT 2 ORDER BY 1 OFFSET 1;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_aggregate_grouping_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT grp, COUNT(*), SUM(n) AS total, AVG(n), MIN(txt), MAX(txt) "
    ...
    failures += expect_node(
        child_at(select, aggregate_where_child),
        MYLITE_SQL_AST_WHERE_CLAUSE,
    ...
    failures += expect_group_item_direction(
        child_at(group_items, 0U),
        MYLITE_SQL_AST_KEY_PART_ORDER_ASC,
        "first group direction"
    );
    failures += expect_literal(
        child_at(child_at(group_items, 1U), 0U),
        MYLITE_SQL_AST_LITERAL_INTEGER,
        "ordinal group expression"
    );
    failures += expect_group_item_direction(
        child_at(group_items, 1U),
        MYLITE_SQL_AST_KEY_PART_ORDER_DESC,
        "second group direction"
    );
    ...
    failures += expect_aggregate_call(
        ...
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_STAR,
        "COUNT",
        "COUNT star aggregate"
    );
    failures += expect_aggregate_call(
        child_at(child_at(select_list, 2U), 0U),
        MYLITE_SQL_AST_AGGREGATE_SUM,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION,
        "SUM",
        "SUM aggregate"
    );
    failures += expect_span_text(
        child_at(child_at(child_at(select_list, 2U), 0U), 1U),
        "n",
        "SUM argument"
    );
    failures += expect_aggregate_call(
        child_at(child_at(select_list, 3U), 0U),
        MYLITE_SQL_AST_AGGREGATE_AVG,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION,
        "AVG",
        "AVG aggregate"
    );
    failures += expect_aggregate_call(
        child_at(child_at(select_list, 4U), 0U),
        MYLITE_SQL_AST_AGGREGATE_MIN,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION,
        "MIN",
        "MIN aggregate"
    );
    failures += expect_aggregate_call(
        child_at(child_at(select_list, aggregate_max_item), 0U),
        MYLITE_SQL_AST_AGGREGATE_MAX,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION,
        "MAX",
        "MAX aggregate"
    );
    ...
    failures += parse_sql(
        "SELECT COUNT(n) FROM t HAVING COUNT(*) > 0 ORDER BY 1 LIMIT 1;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    select = child_at(result.root, 0U);
    ...
    failures += expect_aggregate_call(
        child_at(child_at(child_at(select, 0U), 0U), 0U),
        MYLITE_SQL_AST_AGGREGATE_COUNT,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION,
        "COUNT",
        "COUNT expression aggregate"
    );
    failures += expect_node(having, MYLITE_SQL_AST_HAVING_CLAUSE, "having without group");
    failures += expect_aggregate_call(
        child_at(child_at(having, 0U), 0U),
        MYLITE_SQL_AST_AGGREGATE_COUNT,
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_STAR,
        "COUNT",
        "having COUNT star"
    ...
    failures += parse_sql(
        "SELECT COUNT(DISTINCT n, nullable + 1) AS c FROM t;",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    select = child_at(result.root, 0U);
    ...
    failures += expect_node(
        child_at(child_at(child_at(select_list, 0U), 0U), 1U),
        MYLITE_SQL_AST_EXPRESSION_LIST,
    ...
    failures += expect_node(
        child_at(child_at(child_at(select_list, 1U), 0U), 1U),
        MYLITE_SQL_AST_EXPRESSION_LIST,
        "GROUP_CONCAT distinct arguments"
    );
    failures += expect_node(
        child_at(child_at(child_at(select_list, 1U), 0U), 2U),
        MYLITE_SQL_AST_ORDER_BY_CLAUSE,
    ...
    failures += expect_order_item_direction(
        ...
        MYLITE_SQL_AST_KEY_PART_ORDER_DESC,
        "GROUP_CONCAT order direction"
    );
    failures += expect_literal(
        child_at(child_at(child_at(select_list, 1U), 0U), 3U),
        MYLITE_SQL_AST_LITERAL_STRING,
    ...
    failures += expect_aggregate_call(
        ...
        MYLITE_SQL_AST_AGGREGATE_ARGUMENT_EXPRESSION_LIST,
        "GROUP_CONCAT",
        "GROUP_CONCAT multi argument aggregate"
    );
    failures += expect_child_count(
        child_at(child_at(select_list, 2U), 0U),
        4U,
    ...
    failures += expect_literal(
        child_at(
            child_at(child_at(child_at(child_at(child_at(select_list, 2U), 0U), 2U), 0U), 0U),
            0U
        ),
        MYLITE_SQL_AST_LITERAL_INTEGER,
        "GROUP_CONCAT order ordinal"
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT GROUP_CONCAT(txt SEPARATOR NULL) FROM t;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT GROUP_CONCAT(txt SEPARATOR 123) FROM t;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT GROUP_CONCAT(txt SEPARATOR CONCAT('|', '|')) FROM t;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT GROUP_CONCAT(txt SEPARATOR '|' ORDER BY txt) FROM t;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT a FROM t HAVING a > 0 GROUP BY a;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int test_subquery_expression_syntax(void) {
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val = "
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE EXISTS (SELECT 1 FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE NOT EXISTS (SELECT 1 FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val IN (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val NOT IN (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val > ANY (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += expect_subquery_quantifier(
        predicate,
        MYLITE_SQL_AST_SUBQUERY_QUANTIFIER_ANY,
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val <> SOME (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += expect_subquery_quantifier(
        predicate,
        MYLITE_SQL_AST_SUBQUERY_QUANTIFIER_SOME,
        "some quantified quantifier"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val >= ALL (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += expect_subquery_quantifier(
        predicate,
        MYLITE_SQL_AST_SUBQUERY_QUANTIFIER_ALL,
        "all quantified quantifier"
    );
    mylite_sql_parse_result_deinit(&result);

    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) = ANY "
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) = SOME "
        "(SELECT a, b FROM pair_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) <> ALL "
        "(SELECT a, b FROM pair_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE ROW(val, grp) = ANY "
        "(SELECT a, b FROM pair_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE ROW(val) = ANY (SELECT a FROM pair_t);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) IN (SELECT a, b FROM pair_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) NOT IN (SELECT a, b FROM pair_t);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    predicate = child_at(child_at(child_at(result.root, 0U), 2U), 0U);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE ROW(val, grp) = "
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE (val, grp) < "
        "(SELECT a, b FROM pair_t WHERE a = 10);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t JOIN inner_t i ON EXISTS (SELECT 1);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    join = child_at(child_at(child_at(child_at(result.root, 0U), 1U), 0U), 0U);
    ...
    failures += parse_sql(
        "SELECT grp, COUNT(*) FROM outer_t GROUP BY grp "
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t ORDER BY "
        "(SELECT val FROM inner_t WHERE id = 101);",
        MYLITE_SQL_PARSE_OK,
        &result
    );
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val > any /* comment */ "
    ...
    failures += parse_sql(
        "SELECT ROW(1) = (SELECT val FROM inner_t);",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE val IN ();",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
    ...
    failures += parse_sql(
        "SELECT id FROM outer_t WHERE EXISTS SELECT 1;",
        MYLITE_SQL_PARSE_SYNTAX_ERROR,
        &result
    );
    mylite_sql_parse_result_deinit(&result);
...
static int parse_sql(
    ...
    if (actual != expected_status) {
        fprintf(
            ...
            (int)out_result->error_token.length,
...
static int expect_placeholder_statement(
    ...
    if (node != NULL && node->placeholder_statement_kind != expected_kind) {
        fprintf(
            stderr,
            "%s: expected placeholder kind %d, got %d\n",
            context,
            (int)expected_kind,
            (int)node->placeholder_statement_kind
        );
        failures = 1;
    }
...
static int expect_interval_function_call(
    ...
    if (node->interval_unit != expected_unit) {
        fprintf(
            stderr,
            "%s: expected interval unit %d, got %d\n",
            context,
            (int)expected_unit,
...
static int expect_aggregate_call(
    ...
    if (node->aggregate_kind != expected_kind) {
        fprintf(
            stderr,
            "%s: expected aggregate kind %s, got %s\n",
            context,
    ...
    if (node->aggregate_argument != expected_argument) {
        fprintf(
            stderr,
            "%s: expected aggregate argument %s, got %s\n",
            context,
...
static int expect_join_condition_type(
    ...
    if (node != NULL && node->join_condition_type != expected) {
        fprintf(
            stderr,
            "%s: expected join condition %s, got %s\n",
            context,
...
static int expect_select_duplicate_mode(
    ...
    if (node->select_duplicate_mode != expected) {
        fprintf(
            stderr,
            "%s: expected duplicate mode %s, got %s\n",
            context,
    ...
    if (node->select_duplicate_modifier_count != modifier_count) {
        fprintf(
            stderr,
            "%s: expected %zu duplicate modifiers, got %zu\n",
            context,
            modifier_count,
...
static int expect_union_duplicate_mode(
    ...
    if (node != NULL && node->set_duplicate_mode != expected) {
        fprintf(
            stderr,
            "%s: expected union duplicate mode %s, got %s\n",
            context,
...
static int expect_set_operation(
    ...
    if (node != NULL && node->set_operation != expected) {
        fprintf(
            stderr,
            "%s: expected set operation %s, got %s\n",
            context,
            mylite_sql_ast_set_operation_name(expected),
            mylite_sql_ast_set_operation_name(node->set_operation)
        );
        failures = 1;
    }
    return failures;
...
static int expect_subquery_quantifier(
    ...
    if (node == NULL) {
        fprintf(
            stderr,
            "%s: expected subquery quantifier %s, got null node\n",
            context,
    ...
    if (node->subquery_quantifier != expected) {
        fprintf(
            stderr,
            "%s: expected subquery quantifier %s, got %s\n",
            context,
            mylite_sql_ast_subquery_quantifier_name(expected),
...
static int expect_schema_option(
    ...
    if (node != NULL && node->schema_option != expected) {
        fprintf(
            stderr,
            "%s: expected schema option %s, got %s\n",
            context,
...
static int expect_column_type(
    ...
    if (node != NULL && node->column_type != expected) {
        fprintf(
            stderr,
            "%s: expected column type %s, got %s\n",
            context,
...
static int expect_column_attribute(
    ...
    if (node != NULL && node->column_attribute != expected) {
        fprintf(
            stderr,
            "%s: expected column attribute %s, got %s\n",
            context,
...
static int expect_column_format(
    ...
    if (node != NULL && node->column_format != expected) {
        fprintf(
            stderr,
            "%s: expected column format %s, got %s\n",
            context,
...
static int expect_column_storage(
    ...
    if (node != NULL && node->column_storage != expected) {
        fprintf(
            stderr,
            "%s: expected column storage %s, got %s\n",
            context,
...
static int expect_key_part_order(
    ...
    if (node != NULL && node->key_part_order != expected) {
        fprintf(
            stderr,
            "%s: expected key part order %s, got %s\n",
            context,
...
static int expect_group_item_direction(
    ...
    if (node != NULL && node->key_part_order != expected) {
        fprintf(
            stderr,
            "%s: expected group direction %s, got %s\n",
            context,
            mylite_sql_ast_key_part_order_name(expected),
            mylite_sql_ast_key_part_order_name(node->key_part_order)
        );
...
static int expect_limit_bound(
    ...
    if (node != NULL && (!node->has_limit_bound_value || actual != expected)) {
        fprintf(
            stderr,
            "%s: expected limit bound %llu, got %llu\n",
            context,
...
static int expect_index_algorithm(
    ...
    if (node != NULL && node->index_algorithm != expected) {
        fprintf(
            stderr,
            "%s: expected index algorithm %s, got %s\n",
            context,
...
static int expect_index_option(
    ...
    if (node != NULL && node->index_option != expected) {
        fprintf(
            stderr,
            "%s: expected index option %s, got %s\n",
            context,
...
static int expect_index_class(
    ...
    if (node != NULL && node->index_class != expected) {
        fprintf(
            stderr,
            "%s: expected index class %s, got %s\n",
            context,
...
static int expect_ddl_table_option(
    ...
    if (node != NULL && node->ddl_table_option != expected) {
        fprintf(
            stderr,
            "%s: expected DDL table option %s, got %s\n",
            context,
...
static int expect_alter_table_action(
    ...
    if (node != NULL && node->alter_table_action != expected) {
        fprintf(
            stderr,
            "%s: expected ALTER TABLE action %s, got %s\n",
            context,
    ...
    if (node != NULL && node->alter_table_action_column_keyword != column_keyword) {
        ...
        fprintf(
            stderr,
            "%s: expected ALTER TABLE COLUMN keyword %s, got %s\n",
            context,
            expected_keyword_text,
            actual_keyword_text
        );
        failures = 1;
    }
...
static int expect_alter_table_column_position(
    ...
    if (node != NULL && node->alter_table_column_position != expected) {
        fprintf(
            stderr,
            "%s: expected ALTER TABLE column position %s, got %s\n",
            context,
...
static int expect_table_option(
    ...
    if (node != NULL && node->table_option != expected) {
        fprintf(
            stderr,
            "%s: expected table option %d, got %d\n",
            context,
            (int)expected,
...
static int expect_transaction_access_mode(
    ...
    if (node != NULL && node->transaction_access_mode != expected) {
        fprintf(
            stderr,
            "%s: expected transaction access mode %d, got %d\n",
            context,
            (int)expected,
...
static int expect_transaction_completion(
    ...
    if (node != NULL && node->transaction_chain != expected_chain) {
        fprintf(
            stderr,
            "%s: expected transaction chain %d, got %d\n",
            context,
            (int)expected_chain,
    ...
    if (node != NULL && node->transaction_release != expected_release) {
        fprintf(
            stderr,
            "%s: expected transaction release %d, got %d\n",
            context,
            (int)expected_release,
            (int)node->transaction_release
        );
        failures = 1;
    }
...
static int expect_current_timestamp(
    ...
    if (node != NULL && node->has_column_precision != has_precision) {
        fprintf(
            stderr,
            "%s: expected current timestamp precision flag %d, got %d\n",
            context,
            (int)has_precision,
            (int)node->has_column_precision
        );
        failures = 1;
    }
    ...
    if (node != NULL && has_precision && node->column_precision != precision) {
        fprintf(
            stderr,
            "%s: expected current timestamp precision %llu, got %llu\n",
            context,
            (unsigned long long)precision,
            (unsigned long long)node->column_precision
        );
        failures = 1;
    }
