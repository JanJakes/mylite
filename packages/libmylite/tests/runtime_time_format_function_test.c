...
static int test_no_source_dual_and_do_time_format(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "time_format do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "time_format do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "time_format do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "time_format do warnings"
        );
    }
...
static int test_time_format_warnings_and_diagnostics(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            mylite_result_column_count(result),
            1U,
            "invalid time_format column count"
        );
        failures += mylite_test_expect_size(
            mylite_result_row_count(result),
            1U,
            "invalid time_format row count"
        );
        failures += expect_result_value(result, 0U, 0U, NULL, "invalid time_format value");
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            1U,
            "invalid time_format warnings"
        );
    }
    mylite_result_free(result);
    failures += expect_query(
    ...
    if (failures == 0) {
        ...
        failures += mylite_test_expect_size(
            mylite_result_row_count(result),
            1U,
            "date string time_format row count"
        );
