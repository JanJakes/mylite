    ...
    const struct mylite_dml_mutation_expression_bind_diagnostics *diagnostics)
{
    if (database == NULL || table == NULL || diagnostics == NULL ||
        ...
        diagnostics->set_unsupported_expression_error == NULL) {
        return MYLITE_MISUSE;
    }
    if (expression == NULL) {
        return diagnostics->set_unsupported_clause_error(database);
    }
    ...
    case MYLITE_SQL_AST_QUALIFIED_IDENTIFIER: {
        ...
        if (column_index == table->column_count) {
            ...
            status = diagnostics->set_unknown_column_error(database, reference, clause_context);
            free(reference);
            return status;
        }
        return MYLITE_OK;
    ...
    case MYLITE_SQL_AST_RELEASE_SAVEPOINT_STATEMENT:
        return diagnostics->set_unsupported_expression_error(database, clause_context);
    }
