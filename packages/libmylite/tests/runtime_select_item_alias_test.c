...
static int test_select_alias_length_boundaries(void) {
    ...
    failures +=
        expect_int(make_alias_text(alias64, sizeof(alias64), alias_64_length), 0, "alias64");
    failures +=
        expect_int(make_alias_text(alias65, sizeof(alias65), alias_65_length), 0, "alias65");
    failures +=
        expect_int(make_alias_text(alias256, sizeof(alias256), alias_256_length), 0, "alias256");
    failures +=
        expect_int(make_alias_text(alias257, sizeof(alias257), alias_257_length), 0, "alias257");
    if (failures != 0) {
        return failures;
    }
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_select_item_alias_%d_%s.mylite",
        current_process_id(),
        name
    );
