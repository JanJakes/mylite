...
| `SHOW COLUMNS` / `SHOW FIELDS` | 🟡 | Limited descriptor-driven column listing for persistent base tables; supports `FROM`/`IN`, schema-qualified targets, explicit schema forms, and `LIKE 'pattern'` filters with `Field`, integer, `decimal(M,D)`, `decimal(M,D) unsigned`, `date`, `char(n)`, `varchar(n)`, and baseline `TEXT` family `Type`, `Null`, `Key` including `PRI` for the current single-column primary-key descriptor or first `NOT NULL` unique index when no primary key exists, `UNI` for supported unique-index columns, and `MUL` for supported nonunique secondary-index columns, SQL `NULL`, canonical integer, canonical decimal, or canonical date-string `Default`, and `Extra`, including `auto_increment` for the current auto-increment descriptor and `INVISIBLE` for descriptor-invisible columns; no NUL-producing pattern escapes, `FULL`, `EXTENDED`, `WHERE`, views, privileges, unsupported index metadata, string defaults outside canonical `DATE`, expression defaults, generated hidden columns, or full `INFORMATION_SCHEMA` parity |
...
| `SHOW CREATE TABLE` | 🟡 | Limited descriptor-driven MySQL-style DDL for persistent base tables with current integer-family, exact `DECIMAL`, canonical `DATE`, `CHAR(0..255)`, `VARCHAR(0..255)`, and baseline `TEXT` family descriptors, nullability, implicit nullable-column `DEFAULT NULL` rendering for non-`TEXT` descriptors, dropped-default columns with no default clause, canonical quoted integer, decimal, and date defaults except hidden auto-increment defaults created by `ALTER ... SET DEFAULT`, descriptor-invisible column comments, current single-column primary-key, supported unique/nonunique secondary-index, and auto-increment rendering, fixed InnoDB/utf8mb4 suffix, and fixed enabled `sql_quote_show_create` quoting; no views, temporary tables, unsupported index attributes, string defaults outside canonical `DATE`, expression defaults, named constraints, generated columns, privileges, mutable quote-control state, or disabled rendering |
