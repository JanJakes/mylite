...
static int test_no_source_dual_and_do_greatest_least(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "greatest least do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "greatest least do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "greatest least do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "greatest least do warnings"
        );
    }
