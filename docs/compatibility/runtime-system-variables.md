...
| `sql_warnings` | 🟡 | Limited scalar `SELECT @@sql_warnings` with no scope, `session`, `local`, or `global`; returns MyLite's fixed disabled value `0`; limited fixed no-op `SET` forms may preserve `0`; limited non-strict and `INSERT IGNORE` string truncation plus `INSERT IGNORE` adjustment, duplicate, and supported foreign-key warnings are recorded in diagnostics like MySQL for the documented DML subsets; no mutable global/session state, broader warning-producing DML conversions beyond documented string-truncation and `INSERT IGNORE` subsets, protocol information strings, changed `INSERT` behavior beyond the documented string-truncation and `IGNORE` subsets, or Performance Schema variable tables |
