...
static int test_scalar_predicate_surfaces(void);
static int execute_ok(mylite_db *database, const char *sql);
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected);
static int expect_query_values(mylite_db *database, struct expected_query query);

int main(void) {
    return test_scalar_predicate_surfaces() == 0 ? 0 : 1;
}

static int test_scalar_predicate_surfaces(void) {
    static const char *const between_rows[] = {"1", "alpha", "2", "beta"};
    static const char *const exists_rows[] = {"1"};
    ...
    failures +=
        execute_ok(database, "INSERT INTO t VALUES (1, 'alpha'), (2, 'beta'), (3, 'gamma')");

    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT id, name FROM t WHERE id BETWEEN 1 AND 2 ORDER BY id",
            .values = between_rows,
            .column_count = 2U,
            .row_count = 2U,
            .context = "existing WHERE BETWEEN support",
        }
    );
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT 1 FROM DUAL WHERE EXISTS (SELECT id FROM t WHERE id = 1)",
            .values = exists_rows,
            .column_count = 1U,
            .row_count = 1U,
            .context = "existing WHERE EXISTS support",
        }
    );
