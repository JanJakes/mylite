...
enum {
    ...
    memory_global_total_view_table_usage_row_count = 2,
...
static int test_sys_memory_global_total_views(void);
...
static const char *const memory_global_total_columns[memory_global_total_column_count] = {
    "total_allocated",
};
...
int main(void) {
    return test_sys_memory_global_total_views() == 0 ? 0 : 1;
}
...
static int test_sys_memory_global_total_views(void) {
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .column_names = memory_global_total_columns,
            .column_count = memory_global_total_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys memory global total rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.`x$memory_global_total`",
            .column_names = memory_global_total_columns,
            .column_count = memory_global_total_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x memory global total rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys memory global total row_count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.memory_global_total",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM memory_global_total",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM `x$memory_global_total`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys x memory global total selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM memory_global_total",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = formatted_show_columns_values,
            .row_count = memory_global_total_column_count,
            .context = "sys memory global total show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE `x$memory_global_total`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = raw_show_columns_values,
            .row_count = memory_global_total_column_count,
            .context = "sys x memory global total describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM memory_global_total",
            ...
            .row_count = memory_global_total_column_count,
            .context = "sys memory global total show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM `x$memory_global_total`",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = raw_show_full_columns_values,
            .row_count = memory_global_total_column_count,
            .context = "sys x memory global total show full columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM memory_global_total",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys memory global total show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM `x$memory_global_total`",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x memory global total show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, IS_NULLABLE, "
                   ...
                   "AND TABLE_NAME IN ('memory_global_total', "
                   "'x$memory_global_total') "
                   "ORDER BY TABLE_NAME",
            ...
            .context = "sys memory global total information_schema.columns rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_one,
            .row_count = 1U,
            .context = "sys memory global total information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_two,
            .row_count = 1U,
            .context = "sys memory global total information_schema.columns combined count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys memory global total information_schema.tables rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'memory_global_total'",
            ...
            .row_count = 1U,
            .context = "sys memory global total show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'x$memory_global_total'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = raw_show_table_status_values,
            .row_count = 1U,
            .context = "sys x memory global total show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_SCHEMA, TABLE_NAME, CHECK_OPTION, IS_UPDATABLE, DEFINER, "
                   ...
                   "AND TABLE_NAME IN ('memory_global_total', "
                   "'x$memory_global_total') ORDER BY TABLE_NAME",
            ...
            .row_count = 2U,
            .context = "sys memory global total information_schema.views rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT VIEW_SCHEMA, VIEW_NAME, TABLE_SCHEMA, TABLE_NAME "
                   ...
                   "AND VIEW_NAME IN ('memory_global_total', "
                   "'x$memory_global_total') "
                   "ORDER BY VIEW_NAME",
            ...
            .row_count = memory_global_total_view_table_usage_row_count,
            .context = "sys memory global total view_table_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = NULL,
            .row_count = 0U,
            .context = "sys memory global total empty view_routine_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total empty statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total empty constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total empty key column usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys memory global total empty constraint extensions",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW memory_global_total",
            ...
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `memory_global_total`",
            .context = "sys memory global total show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW `x$memory_global_total`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `x$memory_global_total`",
            .context = "sys x memory global total show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE sys.memory_global_total",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL "
                      "SECURITY INVOKER VIEW `sys`.`memory_global_total`",
            .context = "sys memory global total qualified show create table",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE `x$memory_global_total`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            ...
            .context = "sys x memory global total show create source",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW sys.memory_global_total",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "select format_bytes(sum(`performance_schema`."
                      "`memory_summary_global_by_event_name`.`CURRENT_NUMBER_OF_BYTES_USED`)) AS "
                      "`total_allocated` from `performance_schema`."
                      "`memory_summary_global_by_event_name`",
            .context = "sys memory global total show create expression",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_sys_memory_global_total_views_%d_%s.mylite",
        current_process_id(),
        name
    );
