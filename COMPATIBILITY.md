...
| `ANY` / `SOME` / `ALL` quantified subquery comparisons | ❌ | high | Compare a scalar operand with a one-column subquery using existential or universal three-valued truth semantics. | Parser and AST support already exist, but runtime execution remains deferred. The focused start spec covers the first implementable slice: uncorrelated scalar left operands, one-column subqueries, MySQL-supported quantified comparison operators, current SELECT expression contexts, error 1235 for inner `LIMIT`, error 1241 for multi-column output, warnings, and metadata. See [quantified subquery comparisons spec](docs/specs/quantified-subquery-comparisons/specs.md). |
...
| `IN()` | 🟡 | top | Whether a value is within a set of values | Implemented for expression-list predicates in no-table scalar `SELECT` expressions through MyLite expression evaluation, with parser/AST support and runtime tests for Task 16 verified cases. Task 29 also implements uncorrelated scalar `IN` subqueries in no-table scalar `SELECT` and current table-backed `SELECT` projection, `WHERE`, join `ON`, `HAVING`, and `ORDER BY` contexts. Row expression contexts, full SQL-mode variants, complete collation behavior, broad function/cast interactions, row constructors, correlated subqueries, DML contexts, and quantified forms remain deferred. See [expression operator foundation spec](docs/specs/expression-operator-foundation/specs.md) and [subquery `IN` predicates spec](docs/specs/subquery-in-predicates/specs.md). |
...
| `NOT IN()` | 🟡 | top | Whether a value is not within a set of values | Implemented for expression-list predicates in no-table scalar `SELECT` expressions through MyLite expression evaluation, with parser/AST support and runtime tests for Task 16 verified cases. Task 29 also implements uncorrelated scalar `NOT IN` subqueries in no-table scalar `SELECT` and current table-backed `SELECT` projection, `WHERE`, join `ON`, `HAVING`, and `ORDER BY` contexts. Row expression contexts, full SQL-mode variants, complete collation behavior, broad function/cast interactions, row constructors, correlated subqueries, DML contexts, and quantified forms remain deferred. See [expression operator foundation spec](docs/specs/expression-operator-foundation/specs.md) and [subquery `IN` predicates spec](docs/specs/subquery-in-predicates/specs.md). |
