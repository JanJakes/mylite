...
enum {
    ...
    sql_log_off_value_column_count = 6,
    sql_log_off_label_column_count = 5,
    sql_log_off_diagnostics_column_count = 4,
    sql_log_off_selected_column_count = 2,
    sql_log_off_independent_column_count = 3,
...
static int test_sql_log_off_values_and_persistence(void);
static int test_sql_log_off_qualifiers_and_errors(void);
static int test_independent_sql_log_off_handles(void);
...
int main(void) {
    ...
    failures += test_sql_log_off_values_and_persistence();
    failures += test_sql_log_off_qualifiers_and_errors();
    failures += test_independent_sql_log_off_handles();
...
static int test_sql_log_off_values_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open sql log off file");
    ...
    failures += execute_ok(
        ...
        "@@local.sql_log_off, @@warning_count, ROW_COUNT() FROM DUAL",
        &result
    );
    ...
    failures += execute_ok(
        ...
        "@@session.`sql_log_off`, @@`sql_log_off`, (@@sql_log_off)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_log_off_label_column_count,
            .context = "sql log off labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_log_off, @@sql_log_bin, @@foreign_key_checks, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql log off scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_log_off_diagnostics_column_count,
            .context = "sql log off warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql log off clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_log_off_diagnostics_column_count,
            .context = "sql log off error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql log off clears errors");
    failures += expect_show_count_warnings(database, "0", "sql log off clears error warnings");

    session = mylite_connection_session_state(database);
    failures += mylite_test_expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql log off reads"
    );
    failures += mylite_test_expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql log off reads"
    );
    failures += mylite_test_expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql log off preamble"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_log_off, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_log_off_selected_column_count,
            .context = "sql log off with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql log off does not alter descriptor select",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "explicit descriptor select limit still applies with sql log off",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql log off does not reject descriptor count",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql log off does not reject descriptor select",
        }
    );
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql log off file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_log_off, @@global.sql_log_off",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql log off values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql log off table rows",
        }
    );
...
static int test_sql_log_off_qualifiers_and_errors(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql log off memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_LOG_OFF, @@SESSION.SQL_LOG_OFF, "
        "@@Local.Sql_Log_Off, @@global.`sql_log_off`, "
        "(@@sql_log_off)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_log_off_label_column_count,
            .context = "sql log off qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        database,
        "SELECT @@no_such_sql_log_off_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_log_off_variable'",
        }
    );
    failures += execute_error(
        database,
        "SELECT @@global.no_such_sql_log_off_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_log_off_variable'",
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_log_off_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_log_off_variable'",
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_log_off + 1");
...
static int test_independent_sql_log_off_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first sql log off handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second sql log off handle"
    );
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_log_off_independent_column_count,
            .context = "first handle sql log off variables",
        }
    );
    mylite_result_free(result);
    result = NULL;

    failures += execute_ok(second, "SELECT @@sql_log_off, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_log_off_independent_column_count,
            .context = "second handle sql log off variables",
        }
    );
    mylite_result_free(result);
