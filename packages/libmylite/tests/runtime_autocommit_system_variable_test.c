...
static int test_autocommit_system_variable_values_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open autocommit file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen autocommit file");
...
static int test_autocommit_system_variable_qualifiers_and_errors(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open autocommit memory");
    failures += execute_ok(
        database,
...
static int test_autocommit_session_assignment_and_transactions(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "mutable") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open mutable autocommit");
    failures += execute_statement_ok(database, "CREATE DATABASE app");
    failures += execute_statement_ok(database, "USE app");
...
static int test_independent_autocommit_system_variable_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first autocommit handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second autocommit handle"
    );
