...
static int test_create_table_like_success_persistence_and_preamble(void) {
    ...
    failures += mylite_test_expect_uint64(
        clone_table.descriptor_version,
        1U,
        "clone table descriptor version"
    );
    failures += mylite_test_expect_text(
        clone_table.default_charset,
        source_table.default_charset,
        "clone charset"
    );
    failures += mylite_test_expect_text(
...
static int test_create_table_like_diagnostics(void) {
    ...
    failures +=
        mylite_test_expect_true(sqlite != NULL, "read SQLite handle for future descriptor test");
    if (sqlite != NULL) {
...
static int test_create_table_like_after_source_rename_and_drop(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "source_rename_drop") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open rename/drop file");
    failures += execute_statement_ok(database, "CREATE DATABASE app");
    failures += execute_statement_ok(database, "CREATE TABLE app.src (id INT)");
...
static int test_create_table_like_physical_create_failure_rolls_back_catalog(void) {
    ...
    if (catalog != NULL) {
        failures += mylite_test_expect_uint64(
            catalog->generation,
            3U,
            "failed create-like leaves catalog generation"
        );
    }
...
static int execute_error(mylite_db *database, const char *sql, struct expected_sql_error expected) {
    ...
    failures += mylite_test_expect_true(result == NULL, "failed statement leaves result null");
    mylite_result_free(result);
...
static int expect_create_like_ok(mylite_db *database, const char *sql) {
    ...
    failures += mylite_test_expect_int64(
        ...
        0,
        "create-like affected rows"
    );
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "create-like warning count"
    );
    mylite_result_free(result);
    return failures;
}
...
static int expect_column_matches(
    ...
    failures += mylite_test_expect_text(column.logical_type, expected.logical_type, expected.name);
    failures += mylite_test_expect_text(column.physical_type, "INTEGER", expected.name);
    ...
    failures +=
        mylite_test_expect_int((int)column.default_kind, (int)expected.default_kind, expected.name);
    failures +=
        mylite_test_expect_int64(column.default_integer, expected.default_integer, expected.name);
