...
static int test_set_connection_character_set_success_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open set charset file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen set charset file");
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT @@character_set_client, @@character_set_connection, "
...
static int expect_set_ok(mylite_db *database, const char *sql) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "set charset column count");
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "set charset row count");
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        0,
        "set charset affected rows"
    );
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "set charset warning count"
    );
    mylite_result_free(result);
