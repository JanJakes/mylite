...
static int test_information_schema_innodb_ft_config_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb ft config db");
...
static int test_information_schema_innodb_ft_config_reopen_preamble_and_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first ft config db"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second ft config db"
    );
    failures += expect_query(
        first,
        (struct expected_query){
