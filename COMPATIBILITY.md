...
`ADD (...)` descriptor arrays. Column, key, and DDL descriptors include CST node
anchors for the next semantic AST builder, but the parser does not yet build the
final typed MyLite AST and does not imply runtime support for the statement rows
below. The current prototype parses the WordPress MySQL server query corpus with
...
| `LOAD DATA INFILE` | ❌ | high | Server-side text import syntax, field/line options, user variables, SET clause, warnings, and security restrictions. | Parser prototype exposes decoded file/table/charset metadata, duplicate modifier, partition marker, field and line clauses, ignored-row count, column/user-variable list, SET assignments with recursive expression views, and LOAD DATA `WITH` options. Runtime import and diagnostics are not implemented yet. |
| `LOAD DATA LOCAL INFILE` | ❌ | high | Client-side LOCAL INFILE request flow, security controls, warnings, and protocol interaction. | Parser prototype records the `LOCAL` modifier and shares the `LOAD DATA` parser-view metadata. Protocol flow, file access policy, and runtime warnings are not implemented yet. |
| `LOAD XML INFILE` | ❌ | low | Server-side XML import syntax, row matching, namespaces, SET clause, warnings, and security restrictions. | Parser prototype exposes decoded file/table/charset metadata, duplicate modifier, XML row tag, ignored-row count, column/user-variable list, and SET assignments with recursive expression views. Runtime XML import and diagnostics are not implemented yet. |
| `LOAD XML LOCAL INFILE` | ❌ | low | Client-side XML import request behavior and embedded-compatible diagnostics. | Parser prototype records the `LOCAL` modifier and shares the `LOAD XML` parser-view metadata. Protocol flow, file access policy, and runtime warnings are not implemented yet. |
...
| `SET DEFAULT ROLE` | ❌ | medium | Default role assignment. | Parser prototype exposes a typed role-state view with role option kind, decoded role names/hosts, and decoded target users. Runtime default-role metadata updates, privilege checks, warnings, and diagnostics are not implemented yet. |
...
| `SET ROLE` | ❌ | medium | Active-role selection. | Parser prototype exposes a typed role-state view with role option kind (`DEFAULT`, `NONE`, `ALL`, `ALL EXCEPT`, or explicit role list) and decoded role names/hosts. Runtime active-role state, privilege checks, warnings, and diagnostics are not implemented yet. |
