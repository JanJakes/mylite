# Baseline sys.host_summary_by_statement_type Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.host_summary_by_statement_type` and
`sys.x$host_summary_by_statement_type` views. MySQL uses these sys views to
summarize statement events grouped by Performance Schema host and statement
event type. MyLite does not collect Performance Schema statement summaries, so
both views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.host_summary_by_statement_type` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_host_summary_by_statement_type_views_expectations.sh`.
...
Schema statement-summary counters.
...
SELECT * FROM `x$host_summary_by_statement_type`;
```

Both views return zero rows. This differs from a live MySQL server with
Performance Schema host statement-summary rows, but preserves the queryable sys
view surface without inventing host names, statement event types, statement
counts, latency values, row counters, or full-scan totals.

## Column Metadata

`sys.host_summary_by_statement_type` has eleven columns:

| Column | Type | Null | Default | Collation |
| --- | --- | --- | --- | --- |
| `host` | `varchar(255)` | `YES` | `NULL` | `ascii_general_ci` |
| `statement` | `varchar(128)` | `YES` | `NULL` | `utf8mb4_0900_ai_ci` |
...
| `lock_latency` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `cpu_latency` | `varchar(11)` | `YES` | `NULL` | `utf8mb3_general_ci` |
| `rows_sent` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `rows_examined` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `rows_affected` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `full_scans` | `bigint unsigned` | `NO` | `0` | SQL `NULL` |

`sys.x$host_summary_by_statement_type` has the same first three columns with
raw numeric latency metadata:
...
| `lock_latency` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `cpu_latency` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `rows_sent` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `rows_examined` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `rows_affected` | `bigint unsigned` | `NO` | `NULL` | SQL `NULL` |
| `full_scans` | `bigint unsigned` | `NO` | `0` | SQL `NULL` |
...
`INFORMATION_SCHEMA.COLUMNS` expose this shape, including character
length/octet metadata and unsigned `BIGINT` numeric precision. The views have
no index or constraint metadata, so `SHOW INDEX`,
...
uses `format_pico_time()` for total, maximum, lock, and CPU latency; the raw
`x$` view exposes the corresponding raw timer values. Both definitions filter
out rows with zero total wait time and order by host and descending total wait
time.
...
- Performance Schema statement-summary collection;
- live host/statement-type rows, statement counts, latency totals, row
  counters, or full-scan totals;
- execution of `format_pico_time()` or `substring_index()`;
...
Performance Schema statement counters.
