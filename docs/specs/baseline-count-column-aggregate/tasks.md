...
3. Parser and AST
   - [x] Add an AST node for one-column `COUNT()` aggregate functions.
   - [x] Extend Lemon grammar for no-space `COUNT(identifier)`, including
   ...
   - [x] Add parser tests for supported count-column syntax and unsupported
...
5. Physical SQLite aggregate
   - [x] Generate SQLite `SELECT COUNT("physical_column") FROM
...
6. Tests
   ...
   - [x] Cover supported count-column behavior over existing descriptor integer
...
8. Verification and review
   ...
   - [x] Run the count aggregate CTest entry and relevant parser/min-max/select
     lifecycle entries.
   - [x] Run
