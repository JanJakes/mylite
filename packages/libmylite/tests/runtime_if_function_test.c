...
static int test_if_function_unsupported_forms(void) {
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id FROM t WHERE id = IF(1,1,0)",
            .columns = (const char *const[]){"id"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "IF column predicate RHS",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id FROM t ORDER BY IF(1,id,id)",
            .columns = (const char *const[]){"id"},
            .column_count = 1U,
            .values = (const char *const[]){"1"},
            .row_count = 1U,
            .context = "IF order expression",
        }
    );
    failures += execute_error(
        database,
