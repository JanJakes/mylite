...
static int test_information_schema_static_catalogs(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &database), MYLITE_OK, "open first db");
    if (database == NULL) {
        remove_related_files(first_path);
        remove_related_files(second_path);
        return failures + 1;
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &database), MYLITE_OK, "reopen first db");
    if (database != NULL) {
        failures += expect_query(
            database,
            (struct expected_query){
