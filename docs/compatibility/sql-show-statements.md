...
| `SHOW INDEX` / `SHOW INDEXES` / `SHOW KEYS` | 🟡 | Limited descriptor-resolved persistent base-table introspection with MySQL 8.4.9 column labels, zero rows for no-index tables, one `PRIMARY` BTREE row for the current single-column primary-key descriptor, and one BTREE row per supported secondary index; no unique/fulltext/spatial/composite/prefix/functional/descending index metadata, `EXTENDED`, `WHERE`, temporary tables, views, privileges, or real storage statistics |
