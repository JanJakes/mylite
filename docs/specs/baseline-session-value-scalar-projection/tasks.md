...
- [x] Replace the split session-scalar and scalar-value projection admission
  checks with one mixed scalar projection classifier while keeping one shared
  executor.
- [x] Admit mixed no-source and `FROM DUAL` scalar projection over the current
  session-scalar and scalar-value subsets.
- [x] Preserve scalar value function operand restrictions, especially rejection
  of session/system variables inside `IF()`/`IFNULL()`/`COALESCE()`/`NULLIF()`/
  `ISNULL()`.
- [x] Preserve warning-count and row-count ordering for mixed projections,
  including warning-producing `@@sql_slave_skip_counter` reads.
- [x] Add runtime lifecycle tests for values, labels, aliases, warnings, row
