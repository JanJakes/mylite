...
| `SHOW BINLOG EVENTS` | 🟡 | Limited placeholder exposes MySQL 8.4.9 column labels and one synthetic `Format_desc` event for `binlog.000001`, with `IN`, unsigned `FROM`, and unsigned `LIMIT` filtering over that row plus MySQL-shaped missing-log diagnostics; no live event stream, binary log files, filters beyond the documented options, privileges, or mutable replication state |
| `SHOW CHARACTER SET` / `SHOW CHARSET` | ✅ | MySQL-runtime-verified metadata-only catalog rows for all 41 target character sets with MySQL column labels, warning behavior, `LIKE 'pattern'`, and documented `WHERE` filters; DDL admission, conversion, comparison semantics, and privileges are tracked separately |
| `SHOW COLLATION` | ✅ | MySQL-runtime-verified metadata-only catalog rows for all 286 target collations with MySQL column labels, warning behavior, `LIKE 'pattern'`, and documented `WHERE` filters; DDL admission, conversion, comparison semantics, and privileges are tracked separately |
...
| `SHOW COUNT(*) ERRORS` | ✅ | MySQL-runtime-verified previous-statement error-condition count with MySQL 8.4.9 column label `@@session.error_count` for MyLite's retained diagnostics snapshot, including retained error warning rows and current `max_error_count=0` parse-error snapshot suppression behavior; full diagnostics stacks and `GET DIAGNOSTICS` are tracked separately |
| `SHOW COUNT(*) WARNINGS` | ✅ | MySQL-runtime-verified previous-statement diagnostics count with MySQL 8.4.9 column label; counts MyLite previous error conditions and total current warning/note/error records even when `@@max_error_count` caps retained rows, honors `sql_notes=0` for current note producers, and preserves the documented missing-schema counted-only result behavior; full diagnostics stacks and `GET DIAGNOSTICS` are tracked separately |
| `SHOW CREATE DATABASE` / `SHOW CREATE SCHEMA` | 🟡 | Limited descriptor-driven schema DDL rendering with MySQL 8.4.9 columns, stored schema default charset/collation metadata, fixed default-encryption text, and session `sql_quote_show_create` quote-control for simple schema identifiers; `binary` schema defaults render without an explicit `COLLATE binary` clause; no `IF NOT EXISTS`, encryption mutation, privileges, or system schemas |
| `SHOW CREATE EVENT` | 🟡 | Baseline syntax plus MySQL-shaped no-database and missing-event diagnostics; no event catalog, scheduler, object DDL, result rows for existing events, or privileges. See [baseline SHOW CREATE residuals](../specs/baseline-show-create-residuals/specs.md) |
| `SHOW CREATE FUNCTION` | 🟡 | Baseline syntax plus MySQL-shaped no-database and missing stored-function diagnostics; no stored-function or loadable-function catalog, object DDL, result rows for existing functions, or privileges. See [baseline SHOW CREATE residuals](../specs/baseline-show-create-residuals/specs.md) |
...
| `SHOW CREATE TABLE` | 🟡 | Limited descriptor-driven MySQL-style DDL for persistent base tables, shadowing session temporary tables, baseline metadata-only views, and documented synthetic `sys` views. Base and temporary tables render the supported column, key, constraint, default, generated-column, auto-increment, charset/collation, comment, storage/statistics, fixed InnoDB suffix, and session `sql_quote_show_create` quote-control subset; unsupported index/default/type/table-option details remain omitted. View targets render pre-rendered MySQL-shaped `CREATE VIEW` metadata from stored descriptors or supported synthetic sys definitions; no view execution, updatable views, privileges, or quote-control rewriting for those pre-rendered definitions |
| `SHOW CREATE TRIGGER` | 🟡 | Baseline syntax plus MySQL-shaped no-database, unknown-schema, and missing-trigger diagnostics; no trigger catalog, trigger execution, result rows for existing triggers, or privileges. See [baseline SHOW CREATE residuals](../specs/baseline-show-create-residuals/specs.md) |
| `SHOW CREATE USER` | 🟡 | Baseline embedded `root@%` result for current/root account forms plus MySQL-shaped unknown-account diagnostics; no account store, password hash policy, role/grant expansion, privilege-sensitive redaction, or `print_identified_with_as_hex`. See [baseline SHOW CREATE residuals](../specs/baseline-show-create-residuals/specs.md) |
...
| `SHOW ENGINE` | 🟡 | Limited to `SHOW ENGINE InnoDB STATUS` and `SHOW ENGINE InnoDB LOGS`; non-`InnoDB` engine names are rejected by MyLite's embedded InnoDB-only policy; no Performance Schema engine status, alternate engines, filters, privileges, `MUTEX`, or live engine internals |
| `SHOW ENGINE LOGS` | ✅ | MySQL-runtime-verified `SHOW ENGINE InnoDB LOGS` exposes MySQL 8.4.9 columns `Type`, `Name`, and `Status` with zero rows; no live log rows, non-`InnoDB` engines, filters, privileges, or mutable engine state |
...
| `SHOW ENGINE STATUS` | ✅ | MySQL-runtime-verified `SHOW ENGINE InnoDB STATUS` exposes MySQL 8.4.9 columns `Type`, `Name`, and `Status` with one synthetic `InnoDB` row and stable MyLite-owned status text; no live InnoDB monitor output, non-`InnoDB` status rows, filters, privileges, or mutable engine state |
| `SHOW ENGINES` | ✅ | MySQL-runtime-verified `SHOW [STORAGE] ENGINES` exposes one embedded InnoDB default row with MySQL 8.4.9 column labels, unsupported-modifier diagnostics, and row-count state; related `INFORMATION_SCHEMA.ENGINES` is a separate limited one-row synthetic catalog; no alternate engines, filters, privileges, or plugins |
| `SHOW ERRORS` | ✅ | MySQL-runtime-verified previous-statement error-condition rows with `Level`, `Code`, and `Message`, unsigned decimal `LIMIT` slicing, diagnostic-statement preservation, and retained MyLite error rows; full diagnostics stacks and multiple stored-program errors are tracked separately |
| `SHOW EVENTS` | ✅ | MySQL-runtime-verified empty event introspection with MySQL 8.4.9 column labels, `LIKE 'pattern'` filters, and documented empty-result `WHERE` predicates; unknown explicit schemas are empty successes; related empty `INFORMATION_SCHEMA.EVENTS` metadata is queryable, but there are no NUL-producing pattern escapes, event descriptors, event rows, event DDL, `SHOW CREATE EVENT`, non-empty row predicate evaluation, Event Scheduler, or privileges |
| `SHOW FUNCTION CODE` | ⚪ | Parsed and rejected with stored-program unsupported diagnostics; no routine bytecode or debugger rows |
| `SHOW FUNCTION STATUS` | ✅ | MySQL-runtime-verified empty stored-function introspection with MySQL 8.4.9 column labels and `LIKE 'pattern'` or documented empty-result `WHERE` filters; global and default-schema independent; related empty `INFORMATION_SCHEMA.ROUTINES` metadata is queryable, but there are no NUL-producing pattern escapes, routine descriptors, routine rows, routine DDL, `SHOW CREATE FUNCTION`, or privileges |
...
| `SHOW MASTER STATUS` | ⚪ | Rejected as removed MySQL 8.4 syntax; use `SHOW BINARY LOG STATUS` |
| `SHOW OPEN TABLES` | ✅ | MySQL-runtime-verified embedded empty open-table introspection with MySQL 8.4.9 column labels, `LIKE 'pattern'` filters, and documented empty-result `WHERE` predicates; unknown explicit schemas are empty successes; no NUL-producing pattern escapes, table-cache rows, `In_use` counts, `Name_locked` state, temporary tables, `HANDLER`, table locks, non-empty row predicate evaluation, privileges, or performance-schema metadata |
| `SHOW PARSE_TREE` | ⚪ | Rejected in production-compatible builds; MySQL documents this as debug/development-only parse-tree output and the target MySQL 8.4.9 runtime rejects it |
| `SHOW PLUGINS` | ✅ | MySQL-runtime-verified one-row result for MyLite's embedded active `InnoDB` storage-engine plugin surface with MySQL 8.4.9 column labels, syntax diagnostics for unsupported clauses, warning behavior, and result-set row-count state; related `INFORMATION_SCHEMA.PLUGINS` is a separate limited synthetic catalog; no loadable plugins, disabled plugin rows, alternate engines, or complete MySQL plugin inventory |
| `SHOW PRIVILEGES` | ✅ | MySQL-runtime-verified static MySQL 8.4.9-shaped privilege-name catalog with columns `Privilege`, `Context`, and `Comment`, the observed 73 target-runtime rows, syntax diagnostics for unsupported clauses, warning behavior, and result-set row-count state; no account storage, grant descriptors, privilege enforcement, or mutable dynamic privileges |
| `SHOW PROCEDURE CODE` | ⚪ | Parsed and rejected with stored-program unsupported diagnostics; no routine bytecode or debugger rows |
| `SHOW PROCEDURE STATUS` | ✅ | MySQL-runtime-verified empty stored-procedure introspection with MySQL 8.4.9 column labels and `LIKE 'pattern'` or documented empty-result `WHERE` filters; global and default-schema independent; related empty `INFORMATION_SCHEMA.ROUTINES` metadata is queryable, but there are no NUL-producing pattern escapes, persistent routine descriptors, routine rows, session-local procedure bridge rows, or privileges |
| `SHOW PROCESSLIST` embedded handle row | ✅ | MySQL-runtime-verified current embedded-handle row for `SHOW [FULL] PROCESSLIST` with MySQL 8.4.9 columns, selected-schema `db`, `Info` truncation, deprecation warning, unsupported-clause diagnostics, and result-set row-count state |
| Server-wide processlist instrumentation | 🟡 | MyLite does not expose server-wide threads, sleeping/background rows, privileges, Performance Schema process-list state, `KILL`, or live thread timing/instrumentation; related limited `INFORMATION_SCHEMA.PROCESSLIST` and sys process-list metadata are tracked separately |
...
| `SHOW RELAYLOG EVENTS` | 🟡 | Limited no-replication empty result set with MySQL 8.4.9 column labels `Log_name`, `Pos`, `Event_type`, `Server_id`, `End_log_pos`, and `Info`; accepts `IN`, integer `FROM`, integer `LIMIT` with offset forms, and `FOR CHANNEL ''`; named channels return `3074 / HY000`; no live relay logs, event rows, privilege filtering, relay-log state, non-empty channels, or `WHERE`/`FULL` forms |
...
| `SHOW STATUS` | 🟡 | Limited runtime-owned status registry with MySQL 8.4.9 column labels, optional `GLOBAL`/`SESSION`/`LOCAL` scope, `LIKE 'pattern'` filters, limited trailing `WHERE` predicates over `Variable_name` and `Value`, the target MySQL 8.4.9 `Bytes_%`, `Connections`, `Compression%`, `Prepared_stmt_count`, `Queries`, `Questions`, `Threads_%`, `Uptime%`, `Binlog_%`, `Com_%`, `Created_%`, `Current_tls%`, `Delayed_%`, `Deprecated%`, `Error_log%`, `Handler_%`, `Innodb%`, `Key_%`, `Last_query_%`, `Max_%`, `Mysqlx%`, `Open%`, `Performance_schema_%`, `Select_%`, `Slow_%`, `Sort_%`, `Ssl_%`, `Table_locks_%`, `Table_open_cache_%`, `Tc_log_%`, `Telemetry_%`, `Tls%`, RSA public-key rows, and related server/legacy scalar surfaces plus `Connection_%` diagnostic placeholders; values are embedded placeholders and do not implement live counters, TLS state, network compression, error-log state, InnoDB instrumentation, X Plugin, arbitrary `WHERE` expressions, Performance Schema instrumentation, privileges, or full status-variable coverage |
...
| `SHOW TRIGGERS` | ✅ | MySQL-runtime-verified schema-resolved trigger introspection with MySQL 8.4.9 column labels, optional `FULL`, and `LIKE 'pattern'` filters over table names; returns metadata-only rows for the two built-in `sys.sys_config` triggers and empty user rows otherwise; no NUL-producing pattern escapes, user-created trigger descriptors, trigger execution, trigger DDL, `SHOW CREATE TRIGGER`, evaluated `WHERE` predicates, or privileges |
| `SHOW VARIABLES` | 🟡 | Limited runtime-owned rows for MyLite's current system-variable registry with MySQL 8.4.9 column labels, optional `GLOBAL`/`SESSION`/`LOCAL` scope, `LIKE 'pattern'` filters, limited `WHERE` predicates over `Variable_name` and `Value`, session-local readback for the currently mutable variables, and verified absence for target-build-absent optional/plugin variables; no full MySQL variable catalog, arbitrary expressions, warning-producing numeric filter coercions, privileges, persisted variables, Performance Schema variable-info, persisted-variable, or thread-variable tables, or mutable global state |
| `SHOW WARNINGS` | ✅ | MySQL-runtime-verified previous-statement diagnostics rows with `Level`, `Code`, and `Message`, unsigned decimal `LIMIT` slicing, diagnostic-statement preservation, retained MyLite error/warning/note records, `@@max_error_count` retained-row capping, `sql_notes=0` suppression, and documented missing-schema counted-only result behavior; full diagnostics stacks and `GET DIAGNOSTICS` are tracked separately |
