...
- [ ] Align remaining numeric/string casts and coercions in strict and
      ...
      `YEAR`/`JSON` target slices, and conversion/truncation `IGNORE` demotion
      beyond
      ...
      bytes. Four-byte UTF-8 assigned to `utf8mb3` `VARCHAR` and `TEXT`
...
- [ ] Complete result column metadata for expressions that currently report no
      ...
      `JSON_QUOTE()` / plain-string `JSON_UNQUOTE()` lengths, long-blob
      metadata for LONGTEXT-sized JSON quote/unquote results, and long-blob
      ...
      Scalar subquery result metadata now clears base-column key/default flags,
      keeps numeric `BINARY|NUM` flags, and marks selected values nullable even
      when the selected column is `NOT NULL`.
