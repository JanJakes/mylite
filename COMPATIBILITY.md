...
| `SQL_CALC_FOUND_ROWS` select modifier | 🟡 | Limited to non-distinct descriptor-backed single-base-table column-list and wildcard `SELECT` paths with optional source alias, baseline `WHERE`, one-column `ORDER BY`, and existing `LIMIT`/`OFFSET`; records deprecation warning 1287 and updates `FOUND_ROWS()` state to the pre-limit matched row count; no scalar/`DUAL`, `DISTINCT`, aggregate, grouped, CTAS, `INSERT ... SELECT`, `REPLACE ... SELECT`, joins, `UNION`, `TABLE`, CTEs, subqueries, lock clauses, replication, or protocol metadata. | [SQL query expressions](docs/compatibility/sql-query-expressions.md), [system functions](docs/compatibility/functions-system.md) |
...
| `INSERT ... SELECT` | 🟡 | Limited descriptor-backed `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [INTO] target [(columns)] SELECT ... FROM source` over persistent base tables, reusing the existing source `SELECT` column/alias/WHERE/ORDER/LIMIT subset and descriptor-driven target defaults/nullability/range checks, with no-op priority modifiers and delayed deprecation warning; no `IGNORE`, partitions, joins, expressions, literal projection sources, `TABLE`, row constructors, `ON DUPLICATE KEY UPDATE`, keys, auto-increment, triggers, or warning demotion. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `REPLACE ... SELECT` | 🟡 | Limited no-key-descriptor `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] target [(columns)] SELECT ... FROM source` insert-equivalent path for persistent base tables, reusing the existing source `SELECT` subset and descriptor-driven target defaults/nullability/range checks; duplicate-key delete-insert semantics await primary/unique key descriptors, and no partitions, `TABLE`, joins, literal/expression projection sources, generated values, warning demotion, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
