...
| Subpartitioning | ❌ | low | HASH/KEY subpartition syntax and metadata. | Parser recognizes `SUBPARTITION BY HASH/KEY` method tails and `SUBPARTITIONS` counts using the same method-list validators, and validates explicit subpartition definition lists with non-empty comma-separated `SUBPARTITION name` entries plus partition-option value checks. |
