...
| `SELECT` | ❌ | top | Full query expression surface; see section 2. | Parser records ordinary `SELECT` and `WITH` query statements as query targets and validates nonempty projection lists plus major clause shells for direct, parenthesized, nested, and set-operation RHS `SELECT` terms; query expression AST, table-reference detail, expression semantics, result metadata, and execution semantics are not implemented. |
