# Baseline mysql.innodb_table_stats Tasks

- [x] Verify MySQL 8.4.9 runtime table shape, metadata, stable built-in rows,
      and user-table statistics behavior.
...
- [x] Implement `SELECT` routing for `mysql.innodb_table_stats` and
      unqualified reads after `USE mysql`.
- [x] Synthesize built-in and descriptor-backed table-stat rows.
