# Baseline IFNULL Function Tasks

- [x] Read current compatibility, scalar select, literal projection, `IF()`,
  parser, runtime, result, storage, and test context.
- [x] Research official MySQL 8.4 `IFNULL()` / flow-control documentation.
- [x] Probe MySQL 8.4.9 runtime behavior for non-`NULL` first values, `NULL`
  fallback values, booleans, integer values, labels, nested calls, aliases,
  `FROM DUAL`, warnings, row count, wrong arity, and broader deferred forms.
...
- [x] Add lexer/parser/AST support and parser tests for `IFNULL()`.
- [x] Extend the no-source/`FROM DUAL` scalar projection runtime to evaluate
  supported `IFNULL()` expressions without SQLite SQL.
