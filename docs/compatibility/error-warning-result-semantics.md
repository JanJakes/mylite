...
| IGNORE warning demotion | 🟡 | Limited `INSERT IGNORE ... VALUES` / `SET` demotion for descriptor integer and `NULL` row inputs: explicit `NULL` into numeric `NOT NULL`, omitted no-explicit-default columns, and out-of-range descriptor integer inputs become warnings with adjusted values; omitted numeric `NOT NULL` columns store `0`, while omitted nullable dropped-default columns store `NULL`; no duplicate-key discard, broader type conversion, `INSERT IGNORE ... SELECT`, `UPDATE IGNORE`, `DELETE IGNORE`, or `LOAD DATA IGNORE` demotion |
