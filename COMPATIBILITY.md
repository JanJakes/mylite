...
| `DO` | ❌ | medium | Expression execution with warning and error semantics. | Parser recognizes comma-separated expression lists and rejects dangling separators, dangling operators, malformed nested expression groups, invalid `DEFAULT` values, adjacent top-level operands, and top-level query-clause tails such as `FROM`/`WHERE`/`ORDER BY`/`LIMIT`. |
