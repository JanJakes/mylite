...
| `CASE` | ❌ | top | Simple and searched SQL CASE expression semantics, short-circuit evaluation, metadata, warnings/errors, and supported scalar expression contexts. | Design completed for SQL CASE expressions, distinct from stored-program `CASE` statements, including MySQL 8.4.9 runtime probes, MyLite Lemon-style grammar, AST/runtime/metadata plan, and implementation test expectations for no-table `SELECT`, one-table `SELECT` projection/`WHERE`/`ORDER BY`, `UPDATE` assignment/predicate/order, and `DELETE` predicate/order. Runtime implementation and `INSERT` expression paths remain deferred. See [CASE expression spec](docs/specs/case-expression/specs.md). |
