...
| `ANALYZE TABLE` | ❌ | high | Statistics refresh, histogram update/drop, validation, and result-set metadata. | Parser recognizes table lists and single-table histogram update/drop clauses, including numeric histogram bucket counts, automatic/manual histogram update modes, and single-column text-string histogram data with quoted hex/bit rejection, using the shared identifier grammar for tables and columns. |
