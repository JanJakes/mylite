...
enum {
    ...
    information_schema_columns_row_count = 24,
    ...
    sampled_show_full_columns_row_count = 6,
    host_summary_view_table_usage_row_count = 8,
...
static int test_sys_host_summary_views(void);
...
static const char *const sys_host_summary_columns[sys_host_summary_column_count] = {
    "host",
    "statements",
    "statement_latency",
...
int main(void) {
    return test_sys_host_summary_views() == 0 ? 0 : 1;
}
...
static int test_sys_host_summary_views(void) {
    ...
    static const char *const count_twenty_four[] = {"24"};
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.host_summary",
            .column_names = sys_host_summary_columns,
            .column_count = sys_host_summary_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.`x$host_summary`",
            .column_names = sys_host_summary_columns,
            .column_count = sys_host_summary_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x host summary rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys host summary row_count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.host_summary",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.`x$host_summary`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys x host summary count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM host_summary",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM `x$host_summary`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys x host summary selected count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM host_summary",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = formatted_show_columns_values,
            .row_count = sys_host_summary_column_count,
            .context = "sys host summary show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM `x$host_summary`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = raw_show_columns_values,
            .row_count = sys_host_summary_column_count,
            .context = "sys x host summary show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE host_summary",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = formatted_show_columns_values,
            .row_count = sys_host_summary_column_count,
            .context = "sys host summary describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM host_summary "
                   "WHERE Field IN ('host', 'statement_latency', "
                   "'statement_avg_latency', 'file_io_latency', 'current_memory', "
                   "'total_memory_allocated')",
            ...
            .row_count = sampled_show_full_columns_row_count,
            .context = "sys host summary show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM `x$host_summary` "
                   "WHERE Field IN ('host', 'statement_latency', "
                   "'statement_avg_latency', 'file_io_latency', 'current_memory', "
                   "'total_memory_allocated')",
            .column_names = show_full_columns_columns,
            ...
            .row_count = sampled_show_full_columns_row_count,
            .context = "sys x host summary show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM host_summary",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM `x$host_summary`",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys x host summary show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, IS_NULLABLE, "
                   ...
                   "AND ((TABLE_NAME = 'host_summary') OR "
                   "(TABLE_NAME = 'x$host_summary'))",
            ...
            .context = "sys host summary information_schema.columns rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_twelve,
            .row_count = 1U,
            .context = "sys host summary information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_twelve,
            .row_count = 1U,
            .context = "sys x host summary information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS "
                   ...
                   "((TABLE_NAME = 'host_summary') OR (TABLE_NAME = 'x$host_summary'))",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_twenty_four,
            .row_count = 1U,
            .context = "sys host summary information_schema.columns combined count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys host summary information_schema.tables rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'host_summary'",
            ...
            .row_count = 1U,
            .context = "sys host summary show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name = 'x$host_summary'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = raw_show_table_status_values,
            .row_count = 1U,
            .context = "sys x host summary show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_SCHEMA, TABLE_NAME, CHECK_OPTION, IS_UPDATABLE, DEFINER, "
                   ...
                   "AND ((TABLE_NAME = 'host_summary') OR (TABLE_NAME = 'x$host_summary')) "
                   "ORDER BY TABLE_NAME",
            ...
            .row_count = 2U,
            .context = "sys host summary information_schema.views rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT VIEW_SCHEMA, VIEW_NAME, TABLE_SCHEMA, TABLE_NAME "
                   ...
                   "AND ((VIEW_NAME = 'host_summary') OR (VIEW_NAME = 'x$host_summary'))",
            ...
            .row_count = host_summary_view_table_usage_row_count,
            .context = "sys host summary view_table_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = NULL,
            .row_count = 0U,
            .context = "sys host summary empty view_routine_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary empty statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary empty constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary empty key column usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys host summary empty constraint extensions",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW host_summary",
            ...
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `host_summary`",
            .context = "sys host summary show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW `x$host_summary`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `x$host_summary`",
            .context = "sys x host summary show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE sys.host_summary",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=TEMPTABLE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `sys`.`host_summary`",
            .context = "sys host summary qualified show create table",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE `x$host_summary`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "from (((`performance_schema`.`accounts` join "
                      "`x$host_summary_by_statement_latency` `stmt`",
            .context = "sys x host summary show create source",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_sys_host_summary_views_%d_%s.mylite",
        current_process_id(),
        name
    );
