...
static size_t mylite_semantic_ast_count_statement_sources(
    const MyliteAst *parser_ast, size_t statement_index);
static size_t mylite_semantic_ast_count_statement_tables(
    const MyliteAst *parser_ast, size_t statement_index);
...
static int mylite_semantic_ast_fill_statement_sources(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *statement,
    const MyliteAst *parser_ast, size_t statement_index, size_t *child_index);
static int mylite_semantic_ast_fill_statement_tables(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *statement,
    const MyliteAst *parser_ast, size_t statement_index, size_t *child_index);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_alter_table(
    MyliteSemanticAst *ast, const MyliteAstAlterTable *alter_table);
static size_t mylite_semantic_ast_count_alter_table_children(
    const MyliteAstAlterTable *alter_table);
static int mylite_semantic_ast_fill_alter_table_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *table,
    const MyliteAstAlterTable *alter_table);
static int mylite_semantic_ast_append_alter_table_spec_descriptor(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *parent, size_t *index,
    const MyliteAstAlterTableSpec *spec);
static size_t mylite_semantic_ast_count_alter_table_spec_children(
    const MyliteAstAlterTableSpec *spec);
static int mylite_semantic_ast_fill_alter_table_spec_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *descriptor,
    const MyliteAstAlterTableSpec *spec);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_create_index_table(
    MyliteSemanticAst *ast, const MyliteAstCreateIndex *create_index);
static size_t mylite_semantic_ast_count_create_index_table_children(
    const MyliteAstCreateIndex *create_index);
static int mylite_semantic_ast_fill_create_index_table_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *table,
    const MyliteAstCreateIndex *create_index);
static int mylite_semantic_ast_append_create_index_key_descriptor(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *parent, size_t *index,
    const MyliteAstCreateIndex *create_index);
static size_t mylite_semantic_ast_count_create_index_key_children(
    const MyliteAstCreateIndex *create_index);
static int mylite_semantic_ast_fill_create_index_key_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *descriptor,
    const MyliteAstCreateIndex *create_index);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_insert_table(
    MyliteSemanticAst *ast, const MyliteAstInsertStatement *insert_statement);
static size_t mylite_semantic_ast_count_insert_table_children(
    const MyliteAstInsertStatement *insert_statement);
static int mylite_semantic_ast_fill_insert_table_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *table,
    const MyliteAstInsertStatement *insert_statement);
static int mylite_semantic_ast_append_insert_column_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *parent,
    size_t *child_index, const MyliteAstInsertStatement *insert_statement);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_replace_table(
    MyliteSemanticAst *ast, const MyliteAstReplaceStatement *replace_statement);
static size_t mylite_semantic_ast_count_replace_table_children(
    const MyliteAstReplaceStatement *replace_statement);
static int mylite_semantic_ast_fill_replace_table_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *table,
    const MyliteAstReplaceStatement *replace_statement);
static int mylite_semantic_ast_append_replace_column_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *parent,
    size_t *child_index, const MyliteAstReplaceStatement *replace_statement);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_insert_source(
    MyliteSemanticAst *ast, const MyliteAstInsertStatement *insert_statement);
static size_t mylite_semantic_ast_count_insert_source_children(
    const MyliteAstInsertStatement *insert_statement);
static int mylite_semantic_ast_fill_insert_source_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    const MyliteAstInsertStatement *insert_statement);
static int mylite_semantic_ast_append_insert_value_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    size_t *child_index, const MyliteAstInsertStatement *insert_statement);
static int mylite_semantic_ast_append_insert_set_assignment_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    size_t *child_index, const MyliteAstInsertStatement *insert_statement);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_replace_source(
    MyliteSemanticAst *ast, const MyliteAstReplaceStatement *replace_statement);
static size_t mylite_semantic_ast_count_replace_source_children(
    const MyliteAstReplaceStatement *replace_statement);
static int mylite_semantic_ast_fill_replace_source_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    const MyliteAstReplaceStatement *replace_statement);
static int mylite_semantic_ast_append_replace_value_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    size_t *child_index, const MyliteAstReplaceStatement *replace_statement);
static int mylite_semantic_ast_append_replace_set_assignment_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *source,
    size_t *child_index, const MyliteAstReplaceStatement *replace_statement);
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_source(
    MyliteSemanticAst *ast, MyliteSemanticSourceKind source_kind, size_t start,
    size_t end, size_t child_count);
static MyliteSemanticSourceKind
mylite_semantic_source_kind_from_insert_source_kind(
    MyliteInsertSourceKind source_kind);
static MyliteSemanticSourceKind
mylite_semantic_source_kind_from_replace_source_kind(
...
mylite_semantic_ast_materialize_select_query_block(
    MyliteSemanticAst *ast, const MyliteAstSelectStatement *select_statement,
    size_t query_block_index);
static size_t mylite_semantic_ast_count_select_query_block_children(
    const MyliteAstSelectQueryBlock *query_block);
static size_t mylite_semantic_ast_count_select_query_block_clauses(
    const MyliteAstSelectQueryBlock *query_block);
static int mylite_semantic_ast_fill_select_query_block_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *query_block,
    const MyliteAstSelectStatement *select_statement,
    const MyliteAstSelectQueryBlock *parser_query_block);
static int
mylite_semantic_ast_append_select_query_block_projection_descriptors(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *query_block,
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_create_table(
    MyliteSemanticAst *ast, const MyliteAstCreateTable *create_table);
static size_t mylite_semantic_ast_count_create_table_children(
    const MyliteAstCreateTable *create_table);
static int mylite_semantic_ast_fill_create_table_children(
    MyliteSemanticAst *ast, MyliteSemanticAstNode *table,
    const MyliteAstCreateTable *create_table);
...
size_t mylite_semantic_ast_node_query_block_index(
    const MyliteSemanticAstNode *node) {
  return node == NULL ? 0 : node->query_block_index;
}
...
const char *mylite_semantic_source_kind_name(MyliteSemanticSourceKind kind) {
  switch (kind) {
  case MYLITE_SEMANTIC_SOURCE_UNKNOWN:
    return "unknown";
  case MYLITE_SEMANTIC_SOURCE_VALUES:
    return "values";
  case MYLITE_SEMANTIC_SOURCE_SET:
    return "set";
  case MYLITE_SEMANTIC_SOURCE_SELECT:
    return "select";
  }
  return "unknown";
}
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_statement(
  ...
  size_t child_count =
      ...
      mylite_semantic_ast_count_statement_queries(parser_ast,
                                                  statement_index) +
      mylite_semantic_ast_count_statement_sources(parser_ast,
                                                  statement_index) +
      mylite_semantic_ast_count_statement_tables(parser_ast,
                                                 statement_index) +
...
static size_t mylite_semantic_ast_count_statement_sources(
    const MyliteAst *parser_ast, size_t statement_index) {
  return mylite_ast_insert_statement_view(parser_ast, statement_index) != NULL ||
                 mylite_ast_replace_statement_view(parser_ast,
                                                   statement_index) != NULL
             ? 1
             : 0;
}
...
static size_t mylite_semantic_ast_count_statement_tables(
    const MyliteAst *parser_ast, size_t statement_index) {
  size_t count = 0;
  count += mylite_ast_create_table_view(parser_ast, statement_index) != NULL;
  count += mylite_ast_alter_table_view(parser_ast, statement_index) != NULL;
  count += mylite_ast_create_index_view(parser_ast, statement_index) != NULL;
  count += mylite_ast_insert_statement_view(parser_ast, statement_index) !=
           NULL;
  count += mylite_ast_replace_statement_view(parser_ast, statement_index) !=
           NULL;
  return count;
}
...
static int mylite_semantic_ast_fill_statement_children(
  ...
  if (!mylite_semantic_ast_fill_statement_sources(
          ast, statement, parser_ast, statement_index, &child_index)) {
    return 0;
  }
  if (!mylite_semantic_ast_fill_statement_tables(
          ast, statement, parser_ast, statement_index, &child_index)) {
    return 0;
  }
...
static int mylite_semantic_ast_fill_statement_sources(
  ...
  if (insert_statement != NULL) {
    return mylite_semantic_ast_append_child(
        statement, child_index,
        mylite_semantic_ast_materialize_insert_source(ast, insert_statement));
  }
  ...
  if (replace_statement != NULL) {
    return mylite_semantic_ast_append_child(
        statement, child_index,
        mylite_semantic_ast_materialize_replace_source(ast, replace_statement));
  }
...
static int mylite_semantic_ast_fill_statement_tables(
  ...
  if (create_table != NULL &&
      !mylite_semantic_ast_append_child(
          statement, child_index,
          mylite_semantic_ast_materialize_create_table(ast, create_table))) {
    return 0;
  }
  ...
  if (alter_table != NULL &&
      !mylite_semantic_ast_append_child(
          statement, child_index,
          mylite_semantic_ast_materialize_alter_table(ast, alter_table))) {
    return 0;
  }
  ...
  if (create_index != NULL &&
      !mylite_semantic_ast_append_child(
          statement, child_index,
          mylite_semantic_ast_materialize_create_index_table(ast,
                                                             create_index))) {
    return 0;
  }
  ...
  if (insert_statement != NULL &&
      !mylite_semantic_ast_append_child(
          statement, child_index,
          mylite_semantic_ast_materialize_insert_table(ast,
                                                       insert_statement))) {
    return 0;
  }
  ...
  if (replace_statement != NULL &&
      !mylite_semantic_ast_append_child(
          statement, child_index,
          mylite_semantic_ast_materialize_replace_table(ast,
                                                        replace_statement))) {
    return 0;
  }
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_alter_table(
  ...
  MyliteSemanticAstNode *table = mylite_semantic_ast_new_node(
      ast, MYLITE_SEMANTIC_NODE_TABLE,
      mylite_ast_alter_table_view_start(alter_table),
      mylite_ast_alter_table_view_end(alter_table));
  if (table == NULL) {
    return NULL;
  }
  table->target_kind = MYLITE_STATEMENT_TARGET_TABLE;
  table->target_role = MYLITE_STATEMENT_TARGET_ROLE_PRIMARY;
  const char *value = mylite_ast_alter_table_view_name_value(alter_table);
  size_t value_length =
      mylite_ast_alter_table_view_name_value_length(alter_table);
  if (value == NULL) {
    value = mylite_ast_alter_table_view_schema_value(alter_table);
    value_length =
  ...
  if (!mylite_semantic_ast_copy_node_value(ast, table, value, value_length) ||
      !mylite_semantic_ast_copy_optional_string(
          ...
          mylite_ast_alter_table_view_schema_value(alter_table),
          mylite_ast_alter_table_view_schema_value_length(alter_table)) ||
      !mylite_semantic_ast_set_node_child_count(
          ast, table,
          mylite_semantic_ast_count_alter_table_children(alter_table)) ||
      !mylite_semantic_ast_fill_alter_table_children(ast, table,
                                                     alter_table)) {
    return NULL;
  }
  return table;
...
static size_t mylite_semantic_ast_count_alter_table_children(
  ...
  return mylite_ast_alter_table_view_spec_count(alter_table) +
...
static int mylite_semantic_ast_fill_alter_table_children(
  ...
  for (size_t i = 0; i < mylite_ast_alter_table_view_spec_count(alter_table);
       i++) {
    if (!mylite_semantic_ast_append_alter_table_spec_descriptor(
            ast, table, &child_index,
  ...
  for (size_t i = 0; i < mylite_ast_alter_table_view_option_count(alter_table);
    ...
    if (!mylite_semantic_ast_append_descriptor_child(
            ast, table, &child_index, MYLITE_SEMANTIC_DESCRIPTOR_OPTION,
  ...
  return child_index == table->child_count;
}
...
static int mylite_semantic_ast_append_alter_table_spec_descriptor(
  ...
  size_t value_length = mylite_ast_alter_table_spec_view_name_value_length(spec);
  if (value == NULL) {
  ...
  MyliteSemanticAstNode *descriptor = mylite_semantic_ast_materialize_descriptor(
      ast, MYLITE_SEMANTIC_DESCRIPTOR_ALTER_TABLE_SPEC,
      mylite_ast_alter_table_spec_view_start(spec),
      mylite_ast_alter_table_spec_view_end(spec), value, value_length,
      mylite_semantic_ast_count_alter_table_spec_children(spec));
  if (descriptor == NULL) {
    return 0;
  }

  if (!mylite_semantic_ast_fill_alter_table_spec_children(ast, descriptor,
                                                          spec)) {
    return 0;
  }
...
static int mylite_semantic_ast_fill_alter_table_spec_children(
    ...
    const MyliteAstAlterTableSpec *spec) {
  size_t child_index = 0;
  for (size_t i = 0; i < mylite_ast_alter_table_spec_view_column_count(spec);
       i++) {
    if (!mylite_semantic_ast_append_create_table_column_descriptor(
            ast, descriptor, &child_index,
            mylite_ast_alter_table_spec_view_column_at(spec, i))) {
      return 0;
    }
  }
  for (size_t i = 0; i < mylite_ast_alter_table_spec_view_key_count(spec);
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_create_index_table(
  ...
  if (value == NULL) {
    value = mylite_ast_create_index_view_table_schema_value(create_index);
    value_length =
  ...
  if (!mylite_semantic_ast_copy_node_value(ast, table, value, value_length) ||
      !mylite_semantic_ast_copy_optional_string(
          ...
          mylite_ast_create_index_view_table_schema_value_length(
              create_index)) ||
      !mylite_semantic_ast_set_node_child_count(
          ast, table,
          mylite_semantic_ast_count_create_index_table_children(create_index)) ||
      !mylite_semantic_ast_fill_create_index_table_children(ast, table,
                                                            create_index)) {
    return NULL;
  }
  return table;
...
static int mylite_semantic_ast_fill_create_index_table_children(
    ...
    const MyliteAstCreateIndex *create_index) {
  size_t child_index = 0;
  if (!mylite_semantic_ast_append_create_index_key_descriptor(
          ast, table, &child_index, create_index)) {
    return 0;
  }
  return child_index == table->child_count;
}
...
static int mylite_semantic_ast_append_create_index_key_descriptor(
  ...
  MyliteSemanticAstNode *descriptor = mylite_semantic_ast_materialize_descriptor(
      ...
      mylite_ast_create_index_view_start(create_index),
      mylite_ast_create_index_view_end(create_index),
      mylite_ast_create_index_view_name_value(create_index),
      mylite_ast_create_index_view_name_value_length(create_index),
      mylite_semantic_ast_count_create_index_key_children(create_index));
  if (descriptor == NULL) {
    return 0;
  }
  if (!mylite_semantic_ast_fill_create_index_key_children(ast, descriptor,
                                                          create_index)) {
    return 0;
  }
  return mylite_semantic_ast_append_child(parent, index, descriptor);
...
static size_t mylite_semantic_ast_count_create_index_key_children(
  ...
  return mylite_ast_create_index_view_column_count(create_index) +
...
static int mylite_semantic_ast_fill_create_index_key_children(
  ...
  size_t child_index = 0;
  for (size_t i = 0;
       i < mylite_ast_create_index_view_column_count(create_index); i++) {
  ...
  for (size_t i = 0;
       i < mylite_ast_create_index_view_option_count(create_index); i++) {
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_insert_table(
  ...
  if (value == NULL) {
    value =
        mylite_ast_insert_statement_view_target_schema_value(insert_statement);
    value_length =
  ...
  if (!mylite_semantic_ast_copy_node_value(ast, table, value, value_length) ||
      ...
      !mylite_semantic_ast_set_node_child_count(
          ast, table,
          mylite_semantic_ast_count_insert_table_children(insert_statement)) ||
      !mylite_semantic_ast_fill_insert_table_children(ast, table,
                                                      insert_statement)) {
    return NULL;
  }
  return table;
...
static int mylite_semantic_ast_fill_insert_table_children(
    ...
    const MyliteAstInsertStatement *insert_statement) {
  size_t child_index = 0;
  if (!mylite_semantic_ast_append_insert_column_descriptors(
          ast, table, &child_index, insert_statement)) {
    return 0;
  }
  return child_index == table->child_count;
}
...
static int mylite_semantic_ast_append_insert_column_descriptors(
  ...
  for (size_t i = 0;
    ...
    if (!mylite_semantic_ast_append_descriptor_child(
            ast, parent, child_index, MYLITE_SEMANTIC_DESCRIPTOR_COLUMN,
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_replace_table(
  ...
  table->target_role = MYLITE_STATEMENT_TARGET_ROLE_PRIMARY;
  const char *value =
      mylite_ast_replace_statement_view_target_name_value(replace_statement);
  size_t value_length =
      mylite_ast_replace_statement_view_target_name_value_length(
          replace_statement);
  if (value == NULL) {
    value =
        mylite_ast_replace_statement_view_target_schema_value(replace_statement);
    value_length =
  ...
  if (!mylite_semantic_ast_copy_node_value(ast, table, value, value_length) ||
      ...
      !mylite_semantic_ast_set_node_child_count(
          ast, table,
          mylite_semantic_ast_count_replace_table_children(replace_statement)) ||
      !mylite_semantic_ast_fill_replace_table_children(ast, table,
                                                       replace_statement)) {
    return NULL;
  }
  return table;
...
static int mylite_semantic_ast_fill_replace_table_children(
    ...
    const MyliteAstReplaceStatement *replace_statement) {
  size_t child_index = 0;
  if (!mylite_semantic_ast_append_replace_column_descriptors(
          ast, table, &child_index, replace_statement)) {
    return 0;
  }
  return child_index == table->child_count;
}
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_insert_source(
    MyliteSemanticAst *ast, const MyliteAstInsertStatement *insert_statement) {
  MyliteSemanticAstNode *source = mylite_semantic_ast_materialize_source(
      ast,
      mylite_semantic_source_kind_from_insert_source_kind(
          mylite_ast_insert_statement_view_source_kind(insert_statement)),
      ...
      mylite_semantic_ast_count_insert_source_children(insert_statement));
  if (source == NULL) {
    return NULL;
  }
  if (!mylite_semantic_ast_fill_insert_source_children(ast, source,
                                                       insert_statement)) {
    return NULL;
  }
  return source;
...
static int mylite_semantic_ast_fill_insert_source_children(
  ...
  switch (mylite_ast_insert_statement_view_source_kind(insert_statement)) {
  case MYLITE_INSERT_SOURCE_VALUES:
    if (!mylite_semantic_ast_append_insert_value_descriptors(
            ast, source, &child_index, insert_statement)) {
      return 0;
    }
    break;
  case MYLITE_INSERT_SOURCE_SET:
    if (!mylite_semantic_ast_append_insert_set_assignment_descriptors(
            ast, source, &child_index, insert_statement)) {
      return 0;
    }
    break;
  ...
  return child_index == source->child_count;
}
...
static int mylite_semantic_ast_append_insert_set_assignment_descriptors(
    ...
    size_t *child_index, const MyliteAstInsertStatement *insert_statement) {
  for (size_t i = 0;
       i < mylite_ast_insert_statement_view_set_assignment_count(
               insert_statement);
       i++) {
    const MyliteAstInsertAssignment *assignment =
        mylite_ast_insert_statement_view_set_assignment_at(insert_statement, i);
    if (!mylite_semantic_ast_append_descriptor_with_expression_child(
            ast, source, child_index, MYLITE_SEMANTIC_DESCRIPTOR_ASSIGNMENT,
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_replace_source(
  ...
  MyliteSemanticAstNode *source = mylite_semantic_ast_materialize_source(
      ast,
      mylite_semantic_source_kind_from_replace_source_kind(
          mylite_ast_replace_statement_view_source_kind(replace_statement)),
      mylite_ast_replace_statement_view_source_start(replace_statement),
      mylite_ast_replace_statement_view_source_end(replace_statement),
      mylite_semantic_ast_count_replace_source_children(replace_statement));
  if (source == NULL) {
    return NULL;
  }
  if (!mylite_semantic_ast_fill_replace_source_children(ast, source,
                                                        replace_statement)) {
    return NULL;
  }
  return source;
...
static int mylite_semantic_ast_fill_replace_source_children(
  ...
  switch (mylite_ast_replace_statement_view_source_kind(replace_statement)) {
  case MYLITE_REPLACE_SOURCE_VALUES:
    if (!mylite_semantic_ast_append_replace_value_descriptors(
            ast, source, &child_index, replace_statement)) {
      return 0;
    }
    break;
  case MYLITE_REPLACE_SOURCE_SET:
    if (!mylite_semantic_ast_append_replace_set_assignment_descriptors(
            ast, source, &child_index, replace_statement)) {
      return 0;
    }
    break;
...
static int mylite_semantic_ast_append_replace_set_assignment_descriptors(
  ...
  for (size_t i = 0;
    ...
    if (!mylite_semantic_ast_append_descriptor_with_expression_child(
            ...
            mylite_ast_replace_assignment_view_name_value_length(assignment),
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_source(
    MyliteSemanticAst *ast, MyliteSemanticSourceKind source_kind, size_t start,
    size_t end, size_t child_count) {
  MyliteSemanticAstNode *source =
      mylite_semantic_ast_new_node(ast, MYLITE_SEMANTIC_NODE_SOURCE, start, end);
  if (source == NULL) {
    return NULL;
  }
  source->source_kind = source_kind;
  if (!mylite_semantic_ast_set_node_child_count(ast, source, child_count)) {
    return NULL;
  }
  return source;
}
...
mylite_semantic_source_kind_from_insert_source_kind(
  ...
  switch (source_kind) {
  case MYLITE_INSERT_SOURCE_VALUES:
    return MYLITE_SEMANTIC_SOURCE_VALUES;
  case MYLITE_INSERT_SOURCE_SET:
    return MYLITE_SEMANTIC_SOURCE_SET;
  case MYLITE_INSERT_SOURCE_SELECT:
    return MYLITE_SEMANTIC_SOURCE_SELECT;
  case MYLITE_INSERT_SOURCE_UNKNOWN:
    return MYLITE_SEMANTIC_SOURCE_UNKNOWN;
  }
  return MYLITE_SEMANTIC_SOURCE_UNKNOWN;
}
...
static size_t mylite_semantic_ast_count_select_query_children(
    const MyliteAstSelectStatement *select_statement) {
  return mylite_ast_select_statement_view_query_block_count(select_statement);
}
...
static int mylite_semantic_ast_fill_select_query_children(
  ...
  size_t child_index = 0;
  for (size_t i = 0;
       i < mylite_ast_select_statement_view_query_block_count(select_statement);
       i++) {
    if (!mylite_semantic_ast_append_child(
            query, &child_index,
            mylite_semantic_ast_materialize_select_query_block(
                ast, select_statement, i))) {
      return 0;
    }
  }
...
mylite_semantic_ast_materialize_select_query_block(
    ...
    size_t query_block_index) {
  const MyliteAstSelectQueryBlock *parser_query_block =
      mylite_ast_select_statement_view_query_block_at(select_statement,
                                                      query_block_index);
  if (parser_query_block == NULL) {
    return NULL;
  }
  MyliteSemanticAstNode *query_block = mylite_semantic_ast_new_node(
      ast, MYLITE_SEMANTIC_NODE_QUERY_BLOCK,
      mylite_ast_select_query_block_view_start(parser_query_block),
      mylite_ast_select_query_block_view_end(parser_query_block));
  if (query_block == NULL) {
    return NULL;
  }
  query_block->query_block_index = query_block_index;
  if (!mylite_semantic_ast_set_node_child_count(
          ast, query_block,
          mylite_semantic_ast_count_select_query_block_children(
              parser_query_block))) {
    return NULL;
  }
  if (!mylite_semantic_ast_fill_select_query_block_children(
          ast, query_block, select_statement, parser_query_block)) {
    return NULL;
  }
  return query_block;
}
...
static size_t mylite_semantic_ast_count_select_query_block_children(
    const MyliteAstSelectQueryBlock *query_block) {
  return mylite_ast_select_query_block_view_projection_count(query_block) +
         mylite_semantic_ast_count_select_query_block_clauses(query_block);
}
...
static size_t mylite_semantic_ast_count_select_query_block_clauses(
  ...
  size_t count = 0;
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_from_start(query_block),
      mylite_ast_select_query_block_view_from_end(query_block));
  count += mylite_semantic_ast_count_expression_root(
      mylite_ast_select_query_block_view_where_expression(query_block));
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_group_by_start(query_block),
      mylite_ast_select_query_block_view_group_by_end(query_block));
  count += mylite_semantic_ast_count_expression_root(
      mylite_ast_select_query_block_view_having_expression(query_block));
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_order_by_start(query_block),
      mylite_ast_select_query_block_view_order_by_end(query_block));
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_limit_start(query_block),
      mylite_ast_select_query_block_view_limit_end(query_block));
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_into_start(query_block),
      mylite_ast_select_query_block_view_into_end(query_block));
  count += mylite_semantic_ast_count_clause_span(
      mylite_ast_select_query_block_view_lock_start(query_block),
      mylite_ast_select_query_block_view_lock_end(query_block));
  return count;
}
...
static int mylite_semantic_ast_fill_select_query_block_children(
    ...
    const MyliteAstSelectQueryBlock *parser_query_block) {
  size_t child_index = 0;
  if (!mylite_semantic_ast_append_select_query_block_projection_descriptors(
          ast, query_block, &child_index, select_statement,
          parser_query_block)) {
    return 0;
  }
  if (!mylite_semantic_ast_append_select_query_block_clauses(
          ast, query_block, &child_index, parser_query_block)) {
    return 0;
  }
  return child_index == query_block->child_count;
}
...
mylite_semantic_ast_append_select_query_block_projection_descriptors(
  ...
  size_t projection_index =
      mylite_ast_select_query_block_view_projection_index(parser_query_block);
  size_t projection_count =
      mylite_ast_select_query_block_view_projection_count(parser_query_block);
...
static int mylite_semantic_ast_append_select_query_block_clauses(
  ...
  if (!mylite_semantic_ast_append_clause_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_WHERE,
          mylite_ast_select_query_block_view_where_start(parser_query_block),
          mylite_ast_select_query_block_view_where_end(parser_query_block),
          mylite_ast_select_query_block_view_where_expression(
              parser_query_block))) {
    return 0;
  }
  if (!mylite_semantic_ast_append_clause_span_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_GROUP_BY,
          mylite_ast_select_query_block_view_group_by_start(parser_query_block),
          mylite_ast_select_query_block_view_group_by_end(parser_query_block))) {
    return 0;
  }
  if (!mylite_semantic_ast_append_clause_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_HAVING,
          mylite_ast_select_query_block_view_having_start(parser_query_block),
          mylite_ast_select_query_block_view_having_end(parser_query_block),
          mylite_ast_select_query_block_view_having_expression(
              parser_query_block))) {
    return 0;
  }
  if (!mylite_semantic_ast_append_clause_span_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_ORDER_BY,
          mylite_ast_select_query_block_view_order_by_start(parser_query_block),
          mylite_ast_select_query_block_view_order_by_end(parser_query_block))) {
    return 0;
  }
  if (!mylite_semantic_ast_append_clause_span_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_LIMIT,
          mylite_ast_select_query_block_view_limit_start(parser_query_block),
          mylite_ast_select_query_block_view_limit_end(parser_query_block))) {
    return 0;
  }
  if (!mylite_semantic_ast_append_clause_span_child(
          ast, query_block, child_index, MYLITE_SEMANTIC_CLAUSE_INTO,
          mylite_ast_select_query_block_view_into_start(parser_query_block),
          mylite_ast_select_query_block_view_into_end(parser_query_block))) {
    return 0;
  }
...
static MyliteSemanticAstNode *mylite_semantic_ast_materialize_create_table(
  ...
  table->target_kind = MYLITE_STATEMENT_TARGET_TABLE;
  table->target_role = MYLITE_STATEMENT_TARGET_ROLE_PRIMARY;
  const char *value = mylite_ast_create_table_view_name_value(create_table);
  size_t value_length =
      mylite_ast_create_table_view_name_value_length(create_table);
  if (value == NULL) {
    value = mylite_ast_create_table_view_schema_value(create_table);
    value_length =
  ...
  if (!mylite_semantic_ast_copy_node_value(ast, table, value, value_length) ||
      ...
      !mylite_semantic_ast_fill_create_table_children(ast, table,
                                                      create_table)) {
    return NULL;
  }
  return table;
