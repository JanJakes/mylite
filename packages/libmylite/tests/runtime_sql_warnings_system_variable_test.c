...
static int test_sql_warnings_values_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open sql warnings file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql warnings file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_warnings, @@global.sql_warnings",
        (struct expected_result){
...
static int test_independent_sql_warnings_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first sql warnings handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second sql warnings handle"
    );
