...
| `AND`, `&&` | 🟡 | Limited descriptor-backed `WHERE` boolean expressions over existing `SELECT`/aggregate-source/`DELETE`/`UPDATE` predicate atoms; `AND` binds tighter than `XOR` and `OR`, and `&&` is accepted with MySQL-compatible deprecation warning 1287; specified for upcoming limited no-source/`DUAL` keyword `AND` scalar logical projection, with scalar `&&` deferred. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
...
| `IS` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates for `IS NULL`, `IS TRUE`, `IS FALSE`, and `IS UNKNOWN` only; no literal-left or general expression truth tests. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
| `IS NOT` | 🟡 | Descriptor-driven filtered table `SELECT`, `DELETE`, and `UPDATE` predicates for `IS NOT NULL`, `IS NOT TRUE`, `IS NOT FALSE`, and `IS NOT UNKNOWN` only; no literal-left or general expression truth tests. | [operators](docs/compatibility/operators.md), [SQL query expressions](docs/compatibility/sql-query-expressions.md) |
