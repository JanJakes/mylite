...
static int test_information_schema_triggers_queries(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open triggers query db");
...
static int test_information_schema_triggers_reopen_preamble_and_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first triggers db");
    failures += expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second triggers db");
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first triggers db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_SCHEMA = 'app'",
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_triggers_%s_%d.mylite",
        name,
        current_process_id()
    );
