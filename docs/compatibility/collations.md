...
| Default collation selection | 🟡 | Limited `CREATE DATABASE` / `ALTER DATABASE` descriptor-owned schema default collations for admitted `utf8mb4`, `ascii`, and `binary` defaults; limited `CREATE TABLE`, `CREATE TABLE ... LIKE`, `ALTER TABLE ... [DEFAULT] COLLATE`, same-`utf8mb4` `ALTER TABLE ... CONVERT TO CHARACTER SET`, `SHOW CREATE TABLE`, `SHOW TABLE STATUS`, and `INFORMATION_SCHEMA.TABLES` preservation for admitted `utf8mb4` and `ascii` table default collations; limited `CREATE TABLE` / `CREATE TEMPORARY TABLE` inherited `COLLATE binary` defaults from table or schema metadata normalize inherited non-national `CHAR`, `VARCHAR`, and bare `TEXT` family descriptors to binary string/BLOB descriptors and store table collation `binary`; limited explicit column `COLLATE` metadata for admitted `utf8mb4`, `utf8mb3` alias, `ascii`, and WordPress legacy default collations on `CHAR`, `VARCHAR`, and bare `TEXT` family descriptors where the host DDL action admits that type; standalone column `BINARY` shorthand selects the binary collation for the effective admitted `utf8mb4` or `ascii` character set; limited explicit column `COLLATE binary` normalization for admitted `CHAR`, `VARCHAR`, and bare `TEXT` family definitions to existing binary string/BLOB descriptors; limited `SET NAMES` session readback for admitted `utf8mb4`, `utf8mb3` alias, and WordPress legacy default collations; scalar/`SHOW VARIABLES` readback and session/local assignment for server/database collation variables with coupled charset metadata; supported explicit comparisons, ordering, grouping, primary/unique string keys, and `GROUP_CONCAT(DISTINCT)` share MyLite's UTF-8 `utf8mb4_0900_ai_ci`, `utf8mb4_0900_as_ci`, `utf8mb4_0900_as_cs`, or binary service; no cross-character-set conversion, full UCA 9.0 or locale tailoring, mutable server-global state, or DDL/default admission for catalog-only collations |
| Function result introspection | 🟡 | Limited `CHARSET()`, `COLLATION()`, and `COERCIBILITY()` support for admitted scalar arguments, descriptor-backed column metadata, scalar and single-table row-scalar `CONVERT(value USING utf8mb4\|utf8\|utf8mb3\|latin1)` metadata, limited WordPress legacy `CONVERT(value USING big5\|cp1251\|hebrew\|koi8r\|tis620\|ujis)` metadata and byte-preserving paths, scalar postfix `COLLATE` metadata for admitted `utf8mb4`, `utf8mb3`, and `latin1` collations, reduced-fidelity parser admission for `COLLATE` after charset-introduced literals in descriptor-backed DML/default and targeted predicate value positions, current explicit-collation scalar `CONCAT()` metadata/conflict shapes in no-source, `DUAL`, `DO`, and single-table row-scalar `SELECT` projection contexts, plus limited explicit `COLLATE` predicate and `ORDER BY` behavior for the documented ASCII descriptor/row-scalar subset; no full expression metadata, general conversion, non-ASCII `latin1` transcoding, introduced-literal collation execution semantics, or general comparison semantics |
| Unicode Collation Algorithm families | 🟡 | Shared limited UTF-8 comparison and key generation for the supported 0900 accent/case sensitivity modes, including Unicode case folding, common compatibility expansions, canonical common Latin-1 accent equivalence, accent-position preservation, and deterministic invalid-byte ordering; no complete UCA 9.0 weight tables, contractions, or locale tailoring |
| Binary collations | 🟡 | Full metadata catalog visibility plus descriptor-owned schema defaults through supported `CREATE DATABASE` / `ALTER DATABASE` option forms, explicit column-level `COLLATE binary` normalization to binary string/BLOB descriptors, and limited `CREATE TABLE` / `CREATE TEMPORARY TABLE` inherited table-default `COLLATE binary` behavior for admitted string descriptors, plus metadata admission, standalone column `BINARY` shorthand, session/table/column default preservation for `ascii_bin`, `utf8mb4_bin`, and `utf8mb4_0900_bin`, and exact bytewise comparison, ordering, grouping, key, and aggregate-DISTINCT behavior in the covered paths; no `ALTER TABLE ... DEFAULT COLLATE binary` or general expression coverage outside those paths |
| PAD SPACE and NO PAD collations | 🟡 | Limited default-mode `CHAR` storage/readback trims trailing spaces as verified for MySQL 8.4.9, full metadata catalog rows report MySQL 8.4.9 PAD attributes, and supported 0900 comparisons, string keys, grouping, and aggregate DISTINCT preserve trailing spaces with NO PAD semantics; no complete PAD SPACE execution model or `PAD_CHAR_TO_FULL_LENGTH` mode |
| Collation coercibility rules | 🟡 | Limited `COERCIBILITY()` metadata values for admitted scalar expressions, scalar postfix `COLLATE`, scalar and single-table row-scalar `CONVERT(... USING ...)`, and descriptor-backed columns, plus limited explicit `COLLATE` precedence over the default string-key collation in documented ASCII `WHERE` comparison/`LIKE` predicates and single-table `ORDER BY` keys; no general coercibility-driven comparison, ordering, grouping, distinct, set-operation, conversion, or full expression collation resolution |
...
| `big5_chinese_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and `CONVERT(value USING big5)` result metadata with focused Big5 truncation behavior; big5; id 1; default; sortlen 1; PAD SPACE; no general DDL/default admission, transcoding, storage, or comparison semantics |
...
| `binary` | 🟡 | Limited static catalog row, schema-default metadata through supported `CREATE DATABASE` / `ALTER DATABASE` options, explicit column-level normalization for admitted `CHAR`, `VARCHAR`, and bare `TEXT` family definitions, and limited `CREATE TABLE`, `CREATE TEMPORARY TABLE`, and `ALTER TABLE` inherited default behavior for inherited non-national string descriptors; binary; id 63; default; sortlen 1; NO PAD; no binary comparison semantics |
...
| `cp1251_general_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and byte-preserving `CONVERT(value USING cp1251)` result metadata; cp1251; id 51; default; sortlen 1; PAD SPACE; no general DDL/default admission, transcoding, storage, or comparison semantics |
...
| `hebrew_general_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and byte-preserving `CONVERT(value USING hebrew)` result metadata; hebrew; id 16; default; sortlen 1; PAD SPACE; no general DDL/default admission, transcoding, storage, or comparison semantics |
...
| `koi8r_general_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and byte-preserving `CONVERT(value USING koi8r)` result metadata; koi8r; id 7; default; sortlen 1; PAD SPACE; no general DDL/default admission, transcoding, storage, or comparison semantics |
...
| `latin1_german1_ci` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE latin1_german1_ci` validation and limited ASCII explicit `COLLATE` predicate comparison for descriptor-backed `latin1` strings and `CONVERT(... USING latin1)` operands; latin1; id 5; sortlen 1; PAD SPACE; no DDL/default admission, non-ASCII transcoding, storage conversion, locale-specific weights, or general comparison semantics |
| `latin1_swedish_ci` | 🟡 | Metadata catalog row plus limited descriptor/session readback, scalar `CONVERT(value USING latin1)` default metadata, scalar postfix `COLLATE latin1_swedish_ci` validation for ASCII scalar values, and limited ASCII explicit `COLLATE` predicate comparison for descriptor-backed `latin1` strings and `CONVERT(... USING latin1)` operands; latin1; id 8; default; sortlen 1; PAD SPACE; no non-ASCII transcoding, storage conversion, or general comparison semantics |
...
| `latin1_german2_ci` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE latin1_german2_ci` validation and limited ASCII explicit `COLLATE` predicate comparison for descriptor-backed `latin1` strings and `CONVERT(... USING latin1)` operands; latin1; id 31; sortlen 2; PAD SPACE; no DDL/default admission, non-ASCII transcoding, storage conversion, locale-specific weights, or general comparison semantics |
...
| `latin1_general_ci` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE latin1_general_ci` validation and limited ASCII explicit `COLLATE` predicate comparison for descriptor-backed `latin1` strings and `CONVERT(... USING latin1)` operands; latin1; id 48; sortlen 1; PAD SPACE; no DDL/default admission, non-ASCII transcoding, storage conversion, locale-specific weights, or general comparison semantics |
| `latin1_general_cs` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE latin1_general_cs` validation and limited ASCII explicit case-sensitive `COLLATE` predicate comparison for descriptor-backed `latin1` strings and `CONVERT(... USING latin1)` operands through MyLite's ASCII case-sensitive collation; latin1; id 49; sortlen 1; PAD SPACE; no DDL/default admission, non-ASCII transcoding, storage conversion, locale-specific weights, or general comparison semantics |
...
| `tis620_thai_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and byte-preserving `CONVERT(value USING tis620)` result metadata; tis620; id 18; default; sortlen 4; PAD SPACE; no general DDL/default admission, transcoding, storage, or comparison semantics |
...
| `ujis_japanese_ci` | 🟡 | Metadata catalog row plus limited WordPress descriptor/session readback and pass-through `CONVERT(value USING ujis)` result metadata for UTF-8 fixture values; ujis; id 12; default; sortlen 1; PAD SPACE; no EUC-JP transcoding, storage, or comparison semantics |
...
| `utf8mb3_general_ci` | 🟡 | Metadata catalog row plus limited metadata collation for national `CHAR` / `VARCHAR` aliases in `SHOW CREATE TABLE`, `SHOW FULL COLUMNS`, `INFORMATION_SCHEMA.COLUMNS`, and result-column metadata, scalar `CONVERT(value USING utf8\|utf8mb3)` default metadata, `utf8_general_ci` alias canonicalization, limited WordPress descriptor/session readback, and server/database collation variable assignment readback with MySQL-style deprecation warnings; no storage conversion or general comparison/order/group/distinct semantics |
...
| `utf8mb3_bin` | 🟡 | Metadata catalog row plus limited scalar postfix `COLLATE utf8mb3_bin` metadata for scalar `utf8` / `utf8mb3` conversion results and `utf8_bin` alias canonicalization; utf8mb3; id 83; sortlen 1; PAD SPACE; no storage conversion or comparison semantics |
| `utf8mb3_unicode_ci` | 🟡 | Metadata catalog row plus `utf8_unicode_ci` alias canonicalization and limited WordPress descriptor readback; utf8mb3; id 192; sortlen 8; PAD SPACE; no storage conversion or comparison semantics |
...
| `utf8mb4_bin` | 🟡 | Limited static catalog row, table/column default metadata preservation, `SET NAMES` session readback, server/database collation variable assignment readback, and no MySQL binary comparison semantics; utf8mb4; id 46; sortlen 1; PAD SPACE |
...
| `utf8mb4_0900_ai_ci` | 🟡 | Limited static catalog row, schema/table/column default metadata preservation, `SET NAMES` session readback, fixed scalar `@@collation_server`, descriptor-backed scalar `@@collation_database`, server/database collation variable assignment readback, and shared case/accent-insensitive UTF-8 semantics for covered explicit comparisons, ordering, grouping, primary/unique keys, and aggregate DISTINCT, including canonical common Latin-1 accents and common compatibility expansions; `LIKE` remains limited to its documented subset; utf8mb4; id 255; default; sortlen 0; NO PAD; no complete UCA 9.0 weights, locale tailoring, or general expression coverage |
...
| `utf8mb4_0900_as_cs` | 🟡 | Metadata catalog row plus limited scalar postfix validation and shared accent/case-sensitive UTF-8 semantics for covered explicit comparisons, ordering, grouping, primary/unique keys, and aggregate DISTINCT; `LIKE` remains limited to its documented subset; utf8mb4; id 278; sortlen 0; NO PAD; no DDL/default admission, complete UCA 9.0 weights, locale tailoring, storage conversion, or general expression coverage |
...
| `utf8mb4_0900_as_ci` | 🟡 | Metadata catalog row plus limited scalar postfix validation and shared accent-sensitive/case-insensitive UTF-8 semantics for covered explicit comparisons, ordering, grouping, primary/unique keys, and aggregate DISTINCT; `LIKE` remains limited to its documented subset; utf8mb4; id 305; sortlen 0; NO PAD; no DDL/default admission, complete UCA 9.0 weights, locale tailoring, storage conversion, or general expression coverage |
