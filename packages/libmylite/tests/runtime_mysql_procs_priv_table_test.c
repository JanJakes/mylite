...
static const char routine_type[] = "enum('FUNCTION','PROCEDURE')";
static const char proc_priv_type[] = "set('Execute','Alter Routine','Grant')";

static int test_mysql_procs_priv_table(void);
...
static const char *const mysql_procs_priv_columns[mysql_procs_priv_column_count] = {
    "Host",
    "Db",
    "User",
    "Routine_name",
...
int main(void) {
    return test_mysql_procs_priv_table() == 0 ? 0 : 1;
}

static int test_mysql_procs_priv_table(void) {
    enum {
        show_index_row_count = 6,
        ...
        information_schema_key_column_usage_row_count = 5,
    ...
    static const char *const show_index_values[] = {
        "procs_priv", "0", "PRIMARY", "1",   "Host",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
        "procs_priv", "0", "PRIMARY", "2",   "User",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
        "procs_priv", "0", "PRIMARY", "3",   "Db",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
        "procs_priv", "0", "PRIMARY", "4",   "Routine_name",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
        "procs_priv", "0", "PRIMARY", "5",   "Routine_type",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
        "procs_priv", "1", "Grantor", "1",   "Grantor",
        "A",          "0", NULL,      NULL,  "",
        "BTREE",      "",  "",        "YES", NULL,
    };
    ...
    static const char *const key_column_usage_values[] = {
        "PRIMARY", "Host",         "1", NULL, NULL, NULL, NULL,
    ...
    static const char *const statistics_values[] = {
        "0", "PRIMARY", "1", "Host",         "A", "0", NULL, NULL, "", "BTREE", "", "", "YES", NULL,
    ...
    if (make_test_path(path, sizeof(path), "mysql-procs-priv-table") != 0) {
        return 1;
    }
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM mysql.procs_priv",
            .column_names = mysql_procs_priv_columns,
            .column_count = mysql_procs_priv_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "mysql.procs_priv direct empty read",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM mysql.procs_priv",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "mysql.procs_priv count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM procs_priv",
            .column_names = count_column,
            .column_count = sizeof(count_column) / sizeof(count_column[0]),
            .values = count_zero,
            .row_count = 1U,
            .context = "selected mysql.procs_priv count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESC mysql.procs_priv",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = mysql_procs_priv_column_count,
            .context = "mysql.procs_priv describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM mysql.procs_priv LIKE 'Host'",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_host_values,
            .row_count = 1U,
            .context = "mysql.procs_priv show columns host",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM mysql.procs_priv WHERE Field = 'Timestamp'",
            ...
            .row_count = 1U,
            .context = "mysql.procs_priv show full columns timestamp",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM mysql.procs_priv",
            ...
            .context = "mysql.procs_priv show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_eight,
            .row_count = 1U,
            .context = "mysql.procs_priv information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, COLUMN_KEY, EXTRA, "
                   ...
                   "WHERE TABLE_SCHEMA = 'mysql' AND TABLE_NAME = 'procs_priv' "
                   "AND COLUMN_NAME IN ('Host', 'Routine_name', 'Routine_type', "
                   "'Proc_priv', 'Timestamp') ORDER BY ORDINAL_POSITION",
            ...
            .context = "mysql.procs_priv information_schema.columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.procs_priv information_schema.table_constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "mysql.procs_priv information_schema.key_column_usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "mysql.procs_priv information_schema.table_constraints_extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = show_index_row_count,
            .context = "mysql.procs_priv information_schema.statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "mysql.procs_priv information_schema.tables",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM mysql WHERE Name = 'procs_priv'",
            .column_names = show_table_status_columns,
            .column_count = show_table_status_column_count,
            .values = show_table_status_values,
            .row_count = 1U,
            .context = "mysql.procs_priv show table status",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_mysql_procs_priv_table_%d_%s.mylite",
        current_process_id(),
        name
    );
