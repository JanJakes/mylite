...
enum {
    ...
    information_schema_view_table_usage_row_count = 7,
...
static int test_sys_latest_file_io_views(void);
...
static const char *const latest_file_io_columns[latest_file_io_column_count] = {
    "thread",
    "file",
    "latency",
    "operation",
...
int main(void) {
    return test_sys_latest_file_io_views() == 0 ? 0 : 1;
}
...
static int test_sys_latest_file_io_views(void) {
    ...
    static const char *const show_full_columns_formatted_values[] = {
        "latency",
        "varchar(11)",
        "utf8mb3_general_ci",
        "YES",
    ...
    static const char *const show_full_columns_raw_values[] = {
        "latency",
        "bigint unsigned",
        NULL,
        "YES",
    ...
    static const char *const view_table_usage_values[] = {
        "sys", "latest_file_io",   "information_schema", "PROCESSLIST",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.latest_file_io",
            .column_names = latest_file_io_columns,
            .column_count = latest_file_io_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.latest_file_io direct rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM sys.`x$latest_file_io`",
            .column_names = latest_file_io_columns,
            .column_count = latest_file_io_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.x latest_file_io direct rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys latest_file_io row_count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM sys.latest_file_io",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys.latest_file_io count",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM `x$latest_file_io`",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "sys.x latest_file_io selected schema count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM latest_file_io",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = latest_file_io_column_count,
            .context = "sys.latest_file_io show columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE `x$latest_file_io`",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = NULL,
            .row_count = latest_file_io_column_count,
            .context = "sys.x latest_file_io describe",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM latest_file_io WHERE Field IN "
                   "('latency', 'operation', 'requested')",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_formatted_values,
            .row_count = 3U,
            .context = "sys.latest_file_io show columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM `x$latest_file_io` WHERE Field IN "
                   "('latency', 'operation', 'requested')",
            .column_names = show_columns_columns,
            .column_count = show_columns_column_count,
            .values = show_columns_raw_values,
            .row_count = 3U,
            .context = "sys.x latest_file_io show columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM latest_file_io WHERE Field IN "
                   "('latency', 'operation', 'requested')",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_formatted_values,
            .row_count = 3U,
            .context = "sys.latest_file_io show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW FULL COLUMNS FROM `x$latest_file_io` WHERE Field IN "
                   "('latency', 'operation', 'requested')",
            .column_names = show_full_columns_columns,
            .column_count = show_full_columns_column_count,
            .values = show_full_columns_raw_values,
            .row_count = 3U,
            .context = "sys.x latest_file_io show full columns sample",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM latest_file_io",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.latest_file_io show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM `x$latest_file_io`",
            .column_names = show_index_columns,
            .column_count = show_index_column_count,
            .values = NULL,
            .row_count = 0U,
            .context = "sys.x latest_file_io show index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "sys latest file io information_schema.columns rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_ten,
            .row_count = 1U,
            .context = "sys latest file io information_schema.columns count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys latest file io information_schema.tables rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM sys WHERE Name IN "
            ...
            .row_count = 2U,
            .context = "sys latest file io show table status",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 2U,
            .context = "sys latest file io information_schema.views rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "sys latest file io view_table_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "sys latest file io view_routine_usage rows",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys latest file io empty statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys latest file io empty table constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys latest file io empty key column usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "sys latest file io empty constraint extensions",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW latest_file_io",
            ...
            .needle = "CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `latest_file_io`",
            .context = "sys.latest_file_io show create view",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            ...
            .row_index = 0U,
            .column_index = 1U,
            .needle = "`sys`.`format_path`(`performance_schema`.`events_waits_history_long`."
                      "`OBJECT_NAME`) AS `file`",
            .context = "sys.latest_file_io show create formatted path",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE TABLE `x$latest_file_io`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "CREATE ALGORITHM=MERGE DEFINER=`mysql.sys`@`localhost` SQL SECURITY "
                      "INVOKER VIEW `x$latest_file_io`",
            .context = "sys.x latest_file_io show create table",
        }
    );
    failures += expect_query_contains(
        database,
        (struct expected_query_contains){
            .sql = "SHOW CREATE VIEW `x$latest_file_io`",
            .column_names = show_create_view_columns,
            .column_count = show_create_view_column_count,
            .row_count = 1U,
            .row_index = 0U,
            .column_index = 1U,
            .needle = "`performance_schema`.`events_waits_history_long`.`OBJECT_NAME` AS `file`",
            .context = "sys.x latest_file_io show create raw path",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_runtime_sys_latest_file_io_views_%d_%s.mylite",
        current_process_id(),
        name
    );
