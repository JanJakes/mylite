# Baseline BIT_COUNT Function Tasks

- [x] Read baseline docs, engineering standards, compatibility matrix, existing
  scalar arithmetic/modulo/DIV/bitwise specs, parser/runtime code, and SQLite
  fork policy.
- [x] Research official MySQL 8.4 `BIT_COUNT()` documentation.
...
- [x] Implement parser/AST support for `BIT_COUNT()` and wrong-arity nodes.
- [x] Implement MyLite-owned scalar `BIT_COUNT()` runtime evaluation.
