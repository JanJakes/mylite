...
static int test_information_schema_views_queries(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open views query db");
...
static int test_information_schema_views_reopen_preamble_and_handles(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first views db");
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second views db"
    );
    ...
    failures +=
        mylite_test_expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first views db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = 'app'",
