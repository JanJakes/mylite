...
static int test_values_show_and_scope(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB FT db");
...
static int test_set_diagnostics(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB FT SET db");
    for (size_t index = 0U; index < sizeof(innodb_ft_variables) / sizeof(innodb_ft_variables[0]);
         ++index) {
        const struct innodb_ft_variable *variable = &innodb_ft_variables[index];
...
static int test_session_mutation(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB FT session db");
...
static int test_user_variable_assignments(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open InnoDB FT user-var db");
    for (size_t index = 0U; index < sizeof(innodb_ft_variables) / sizeof(innodb_ft_variables[0]);
         ++index) {
        const struct innodb_ft_variable *variable = &innodb_ft_variables[index];
