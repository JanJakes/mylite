...
%fallback ATOM BOOLEAN_NUMBER ENCRYPTION_VALUE FACTOR_NUMBER NUMBER_LITERAL SIGNED_NUMBER_LITERAL SQLSTATE_VALUE STRING_LITERAL.
...
view_values_check_union_tail ::= values_union_option LP dml_write_query_start dml_write_parenthesized_query_tail RP.
...
drop_tablespace_engine_tail ::= drop_tablespace_options.

drop_tablespace_options ::= drop_tablespace_option.
...
alter_table_tail ::= alter_table_add_partition_action.
...
alter_table_algorithm_lock_after_comma ::= alter_table_add_partition_action.
...
alter_table_add_partition_action ::= ADD PARTITION table_admin_optional_binlog alter_table_add_partition_tail.

alter_table_add_partition_tail ::= .
alter_table_add_partition_tail ::= LP alter_table_reorganize_definitions RP.
alter_table_add_partition_tail ::= PARTITIONS alter_table_partition_count.
...
load_ignore_count ::= BOOLEAN_NUMBER(A). {
  mylite_parser_require_unsigned_integer_literal(ctx, A);
}
load_ignore_count ::= FACTOR_NUMBER(A). {
  mylite_parser_require_unsigned_integer_literal(ctx, A);
}
load_ignore_count ::= NUMBER_LITERAL(A). {
  mylite_parser_require_unsigned_integer_literal(ctx, A);
}
...
keyword_not_select_clause ::= NO_WAIT.
...
keyword_not_select_clause ::= SQL_NO_CACHE.
...
keyword_not_select_clause ::= PARTITIONS.
