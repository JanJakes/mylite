...
| `JSON` | 🟡 | high | Binary JSON semantics, validation, comparison, partial update metadata, and generated-column interactions. | The first scalar foundation exposes JSON-typed result metadata for `JSON_ARRAY()` and `JSON_OBJECT()` and validates/serializes JSON scalar function results. JSON column storage, binary JSON layout, comparison, path evaluation, generated-column interactions, partial update metadata, and indexing remain deferred. See [JSON scalar foundation spec](docs/specs/json-scalar-foundation/specs.md). |
...
| JSON edge cases | 🟡 | top | Duplicate keys, ordering, binary JSON storage-visible behavior, path errors, and partial update metadata. | Started for `JSON_OBJECT()` duplicate-key last-wins behavior and MySQL-observed key-order normalization in the scalar foundation. Binary JSON storage-visible behavior, path errors, partial update metadata, comparison ordering, and JSON column edge cases remain deferred. See [JSON scalar foundation spec](docs/specs/json-scalar-foundation/specs.md). |
