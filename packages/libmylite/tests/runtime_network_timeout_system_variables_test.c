...
static int test_values_show_and_scope(void) {
    ...
    failures += mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open network db");
...
static int test_set_and_diagnostics(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open network SET db");
