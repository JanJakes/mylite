...
static uint64_t
quote_function_result_length(mylite_db *database, const struct mylite_select_plan *plan,
    ...
    const struct mylite_expression_descriptor_string_callbacks *callbacks)
{
    ...
    if (mylite_function_name_is_quote(name)) {
        return quote_function_result_length(database, plan, expression, callbacks);
    }
