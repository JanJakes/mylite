...
static int test_server_build_values_and_show_rows(void) {
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open server build memory");
...
static int test_server_build_file_safety_and_independent_handles(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open server build file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen server build file");
    failures += expect_query_values(
        database,
        (struct expected_query){
            .sql = "SELECT @@protocol_version, @@version_compile_machine, "
    ...
    failures += expect_int(mylite_open_memory(&first), MYLITE_OK, "open first server build handle");
    failures +=
        expect_int(mylite_open_memory(&second), MYLITE_OK, "open second server build handle");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_server_build_system_variables_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
