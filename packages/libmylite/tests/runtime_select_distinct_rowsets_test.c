...
static int test_distinct_rowset_success_and_reopen(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen distinct database");
...
static int open_app_database(
    ...
    remove_related_files(path);
    failures +=
        mylite_test_expect_int(mylite_open(path, out_database), MYLITE_OK, "open distinct file");
