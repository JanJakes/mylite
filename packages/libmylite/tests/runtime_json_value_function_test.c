...
static int test_no_source_dual_do_and_warning_json_value(void) {
    ...
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "json_value do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "json_value do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "json_value do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "json_value do warnings");
    }
...
static int test_table_backed_json_value_and_reopen(void) {
    ...
    mylite_close(database);
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen json_value");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite-json-value-function-%s-%d.mylite",
        name,
        current_process_id()
    );
