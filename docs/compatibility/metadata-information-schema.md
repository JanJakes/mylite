...
| `INFORMATION_SCHEMA.COLUMNS` | 🟡 | Limited queryable synthetic rows for supported `information_schema` system views and MyLite descriptor columns, including current integer/exact `DECIMAL`/approximate `FLOAT` and `DOUBLE`/canonical `DATE`/canonical `TIME`/canonical `DATETIME`/canonical `TIMESTAMP`/`CHAR`/`VARCHAR`/baseline `TEXT` family/binary string/`BIT` metadata, nullability, integer, decimal, approximate, literal `CHAR`/`VARCHAR` string, canonical date-string, canonical time-string, canonical datetime-string, canonical timestamp-string, compatible `BIT` defaults, and `CURRENT_TIMESTAMP` defaults, visibility, primary-key plus supported unique/nonunique secondary-index `COLUMN_KEY` markers, `DEFAULT_GENERATED`, `on update CURRENT_TIMESTAMP`, auto-increment `EXTRA`, decimal and approximate `NUMERIC_PRECISION` / `NUMERIC_SCALE` metadata, binary string byte-length metadata, `BIT` numeric precision metadata with SQL `NULL` character set and collation names, `NULL` temporal precision for `DATE`, and `DATETIME_PRECISION = 0` for `TIME`, `DATETIME`, and `TIMESTAMP`; no privileges, generated columns, comments, binary string defaults, explicit `TEXT`-family string defaults, string/hex defaults for `BIT`, warning-producing overlength string-default truncation, full charset/collation behavior, or complete MySQL system catalogs |
