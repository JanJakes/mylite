...
| Default expressions | 🟡 | Limited descriptor-owned defaults in supported integer-family column definitions: `DEFAULT NULL` for nullable columns plus decimal integer literals with optional unary sign and `TRUE`/`FALSE` within MyLite's current physical signed-64 range; supported by `CREATE TABLE`, `ADD COLUMN`, `MODIFY COLUMN`, `CHANGE COLUMN`, omitted-column `INSERT`, `SHOW COLUMNS`, and `SHOW CREATE TABLE`; no string/decimal/float/hex/bit defaults, general expression defaults, `ALTER COLUMN SET/DROP DEFAULT`, or DML `DEFAULT` keyword values. | [SQL table DDL](docs/compatibility/sql-table-ddl.md) |
