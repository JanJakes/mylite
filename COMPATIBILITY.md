...
| `ALTER VIEW` | ❌ | high | View replacement while preserving MySQL metadata and security semantics. | Parser recognizes column lists, view body starts, and `WITH [CASCADED|LOCAL] CHECK OPTION` tails on non-plain-`SELECT` bodies including parenthesized query expressions, with validation for parenthesized query `ORDER BY`/`LIMIT` suffixes. |
...
| `CREATE TABLE ... SELECT` | ❌ | high | CTAS type inference, default handling, indexes, and atomicity. | Parser recognizes no-definition and post-definition CTAS forms, including parenthesized query bodies, and validates parenthesized query `ORDER BY`/`LIMIT` suffixes. |
...
| `INSERT ... SELECT` | ❌ | high | Insert from query expression with locking, defaults, and metadata inference. | Parser recognizes parenthesized query payloads and validates their outer `ORDER BY`/`LIMIT` tails plus `ON DUPLICATE KEY UPDATE` continuations. |
