...
static int copy_update_sqlite_column_value(
    ...
    case SQLITE_NULL:
        *out_value = (struct mylite_expression_value){.kind = MYLITE_EXPRESSION_VALUE_NULL};
        return mylite_field_descriptor_apply_expression_value_metadata(descriptor, out_value);
    case SQLITE_INTEGER:
    ...
    case SQLITE_BLOB: {
        ...
        status = mylite_field_descriptor_apply_expression_value_metadata(descriptor, out_value);
        if (status != 0) {
