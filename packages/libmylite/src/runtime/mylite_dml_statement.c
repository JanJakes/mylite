...
static int resolve_multi_delete_target(mylite_db *database,
...
static int execute_multi_delete_statement(mylite_stmt *stmt)
{
    ...
    if (status == MYLITE_OK) {
        status = resolve_multi_delete_targets(stmt->database, &stmt->delete_plan,
                                              &joined_stmt.select_plan, &target_table_indexes);
    }
    if (status == MYLITE_OK) {
    ...
    if (status == MYLITE_OK) {
        status = mylite_dml_execute_multi_delete_rows_transaction(
            stmt->database, &joined_stmt.select_plan, target_table_indexes, rowsets,
            stmt->delete_plan.target_count, &affected_rows);
        if (status == MYLITE_OK) {
            stmt->affected_rows = affected_rows;
        }
    }
...
static int bind_multi_delete_select_plan(mylite_stmt *stmt, mylite_stmt *joined_stmt)
{
    ...
    if (status == MYLITE_OK && where_clause != NULL) {
        status = mylite_select_bind_where_clause(stmt->database, where_clause,
                                                 &joined_stmt->select_plan, predicate_callbacks);
                                       ...
                                       struct mylite_update_rowset *rowsets)
{
    for (size_t target_index = 0U; target_index < target_count; ++target_index) {
        ...
        int status = add_unique_multi_delete_rowid(database, &rowsets[target_index],
                                                 ...
                                                 const struct mylite_delete_target *target)
{
    ...
    int status = mylite_diagnostics_set_error_message_parts(database, "Unknown table '", table_name,
                                                            "' in MULTI DELETE");

    if (status == MYLITE_OK) {
        status = mylite_diagnostics_append_error(database, MYLITE_MYSQL_ER_UNKNOWN_TABLE,
