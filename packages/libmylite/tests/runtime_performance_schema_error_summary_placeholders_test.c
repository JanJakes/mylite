...
enum {
    ...
    show_account_columns_row_count = 9,
    show_columns_column_count = 6,
    show_global_columns_row_count = 7,
    show_index_account_row_count = 3,
    show_index_column_count = 15,
    show_index_global_row_count = 1,
...
static int test_performance_schema_error_summary_placeholders(void);
...
int main(void) {
    return test_performance_schema_error_summary_placeholders() == 0 ? 0 : 1;
}
...
static int test_performance_schema_error_summary_placeholders(void) {
    ...
    static const char *const show_global_columns_rows[] = {
        "ERROR_NUMBER",
        "int",
        "YES",
        "UNI",
    ...
    static const char *const show_index_global_rows[] = {
    ...
    static const char *const information_schema_constraints_rows[] = {
        "events_errors_summary_by_account_by_error", "ACCOUNT",      "UNIQUE", "YES",
    ...
    static const char *const information_schema_key_usage_rows[] = {
        "events_errors_summary_by_account_by_error", "ACCOUNT",      "USER",         "1",
    ...
    static const char *const information_schema_constraint_extensions_rows[] = {
        "events_errors_summary_by_account_by_error", "ACCOUNT",      NULL, NULL,
        "events_errors_summary_by_host_by_error",    "HOST",         NULL, NULL,
        "events_errors_summary_by_thread_by_error",  "THREAD_ID",    NULL, NULL,
        "events_errors_summary_by_user_by_error",    "USER",         NULL, NULL,
        "events_errors_summary_global_by_error",     "ERROR_NUMBER", NULL, NULL,
    };
    ...
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.events_errors_summary_by_account_by_error",
        "events_errors_summary_by_account_by_error count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.events_errors_summary_by_host_by_error",
        "events_errors_summary_by_host_by_error count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.events_errors_summary_by_thread_by_error",
        "events_errors_summary_by_thread_by_error count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.events_errors_summary_by_user_by_error",
        "events_errors_summary_by_user_by_error count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.events_errors_summary_global_by_error",
        "events_errors_summary_global_by_error count"
    );
    failures += execute_ok(database, "USE performance_schema");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM events_errors_summary_global_by_error",
            .column_names = one_count_column,
            .values = zero_count,
            .column_count = 1U,
            .row_count = 1U,
            .context = "selected schema error summary read",
        }
    );
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .context = "show account error summary columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "DESCRIBE performance_schema.events_errors_summary_global_by_error",
            .column_names = show_columns_columns,
            .values = show_global_columns_rows,
            .column_count = show_columns_column_count,
            .row_count = show_global_columns_row_count,
            .context = "describe global error summary",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "show account error summary index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM performance_schema.events_errors_summary_global_by_error",
            .column_names = show_index_columns,
            .values = show_index_global_rows,
            .column_count = show_index_column_count,
            .row_count = show_index_global_row_count,
            .context = "show global error summary index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW TABLE STATUS FROM performance_schema "
                   "WHERE Name = 'events_errors_summary_global_by_error' "
            ...
            .row_count = 1U,
            .context = "show table status for global error summary",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_TYPE, "
                   ...
                   "AND ((TABLE_NAME = 'events_errors_summary_by_account_by_error' "
            ...
            .context = "information schema error summary columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema error summary statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema error summary constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema error summary key usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema error summary constraint extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema error summary tables",
        }
    );
    failures += execute_error(
        database,
        "INSERT INTO performance_schema.events_errors_summary_global_by_error "
        "(ERROR_NUMBER) VALUES (1064)",
        (struct expected_sql_error){
