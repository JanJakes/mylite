...
static int test_no_source_dual_and_do_string_slices(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "string slice do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "string slice do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "string slice do affected"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "string slice do warnings"
        );
    }
...
static int test_table_backed_string_slices_and_reopen(void) {
    ...
    mylite_close(database);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen string slice file");
