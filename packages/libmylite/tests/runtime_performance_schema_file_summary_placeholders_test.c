...
enum {
    ...
    information_schema_constraint_extensions_row_count = 4,
    ...
    show_index_instance_row_count = 3,
    show_instance_columns_row_count = 4,
...
static int test_performance_schema_file_summary_placeholders(void);
...
int main(void) {
    return test_performance_schema_file_summary_placeholders() == 0 ? 0 : 1;
}
...
static int test_performance_schema_file_summary_placeholders(void) {
    ...
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.file_summary_by_event_name",
        "file_summary_by_event_name count"
    );
    failures += expect_empty_count(
        database,
        "SELECT COUNT(*) FROM performance_schema.file_summary_by_instance",
        "file_summary_by_instance count"
    );
    failures += execute_ok(database, "USE performance_schema");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM file_summary_by_event_name",
            .column_names = one_count_column,
            .values = zero_count,
            .column_count = 1U,
            .row_count = 1U,
            .context = "selected schema file summary read",
        }
    );
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM performance_schema.file_summary_by_event_name "
                   "WHERE Field IN ("
            ...
            .context = "show file summary by event columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW COLUMNS FROM performance_schema.file_summary_by_instance "
                   "WHERE "
                   "Field IN ("
                   "'FILE_NAME', "
                   "'EVENT_NAME', "
            ...
            .context = "describe file summary by instance columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SHOW INDEX FROM performance_schema.file_summary_by_instance",
            ...
            .context = "show file summary by instance index",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "show table status for file summary",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_TYPE, "
                   ...
                   "AND ((TABLE_NAME = 'file_summary_by_event_name' "
                   "AND COLUMN_NAME IN ("
                   "'EVENT_NAME', 'SUM_NUMBER_OF_BYTES_READ', 'MAX_TIMER_MISC')) "
            ...
            .context = "information schema file summary columns",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema file summary statistics",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema file summary constraints",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema file summary key usage",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema file summary constraint extensions",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .context = "information schema file summary tables",
        }
    );
    failures += execute_error(
        database,
        "INSERT INTO performance_schema.file_summary_by_event_name "
        "(EVENT_NAME) VALUES ('wait/io/file/sql/binlog')",
        (struct expected_sql_error){
