# Baseline sys.host_summary_by_stages Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.host_summary_by_stages` and
`sys.x$host_summary_by_stages` views. MySQL uses these sys views to
summarize statement stage events grouped by Performance Schema host and stage
event name. MyLite does not collect Performance Schema stage summaries, so both
views are metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.host_summary_by_stages` and
  `sys.x$host_summary_by_stages`:
  <https://dev.mysql.com/doc/refman/8.4/en/sys-host-summary-by-stages.html>
- MySQL 8.4 Reference Manual, Performance Schema stage summary tables:
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_host_summary_by_stages_views_expectations.sh`.
...
SELECT * FROM `x$host_summary_by_stages`;
```

Both views return zero rows. This differs from a live MySQL server with
Performance Schema host stage-summary rows, but preserves the queryable sys
view surface without inventing host names, event names, stage counts, or latency
values.

## Column Metadata

`sys.host_summary_by_stages` has five columns:
...
`sys.x$host_summary_by_stages` has the same first three columns with raw
...
uses `format_pico_time()` for total and average latency; the raw `x$` view
...
- Performance Schema stage-summary collection;
- live host/stage rows, stage counts, or latency totals;
...
Performance Schema stage counters.
