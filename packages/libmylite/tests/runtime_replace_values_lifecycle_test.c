...
static int test_replace_values_success_persistence_rename_and_drop(void) {
    ...
    failures += mylite_test_expect_int(
        physical_rows,
        replace_success_row_count,
        "physical rows after replace"
    );
...
static int expect_replace_ok(mylite_db *database, const char *sql, int64_t affected_rows) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "replace column count");
    failures += mylite_test_expect_size(mylite_result_row_count(result), 0U, "replace row count");
    ...
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "replace warning count");
    mylite_result_free(result);
