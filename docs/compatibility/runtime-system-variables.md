...
| `version` | 🟡 | Limited read-only scalar `SELECT @@version` with no scope or `global`, plus matching `SHOW VARIABLES` rows; returns the fixed MySQL 8.4.9 compatibility version string; no `session`/`local`, `SET`, protocol handshake version reporting, or configurable server-version identity; version compile placeholders are documented as separate variables |
| `version_comment` | 🟡 | Limited read-only scalar `SELECT @@version_comment` with no scope or `global`, plus matching `SHOW VARIABLES` rows; returns the fixed MySQL 8.4.9 community-server comment; no `session`/`local`, `SET`, protocol metadata, or configurable build comments; version compile placeholders are documented as separate variables |
