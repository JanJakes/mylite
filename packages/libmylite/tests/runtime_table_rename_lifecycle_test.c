...
static int test_multi_table_rename_lifecycle(void);
...
int main(void) {
    ...
    failures += test_multi_table_rename_lifecycle();
...
static int test_rename_preserves_catalog_and_physical_state(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "preserve") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);

    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open rename file");
    ...
    failures += mylite_test_expect_int64(after.table_id, before.table_id, "rename keeps table id");
    failures += mylite_test_expect_text(after.name, "renamed_target", "renamed logical table name");
    failures += mylite_test_expect_text(
        after.physical_name,
        before.physical_name,
        "rename keeps physical name"
    );
    failures += mylite_test_expect_uint64(
    ...
    failures += mylite_test_expect_text(
        after_column.logical_type,
        before_column.logical_type,
        "column logical type"
    );
    failures += mylite_test_expect_text(
    ...
    failures +=
        mylite_test_expect_int(has_physical_table, 1, "physical table remains after rename");
    failures += mylite_test_expect_int(physical_rows, 1, "physical table rows remain after rename");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen renamed file");
    failures += mylite_test_expect_int(
        mylite_catalog_read_schema_by_name(database, "app", &schema),
        MYLITE_OK,
    ...
    failures += mylite_test_expect_int(physical_rows, 1, "reopened physical table rows remain");
    ...
    failures +=
        mylite_test_expect_int(has_physical_table, 0, "drop removes renamed physical table");
...
static int test_alter_table_rename_preserves_catalog_and_physical_state(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "alter_preserve") != 0) {
        return 1;
    }
    remove_related_files(path);
    mylite_file_preamble_init(expected_preamble);
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alter rename file");
    failures += seed_schema(database, "app");
    failures += execute_ok(database, "USE app", &result);
    ...
    failures +=
        mylite_test_expect_int64(after.table_id, before.table_id, "alter rename keeps table id");
    failures +=
        mylite_test_expect_text(after.name, "alter_target", "alter renamed logical table name");
    failures += mylite_test_expect_text(
        after.physical_name,
        before.physical_name,
        "alter rename keeps physical name"
    );
    failures += mylite_test_expect_uint64(
        after.descriptor_version,
        before.descriptor_version + 1U,
    ...
    failures += mylite_test_expect_int(physical_rows, 2, "alter rename preserves physical rows");
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 2U, "show create column count");
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 1U, "show create row count");
    failures += mylite_test_expect_text(
    ...
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        1,
        "update renamed table affected rows"
    );
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "SELECT id, amount FROM alter_target ORDER BY id", &result);
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 2U, "renamed select row count");
    failures +=
        mylite_test_expect_text(mylite_result_value_text(result, 0U, 0U), "1", "renamed row 1 id");
    failures += mylite_test_expect_text(
        mylite_result_value_text(result, 0U, 1U),
        "10",
        "renamed row 1 amount"
    );
    failures +=
        mylite_test_expect_text(mylite_result_value_text(result, 1U, 0U), "2", "renamed row 2 id");
    failures += mylite_test_expect_text(
        mylite_result_value_text(result, 1U, 1U),
        "30",
        "renamed row 2 amount"
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 2U, "reopened renamed row count");
    failures += mylite_test_expect_text(
        mylite_result_value_text(result, 1U, 1U),
        "30",
        "reopened update value"
    );
    mylite_result_free(result);
    result = NULL;
...
static int test_cross_schema_and_independent_name_resolution(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "cross_schema") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open cross-schema file");
    failures += seed_schema(database, "app");
    ...
    failures += mylite_test_expect_int64(
        table.schema_id,
        archive.schema_id,
        "cross-schema target schema id"
    );
    failures += mylite_test_expect_int64(
        table.table_id,
        before_cross.table_id,
        "cross-schema keeps table id"
    );
    failures += mylite_test_expect_text(
        table.physical_name,
...
static int test_multi_table_rename_lifecycle(void) {
    ...
    struct mylite_catalog_schema_descriptor app = {0};
    struct mylite_catalog_schema_descriptor archive = {0};
    struct mylite_catalog_table_descriptor before_a = {0};
    struct mylite_catalog_table_descriptor before_b = {0};
    struct mylite_catalog_table_descriptor after = {0};
    const struct mylite_catalog *catalog = NULL;
    const struct mylite_session_state *session = NULL;
    uint64_t generation_before_rename = 0U;
    ...
    if (mylite_test_make_path(path, sizeof(path), "multi") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open multi rename file");
    failures += seed_schema(database, "app");
    failures += seed_schema(database, "archive");
    failures += execute_ok(database, "USE app", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "CREATE TABLE a (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "CREATE TABLE b (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += mylite_test_expect_int(
        mylite_catalog_read_schema_by_name(database, "app", &app),
        MYLITE_OK,
        "read app schema for multi rename"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_schema_by_name(database, "archive", &archive),
        MYLITE_OK,
        "read archive schema for multi rename"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "a", &before_a),
        MYLITE_OK,
        "read first multi rename source"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "b", &before_b),
        MYLITE_OK,
        "read second multi rename source"
    );
    catalog = mylite_connection_catalog_for_test(database);
    if (catalog != NULL) {
        generation_before_rename = catalog->generation;
    ...
    failures += execute_ok(database, "RENAME TABLE a TO c, b TO d", &result);
    failures += expect_empty_result(result, "multi rename result");
    mylite_result_free(result);
    result = NULL;
    catalog = mylite_connection_catalog_for_test(database);
    if (catalog != NULL) {
        failures += mylite_test_expect_uint64(
            catalog->generation,
            generation_before_rename + 1U,
            "multi rename advances catalog once"
        );
    }
    session = mylite_connection_session_state(database);
    if (session != NULL) {
        failures += mylite_test_expect_uint64(
            session->sqlite_schema_generation,
            sqlite_generation_before_rename,
            "multi rename leaves sqlite schema generation unchanged"
        );
    }
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "c", &after),
        MYLITE_OK,
        "multi rename first target exists"
    );
    failures +=
        mylite_test_expect_int64(after.table_id, before_a.table_id, "multi rename first table id");
    failures +=
        mylite_test_expect_text(after.physical_name, before_a.physical_name, "first physical name");
    failures += mylite_test_expect_uint64(
        after.descriptor_version,
        before_a.descriptor_version + 1U,
        "first version"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "d", &after),
        MYLITE_OK,
        "multi rename second target exists"
    );
    failures +=
        mylite_test_expect_int64(after.table_id, before_b.table_id, "multi rename second table id");
    failures += mylite_test_expect_text(
        after.physical_name,
        before_b.physical_name,
        "second physical name"
    );
    failures += mylite_test_expect_uint64(
        after.descriptor_version,
        before_b.descriptor_version + 1U,
        "second version"
    );

    failures += execute_ok(database, "CREATE TABLE app.x (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "CREATE TABLE archive.y (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(
        database,
        "RENAME TABLE app.x TO archive.x_new, archive.y TO app.y_new",
        &result
    );
    failures += expect_empty_result(result, "cross-schema multi rename result");
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_table(database, "SHOW TABLES FROM archive LIKE 'x_new'", "x_new");
    failures += expect_show_table(database, "SHOW TABLES FROM app LIKE 'y_new'", "y_new");

    failures += execute_ok(database, "CREATE TABLE s1 (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "CREATE TABLE s2 (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "INSERT INTO s1 VALUES (1)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "INSERT INTO s2 VALUES (2)", &result);
    mylite_result_free(result);
    result = NULL;
    failures +=
        execute_ok(database, "RENAME TABLE s1 TO tmp_swap, s2 TO s1, tmp_swap TO s2", &result);
    failures += expect_empty_result(result, "left-to-right swap result");
    mylite_result_free(result);
    result = NULL;
    failures += expect_single_value(database, "SELECT id FROM s1", "2");
    failures += expect_single_value(database, "SELECT id FROM s2", "1");
    ...
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "RENAME TABLE A TO B, a TO b", &result);
    failures += expect_empty_result(result, "case-distinct multi rename result");
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_table(database, "SHOW TABLES LIKE 'B'", "B");
    failures += expect_show_table(database, "SHOW TABLES LIKE 'b'", "b");

    failures += execute_ok(database, "CREATE TABLE fail_a (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    failures += execute_ok(database, "CREATE TABLE fail_b (id INT)", &result);
    mylite_result_free(result);
    result = NULL;
    catalog = mylite_connection_catalog_for_test(database);
    if (catalog != NULL) {
    ...
    failures += execute_error(
        database,
        "RENAME TABLE fail_a TO fail_c, missing TO fail_d",
        (struct expected_sql_error){
    ...
    failures += execute_error(
        database,
        "RENAME TABLE fail_a TO fail_b, fail_b TO fail_c",
        (struct expected_sql_error){
            .code = mysql_error_table_exists,
            .sqlstate = "42S01",
            .message_part = "Table 'fail_b' already exists",
        }
    );
    failures += execute_error(
        database,
        "RENAME TABLE fail_a TO fail_c, fail_b TO fail_c",
        (struct expected_sql_error){
            .code = mysql_error_table_exists,
            .sqlstate = "42S01",
            .message_part = "Table 'fail_c' already exists",
        }
    );
    failures += execute_error(
        database,
        "RENAME TABLE fail_a TO fail_c, fail_a TO fail_d",
        (struct expected_sql_error){
            .code = mysql_error_table_does_not_exist,
            .sqlstate = "42S02",
            .message_part = "Table 'app.fail_a' doesn't exist",
        }
    );
    catalog = mylite_connection_catalog_for_test(database);
    if (catalog != NULL) {
        failures += mylite_test_expect_uint64(
            ...
            "multi rename failures leave generation unchanged"
        );
    }
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "fail_a", &after),
        MYLITE_OK,
        "multi rename failure keeps first source"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "fail_b", &after),
        MYLITE_OK,
        "multi rename failure keeps second source"
    );
    failures += mylite_test_expect_int(
        mylite_catalog_read_table_by_name(database, app.schema_id, "fail_c", &after),
        MYLITE_ERROR,
        "multi rename failure leaves target absent"
    );
...
static int test_alter_table_rename_schema_resolution_and_noop(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "alter_schema_noop") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alter schema file");
    failures += seed_schema(database, "app");
    failures += seed_schema(database, "archive");
    failures += mylite_test_expect_int(
        mylite_catalog_read_schema_by_name(database, "app", &app),
    ...
    failures += mylite_test_expect_int64(
        table.schema_id,
        archive.schema_id,
        "alter cross-schema target schema"
    );
    ...
    failures += mylite_test_expect_int64(
        after_noop.table_id,
        before_noop.table_id,
        "alter noop keeps table id"
    );
    failures += mylite_test_expect_uint64(
    ...
    failures += mylite_test_expect_int(physical_rows, 1, "alter noop preserves physical row");
...
static int test_failure_diagnostics_and_unwinding(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open failure file");
    failures += seed_schema(database, "app");
    failures += seed_schema(database, "archive");
    ...
    failures += execute_error(
        database,
        "RENAME TABLE duplicate_target TO renamed_duplicate,",
        (struct expected_sql_error){
...
static int test_alter_table_rename_failure_diagnostics_and_unwinding(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "alter_failures") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open alter failures file");
    failures += seed_schema(database, "app");
    failures += seed_schema(database, "archive");
    failures += execute_ok(database, "CREATE TABLE app.qualified_source (id INT)", &result);
    mylite_result_free(result);
...
static int test_catalog_failure_rolls_back_rename(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "catalog_failure") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int test_alter_table_rename_catalog_failure_rolls_back(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "alter_catalog_failure") != 0) {
        return 1;
    }
    remove_related_files(path);
    ...
    failures += mylite_test_expect_int(
        mylite_open(path, &database),
        MYLITE_OK,
        "open alter catalog-failure file"
    );
    failures += seed_schema(database, "app");
    failures += execute_ok(database, "USE app", &result);
    mylite_result_free(result);
...
static int test_independent_file_backed_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first rename file"
    );
    failures += mylite_test_expect_int(
        mylite_open(second_path, &second),
        MYLITE_OK,
        "open second rename file"
    );
...
static int expect_show_table(mylite_db *database, const char *sql, const char *expected_name) {
    ...
    failures +=
        mylite_test_expect_text(mylite_result_value_text(result, 0U, 0U), expected_name, sql);
    mylite_result_free(result);
...
static int expect_single_value(mylite_db *database, const char *sql, const char *expected) {
    ...
    failures += mylite_test_expect_size(mylite_result_column_count(result), 1U, sql);
    failures += mylite_test_expect_size(mylite_result_row_count(result), 1U, sql);
    failures += mylite_test_expect_text(mylite_result_value_text(result, 0U, 0U), expected, sql);
    mylite_result_free(result);
