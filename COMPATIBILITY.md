...
| `INSERT ... VALUES` | 🟡 | Limited single- and multi-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [IGNORE] [INTO] table ... VALUES ...` into persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, no-column-list mapping to visible descriptor columns only, explicit column-list access to invisible columns, omitted-column descriptor default filling, strict range/nullability/no-default checks, limited `IGNORE` warning demotion for omitted no-default, `NULL` into `NOT NULL`, and integer range clipping, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no DML `DEFAULT` keyword values, keys, generated values, duplicate-key discard, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `INSERT ... SET` | 🟡 | Limited one-row `INSERT [LOW_PRIORITY \| HIGH_PRIORITY \| DELAYED] [IGNORE] [INTO] table SET column = integer_or_NULL_or_boolean[, ...]` into persistent base tables with descriptor assignment resolution including explicitly named invisible columns, omitted-column descriptor default filling, strict required-column/range/nullability diagnostics, limited `IGNORE` warning demotion for omitted no-default, `NULL` into `NOT NULL`, and integer range clipping, affected rows, statement atomicity, no-op priority modifiers, and delayed deprecation warning; no qualified assignment targets, expressions, DML `DEFAULT` keyword values, generated values, duplicate-key discard, or insert ids. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `INSERT IGNORE` | 🟡 | Limited `INSERT IGNORE ... VALUES` and `INSERT IGNORE ... SET` warning demotion for descriptor integer/`NULL` inputs: omitted no-explicit-default columns warn and store `0` for numeric `NOT NULL` or `NULL` for nullable dropped-default columns, explicit `NULL` into numeric `NOT NULL` stores `0` with warnings, out-of-range integer values clip to the supported descriptor range with warnings, and non-ignorable syntax/name/shape errors remain errors; no `INSERT IGNORE ... SELECT`, duplicate-key discard, partitions, generated values, general expression conversion, or full non-strict mode behavior. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `REPLACE ... VALUES` | 🟡 | Limited no-key-descriptor `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] table [(columns)] VALUES (...)` insert-equivalent path for persistent base tables with integer/`NULL`/`TRUE`/`FALSE` values, descriptor column resolution, omitted-column defaults, strict diagnostics, affected rows, statement atomicity, and delayed deprecation warning; duplicate-key delete-insert semantics await primary/unique key descriptors, and no `VALUE`, row constructors, partitions, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
| `REPLACE ... SET` | 🟡 | Limited no-key-descriptor one-row `REPLACE [LOW_PRIORITY \| DELAYED] [INTO] table SET column = integer_or_NULL_or_boolean[, ...]` insert-equivalent path for persistent base tables with descriptor assignment resolution, omitted-column defaults, strict diagnostics, one affected row, statement atomicity, and delayed deprecation warning; duplicate-key delete-insert semantics await primary/unique key descriptors, and no partitions, qualified assignment targets, expressions, DML `DEFAULT`, or query forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
...
| `UPDATE` (single-table) | 🟡 | Limited persistent base-table `UPDATE table SET column = integer_or_NULL_or_boolean` with optional baseline `WHERE` predicate boolean expression, one unqualified descriptor `ORDER BY` column, and signed-64-range `LIMIT row_count`; assignment, predicate, and ordering columns may explicitly name invisible descriptor columns; changed-row affected counts; no aliases, partitions, modifiers, joins, multiple assignments, expression assignments, full ordering, or offset forms. | [SQL table DML](docs/compatibility/sql-table-dml.md) |
