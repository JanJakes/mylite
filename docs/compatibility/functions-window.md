...
baseline. `JSON_ARRAYAGG()` and `JSON_OBJECTAGG()` windows execute in the
documented JSON aggregate-window baseline. `DISTINCT` aggregate windows return
MySQL's `1235` unsupported diagnostic. `GROUP_CONCAT()` windows return MySQL's
own unsupported diagnostic, and broader aggregate-window placeholders remain
parse-level compatibility only; see
...
[baseline JSON aggregate window functions](../specs/baseline-json-aggregate-window-functions/specs.md),
...
| JSON aggregate windows | ✅ | Projection-only `JSON_ARRAYAGG()` and `JSON_OBJECTAGG()` windows over the current no-source, `DUAL`, one-table, named-window, partition/order/frame row-scalar window envelope. See [JSON aggregate windows](../specs/baseline-json-aggregate-window-functions/specs.md). |
