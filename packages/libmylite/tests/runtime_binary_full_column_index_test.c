...
static int test_full_binary_index_metadata_dml_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open binary full file");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen binary full file");
...
static int test_full_binary_index_diagnostics(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open binary full diagnostics");
    failures += create_schema(database);
    failures +=
...
static int test_full_binary_index_independent_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first binary full");
    failures += expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second binary full");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_binary_full_column_index_%d_%s.mylite",
        current_process_id(),
        name
    );
