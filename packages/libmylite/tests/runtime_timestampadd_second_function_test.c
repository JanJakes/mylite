...
static int test_no_source_dual_and_do_timestampadd(void) {
    ...
    if (failures == 0) {
        failures += mylite_test_expect_size(
            ...
            0U,
            "timestampadd do columns"
        );
        failures +=
            mylite_test_expect_size(mylite_result_row_count(result), 0U, "timestampadd do rows");
        failures += mylite_test_expect_int64(
            mylite_result_affected_rows(result),
            0,
            "timestampadd do rows"
        );
        failures += mylite_test_expect_size(
            mylite_result_warning_count(result),
            0U,
            "timestampadd do warnings"
        );
    }
...
static int test_table_backed_timestampadd_and_reopen(void) {
    ...
    database = NULL;
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen timestampadd file");
    failures += execute_ok(database, "USE app", NULL);
    failures += execute_ok(database, "SET time_zone = '+00:00'", NULL);
    failures += expect_query(
        database,
