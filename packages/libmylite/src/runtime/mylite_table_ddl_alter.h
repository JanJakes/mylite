...
struct mylite_table_ddl_alter_callbacks {
    void *user_data;
    int (*validate_primary_key_part_not_null)(
        void *user_data,
...
const struct mylite_alter_table_column *mylite_table_ddl_find_alter_table_column(
    const struct mylite_alter_table_model *model,
    const char *name
);
size_t mylite_table_ddl_alter_table_column_index(
    const struct mylite_alter_table_model *model,
    const char *name
);
int mylite_table_ddl_set_alter_table_duplicate_column_error(
    mylite_db *database,
    const char *column_name
);
int mylite_table_ddl_set_alter_table_unknown_column_error(
    mylite_db *database,
    const char *table_name,
    const char *column_name
);
int mylite_table_ddl_set_alter_table_cant_drop_column_error(
    mylite_db *database,
    const char *column_name
);
