...
static int validate_union_operand_column_counts(
    mylite_db *database,
    const struct mylite_union_plan *plan
);

static int bind_union_query_clauses(
    mylite_db *database,
...
static int collect_union_query_operands(
    mylite_db *database,
...
static int append_union_query_operand(
    mylite_db *database,
    struct mylite_union_plan *plan,
    mylite_stmt *operand,
...
static int prepare_union_query_operand(
    mylite_db *database,
    const struct mylite_sql_ast_node *node,
    const char *sql,
    size_t sql_length,
...
static const struct mylite_sql_ast_node *unwrap_union_query_primary(
    const struct mylite_sql_ast_node *node
);
...
static int add_union_output_column(
    mylite_db *database,
    ...
    struct mylite_union_plan *plan, const struct mylite_select_union_prepare_callbacks *callbacks)
{
    ...
    if (node->kind == MYLITE_SQL_AST_UNION_EXPRESSION) {
        ...
        status = prepare_union_query_operand(
            database,
            right,
            sql,
            sql_length,
        ...
        status = append_union_query_operand(
            database,
            plan,
            right_operand,
            node->set_operation,
            node->set_duplicate_mode,
            true
        );
        if (status != MYLITE_OK) {
...
static int append_union_query_operand(
    ...
    if (has_operator) {
        enum mylite_sql_ast_set_operation *operations = (enum mylite_sql_ast_set_operation *)
            realloc((void *)plan->operations, plan->operand_count * sizeof(*plan->operations));
        ...
        operators = (enum mylite_sql_ast_set_duplicate_mode *)
            realloc((void *)plan->operators, plan->operand_count * sizeof(*plan->operators));
        if (operators == NULL) {
            (void)mylite_diagnostics_set_error_message(database, "out of memory");
            return MYLITE_NOMEM;
        }
...
static int set_union_sql_calc_found_rows_placement_error(mylite_db *database) {
    ...
    if (status == MYLITE_OK) {
        status = mylite_diagnostics_append_error(
            database,
            MYLITE_MYSQL_ER_WRONG_USAGE_OF_SQL_CALC_FOUND_ROWS,
            message
        );
    }
...
static int initialize_union_output_plan(mylite_stmt *stmt) {
    ...
    for (size_t index = 0U; index < stmt->result_metadata.column_count; ++index) {
        int status = add_union_output_column(
            stmt->database,
            &stmt->select_plan,
    ...
    for (size_t operand_index = 1U; operand_index < stmt->union_plan.operand_count;
        ...
        for (size_t column_index = 0U; column_index < stmt->result_metadata.column_count;
            ...
            mylite_expression_descriptor_merge_union_operand(
                stmt->database,
                &stmt->result_metadata.columns[column_index].descriptor,
                &descriptor
            );
        }
    }
