...
static int test_no_source_dual_and_do_reverse(void);
static int test_table_backed_reverse_and_reopen(void);
static int test_reverse_diagnostics(void);
...
int main(void) {
    ...
    failures += test_no_source_dual_and_do_reverse();
    failures += test_table_backed_reverse_and_reopen();
    failures += test_reverse_diagnostics();
...
static int test_no_source_dual_and_do_reverse(void) {
    ...
    static const char *const values_dual[] = {"cba", "cba"};
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT REVERSE('abc') AS ascii_value, REVERSE('') AS empty_value, "
                   "REVERSE(NULL) AS null_value, REVERSE(12345) AS int_value, "
                   "REVERSE(-7) AS neg_value, REVERSE(TRUE) AS true_value, "
                   "REVERSE(FALSE) AS false_value, REVERSE('\303\251a') AS utf8_value, "
                   "REVERSE('\360\237\231\202a') AS emoji_value, "
                   "REVERSE(DATABASE()) AS session_value, "
                   "REVERSE(@@warning_count) AS system_value",
            .columns = columns_no_source,
            .column_count = sizeof(columns_no_source) / sizeof(columns_no_source[0]),
            .values = values_no_source,
            .row_count = 1U,
            .context = "no-source reverse values",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT REVERSE ('abc') AS a, REVERSE(('abc')) AS b FROM DUAL",
            .columns = columns_dual,
            .column_count = sizeof(columns_dual) / sizeof(columns_dual[0]),
            .values = values_dual,
            .row_count = 1U,
            .context = "dual reverse whitespace",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "row count after reverse select",
        }
    );

    failures += execute_ok(database, "DO REVERSE('abc'), REVERSE(NULL)", &result);
    if (failures == 0) {
        failures += expect_size(mylite_result_column_count(result), 0U, "reverse do columns");
        failures += expect_size(mylite_result_row_count(result), 0U, "reverse do rows");
        failures += expect_int64(mylite_result_affected_rows(result), 0, "reverse do affected");
        failures += expect_size(mylite_result_warning_count(result), 0U, "reverse do warnings");
    }
    ...
    failures += expect_query(
        ...
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "row count after reverse do",
        }
    );
...
static int test_table_backed_reverse_and_reopen(void) {
    ...
    static const char *const values_reopen[] = {"1", "cbA"};
    char path[test_path_capacity];
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id, REVERSE(v) AS rv, REVERSE(c) AS rc, REVERSE(txt) AS rtxt, "
                   "REVERSE(i) AS ri, REVERSE(d) AS rd, REVERSE(y) AS ry, "
                   "REVERSE(dt) AS rdt, REVERSE(tm) AS rtm, REVERSE(dttm) AS rdttm, "
                   "REVERSE(ts) AS rts FROM t ORDER BY id",
            .columns = columns_table,
            .column_count = sizeof(columns_table) / sizeof(columns_table[0]),
            .values = values_table,
            .row_count = 2U,
            .context = "table reverse values",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id, REVERSE(v) AS rv FROM t WHERE id >= 1 ORDER BY id ASC LIMIT 1",
    ...
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT REVERSE(v), REVERSE(v) AS reversed FROM t WHERE id = 1",
            .columns = columns_labels,
            .column_count = sizeof(columns_labels) / sizeof(columns_labels[0]),
            .values = values_labels,
            .row_count = 1U,
            .context = "reverse labels",
        }
    );
    ...
    mylite_close(database);
    database = NULL;
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen reverse");
    failures += execute_ok(database, "USE app", NULL);
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT id, REVERSE(v) AS rv FROM t WHERE id = 1",
            .columns = columns_reopen,
            .column_count = sizeof(columns_reopen) / sizeof(columns_reopen[0]),
            .values = values_reopen,
            .row_count = 1U,
            .context = "reopen reverse values",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written =
        snprintf(path, path_size, "/tmp/mylite-reverse-%s-%d.mylite", name, current_process_id());
