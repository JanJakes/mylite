...
static int test_varchar_wide_length_lifecycle(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "wide") != 0) {
        return 1;
    }
    remove_related_files(path);
    ...
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open wide file");
    failures += expect_statement_ok(database, "CREATE DATABASE app");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_statement_ok(
        database,
    ...
    mylite_close(database);
    database = NULL;
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen wide file");
    failures += expect_statement_ok(database, "USE app");
    failures += expect_query_values(
        database,
        (struct expected_query){
...
static int test_varchar_diagnostics(void) {
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .column_count = 3U,
            .row_count = 1U,
            .context = "varchar numeric and hex literal DML",
        }
    );
    failures += expect_statement_ok(database, "CREATE TABLE nul_varchar (id INT, v VARCHAR(20))");
    failures += expect_dml_ok(database, "INSERT INTO nul_varchar VALUES (1, 'a\\0b')", 1);
    failures += execute_ok(database, "SELECT v FROM nul_varchar WHERE id = 1", &result);
    if (result != NULL) {
        const unsigned char *actual =
        ...
        failures += mylite_test_expect_size(
            ...
            sizeof(nul_varchar_value),
            "varchar NUL byte result size"
        );
        failures += mylite_test_expect_true(actual != NULL, "varchar NUL byte result value");
        if (actual != NULL) {
            failures += expect_bytes(
                actual,
                nul_varchar_value,
                sizeof(nul_varchar_value),
                "varchar NUL byte result bytes"
            );
        }
    }
    mylite_result_free(result);
    result = NULL;
    failures += expect_statement_ok(database, "CREATE TABLE nul_varchar_copy (v VARCHAR(20))");
    failures += expect_dml_ok(
        database,
        "INSERT INTO nul_varchar_copy SELECT v FROM nul_varchar WHERE id = 1",
        1
    );
    failures += execute_ok(database, "SELECT v FROM nul_varchar_copy", &result);
    if (result != NULL) {
        const unsigned char *actual =
            (const unsigned char *)mylite_result_value_bytes(result, 0U, 0U);
        ...
        failures += mylite_test_expect_size(
            mylite_result_value_size(result, 0U, 0U),
            sizeof(nul_varchar_value),
            "varchar INSERT SELECT NUL byte result size"
        );
        failures +=
            mylite_test_expect_true(actual != NULL, "varchar INSERT SELECT NUL byte result value");
        if (actual != NULL) {
            failures += expect_bytes(
                actual,
                nul_varchar_value,
                sizeof(nul_varchar_value),
                "varchar INSERT SELECT NUL bytes"
            );
        }
    }
    mylite_result_free(result);
    ...
    failures +=
        expect_statement_ok(database, "CREATE TABLE numeric_default (v VARCHAR(2) DEFAULT 1)");
    ...
    failures += expect_query_values(
        ...
        (struct expected_query){
            ...
            .column_count = 1U,
            .row_count = 1U,
            .context = "varchar numeric literal default",
        }
    );
    failures += execute_error(
        database,
