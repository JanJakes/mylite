...
static int test_information_schema_innodb_foreign_rows(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb foreign db");
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen innodb foreign db");
    failures += expect_query(
        database,
        (struct expected_query){
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_innodb_foreign_%s_%d.mylite",
        name,
        current_process_id()
    );
