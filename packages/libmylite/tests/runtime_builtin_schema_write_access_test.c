...
static int expect_write_rejected(
    ...
    failures += mylite_test_expect_int(mylite_errcode(database), schema->code, sql);
    failures += mylite_test_expect_text_or_null(mylite_sqlstate(database), schema->sqlstate, sql);
    failures += mylite_test_expect_contains(mylite_errmsg(database), schema->message_part, sql);
    failures += mylite_test_expect_size(mylite_result_column_count(result), 0U, sql);
    mylite_result_free(result);
    return failures;
}
