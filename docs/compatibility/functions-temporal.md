...
| `CURDATE()` | 🟡 | Limited zero-fractional statement date in no-source, `FROM DUAL`, `DO`, row-scalar projection, compatible `DATE` DML assignment, and parenthesized generated `DATE` defaults; respects the limited session `timestamp` override, uses fixed UTC formatting, and has no mutable time zones or general temporal expression evaluation |
...
| `CURRENT_TIME(), CURRENT_TIME` | 🟡 | Limited zero-fractional statement time synonyms for `CURTIME()` in no-source, `FROM DUAL`, `DO`, row-scalar projection, compatible `TIME` DML assignment, and parenthesized generated `TIME` defaults; no fractional precision, mutable time zones, or general temporal expression evaluation |
...
| `CURTIME()` | 🟡 | Limited zero-fractional statement time in the same supported positions as `CURRENT_TIME`, including parenthesized generated `TIME` defaults; respects the limited session `timestamp` override and uses fixed UTC formatting |
