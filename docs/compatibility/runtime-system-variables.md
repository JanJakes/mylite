...
| `character_set_client` | 🟡 | Limited scalar `SELECT @@character_set_client` with no scope, `session`, `local`, or `global`; returns the current session connection charset readback, currently `utf8mb4` after admitted `SET NAMES` / `SET CHARACTER SET` forms; no mutable conversion, client negotiation, or protocol charset metadata |
| `character_set_connection` | 🟡 | Limited scalar `SELECT @@character_set_connection` with no scope, `session`, `local`, or `global`; returns the current session connection charset readback, currently `utf8mb4` after admitted `SET NAMES` / `SET CHARACTER SET` forms; no mutable conversion, string literal semantics, or protocol charset metadata |
...
| `character_set_results` | 🟡 | Limited scalar `SELECT @@character_set_results` with no scope, `session`, `local`, or `global`; returns the current session result charset readback, currently `utf8mb4` after admitted `SET NAMES` / `SET CHARACTER SET` forms; no mutable result conversion, `NULL` results mode, or protocol charset metadata |
