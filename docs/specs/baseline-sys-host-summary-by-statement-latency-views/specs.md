# Baseline sys.host_summary_by_statement_latency Views

This slice adds MySQL-shaped metadata and deterministic read-only empty rows for
the `sys.host_summary_by_statement_latency` and
`sys.x$host_summary_by_statement_latency` views. MySQL uses these sys views to
summarize statement events grouped by Performance Schema host. MyLite does not
collect Performance Schema statement summaries, so both views are
metadata-complete empty placeholders.

## Compatibility Authority

- MySQL 8.4 Reference Manual, `sys.host_summary_by_statement_latency` and
...
- Observed MySQL 8.4.9 runtime behavior captured by
  `packages/libmylite/tests/mysql_baseline_sys_host_summary_by_statement_latency_views_expectations.sh`.
...
SELECT * FROM `x$host_summary_by_statement_latency`;
```
...
view surface without inventing host names, statement counts, latency values,
row counters, or full-scan totals.

## Column Metadata

`sys.host_summary_by_statement_latency` has ten columns:
...
| `rows_sent` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `rows_examined` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `rows_affected` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `full_scans` | `decimal(43,0)` | `YES` | `NULL` | SQL `NULL` |

`sys.x$host_summary_by_statement_latency` uses raw numeric latency metadata:
...
| `lock_latency` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `cpu_latency` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `rows_sent` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `rows_examined` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `rows_affected` | `decimal(42,0)` | `YES` | `NULL` | SQL `NULL` |
| `full_scans` | `decimal(43,0)` | `YES` | `NULL` | SQL `NULL` |
...
length/octet metadata, decimal precision and scale, and unsigned `BIGINT`
precision. The views have no index or constraint metadata, so `SHOW INDEX`,
...
`x$` view exposes the corresponding raw timer values.
...
- live host statement-summary rows, statement counts, latency totals, row
