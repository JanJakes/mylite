...
| `START REPLICA` | ❌ | low | Replica start syntax, channels, threads, and until conditions. | Parser recognizes `IO_THREAD`/`RELAY_THREAD` and `SQL_THREAD`, until, connection, and channel clauses with string-literal log/GTID/user option values, source/relay log coordinate pairs in either MySQL parser order, numeric log positions, and string-literal channel names. |
...
| `HELP` | ❌ | low | Server help lookup result-set semantics. | Parser recognizes no-topic help plus string, identifier, numeric, and keyword help topics. |
