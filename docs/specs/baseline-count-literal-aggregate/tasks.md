...
3. Parser and AST
   - [ ] Add an AST node for one-literal `COUNT()` aggregate functions.
   - [ ] Extend Lemon grammar for no-space `COUNT(count_literal)`, including
   ...
   - [ ] Add parser tests for supported count-literal syntax and unsupported
     argument/spacing forms.

4. Analyzer/planner/runtime execution
   - [ ] Extend the count aggregate select path for `COUNT(integer)` and
...
5. Physical SQLite aggregate
   ...
   - [ ] Bind the count-literal parameter and predicate values through
     prepared statements.
...
8. Verification and review
   ...
   - [ ] Run the count aggregate CTest entry and relevant parser/count-column
     lifecycle entries.
   - [ ] Run
