...
1. MySQL research and design
   ...
   - Specify exact/default global no-op assignment forms that preserve `0`.
...
2. Runtime resolver
   ...
   - Preserve existing unknown-variable, unsupported-expression,
     non-global/nonzero unsupported-`SET`, and quoted-scope diagnostics.
...
3. Runtime tests
   ...
   - Cover values, labels, scopes, quoted final names, `FROM DUAL`, selected
     ...
     names, global-only diagnostics, quoted-scope rejection, accepted fixed
     global `SET`, rejected non-global and nonzero `SET`, rejected deprecated
     alias, persistence, preamble preservation, unchanged generations,
...
5. Compatibility docs
   ...
   - Do not claim mutable `sql_replica_skip_counter` state, nonzero `SET`,
...
- Do not implement nonzero `SET`, startup options, persisted variables,
  mutable global state, `START REPLICA`, `STOP REPLICA`, replication channels,
  relay logs, binary logs, event skipping, GTID checks, anonymous-transaction
  assignment, source metadata, applier workers, replication status, the
  deprecated `sql_slave_skip_counter` alias, deprecation warnings, privileges,
