...
| `REPLACE ... SET` | ❌ | high | SET-form replace semantics. | Parser validates comma-separated assignment lists, dotted assignment targets, nested expression values, rejects repeated `SET` continuations, and rejects unsupported row-alias suffixes. |
...
| `UPDATE` (single-table) | ❌ | top | Assignment order, generated columns, ORDER BY, LIMIT, LOW_PRIORITY, and IGNORE. | Parser validates comma-separated assignment lists, dotted assignment targets, nested expression values, rejects repeated `SET` continuations, and incomplete, repeated, or out-of-order `WHERE`/`ORDER BY`/`LIMIT` tails. |
