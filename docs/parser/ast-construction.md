...
- `ALTER TABLE` semantic nodes now mirror that object shape: the statement owns
  ...
  descriptors.
- `CREATE INDEX` semantic nodes now use a decoded indexed-table node that owns
  one key descriptor for the index. That key descriptor carries the decoded
  index name and owns key-part and key-option descriptor children.
...
- `SELECT` statements expose parser-level query views with CTE/set-operation
  ...
  expressions. Query-block handles preserve block spans, projection ranges,
  block-local clause anchors, and decoded named table-reference descriptors for
  SELECT `FROM` clauses.
...
- The semantic AST now gives `INSERT` and `REPLACE` statements one target table
  node with decoded table/schema values. Explicit target column descriptors live
  under that table node while VALUES/SET payload descriptors stay under the
  source node.
...
- Semantic AST construction now materializes a separate arena-owned graph with
  program, statement, query, source, row, target, descriptor, clause,
  table-reference, data-type, data-type-element, data-type-attribute, and
  expression nodes. Descriptor nodes preserve typed parser-view list items such
  as projections, values, assignments, DDL columns/keys/options, table locks,
  table-maintenance targets, replication options, stored objects, flush
  targets/plugins, LOAD items/options/assignments, accounts, privilege items,
  roles, and EXECUTE variables without retaining the parser CST.
...
- typed `SELECT` query-block descriptors with block spans, projection ranges,
  ...
  `INTO`, and locking clause spans, decoded named table-reference handles, plus
  block-local `WHERE` and `HAVING` expression handles
- typed SELECT table-reference descriptors for named table factors with full
  factor span, schema/name/alias spans, and length-aware decoded schema, name,
  and alias values. Derived tables, subqueries, and richer join structure remain
  future table-expression work; the query block still preserves the full `FROM`
  clause span.
...
expression kind/literal/operator, spans, copied values, copied table-reference
schema/alias values, children, node count, statement count, and allocated
bytes. `SELECT` statements now own one semantic query node; the query node owns
query-block nodes. Each query-block node owns its projection descriptors plus
`FROM`, `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY`, `LIMIT`, `INTO`, and locking
clause nodes for that block.
...
and SET payload descriptors live directly under the source node. `INSERT` and
`REPLACE` statements also own one semantic table node for the decoded target
table/schema; explicit target column descriptors live under that table node.
...
key-part, and key-option payload descriptors. `CREATE INDEX` statements own a
semantic indexed-table node; that table node owns one key descriptor for the
index, and the key descriptor owns the key-part and option descriptors.
Remaining DML descriptor placement stays statement-scoped until those statement
...
nodes now also own a data-type child with the parser's type family, exact kind,
SQLite-oriented storage class, flags, numeric parameters, type span, raw
type-name value, decoded enum/set element children, and type-attribute children
for `UNSIGNED`, `ZEROFILL`, `BINARY`, charset, and collation.
...
semantic AST object: 78K on disk, 38,159 bytes text/data/other
...
- Extend semantic DDL object nodes into `DROP`, `RENAME`, and `TRUNCATE`
  statement families.
