...
| `CHANGE REPLICATION SOURCE TO` | ❌ | low | Source connection/channel options and diagnostics. | Parser records explicit `FOR CHANNEL` names and default-channel operations, including legacy `CHANGE MASTER TO` routing for compatibility; source option semantics are not implemented. |
