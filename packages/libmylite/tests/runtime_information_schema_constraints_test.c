...
static int test_information_schema_constraints_queries(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "constraints") != 0) {
        return 1;
    }
    remove_related_files(path);

    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open constraints db");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen constraints db");
