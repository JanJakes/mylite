...
| --- | --- | --- |
| `DESCRIBE` / `DESC` | 🟡 | Limited table-column introspection alias for the supported `SHOW COLUMNS` subset, including supported `mysql.component`, `mysql.func`, `mysql.plugin`, `mysql.servers`, `mysql.gtid_executed`, `mysql.general_log`, `mysql.slow_log`, `mysql.innodb_table_stats`, and `mysql.innodb_index_stats` system-table metadata; no column filters, wildcard patterns, execution-plan `EXPLAIN`, formats, or statement analysis |
