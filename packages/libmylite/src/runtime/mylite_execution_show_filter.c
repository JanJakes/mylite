...
int mylite_execution_make_show_like_filter(
    ...
    if (out_filter == NULL) {
        mylite_execution_diagnostics_set_runtime_error(database, "invalid SHOW LIKE filter");
        return MYLITE_ERROR;
    }
...
static int decode_show_like_pattern(
    ...
    if (out_pattern == NULL) {
        mylite_execution_diagnostics_set_runtime_error(database, "invalid SHOW LIKE pattern");
        return MYLITE_ERROR;
    }
    *out_pattern = NULL;
    if (pattern_node != NULL && pattern_node->kind == MYLITE_SQL_AST_IDENTIFIER) {
        char identifier[MYLITE_CATALOG_IDENTIFIER_CAPACITY];

        rc = mylite_execution_copy_identifier_text(
            pattern_node,
            identifier,
            sizeof(identifier),
            database
        ...
        return mylite_execution_duplicate_text(database, identifier, out_pattern);
    }
    if (pattern_node == NULL || pattern_node->kind != MYLITE_SQL_AST_LITERAL ||
        ...
        mylite_execution_diagnostics_set_unsupported_error(
            database,
            "SHOW LIKE supports only string literal patterns"
        );
        return MYLITE_ERROR;
    }
...
static int append_decoded_string_escape(
    ...
    case '0':
        mylite_execution_diagnostics_set_unsupported_error(
            database,
            "SHOW LIKE does not support NUL bytes in patterns"
        );
        return MYLITE_ERROR;
    case 'n':
...
static bool show_like_pattern_item_matches(
    ...
    if (request.escape_character != '\0' && pattern_byte == request.escape_character &&
