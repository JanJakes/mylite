# Baseline COALESCE Function Tasks

- [x] Read current compatibility, scalar select, literal projection, `IF()`,
  `IFNULL()`, parser, runtime, result, storage, and test context.
- [x] Research official MySQL 8.4 `COALESCE()` / comparison-function
  documentation.
- [x] Probe MySQL 8.4.9 runtime behavior for argument count, non-`NULL` first
  values, fallback values, all-`NULL` values, booleans, integer values, labels,
  nested calls, aliases, identifiers, `FROM DUAL`, warnings, row count, syntax
  errors, and broader deferred forms.
...
- [x] Add lexer/parser/AST support and parser tests for `COALESCE()`.
- [x] Extend the no-source/`FROM DUAL` scalar projection runtime to evaluate
  supported `COALESCE()` expressions without SQLite SQL.
