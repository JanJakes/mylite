...
static int test_information_schema_files_queries(void) {
    ...
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open files db");
...
static int expect_columns_metadata(mylite_db *database) {
    ...
    failures += mylite_test_expect_int(rc, MYLITE_OK, "files columns metadata");
    if (rc != MYLITE_OK) {
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 0, "files columns metadata");
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "files columns metadata");
