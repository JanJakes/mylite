...
| `ADD PRIMARY KEY` | 🟡 | top | Primary key addition and data validation. | Implemented for supported MyLite base tables with existing-row duplicate/`NULL` validation, implicit `NOT NULL` column metadata rewrite, statistics metadata, write-path duplicate enforcement, mixed column/key atomicity, and rollback. `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` / `KEY_COLUMN_USAGE`, generated-column dependencies, foreign-key dependencies, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `DROP PRIMARY KEY` | 🟡 | top | Primary key removal and auto-increment restrictions. | Implemented for metadata-backed primary-key removal while preserving `NOT NULL` column state and rejecting `AUTO_INCREMENT` dependency. Future foreign-key dependency checks, constraint metadata tables, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
| `ADD UNIQUE` | 🟡 | top | Unique index addition and duplicate validation. | Implemented for supported MyLite base tables using standalone index semantics: duplicate validation, nullable unique-key behavior, generated/named index behavior, visibility, prefix parts, statistics metadata, DML conflict integration, and mixed column/key default-backfill validation. Functional/multi-valued key parts, full constraint metadata tables, optimizer use, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
...
| `RENAME INDEX` / `RENAME KEY` | 🟡 | high | Index rename semantics. | Implemented as metadata-only rename of non-primary indexes while preserving key parts, visibility, uniqueness, conflict order, and statistics rows. Exact primary-index rename diagnostics, constraint metadata tables, and implicit commits remain deferred. See [ALTER TABLE key and constraint operations spec](docs/specs/alter-table-key-constraint-operations/specs.md). |
