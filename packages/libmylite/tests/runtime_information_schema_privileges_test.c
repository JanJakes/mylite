...
static int test_information_schema_privileges_queries(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open privilege db");
    if (database == NULL) {
        remove_related_files(path);
        return failures + 1;
    }
...
static int test_information_schema_privileges_reopen_preamble_and_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first privilege db");
    failures +=
        expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second privilege db");
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first privilege db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.USER_PRIVILEGES",
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_information_schema_privileges_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
