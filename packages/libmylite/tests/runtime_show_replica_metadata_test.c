...
static const char *const show_replica_status_columns[show_replica_status_column_count] = {
    "Replica_IO_State",
    "Source_Host",
    "Source_User",
    "Source_Port",
...
static const char *const show_replicas_columns[show_replicas_column_count] = {
    "Server_Id",
    "Host",
    "Port",
    "Source_Id",
...
static const struct expected_show_result show_replica_status = {
    ...
    .column_count = show_replica_status_column_count,
};

static const struct expected_show_result show_replicas = {
    .sql = "SHOW REPLICAS",
...
static int test_show_replica_metadata_file_reopen_and_preamble(void);
static int test_independent_show_replica_metadata_handles(void);
static int test_show_replica_metadata_unsupported_diagnostics(void);
static int expect_show_replica_metadata(mylite_db *database, struct expected_show_result expected);
...
int main(void) {
    ...
    failures += test_show_replica_metadata_results();
    failures += test_show_replica_metadata_file_reopen_and_preamble();
    failures += test_independent_show_replica_metadata_handles();
    failures += test_show_replica_metadata_unsupported_diagnostics();
...
static int test_show_replica_metadata_file_reopen_and_preamble(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open replica file");
    if (database == NULL) {
        remove_related_files(path);
        return failures;
    }
    ...
    if (session == NULL) {
        fprintf(stderr, "replica file: expected session state\n");
        mylite_close(database);
        remove_related_files(path);
        return failures + 1;
    }
    ...
    failures += expect_int64(
        ...
        "catalog generation after replica metadata"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation after replica metadata"
    );
    ...
    failures += expect_bytes(
        ...
        "preamble after replica metadata"
    );
    mylite_close(database);
    database = NULL;

    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "reopen replica file");
    if (database == NULL) {
        remove_related_files(path);
        return failures;
    }
...
static int test_independent_show_replica_metadata_handles(void) {
    ...
    failures += expect_int(mylite_open_memory(&first), MYLITE_OK, "open first replica handle");
...
static int test_show_replica_metadata_unsupported_diagnostics(void) {
    static const struct expected_sql_error errors[] = {
        {
            .sql = "SHOW REPLICA STATUS FOR CHANNEL 'default'",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICA STATUS LIKE '%'",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICA STATUS WHERE Channel_Name = ''",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICA STATUS LIMIT 1",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW FULL REPLICA STATUS",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICAS LIKE '%'",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICAS WHERE Host IS NOT NULL",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW REPLICAS LIMIT 1",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
        {
            .sql = "SHOW FULL REPLICAS",
            .code = mysql_error_parse,
            .sqlstate = "42000",
            .message_part = "You have an error in your SQL syntax",
        },
    ...
    failures += expect_int(mylite_open_memory(&database), MYLITE_OK, "open replica diagnostics");
...
static int expect_show_replica_metadata(mylite_db *database, struct expected_show_result expected) {
    ...
    failures += expect_size(
        ...
        "replica metadata column count"
    );
    failures += expect_size(mylite_result_row_count(result), 0U, "replica metadata row count");
    failures += expect_int64(mylite_result_affected_rows(result), 0, "replica affected rows");
    failures += expect_size(mylite_result_warning_count(result), 0U, "replica warning count");
...
static int expect_row_count_status(mylite_db *database, const char *context) {
    ...
    failures += expect_size(
        ...
        "replica status column count"
    );
    failures += expect_size(mylite_result_row_count(result), 1U, "replica status row count");
    failures +=
        expect_text_or_null(mylite_result_value_text(result, 0U, 0U), "0", "replica warning count");
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    ...
    written = snprintf(
        path,
        path_size,
        "%s/mylite_runtime_show_replica_metadata_%d_%s.mylite",
        directory,
        current_process_id(),
        name
    );
