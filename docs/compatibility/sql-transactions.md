...
| --- | --- | --- |
| `START TRANSACTION` | 🟡 | Limited explicit user transaction start for current supported transactional DML; starting while active commits the previous user transaction first; pending `SET TRANSACTION` characteristics are applied; no `WITH CONSISTENT SNAPSHOT`, statement-level `READ WRITE` / `READ ONLY` modifiers, locks, protocol flags, or XA |
...
| `SET TRANSACTION` | 🟡 | Limited `SET [SESSION] TRANSACTION` isolation/access characteristics for the current connection; `READ ONLY` rejects persistent-table DML while allowing temporary-table DML; isolation is tracked without adding new concurrency semantics; no `GLOBAL`, system-variable readback, `START TRANSACTION` modifiers, privileges, lock semantics, or protocol flags |
