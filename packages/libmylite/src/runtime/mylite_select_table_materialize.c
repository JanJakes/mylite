...
static int materialize_ordered_table_select_result(
    mylite_stmt *stmt,
    const struct mylite_select_eval_callbacks *callbacks
);

static int materialize_unordered_table_select_result(
    mylite_stmt *stmt,
    const struct mylite_select_eval_callbacks *callbacks
);

static int append_unordered_table_select_matched_row(
    mylite_stmt *stmt,
...
static int append_unordered_table_select_limited_row(
    mylite_stmt *stmt,
    struct mylite_table_select_row *row,
    struct mylite_unordered_table_select_append_state *state
);
...
static int materialize_unordered_table_select_result(
    ...
    while ((rc = sqlite3_step(stmt->sqlite_stmt)) == SQLITE_ROW) {
        ...
        status = append_unordered_table_select_matched_row(
            stmt,
            &row,
            &append_state,
            distinct,
