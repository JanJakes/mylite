...
| `SET` | 🟡 | top | Variable assignment, user variables, system variables, persisted variables, names, charset, and transaction forms. | `SET NAMES`, `SET CHARACTER SET`, and session/local `SET sql_mode` are implemented for the currently supported connection-state surfaces, including `DEFAULT` and the focused `REPLACE(@@SESSION.sql_mode, 'ONLY_FULL_GROUP_BY', '')` removal idiom. General variable assignment, user variables, persisted variables, mutable global variables, and transaction forms are deferred. See [character set/collation foundation spec](docs/specs/character-set-collation-foundation/specs.md) and [SQL mode and ONLY_FULL_GROUP_BY spec](docs/specs/sql-mode-only-full-group-by/specs.md). |
...
| `GROUP_CONCAT()` | 🟡 | top | Return a concatenated string | Implemented for current aggregate execution surfaces, including table-backed implicit groups, explicit `GROUP BY`, joined row sources, no-table aggregate calls, multiple argument expressions, `NULL` tuple skipping, aggregate-local `DISTINCT`, aggregate-local `ORDER BY` by expression or argument ordinal, literal `SEPARATOR`, alias use in `HAVING`/statement `ORDER BY`, BLOB-like result metadata at the default 1024-byte limit, and warning 1260 on truncation. Mutable `group_concat_max_len`, exact collation/charset comparison fidelity, spill-to-storage for very large groups, window execution, and subqueries as row sources remain deferred. See [GROUP_CONCAT function spec](docs/specs/group-concat-function/specs.md) and [aggregate functions and grouping spec](docs/specs/aggregate-grouping/specs.md). |
