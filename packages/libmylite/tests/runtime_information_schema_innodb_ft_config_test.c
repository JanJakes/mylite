...
static int test_information_schema_innodb_ft_config_queries(void);
static int test_information_schema_innodb_ft_config_reopen_preamble_and_handles(void);
...
int main(void) {
    ...
    failures += test_information_schema_innodb_ft_config_queries();
    failures += test_information_schema_innodb_ft_config_reopen_preamble_and_handles();

    return failures == 0 ? 0 : 1;
}

static int test_information_schema_innodb_ft_config_queries(void) {
    static const char *const config_columns[] = {
        "KEY",
        "VALUE",
    };
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open innodb ft config db");
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT * FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG",
            .column_names = config_columns,
            .column_count = sizeof(config_columns) / sizeof(config_columns[0]),
            .values = empty_values,
            .row_count = 0U,
            .context = "empty innodb ft config wildcard",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG",
            .column_names = count_column,
            .column_count = 1U,
            .values = count_zero,
            .row_count = 1U,
            .context = "innodb ft config count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "case-insensitive innodb ft config table",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT `KEY`, VALUE FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG "
                   "WHERE VALUE = ''",
            .column_names = config_columns,
            .column_count = sizeof(config_columns) / sizeof(config_columns[0]),
            .values = empty_values,
            .row_count = 0U,
            .context = "innodb ft config quoted key projection",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT c.VALUE FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG AS c "
                   "WHERE c.VALUE = ''",
            .column_names = value_column,
            .column_count = 1U,
            .values = empty_values,
            .row_count = 0U,
            .context = "innodb ft config alias predicate",
        }
    );
    failures += expect_statement_ok(
        ...
        (struct expected_statement){
            ...
            .context = "use information_schema for innodb ft config",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "unqualified innodb ft config count",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            ...
            .row_count = 1U,
            .context = "innodb ft config system table row",
        }
    );
    failures += expect_query(
        database,
        (struct expected_query){
            .sql = "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_DEFAULT, "
                   ...
                   "AND TABLE_NAME = 'INNODB_FT_CONFIG' ORDER BY ORDINAL_POSITION",
            ...
            .context = "innodb ft config columns metadata",
        }
    );
    failures += expect_row_count_status(database, "innodb ft config row count status");
...
static int test_information_schema_innodb_ft_config_reopen_preamble_and_handles(void) {
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "open first ft config db");
    failures +=
        expect_int(mylite_open(second_path, &second), MYLITE_OK, "open second ft config db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG",
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "first handle innodb ft config count",
        }
    );
    failures += expect_query(
        second,
        (struct expected_query){
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "second handle innodb ft config count",
        }
    );
    ...
    failures += expect_bytes(
        ...
        "preamble after innodb ft config metadata query"
    );
    ...
    failures += expect_int(mylite_open(first_path, &first), MYLITE_OK, "reopen first ft config db");
    failures += expect_query(
        first,
        (struct expected_query){
            .sql = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.INNODB_FT_CONFIG",
            ...
            .values = count_zero,
            .row_count = 1U,
            .context = "reopened innodb ft config count",
        }
    );
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "/tmp/mylite_information_schema_innodb_ft_config_%s_%d.mylite",
        name,
        current_process_id()
    );
