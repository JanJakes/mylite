...
static int test_insert_set_success_persistence_rename_and_drop(void) {
    ...
    failures +=
        mylite_test_expect_int64(mylite_result_affected_rows(result), 1, "update inserted row");
    mylite_result_free(result);
    result = NULL;
    failures += expect_query_values(
        database,
    ...
    failures += mylite_test_expect_int(physical_rows, 3, "physical rows after insert set");
...
static int expect_insert_ok(mylite_db *database, const char *sql) {
    ...
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 0U, "insert set column count");
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "insert set row count");
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        1,
        "insert set affected rows"
    );
    failures += mylite_test_expect_size(
        mylite_result_warning_count(result),
        0U,
        "insert set warning count"
    );
    mylite_result_free(result);
