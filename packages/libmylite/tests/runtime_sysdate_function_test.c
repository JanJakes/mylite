...
static int test_sysdate_scalar_and_do(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open scalar memory");
...
static int test_sysdate_dml_persistence_and_file_safety(void) {
    ...
    failures += mylite_test_expect_int(
        mylite_open_memory(&independent),
        MYLITE_OK,
        "open independent memory"
    );
    ...
    mylite_close(database);
    database = NULL;
    failures += mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen dml file");
...
static int expect_row_scalar_sysdate(mylite_db *database) {
    ...
    after = time(NULL);
    failures +=
        mylite_test_expect_size(mylite_result_column_count(result), 2U, "row-scalar column count");
    failures += mylite_test_expect_size(
        mylite_result_row_count(result),
        sysdate_dml_row_count,
        "row-scalar row count"
    );
    ...
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        0,
        "row-scalar affected rows"
    );
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "row-scalar warnings");
    mylite_result_free(result);
...
static int expect_sysdate_table_rows(
    ...
    failures += mylite_test_expect_size(mylite_result_column_count(result), 3U, query.context);
    failures += mylite_test_expect_size(
        mylite_result_row_count(result),
        sysdate_dml_row_count,
        query.context
