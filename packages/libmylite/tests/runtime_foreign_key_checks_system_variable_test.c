...
static int test_foreign_key_checks_values_and_persistence(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open foreign key file");
    ...
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "reopen foreign key file");
    failures += expect_query_result(
        database,
        "SELECT @@foreign_key_checks, @@global.foreign_key_checks",
        (struct expected_result){
...
static int test_mutable_foreign_key_checks_set_values(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open mutable FK memory");
...
static int test_disabled_foreign_key_checks_dml(void) {
    ...
    failures += mylite_test_make_path(path, sizeof(path), "disabled_checks");
    remove_related_files(path);
    failures +=
        mylite_test_expect_int(mylite_open(path, &database), MYLITE_OK, "open disabled FK file");
    failures += execute_statement_ok(database, "CREATE DATABASE app");
    failures += execute_statement_ok(database, "USE app");
...
static int test_foreign_key_checks_qualifiers_and_errors(void) {
    ...
    failures +=
        mylite_test_expect_int(mylite_open_memory(&database), MYLITE_OK, "open foreign key memory");
    failures += execute_ok(
        database,
...
static int test_independent_foreign_key_checks_handles(void) {
    ...
    failures += mylite_test_expect_int(
        ...
        MYLITE_OK,
        "open first foreign key handle"
    );
    failures += mylite_test_expect_int(
        mylite_open_memory(&second),
        MYLITE_OK,
        "open second foreign key handle"
    );
