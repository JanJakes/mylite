...
static int test_drop_added_and_composite_primary_keys(void) {
    ...
    if (mylite_test_make_path(path, sizeof(path), "added_composite") != 0) {
        return 1;
    }
    remove_related_files(path);
...
static int expect_drop_primary_key_ok(mylite_db *database, const char *sql, int64_t affected_rows) {
    ...
    failures += mylite_test_expect_size(
        ...
        0U,
        "DROP PRIMARY column count"
    );
    failures +=
        mylite_test_expect_size(mylite_result_row_count(result), 0U, "DROP PRIMARY row count");
    failures += mylite_test_expect_int64(
        mylite_result_affected_rows(result),
        affected_rows,
        "DROP PRIMARY affected"
    );
    failures +=
        mylite_test_expect_size(mylite_result_warning_count(result), 0U, "DROP PRIMARY warnings");
    mylite_result_free(result);
