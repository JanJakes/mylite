...
enum {
    ...
    sql_buffer_result_value_column_count = 6,
    sql_buffer_result_label_column_count = 5,
    sql_buffer_result_diagnostics_column_count = 4,
    sql_buffer_result_selected_column_count = 2,
    sql_buffer_result_independent_column_count = 3,
...
static int test_sql_buffer_result_values_and_persistence(void);
static int test_sql_buffer_result_qualifiers_and_errors(void);
static int test_independent_sql_buffer_result_handles(void);
...
int main(void) {
    ...
    failures += test_sql_buffer_result_values_and_persistence();
    failures += test_sql_buffer_result_qualifiers_and_errors();
    failures += test_independent_sql_buffer_result_handles();
...
static int test_sql_buffer_result_values_and_persistence(void) {
    ...
    failures += expect_int(mylite_open(path, &database), MYLITE_OK, "open sql buffer result file");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "sql buffer result values",
        }
    );
    ...
    failures += execute_ok(
        database,
        "SELECT @@SQL_BUFFER_RESULT, @@Global.Sql_Buffer_Result, "
        "@@session.`sql_buffer_result`, @@`sql_buffer_result`, (@@sql_buffer_result)",
        &result
    );
    failures += expect_result(
        result,
        (struct expected_result){
            ...
            .values = label_values,
            .count = sql_buffer_result_label_column_count,
            .context = "sql buffer result labels",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_ok(
        database,
        "SELECT @@sql_buffer_result, @@foreign_key_checks, @@unique_checks, "
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .context = "mixed sql buffer result scalar values",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_buffer_result_diagnostics_column_count,
            .context = "sql buffer result warning diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_warnings(database, "0", "sql buffer result clears warnings");
    ...
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .values = error_values,
            .count = sql_buffer_result_diagnostics_column_count,
            .context = "sql buffer result error diagnostics",
        }
    );
    mylite_result_free(result);
    result = NULL;
    failures += expect_show_count_errors(database, "0", "sql buffer result clears errors");
    failures +=
        expect_show_count_warnings(database, "0", "sql buffer result clears error warnings");

    session = mylite_connection_session_state(database);
    failures += expect_int64(
        (int64_t)session->catalog_generation,
        (int64_t)catalog_generation,
        "catalog generation unchanged by sql buffer result reads"
    );
    failures += expect_int64(
        (int64_t)session->sqlite_schema_generation,
        (int64_t)sqlite_schema_generation,
        "sqlite schema generation unchanged by sql buffer result reads"
    );
    failures += expect_int(
        read_file_at(path, 0L, actual_preamble, sizeof(actual_preamble)),
        0,
        "read sql buffer result preamble"
    );
    ...
    failures += expect_query_result(
        database,
        "SELECT @@sql_buffer_result, DATABASE()",
        (struct expected_result){
            .columns = selected_columns,
            .values = selected_values,
            .count = sql_buffer_result_selected_column_count,
            .context = "sql buffer result with selected database",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .context = "sql buffer result does not alter descriptor select",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "explicit descriptor select limit still applies with sql buffer result",
        }
    );
    ...
    failures +=
        expect_int(mylite_open(path, &database), MYLITE_OK, "reopen sql buffer result file");
    failures += expect_query_result(
        database,
        "SELECT @@sql_buffer_result, @@global.sql_buffer_result",
        (struct expected_result){
            ...
            .values = value_values,
            .count = 2U,
            .context = "reopened sql buffer result values",
        }
    );
    failures += expect_query_result(
        database,
        ...
        (struct expected_result){
            ...
            .values = table_values,
            .count = sizeof(table_columns) / sizeof(table_columns[0]),
            .context = "reopened sql buffer result table rows",
        }
    );
...
static int test_sql_buffer_result_qualifiers_and_errors(void) {
    ...
    failures +=
        expect_int(mylite_open_memory(&database), MYLITE_OK, "open sql buffer result memory");
    failures += execute_ok(
        database,
        "SELECT @@SQL_BUFFER_RESULT, @@SESSION.SQL_BUFFER_RESULT, "
        "@@Local.Sql_Buffer_Result, @@global.`sql_buffer_result`, "
        "(@@sql_buffer_result)",
        &result
    );
    failures += expect_result(
        ...
        (struct expected_result){
            ...
            .count = sql_buffer_result_label_column_count,
            .context = "sql buffer result qualifiers",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures += execute_error(
        ...
        (struct expected_sql_error){
            ...
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_buffer_result_variable'",
        }
    );
    failures += execute_error(
        database,
    ...
    failures += execute_error(
        database,
        "SELECT @@session.no_such_sql_buffer_result_variable",
        (struct expected_sql_error){
            .code = mysql_error_unknown_system_variable,
            .sqlstate = "HY000",
            .message_part = "Unknown system variable 'no_such_sql_buffer_result_variable'",
    ...
    failures += execute_statement_ok(database, "SELECT @@sql_buffer_result + 1");
...
static int test_independent_sql_buffer_result_handles(void) {
    ...
    failures +=
        execute_ok(first, "SELECT @@sql_buffer_result, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            .values = first_values,
            .count = sql_buffer_result_independent_column_count,
            .context = "first handle sql buffer result variables",
        }
    );
    mylite_result_free(result);
    result = NULL;
    ...
    failures +=
        execute_ok(second, "SELECT @@sql_buffer_result, @@warning_count, @@error_count", &result);
    failures += expect_result(
        result,
        (struct expected_result){
            .columns = columns,
            ...
            .count = sql_buffer_result_independent_column_count,
            .context = "second handle sql buffer result variables",
        }
    );
    mylite_result_free(result);
...
static int make_test_path(char *path, size_t path_size, const char *name) {
    int written = snprintf(
        path,
        path_size,
        "%s/mylite_sql_buffer_result_system_variable_%d_%s.mylite",
        P_tmpdir,
        current_process_id(),
        name
    );
